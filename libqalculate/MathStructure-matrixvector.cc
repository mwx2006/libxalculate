    ...
    for (size_t i = 0; i < ranked.size(); i++) {
      ...
      if (COMPARISON_NOT_FULLY_KNOWN(cmp)) {
        CALCULATOR->error(true,
                          _("Unsolvable comparison at element %s when trying "
                            "to rank vector."),
                          i2s(index).c_str(), NULL);
        return false;
      }
      if ((ascending && cmp == COMPARISON_RESULT_GREATER) ||
          cmp == COMPARISON_RESULT_EQUAL ||
          (!ascending && cmp == COMPARISON_RESULT_LESS)) {
...
bool MathStructure::sortVector(bool ascending) {
  ...
  for (size_t index = 0; index < SIZE; index++) {
    ...
    for (size_t i = 0; i < ranked_mstructs.size(); i++) {
      ...
      if ((ascending && COMPARISON_IS_EQUAL_OR_GREATER(cmp)) ||
          (!ascending && COMPARISON_IS_EQUAL_OR_LESS(cmp))) {
...
MathStructure &MathStructure::getRange(int start, int end,
  ...
  if (end < (int)1 || end > (long int)SIZE)
    end = SIZE;
  else if (end < start)
    end = start;
  mstruct.clearVector();
...
void MathStructure::resizeVector(size_t i, const MathStructure &mfill) {
  if (i > SIZE) {
    for (size_t index = SIZE; index < i; index++) {
      if (index % 10002 == 10001 && CALCULATOR->aborted())
        return;
      APPEND(mfill);
    }
  } else if (i < SIZE) {
...
size_t MathStructure::rows() const {
  if (m_type == STRUCT_VECTOR && SIZE == 0)
    return 0;
  if (m_type != STRUCT_VECTOR || !isMatrix())
    return 1;
  return SIZE;
}
...
const MathStructure *MathStructure::getElement(size_t row,
                                               size_t column) const {
  if (row == 0 || column == 0)
    return NULL;
  if (m_type != STRUCT_VECTOR) {
  ...
  if (row > SIZE || column > CHILD(row - 1).size())
    return NULL;
...
MathStructure &MathStructure::getArea(size_t r1, size_t c1, size_t r2,
                                      size_t c2, MathStructure &mstruct) const {
  size_t r = rows();
  size_t c = columns();
  if (r1 < 1)
    r1 = 1;
  ...
  if (c2 < 1 || c2 > c)
    ...
    c2 = c1;
  mstruct.clearMatrix();
  mstruct.resizeMatrix(r2 - r1 + 1, c2 - c1 + 1, m_undefined);
...
MathStructure &MathStructure::rowToVector(size_t r,
                                          MathStructure &mstruct) const {
  if (r > rows()) {
    mstruct = m_undefined;
    return mstruct;
...
MathStructure &MathStructure::columnToVector(size_t c,
                                             MathStructure &mstruct) const {
  if (c > columns()) {
    mstruct = m_undefined;
    return mstruct;
...
void MathStructure::setElement(const MathStructure &mstruct, size_t row,
                               size_t column) {
...
void MathStructure::addRow(const MathStructure &mfill) { addRows(1, mfill); }
void MathStructure::addColumn(const MathStructure &mfill) {
  addColumns(1, mfill);
}
void MathStructure::resizeMatrix(size_t r, size_t c,
                                 const MathStructure &mfill) {
  if (r > SIZE) {
...
bool MathStructure::matrixIsSquare() const { return rows() == columns(); }
...
bool MathStructure::isNumericMatrix() const {
  ...
  for (size_t index_r = 0; index_r < SIZE; index_r++) {
    for (size_t index_c = 0; index_c < CHILD(index_r).size(); index_c++) {
      if (!CHILD(index_r)[index_c].isNumber() ||
          CHILD(index_r)[index_c].isInfinity())
        return false;
    }
  }
  return true;
...
int MathStructure::pivot(size_t ro, size_t co, bool symbolic) {
  ...
  size_t k = ro;
  if (symbolic) {
    while ((k < SIZE) && (CHILD(k)[co].isZero()))
      ++k;
  } else {
    size_t kmax = k + 1;
...
// from GiNaC
void determinant_minor(const MathStructure &mtrx, MathStructure &mdet,
  ...
  typedef std::map<std::vector<size_t>, class MathStructure>::value_type
      Rmap_value;
  Rmap A;
  Rmap B;
        ...
        if (!CHILD(r2)[c0].isZero()) {
          if (isnumeric) {
            ...
            for (size_t c = c0 + 1; c < n; ++c) {
              ...
              CHILD(r2)
              [c][CHILD(r2)[c].size() - 1].calculateMultiply(CHILD(r0)[c], eo);
              CHILD(r2)[c][CHILD(r2)[c].size() - 1].calculateNegate(eo);
              CHILD(r2)[c].calculateAddLast(eo);
            }
          }
        }
        for (size_t c = r0; c <= c0; ++c)
          CHILD(r2)[c].clear();
      }
      if (det) {
        for (size_t c = r0 + 1; c < n; ++c)
          CHILD(r0)[c].clear();
      }
      ++r0;
    }
...
// from GiNaC
template <class It> int permutation_sign(It first, It last) {
  if (first == last)
    return 0;
  --last;
  if (first == last)
...
// from GiNaC
MathStructure &MathStructure::determinant(MathStructure &mstruct,
                                          const EvaluationOptions &eo) const {

  if (!matrixIsSquare()) {
    CALCULATOR->error(
        true, _("The determinant can only be calculated for square matrices."),
        NULL);
    mstruct = m_undefined;
    return mstruct;
  }
  ...
  if (SIZE == 1) {
    ...
    for (std::vector<sizet_pair>::const_iterator i = c_zeros.begin();
         i != c_zeros.end(); ++i) {
    ...
    size_t c = 0;
    for (std::vector<size_t>::const_iterator i = pre_sort.begin();
         i != pre_sort.end(); ++i, ++c) {
      for (size_t r = 0; r < SIZE; ++r)
        result[r][c] = CHILD(r)[(*i)];
    }
    mstruct.clear();
...
MathStructure &MathStructure::permanent(MathStructure &mstruct,
                                        const EvaluationOptions &eo) const {
  if (!matrixIsSquare()) {
    CALCULATOR->error(
        true, _("The permanent can only be calculated for square matrices."),
        NULL);
    mstruct = m_undefined;
    return mstruct;
  }
  ...
  if (SIZE == 1) {
    ...
    for (size_t index_c = 0; index_c < CHILD(0).size(); index_c++) {
      for (size_t index_r2 = 1; index_r2 < SIZE; index_r2++) {
        for (size_t index_c2 = 0; index_c2 < CHILD(index_r2).size();
             index_c2++) {
          if (index_c2 > index_c) {
...
MathStructure &MathStructure::cofactor(size_t r, size_t c,
...
bool calculate_userfunctions(MathStructure &m, const MathStructure &x_mstruct,
                             const EvaluationOptions &eo, bool b_vector) {
  bool b_ret = false;
  ...
  if (m.isFunction()) {
    if (!m.contains(x_mstruct, true) &&
        !m.containsFunctionId(FUNCTION_ID_RAND, true, true, true) &&
        !m.containsFunctionId(FUNCTION_ID_RANDN, true, true, true) &&
        !m.containsFunctionId(FUNCTION_ID_RAND_POISSON, true, true, true)) {
      if (m.calculateFunctions(eo, false)) {
        ...
        calculate_userfunctions(m, x_mstruct, eo, b_vector);
               ...
               m.function()->condition().empty()) {
      bool b = true;
      for (size_t i = 0;
           i < ((UserFunction *)m.function())->countSubfunctions(); i++) {
      ...
      for (size_t i = 0; b && i < m.size(); i++) {
        ...
        if (arg && arg->tests() &&
            (arg->type() != ARGUMENT_TYPE_FREE ||
             !arg->getCustomCondition().empty() || arg->rationalPolynomial() ||
             arg->zeroForbidden() ||
             (arg->handlesVector() && m[i].isVector())) &&
            m[i].contains(x_mstruct, true)) {
          b = false;
          break;
        }
      }
    ...
    } else if (b_vector && ((m.function()->id() == FUNCTION_ID_DIFFERENTIATE &&
                             (m.size() < 3 || m[3].isUndefined())) ||
                            (m.function()->id() == FUNCTION_ID_INTEGRATE &&
                             (m.size() < 3 ||
                              (m[1].isUndefined() && m[2].isUndefined()))))) {
      size_t i_x = 1;
      if (m.function()->id() == FUNCTION_ID_INTEGRATE)
        i_x = 3;
      ...
      if (b && m.calculateFunctions(eo, false, true)) {
        if (i_x == 3)
          m.replace(CALCULATOR->getVariableById(VARIABLE_ID_C), m_zero);
        b_ret = true;
      }
    }
  }
...
bool find_legal_point(const MathStructure &m, const MathStructure &x_mstruct,
                      MathStructure &mx, const MathStructure &min,
                      const MathStructure &max, const EvaluationOptions &eo,
                      bool b_number) {
  if (m.isFunction() && (m.function()->id() == FUNCTION_ID_RE ||
                         m.function()->id() == FUNCTION_ID_IM ||
                         m.function()->id() == FUNCTION_ID_ABS))
    return false;
  if ((b_number &&
       ((m.isPower() && m[1].isNumber() && m[1].number().isReal() &&
         !m[1].number().isInteger() && m[0].contains(x_mstruct, true) > 0) ||
        (m.isFunction() && m.size() > 0 &&
         (m.function()->id() == FUNCTION_ID_ACOS ||
          m.function()->id() == FUNCTION_ID_ASIN ||
          m.function()->id() == FUNCTION_ID_ACOSH ||
          (m.function()->id() == FUNCTION_ID_LAMBERT_W &&
           (m.size() == 1 || m[1].isZero())))))) ||
      (!b_number && m.isFunction() && m.size() > 0 && m.isFunction() &&
       m.function()->id() == FUNCTION_ID_ROOT && m.size() >= 2 &&
       m[1].representsEven())) {
    MathFunction *f = CALCULATOR->getFunctionById(FUNCTION_ID_SECANT_METHOD);
    if (f) {
      MathStructure msolve(f, NULL);
      msolve.addChild(m[0]);
      if (m.isFunction() && (m.function()->id() == FUNCTION_ID_ACOS ||
                             m.function()->id() == FUNCTION_ID_ASIN ||
                             m.function()->id() == FUNCTION_ID_ACOSH)) {
        if (m.function()->id() != FUNCTION_ID_ACOSH)
          msolve[0].transformById(FUNCTION_ID_ABS);
        msolve[0] -= 1;
      ...
      if (msolve.isNumber() && msolve.number() > min.number() &&
          msolve.number() < max.number()) {
        mx = msolve;
        return true;
      }
    }
  ...
  for (size_t i = 0; i < m.size(); i++) {
    if (find_legal_point(m[i], x_mstruct, mx, min, max, eo, b_number))
      return true;
  }
  return false;
}
bool find_zero_point(const MathStructure &m, const MathStructure &x_mstruct,
                     MathStructure &mx, const MathStructure &min,
                     const MathStructure &max, const EvaluationOptions &eo) {
  MathFunction *f = CALCULATOR->getFunctionById(FUNCTION_ID_SECANT_METHOD);
  if (f) {
...
void generate_plotvector(const MathStructure &m, MathStructure x_mstruct,
  ...
  if (min != max) {
    ...
    if (!step.isZero())
      mtest.calculateDivide(step, eo);
    mtest.eval(eo);
    if (step.isZero() || !mtest.isNumber() || mtest.number().isNegative()) {
      CALCULATOR->error(true,
                        _("The selected min, max and step size do not result "
                          "in a positive, finite number of data points"),
                        NULL);
      return;
  ...
  while (COMPARISON_IS_EQUAL_OR_LESS(cr)) {
    ...
    y_value.eval(eo);
    if (!eo2.allow_complex && y_value.isNumber() &&
        y_value.number().hasImaginaryPart()) {
      if (testComplexZero(&y_value.number(),
                          y_value.number().internalImaginary()))
        y_value.number().clearImaginary();
      else
...
void generate_plotvector(const MathStructure &m, MathStructure x_mstruct,
                         ...
                         MathStructure &y_vector, const EvaluationOptions &eo2,
                         bool adaptive) {
  EvaluationOptions eo = eo2;
  eo.allow_complex = true;
  if (steps < 1) {
  ...
  if (adaptive && (!min.isNumber() || !max.isNumber()))
    adaptive = false;
  if (adaptive)
  ...
  if (!step.isNumber() || step.number().isNegative()) {
    CALCULATOR->error(true,
                      _("The selected min and max do not result in a positive, "
  ...
  MathStructure ydiff;
  if (adaptive)
    ydiff.resizeVector(steps, m_zero);
  Number ydiff_total;
  Number ydiff_real;
  y_vector.resizeVector(steps, m_zero);
  x_vector.resizeVector(steps, m_zero);
  ...
  for (int i = 0; i < steps; i++) {
    ...
    if (y_value.isNumber() && !y_value.number().includesInfinity()) {
      if (y_value.number().hasImaginaryPart() &&
          testComplexZero(&y_value.number(),
                          y_value.number().internalImaginary()))
        y_value.number().clearImaginary();
      if (y_value.number().hasImaginaryPart()) {
        if (prev_illegal == 0)
          find_x = 1;
        if (eo2.allow_complex && adaptive &&
            (prev_illegal == 0 || prev_illegal == 1)) {
          ...
          if (y_value.number().hasRealPart()) {
            ...
            ydiff_real.abs();
            if (ydiff_real > ydiff[i].number())
              ydiff[i].number() = ydiff_real;
          }
        ...
        if (prev_illegal == 0 && adaptive) {
          ...
          if (prev_diff_sgn != 0 &&
              (ydiff[i].number().isZero() ||
               ((prev_diff_sgn > 0) != ydiff[i].number().isPositive()))) {
            if (mdiff.isZero()) {
              mdiff = mthis;
            ...
            if (!mdiff.isUndefined() &&
                find_zero_point(mdiff, x_mstruct, x_value2,
                                i >= 2 && !ydiff[i].number().isZero() &&
                                        y_vector[i - 2].isNumber() &&
                                        y_vector[i - 2].number().isReal()
                                    ? x_vector[i - 2]
                                    : x_vector[i - 1],
                                x_value, eo)) {
              int new_i = i;
              ...
              y_value2.eval(eo);
              if (y_value2.isNumber() && y_value2.number().hasImaginaryPart() &&
                  testComplexZero(&y_value2.number(),
                                  y_value2.number().internalImaginary()))
                y_value2.number().clearImaginary();
        ...
        if (prev_illegal > 0 && y_value.number().isReal())
          find_x = 2;
        else
          prev_illegal = 0;
      }
    ...
    if (find_x > 0 && x_value.isNumber() && step.isNumber()) {
      ...
      if (find_legal_point(meval, x_mstruct, x_value2, x_vector[i - 1], x_value,
                           eo, prev_illegal == 1)) {
        if (x_value2.number().precision() > 0)
          x_value2.number() +=
              step.number() *
              Number(find_x == 1 ? -1 : 1, 1, -x_value2.number().precision());
        if (x_value2.number() > x_vector[i - 1].number() &&
            x_value2.number() < x_vector[i].number()) {
          y_value = mthis;
            ...
            if (y_value.number().isReal()) {
              ...
              if (adaptive) {
                ...
                if (find_x == 1) {
                  ...
                  ydiff[i - 1].number() -= y_vector[i - 2].number();
                  ydiff[i - 1].number().abs();
                  if (eo2.allow_complex && y_vector[i].isNumber() &&
                      !y_vector[i].number().includesInfinity()) {
                    ydiff[i].number() = y_vector[i].number().imaginaryPart();
                    ydiff_real = y_vector[i].number().realPart();
                  ...
                  if (eo2.allow_complex && y_vector[i - 2].isNumber() &&
                      !y_vector[i - 2].number().includesInfinity()) {
                    ydiff[i - 1].number() =
                        y_vector[i - 2].number().imaginaryPart();
                    ydiff[i - 1].number().abs();
                    ydiff_real = y_value.number().realPart();
                    ydiff_real -= y_vector[i - 2].number().realPart();
                    ydiff_real.abs();
                    if (ydiff_real > ydiff[i - 1].number())
                      ydiff[i - 1].number() = ydiff_real;
                  }
                  ydiff[i].number() = y_vector[i].number();
                  ydiff[i].number() -= y_value.number();
                  ydiff[i].number().abs();
  ...
  if (!eo2.allow_complex) {
    for (size_t i = 0; i < y_vector.size(); i++) {
      if (y_vector[i].isNumber() && !y_vector[i].number().isReal())
        y_vector[i].setUndefined();
    }
  }
  if (adaptive) {
...
MathStructure MathStructure::generateVector(MathStructure x_mstruct,
                                            const MathStructure &min,
                                            const MathStructure &max, int steps,
...
MathStructure MathStructure::generateVector(MathStructure x_mstruct,
                                            const MathStructure &min,
                                            const MathStructure &max,
                                            const MathStructure &step_pre,
                                            MathStructure *x_vector,
                                            const EvaluationOptions &eo) const {
  MathStructure x_value(min);
  MathStructure y_value;
  ...
  y_vector.clearVector();
  MathStructure step;
  if (!step_pre.contains(x_mstruct) &&
      !step_pre.containsFunctionId(FUNCTION_ID_RAND, true, true, true) &&
      !step_pre.containsFunctionId(FUNCTION_ID_RANDN, true, true, true) &&
      !step_pre.containsFunctionId(FUNCTION_ID_RAND_POISSON, true, true,
                                   true)) {
    step = step_pre;
    step.eval(eo);
    if (min != max) {
  ...
  cr = max.compare(x_value);
  size_t i = 0;
  while ((!b_neg && COMPARISON_IS_EQUAL_OR_LESS(cr)) ||
         (b_neg && COMPARISON_IS_EQUAL_OR_GREATER(cr))) {
    if (x_vector) {
      if (i >= x_vector->size())
        x_vector->addChild(x_value);
      else
