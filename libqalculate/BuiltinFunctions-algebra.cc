...
int SumFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
  ...
  if (eo2.approximation == APPROXIMATION_TRY_EXACT) {
    ...
    nr.subtract(i_nr);
    if (nr.isGreaterThan(100))
      eo2.approximation = APPROXIMATION_APPROXIMATE;
  }
  MathStructure mbak(m1);
  vector<Variable *> vars;
  if (eo.interval_calculation == INTERVAL_CALCULATION_VARIANCE_FORMULA ||
  ...
  while (i_nr.isLessThanOrEqualTo(vargs[2].number())) {
    if (CALCULATOR->aborted()) {
      if (!started) {
        ...
        mstruct.add(
            MathStructure(this, &vargs[0], &mmin, &vargs[2], &vargs[3], NULL),
            true);
        break;
      }
    }
  ...
  for (size_t i = 0; i < vars.size(); i++) {
    if (vars[i]->isKnown())
      mstruct.replace(vars[i], ((KnownVariable *)vars[i])->get());
    else
      mstruct.replace(vars[i], ((UnknownVariable *)vars[i])->interval());
    vars[i]->destroy();
  }
  return 1;
}
...
int ProductFunction::calculate(MathStructure &mstruct,
...
bool is_comparison_structure(const MathStructure &mstruct,
                             const MathStructure &xvar, bool *bce = NULL,
                             bool do_bce_or = false);
bool is_comparison_structure(const MathStructure &mstruct,
                             const MathStructure &xvar, bool *bce,
                             bool do_bce_or) {
  if (mstruct.isComparison()) {
    if (bce)
      *bce =
          mstruct.comparisonType() == COMPARISON_EQUALS && mstruct[0] == xvar;
    return true;
  }
  if (bce && do_bce_or && mstruct.isLogicalOr()) {
    *bce = true;
    for (size_t i = 0; i < mstruct.size(); i++) {
      bool bcei = false;
      if (!is_comparison_structure(mstruct[i], xvar, &bcei, false))
        return false;
      if (!bcei)
        *bce = false;
    }
  ...
  if (mstruct.isLogicalAnd()) {
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (is_comparison_structure(mstruct[i], xvar))
        return true;
    }
    return true;
...
MathStructure *solve_handle_logical_and(MathStructure &mstruct,
                                        MathStructure **mtruefor,
                                        ComparisonType ct, bool &b_partial,
                                        const MathStructure &x_var) {
  MathStructure *mcondition = NULL;
  for (size_t i2 = 0; i2 < mstruct.size();) {
    if (ct == COMPARISON_EQUALS) {
      if (mstruct[i2].isComparison() && ct == mstruct[i2].comparisonType() &&
          mstruct[i2][0].contains(x_var)) {
...
void simplify_constant(MathStructure &mstruct, const MathStructure &x_var,
                       const MathStructure &y_var, const MathStructure &c_var,
                       bool in_comparison = false, bool in_or = false,
                       bool in_and = false);
void simplify_constant(MathStructure &mstruct, const MathStructure &x_var,
                       const MathStructure &y_var, const MathStructure &c_var,
                       bool in_comparison, bool in_or, bool in_and) {
  if (!in_comparison && mstruct.isComparison()) {
    if (mstruct[0] == c_var) {
      ...
      if (mstruct[1].contains(y_var, true) <= 0)
        simplify_constant(mstruct[1], x_var, y_var, c_var, true);
    } else if (mstruct[0].contains(y_var, true) <= 0 &&
               mstruct.contains(c_var, true) > 0) {
      if (in_or)
        mstruct.clear(true);
      else
        mstruct.set(1, 1, 0);
  ...
  if (in_comparison) {
    if (mstruct.contains(y_var, true) <= 0 &&
        mstruct.contains(x_var, true) <= 0 &&
        mstruct.contains(c_var, true) > 0) {
      mstruct = c_var;
      return;
    }
  ...
  if (in_comparison) {
    ...
    if (!b_cx && n_c >= 1 &&
        (mstruct.isAddition() || mstruct.isMultiplication())) {
      bool b_c = false;
      ...
      if (b_cx)
        simplify_constant(mstruct[i_c], x_var, y_var, c_var, true);
      else
        mstruct[i_c] = c_var;
    }
  } else {
    for (size_t i = 0; i < mstruct.size(); i++) {
      simplify_constant(mstruct[i], x_var, y_var, c_var, false,
                        mstruct.isLogicalOr(), mstruct.isLogicalAnd());
    }
  }
}

int test_equation(MathStructure &mstruct, const EvaluationOptions &eo,
                  const MathStructure &x_var, const MathStructure &y_var,
                  const MathStructure &x_value, const MathStructure &y_value) {
  if (mstruct.isComparison() && mstruct.comparisonType() == COMPARISON_EQUALS &&
      mstruct[0] == y_var) {
...
int solve_equation(MathStructure &mstruct, const MathStructure &m_eqn,
                   const MathStructure &y_var, const EvaluationOptions &eo,
                   bool dsolve = false,
                   const MathStructure &x_var = m_undefined,
                   const MathStructure &c_var = m_undefined,
                   const MathStructure &x_value = m_undefined,
                   const MathStructure &y_value = m_undefined) {
  ...
  while (true) {
    ...
    if (itry == 1) {
      if (ierror == 1) {
        if (!dsolve)
          CALCULATOR->error(
              true,
              _("No equality or inequality to solve. The entered expression to "
                "solve is not correct (e.g. \"x + 5 = 3\" is correct)"),
              NULL);
        return -1;
      } else {
        first_error = ierror;
    ...
    if (itry == 2) {
      if (y_var.isVariable() &&
          y_var.variable()->subtype() == SUBTYPE_UNKNOWN_VARIABLE) {
    ...
    if (itry > 3) {
      if (as != ASSUMPTION_SIGN_UNKNOWN)
        assumptions->setSign(as);
      if (at > ASSUMPTION_TYPE_NUMBER)
        assumptions->setType(at);
      if (assumptions_added)
        ((UnknownVariable *)y_var.variable())->setAssumptions(NULL);
      switch (first_error) {
      case 2: {
        CALCULATOR->error(
            true,
            _("The comparison is true for all %s (with current assumptions)."),
            format_and_print(y_var).c_str(), NULL);
        break;
      }
      case 3: {
        CALCULATOR->error(
            true,
            _("No possible solution was found (with current assumptions)."),
            NULL);
        break;
      }
      case 4: {
        CALCULATOR->error(true, _("Was unable to completely isolate %s."),
                          format_and_print(y_var).c_str(), NULL);
        break;
      }
      case 7: {
        CALCULATOR->error(false, _("The comparison is true for all %s if %s."),
                          format_and_print(y_var).c_str(), strueforall.c_str(),
                          NULL);
        break;
      }
      default: {
        CALCULATOR->error(true, _("Was unable to isolate %s."),
                          format_and_print(y_var).c_str(), NULL);
        break;
      }
      }
    ...
    if (m_eqn.isComparison()) {
      ...
      mstruct = m_eqn;
      b = true;
    } else if (m_eqn.isLogicalAnd() && m_eqn.size() > 0 &&
               m_eqn[0].isComparison()) {
      ...
      mstruct = m_eqn;
      b = true;
    } else if (m_eqn.isLogicalOr() && m_eqn.size() > 0 &&
               m_eqn[0].isComparison()) {
      ct = m_eqn[0].comparisonType();
      mstruct = m_eqn;
      b = true;
    } else if (m_eqn.isLogicalOr() && m_eqn.size() > 0 &&
               m_eqn[0].isLogicalAnd() && m_eqn[0].size() > 0 &&
               m_eqn[0][0].isComparison()) {
      ct = m_eqn[0][0].comparisonType();
      for (size_t i = 0; i < m_eqn[0].size(); i++) {
        if (m_eqn[0][i].isComparison() &&
            m_eqn[0][i].contains(y_var, true) > 0) {
      ...
      mstruct = m_eqn;
      b = true;
    } else if (m_eqn.isVariable() && m_eqn.variable()->isKnown() &&
               (eo.approximation != APPROXIMATION_EXACT ||
                !m_eqn.variable()->isApproximate()) &&
               ((KnownVariable *)m_eqn.variable())->get().isComparison()) {
      ...
      if (mstruct.isComparison()) {
        ...
        b = true;
      } else if (mstruct.isLogicalOr() && mstruct.size() > 0 &&
                 mstruct[0].isComparison()) {
        ct = mstruct[0].comparisonType();
        b = true;
      } else if (mstruct.isLogicalOr() && mstruct.size() > 0 &&
                 mstruct[0].isLogicalAnd() && mstruct[0].size() > 0 &&
                 mstruct[0][0].isComparison()) {
    ...
    if (dsolve) {
      if (x_value.isUndefined() || y_value.isUndefined()) {
        ...
        int test_r =
            test_equation(mstruct, eo2, x_var, y_var, x_value, y_value);
        if (test_r < 0) {
          ierror = 8;
          continue;
        } else if (test_r > 0) {
    ...
    if (mstruct.isComparison()) {
      ...
      if (itry > 1) {
        ...
        if (itry == 2) {
          CALCULATOR->error(
              false,
              _("Was unable to isolate %s with the current assumptions. The "
                "assumed sign was therefore temporarily set as unknown."),
              format_and_print(y_var).c_str(), NULL);
        } else if (itry == 3) {
          assumptions->setType(at);
          CALCULATOR->error(false,
                            _("Was unable to isolate %s with the current "
                              "assumptions. The assumed type and sign was "
      ...
      MathStructure *mcondition =
          solve_handle_logical_and(mstruct, &mtruefor, ct, b_partial, y_var);
      if ((!mstruct.isComparison() && !mstruct.isLogicalAnd()) ||
          (ct == COMPARISON_EQUALS &&
           (!mstruct.isComparison() ||
            mstruct.comparisonType() != COMPARISON_EQUALS ||
            mstruct[0] != y_var)) ||
          !mstruct.contains(y_var)) {
        if (mtruefor)
      ...
      if (mcondition) {
        CALCULATOR->error(false, _("The solution requires that %s."),
                          format_and_print(*mcondition).c_str(), NULL);
        delete mcondition;
      }
      if (mtruefor) {
        CALCULATOR->error(false, _("The comparison is true for all %s if %s."),
                          format_and_print(y_var).c_str(),
                          format_and_print(*mtruefor).c_str(), NULL);
        delete mtruefor;
      }
      if (ct == COMPARISON_EQUALS)
        mstruct.setToChild(2, true);
      ...
      for (size_t i = 0; i < mstruct.size();) {
        ...
        if (mstruct[i].isLogicalAnd()) {
          mcondition = solve_handle_logical_and(mstruct[i], &mtruefor, ct,
                                                b_partial, y_var);
          b_and = true;
        }
        if (!mstruct[i].isZero()) {
          for (size_t i2 = 0; i2 < i; i2++) {
            if (mstruct[i2] == mstruct[i]) {
              ...
              if (mcondition && mconditions[i2]) {
                mconditions[i2]->add_nocopy(mcondition, OPERATION_LOGICAL_OR,
                                            true);
              }
              break;
            }
        ...
        bool b_del = false;
        if ((!mstruct[i].isComparison() && !mstruct[i].isLogicalAnd()) ||
            (ct == COMPARISON_EQUALS &&
             (!mstruct[i].isComparison() ||
              mstruct[i].comparisonType() != COMPARISON_EQUALS)) ||
            !mstruct[i].contains(y_var)) {
          b_del = true;
      ...
      if (ct == COMPARISON_EQUALS) {
        for (size_t i = 0; i < mstruct.size(); i++) {
          if (mstruct[i].isComparison() && mstruct[i].comparisonType() == ct)
      ...
      if (mconditions.size() == 1) {
        if (mconditions[0]) {
          CALCULATOR->error(false, _("The solution requires that %s."),
                            format_and_print(*mconditions[0]).c_str(), NULL);
        ...
        for (size_t i = 0; i < mconditions.size(); i++) {
          if (mconditions[i]) {
            CALCULATOR->error(false, _("Solution %s requires that %s."),
                              i2s(i + 1).c_str(),
                              format_and_print(*mconditions[i]).c_str(), NULL);
...
bool test_functions_comparison(const MathStructure &m,
                               const EvaluationOptions &eo) {
  for (size_t i = 0; i < m.size(); i++) {
    if (test_functions_comparison(m[i], eo))
      return true;
  }
  if (m.isFunction() && m.function()->subtype() == SUBTYPE_USER_FUNCTION) {
    ...
    mtest.eval(eo);
    CALCULATOR->endTemporaryStopMessages();
    if (mtest.containsType(STRUCT_COMPARISON, false, true, true) > 0)
      return true;
  } else if (m.isVariable() && m.variable()->isKnown()) {
    return test_functions_comparison(((KnownVariable *)m.variable())->get(),
                                     eo);
  }
  return false;
}
int SolveFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                             const EvaluationOptions &eo) {
  if (vargs[0].containsType(STRUCT_COMPARISON, false, true, true) <= 0 &&
      !test_functions_comparison(vargs[0], eo)) {
...
int SolveMultipleFunction::calculate(MathStructure &mstruct,
                                     const MathStructure &vargs,
                                     const EvaluationOptions &eo) {
  ...
  mv.clearVector();
  MathStructure mand;
  mand.resizeVector(eorder.size(), mv);
  if (mand.size() < eorder.size())
    return 0;
  ...
  for (size_t i = 0; i < eorder.size(); i++) {
    ...
    for (size_t i2 = 0; i2 < i; i2++) {
      if (mstruct[i2].isVector()) {
        ...
        for (size_t i4 = 1; i4 < mstruct[i2].size(); i4++)
          msolve.addChild(msolve[0]);
    ...
    if (msolve.isComparison()) {
      if (msolve[0] != vargs[1][i]) {
        if (!b) {
          CALCULATOR->error(
              true,
              _("Unable to isolate %s.\n\nYou might need to place the "
                "equations and variables in an appropriate order so that each "
                "equation at least contains the corresponding variable (if "
        ...
        if (msolve.comparisonType() == COMPARISON_EQUALS) {
          ...
          CALCULATOR->error(true, _("Inequalities are not allowed in %s()."),
                            preferredName().name.c_str(), NULL);
          return 0;
        }
      }
      ...
      for (size_t i2 = 0; i2 < msolve.size(); i2++) {
        if (msolve[i2].isLogicalAnd()) {
          ...
          for (size_t i4 = 0; i4 < msolve[i2].size(); i4++) {
            if (!b && msolve[i2][i4].isComparison() &&
                msolve[i2][i4].comparisonType() == COMPARISON_EQUALS &&
                msolve[i2][i4][0] == vargs[1][i]) {
          ...
          msolve[i2].setToChild(i_solve + 1, true);
                   ...
                   msolve[i2].comparisonType() == COMPARISON_EQUALS &&
                   msolve[i2][0] == vargs[1][i]) {
      ...
      for (size_t i2 = 0; i2 < mstruct[i].size(); i2++) {
        ...
        for (size_t i3 = i2 + 1; i3 < mstruct[i].size();) {
          ...
          if (cr == COMPARISON_RESULT_EQUAL ||
              cr == COMPARISON_RESULT_EQUAL_LIMITS) {
            ...
            if (cr == COMPARISON_RESULT_UNKNOWN &&
                (!b_eval[i2] || !b_eval[i3])) {
              if (!b_eval[i2])
                mstruct[i][i2].eval(eo);
              if (!b_eval[i3])
                mstruct[i][i3].eval(eo);
              b_eval[i2] = true;
              b_eval[i3] = true;
              ComparisonResult cr = mstruct[i][i2].compare(mstruct[i][i3]);
              if (cr == COMPARISON_RESULT_EQUAL ||
...
MathStructure *find_deqn(MathStructure &mstruct) {
  if (mstruct.isFunction() &&
      mstruct.function()->id() == FUNCTION_ID_DIFFERENTIATE)
    return &mstruct;
  for (size_t i = 0; i < mstruct.size(); i++) {
...
bool contains_ignore_diff(const MathStructure &m, const MathStructure &mstruct,
                          const MathStructure &mdiff);
bool contains_ignore_diff(const MathStructure &m, const MathStructure &mstruct,
                          const MathStructure &mdiff) {
  if (m.equals(mstruct))
    return true;
  if (m.equals(mdiff))
    return false;
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_ignore_diff(m[i], mstruct, mdiff))
      return true;
  }
  if (m.isVariable() && m.variable()->isKnown()) {
    return contains_ignore_diff(((KnownVariable *)m.variable())->get(), mstruct,
                                mdiff);
  } else if (m.isVariable()) {
    if (mstruct.isNumber() || !m.representsNumber())
      return true;
  } else if (m.isAborted()) {
    return true;
  }
...
void add_C(MathStructure &m_eqn, const MathStructure &m_x,
           const MathStructure &m_y, const MathStructure &x_value,
           const MathStructure &y_value) {
...
bool dsolve(MathStructure &m_eqn, const EvaluationOptions &eo,
            const MathStructure &m_diff, const MathStructure &y_value,
            const MathStructure &x_value) {
      ...
      for (size_t i = 0; i < m_eqn[1].size(); i++) {
        if (m_eqn[1][i].containsRepresentativeOf(m_y, true, true) != 0) {
          ...
          if (m_fy.isOne())
            ...
            m_fy.multiply(m_eqn[1][i], true);
        } else {
          if (m_fx.isOne())
            m_fx = m_eqn[1][i];
          else
            m_fx.multiply(m_eqn[1][i], true);
        }
      }
      if (b) {
        // y'=f(x)*f(y)
        m_fy.inverse();
        if (m_fy.integrate(m_y, eo, true, false) > 0 &&
            m_fx.integrate(m_x, eo, true, false) > 0) {
          m_eqn[0] = m_fy;
          m_eqn[1] = m_fx;
        } else {
          b = false;
        ...
        for (size_t i = 0; i < m_eqn[1].size(); i++) {
          if (m_eqn[1][i] == m_y) {
            if (m_muly.isZero())
              ...
              m_muly.add(m_one, true);
          } else if (m_eqn[1][i].containsRepresentativeOf(m_y, true, true) !=
                     0) {
            if (m_left.isZero() && m_eqn[1][i].isPower() &&
                m_eqn[1][i][0] == m_y &&
                (m_mul_exp.isZero() || m_eqn[1][i][1] == m_exp)) {
              if (m_mul_exp.isZero()) {
                m_exp = m_eqn[1][i][1];
                m_mul_exp = m_one;
              } else {
              ...
              for (size_t i2 = 0; i2 < m_eqn[1][i].size(); i2++) {
                if (!b2 && m_eqn[1][i][i2] == m_y) {
                  i_my = i2;
                  b2 = true;
                } else if (!b2 && m_left.isZero() &&
                           m_eqn[1][i][i2].isPower() &&
                           m_eqn[1][i][i2][0] == m_y &&
                           (m_mul_exp.isZero() ||
                            m_eqn[1][i][i2][1] == m_exp)) {
                  i_my = i2;
                  b2 = true;
                  b2_exp = true;
                } else if (m_eqn[1][i][i2].containsRepresentativeOf(
                               m_y, true, true) != 0) {
                  b2 = false;
                  break;
                }
            ...
            if (m_left.isZero())
              ...
              m_left.add(m_eqn[1][i], true);
          }
        }
        if (b && !m_muly.isZero()) {
          if (!m_mul_exp.isZero()) {
            if (m_exp.isOne() || !m_left.isZero())
              return false;
            // y' = f(x)*y+g(x)*y^c
            b = false;
            ...
            if (m_y1_integ.integrate(m_x, eo, true, false) > 0) {
              ...
              m_y1_exp *= m_y1_integ;
              m_y1_exp.transform(STRUCT_POWER,
            ...
            if (integ_fac.integrate(m_x, eo, true, false) > 0) {
              ...
              if (m_u.integrate(var, eo, false, false) > 0) {
                ...
                if (m_eqn2.isComparison() &&
                    m_eqn2.comparisonType() == COMPARISON_EQUALS &&
                    m_eqn2[0] == var) {
                  integ_fac = m_eqn2[1];
                  ...
                  for (size_t i = 0; i < m_eqn2.size(); i++) {
                    if (!m_eqn2[i].isComparison() ||
                        m_eqn2[i].comparisonType() != COMPARISON_EQUALS ||
                        m_eqn2[i][0] != var) {
                      b = false;
                      break;
                    }
...
void protect_mdiff(MathStructure &m, const MathStructure &mdiff,
                   const EvaluationOptions &eo, bool b_top = true) {
  if (m == mdiff) {
    m.setProtected(true);
  } else {
    for (size_t i = 0; i < m.size(); i++) {
...
int DSolveFunction::calculate(MathStructure &mstruct,
                              const MathStructure &vargs,
                              const EvaluationOptions &eo) {
  MathStructure m_eqn(vargs[0]);
  EvaluationOptions eo2 = eo;
  eo2.isolate_x = false;
  eo2.protected_function =
      CALCULATOR->getFunctionById(FUNCTION_ID_DIFFERENTIATE);
  m_eqn.eval(eo2);
  MathStructure *m_diff_p = NULL;
  if (m_eqn.isLogicalAnd()) {
    for (size_t i = 0; i < m_eqn.size(); i++) {
      if (m_eqn[i].isComparison() &&
          m_eqn.comparisonType() == COMPARISON_EQUALS) {
        m_diff_p = find_deqn(m_eqn[i]);
        if (m_diff_p)
          break;
      }
    }
  } else if (m_eqn.isComparison() &&
             m_eqn.comparisonType() == COMPARISON_EQUALS) {
  ...
  if (m_diff.size() < 3 ||
      (!m_diff[0].isSymbolic() && !m_diff[0].isVariable()) ||
      (!m_diff[1].isSymbolic() && !m_diff[1].isVariable()) ||
      !m_diff[2].isInteger() || !m_diff[2].number().isPositive() ||
      !m_diff[2].number().isLessThanOrEqualTo(10)) {
    CALCULATOR->error(true, _("No differential equation found."), NULL);
    mstruct = m_eqn;
    return -1;
  }
  ...
  if (m_eqn.isLogicalAnd()) {
    for (size_t i = 0; i < m_eqn.size(); i++) {
      if (m_eqn[i].isComparison() &&
          m_eqn[i].comparisonType() == COMPARISON_EQUALS &&
          m_eqn[i][0] == m_diff) {
    ...
    for (size_t i = 0; i < m_eqn.size(); i++) {
      if (m_eqn[i].isComparison() &&
        ...
        for (size_t i2 = 0; i2 < m_eqn[i].size(); i2++) {
          if (m_eqn[i][i2].isComparison() &&
              m_eqn[i][i2].comparisonType() == COMPARISON_EQUALS &&
              m_eqn[i][i2][0] == m_diff) {
  ...
  if (solve_equation(msolve, m_eqn, m_diff[0], eo, true, m_diff[1],
                     MathStructure(CALCULATOR->getVariableById(VARIABLE_ID_C)),
                     vargs[2], vargs[1]) <= 0) {
    CALCULATOR->error(true, _("Unable to solve differential equation."), NULL);
    protect_mdiff(mstruct, m_diff, eo);
    return -1;
...
NewtonRaphsonFunction::NewtonRaphsonFunction()
    : MathFunction("newtonsolve", 2, 5) {
  ...
  setArgumentDefinition(5, new IntegerArgument("", ARGUMENT_MIN_MAX_NONE, true,
                                               true, INTEGER_TYPE_UINT));
  setDefaultValue(5, "1000");
}
int NewtonRaphsonFunction::calculate(MathStructure &mstruct,
                                     const MathStructure &vargs,
                                     const EvaluationOptions &eo) {
  int ret = 0;
  ...
  if (mdiff.containsFunction(
          CALCULATOR->getFunctionById(FUNCTION_ID_DIFFERENTIATE)))
    return ret;
  mfunc /= mdiff;
  Number nr_prec(1, 1,
                 vargs[3].number() <= 0
                     ? -(PRECISION - vargs[3].number().intValue())
                     : -vargs[3].number().intValue());
  Number nr_mprec(nr_prec);
  nr_mprec.negate();
...
#define TEST_ZERO                                                              \
  ...
  CALCULATOR->endTemporaryStopMessages();                                      \
  CALCULATOR->endTemporaryEnableIntervalArithmetic();                          \
  if (mztest.isNumber() && !mztest.number().isNonZero())                       \
    compare_with_1 = true;                                                     \
  zero_tested = true;
...
#define TEST_VALUE                                                             \
  bool b = false;                                                              \
  if (eo2.interval_calculation == INTERVAL_CALCULATION_VARIANCE_FORMULA &&     \
      vargs[0].containsInterval(true, true, false, 1, true)) {                 \
    b = true;                                                                  \
  } else {                                                                     \
    TEST_VALUE_B                                                               \
    ...
    for (size_t i = 0; i < blocked_messages.size(); i++) {                     \
      if (blocked_messages[i].category() ==                                    \
          MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT) {                       \
        b = true;                                                              \
        break;                                                                 \
      }                                                                        \
    }                                                                          \
    if (!b && !mtest.representsNonZero())                                      \
      b = true;                                                                \
  }
...
#define TEST_VALUE_STRICT                                                      \
  ...
  TEST_VALUE_B                                                                 \
  CALCULATOR->endTemporaryStopMessages();                                      \
  if (mtest.isNumber() && !mtest.number().isNonZero())                         \
    b = true;
  ...
  while (true) {
    ...
    if (!x_if.isNumber())
      break;
    if (x_if.isZero() && x_i.isZero()) {
      ret = 1;
      break;
    }
    if (iter > 0)
      x_itest = x_if.number();
    if ((iter > 0 && !compare_with_1 && !x_itest.divide(x_i)) ||
        !x_itest.abs() || !x_i.subtract(x_if.number()))
      break;
    if (iter > 0) {
      if (x_i.hasImaginaryPart()) {
        if ((x_itest.realPart() < nr_prec &&
             x_itest.imaginaryPart() < nr_prec) ||
            (!x_i.isNonZero() &&
             (x_i.realPart() < nr_prec && x_i.imaginaryPart() < nr_prec &&
              x_if.number().realPart() < nr_prec &&
              x_if.number().imaginaryPart() < nr_prec))) {
          TEST_VALUE_XIF
          if (b) {
        ...
        if (!zero_tested &&
            ((iter >= max_iter - 1 && x_i.realPart().isFraction() &&
              x_i.imaginaryPart().isFraction()) ||
             (iter > 10 && x_i.realPart() < nr_prec &&
              x_i.realPart() > nr_mprec && x_if.number().realPart() < nr_prec &&
              x_if.number().realPart() > nr_mprec &&
              x_i.imaginaryPart() < nr_prec && x_i.imaginaryPart() > nr_mprec &&
              x_if.number().imaginaryPart() < nr_prec &&
              x_if.number().imaginaryPart() > nr_mprec))) {
          TEST_ZERO
        }
        int prec = x_i.precision(true);
        if ((!x_i.isNonZero() || (prec >= 0 && prec < precbak)) &&
            PRECISION * 5 < 1000) {
          CALCULATOR->setPrecision(PRECISION * 10 > 1000 ? 1000
                                                         : PRECISION * 10);
          CALCULATOR->endTemporaryStopMessages();
          goto nrf_begin;
        }
        ...
        if (x_itest < nr_prec ||
            (!x_i.isNonZero() && x_i < nr_prec && x_if.number() < nr_prec)) {
          TEST_VALUE_XIF
          if (b) {
            x_i.setUncertainty(x_if.number(),
                               !CALCULATOR->usesIntervalArithmetic());
        ...
        if (!zero_tested &&
            ((iter >= max_iter - 1 && x_i.isFraction()) ||
             (iter > 10 && x_i < nr_prec && x_i > nr_mprec &&
              x_if.number() < nr_prec && x_if.number() > nr_mprec))) {
          TEST_ZERO
        }
        int prec = x_i.precision(true);
        if ((!x_i.isNonZero() || (prec >= 0 && prec < precbak)) &&
...
SecantMethodFunction::SecantMethodFunction()
    : MathFunction("secantsolve", 3, 6) {
  setArgumentDefinition(2, new NumberArgument());
  ...
  setArgumentDefinition(6, new IntegerArgument("", ARGUMENT_MIN_MAX_NONE, true,
                                               true, INTEGER_TYPE_UINT));
  setDefaultValue(6, "1000");
}
int SecantMethodFunction::calculate(MathStructure &mstruct,
                                    const MathStructure &vargs,
                                    const EvaluationOptions &eo) {
  int ret = 0;
  MathStructure mfunc(vargs[0]);
  ...
  Number nr_prec(1, 1,
                 vargs[4].number() <= 0
                     ? -(PRECISION - vargs[4].number().intValue())
                     : -vargs[4].number().intValue());
  Number nr_mprec(nr_prec);
  nr_mprec.negate();
  int precbak = PRECISION;
  KnownVariable *v = new KnownVariable("", "", m_zero);
...
secm_begin:
  ...
  if (m_if.isNumber()) {
    ...
    while (true) {
      ...
      v->setName(format_and_print(x_i));
      m_if.eval(eo2);
      if (!m_if.isNumber() || !x0.negate() || !x0.add(x_i) || !f0.negate() ||
          !f0.add(m_if.number()) || !x0.divide(f0) ||
          !x0.multiply(m_if.number()))
        break;
      x_fi = x0;
      x0 = x_i;
      f0 = m_if.number();
      if (x_fi.isZero() && x_i.isZero()) {
        ret = 1;
        break;
      }
      if (iter > 0)
      ...
      if (iter > 0) {
        if (x_i.hasImaginaryPart()) {
          if ((x_itest.realPart() < nr_prec &&
              ...
              (!x_i.isNonZero() &&
               (x_i.realPart() < nr_prec && x_i.imaginaryPart() < nr_prec &&
                x_fi.realPart() < nr_prec && x_fi.imaginaryPart() < nr_prec))) {
            TEST_VALUE_XFI
            if (b) {
          ...
          if (!zero_tested &&
              ((iter >= max_iter - 1 && x_i.realPart().isFraction() &&
                x_i.imaginaryPart().isFraction()) ||
               (iter > 10 && x_i.realPart() < nr_prec &&
                x_i.realPart() > nr_mprec && x_fi.realPart() < nr_prec &&
                x_fi.realPart() > nr_mprec && x_i.imaginaryPart() < nr_prec &&
                x_i.imaginaryPart() > nr_mprec &&
                x_fi.imaginaryPart() < nr_prec &&
                x_fi.imaginaryPart() > nr_mprec))) {
            TEST_ZERO
          }
          int prec = x_i.precision(true);
          if ((!x_i.isNonZero() || (prec >= 0 && prec < precbak)) &&
          ...
          if (x_itest < nr_prec ||
              (!x_i.isNonZero() && x_i < nr_prec && x_fi < nr_prec)) {
            TEST_VALUE_XFI
            if (b) {
              x_i.setUncertainty(x_fi, !CALCULATOR->usesIntervalArithmetic());
              ret = 1;
          ...
          if (!zero_tested && ((iter >= max_iter - 1 && x_i.isFraction()) ||
                               (iter > 10 && x_i < nr_prec && x_i > nr_mprec &&
                                x_fi < nr_prec && x_fi > nr_mprec))) {
            TEST_ZERO
          }
          int prec = x_i.precision(true);
          if ((!x_i.isNonZero() || (prec >= 0 && prec < precbak)) &&
