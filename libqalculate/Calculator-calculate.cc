...
void autoConvert(const MathStructure &morig, MathStructure &mconv,
  ...
  if (eo.mixed_units_conversion != MIXED_UNITS_CONVERSION_NONE)
    mconv.set(CALCULATOR->convertToMixedUnits(mconv, eo));
}
...
void CalculateThread::run() {
  ...
  while (true) {
    ...
    if (b_parse) {
      mstruct->setAborted();
      if (CALCULATOR->tmp_parsedstruct)
        CALCULATOR->tmp_parsedstruct->setAborted();
      // if(CALCULATOR->tmp_tostruct) CALCULATOR->tmp_tostruct->setUndefined();
      if (CALCULATOR->expression_to_calculate.find_first_of(ID_WRAPS) !=
          string::npos) {
        ...
        mstruct->set(CALCULATOR->calculate(
            str, CALCULATOR->tmp_evaluationoptions,
            CALCULATOR->tmp_parsedstruct, CALCULATOR->tmp_tostruct,
            CALCULATOR->tmp_maketodivision));
      } else {
        mstruct->set(CALCULATOR->calculate(
            CALCULATOR->expression_to_calculate,
            CALCULATOR->tmp_evaluationoptions, CALCULATOR->tmp_parsedstruct,
            CALCULATOR->tmp_tostruct, CALCULATOR->tmp_maketodivision));
      }
    } else {
      MathStructure meval(*mstruct);
      mstruct->setAborted();
      mstruct->set(CALCULATOR->calculate(
          meval, CALCULATOR->tmp_evaluationoptions,
          CALCULATOR->tmp_tostruct ? CALCULATOR->tmp_tostruct->symbol() : ""));
    }
    ...
    case PROC_RPN_OPERATION_F: {
      for (size_t i = 0; (CALCULATOR->tmp_proc_registers < 0 ||
                          (int)i < CALCULATOR->tmp_proc_registers - 1) &&
                         CALCULATOR->RPNStackSize() > 1;
           i++) {
        CALCULATOR->deleteRPNRegister(1);
      }
      if (CALCULATOR->RPNStackSize() > 0 &&
          CALCULATOR->tmp_proc_registers != 0) {
        CALCULATOR->setRPNRegister(1, mstruct, false);
      } else {
        CALCULATOR->RPNStackEnter(mstruct, false);
      }
...
bool Calculator::abort() {
  ...
  if (!calculate_thread->running) {
    ...
    if (b_busy) {
      ...
      i_start_interval = 0;
      if (tmp_rpn_mstruct)
        tmp_rpn_mstruct->unref();
      tmp_rpn_mstruct = NULL;

      // thread cancellation is not safe
      if (i_precision > 10000) {
        error(true,
              _("The calculation has been forcibly terminated. Please restart "
                "the application."),
              NULL);
      } else {
        error(true,
              _("The calculation has been forcibly terminated. Please restart "
                "the application and report this as a bug."),
              NULL);
      }
...
bool Calculator::busy() { return b_busy; }
void Calculator::terminateThreads() {
  if (calculate_thread->running) {
    if (!calculate_thread->write(false) || !calculate_thread->write(NULL))
      calculate_thread->cancel();
...
bool Calculator::calculateRPNRegister(size_t index, int msecs,
                                      const EvaluationOptions &eo) {
  if (index <= 0 || index > rpn_stack.size())
    return false;
  return calculateRPN(new MathStructure(*rpn_stack[rpn_stack.size() - index]),
                      PROC_RPN_SET, index, msecs, eo);
}

bool Calculator::calculateRPN(MathStructure *mstruct, int command, size_t index,
                              int msecs, const EvaluationOptions &eo,
                              int function_arguments) {
  b_busy = true;
  if (!calculate_thread->running && !calculate_thread->start()) {
    mstruct->setAborted();
    return false;
  }
  ...
  tmp_tostruct = NULL;
  if (!calculate_thread->write(false)) {
    calculate_thread->cancel();
    mstruct->setAborted();
    return false;
  }
  if (!calculate_thread->write((void *)mstruct)) {
    calculate_thread->cancel();
    mstruct->setAborted();
    return false;
  }
...
bool Calculator::calculateRPN(string str, int command, size_t index, int msecs,
                              const EvaluationOptions &eo,
                              MathStructure *parsed_struct,
                              MathStructure *to_struct, bool make_to_division,
                              int function_arguments) {
  ...
  tmp_proc_registers = function_arguments;
  if (!calculate_thread->write(true)) {
    calculate_thread->cancel();
    mstruct->setAborted();
    return false;
  }
...
bool Calculator::calculateRPN(MathOperation op, int msecs,
                              const EvaluationOptions &eo,
                              MathStructure *parsed_struct) {
...
bool Calculator::calculateRPN(MathFunction *f, int msecs,
                              const EvaluationOptions &eo,
                              MathStructure *parsed_struct) {
  ...
  if (f->args() != 0) {
    size_t i = f->minargs();
    if (f == f_logn && rpn_stack.size() > 1)
      i = 2;
    bool fill_vector =
        (i > 0 && f->getArgumentDefinition(i) &&
         f->getArgumentDefinition(i)->type() == ARGUMENT_TYPE_VECTOR);
    if (fill_vector && rpn_stack.size() < i)
      fill_vector = false;
    if (fill_vector && rpn_stack.size() > 0 && rpn_stack.back()->isVector())
      fill_vector = false;
    if (fill_vector) {
      i = rpn_stack.size();
    } else if (i < 1) {
    ...
    for (; i > 0; i--) {
      if (i > rpn_stack.size()) {
        error(false,
              _("Stack is empty. Filling remaining function arguments with "
                "zeroes."),
              NULL);
        mstruct->addChild(m_zero);
      } else {
        if (fill_vector && rpn_stack.size() - i == (size_t)f->minargs() - 1)
          mstruct->addChild(m_empty_vector);
        if (fill_vector && rpn_stack.size() - i >= (size_t)f->minargs() - 1)
          mstruct->getChild(f->minargs())
              ->addChild(*rpn_stack[rpn_stack.size() - i]);
        else
          mstruct->addChild(*rpn_stack[rpn_stack.size() - i]);
        iregs++;
      }
      if (!fill_vector && f->getArgumentDefinition(i) &&
          f->getArgumentDefinition(i)->type() == ARGUMENT_TYPE_ANGLE) {
        Unit *u = default_angle_unit(eo);
        if (u)
          (*mstruct)[i - 1].multiply(u);
      }
    }
    if (fill_vector)
  ...
  if (parsed_struct)
    parsed_struct->set(*mstruct);
  return calculateRPN(mstruct, PROC_RPN_OPERATION_F, 0, msecs, eo, iregs);
}
bool Calculator::calculateRPNBitwiseNot(int msecs, const EvaluationOptions &eo,
                                        MathStructure *parsed_struct) {
  MathStructure *mstruct;
  if (rpn_stack.size() == 0) {
    mstruct = new MathStructure();
...
bool Calculator::calculateRPNLogicalNot(int msecs, const EvaluationOptions &eo,
                                        MathStructure *parsed_struct) {
  MathStructure *mstruct;
  if (rpn_stack.size() == 0) {
    mstruct = new MathStructure();
...
MathStructure *
Calculator::calculateRPNBitwiseNot(const EvaluationOptions &eo,
                                   MathStructure *parsed_struct) {
  current_stage = MESSAGE_STAGE_PARSING;
  MathStructure *mstruct;
  if (rpn_stack.size() == 0) {
...
MathStructure *
Calculator::calculateRPNLogicalNot(const EvaluationOptions &eo,
                                   MathStructure *parsed_struct) {
  current_stage = MESSAGE_STAGE_PARSING;
  MathStructure *mstruct;
  if (rpn_stack.size() == 0) {
...
bool Calculator::RPNStackEnter(MathStructure *mstruct, int msecs,
                               const EvaluationOptions &eo) {
  return calculateRPN(mstruct, PROC_RPN_ADD, 0, msecs, eo);
}
bool Calculator::RPNStackEnter(string str, int msecs,
                               const EvaluationOptions &eo,
                               MathStructure *parsed_struct,
  ...
  return calculateRPN(str, PROC_RPN_ADD, 0, msecs, eo, parsed_struct, to_struct,
                      make_to_division);
}
...
void Calculator::RPNStackEnter(string str, const EvaluationOptions &eo,
                               MathStructure *parsed_struct,
                               MathStructure *to_struct,
                               bool make_to_division) {
  remove_blank_ends(str);
  if (str.empty() && rpn_stack.size() > 0)
    ...
    rpn_stack.push_back(new MathStructure(
        calculate(str, eo, parsed_struct, to_struct, make_to_division)));
}
bool Calculator::setRPNRegister(size_t index, MathStructure *mstruct, int msecs,
                                const EvaluationOptions &eo) {
  if (mstruct == NULL) {
...
bool Calculator::setRPNRegister(size_t index, string str, int msecs,
                                const EvaluationOptions &eo,
                                MathStructure *parsed_struct,
                                MathStructure *to_struct,
                                bool make_to_division) {
  if (index <= 0 || index > rpn_stack.size())
    return false;
  return calculateRPN(str, PROC_RPN_SET, index, msecs, eo, parsed_struct,
                      to_struct, make_to_division);
}
...
void Calculator::setRPNRegister(size_t index, string str,
                                const EvaluationOptions &eo,
                                MathStructure *parsed_struct,
                                MathStructure *to_struct,
                                bool make_to_division) {
...
size_t Calculator::RPNStackSize() const { return rpn_stack.size(); }
...
int has_information_unit(const MathStructure &m, bool top) {
  if (m.isUnit_exp()) {
    if (m.isUnit()) {
      if (m.unit()->baseUnit()->referenceName() == "bit")
        return 1;
    } else {
      if (m[0].unit()->baseUnit()->referenceName() == "bit") {
        if (m[1].isInteger() && m[1].number().isPositive())
          return 1;
        return 2;
      }
    }
  ...
  for (size_t i = 0; i < m.size(); i++) {
    ...
    if (ret > 0) {
      if (ret == 1 && top && m.isMultiplication() && m[0].isNumber() &&
          m[0].number().isFraction())
        return 2;
      return ret;
    }
  }
...
// test if contains decimal value (if test_orig = true, check for decimal point
// in original expression)
bool contains_decimal(const MathStructure &m,
                      const string *original_expression = NULL) {
  if (original_expression && !original_expression->empty())
    return original_expression->find(DOT) != string::npos;
  if (m.isNumber())
    return !m.number().isInteger();
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_decimal(m[i]))
      return true;
  }
  return false;
}
...
int test_frac(const MathStructure &m, bool test_combined = true,
              int limit = 1000) {
  if (m.isNumber()) {
    if (!m.number().isRational())
      return 0;
    if (limit < 0 || m.number().isInteger())
      return 1;
    if (m.number().denominatorIsLessThan(limit)) {
      if (m.number().numeratorIsLessThan(limit) &&
          m.number().numeratorIsGreaterThan(-limit))
        return 1;
      else if (test_combined)
        return 2;
    }
    return 0;
  }
  if (test_combined && m.isNegate())
    return test_frac(m[0], true, limit);
  for (size_t i = 0; i < m.size(); i++) {
    if (!test_frac(m[i], false, limit))
      return 0;
  }
  return 1;
}

void print_m(PrintOptions &po, const EvaluationOptions &evalops, string &str,
             vector<string> &results_v, MathStructure &m,
             const MathStructure *mresult, const string &original_expression,
             const MathStructure *mparse, int dfrac, int dappr, bool cplx_angle,
             bool only_cmp = false, bool format = false, int colorize = 0,
             int tagtype = TAG_TYPE_HTML, int max_length = -1) {
  bool b_exact = !mresult->isApproximate();
  // avoid multiple results with inequalities
  if ((dfrac != 0 || dappr < 0) && (only_cmp || dfrac <= 0) && b_exact &&
      m.isComparison() && m.comparisonType() != COMPARISON_EQUALS &&
      po.number_fraction_format == FRACTION_DECIMAL) {
    po.number_fraction_format =
        (dfrac != 0 && (!m[0].isSymbolic() || !m[0].isVariable()))
            ? FRACTION_FRACTIONAL
  ...
  if (m.isComparison() && m.size() == 3 &&
      m.comparisonType() == COMPARISON_EQUALS) {
    if (m[2].isUndefined()) {
  ...
  // do not show simple fractions in auto modes if result includes units or
  // expression contains decimals
  if (!CALCULATOR->aborted() &&
      (b_cmp3 ||
       ((dfrac || (dappr && po.is_approximate && *po.is_approximate)) &&
        (!only_cmp ||
         (m.isComparison() && m.comparisonType() == COMPARISON_EQUALS)) &&
        po.base == 10 && b_exact &&
        (po.number_fraction_format == FRACTION_DECIMAL_EXACT ||
         po.number_fraction_format == FRACTION_DECIMAL) &&
        (dfrac > 0 || dappr > 0 ||
         mresult->containsType(STRUCT_UNIT, false, true, true) <= 0)))) {
    bool do_frac = false, do_mixed = false;
    const MathStructure *mcmp = mresult;
    if (b_cmp3) {
      do_frac = true;
      mcmp = &m[2];
    } else if (!mparse || ((dfrac > 0 || dappr > 0 ||
                            !contains_decimal(*mparse, &original_expression)) &&
                           (!mparse->isNumber() || !mresult->isNumber()))) {
      if (contains_decimal(m)) {
        if (m.isComparison() && m.comparisonType() == COMPARISON_EQUALS) {
          mcmp = mresult->getChild(2);
        }
        int itf =
            test_frac(*mcmp, !only_cmp, dfrac > 0 || dappr > 0 ? -1 : 1000);
        if (itf && !mcmp->isInteger()) {
          ...
          if (mcmp == mresult && mparse) {
            if (mcmp->isNumber() && mcmp->number().isRational()) {
              do_frac =
                  itf == 1 &&
                  ((evalops.parse_options.base != po.base) ||
                   ((!mparse->isDivision() ||
                     (*mparse)[1] != mcmp->number().denominator() ||
                     ((*mparse)[0] != mcmp->number().numerator() &&
                      (!(*mparse)[0].isNegate() ||
                       (*mparse)[0][0] != -mcmp->number().numerator()))) &&
                    (!mparse->isNegate() || !(*mparse)[0].isDivision() ||
                     (*mparse)[0][1] != mcmp->number().denominator() ||
                     (*mparse)[0][0] != -mcmp->number().numerator())));
              do_mixed = (dfrac != 0 || !do_frac) &&
                         !mcmp->number().isNegative() &&
                         !mcmp->number().isFraction();
            } else if (do_frac && mresult->isFunction() &&
                       mparse->isFunction() &&
                       mresult->function() == mparse->function()) {
              do_frac = false;
            }
          }
        }
    ...
    if (do_frac) {
      ...
      if (b_approx || results_v[results_v.size() - 1] == str) {
        ...
        if (cplx_angle)
          replace_result_cis(results_v.back());
      }
    }
    if (do_mixed) {
      b_approx = false;
    ...
    if (m.isComparison() && m.comparisonType() == COMPARISON_EQUALS &&
      ...
      if (ipos == string::npos) {
        ...
        if (ipos != string::npos)
          ipos2 = ipos + strlen(" " SIGN_ALMOST_EQUAL " ");
      ...
      if (ipos != string::npos) {
        for (size_t i = results_v.size(); i > 0; i--) {
          if ((max_length < 0 ||
               (unicode_length(str) + unicode_length(results_v[i - 1]) + 3 <=
                (size_t)max_length)) &&
              results_v[i - 1] != str.substr(ipos2)) {
...
// check for for unknown variables/symbols not on the left side of
// (in)equalities
...
bool test_power_func(const MathStructure &m) {
  if (m.isFunction() ||
      (m.isPower() && !m[0].containsType(STRUCT_UNIT, false, false, false) &&
       !m[1].isInteger()))
    return true;
  for (size_t i = 0; i < m.size(); i++) {
    if (test_power_func(m[i]))
      return true;
  }
  return false;
}
...
void print_dual(const MathStructure &mresult, const string &original_expression,
                const MathStructure &mparse, MathStructure &mexact,
                string &result_str, vector<string> &results_v, PrintOptions &po,
                const EvaluationOptions &evalops,
                AutomaticFractionFormat auto_frac,
                AutomaticApproximation auto_approx, bool cplx_angle,
                bool *exact_cmp, bool b_parsed, bool format, int colorize,
                int tagtype, int max_length) {
  print_dual(mresult, original_expression, mparse, mexact, result_str,
             results_v, po, evalops, auto_frac, auto_approx, cplx_angle,
             exact_cmp, b_parsed, format, colorize, tagtype, max_length, false);
}
void print_dual(const MathStructure &mresult, const string &original_expression,
                const MathStructure &mparse, MathStructure &mexact,
                string &result_str, vector<string> &results_v, PrintOptions &po,
                ...
                AutomaticApproximation auto_approx, bool cplx_angle,
                bool *exact_cmp, bool b_parsed, bool format, int colorize,
                int tagtype, int max_length, bool converted) {
  ...
  if (po.base == BASE_TIME) {
    ...
    if (m.isUnit() && m.unit()->baseUnit()->referenceName() == "s") {
      b = true;
    } else if (m.isMultiplication() && m.size() == 2 && m[0].isNumber() &&
               m[1].isUnit() &&
               m[1].unit()->baseUnit()->referenceName() == "s") {
      b = true;
    } else if (m.isAddition() && m.size() > 0) {
      b = true;
      for (size_t i = 0; i < m.size(); i++) {
        if (m[i].isUnit() && m[i].unit()->baseUnit()->referenceName() == "s") {
        } else if (m[i].isMultiplication() && m[i].size() == 2 &&
                   m[i][0].isNumber() && m[i][1].isUnit() &&
                   m[i][1].unit()->baseUnit()->referenceName() == "s") {
        } else {
          b = false;
          break;
        }
  ...
  if (!po.is_approximate)
    po.is_approximate = &tmp_approx;

  // dual/auto approximation: requires that the value is approximate and exact
  // result is set
  bool do_exact = !mexact.isUndefined() && m.isApproximate();

  // If parsed value is number (simple fractions are parsed as division) only
  // show result as combined fraction
  if (auto_frac != AUTOMATIC_FRACTION_OFF && po.base == 10 &&
      po.base == evalops.parse_options.base && !m.isApproximate() && b_parsed &&
      mparse.isNumber() && !mparse.isInteger() && !converted) {
    po.number_fraction_format = FRACTION_COMBINED;
    // with auto fractions show expressions with unknown variables/symbols only
    // using simple fractions (if not parsed value contains decimals)
  } else if ((auto_frac == AUTOMATIC_FRACTION_AUTO ||
              auto_frac == AUTOMATIC_FRACTION_SINGLE) &&
             !m.isApproximate() &&
             (test_fr_unknowns(m) ||
              (m.containsType(STRUCT_ADDITION) && test_power_func(m))) &&
             test_frac(m, false, -1) &&
             (!b_parsed || !contains_decimal(mparse, &original_expression))) {
    po.number_fraction_format = FRACTION_FRACTIONAL;
    po.restrict_fraction_length = true;
    // with auto fractions and exact approximation (or inequality) do not show
    // approximate decimal fractions
  } else if (auto_frac != AUTOMATIC_FRACTION_OFF &&
             po.number_fraction_format == FRACTION_DECIMAL &&
             (evalops.approximation == APPROXIMATION_EXACT ||
              (auto_frac != AUTOMATIC_FRACTION_DUAL && m.isComparison() &&
               m.comparisonType() != COMPARISON_EQUALS))) {
    po.number_fraction_format = FRACTION_DECIMAL_EXACT;
  }

  // for equalities, append exact values to approximate value (as third part of
  // equalities)
  if (do_exact) {
  ...
  if (auto_frac != AUTOMATIC_FRACTION_OFF && po.base == 10 &&
      ...
      po.number_fraction_format == FRACTION_DECIMAL_EXACT &&
      !m.isApproximate() && mresult.isNumber() &&
      !mresult.number().isFraction() && !mresult.number().isNegative() &&
      b_parsed && mparse.equals(m)) {
    po.number_fraction_format = FRACTION_COMBINED;
    m = mresult;
    m.removeDefaultAngleUnit(evalops);
    m.format(po);
  ...
  int dappr = 0, dfrac = 0;
  if (auto_frac == AUTOMATIC_FRACTION_AUTO)
    dfrac = -1;
  else if (auto_frac == AUTOMATIC_FRACTION_DUAL)
    dfrac = 1;
  if (auto_approx == AUTOMATIC_APPROXIMATION_AUTO)
    dappr = -1;
  else if (auto_approx == AUTOMATIC_APPROXIMATION_DUAL)
    dappr = 1;
  if (m.isLogicalOr()) {
    if (max_length > 0)
      max_length /= m.size();
    ...
    for (size_t i = 0; i < m.size(); i++) {
      if (CALCULATOR->aborted()) {
        result_str = CALCULATOR->abortedMessage();
        break;
      }
      if (i == 0) {
        result_str = "";
      ...
      if (m[i].isLogicalAnd() &&
          (po.preserve_format || m[i].size() != 2 || !m[i][0].isComparison() ||
           !m[i][1].isComparison() ||
           m[i][0].comparisonType() == COMPARISON_EQUALS ||
           m[i][0].comparisonType() == COMPARISON_NOT_EQUALS ||
           m[i][1].comparisonType() == COMPARISON_EQUALS ||
           m[i][1].comparisonType() == COMPARISON_NOT_EQUALS ||
           m[i][0][0] != m[i][1][0])) {
        ...
        for (size_t i2 = 0; i2 < m[i].size(); i2++) {
          ...
          bool b_wrap =
              m[i][i2].needsParenthesis(po, ips, m[i], i2 + 1, true, true);
          string str;
          print_m(po, evalops, str, results_v, m[i][i2], &m[i][i2],
                  original_expression, &mparse, dfrac, dappr, cplx_angle, true,
                  format, colorize, tagtype, ml);
          if (b_wrap)
            result_str += "(";
          result_str += str;
          if (b_wrap)
        ...
        string str;
        print_m(po, evalops, str, results_v, m[i], &m[i], original_expression,
                &mparse, dfrac, dappr, cplx_angle, true, format, colorize,
                tagtype, max_length);
        if (b_wrap)
          result_str += "(";
        result_str += str;
        ...
        b_approx = *po.is_approximate;
             ...
             (po.preserve_format || m.size() != 2 || !m[0].isComparison() ||
              !m[1].isComparison() ||
              m[0].comparisonType() == COMPARISON_EQUALS ||
              m[0].comparisonType() == COMPARISON_NOT_EQUALS ||
              m[1].comparisonType() == COMPARISON_EQUALS ||
              m[1].comparisonType() == COMPARISON_NOT_EQUALS ||
              m[0][0] != m[1][0])) {
    InternalPrintStruct ips;
    bool b_approx = false;
    max_length /= m.size();
    ...
    print_m(po, evalops, result_str, results_v, m, &mresult,
            original_expression, &mparse, dfrac, dappr, cplx_angle, false,
            format, colorize, tagtype, max_length);
    if (do_exact && (m.type() != STRUCT_COMPARISON ||
                     m.comparisonType() != COMPARISON_EQUALS)) {
      bool *old_approx = po.is_approximate;
      bool b_approx = false;
      po.is_approximate = &b_approx;
...
bool test_simplified2(const MathStructure &m1, const MathStructure &m2) {
  ...
  if (m1.size() == 0)
    return m1.equals(m2, true, true);
  for (size_t i = 0; i < m1.size(); i++) {
    if (!test_simplified2(m1[i], m2[i]))
      return false;
  }
  return true;
}
bool test_simplified(const MathStructure &m, bool top = true) {
  if (m.isFunction() || (m.isVariable() && m.variable()->isKnown()) ||
      (m.isUnit() && (m.unit()->hasApproximateRelationToBase() ||
                      (m.unit()->isCurrency() &&
                       m.unit() != CALCULATOR->getLocalCurrency()))))
    return false;
  for (size_t i = 0; i < m.size(); i++) {
    if (!test_simplified(m[i], false))
      return false;
  }
  if (m.isPower() && m[0].containsType(STRUCT_NUMBER))
    return false;
  if (!top && m.isNumber() && m.number().isFloatingPoint())
    return false;
  return true;
}
...
void replace_negdiv(MathStructure &m) {
  ...
  if (m.isNegate()) {
    if (m[0].isNumber()) {
      ...
      m.setToChild(1);
      return replace_negdiv(m);
    } else if ((m[0].isDivision() || m[0].isInverse() ||
  ...
  if (m.isFunction()) {
    ...
    } else if (m.function()->id() == FUNCTION_ID_ROOT && m.size() == 2 &&
               m[1].isInteger()) {
      if (m[0].representsNonNegative()) {
        m.setType(STRUCT_POWER);
        m[1].number().recip();
...
void replace_zero_symbol(MathStructure &m) {
  if (m.isFunction()) {
    for (size_t i = 1; i < m.size(); i++) {
      ...
      if (arg && arg->type() == ARGUMENT_TYPE_SYMBOLIC &&
          (m[i].isZero() || m[i].isUndefined())) {
        m[i].set(m[0].find_x_var(), true);
        if (m[i].isUndefined() && m[0].isVariable() &&
            m[0].variable()->isKnown())
          m[i].set(((KnownVariable *)m[0].variable())->get().find_x_var(),
                   true);
        if (m[i].isUndefined())
          m[i].set(CALCULATOR->getVariableById(VARIABLE_ID_X), true);
      }
    }
  } else if (m.isVariable() && m.variable() == CALCULATOR->getVariableById(
                                                   VARIABLE_ID_UNDEFINED)) {
    m.setUndefined();
  }
  for (size_t i = 0; i < m.size(); i++) {
  ...
  if (m1.isNumber() && m2.isNumber()) {
    ...
    if (m1.type() == m2.type() && m1.size() == m2.size()) {
      ...
      if (m1.type() == STRUCT_COMPARISON) {
        if (m1.comparisonType() != m2.comparisonType())
          return false;
      } else if (m1.type() == STRUCT_FUNCTION) {
        if (m1.function() != m2.function())
          return false;
      }
      for (size_t i = 0; i < m1.size(); i++) {
        if (!comparison_compare(m1[i], m2[i]))
          return false;
      }
      return true;
    } else if (m1.isAddition() && m2.isAddition()) {
      if (m1.size() + 1 == m2.size() && m2.last().isNumber() &&
          !m2.last().number().isNonZero()) {
        for (size_t i = 0; i < m1.size(); i++) {
          if (!comparison_compare(m1[i], m2[i]))
            return false;
        }
        ...
        return true;
      } else if (m2.size() + 1 == m1.size() && m1.last().isNumber() &&
                 !m1.last().number().isNonZero()) {
      ...
      if (m1.size() + 1 == m2.size() && m2[0].isNumber() &&
          !m2[0].number().imaginaryPartIsNonZero()) {
        for (size_t i = 0; i < m1.size(); i++) {
          if (!comparison_compare(m1[i], m2[i + 1]))
            return false;
        }
        ...
        m2.delChild(1, true);
        return true;
      } else if (m2.size() + 1 == m1.size() && m1[0].isNumber() &&
                 !m1[0].number().imaginaryPartIsNonZero()) {
        for (size_t i = 0; i < m2.size(); i++) {
          if (!comparison_compare(m1[i + 1], m2[i]))
            return false;
        }
    ...
    } else if (m2.isAddition() && m2.size() == 2 && m2.last().isNumber() &&
               !m2.last().number().isNonZero()) {
      if (comparison_compare(m1, m2[0])) {
        m2.delChild(m2.size(), true);
        return true;
      }
    } else if (m1.isAddition() && m1.size() == 2 && m1.last().isNumber() &&
               !m1.last().number().isNonZero()) {
      if (comparison_compare(m1[0], m2)) {
        return true;
      }
    } else if (m2.isMultiplication() && m2.size() == 2 && m2[0].isNumber() &&
               !m2[0].number().imaginaryPartIsNonZero()) {
      if (!comparison_compare(m1, m2[1]))
        return false;
      ComparisonResult cr = m2[0].number().compare(nr_one, true);
      if (cr != COMPARISON_RESULT_EQUAL && cr <= COMPARISON_RESULT_UNKNOWN)
        return false;
      m2.delChild(1, true);
      return true;
    } else if (m1.isMultiplication() && m1.size() == 2 && m1[0].isNumber() &&
               !m1[0].number().imaginaryPartIsNonZero()) {
      if (!comparison_compare(m1[1], m2))
        return false;
      ComparisonResult cr = m1[0].number().compare(nr_one, true);
      if (cr != COMPARISON_RESULT_EQUAL && cr <= COMPARISON_RESULT_UNKNOWN)
        return false;
...
bool test_max_addition_size(const MathStructure &m, size_t n) {
  if (m.isAddition() && m.size() > n)
    return true;
  for (size_t i = 0; i < m.size(); i++) {
    if (test_max_addition_size(m[i], n))
      return true;
  }
  return false;
}

void calculate_dual_exact(MathStructure &mstruct_exact, MathStructure *mstruct,
                          const string &original_expression,
                          const MathStructure *parsed_mstruct,
                          EvaluationOptions &evalops,
                          AutomaticApproximation auto_approx, int msecs,
                          int max_size) {
  ...
  if (dual_approximation != 0 &&
      ...
      (dual_approximation > 0 ||
       (!mstruct->containsType(STRUCT_UNIT, false, false, false) &&
        !parsed_mstruct->containsType(STRUCT_UNIT, false, false, false) &&
        original_expression.find(DOT) == string::npos)) &&
      !parsed_mstruct->containsFunctionId(FUNCTION_ID_SAVE) &&
      !parsed_mstruct->containsFunctionId(FUNCTION_ID_PLOT) &&
      !parsed_mstruct->containsFunctionId(FUNCTION_ID_RAND) &&
      !parsed_mstruct->containsFunctionId(FUNCTION_ID_RANDN) &&
      !parsed_mstruct->containsFunctionId(FUNCTION_ID_RAND_POISSON) &&
      !parsed_mstruct->containsInterval(true, false, false, false, true)) {
    evalops.approximation = APPROXIMATION_EXACT;
    evalops.expand = -2;
    CALCULATOR->beginTemporaryStopMessages();
    if (msecs > 0)
      CALCULATOR->startControl(msecs);
    MathStructure tmp_parse;
    mstruct_exact =
        CALCULATOR->calculate(original_expression, evalops, &tmp_parse);
    if (CALCULATOR->aborted() || mstruct_exact.isApproximate() ||
        (parsed_mstruct && !tmp_parse.equals(*parsed_mstruct, true, true)) ||
        (dual_approximation < 0 && max_size > 0 &&
         (test_max_addition_size(mstruct_exact, (size_t)max_size) ||
          mstruct_exact.countTotalChildren(false) > (size_t)max_size * 6))) {
      mstruct_exact.setUndefined();
    } else if (test_simplified(mstruct_exact) ||
               test_simplified2(mstruct_exact, *mstruct)) {
    ...
    if (mstruct_exact.containsType(STRUCT_COMPARISON)) {
      ...
      if (mcmp->isLogicalAnd() && mcmp->size() > 0)
        mcmp = &(*mcmp)[0];
      if (mcmp->isComparison() &&
          (mcmp->comparisonType() == COMPARISON_EQUALS ||
           dual_approximation > 0) &&
          !(*mcmp)[0].isNumber()) {
        if (mstruct_exact.isLogicalAnd() && mstruct_exact.size() > 0)
          mstruct_exact.setToChild(1);
        if (mstruct_exact.isComparison() &&
            mstruct_exact.comparisonType() == mcmp->comparisonType() &&
            mstruct_exact[0] == (*mcmp)[0]) {
          ...
          for (size_t i = 0; i < mstruct_exact.size(); i++) {
            if (mstruct_exact[i].isLogicalAnd() && mstruct_exact[i].size() > 0)
              mstruct_exact[i].setToChild(1);
            if (mstruct_exact[i].isComparison() &&
                mstruct_exact[i].comparisonType() == mcmp->comparisonType() &&
                mstruct_exact[i][0] == (*mcmp)[0]) {
        ...
        if (mstruct_exact.isLogicalOr() &&
            mstruct_exact.size() >= mcmp->size()) {
          b = true;
          for (size_t i = 0; i < mcmp->size() && b; i++) {
            MathStructure *mcmpi = &(*mcmp)[i];
            if (mcmpi->isLogicalAnd() && mcmpi->size() > 0)
              mcmpi = &(*mcmpi)[0];
            if (!mcmpi->isComparison() ||
                mcmpi->comparisonType() != COMPARISON_EQUALS ||
                (*mcmpi)[0].isNumber()) {
              b = false;
              break;
            }
            for (size_t i2 = 0; i2 < mstruct_exact.size();) {
              if (mstruct_exact[i2].isProtected()) {
                ...
                if (mstruct_exact[i2].isLogicalAnd() &&
                    mstruct_exact[i2].size() > 0)
                  mstruct_exact[i2].setToChild(1);
                if (mstruct_exact[i2].isComparison() &&
                    mstruct_exact[i2].comparisonType() ==
                        mcmpi->comparisonType() &&
                    mstruct_exact[i2][0] == (*mcmpi)[0]) {
                  ...
                  if (comparison_compare(mtest, m2)) {
                    if (!mtest.isApproximate()) {
                      (*mcmpi)[1].set(mtest);
                      mstruct_exact[i2][1].setUndefined();
                    } else
                      (*mcmpi)[1].set(m2);
                ...
                if (mstruct_exact.size() <= mcmp->size()) {
                  b = false;
                  break;
                }
          ...
          if (b) {
            for (size_t i2 = 0; i2 < mstruct_exact.size(); i2++)
              mstruct_exact[i2].setProtected(false);
          }
        }
      }
      if (!b)
    ...
    if (!mstruct_exact.isUndefined()) {
      ...
      mtest.evalSort(true);
      if (mstruct_exact.equals(mtest, true, true))
        mstruct_exact.setUndefined();
    }
    if (msecs > 0)
      CALCULATOR->stopControl();
...
bool expression_contains_save_function(const string &str,
                                       const ParseOptions &po,
                                       bool only_equals) {
  if (str.length() < 2 || po.base == BASE_UNICODE ||
      (po.base == BASE_CUSTOM && CALCULATOR->customInputBase() > 62))
    return false;
  size_t i = str.find("=", 1);
  if (!only_equals) {
    if (i != string::npos && ((i > 0 && str[i - 1] == ':') ||
  ...
  if (i_name2 == string::npos)
    return false;
  bool b_quote = i_name2 - i_name1 >= 2 &&
                 ((str[i_name2] == '\'' || str[i_name2] == '\"') &&
                  str[i_name1] == str[i_name2] &&
                  str.rfind(str[i_name1], i_name2 - 1) == i_name1);
  if (!b_quote)
    b_quote = (i_name2 - i_name1 >= 3 &&
               ((str[i_name1] == '\xe2' && str[i_name1 + 1] == '\x80') ||
                (str[i_name1] == '\xc2' &&
                 (str[i_name1 + 1] == '\xab' || str[i_name1 + 1] == '\xbb'))));
  for (size_t i2 = i_name1; b_quote && i2 < i_name2; i2++) {
    if (str[i2] == '\xe2' || str[i2] == '\xc2') {
      ...
      b_quote = (name.length() >= 2 && (name[0] == '\'' || name[0] == '\"') &&
                 name[0] == name[name.length() - 1] &&
                 name.rfind(name[0], name.length() - 2) == 0);
      break;
    }
  }
  if (!b_quote && (str[i_name2] == ':' || str[i_name2] == '!' ||
                   str[i_name2] == '<' || str[i_name2] == '>'))
    return false;
  bool b_func = false;
  if (!b_quote && i_name2 - i_name1 >= 2 &&
      str[i_name2] == RIGHT_PARENTHESIS_CH) {
    i_name2 = str.find_last_not_of(SPACES, i_name2 - 1);
    if (i_name2 == string::npos || i_name2 == 0 ||
        str[i_name2] != LEFT_PARENTHESIS_CH)
      return false;
    i_name2 = str.find_last_not_of(SPACES, i_name2 - 1);
    if (i_name2 == string::npos)
      return false;
  ...
  if (!b_quote && !CALCULATOR->variableNameIsValid(
                      str.substr(i_name1, i_name2 - i_name1 + 1)))
    return false;
  string name = str.substr(i_name1, i_name2 - i_name1 + 1);
  if (!b_quote) {
    ...
    if (!b_quote && name.length() > 1 && name[name.length() - 1] == 'x') {
      MathFunction *f =
          CALCULATOR->getActiveFunction(name.substr(0, name.length() - 1));
      if (f && f->minargs() <= 1 && f->maxargs() != 0)
        return false;
      if (!f &&
          CALCULATOR->getActiveVariable(name.substr(0, name.length() - 1)))
        return false;
    }
  }
  ...
  if (i2 != string::npos && str.rfind("#", i2 - 1) == string::npos) {
    ...
    if (i2 != string::npos) {
      ...
      ExpressionItem *item2 =
          item1 ? CALCULATOR->getActiveExpressionItem(name, item1) : NULL;
      if (item1) {
        MathStructure mtest;
        CALCULATOR->beginTemporaryStopMessages();
        CALCULATOR->parse(&mtest, str.substr(i + 1, str.length() - (i + 1)),
                          po);
        CALCULATOR->endTemporaryStopMessages();
        if (!b_func && item1->type() == TYPE_VARIABLE &&
            !((Variable *)item1)->isKnown() &&
            mtest.contains((Variable *)item1, true, true, false))
          return false;
        else if (!b_func && item1->type() == TYPE_UNIT &&
                 mtest.contains((Unit *)item1, true, true, false))
          return false;
        else if (b_func && item1->type() == TYPE_FUNCTION &&
                 mtest.containsFunction((MathFunction *)item1, true, true,
                                        false))
          return false;
        if (!b_func && item2 && item2->type() == TYPE_VARIABLE &&
            !((Variable *)item2)->isKnown() &&
            mtest.contains((Variable *)item2, true, true, false))
          return false;
        else if (!b_func && item2 && item2->type() == TYPE_UNIT &&
                 mtest.contains((Unit *)item2, true, true, false))
          return false;
        else if (b_func && item2 && item2->type() == TYPE_FUNCTION &&
                 mtest.containsFunction((MathFunction *)item2, true, true,
                                        false))
          return false;
      }
    }
...
string Calculator::calculateAndPrint(string str, int msecs,
                                     const EvaluationOptions &eo,
                                     const PrintOptions &po) {
  return calculateAndPrint(str, msecs, eo, po, AUTOMATIC_FRACTION_OFF,
                           AUTOMATIC_APPROXIMATION_OFF, NULL, -1, NULL, false,
                           0, TAG_TYPE_HTML);
}
string Calculator::calculateAndPrint(string str, int msecs,
                                     const EvaluationOptions &eo,
  ...
  return calculateAndPrint(str, msecs, eo, po, AUTOMATIC_FRACTION_OFF,
                           AUTOMATIC_APPROXIMATION_OFF, parsed_expression, -1,
                           NULL, false, 0, TAG_TYPE_HTML);
}
string Calculator::calculateAndPrint(
    string str, int msecs, const EvaluationOptions &eo, const PrintOptions &po,
    AutomaticFractionFormat auto_fraction, AutomaticApproximation auto_approx,
    std::string *parsed_expression, int max_length,
    bool *result_is_comparison) {
  return calculateAndPrint(str, msecs, eo, po, AUTOMATIC_FRACTION_OFF,
                           AUTOMATIC_APPROXIMATION_OFF, parsed_expression,
                           max_length, result_is_comparison, false, 0,
                           TAG_TYPE_HTML);
}

long int get_fixed_denominator2(const string &str, NumberFractionFormat &nff,
                                bool b_minus, int frac) {
  long int fden = 0;
  if ((frac > 0 &&
       EQUALS_IGNORECASE_AND_LOCAL(str, "fraction", _("fraction"))) ||
      (frac == 2 && str == "frac")) {
    fden = -1;
    if (b_minus)
      nff = FRACTION_FRACTIONAL;
    else
      nff = FRACTION_COMBINED;
  } else {
    if (str.length() > 2 && str[0] == '1' && str[1] == '/' &&
        str.find_first_not_of(NUMBERS SPACES, 2) == string::npos) {
      fden = s2i(str.substr(2, str.length() - 2));
    } else if (str.length() > 1 && str[0] == '/' &&
               str.find_first_not_of(NUMBERS SPACES, 1) == string::npos) {
      ...
      fden = 2;
               ...
               str.find("ths", str.length() - 3) != string::npos &&
               str.find_first_not_of(NUMBERS SPACES) == str.length() - 3) {
      fden = s2i(str.substr(0, str.length() - 3));
    }
    if (fden > 1) {
      if (b_minus)
        nff = FRACTION_FRACTIONAL_FIXED_DENOMINATOR;
      else
        nff = FRACTION_COMBINED_FIXED_DENOMINATOR;
    }
  }
  return fden;
}
long int get_fixed_denominator(const string &str, NumberFractionFormat &nff,
                               int frac, bool *has_sign) {
  size_t n = 0;
  if (str[0] == '-' || str[0] == '+')
    n = 1;
  if (has_sign)
    *has_sign = (n > 0);
  if (n > 0)
    return get_fixed_denominator2(str.substr(n, str.length() - n), nff,
                                  str[0] == '-', frac);
...
string Calculator::calculateAndPrint(
    string str, int msecs, const EvaluationOptions &eo, const PrintOptions &po,
    AutomaticFractionFormat auto_fraction, AutomaticApproximation auto_approx,
    std::string *parsed_expression, int max_length, bool *result_is_comparison,
    bool format, int colorize, int tagtype) {
  ...
  if (auto_fraction != AUTOMATIC_FRACTION_OFF)
    printops.number_fraction_format = FRACTION_DECIMAL;
  if (auto_approx != AUTOMATIC_APPROXIMATION_OFF)
    evalops.approximation = APPROXIMATION_TRY_EXACT;

  MathStructure mstruct;
  bool do_bases = false, do_factors = false, do_pfe = false,
       do_calendars = false, do_expand = false, do_binary_prefixes = false,
    ...
    while (true) {
      ...
      if (equalsIgnoreCase(to_str, "hex") ||
        ...
        printops.base = BASE_BINARY;
                 ...
                 EQUALS_IGNORECASE_AND_LOCAL(to_str, "decimal", _("decimal"))) {
      ...
      } else if (equalsIgnoreCase(to_str, "binary32") ||
                 equalsIgnoreCase(to_str, "float") ||
                 equalsIgnoreCase(to_str, "fp32")) {
        printops.base = BASE_FP32;
      } else if (equalsIgnoreCase(to_str, "binary64") ||
                 equalsIgnoreCase(to_str, "double") ||
                 equalsIgnoreCase(to_str, "fp64")) {
        printops.base = BASE_FP64;
      } else if (equalsIgnoreCase(to_str, "binary16") ||
                 equalsIgnoreCase(to_str, "fp16")) {
        printops.base = BASE_FP16;
      } else if (equalsIgnoreCase(to_str, "fp80")) {
        printops.base = BASE_FP80;
      } else if (equalsIgnoreCase(to_str, "binary128") ||
                 equalsIgnoreCase(to_str, "fp128")) {
        ...
        evalops.complex_number_form = COMPLEX_NUMBER_FORM_CIS;
                 ...
                 equalsIgnoreCase(to_str, "factor")) {
        ...
        // decimal fraction
      } else if (EQUALS_IGNORECASE_AND_LOCAL(to_str, "decimals",
                                             _("decimals"))) {
        ...
        long int fden =
            get_fixed_denominator(to_str2, nff, 1, &fixed_fraction_has_sign);
        if (fden != 0) {
          auto_fraction = AUTOMATIC_FRACTION_OFF;
          printops.restrict_fraction_length = false;
          printops.number_fraction_format = nff;
          if (fden > 1)
            priv->fixed_denominator = fden;
        } else {
          // ? in front of unit expression is interpreted as a request for the
          // optimal prefix.
          evalops.parse_options.units_enabled = true;
          if (to_str[0] == '?' || (to_str.length() > 1 && to_str[1] == '?' &&
                                   (to_str[0] == 'a' || to_str[0] == 'd'))) {
            printops.use_unit_prefixes = true;
            printops.use_prefixes_for_currencies = true;
            printops.use_prefixes_for_all_units = true;
            if (to_str[0] == 'a')
          ...
          if (delay_complex != (cnf != COMPLEX_NUMBER_FORM_POLAR &&
                                cnf != COMPLEX_NUMBER_FORM_CIS) &&
              u && u->baseUnit() == getRadUnit() && u->baseExponent() == 1)
  ...
  if (!had_to_expression &&
      ...
      ((parsed_struct.isMultiplication() && parsed_struct.size() == 2 &&
        parsed_struct[0].isNumber() && parsed_struct[1].isUnit_exp() &&
        parsed_struct.equals(mstruct)) ||
       (parsed_struct.isNegate() && parsed_struct[0].isMultiplication() &&
        parsed_struct[0].size() == 2 && parsed_struct[0][0].isNumber() &&
        parsed_struct[0][1].isUnit_exp() && mstruct.isMultiplication() &&
        mstruct.size() == 2 && mstruct[1] == parsed_struct[0][1] &&
        mstruct[0].isNumber() &&
        parsed_struct[0][0].number() == -mstruct[0].number()) ||
       (parsed_struct.isUnit_exp() && parsed_struct.equals(mstruct)))) {
    Unit *u = NULL;
    ...
    if (munit->isUnit())
      ...
      u = (*munit)[0].unit();
    if (u && u->isCurrency()) {
      if (evalops.local_currency_conversion && getLocalCurrency() &&
          u != getLocalCurrency()) {
      ...
      if (evalops.auto_post_conversion == POST_CONVERSION_OPTIMAL) {
        if (munit->isUnit() && u->referenceName() == "oF") {
          u = getActiveUnit("oC");
          if (u)
            mstruct.set(convert(mstruct, u, evalops, true, false));
      ...
      if (evalops.approximation == APPROXIMATION_EXACT &&
        ...
        if (evalops.auto_post_conversion == POST_CONVERSION_BASE)
          mstruct.set(convertToBaseUnits(mstruct, evalops));
        else
          mstruct.set(convertToOptimalUnit(mstruct, evalops, true));
      }
    }
  ...
  mstruct_exact.setUndefined();
  if (!aborted() && (!do_calendars || !mstruct.isDateTime()) && !do_bases &&
      !units_changed && auto_approx != AUTOMATIC_APPROXIMATION_OFF &&
      (auto_approx == AUTOMATIC_APPROXIMATION_DUAL ||
       printops.base == BASE_DECIMAL)) {
    bool b = true;
    if (msecs > 0) {
      long int usec, sec;
    ...
    if (b) {
      if (delay_complex)
        evalops.complex_number_form = COMPLEX_NUMBER_FORM_RECTANGULAR;
      calculate_dual_exact(mstruct_exact, &mstruct, str, &parsed_struct,
                           evalops, auto_approx, 0, max_length);
      if (aborted())
        mstruct_exact.setUndefined();
  ...
  printops.allow_factorization = printops.allow_factorization ||
                                 evalops.structuring == STRUCTURING_FACTORIZE ||
                                 do_factors;
  ...
  if (!printops.is_approximate)
    printops.is_approximate = &tmp_approx;

  if (do_calendars && mstruct.isDateTime()) {
    // handle "to calendars"
    bool b_fail;
    long int y, m, d;
#define PRINT_CALENDAR(x, c)                                                   \
  if (!result.empty()) {                                                       \
    result += "\n";                                                            \
  }                                                                            \
  result += x;                                                                 \
  result += " ";                                                               \
  b_fail = !dateToCalendar(*mstruct.datetime(), y, m, d, c);                   \
  if (b_fail) {                                                                \
    result += _("failed");                                                     \
  } else {                                                                     \
    result += i2s(d);                                                          \
    result += " ";                                                             \
    result += monthName(m, c, true);                                           \
    result += " ";                                                             \
    result += i2s(y);                                                          \
  }
    ...
    if (!b_fail) {
      result += " (";
      result += chineseStemName(st);
      result += string(" ");
      result += chineseBranchName(br);
      result += ")";
    }
    ...
    // b? in front of unit expression: use binary prefixes
    printops.use_unit_prefixes = true;
    // check for information units in unit expression: if found use only binary
    // prefixes for information units
  ...
  if (result_is_comparison)
    *result_is_comparison = false;

  if (auto_fraction != AUTOMATIC_FRACTION_OFF ||
      auto_approx != AUTOMATIC_APPROXIMATION_OFF) {
    print_dual(mstruct, str, parsed_struct, mstruct_exact, result, alt_results,
               printops, evalops, auto_fraction, auto_approx,
               complex_angle_form, &exact_comparison, true, format, colorize,
               tagtype, max_length, had_to_expression);
    if (!alt_results.empty()) {
      bool use_par = mstruct.isComparison() || mstruct.isLogicalAnd() ||
                     mstruct.isLogicalOr();
      str = result;
      result = "";
      size_t equals_length = 3;
      if (!printops.use_unicode_signs &&
          (mstruct.isApproximate() || *printops.is_approximate))
        equals_length += 1 + strlen(_("approx."));
      for (size_t i = 0; i < alt_results.size();) {
        if (max_length > 0 && unicode_length(str) + unicode_length(result) +
                                      unicode_length(alt_results[i]) +
                                      equals_length >
                                  (size_t)max_length) {
    ...
    } else if (mstruct.isComparison() || mstruct.isLogicalOr() ||
               mstruct.isLogicalAnd()) {
      if (result_is_comparison) {
    ...
    if (po.spell_out_logical_operators &&
        test_parsed_comparison(parsed_struct)) {
      if (mstruct.isZero()) {
    ...
    if (printops.base == BASE_TIME) {
      bool b = false;
      if (mstruct.isUnit() &&
          mstruct.unit()->baseUnit()->referenceName() == "s") {
        b = true;
      } else if (mstruct.isMultiplication() && mstruct.size() == 2 &&
                 mstruct[0].isNumber() && mstruct[1].isUnit() &&
                 mstruct[1].unit()->baseUnit()->referenceName() == "s") {
        b = true;
      } else if (mstruct.isAddition() && mstruct.size() > 0) {
        b = true;
        for (size_t i = 0; i < mstruct.size(); i++) {
          if (mstruct[i].isUnit() &&
              mstruct[i].unit()->baseUnit()->referenceName() == "s") {
          } else if (mstruct[i].isMultiplication() && mstruct[i].size() == 2 &&
                     mstruct[i][0].isNumber() && mstruct[i][1].isUnit() &&
                     mstruct[i][1].unit()->baseUnit()->referenceName() == "s") {
          } else {
            b = false;
            break;
          }
    ...
    if (result_is_comparison &&
        (mstruct.isComparison() || mstruct.isLogicalOr() ||
         mstruct.isLogicalAnd()))
      *result_is_comparison = true;
...
after_print:
  ...
  if (parsed_expression) {
    ...
    po_parsed.hexadecimal_twos_complement =
        printops.hexadecimal_twos_complement;
    po_parsed.base = evalops.parse_options.base;
    Number nr_base;
    if (po_parsed.base == BASE_CUSTOM &&
        (usesIntervalArithmetic() || customInputBase().isRational()) &&
        (customInputBase().isInteger() || !customInputBase().isNegative()) &&
        (customInputBase() > 1 || customInputBase() < -1)) {
      nr_base = customOutputBase();
      setCustomOutputBase(customInputBase());
    } else if (po_parsed.base == BASE_CUSTOM ||
               (po_parsed.base < BASE_CUSTOM && !usesIntervalArithmetic() &&
                po_parsed.base != BASE_UNICODE)) {
      po_parsed.base = 10;
      po_parsed.min_exp = 6;
    ...
    po_parsed.spell_out_logical_operators =
    ...
    *parsed_expression =
        parsed_struct.print(po_parsed, format, colorize, tagtype);
  }
  if (msecs > 0)
    stopControl();

  printops.is_approximate = save_is_approximate;
  if (po.is_approximate && (exact_comparison || !alt_results.empty()))
    *po.is_approximate = false;
  else if (po.is_approximate && mstruct.isApproximate())
    *po.is_approximate = true;

  return result;
}
bool Calculator::calculate(MathStructure *mstruct, string str, int msecs,
                           const EvaluationOptions &eo,
                           MathStructure *parsed_struct,
                           MathStructure *to_struct, bool make_to_division) {
...
bool Calculator::calculate(MathStructure *mstruct, int msecs,
                           const EvaluationOptions &eo, string to_str) {
  ...
  if (!to_str.empty())
    tmp_tostruct = new MathStructure(to_str);
  else
    tmp_tostruct = NULL;
...
bool Calculator::hasToExpression(const string &str,
                                 bool allow_empty_from) const {
  if (str.empty())
    return false;
  size_t i = str.rfind("->");
  if (i != string::npos && (allow_empty_from || i > 0))
    return true;
  i = str.rfind("→");
  if (i != string::npos && (allow_empty_from || i > 0))
    return true;
  ...
  while (true) {
    ...
    if (i == string::npos || i >= str.length() - 2)
      break;
    if ((unsigned char)str[i + 2] >= 0x94 && (unsigned char)str[i + 2] <= 0xbf)
      return true;
  }
  i = allow_empty_from ? 0 : 1;
  size_t i2 = i;
  ...
  while (true) {
    i2 = str.find(_("to"), i);
    i = str.find("to", i);
    if (i2 != string::npos && (i == string::npos || i2 < i)) {
      l = strlen(_("to"));
      i = i2;
    } else
      l = 2;
    ...
    if (((i > 0 && is_in(SPACES, str[i - 1])) ||
         (allow_empty_from && i == 0)) &&
        i + l < str.length() && is_in(SPACES, str[i + l]))
      return true;
    i++;
  }
  return false;
}
bool Calculator::hasToExpression(const string &str, bool allow_empty_from,
                                 const EvaluationOptions &eo) const {
  if (eo.parse_options.base == BASE_UNICODE ||
      (eo.parse_options.base == BASE_CUSTOM && priv->custom_input_base_i > 62))
    return false;
  if (str.empty())
    return false;
  size_t i = str.rfind("->");
  ...
  while (true) {
    ...
    if (i == string::npos || i >= str.length() - 2)
      break;
    if ((unsigned char)str[i + 2] >= 148 && (unsigned char)str[i + 2] <= 191)
      return true;
  }
  i = allow_empty_from ? 0 : 1;
  size_t i2 = i;
...
bool Calculator::separateToExpression(string &str, string &to_str,
                                      const EvaluationOptions &eo,
                                      bool keep_modifiers,
                                      bool allow_empty_from) const {
  if (eo.parse_options.base == BASE_UNICODE ||
  ...
  while (true) {
    // dingbat arrows
    i = str.find("\xe2\x9e", i);
    if (i == string::npos || (i_arrow != string::npos && i > i_arrow) ||
        i >= str.length() - 2)
      break;
    if ((unsigned char)str[i + 2] >= 148 && (unsigned char)str[i + 2] <= 191) {
      i_arrow = i;
  ...
  while (true) {
    ...
    if (i_arrow != string::npos && (i == string::npos || i_arrow < i)) {
      l = l_arrow;
      i = i_arrow;
      b_arrow = true;
    }
    if (i == string::npos)
      break;
    if (b_arrow || (((i > 0 && is_in(SPACES, str[i - 1])) ||
                     (allow_empty_from && i == 0)) &&
                    i + l < str.length() && is_in(SPACES, str[i + l]))) {
      to_str = str.substr(i + l, str.length() - i - l);
      if (!b_arrow && to_str.empty())
        return false;
      remove_blank_ends(to_str);
      if (!to_str.empty()) {
        ...
        if (!keep_modifiers && (to_str[0] == '0' || to_str[0] == '?' ||
                                to_str[0] == '+' || to_str[0] == '-')) {
          to_str = to_str.substr(1, to_str.length() - 1);
          remove_blank_ends(to_str);
        } else if (!keep_modifiers && to_str.length() > 1 && to_str[1] == '?' &&
                   (to_str[0] == 'b' || to_str[0] == 'a' || to_str[0] == 'd')) {
...
bool Calculator::hasWhereExpression(const string &str,
                                    const EvaluationOptions &eo) const {
  if (eo.parse_options.base == BASE_UNICODE ||
      (eo.parse_options.base == BASE_CUSTOM && priv->custom_input_base_i > 62))
    return false;
  ...
  while (i != 0) {
    ...
    if (i2 != string::npos && (i == string::npos || i < i2)) {
      l = strlen(_("where"));
      i = i2;
    } else
      l = 5;
    if (i == string::npos)
      break;
    if (i > 0 && is_in(SPACES, str[i - 1]) && i + l < str.length() &&
        is_in(SPACES, str[i + l]))
      return true;
  }
  if ((i = str.rfind("/.", str.length() - 2)) != string::npos &&
      eo.parse_options.base >= 2 && eo.parse_options.base <= 10 &&
      (str[i + 2] < '0' || str[i + 2] > '9'))
    return true;
  return false;
}
bool Calculator::separateWhereExpression(string &str, string &to_str,
                                         const EvaluationOptions &eo) const {
  if (eo.parse_options.base == BASE_UNICODE ||
      (eo.parse_options.base == BASE_CUSTOM && priv->custom_input_base_i > 62))
    return false;
  ...
  size_t i = 0;
  if ((i = str.rfind("/.", str.length() - 2)) != string::npos &&
      i != str.length() - 2 && eo.parse_options.base >= 2 &&
      eo.parse_options.base <= 10 && (str[i + 2] < '0' || str[i + 2] > '9')) {
...
bool calculate_rand(MathStructure &mstruct, const EvaluationOptions &eo) {
  if (mstruct.isFunction() &&
      (mstruct.function()->id() == FUNCTION_ID_RAND ||
       mstruct.function()->id() == FUNCTION_ID_RANDN ||
       mstruct.function()->id() == FUNCTION_ID_RAND_POISSON)) {
    mstruct.unformat(eo);
...
bool calculate_ans(MathStructure &mstruct, const EvaluationOptions &eo) {
  if (mstruct.isFunction() && (mstruct.function()->hasName("answer") ||
                               mstruct.function()->hasName("expression"))) {
    mstruct.unformat(eo);
    mstruct.calculateFunctions(eo, false);
    return true;
  } else if (mstruct.isVariable() && mstruct.variable()->isKnown() &&
             (mstruct.variable()->referenceName() == "ans" ||
              (mstruct.variable()->referenceName().length() == 4 &&
               mstruct.variable()->referenceName().substr(0, 3) == "ans" &&
               is_in(NUMBERS, mstruct.variable()->referenceName()[3])))) {
...
bool handle_where_expression(MathStructure &m, MathStructure &mstruct,
                             const EvaluationOptions &eo,
                             vector<UnknownVariable *> &vars,
                             vector<MathStructure> &varms, bool empty_func,
                             bool do_eval = true) {
  if (m.isComparison()) {
    if (m.comparisonType() == COMPARISON_EQUALS) {
      // x=y
      if (m[0].size() > 0 && do_eval) {
        // not a single variable (or empty function) on the left side of the
        // comparison: perform calculation
        MathStructure m2(m);
        MathStructure xvar = m[0].find_x_var();
        EvaluationOptions eo2 = eo;
        eo2.isolate_x = true;
        if (!xvar.isUndefined())
          eo2.isolate_var = &xvar;
        m2.eval(eo2);
        if (m2.isComparison())
          return handle_where_expression(m2, mstruct, eo, vars, varms, false,
                                         false);
      }
      if (m[0].isFunction() && m[1].isFunction() &&
          (m[0].size() == 0 ||
           (empty_func && m[0].function()->minargs() == 0)) &&
          (m[1].size() == 0 ||
           (empty_func && m[1].function()->minargs() == 0))) {
        // if left value is a function without any arguments, do function
        // replacement
        if (!replace_function(mstruct, m[0].function(), m[1].function(), eo))
          CALCULATOR->error(false, _("Original value (%s) was not found."),
                            (m[0].function()->name() + "()").c_str(), NULL);
      } else {
        if (mstruct.countOccurrences(m[0], true) > 1) {
          ...
          if (m[1].containsInterval(true, false, false, 0, true)) {
            // replace interval with variable if multiple occurrences if
            // original value
            MathStructure mv(m[1]);
            replace_f_interval(mv, eo);
            replace_intervals_f(mv);
            if (!mstruct.replace(m[0], mv, false, false, true))
              CALCULATOR->error(false, _("Original value (%s) was not found."),
                                format_and_print(m[0]).c_str(), NULL);
          } else {
            if (!mstruct.replace(m[0], m[1], false, false, true))
              CALCULATOR->error(false, _("Original value (%s) was not found."),
                                format_and_print(m[0]).c_str(), NULL);
          }
        } else {
      ...
      return true;
    } else if (m[0].isSymbolic() ||
               (m[0].isVariable() && !m[0].variable()->isKnown())) {
      // inequality found
          ...
          if (ass) {
            ...
            if (m.comparisonType() == COMPARISON_EQUALS_GREATER) {
              ...
              if (!ass->max() || (*ass->max() > m[1].number())) {
                ass->setMax(&m[1].number());
                ass->setIncludeEqualsMax(true);
                return true;
              ...
              if (!ass->min() ||
                  (ass->includeEqualsMin() && *ass->min() <= m[1].number()) ||
                  (!ass->includeEqualsMin() && *ass->min() < m[1].number())) {
                ass->setMin(&m[1].number());
                ass->setIncludeEqualsMin(false);
                return true;
              } else if ((ass->includeEqualsMin() &&
                          *ass->min() > m[1].number()) ||
                         (!ass->includeEqualsMin() &&
                          *ass->min() >= m[1].number())) {
                return true;
              }
            } else if (m.comparisonType() == COMPARISON_LESS) {
              if (!ass->max() ||
                  (ass->includeEqualsMax() && *ass->max() >= m[1].number()) ||
                  (!ass->includeEqualsMax() && *ass->max() > m[1].number())) {
                ass->setMax(&m[1].number());
                ass->setIncludeEqualsMax(false);
                return true;
              } else if ((ass->includeEqualsMax() &&
                          *ass->max() < m[1].number()) ||
                         (!ass->includeEqualsMax() &&
                          *ass->max() <= m[1].number())) {
                return true;
              }
            }
            ...
            UnknownVariable *var =
                new UnknownVariable("", format_and_print(m[0]));
            ass = new Assumptions();
            if (m[1].isZero()) {
              if (m.comparisonType() == COMPARISON_EQUALS_GREATER)
                ass->setSign(ASSUMPTION_SIGN_NONNEGATIVE);
              else if (m.comparisonType() == COMPARISON_EQUALS_LESS)
                ass->setSign(ASSUMPTION_SIGN_NONPOSITIVE);
              else if (m.comparisonType() == COMPARISON_GREATER)
                ass->setSign(ASSUMPTION_SIGN_POSITIVE);
              else if (m.comparisonType() == COMPARISON_LESS)
                ass->setSign(ASSUMPTION_SIGN_NEGATIVE);
              else if (m.comparisonType() == COMPARISON_NOT_EQUALS)
                ass->setSign(ASSUMPTION_SIGN_NONZERO);
            } else {
              if (m.comparisonType() == COMPARISON_EQUALS_GREATER) {
                ass->setMin(&m[1].number());
                ass->setIncludeEqualsMin(true);
            ...
            if (!mstruct.replace(m[0], u_var, false, false, true))
              CALCULATOR->error(false, _("Original value (%s) was not found."),
                                format_and_print(m[0]).c_str(), NULL);
            return true;
          }
      ...
      // inequality without a single variable on the left side: calculate
    ...
    bool ret = true;
    for (size_t i = 0; i < m.size(); i++) {
      if (!handle_where_expression(m[i], mstruct, eo, vars, varms, empty_func,
                                   do_eval))
        ret = false;
    }
    return ret;
  }
  CALCULATOR->error(true, _("Unhandled \"where\" expression: %s"),
                    format_and_print(m).c_str(), NULL);
  return false;
}
void replace_unregistered_variables(MathStructure &m) {
  if (m.isVariable() && m.variable()->isKnown() &&
      !m.variable()->isRegistered()) {
...
MathStructure Calculator::calculate(string str, const EvaluationOptions &eo,
  ...
  if (!str_where.empty()) {
    ...
    // any replacements from "where" expression
  ...
  if (!aborted()) {
    ...
    if (u) {
      ...
      if (provided_to) {
        mstruct.set(
            convert(mstruct, str2, eo, to_struct, false, parsed_struct));
      } else {
        string str2b;
        while (true) {
          ...
          if (to_struct && !to_struct->isUndefined()) {
            ...
            if (!mto.isUndefined())
              to_struct->multiply(mto, true);
          } else {
            mstruct.set(
                convert(mstruct, str2, eo, to_struct, true, parsed_struct));
          }
          if (str2b.empty())
            break;
          str2 = str2b;
  ...
  if (aborted())
    replace_unregistered_variables(mstruct);
...
string Calculator::print(const MathStructure &mstruct, int msecs,
                         const PrintOptions &po) {
  startControl(msecs);
...
string Calculator::print(const MathStructure &mstruct, int msecs,
                         const PrintOptions &po, bool format, int colorize,
                         int tagtype) {
  startControl(msecs);
  MathStructure mstruct2(mstruct);
  mstruct2.format(po);
...
void Calculator::abortPrint() { abort(); }
bool Calculator::printingAborted() { return aborted(); }
string Calculator::printingAbortedMessage() const { return abortedMessage(); }
string Calculator::timedOutString() const { return _("timed out"); }
bool Calculator::printingControlled() const { return isControlled(); }
void Calculator::stopPrintControl() { stopControl(); }
...
bool Calculator::aborted() {
  ...
  if (i_timeout > 0) {
    ...
    struct timeval tv;
    gettimeofday(&tv, NULL);
    if (tv.tv_sec > t_end.tv_sec ||
        (tv.tv_sec == t_end.tv_sec && tv.tv_usec > t_end.tv_usec)) {
#else
    struct timespec tv;
    clock_gettime(CLOCK_MONOTONIC, &tv);
    if (tv.tv_sec > t_end.tv_sec ||
        (tv.tv_sec == t_end.tv_sec && tv.tv_nsec / 1000 > t_end.tv_usec)) {
#endif
      i_aborted = 2;
      return true;
    }
...
bool Calculator::isControlled() const { return b_controlled; }
