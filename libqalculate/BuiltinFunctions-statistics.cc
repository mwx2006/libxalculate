...
int TotalFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
...
int PercentileFunction::calculate(MathStructure &mstruct,
  ...
  if (!v.sortVector()) {
    ...
    case 6: {
      pfr *= (long int)v.countChildren() + 1;
      break;
    }
    case 7: {
      pfr *= (long int)v.countChildren() - 1;
    ...
    case 9: {
      pfr *= Number(v.countChildren() * 4 + 1, 4);
      pfr += Number(3, 8);
      break;
    }
    case 8: {
    ...
    default: {
      pfr *= Number(v.countChildren() * 3 + 1, 3);
      pfr += Number(1, 3);
      break;
    }
    }
...
int MinFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                           const EvaluationOptions &) {
    ...
    if (min == NULL) {
      ...
      if (cmp == COMPARISON_RESULT_LESS) {
        ...
        if (CALCULATOR->showArgumentErrors()) {
          CALCULATOR->error(true, _("Unsolvable comparison in %s()."),
                            name().c_str(), NULL);
        }
...
int MaxFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                           const EvaluationOptions &) {
  ComparisonResult cmp;
...
int ModeFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                            const EvaluationOptions &) {
  if (vargs[0].size() <= 0) {
    return 0;
  }
  ...
  for (size_t index = 0; index < is.size(); index++) {
    if (is[index] > n ||
        (is[index] == n &&
         comparison_is_equal_or_less(value->compare(*vargs_nodup[index])))) {
      n = is[index];
...
RandFunction::RandFunction() : MathFunction("rand", 0, 2) {
  ...
  setDefaultValue(1, "0");
  IntegerArgument *iarg = new IntegerArgument("", ARGUMENT_MIN_MAX_POSITIVE,
...
int RandFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                            const EvaluationOptions &) {
  size_t n = (size_t)vargs[1].number().uintValue();
  if (n > 1) {
    mstruct.clearVector();
    mstruct.resizeVector(n, m_zero);
  }
  Number nr;
  for (size_t i = 0; i < n; i++) {
    if (n > 1 && CALCULATOR->aborted())
      return 0;
...
bool RandFunction::representsReal(const MathStructure &, bool) const {
  return true;
}
bool RandFunction::representsInteger(const MathStructure &vargs, bool) const {
  return vargs.size() > 0 && vargs[0].isNumber() &&
         vargs[0].number().isPositive();
}
bool RandFunction::representsNonNegative(const MathStructure &, bool) const {
  return true;
}
...
int RandnFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                             const EvaluationOptions &) {
...
bool RandnFunction::representsReal(const MathStructure &, bool) const {
  return true;
}
bool RandnFunction::representsNonComplex(const MathStructure &, bool) const {
  return true;
}
bool RandnFunction::representsNumber(const MathStructure &, bool) const {
  return true;
}
...
int RandPoissonFunction::calculate(MathStructure &mstruct,
                                   const MathStructure &vargs,
                                   const EvaluationOptions &) {
  size_t n = (size_t)vargs[1].number().uintValue();
  if (n > 1) {
...
bool RandPoissonFunction::representsReal(const MathStructure &, bool) const {
  return true;
}
bool RandPoissonFunction::representsInteger(const MathStructure &vargs,
                                            bool) const {
  return true;
}
bool RandPoissonFunction::representsNonNegative(const MathStructure &,
                                                bool) const {
  return true;
}
