...
Unit::Unit(string cat_, string name_, string plural_, string singular_,
           string title_, bool is_local, bool is_builtin, bool is_active)
    : ExpressionItem(cat_, "", title_, "", is_local, is_builtin, is_active) {
  ...
  if (!name_.empty()) {
    ...
    if (i != string::npos && i > 0 && i < name_.length() - 1 &&
        name_.find('_', i + 1) == string::npos)
      names[0].suffix = true;
    else
  ...
  if (!singular_.empty()) {
    ...
    names[names.size() - 1].case_sensitive =
        text_length_is_one(names[names.size() - 1].name);
...
Unit::Unit(const Unit *unit) { set(unit); }
Unit::~Unit() {}
ExpressionItem *Unit::copy() const { return new Unit(this); }
...
bool Unit::isSIUnit() const { return b_si; }
...
const string &Unit::system() const { return ssystem; }
bool Unit::useWithPrefixesByDefault() const { return b_use_with_prefixes % 2; }
...
void Unit::setUseWithPrefixesByDefault(bool use_with_prefixes) {
  b_use_with_prefixes = (use_with_prefixes ? 1 : 0) +
                        (b_use_with_prefixes - (b_use_with_prefixes % 2));
}
void Unit::setMaxPreferredPrefix(int exp) {
  ...
  b_use_with_prefixes = (b_use_with_prefixes % 2) + (exp * 2) +
                        (b_use_with_prefixes - (b_use_with_prefixes % 62));
}
void Unit::setMinPreferredPrefix(int exp) {
  ...
  b_use_with_prefixes = (b_use_with_prefixes % 62) + (exp * 62) +
                        (b_use_with_prefixes - (b_use_with_prefixes % 1922));
}
...
string Unit::print(const PrintOptions &po, bool format, int tagtype, bool input,
                   bool plural) const {
  if (input) {
    preferredInputName(po.abbreviate_names, po.use_unicode_signs, plural,
                       po.use_reference_names ||
                           (po.preserve_format && isCurrency()),
                       po.can_display_unicode_string_function,
                       po.can_display_unicode_string_arg)
        .formattedName(
            TYPE_UNIT, !po.use_reference_names,
            format && tagtype == TAG_TYPE_HTML,
  ...
  return preferredDisplayName(po.abbreviate_names, po.use_unicode_signs, plural,
                              po.use_reference_names ||
                                  (po.preserve_format && isCurrency()),
                              po.can_display_unicode_string_function,
                              po.can_display_unicode_string_arg)
...
string Unit::print(bool plural_, bool short_, bool use_unicode,
                   bool (*can_display_unicode_string_function)(const char *,
                                                               void *),
                   void *can_display_unicode_string_arg) const {
  return preferredName(short_, use_unicode, plural_, false,
                       can_display_unicode_string_function,
                       can_display_unicode_string_arg)
      .name;
}
const string &
Unit::plural(bool return_singular_if_no_plural, bool use_unicode,
             bool (*can_display_unicode_string_function)(const char *, void *),
             void *can_display_unicode_string_arg) const {
  const ExpressionName *ename = &preferredName(
      false, use_unicode, true, false, can_display_unicode_string_function,
      can_display_unicode_string_arg);
  if (!return_singular_if_no_plural && !ename->plural)
    return empty_string;
  return ename->name;
}
const string &Unit::singular(
    bool return_abbreviation_if_no_singular, bool use_unicode,
    bool (*can_display_unicode_string_function)(const char *, void *),
    void *can_display_unicode_string_arg) const {
  const ExpressionName *ename = &preferredName(
      false, use_unicode, false, false, can_display_unicode_string_function,
      can_display_unicode_string_arg);
  if (!return_abbreviation_if_no_singular && ename->abbreviation)
    return empty_string;
  return ename->name;
}
const string &Unit::abbreviation(
    bool return_singular_if_no_abbreviation, bool use_unicode,
    bool (*can_display_unicode_string_function)(const char *, void *),
    void *can_display_unicode_string_arg) const {
  const ExpressionName *ename = &preferredName(
      true, use_unicode, false, false, can_display_unicode_string_function,
      can_display_unicode_string_arg);
  if (!return_singular_if_no_abbreviation && !ename->abbreviation)
    return empty_string;
  return ename->name;
}
Unit *Unit::baseUnit() const { return (Unit *)this; }
MathStructure &Unit::convertToBaseUnit(MathStructure &mvalue,
                                       MathStructure &) const {
  return mvalue;
}
MathStructure &Unit::convertFromBaseUnit(MathStructure &mvalue,
                                         MathStructure &) const {
  return mvalue;
}
...
MathStructure Unit::convertToBaseUnit() const { return MathStructure(1, 1); }
MathStructure Unit::convertFromBaseUnit() const { return MathStructure(1, 1); }
int Unit::baseExponent(int exp) const { return exp; }
int Unit::type() const { return TYPE_UNIT; }
int Unit::subtype() const { return SUBTYPE_BASE_UNIT; }
bool Unit::isChildOf(Unit *) const { return false; }
...
bool Unit::hasNonlinearRelationTo(Unit *u) const {
  ...
  if (ub2 != this) {
    ...
    if (ub2->subtype() == SUBTYPE_COMPOSITE_UNIT) {
      if (u->hasNonlinearRelationTo(ub2) &&
          (((CompositeUnit *)ub2))->containsRelativeTo(baseUnit())) {
        return true;
      }
    }
    return false;
  ...
  while (true) {
    if (fbu == this)
      return false;
    if (fbu->subtype() != SUBTYPE_ALIAS_UNIT)
      return false;
    if (((AliasUnit *)fbu)->hasNonlinearExpression())
      return true;
    fbu = (Unit *)((AliasUnit *)fbu)->firstBaseUnit();
  }
}
bool Unit::hasApproximateRelationTo(
    Unit *u, bool check_variables, bool ignore_high_precision_intervals) const {
  if (u == this)
    return false;
  Unit *ub2 = u->baseUnit();
  if (ub2 != this) {
    if (subtype() == SUBTYPE_COMPOSITE_UNIT) {
      const CompositeUnit *cu = (CompositeUnit *)this;
      for (size_t i = 1; i <= cu->countUnits(); i++) {
        if (cu->get(i)->hasApproximateRelationTo(
                u, check_variables, ignore_high_precision_intervals))
          return true;
      }
      return false;
    }
    if (ub2->subtype() == SUBTYPE_COMPOSITE_UNIT) {
      if ((((CompositeUnit *)ub2))->containsRelativeTo(baseUnit()) &&
          u->hasApproximateRelationTo(ub2, check_variables,
                                      ignore_high_precision_intervals))
        return true;
    }
    return false;
  }
  ...
  while (true) {
    ...
    if (fbu->subtype() != SUBTYPE_ALIAS_UNIT)
      return false;
    if (((AliasUnit *)fbu)
            ->hasApproximateExpression(check_variables,
                                       ignore_high_precision_intervals))
      return true;
    fbu = (Unit *)((AliasUnit *)fbu)->firstBaseUnit();
  }
  ...
  if (u == this) {
    ...
    if (isCurrency() && u->isCurrency()) {
      ...
      if (u->subtype() == SUBTYPE_ALIAS_UNIT && u->isBuiltin()) {
        ...
        if (u == CALCULATOR->getUnitById(UNIT_ID_BTC) ||
            u_parent == CALCULATOR->getUnitById(UNIT_ID_BTC)) {
          if (u == CALCULATOR->getUnitById(UNIT_ID_BTC) ||
              this != CALCULATOR->getUnitById(UNIT_ID_BTC))
            i = i | 0b0010;
        } else if (u == CALCULATOR->getUnitById(UNIT_ID_BYN) ||
                   u_parent == CALCULATOR->getUnitById(UNIT_ID_BYN)) {
          if (u == CALCULATOR->getUnitById(UNIT_ID_BYN) ||
              this != CALCULATOR->getUnitById(UNIT_ID_BYN))
            i = i | 0b1000;
        } else if (u_parent == CALCULATOR->getUnitById(UNIT_ID_EURO) &&
                   !u->isHidden()) {
          if (subtype() != SUBTYPE_ALIAS_UNIT ||
              ((AliasUnit *)this)->firstBaseUnit() != u)
            i = i | 0b0001;
        } else {
          if (this == CALCULATOR->getUnitById(UNIT_ID_EURO))
            i = i | 0b0001;
          i = i | 0b0100;
        }
      }
      if (subtype() == SUBTYPE_ALIAS_UNIT && isBuiltin()) {
        ...
        if (this == CALCULATOR->getUnitById(UNIT_ID_BTC) ||
            u_parent == CALCULATOR->getUnitById(UNIT_ID_BTC)) {
          if (this == CALCULATOR->getUnitById(UNIT_ID_BTC) ||
              u != CALCULATOR->getUnitById(UNIT_ID_BTC)) {
            if (i & 0b0100)
              i = i | 0b0001;
            i = i | 0b0010;
          }
        } else if (this == CALCULATOR->getUnitById(UNIT_ID_BYN) ||
                   u_parent == CALCULATOR->getUnitById(UNIT_ID_BYN)) {
          if (this == CALCULATOR->getUnitById(UNIT_ID_BYN) ||
              u != CALCULATOR->getUnitById(UNIT_ID_BYN)) {
            if (i & 0b0100)
              i = i | 0b0001;
            i = i | 0b1000;
          }
          ...
          if (u->subtype() != SUBTYPE_ALIAS_UNIT ||
              ((AliasUnit *)u)->firstBaseUnit() != this)
            i = i | 0b0001;
        } else {
          if ((i & 0b1000) || (i & 0b0010) ||
              u == CALCULATOR->getUnitById(UNIT_ID_EURO))
            i = i | 0b0001;
          i = i | 0b0100;
        }
      }
...
AliasUnit::AliasUnit(string cat_, string name_, string plural_,
                     string short_name_, string title_, Unit *alias,
                     string relation, int exp, string inverse, bool is_local,
                     bool is_builtin, bool is_active)
    : Unit(cat_, name_, plural_, short_name_, title_, is_local, is_builtin,
           is_active) {
  o_unit = (Unit *)alias;
...
AliasUnit::AliasUnit(const AliasUnit *unit) { set(unit); }
AliasUnit::~AliasUnit() {}
ExpressionItem *AliasUnit::copy() const { return new AliasUnit(this); }
...
Unit *AliasUnit::baseUnit() const { return o_unit->baseUnit(); }
Unit *AliasUnit::firstBaseUnit() const { return o_unit; }
void AliasUnit::setBaseUnit(Unit *alias) {
  o_unit = (Unit *)alias;
  setChanged(true);
}
string AliasUnit::expression() const { return svalue; }
string AliasUnit::inverseExpression() const { return sinverse; }
string AliasUnit::uncertainty(bool *is_relative) const {
  if (is_relative)
    *is_relative = b_relative_uncertainty;
  return suncertainty;
}
...
MathStructure &AliasUnit::convertToBaseUnit(MathStructure &mvalue,
                                            MathStructure &mexp) const {
  convertToFirstBaseUnit(mvalue, mexp);
  return o_unit->convertToBaseUnit(mvalue, mexp);
}
MathStructure &AliasUnit::convertFromBaseUnit(MathStructure &mvalue,
                                              MathStructure &mexp) const {
...
MathStructure &AliasUnit::convertFromFirstBaseUnit(MathStructure &mvalue,
                                                   MathStructure &mexp) const {
  if (i_exp != 1)
    mexp /= i_exp;
  ParseOptions po;
  if (isApproximate() && suncertainty.empty() && precision() == -1) {
    if (sinverse.find(DOT) != string::npos || svalue.find(DOT) != string::npos)
  ...
  if (sinverse.empty()) {
    if (svalue.find("\\x") != string::npos) {
      ...
      if (precision() > 0 &&
          (mvalue.precision() < 0 || precision() < mvalue.precision()))
        mvalue.setPrecision(precision(), true);
      if (isApproximate())
        mvalue.setApproximate(true, true);
    } else {
      ...
      if (!suncertainty.empty()) {
        ...
        if (i != string::npos && i > 2 &&
            (i == svalue.length() - 1 ||
             (i < svalue.length() - 2 &&
              (svalue[i + 1] == 'E' || svalue[i + 1] == 'e')))) {
          size_t i2 = svalue.rfind('(');
          if (i2 != string::npos && i2 < i - 1) {
            if (svalue.find_first_not_of(
                    NUMBER_ELEMENTS SPACES,
                    svalue[0] == '-' || svalue[0] == '+' ? 1 : 0) == i2 &&
                svalue.find_first_not_of(NUMBERS SPACES, i2 + 1) == i &&
                (i == svalue.length() - 1 ||
                 svalue.find_first_not_of(NUMBER_ELEMENTS SPACES,
                                          svalue[i + 2] == '-' ||
                                                  svalue[i + 2] == '+'
                                              ? i + 3
                                              : i + 2) == string::npos)) {
      ...
      if (!suncertainty.empty()) {
        ...
        if (mstruct->isNumber()) {
          if (b_relative_uncertainty)
            mstruct->number().setRelativeUncertainty(nr_u);
          else
          ...
          mstruct->numberUpdated();
                   ...
                   (*mstruct)[0].isNumber()) {
          if (b_relative_uncertainty)
            (*mstruct)[0].number().setRelativeUncertainty(nr_u);
          else
        ...
        if (mstruct->isNumber()) {
          if (mstruct->number().precision() < 1 ||
              precision() < mstruct->number().precision()) {
            mstruct->number().setPrecision(precision());
            mstruct->numberUpdated();
          }
        } else if (mstruct->isMultiplication() &&
                   mstruct->getChild(1)->isNumber()) {
          if (mstruct->getChild(1)->number().precision() < 0 ||
              precision() < mstruct->getChild(1)->number().precision()) {
            mstruct->getChild(1)->number().setPrecision(precision());
            mstruct->getChild(1)->numberUpdated();
            mstruct->childUpdated(1);
          }
        } else if (mstruct->precision() < 0 ||
                   precision() < mstruct->precision()) {
...
MathStructure &AliasUnit::convertToFirstBaseUnit(MathStructure &mvalue,
                                                 MathStructure &mexp) const {
  ParseOptions po;
  if (isApproximate() && suncertainty.empty() && precision() == -1) {
    if (svalue.find(DOT) != string::npos)
      po.read_precision = READ_PRECISION_WHEN_DECIMALS;
    else
      po.read_precision = ALWAYS_READ_PRECISION;
  }
...
int AliasUnit::firstBaseExponent() const { return i_exp; }
...
void AliasUnit::setMixWithBaseMinimum(int mix_minimum) {
  i_mix_min = mix_minimum;
}
int AliasUnit::subtype() const { return SUBTYPE_ALIAS_UNIT; }
bool AliasUnit::isChildOf(Unit *u) const {
  ...
  while (1) {
    ...
    if (u == u2)
      return true;
    if (u2->subtype() != SUBTYPE_ALIAS_UNIT)
      return false;
  }
  return false;
}
...
bool AliasUnit::hasNonlinearRelationTo(Unit *u) const {
  ...
  if (ub2 != ub) {
    if (ub->subtype() == SUBTYPE_COMPOSITE_UNIT) {
      if (hasNonlinearRelationTo(ub))
        return ((CompositeUnit *)ub)->containsRelativeTo(u);
      for (size_t i = 1; i <= ((CompositeUnit *)ub)->countUnits(); i++) {
        if (((CompositeUnit *)ub)->get(i)->hasNonlinearRelationTo(u))
          return true;
      }
      return false;
    }
    if (ub2->baseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT) {
      if (((CompositeUnit *)ub2)->containsRelativeTo(baseUnit()) &&
          (u->hasNonlinearRelationTo(ub2) || hasNonlinearRelationTo(ub)))
        return true;
    }
    return false;
  }
  if (isParentOf(u)) {
    ...
    return hasNonlinearRelationTo(baseUnit()) ||
           u->hasNonlinearRelationTo(u->baseUnit());
  }
}
bool AliasUnit::hasApproximateExpression(
    bool check_variables, bool ignore_high_precision_intervals) const {
  if (isApproximate())
    return true;
  if (svalue.find_first_not_of(NUMBER_ELEMENTS EXPS) == string::npos)
    return false;
  MathStructure m(1, 1, 0), mexp(1, 1, 0);
  convertToFirstBaseUnit(m, mexp);
  return m.containsInterval(true, check_variables, false,
                            ignore_high_precision_intervals ? 1 : 0, true);
}
bool AliasUnit::hasApproximateRelationTo(
    Unit *u, bool check_variables, bool ignore_high_precision_intervals) const {
  if (u == this)
    return false;
  Unit *ub = baseUnit();
  ...
  if (ub2 != ub) {
    if (ub->subtype() == SUBTYPE_COMPOSITE_UNIT) {
      if (((CompositeUnit *)ub)->containsRelativeTo(u) &&
          hasApproximateRelationTo(ub, check_variables,
                                   ignore_high_precision_intervals))
        return true;
      for (size_t i = 1; i <= ((CompositeUnit *)ub)->countUnits(); i++) {
        if (((CompositeUnit *)ub)
                ->get(i)
                ->hasApproximateRelationTo(u, check_variables,
                                           ignore_high_precision_intervals))
          return true;
      }
      return false;
  ...
  if (isParentOf(u)) {
    ...
    while (true) {
      if ((const Unit *)fbu == this)
        return false;
      if (((AliasUnit *)fbu)->hasApproximateExpression(check_variables))
        return true;
      if (fbu->subtype() != SUBTYPE_ALIAS_UNIT)
        return false;
      fbu = (Unit *)((AliasUnit *)fbu)->firstBaseUnit();
    ...
    return hasApproximateRelationTo(baseUnit(), check_variables,
                                    ignore_high_precision_intervals) ||
           u->hasApproximateRelationTo(u->baseUnit(), check_variables,
                                       ignore_high_precision_intervals);
  }
}
bool AliasUnit::containsRelativeTo(Unit *u) const {
  if (!u || u == this)
    return false;
  return baseUnit() == u->baseUnit() ||
         baseUnit()->containsRelativeTo(u->baseUnit());
}
bool AliasUnit::hasNonlinearRelationToBase() const {
  return hasNonlinearRelationTo(baseUnit()) ||
         baseUnit()->hasNonlinearRelationToBase();
}
bool AliasUnit::hasApproximateRelationToBase(
    bool check_variables, bool ignore_high_precision_intervals) const {
  return hasApproximateRelationTo(baseUnit(), check_variables,
                                  ignore_high_precision_intervals) ||
         baseUnit()->hasApproximateRelationToBase(
             check_variables, ignore_high_precision_intervals);
}

AliasUnit_Composite::AliasUnit_Composite(Unit *alias, int exp, Prefix *prefix_)
    : AliasUnit("", alias->referenceName(), "", "", "", alias, "", exp, "") {
...
string AliasUnit_Composite::print(const PrintOptions &po, bool format,
                                  int tagtype, bool input, bool plural) const {
  string str = "";
  const ExpressionName *ename;
  if (input) {
    ename = &o_unit->preferredInputName(
        po.abbreviate_names, po.use_unicode_signs, plural,
        po.use_reference_names || (po.preserve_format && o_unit->isCurrency()),
        po.can_display_unicode_string_function,
        po.can_display_unicode_string_arg);
    if (prefixv)
      str = prefixv
                ->preferredInputName(ename->abbreviation, po.use_unicode_signs,
                                     plural, po.use_reference_names,
                                     po.can_display_unicode_string_function,
                                     po.can_display_unicode_string_arg)
                .name;
  } else {
    ename = &o_unit->preferredDisplayName(
        po.abbreviate_names, po.use_unicode_signs, plural,
        po.use_reference_names || (po.preserve_format && o_unit->isCurrency()),
        po.can_display_unicode_string_function,
        po.can_display_unicode_string_arg);
  ...
  str += ename->formattedName(
      TYPE_UNIT, !po.use_reference_names, format && tagtype == TAG_TYPE_HTML,
      format && tagtype == TAG_TYPE_TERMINAL && po.use_unicode_signs ? 1 : 0,
      !po.use_reference_names && !po.preserve_format,
      po.hide_underscore_spaces);
  return str;
}
string AliasUnit_Composite::print(
    bool plural_, bool short_, bool use_unicode,
    bool (*can_display_unicode_string_function)(const char *, void *),
    void *can_display_unicode_string_arg) const {
  string str = "";
  const ExpressionName *ename = &o_unit->preferredName(
      short_, use_unicode, plural_, false, can_display_unicode_string_function,
      can_display_unicode_string_arg);
  if (prefixv) {
    str += prefixv
               ->preferredName(ename->abbreviation, use_unicode, plural_, false,
                               can_display_unicode_string_function,
                               can_display_unicode_string_arg)
               .name;
  }
...
Prefix *AliasUnit_Composite::prefix() const { return prefixv; }
int AliasUnit_Composite::prefixExponent() const {
  if (prefixv && prefixv->type() == PREFIX_DECIMAL)
    return ((DecimalPrefix *)prefixv)->exponent();
  if (prefixv && prefixv->type() == PREFIX_BINARY)
    return ((BinaryPrefix *)prefixv)->exponent();
  return 0;
}
...
MathStructure &
AliasUnit_Composite::convertToFirstBaseUnit(MathStructure &mvalue,
                                            MathStructure &mexp) const {
  if (prefixv) {
...
MathStructure &
AliasUnit_Composite::convertFromFirstBaseUnit(MathStructure &mvalue,
                                              MathStructure &mexp) const {
  if (i_exp != 1)
    mexp /= i_exp;
  if (prefixv) {
...
CompositeUnit::CompositeUnit(string cat_, string name_, string title_,
                             string base_expression_, bool is_local,
                             bool is_builtin, bool is_active)
    : Unit(cat_, name_, "", "", title_, is_local, is_builtin, is_active) {
  setBaseExpression(base_expression_);
  setChanged(false);
}
CompositeUnit::CompositeUnit(const CompositeUnit *unit) { set(unit); }
CompositeUnit::~CompositeUnit() { clear(); }
ExpressionItem *CompositeUnit::copy() const { return new CompositeUnit(this); }
...
Unit *CompositeUnit::get(size_t index, int *exp, Prefix **prefix) const {
  if (index > 0 && index <= units.size()) {
    if (exp)
      *exp = units[index - 1]->firstBaseExponent();
    if (prefix)
      *prefix = (Prefix *)units[index - 1]->prefix();
...
void CompositeUnit::setPrefix(size_t index, Prefix *prefix) {
  if (index > 0 && index <= units.size()) {
    units[index - 1]->set(units[index - 1]->firstBaseUnit(),
                          units[index - 1]->firstBaseExponent(), prefix);
  }
}
size_t CompositeUnit::countUnits() const { return units.size(); }
...
string CompositeUnit::print(const PrintOptions &po, bool format, int tagtype,
                            bool input, bool plural) const {
  string str = "";
    ...
    if (units[i]->firstBaseExponent() != 0) {
      if (!b && units[i]->firstBaseExponent() < 0 && i > 0) {
        if (po.use_unicode_signs &&
            po.division_sign == DIVISION_SIGN_DIVISION &&
            (!po.can_display_unicode_string_function ||
             (*po.can_display_unicode_string_function)(
                 SIGN_DIVISION, po.can_display_unicode_string_arg))) {
          str += SIGN_DIVISION;
        } else if (!input && po.use_unicode_signs &&
      ...
      str += units[i]->print(po, format, tagtype, input,
                             plural && i == 0 &&
                                 units[i]->firstBaseExponent() > 0);
      if (po.abbreviate_names && po.use_unicode_signs &&
          units[i]->firstBaseExponent() != (b ? -1 : 1) && str.length() >= 2 &&
          str[str.length() - 1] == (char)-80 &&
          str[str.length() - 2] == (char)-62) {
        ...
        if (units[i]->firstBaseExponent() != -1) {
          if (format && tagtype == TAG_TYPE_HTML) {
            ...
            str += "</sup>";
                     ...
                     units[i]->firstBaseExponent() == -2 &&
                     (!po.can_display_unicode_string_function ||
                      (*po.can_display_unicode_string_function)(
                          SIGN_POWER_2, po.can_display_unicode_string_arg)))
            str += SIGN_POWER_2;
          else if (po.use_unicode_signs &&
                   units[i]->firstBaseExponent() == -3 &&
                   (!po.can_display_unicode_string_function ||
                    (*po.can_display_unicode_string_function)(
            ...
            str += "</sup>";
                     ...
                     (!po.can_display_unicode_string_function ||
                      (*po.can_display_unicode_string_function)(
                          SIGN_POWER_2, po.can_display_unicode_string_arg)))
            str += SIGN_POWER_2;
          else if (po.use_unicode_signs && units[i]->firstBaseExponent() == 3 &&
                   (!po.can_display_unicode_string_function ||
                    (*po.can_display_unicode_string_function)(
                        SIGN_POWER_3, po.can_display_unicode_string_arg)))
            str += SIGN_POWER_3;
...
string CompositeUnit::print(
  ...
  for (size_t i = 0; i < units.size(); i++) {
    if (units[i]->firstBaseExponent() != 0) {
      if (!b && units[i]->firstBaseExponent() < 0 && i > 0) {
        ...
        if (i > 0) {
          if (use_unicode &&
              (!can_display_unicode_string_function ||
               (*can_display_unicode_string_function)(
                   SIGN_MIDDLEDOT, can_display_unicode_string_arg)))
            str += SIGN_MIDDLEDOT;
          else
            str += " ";
        }
      ...
      if (short_ && use_unicode &&
          units[i]->firstBaseExponent() != (b ? -1 : 1) && str.length() >= 2 &&
          str[str.length() - 1] == (char)-80 &&
          str[str.length() - 2] == (char)-62) {
        str.erase(str.length() - 2, 2);
        str += units[i]->print(
            plural_ && i == 0 && units[i]->firstBaseExponent() > 0, short_,
            false, can_display_unicode_string_function,
            can_display_unicode_string_arg);
      }
      if (b) {
        if (units[i]->firstBaseExponent() != -1) {
          if (use_unicode && units[i]->firstBaseExponent() == -2 &&
              (!can_display_unicode_string_function ||
               (*can_display_unicode_string_function)(
                   SIGN_POWER_2, can_display_unicode_string_arg)))
            str += SIGN_POWER_2;
          else if (use_unicode && units[i]->firstBaseExponent() == -3 &&
                   (!can_display_unicode_string_function ||
                    (*can_display_unicode_string_function)(
                        SIGN_POWER_3, can_display_unicode_string_arg)))
            str += SIGN_POWER_3;
          else {
            str += "^";
            str += i2s(-units[i]->firstBaseExponent());
        ...
        if (units[i]->firstBaseExponent() != 1) {
          if (use_unicode && units[i]->firstBaseExponent() == 2 &&
            ...
            if (units[i]->firstBaseExponent() < 0 &&
                (!can_display_unicode_string_function ||
                 (*can_display_unicode_string_function)(
                     SIGN_MINUS, can_display_unicode_string_arg))) {
              str += SIGN_MINUS;
              str += i2s(-units[i]->firstBaseExponent());
            } else {
              str += i2s(units[i]->firstBaseExponent());
...
bool CompositeUnit::containsRelativeTo(Unit *u) const {
  ...
  if (u->baseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT) {
    ...
    for (size_t i = 1; i <= cu->countUnits(); i++) {
      if (containsRelativeTo(cu->get(i)->baseUnit()))
        return true;
    }
    return false;
  }
...
bool CompositeUnit::hasNonlinearRelationToBase() const {
  for (size_t i = 0; i < units.size(); i++) {
    if (units[i]->hasNonlinearRelationToBase())
      return true;
  }
  return false;
}
bool CompositeUnit::hasApproximateRelationToBase(
    bool check_variables, bool ignore_high_precision_intervals) const {
  for (size_t i = 0; i < units.size(); i++) {
    if (units[i]->hasApproximateRelationToBase(check_variables,
                                               ignore_high_precision_intervals))
      return true;
  }
  return false;
...
MathStructure
CompositeUnit::generateMathStructure(bool make_division,
                                     bool set_null_prefixes) const {
  ...
  for (size_t i = 0; i < units.size(); i++) {
    ...
    if (!has_p || units[i]->prefix()) {
      ...
      mstruct2.set(units[i]->firstBaseUnit(),
                   CALCULATOR->getDecimalNullPrefix());
    }
...
bool replace_variables(MathStructure &m) {
  ...
  if (m.isVariable() && m.variable()->isKnown()) {
    Unit *u =
        CALCULATOR->getActiveUnit(m.variable()->referenceName() + "_unit");
    if (!u) {
      if (m.variable()->referenceName() == "bohr_radius")
        u = CALCULATOR->getActiveUnit("bohr_unit");
      else if (m.variable()->referenceName() == "elementary_charge")
        u = CALCULATOR->getActiveUnit("e_unit");
      else if (m.variable()->referenceName() == "electron_mass")
...
void CompositeUnit::setBaseExpression(string base_expression_) {
  ...
  while (true) {
    ...
    } else if (mstruct.isPower() && mstruct[0].isUnit() &&
               mstruct[1].isInteger()) {
      add(mstruct[0].unit(), mstruct[1].number().intValue(),
          mstruct[0].prefix());
    } else if (mstruct.isMultiplication()) {
      for (size_t i = 0; i < mstruct.size(); i++) {
        if (mstruct[i].isUnit()) {
          add(mstruct[i].unit(), 1, mstruct[i].prefix());
        } else if (mstruct[i].isPower() && mstruct[i][0].isUnit() &&
                   mstruct[i][1].isInteger()) {
          add(mstruct[i][0].unit(), mstruct[i][1].number().intValue(),
              mstruct[i][0].prefix());
          ...
          for (size_t i2 = 0; i2 < mstruct[i].size(); i2++) {
            ...
            } else if (mstruct[i][i2].isPower() && mstruct[i][i2][0].isUnit() &&
                       mstruct[i][i2][1].isInteger()) {
              add(mstruct[i][i2][0].unit(),
                  mstruct[i][i2][1].number().intValue(),
                  mstruct[i][i2][0].prefix());
            } else {
              had_errors = true;
            }
          }
  ...
  if (conversion_variant && had_errors) {
    ...
    if (CALCULATOR->endTemporaryStopMessages() > 0)
      had_errors = true;
    if (had_errors)
      CALCULATOR->error(false, _("Error(s) in unitexpression."), NULL);
  }
  setChanged(true);
}
