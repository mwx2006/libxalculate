...
bool sr_gcd(const MathStructure &m1, const MathStructure &m2,
            MathStructure &mgcd, const sym_desc_vec &sym_stats, size_t var_i,
            const EvaluationOptions &eo) {

  if (var_i >= sym_stats.size())
    return false;
  ...
  if (!MathStructure::gcd(cont_c, cont_d, gamma, eo, NULL, NULL, false))
    return false;
  mgcd = gamma;
  if (ddeg.isZero()) {
    return true;
...
bool interpolate(const MathStructure &gamma, const Number &xi,
                 const MathStructure &xvar, MathStructure &minterp,
                 const EvaluationOptions &eo) {
    ...
    if (minterp.isZero() && !gi.isZero()) {
      ...
      if (i != 0) {
        if (minterp.isOne()) {
          ...
          if (i != 1)
            minterp[minterp.size() - 1].raise(i);
      ...
      if (i != 0) {
        if (minterp[minterp.size() - 1].isOne()) {
          ...
          if (i != 1)
            minterp[minterp.size() - 1][minterp[minterp.size() - 1].size() - 1]
                .raise(i);
...
bool heur_gcd(const MathStructure &m1, const MathStructure &m2,
              MathStructure &mgcd, const EvaluationOptions &eo,
              MathStructure *ca, MathStructure *cb, sym_desc_vec &sym_stats,
              size_t var_i) {
  ...
  if (m1.isNumber() && m2.isNumber()) {
    mgcd = m1;
    if (!m1.isInteger() || !m2.isInteger() || !mgcd.number().gcd(m2.number()))
      mgcd.set(1, 1, 0);
    if (ca) {
      *ca = m1;
  ...
  if (maxdeg2.isGreaterThan(maxdeg))
    maxdeg = maxdeg2;
  ...
  for (int t = 0; t < 6; t++) {
    ...
    if (!xi.isInteger() ||
        (maxdeg * xi.integerLength()).isGreaterThan(100000L)) {
      return false;
    }
    ...
    if (heur_gcd(psub, qsub, gamma, eo, &cp, &cq, sym_stats, var_i + 1)) {
      ...
      if (!interpolate(gamma, xi, xvar, mgcd, eo))
        return false;
      ...
      if (divide_in_z(p, mgcd, ca ? *ca : dummy, sym_stats, var_i, eo) &&
          divide_in_z(q, mgcd, cb ? *cb : dummy, sym_stats, var_i, eo)) {
...
bool MathStructure::lcm(const MathStructure &m1, const MathStructure &m2,
                        MathStructure &mlcm, const EvaluationOptions &eo,
                        bool check_args) {
  if (m1.isNumber() && m2.isNumber()) {
    mlcm = m1;
    if (!mlcm.isInteger() || !m2.isInteger())
      return mlcm.number().multiply(m2.number());
    return mlcm.number().lcm(m2.number());
  }
  if (check_args &&
      (!m1.isRationalPolynomial() || !m2.isRationalPolynomial())) {
    return false;
  }
  MathStructure ca, cb;
  if (!MathStructure::gcd(m1, m2, mlcm, eo, &ca, &cb, false))
    return false;
...
bool has_noninteger_coefficient(const MathStructure &mstruct) {
  if (mstruct.isNumber() && mstruct.number().isRational() &&
    ...
    return true;
  if (mstruct.isFunction() || mstruct.isPower())
    return false;
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (has_noninteger_coefficient(mstruct[i]))
      return true;
  }
  return false;
}

bool MathStructure::gcd(const MathStructure &m1, const MathStructure &m2,
                        MathStructure &mresult, const EvaluationOptions &eo2,
                        MathStructure *ca, MathStructure *cb, bool check_args) {
  ...
  if (m1.isNumber() && m2.isNumber()) {
    mresult = m1;
    if (!m1.isInteger() || !m2.isInteger() ||
        !mresult.number().gcd(m2.number()))
      mresult.set(1, 1, 0);
    if (ca || cb) {
      if (mresult.isZero()) {
        if (ca)
  ...
  if (has_noninteger_coefficient(m1) || has_noninteger_coefficient(m2)) {
    ...
    if (!nlcm1.isInteger() || !nlcm2.isInteger())
      nlcm1.multiply(nlcm2);
    else
      nlcm1.lcm(nlcm2);
    if (!nlcm1.isOne()) {
      ...
      if (mtmp1.equals(m1, true, true) || mtmp2.equals(m2, true, true))
        return false;
  ...
  factored_1:
    ...
    for (size_t i = 0; i < m1.size(); i++) {
      ...
      MathStructure::gcd(m1[i], part_2, mresult[i], eo, &part_ca, &part_cb,
                         false);
      if (CALCULATOR->aborted())
        return false;
  ...
  factored_2:
    ...
    for (size_t i = 0; i < m2.size(); i++) {
      mresult.addChild(m_zero);
      MathStructure::gcd(part_1, m2[i], mresult[i], eo, &part_ca, &part_cb,
                         false);
      if (CALCULATOR->aborted())
        return false;
  ...
  if (m1.isPower()) {
    ...
    if (m2.isPower()) {
      if (m1[0] == m2[0]) {
        ...
        MathStructure p_gcd;
        if (!MathStructure::gcd(m1[0], m2[0], p_gcd, eo, &p_co, &pb_co, false))
          return false;
        if (p_gcd.isOne()) {
          return true;
        } else {
          if (m1[1].number().isLessThan(m2[1].number())) {
            ...
            bool b = MathStructure::gcd(
                p_co, p_gcd, mresult[mresult.size() - 1], eo, ca, cb, false);
            ...
            bool b = MathStructure::gcd(
                p_gcd, pb_co, mresult[mresult.size() - 1], eo, ca, cb, false);
            mresult.calculateMultiplyLast(eo);
            return b;
          }
        }
      ...
      MathStructure p_gcd;
      if (!MathStructure::gcd(m1[0], m2, p_gcd, eo, &p_co, &bpart_co, false))
        return false;
      if (p_gcd.isOne()) {
        return true;
      } else {
        ...
        p_gcd.calculateMultiply(p_co, eo);
        bool b = MathStructure::gcd(
            p_gcd, bpart_co, mresult[mresult.size() - 1], eo, ca, cb, false);
        mresult.calculateMultiplyLast(eo);
        return b;
      }
    }
    ...
    MathStructure p_gcd;
    if (!MathStructure::gcd(m1, m2[0], p_gcd, eo, &apart_co, &p_co, false))
      return false;
    if (p_gcd.isOne()) {
      return true;
    } else {
      ...
      p_gcd.calculateRaiseExponent(eo);
      p_gcd.calculateMultiply(p_co, eo);
      bool b = MathStructure::gcd(apart_co, p_gcd, mresult[mresult.size() - 1],
                                  eo, ca, cb, false);
      mresult.calculateMultiplyLast(eo);
      return b;
    }
  ...
  format_and_print(sym_stats[i].sym)); m1b.replace(sym_stats[i].sym, var);
                  ...
                  for(i = 0; i < sym_stats.size(); i++) {
                          if(sym_stats[i].sym.size() > 0) {
                                  ...
                                  if(ca) ca->replace(vars[i2],
  sym_stats[i].sym); if(cb) cb->replace(vars[i2], sym_stats[i].sym);
                                  vars[i2]->destroy();
                                  i2++;
                          }
                  }
  ...
  if (m.isPower() && (!b_unknown || m[0].containsUnknowns())) {
    ...
    if (!nr.isZero()) {
      ...
      return nr;
             ...
             ((b_unknown && m.isUnknown()) || (!b_unknown && !m.isNumber()))) {
    return nr_one;
  }
  Number nr;
...
void replace_fracpow(MathStructure &mstruct, vector<UnknownVariable *> &uv,
  ...
  if (mstruct.isFunction())
    return;
  if (!b_top && mstruct.isPower() && mstruct[1].isNumber() &&
      mstruct[1].number().isRational() && !mstruct[1].number().isInteger() &&
      mstruct[0].isRationalPolynomial()) {
    ...
    UnknownVariable *var = new UnknownVariable(
        "", string(LEFT_PARENTHESIS) + format_and_print(mstruct) +
                RIGHT_PARENTHESIS);
...
bool restore_fracpow(MathStructure &mstruct, UnknownVariable *uv,
                     const EvaluationOptions &eo, bool b_eval) {
  if (mstruct.isPower() && mstruct[0].isVariable() &&
      mstruct[0].variable() == uv && mstruct[1].isInteger()) {
  ...
  if (b_ret && b_eval)
    return mstruct.calculatesub(eo, eo, false);
  return false;
}

bool do_simplification(MathStructure &mstruct, const EvaluationOptions &eo,
                       bool combine_divisions, bool only_gcd, bool combine_only,
                       bool recursive, bool limit_size, int i_run) {

  if (!eo.expand || !eo.assume_denominators_nonzero || mstruct.size() == 0)
    return false;

  if (!combine_only && combine_divisions && i_run == 1) {
    bool b_ret = do_simplification(mstruct, eo, combine_divisions, only_gcd,
                                   combine_only, recursive, limit_size, -1);
    vector<UnknownVariable *> uv;
    replace_fracpow(mstruct, uv);
    if (uv.size() > 0) {
      MathStructure mbak(mstruct);
      mstruct.evalSort(true);
      bool b = do_simplification(mstruct, eo, combine_divisions, only_gcd,
                                 combine_only, recursive, limit_size, -1) &&
               mbak != mstruct;
      ...
      if (b) {
        b_ret = true;
        if (mstruct.calculatesub(eo, eo))
          do_simplification(mstruct, eo, combine_divisions, only_gcd,
                            combine_only, recursive, limit_size, -1);
      }
    }
    return b_ret;
  ...
  if (recursive) {
    bool b = false;
    for (size_t i = 0; i < mstruct.size(); i++) {
      b = do_simplification(mstruct[i], eo, combine_divisions,
                            only_gcd || (!mstruct.isComparison() &&
                                         !mstruct.isLogicalAnd() &&
                                         !mstruct.isLogicalOr()),
                            combine_only, true, limit_size, i_run) ||
          b;
      if (CALCULATOR->aborted())
        return b;
    }
    ...
    return do_simplification(mstruct, eo, combine_divisions, only_gcd,
                             combine_only, false, limit_size, i_run) ||
           b;
  }
  if (mstruct.isPower() && mstruct[1].isNumber() &&
      mstruct[1].number().isRational() && !mstruct[1].number().isInteger() &&
      mstruct[0].isAddition() && mstruct[0].isRationalPolynomial()) {
    MathStructure msqrfree(mstruct[0]);
    if (sqrfree(msqrfree, eo) && msqrfree.isPower() &&
        msqrfree.calculateRaise(mstruct[1], eo)) {
      mstruct = msqrfree;
      return true;
    }
  } else if (mstruct.isFunction() &&
             mstruct.function()->id() == FUNCTION_ID_ROOT &&
             VALID_ROOT(mstruct) && mstruct[0].isAddition() &&
             mstruct[0].isRationalPolynomial()) {
    MathStructure msqrfree(mstruct[0]);
    if (sqrfree(msqrfree, eo) && msqrfree.isPower() &&
        msqrfree[1].isInteger() && msqrfree[1].number().isPositive()) {
      if (msqrfree[1] == mstruct[1]) {
        if (msqrfree[1].number().isEven()) {
          if (!msqrfree[0].representsReal(true))
            return false;
          msqrfree.delChild(2);
  ...
  if (!mstruct.isAddition() && !mstruct.isMultiplication())
    return false;
  ...
  if (combine_divisions) {
    ...
    if (!mstruct.isAddition())
      mstruct.transform(STRUCT_ADDITION);
    ...
    for (size_t i = 0; i < mstruct.size(); i++) {
      ...
      if (mstruct[i].isMultiplication()) {
        ...
        for (size_t i2 = 0; i2 < mstruct[i].size(); i2++) {
          if (mstruct[i][i2].isPower() && mstruct[i][i2][1].isInteger() &&
              mstruct[i][i2][1].number().isNegative()) {
            if (b_rat_m) {
              bool b_rat = mstruct[i][i2][0].isRationalPolynomial();
              if (!b_rat && mstruct[i][i2][0].isAddition() &&
                  mstruct[i][i2][0].size() > 1) {
                b_rat = true;
                for (size_t i3 = 0; i3 < mstruct[i][i2][0].size(); i3++) {
                  if (!mstruct[i][i2][0][i3].representsZero(true) &&
                      !mstruct[i][i2][0][i3].isRationalPolynomial()) {
                    b_rat = false;
                    break;
                  }
                }
            ...
            if (b_minone) {
              if (div.isZero())
                ...
                div.multiply(mstruct[i][i2][0], true);
            } else {
              mstruct[i][i2][1].number().negate();
              if (div.isZero())
                div = mstruct[i][i2];
              else
                div.multiply(mstruct[i][i2], true);
              mstruct[i][i2][1].number().negate();
            }
          } else {
            if (b_rat_m && !mstruct[i][i2].isRationalPolynomial() &&
                !mstruct[i][i2].representsZero(true))
              b_rat_m = false;
            if (!b_num) {
              b_num = true;
              num = mstruct[i][i2];
            } else
              num.multiply(mstruct[i][i2], true);
          }
        }
        if (!div.isZero()) {
          bool b_found = false;
          for (size_t i3 = 0; i3 < (b_rat_m ? divs.size() : nrdivs.size());
               i3++) {
            if ((b_rat_m && divs[i3] == div) ||
                (!b_rat_m && nrdivs[i3] == div)) {
                ...
                if (num.isAddition()) {
                  for (size_t i4 = 0; i4 < num.size(); i4++) {
                    if (b_rat_m)
                      ...
                      nrnums[i3].add(num[i4], true);
                  }
                } else {
                  if (b_rat_m)
                    nums[i3].add(num, true);
                  else
                    nrnums[i3].add(num, true);
                }
              }
              b_found = true;
              b = true;
          ...
          if (!b_found &&
              (eo.assume_denominators_nonzero || div.representsNonZero(true)) &&
              !div.representsZero(true)) {
            if (!num.representsZero(true)) {
              if (b_rat_m) {
                divs.addChild(div);
                nums.addChild(num);
      ...
      } else if (mstruct[i].isPower() && mstruct[i][1].isInteger() &&
                 mstruct[i][1].number().isNegative()) {
        ...
        if (!b_rat && mstruct[i][0].isAddition() && mstruct[i][0].size() > 1) {
          ...
          for (size_t i2 = 0; i2 < mstruct[i][0].size(); i2++) {
            if (!mstruct[i][0][i2].representsZero(true) &&
                !mstruct[i][0].isRationalPolynomial()) {
              b_rat = false;
              break;
            }
          }
        ...
        if (!b_minone)
          mstruct[i][1].number().negate();
        bool b_found = false;
        for (size_t i3 = 0; i3 < (b_rat ? divs.size() : nrdivs.size()); i3++) {
          if ((b_minone && ((b_rat && divs[i3] == mstruct[i][0]) ||
                            (!b_rat && nrdivs[i3] == mstruct[i][0]))) ||
              (!b_minone && ((b_rat && divs[i3] == mstruct[i]) ||
                             (!b_rat && nrdivs[i3] == mstruct[i])))) {
            if (b_rat)
              nums[i3].add(m_one, true);
            else
              nrnums[i3].add(m_one, true);
            b_found = true;
            b = true;
            break;
          }
        ...
        if (!b_found &&
            (eo.assume_denominators_nonzero ||
             mstruct[i][0].representsNonZero(true)) &&
            !mstruct[i][0].representsZero(true)) {
          if (b_rat) {
            if (b_minone)
              divs.addChild(mstruct[i][0]);
            else
              divs.addChild(mstruct[i]);
            nums.addChild(m_one);
          } else {
            if (b_minone)
              nrdivs.addChild(mstruct[i][0]);
            else
              nrdivs.addChild(mstruct[i]);
      ...
      if (!b) {
        if (i_run <= 1 && mstruct[i].isRationalPolynomial())
          numleft.addChild(mstruct[i]);
        else
          mleft.addChild(mstruct[i]);
      }
    }
    ...
    for (size_t i = 0; i < nrdivs.size(); i++) {
      ...
      if (nrnums[i].isAddition()) {
        ...
        MathStructure mnew;
        if (factorize_find_multiplier(nrnums[i], mnew, factor_mstruct) &&
      ...
      if (b) {
        ...
        if (nrdivs[i].isPower() && nrdivs[i][1].isNumber())
          nrdivs[i][1].number().negate();
        else
          nrdivs[i] ^= nr_minus_one;
        nrnums[i] *= nrdivs[i];
      }
    ...
    if (i_run > 1) {
      for (size_t i = 0; i < divs.size();) {
        bool b = true;
        if (!divs[i].isRationalPolynomial() ||
            !nums[i].isRationalPolynomial()) {
          if (mstruct.size() == 1)
            mstruct.setToChild(1);
          return false;
        }
        if (!combine_only && divs[i].isAddition() && nums[i].isAddition()) {
          ...
          if (MathStructure::gcd(nums[i], divs[i], mgcd, eo2, &ca, &cb,
                                 false) &&
              !mgcd.isOne() &&
              (!cb.isAddition() || !ca.isAddition() ||
               ca.size() + cb.size() <= nums[i].size() + divs[i].size())) {
            if (mgcd.representsNonZero(true) ||
                !eo.warn_about_denominators_assumed_nonzero ||
                warn_about_denominators_assumed_nonzero(mgcd, eo)) {
              if (cb.isOne()) {
                numleft.addChild(ca);
                nums.delChild(i + 1);
                divs.delChild(i + 1);
      ...
      while (divs.size() > 0) {
        bool b = true;
        if (!divs[0].isRationalPolynomial() ||
            !nums[0].isRationalPolynomial()) {
          if (mstruct.size() == 1)
            mstruct.setToChild(1);
          return false;
        }
        if (!combine_only && divs[0].isAddition() && nums[0].isAddition()) {
          MathStructure ca, cb, mgcd;
          if (MathStructure::gcd(nums[0], divs[0], mgcd, eo2, &ca, &cb,
                                 false) &&
              !mgcd.isOne() &&
              (!cb.isAddition() || !ca.isAddition() ||
               ca.size() + cb.size() <= nums[0].size() + divs[0].size())) {
            if (mgcd.representsNonZero(true) ||
                !eo.warn_about_denominators_assumed_nonzero ||
                warn_about_denominators_assumed_nonzero(mgcd, eo)) {
              if (cb.isOne()) {
        ...
        if (b && divs.size() > 1) {
          if (!combine_only && divs[1].isAddition() && nums[1].isAddition()) {
            ...
            if (MathStructure::gcd(nums[1], divs[1], mgcd, eo3, &ca, &cb,
                                   false) &&
                !mgcd.isOne() &&
                (!cb.isAddition() || !ca.isAddition() ||
                 ca.size() + cb.size() <= nums[1].size() + divs[1].size())) {
              if (mgcd.representsNonZero(true) ||
                  !eo.warn_about_denominators_assumed_nonzero ||
                  warn_about_denominators_assumed_nonzero(mgcd, eo)) {
                if (cb.isOne()) {
          ...
          if (b) {
            MathStructure ca, cb, mgcd;
            b = MathStructure::gcd(divs[0], divs[1], mgcd, eo2, &ca, &cb,
                                   false) &&
                !mgcd.isOne();
            if (CALCULATOR->aborted()) {
              if (mstruct.size() == 1)
            ...
            if (b) {
              if (limit_size &&
                  ((cb.isAddition() && ((divs[0].isAddition() &&
                                         divs[0].size() * cb.size() > 200) ||
                                        (nums[0].isAddition() &&
                                         nums[0].size() * cb.size() > 200))) ||
                   (ca.isAddition() && nums[1].isAddition() &&
                    nums[1].size() * ca.size() > 200))) {
                b_merge = false;
              } else {
                if (!cb.isOne()) {
              ...
              if (limit_size &&
                  ((divs[1].isAddition() &&
                    ((divs[0].isAddition() &&
                      divs[0].size() * divs[1].size() > 200) ||
                     (nums[0].isAddition() &&
                      nums[0].size() * divs[1].size() > 200))) ||
                   (divs[0].isAddition() && nums[1].isAddition() &&
                    nums[1].size() * divs[0].size() > 200))) {
                b_merge = false;
              } else {
            ...
            if (b_merge) {
              ...
              if (nums[0].isAddition())
                size_1 += nums[0].size() - 1;
              if (divs[0].isAddition())
                size_1 += divs[0].size() - 1;
              if (nums[1].isAddition())
                size_2 += nums[1].size() - 1;
              if (divs[1].isAddition())
                size_2 += divs[1].size() - 1;
              if (size_1 > size_2) {
          ...
          if (limit_size && divs[0].isAddition() && numleft.size() > 0 &&
              divs[0].size() * numleft.size() > 200)
            break;
          if (numleft.size() == 1)
            numleft.setToChild(1);
          else if (numleft.size() > 1)
            numleft.setType(STRUCT_ADDITION);
      ...
      while (!combine_only && !only_gcd && divs.size() > 0 &&
             divs[0].isAddition() && !nums[0].isNumber()) {
        bool b_unknown = divs[0].containsUnknowns();
        if (!b_unknown || nums[0].containsUnknowns()) {
          MathStructure mcomps;
          mcomps.resizeVector(nums[0].isAddition() ? nums[0].size() : 1,
                              m_zero);
          if (nums[0].isAddition()) {
            if (mcomps.size() < nums[0].size()) {
              if (mstruct.size() == 1)
                mstruct.setToChild(1);
              return false;
            }
            for (size_t i = 0; i < nums[0].size(); i++) {
              if ((b_unknown && nums[0][i].containsUnknowns()) ||
                  (!b_unknown && !nums[0][i].isNumber())) {
                mcomps[i].setType(STRUCT_MULTIPLICATION);
                if (nums[0][i].isMultiplication()) {
                  for (size_t i2 = 0; i2 < nums[0][i].size(); i2++) {
                    if ((b_unknown && nums[0][i][i2].containsUnknowns()) ||
                        (!b_unknown && !nums[0][i][i2].isNumber())) {
                      nums[0][i][i2].ref();
            ...
            if (nums[0].isMultiplication()) {
              for (size_t i2 = 0; i2 < nums[0].size(); i2++) {
                if ((b_unknown && nums[0][i2].containsUnknowns()) ||
                    (!b_unknown && !nums[0][i2].isNumber())) {
                  nums[0][i2].ref();
          ...
          for (size_t i = 0; i < divs[0].size(); i++) {
            if ((b_unknown && divs[0][i].containsUnknowns()) ||
                (!b_unknown && !divs[0][i].isNumber())) {
              MathStructure mcomp1;
              mcomp1.setType(STRUCT_MULTIPLICATION);
              if (divs[0][i].isMultiplication()) {
                for (size_t i2 = 0; i2 < divs[0][i].size(); i2++) {
                  if ((b_unknown && divs[0][i][i2].containsUnknowns()) ||
                      (!b_unknown && !divs[0][i][i2].isNumber())) {
                    divs[0][i][i2].ref();
          ...
          if (b_found) {
            ...
            if (polynomial_long_division(
                    nums[0].isAddition() ? nums[0][matches[i_selected]]
                                         : nums[0],
                    divs[0], m_zero, mquo, mrem, eo2, false) &&
                !mquo.isZero() &&
                mrem != (nums[0].isAddition() ? nums[0][matches[i_selected]]
                                              : nums[0])) {
              if (CALCULATOR->aborted())
                return false;
              if ((nums[0].isAddition() && nums[0].size() > 1) ||
                  !mrem.isZero() || divs[0].representsNonZero(true) ||
                  (eo.warn_about_denominators_assumed_nonzero &&
                   !warn_about_denominators_assumed_nonzero(divs[0], eo))) {
                mleft.addChild(mquo);
                if (nums[0].isAddition()) {
      ...
      if (!combine_only && !only_gcd && divs.size() > 0 &&
          divs[0].isAddition()) {
        bool b_unknown =
            divs[0].containsUnknowns() || nums[0].containsUnknowns();
        MathStructure mquo, mrem;
        ...
        for (size_t i = 0; i < symsd.size();) {
          ...
          if (!b_unknown || symsd[i].containsUnknowns()) {
            for (size_t i2 = 0; i2 < symsn.size(); i2++) {
              if (symsn[i2] == symsd[i]) {
                b_found =
                    (nums[0].degree(symsd[i]) >= divs[0].degree(symsd[i]));
                break;
              }
            }
          }
          if (b_found)
            ...
            symsd.erase(symsd.begin() + i);
        }
        for (size_t i = 0; i < symsd.size(); i++) {
          if (polynomial_long_division(nums[0], divs[0], symsd[i], mquo, mrem,
                                       eo2, false) &&
              !mquo.isZero() && mrem != nums[0]) {
            if (CALCULATOR->aborted()) {
              if (mstruct.size() == 1)
                mstruct.setToChild(1);
              return false;
            }
            if (!mrem.isZero() || divs[0].representsNonZero(true) ||
                (eo.warn_about_denominators_assumed_nonzero &&
                 !warn_about_denominators_assumed_nonzero(divs[0], eo))) {
              if (mrem.isZero()) {
                mleft.addChild(mquo);
                ...
                if (!mquo.isZero()) {
                  ...
                  if (point > 0) {
                    ...
                    MathStructure ca, cb, mgcd;
                    if (mrem.isAddition() &&
                        MathStructure::gcd(mrem, divs[0], mgcd, eo2, &ca, &cb,
                                           false) &&
                        !mgcd.isOne() &&
                        (!cb.isAddition() || !ca.isAddition() ||
                         ca.size() + cb.size() <=
                             mrem.size() + divs[0].size())) {
                      mrem = ca;
                      divs[0] = cb;
                    }
    ...
    for (size_t i = 0; i < divs.size() && (i == 0 || i_run > 1); i++) {
      if (i == 0)
        ...
        mstruct.add(nums[i], i > 1);
      if (only_gcd || combine_only) {
        divs[i].inverse();
        if (i == 0)
          mstruct.multiply(divs[i], true);
        else
          mstruct.last().multiply(divs[i], true);
      } else {
        if (i == 0)
          mstruct.calculateDivide(divs[i], eo);
        else
          mstruct.last().calculateDivide(divs[i], eo);
        if (i == 0 && i_run <= 1 && mstruct.isAddition()) {
          do_simplification(mstruct, eo, true, false, false, false, limit_size,
                            2);
        }
      }
    }
    ...
    for (size_t i = 0; i < mleft.size(); i++) {
      if (i == 0 && mstruct.isZero())
        mstruct = mleft[i];
      else
        mstruct.calculateAdd(mleft[i], eo);
    }
    for (size_t i = 0; i < numleft.size(); i++) {
      if (i == 0 && mstruct.isZero())
        mstruct = numleft[i];
      else
        mstruct.calculateAdd(numleft[i], eo);
    }
  ...
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (mstruct[i].isMultiplication()) {
      for (size_t i2 = 0; i2 < mstruct[i].size(); i2++) {
        if (mstruct[i][i2].isPower() &&
            (mstruct[i][i2][0].isAddition() || combine_divisions) &&
            mstruct[i][i2][1].isMinusOne() &&
            mstruct[i][i2][0].isRationalPolynomial()) {
          bool b_found = false;
          ...
          if (!b_found)
            divs.addChild(mstruct[i][i2][0]);
          break;
        }
      }
    } else if (mstruct[i].isPower() &&
               (mstruct[i][0].isAddition() || combine_divisions) &&
  ...
  for (size_t i = 0; i < divs.size();) {
    if ((!combine_divisions && divs[i].number().isZero()) ||
        (!eo.assume_denominators_nonzero && !divs[i].representsNonZero(true)) ||
        divs[i].representsZero(true))
      divs.delChild(i + 1);
    else
      i++;
  }
  ...
  for (size_t i = 0; i < mstruct.size(); i++) {
        ...
        if (mstruct[i][i2].isPower() &&
          ...
          for (size_t i3 = 0; i3 < divs.size(); i3++) {
            if (divs[i3] == mstruct[i][i2][0]) {
              ...
              else if (mstruct[i].size() == 2)
                nums[i3].addChild(mstruct[i][i2 == 0 ? 1 : 0]);
              else {
                nums[i3].addChild(mstruct[i]);
                nums[i3][nums[i3].size() - 1].delChild(i2 + 1);
              }
              b = true;
              break;
            }
    ...
    if (!b && combine_divisions) {
      if (mstruct[i].isRationalPolynomial())
        numleft.addChild(mstruct[i]);
      else
        mleft.addChild(mstruct[i]);
    }
  ...
  for (size_t i = 0; i < divs.size(); i++) {
    ...
    if (nums[i].size() > 1 && divs[i].isAddition()) {
      ...
      if (nums[i].isRationalPolynomial() &&
          nums[i].degree(xvar).isLessThan(100) &&
          divs[i].degree(xvar).isLessThan(100)) {
        MathStructure mquo, mrem, ca, cb;
        if (MathStructure::gcd(nums[i], divs[i], mquo, eo2, &ca, &cb, false) &&
            !mquo.isOne()) {
          if (!mquo.representsNonZero(true) &&
              eo.warn_about_denominators_assumed_nonzero &&
              !warn_about_denominators_assumed_nonzero(mquo, eo)) {
            nums[i].clear();
          } else {
            points[i] = 1;
          ...
          if (ca.isOne()) {
            ...
            nums[i].calculateDivide(cb, eo);
                   ...
                   polynomial_long_division(nums[i], divs[i], xvar, mquo, mrem,
                                            eo2, false) &&
                   !mquo.isZero() && mrem != nums[i]) {
          if (mrem.isZero() && !divs[i].representsNonZero(true) &&
              eo.warn_about_denominators_assumed_nonzero &&
              !warn_about_denominators_assumed_nonzero(divs[i], eo)) {
            nums[i].clear();
          } else {
            long int point = 1;
            if (!mrem.isZero())
  ...
  if (b_ready_candidate) {
    ...
    for (size_t i = 0; i < nums.size(); i++) {
      if (!nums[i].isZero() && points[i] < 0)
        nums[i].clear();
    }
  } else {
    // no simplifying polynomial division found; see if result can be combined
    // with other terms
    b = false;
    for (size_t i = 0; i < nums.size(); i++) {
      if (!nums[i].isZero()) {
            ...
            if (!nums[i][i2].contains(divs[i], false, false, false)) {
              ...
              for (size_t i3 = 0; i3 < mtest1.size(); i3++) {
                if (!mtest1[i3].contains(divs[i], false, false, false)) {
                  ...
                  if (ret == 0)
                    ret = mtest2.merge_addition(mtest1[i3], eo);
                  if (ret > 0) {
                    b2 = true;
                    point++;
                    if (mtest2.isZero())
  ...
  for (size_t i = 0; i < mstruct.size();) {
    ...
    if (mstruct[i].isMultiplication()) {
      for (size_t i2 = 0; i2 < mstruct[i].size(); i2++) {
        if (mstruct[i][i2].isPower() && mstruct[i][i2][0].isAddition() &&
            mstruct[i][i2][1].isMinusOne() &&
            mstruct[i][i2][0].isRationalPolynomial()) {
          for (size_t i3 = 0; i3 < divs.size(); i3++) {
            if (divs[i3] == mstruct[i][i2][0]) {
    ...
    } else if (mstruct[i].isPower() && mstruct[i][0].isAddition() &&
               mstruct[i][1].isMinusOne() &&
               mstruct[i][0].isRationalPolynomial()) {
      for (size_t i3 = 0; i3 < divs.size(); i3++) {
        if (divs[i3] == mstruct[i][0]) {
