...
bool MathStructure::decomposeFractions(const MathStructure &x_var,
                                       const EvaluationOptions &eo) {
  MathStructure mtest2;
  bool b = false;
  int mmul_i = 0;
  if (isPower()) {
    if (CHILD(1).isMinusOne() && CHILD(0).isMultiplication() &&
        CHILD(0).size() >= 2) {
      mtest2 = CHILD(0);
      b = true;
    }
    ...
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).isPower() && CHILD(i)[1].isMinusOne() &&
          (CHILD(i)[0].isPower() || CHILD(i)[0].isMultiplication())) {
        mtest2 = CHILD(i)[0];
        b = true;
      } else if (CHILD(i) == x_var) {
        mmul_i = 1;
      } else if (CHILD(i).isPower() && CHILD(i)[0] == x_var &&
                 CHILD(i)[1].isInteger() && CHILD(i)[1].number().isPositive() &&
                 CHILD(i)[1].number().isLessThan(100)) {
  ...
  if (b) {
    if (!mtest2.isMultiplication())
      mtest2.transform(STRUCT_MULTIPLICATION);
    ...
    for (size_t i = 0; i < mtest2.size() && b; i++) {
      ...
      if (mtest2[i].isPower()) {
        if (!mtest2[i][1].isInteger() || !mtest2[i][1].number().isPositive() ||
            mtest2[i][1].isOne() || mtest2[i][1].number().isGreaterThan(100)) {
          b = false;
          break;
        }
        mfactor = mtest2[i][0];
        ...
        for (size_t i2 = 0; i2 < mfactor.size() && b; i2++) {
          if (mfactor[i2].isMultiplication()) {
            ...
            for (size_t i3 = 0; i3 < mfactor[i2].size() && b; i3++) {
              if (!b_x && mfactor[i2][i3].isPower() &&
                  mfactor[i2][i3][0] == x_var) {
                if (!mfactor[i2][i3][1].isInteger() ||
                    !mfactor[i2][i3][1].number().isPositive() ||
                    mfactor[i2][i3][1].isOne() ||
                    mfactor[i2][i3][1].number().isGreaterThan(100)) {
                  b = false;
                } else if (mfactor[i2][i3][1].number().intValue() >
                           i_degrees[i]) {
                ...
                b_x = true;
              } else if (mfactor[i2][i3].containsRepresentativeOf(x_var, true,
                                                                  true) != 0) {
                b = false;
              }
            }
            ...
            if (!mfactor[i2][1].isInteger() ||
                !mfactor[i2][1].number().isPositive() ||
                mfactor[i2][1].isOne() ||
                mfactor[i2][1].number().isGreaterThan(100)) {
              b = false;
            } else if (mfactor[i2][1].number().intValue() > i_degrees[i]) {
              i_degrees[i] = mfactor[i2][1].number().intValue();
            }
          } else if (mfactor[i2] == x_var) {
          } else if (mfactor[i2].containsRepresentativeOf(x_var, true, true) !=
                     0) {
            b = false;
          } else {
            b2 = true;
    ...
    if (b) {
      ...
      for (size_t i = 0; i < mtest2.size(); i++) {
        ...
        if (i_degrees[i] == 1) {
          ...
          if (mx.isLogicalOr())
            mx.setToChild(1);
          if (!mx.isComparison() || mx.comparisonType() != COMPARISON_EQUALS ||
              mx[0] != var) {
            b = false;
            break;
          }
    ...
    if (b) {
      ...
      for (size_t i = 0; i < mtest2.size(); i++) {
        ...
        if (mtest2[i].isPower()) {
          ...
          for (int i_exp_n = mtest2[i][1].number().intValue() -
                             (i_degrees[i] == 1 ? 1 : 0);
               i_exp_n > 0; i_exp_n--) {
            if (i_exp_n == 1) {
            ...
            if (i_exp == i_exp_n) {
              ...
              mfacs.addChild(mtest2);
              if (i_exp - i_exp_n == 1)
                mfacs.last()[i].setToChild(1);
              else
                mfacs.last()[i][1].number() = i_exp - i_exp_n;
            }
            if (i_degrees[i] == 1) {
              UnknownVariable *var =
                  new UnknownVariable("", string("a") + i2s(mtest3.size()));
              var->setAssumptions(new Assumptions());
              ...
              for (int i2 = 1; i2 <= i_degrees[i]; i2++) {
                UnknownVariable *var = new UnknownVariable(
                    "", string("a") + i2s(mtest3.size()) + i2s(i2));
                var->setAssumptions(new Assumptions());
                if (i2 == 1) {
                  ...
                  if (i2 > 2)
                    mnums3.last().last().last() ^= Number(i2 - 1, 1);
                }
                vars.push_back(var);
              }
            }
            if (i_exp != i_exp_n || mtest2.size() > 1)
              mfacs.last() *= mnums3.last();
            else
              mfacs.addChild(mnums3.last());
            b_solve = true;
          }
        ...
        if (b) {
          ...
          for (size_t i = 0; i < m_eqs.size(); i++) {
            for (size_t i2 = 0; i2 < mfac_expand.size();) {
              ...
              bool b_add = false;
              if (i == 0) {
                if (!mfac_expand[i2].contains(x_var))
                  b_add = true;
              } else {
                if (mfac_expand[i2].isMultiplication() &&
                    mfac_expand[i2].size() >= 2) {
                  for (size_t i3 = 0; i3 < mfac_expand[i2].size(); i3++) {
                    if (i == 1 && mfac_expand[i2][i3] == x_var)
                      b_add = true;
                    else if (i > 1 && mfac_expand[i2][i3].isPower() &&
                             mfac_expand[i2][i3][0] == x_var &&
                             mfac_expand[i2][i3][1] == i)
                      b_add = true;
                    if (b_add) {
                  ...
                  if (i == 1 && mfac_expand[i2] == x_var)
                    b_add = true;
                  else if (i > 1 && mfac_expand[i2].isPower() &&
                           mfac_expand[i2][0] == x_var &&
                           mfac_expand[i2][1] == i)
                    b_add = true;
                  if (b_add)
                    mfac_expand[i2].set(1, 1, 0);
                }
              }
              if (b_add) {
                if (m_eqs[i].isZero())
                  m_eqs[i] = mfac_expand[i2];
                else
                  m_eqs[i].add(mfac_expand[i2], true);
          ...
          if (mfac_expand.size() == 0 && m_eqs.size() >= vars.size()) {
            for (size_t i = 0; i < m_eqs.size(); i++) {
              if (!m_eqs[i].isZero()) {
                m_eqs[i].transform(COMPARISON_EQUALS,
                                   i == (size_t)mmul_i ? m_one : m_zero);
              }
            }
            ...
            if (b) {
              ...
              for (size_t i = 0; i < m_eqs.size() && b; i++) {
                for (size_t i2 = 0; i2 < vars_m.size(); i2++) {
                  if (m_eqs[i].isComparison() &&
                      m_eqs[i][0].contains(vars_m[i2], true)) {
                    if (CALCULATOR->aborted() ||
                        m_eqs[i].countTotalChildren() > 1000)
                      return false;
                    m_eqs[i].isolate_x(eo, vars_m[i2]);
                    if (CALCULATOR->aborted() ||
                        m_eqs[i].countTotalChildren() > 10000)
                      return false;
                    m_eqs[i].calculatesub(eo, eo, true);
                    if (m_eqs[i].isLogicalOr())
                      m_eqs[i].setToChild(1);
                    if (m_eqs[i].isComparison() &&
                        m_eqs[i].comparisonType() == COMPARISON_EQUALS &&
                        m_eqs[i][0] == vars_m[i2]) {
                      for (size_t i3 = 0; i3 < m_eqs.size(); i3++) {
                        if (i3 != i) {
                          if (CALCULATOR->aborted())
                            return false;
                          m_eqs[i3][0].calculateReplace(vars_m[i2], m_eqs[i][1],
                                                        eo);
                          if (CALCULATOR->aborted())
                            return false;
                          m_eqs[i3][1].calculateReplace(vars_m[i2], m_eqs[i][1],
                                                        eo);
                        }
                      }
...
bool expand_partial_fractions(MathStructure &m, const EvaluationOptions &eo,
  ...
  if (mtest.isPower() && mtest[1].representsNegative()) {
    ...
    if (!x_var.isUndefined() &&
        mtest[0].factorize(eo, false, 0, 0, false, false, NULL, x_var)) {
    ...
    for (size_t i = 0; i < mtest.size(); i++) {
      if (mtest[i].isPower() && mtest[i][1].isMinusOne() &&
          mtest[i][0].isAddition()) {
        ...
        if (!x_var.isUndefined()) {
          ...
          if (mfac.contains(x_var, true)) {
            ...
            b_poly = polynomial_long_division(mfac, mtest[i][0], x_var, mquo,
                                              mrem, eo, true);
          ...
          if (b_poly && mtest[i][0].factorize(eo, false, 0, 0, false, false,
                                              NULL, x_var)) {
            MathStructure mmul(1, 1, 0);
            while (mtest[i][0].isMultiplication() && mtest[i][0].size() >= 2 &&
                   !mtest[i][0][0].containsRepresentativeOf(x_var, true)) {
              if (mmul.isOne()) {
                mmul = mtest[i][0][0];
