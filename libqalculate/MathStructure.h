...
/**
 * A MathStructure can both be container representing an operation with an
 *ordered list of children or simple value representing a number, , variable
 *etc. The children of a container might be of any type, allowing a tree-like
 *nested structure.
 *
 * These are the most common conatiner/operation types:
 *	- \b Addition: contains two or more children, representing terms
 *(x+y+...)
 *	- \b Multiplication: contains two or more children, representing factors
 *(x*y*...)
 *	- \b Power: contains exactly two children, representing base and
 *exponent (x^y)
 *	- \b Function: contains a two or more children, representing arguments,
 *and a reference to a MathFunction object ( f(x,y,...) )
 *	- \b Comparison: an equality or inequality containing exactly two
 *children, represening the expressions right and left of the sign, specified
 *with a ComparisonType (x=y, x!=y, x&gt;y, ...)
 *	- \b Vector: contains zero or more children, representing elements in a
 *vector ( [x, y, z, ...] )
 *
 * Also available are containers representing logical and bitwise operations.
 * Subtraction is represented by an addition structure with negated children and
 *division by a multiplication structure with inverted children. Matrices is
 *represented by a vector with vectors as children.
 *
 ...
 *	- \b Division: contains exactly two children representing numerator and
 *denominator (x/y)
 *
 * The following value types are available:
 *	- \b Number: has a Number object, representing a rational, floating
 *point, complex or infinite numeric value
 *	- \b Variable: has a reference to a Variable object, with a known or
 *unknown value
 *	- \b Symbolic: has an associated text string, with assumptions about the
 *represented value controlled by the default assumptions
 *	- \b Unit: has a reference to a Unit object, and might in a formatted
 *structure also have a reference to a Prefix object
 *	- \b Undefined: represents an undefined value
 *
 * To create a MathStructure, you can either create a simple structure using the
 *constructors and then expanding it with structural operations, or use the
 *parse or calculation functions of the global Calculator object to convert an
 *expression string.
 *
 ...
 *normally be used. The EvaluationOptions passed to eval() allows much control
 *over the process and the outcome. \code EvaluationOptions eo;
 * mstruct.eval(eo);
 * \endcode
 *
 * After that, to display the result, you should first format the structure
 *using format() and then display it using print(), passing the PrintOptions to
 *both. \code PrintOptions po; mstruct.format(po); std::cout <<
 *mstruct.print(po) << std::endl; \endcode
 *
 * Most low-level functions expect the structure to be unformatted och require
 *that unformat() is called after an expression string has been parsed or
 *format() has been called.
 *
 * To access a child structure either the [] operator or the safer getChild()
 *can be used. Note however that the index passed to the operator start at zero
 *and the index argument for getChild() starts at one. \code MathStructure
 *mstruct(5); mstruct += 2; std::cout << mstruct.print() << std::endl; //
 *output: "5 + 2" std::cout << mstruct.getChild(1)->print() << std::endl; //
 *output: "5" std::cout << mstruct[1].print() << std::endl; // output: "2"
 * \endcode
 *
 * MathStructure uses reference count for management of objects allocated with
 *new. Call ref() when starting to use the object and unref() when done. Note
 *that the reference count is initialized to 1 in the constructors, so ref()
 *should not be called after the object creation. This system is used for all
 *child objects, so the following is perfectly legal: \code MathStructure
 **mchild_p = mstruct->getChild(1); mchild_p->ref(); // mchild_p reference count
 *= 2 mstruct->unref(); //mstruct reference count = 0, mstruct deleted, mchild_p
 *reference count = 1
 * (...)
...
protected:
  ...
  bool isolate_x_sub(const EvaluationOptions &eo, EvaluationOptions &eo2,
                     const MathStructure &x_var, MathStructure *morig = NULL);
  void init();
...
public:
  ...
  /** Create a new numeric structure (value=num/den*10^exp10). Equivalent to
   * MathStructure(Number(num, den, exp10)).
   *
  ...
  /** Create a new numeric structure with floating point value. Uses
   * Number::setFloat().
   *
  ...
  //@{
  /** Set the structure to a copy of another structure. Child structures are
   * copied.
   *
   * @param o The structure to copy.
   * @param merge_precision Preserve the current precision (unless the new value
   * has a lower precision).
   */
  void set(const MathStructure &o, bool merge_precision = false);
  /** Set the structure to a copy of another structure. Pointers to child
  ...
  /** Set the structure to a number (num/den*10^exp10). Equivalent to
   * set(Number(num, den, exp10), precerve_precision).
   *
   ...
   * @param preserve_precision Preserve the current precision (unless the new
   * value has a lower precision).
   */
  void set(int num, int den = 1, int exp10 = 0,
           bool preserve_precision = false);
  void set(long int num, long int den, long int exp10 = 0L,
           bool preserve_precision = false);
   ...
   * @param force_symbol Do not check for undefined or date value.
   */
  void set(std::string sym, bool preserve_precision = false,
  ...
  /** Set the structure to a number with a floating point value. Uses
   * Number::setFloat().
   *
  ...
  /** @name Functions for nested structures (power, muliplication, addition,
   * vector, etc) */
  //@{
  ...
  void setToChild(size_t index, bool merge_precision = false,
                  MathStructure *mparent = NULL, size_t index_this = 1);
  ...
  bool representsUndefined(bool include_children = false,
                           bool include_infinite = false,
                           bool be_strict = false) const;
   ...
   * These operators transforms or adds to the structure without doing any
   * calculations
   */
  //@{
   ...
   * These functions transforms or adds to the structure without doing any
   * calculations
   */
  //@{
  ...
  bool equals(const MathStructure &o, bool allow_interval = false,
              bool allow_infinity = false) const;
  bool equals(const Number &o, bool allow_interval = false,
              bool allow_infinity = false) const;
  ...
  ComparisonResult compareApproximately(
  ...
  bool calculateMergeIndex(size_t index, const EvaluationOptions &eo,
                           const EvaluationOptions &feo,
                           MathStructure *mparent = NULL,
                           size_t index_this = 1);
  bool calculateLogicalOrLast(const EvaluationOptions &eo,
                              bool check_size = true,
                              MathStructure *mparent = NULL,
                              size_t index_this = 1);
  bool calculateLogicalOrIndex(size_t index, const EvaluationOptions &eo,
                               bool check_size = true,
                               MathStructure *mparent = NULL,
                               size_t index_this = 1);
  bool calculateLogicalOr(const MathStructure &mor, const EvaluationOptions &eo,
                          MathStructure *mparent = NULL, size_t index_this = 1);
  bool calculateLogicalXorLast(const EvaluationOptions &eo,
                               MathStructure *mparent = NULL,
                               size_t index_this = 1);
  bool calculateLogicalXor(const MathStructure &mxor,
                           const EvaluationOptions &eo,
                           MathStructure *mparent = NULL,
                           size_t index_this = 1);
  bool calculateLogicalAndLast(const EvaluationOptions &eo,
                               bool check_size = true,
                               MathStructure *mparent = NULL,
                               size_t index_this = 1);
  bool calculateLogicalAndIndex(size_t index, const EvaluationOptions &eo,
                                bool check_size = true,
                                MathStructure *mparent = NULL,
                                size_t index_this = 1);
  bool calculateLogicalAnd(const MathStructure &mand,
                           const EvaluationOptions &eo,
                           MathStructure *mparent = NULL,
                           size_t index_this = 1);
  bool calculateLogicalNot(const EvaluationOptions &eo,
                           MathStructure *mparent = NULL,
                           size_t index_this = 1);
  bool calculateBitwiseNot(const EvaluationOptions &eo,
                           MathStructure *mparent = NULL,
                           size_t index_this = 1);
  ...
  bool calculateRaise(const MathStructure &mexp, const EvaluationOptions &eo,
                      MathStructure *mparent = NULL, size_t index_this = 1);
  bool calculateBitwiseOrLast(const EvaluationOptions &eo,
                              bool check_size = true,
                              MathStructure *mparent = NULL,
                              size_t index_this = 1);
  bool calculateBitwiseOrIndex(size_t index, const EvaluationOptions &eo,
                               bool check_size = true,
                               MathStructure *mparent = NULL,
                               size_t index_this = 1);
  bool calculateBitwiseOr(const MathStructure &mor, const EvaluationOptions &eo,
                          MathStructure *mparent = NULL, size_t index_this = 1);
  bool calculateBitwiseXorLast(const EvaluationOptions &eo,
                               bool check_size = true,
                               MathStructure *mparent = NULL,
                               size_t index_this = 1);
  bool calculateBitwiseXorIndex(size_t index, const EvaluationOptions &eo,
                                bool check_size = true,
                                MathStructure *mparent = NULL,
                                size_t index_this = 1);
  bool calculateBitwiseXor(const MathStructure &mxor,
                           const EvaluationOptions &eo,
                           MathStructure *mparent = NULL,
                           size_t index_this = 1);
  bool calculateBitwiseAndLast(const EvaluationOptions &eo,
                               bool check_size = true,
                               MathStructure *mparent = NULL,
                               size_t index_this = 1);
  bool calculateBitwiseAndIndex(size_t index, const EvaluationOptions &eo,
                                bool check_size = true,
                                MathStructure *mparent = NULL,
                                size_t index_this = 1);
  bool calculateBitwiseAnd(const MathStructure &mand,
                           const EvaluationOptions &eo,
                           MathStructure *mparent = NULL,
                           size_t index_this = 1);
  ...
  bool calculateDivide(const MathStructure &mdiv, const EvaluationOptions &eo,
                       MathStructure *mparent = NULL, size_t index_this = 1);
  bool calculateAddLast(const EvaluationOptions &eo, bool check_size = true,
                        MathStructure *mparent = NULL, size_t index_this = 1);
  bool calculateAddIndex(size_t index, const EvaluationOptions &eo,
                         bool check_size = true, MathStructure *mparent = NULL,
                         size_t index_this = 1);
  ...
  int merge_addition(MathStructure &mstruct, const EvaluationOptions &eo,
                     MathStructure *mparent = NULL, size_t index_this = 1,
                     size_t index_that = 2, bool reversed = false);
  int merge_multiplication(MathStructure &mstruct, const EvaluationOptions &eo,
                           MathStructure *mparent = NULL, size_t index_this = 1,
                           size_t index_that = 2, bool reversed = false,
                           bool do_append = true);
  int merge_power(MathStructure &mstruct, const EvaluationOptions &eo,
                  MathStructure *mparent = NULL, size_t index_this = 1,
                  size_t index_that = 2, bool reversed = false);
  int merge_logical_and(MathStructure &mstruct, const EvaluationOptions &eo,
                        MathStructure *mparent = NULL, size_t index_this = 1,
                        size_t index_that = 2, bool reversed = false);
  int merge_logical_or(MathStructure &mstruct, const EvaluationOptions &eo,
                       MathStructure *mparent = NULL, size_t index_this = 1,
                       size_t index_that = 2, bool reversed = false);
  int merge_logical_xor(MathStructure &mstruct, const EvaluationOptions &eo,
                        MathStructure *mparent = NULL, size_t index_this = 1,
                        size_t index_that = 2, bool reversed = false);
  ...
  bool calculatesub(const EvaluationOptions &eo, const EvaluationOptions &feo,
                    bool recursive = true, MathStructure *mparent = NULL,
                    size_t index_this = 1);
  ...
  //@{
  void sort(const PrintOptions &po = default_print_options,
            bool recursive = true);
  bool
  improve_division_multipliers(const PrintOptions &po = default_print_options,
                               MathStructure *parent = NULL);
  void setPrefixes(const PrintOptions &po = default_print_options,
                   MathStructure *parent = NULL, size_t pindex = 0);
  void prefixCurrencies(const PrintOptions &po = default_print_options);
  void format(const PrintOptions &po = default_print_options);
  void formatsub(const PrintOptions &po = default_print_options,
                 MathStructure *parent = NULL, size_t pindex = 0,
                 bool recursive = true, MathStructure *top_parent = NULL);
  void postFormatUnits(const PrintOptions &po = default_print_options,
                       MathStructure *parent = NULL, size_t pindex = 0);
  bool factorizeUnits();
  void unformat(const EvaluationOptions &eo = default_evaluation_options);
  bool needsParenthesis(const PrintOptions &po, const InternalPrintStruct &ips,
                        const MathStructure &parent, size_t index,
                        bool flat_division = true,
                        bool flat_power = true) const;
  bool removeDefaultAngleUnit(
      const EvaluationOptions &eo = default_evaluation_options);

  int neededMultiplicationSign(const PrintOptions &po,
                               const InternalPrintStruct &ips,
                               const MathStructure &parent, size_t index,
                               bool par, bool par_prev,
                               bool flat_division = true,
                               bool flat_power = true) const;

  std::string print(const PrintOptions &po = default_print_options,
                    const InternalPrintStruct &ips = top_ips) const;
  std::string print(const PrintOptions &po, bool format, int colorize = 0,
  ...
  MathStructure &getArea(size_t r1, size_t c1, size_t r2, size_t c2,
                         MathStructure &mstruct) const;
  ...
  int gaussianElimination(
      const EvaluationOptions &eo = default_evaluation_options,
      bool det = false);
  MathStructure &determinant(MathStructure &mstruct,
                             const EvaluationOptions &eo) const;
  MathStructure &permanent(MathStructure &mstruct,
                           const EvaluationOptions &eo) const;
  ...
  MathStructure &cofactor(size_t r, size_t c, MathStructure &mstruct,
                          const EvaluationOptions &eo) const;
  //@}
  ...
  bool syncUnits(bool sync_nonlinear_relations = false,
                 bool *found_nonlinear_relations = NULL,
                 bool calculate_new_functions = false,
                 const EvaluationOptions &feo = default_evaluation_options);
  ...
  bool
  convertToBaseUnits(bool convert_nonlinear_relations = false,
                     bool *found_nonlinear_relations = NULL,
                     bool calculate_new_functions = false,
                     const EvaluationOptions &feo = default_evaluation_options,
                     bool avoid_approximate_variables = false);
  bool convert(Unit *u, bool convert_nonlinear_relations = false,
               bool *found_nonlinear_relations = NULL,
               bool calculate_new_functions = false,
               const EvaluationOptions &feo = default_evaluation_options,
               Prefix *new_prefix = NULL);
  bool convert(const MathStructure unit_mstruct,
               bool convert_nonlinear_relations = false,
               bool *found_nonlinear_relations = NULL,
               bool calculate_new_functions = false,
               const EvaluationOptions &feo = default_evaluation_options);
  //@}
  ...
  //@{
  int contains(const MathStructure &mstruct, bool structural_only = true,
               bool check_variables = false, bool check_functions = false,
               bool loose_equals = false) const;
  size_t countOccurrences(const MathStructure &mstruct) const;
  size_t countOccurrences(const MathStructure &mstruct,
                          bool check_variables) const;
  int containsRepresentativeOf(const MathStructure &mstruct,
                               bool check_variables = false,
                               bool check_functions = false) const;
  int containsType(StructureType mtype, bool structural_only = true,
                   bool check_variables = false,
                   bool check_functions = false) const;
  int containsRepresentativeOfType(StructureType mtype,
                                   bool check_variables = false,
                                   bool check_functions = false) const;
  int containsFunction(MathFunction *f, bool structural_only = true,
                       bool check_variables = false,
                       bool check_functions = false) const;
  int containsFunctionId(int id, bool structural_only = true,
                         bool check_variables = false,
                         bool check_functions = false) const;
  int containsInterval(bool structural_only = true,
                       bool check_variables = false,
                       bool check_functions = false,
                       int ignore_high_precision_interval = 0,
                       bool include_interval_function = false) const;
  int containsInfinity(bool structural_only = true,
                       bool check_variables = false,
                       bool check_functions = false) const;
  ...
  bool replace(const MathStructure &mfrom, const MathStructure &mto,
               bool once_only = false, bool exclude_function_arguments = false);
  bool replace(const MathStructure &mfrom, const MathStructure &mto,
               bool once_only, bool exclude_function_arguments,
               bool replace_in_variables);
  bool replace(Variable *v, const MathStructure &mto);
  bool calculateReplace(const MathStructure &mfrom, const MathStructure &mto,
                        const EvaluationOptions &eo,
                        bool exclude_function_arguments = false);
  bool replace(const MathStructure &mfrom1, const MathStructure &mto1,
               const MathStructure &mfrom2, const MathStructure &mto2);
  bool removeType(StructureType mtype);
  //@}

  /** @name Functions to generate vectors for plotting */
  //@{
  MathStructure generateVector(
      MathStructure x_mstruct, const MathStructure &min,
      const MathStructure &max, int steps, MathStructure *x_vector = NULL,
      const EvaluationOptions &eo = default_evaluation_options) const;
  MathStructure generateVector(
      MathStructure x_mstruct, const MathStructure &min,
      const MathStructure &max, const MathStructure &step,
      MathStructure *x_vector = NULL,
      const EvaluationOptions &eo = default_evaluation_options) const;
  MathStructure generateVector(
      MathStructure x_mstruct, const MathStructure &x_vector,
      const EvaluationOptions &eo = default_evaluation_options) const;
  //@}
  ...
  bool integrate(const MathStructure &lower_limit,
                 const MathStructure &upper_limit,
                 const MathStructure &x_var_pre,
                 const EvaluationOptions &eo = default_evaluation_options,
                 bool force_numerical = false, bool simplify_first = true);
  int integrate(const MathStructure &x_var, const EvaluationOptions &eo,
                bool simplify_first = true, int use_abs = 1,
                bool definite_integral = false, bool try_abs = true,
                int max_part_depth = 5,
                std::vector<MathStructure *> *parent_parts = NULL);
  //@}

  /** @name Functions for polynomials */
  //@{
  bool expand(const EvaluationOptions &eo = default_evaluation_options,
              bool unfactorize = true);
  ...
  bool factorize(const EvaluationOptions &eo = default_evaluation_options,
                 bool unfactorize = true, int term_combination_levels = 0,
                 int max_msecs = 1000, bool only_integers = true,
                 int recursive = 1, struct timeval *endtime_p = NULL,
                 const MathStructure &force_factorization = m_undefined,
                 bool complete_square = false, bool only_sqrfree = false,
                 int max_degree_factor = -1);
  bool expandPartialFractions(const EvaluationOptions &eo);
  bool structure(StructuringMode structuring, const EvaluationOptions &eo,
                 bool restore_first = true);
  /** If the structure represents a rational polynomial.
   * This is true for
   *	- rational numbers;
   *	- functions, units, variables and symbols that do not represent a matrix
   *or undefined;
   *	- a power with a positive integer exponent and any of the previous as
   *base;
   ...
   */
  bool isRationalPolynomial(bool allow_non_rational_coefficient = false,
                            bool allow_interval_coefficient = false) const;
  ...
  void coefficient(const MathStructure &xvar, const Number &pownr,
                   MathStructure &mcoeff) const;
  Number maxCoefficient();
  int polynomialUnit(const MathStructure &xvar) const;
  void polynomialContent(const MathStructure &xvar, MathStructure &mcontent,
                         const EvaluationOptions &eo) const;
  void polynomialPrimpart(const MathStructure &xvar, MathStructure &mprim,
                          const EvaluationOptions &eo) const;
  void polynomialPrimpart(const MathStructure &xvar, const MathStructure &c,
                          MathStructure &mprim,
                          const EvaluationOptions &eo) const;
  void polynomialUnitContentPrimpart(const MathStructure &xvar, int &munit,
                                     MathStructure &mcontent,
                                     MathStructure &mprim,
                                     const EvaluationOptions &eo) const;
  //@}
  ...
  bool calculateLimit(const MathStructure &x_var, const MathStructure &limit,
                      const EvaluationOptions &eo_pre,
                      int approach_direction = 0);

  bool decomposeFractions(const MathStructure &x_var,
                          const EvaluationOptions &eo);

  static bool polynomialDivide(const MathStructure &mnum,
                               const MathStructure &mden,
                               MathStructure &mquotient,
                               const EvaluationOptions &eo,
                               bool check_args = true);
  static bool
  polynomialQuotient(const MathStructure &mnum, const MathStructure &mden,
                     const MathStructure &xvar, MathStructure &mquotient,
                     const EvaluationOptions &eo, bool check_args = true);
  static bool lcm(const MathStructure &m1, const MathStructure &m2,
                  MathStructure &mlcm, const EvaluationOptions &eo,
                  bool check_args = true);
  ...
  bool isolate_x(const EvaluationOptions &eo,
                 const MathStructure &x_var = m_undefined,
                 bool check_result = false);
  bool isolate_x(const EvaluationOptions &eo, const EvaluationOptions &feo,
                 const MathStructure &x_var = m_undefined,
                 bool check_result = false);
  //@}
