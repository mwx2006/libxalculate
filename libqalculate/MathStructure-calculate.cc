...
bool warn_test_interval(MathStructure &mnonzero, const EvaluationOptions &eo2) {
  if (mnonzero.isComparison() && mnonzero[0].isVariable() &&
      !mnonzero[0].variable()->isKnown() &&
      !((UnknownVariable *)mnonzero[0].variable())->interval().isUndefined()) {
    /*if(((UnknownVariable*) mnonzero[0].variable())->interval().isNumber() &&
    mnonzero[1].isNumber()) { MathStructure mbak(mnonzero); mnonzero[0] =
    ((UnknownVariable*) mnonzero[0].variable())->interval(); mnonzero.eval(eo2);
            if(!mnonzero.isNumber()) mnonzero.clear();
            return true;
    } else*/
    if (((UnknownVariable *)mnonzero[0].variable())
            ->interval()
            .containsInterval(true)) {
      MathStructure mbak(mnonzero);
      mnonzero[0] = ((UnknownVariable *)mnonzero[0].variable())->interval();
      mnonzero.eval(eo2);
      if (mnonzero.isComparison())
        mnonzero = mbak;
      else
        return true;
    } else {
    ...
    for (size_t i = 0; i < mnonzero.size(); i++) {
      if (warn_test_interval(mnonzero[i], eo2))
        b_ret = true;
    }
    if (b_ret) {
...
bool warn_about_assumed_not_value(const MathStructure &mstruct,
                                  const MathStructure &mvalue,
  ...
  if (mnonzero.isOne())
    return true;
  if (mvalue.isZero() && mnonzero.isComparison() &&
      mnonzero.comparisonType() == COMPARISON_NOT_EQUALS &&
      mnonzero[1].isZero() && mnonzero[0].representsApproximatelyZero(true))
    return false;
  CALCULATOR->error(false, _("Required assumption: %s."),
                    format_and_print(mnonzero).c_str(), NULL);
  return true;
}
bool warn_about_denominators_assumed_nonzero(const MathStructure &mstruct,
                                             const EvaluationOptions &eo) {
  CALCULATOR->beginTemporaryStopMessages();
  EvaluationOptions eo2 = eo;
  eo2.assume_denominators_nonzero = false;
  ...
  if (mnonzero.isComparison() &&
    ...
    return false;
  CALCULATOR->error(
      false, _("To avoid division by zero, the following must be true: %s."),
      format_and_print(mnonzero).c_str(), NULL);
  return true;
}
bool warn_about_denominators_assumed_nonzero_or_positive(
    const MathStructure &mstruct, const MathStructure &mstruct2,
    const EvaluationOptions &eo) {
  CALCULATOR->beginTemporaryStopMessages();
  EvaluationOptions eo2 = eo;
  eo2.assume_denominators_nonzero = false;
...
bool warn_about_denominators_assumed_nonzero_llgg(const MathStructure &mstruct,
                                                  const MathStructure &mstruct2,
                                                  const MathStructure &mstruct3,
                                                  const EvaluationOptions &eo) {
  CALCULATOR->beginTemporaryStopMessages();
  EvaluationOptions eo2 = eo;
  eo2.assume_denominators_nonzero = false;
...
int MathStructure::merge_addition(MathStructure &mstruct,
                                  const EvaluationOptions &eo,
  ...
  if (mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    // both terms are numbers try Number::add() (might never fail for infinite
    // values)
    Number nr(o_number);
    if (nr.add(mstruct.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || mstruct.number().isApproximate())) {
      if (o_number == nr) {
        o_number = nr;
        numberUpdated();
        return 2;
  ...
  if (representsUndefined() || mstruct.representsUndefined())
    return -1;
  switch (m_type) {
  case STRUCT_VECTOR: {
    switch (mstruct.type()) {
    ...
    case STRUCT_VECTOR: {
      ...
      if (!b1 && !representsNonMatrix())
        return -1;
      if (!b2 && !mstruct.representsNonMatrix())
        return -1;
      if (b2 && mstruct.columns() == 1 &&
          ((!b1 && SIZE > 0) || (b1 && SIZE == mstruct.size()))) {
        if (!b1) {
        ...
        return 1;
      } else if (b1 && columns() == 1 &&
                 ((!b2 && mstruct.size() > 0) ||
                  (b2 && SIZE == mstruct.size()))) {
        if (!b2) {
          for (size_t i = 0; i < SIZE; i++) {
  ...
  case STRUCT_MULTIPLICATION: {
    ...
    case STRUCT_MULTIPLICATION: {
      ...
      if (SIZE - i1 == mstruct.size() - i2) {
        ...
        if (b) {
          ...
          if (i2 == 0) {
            ...
            if (!CHILD(0).number().add(mstruct[0].number()))
              return -1;
          }
          MERGE_APPROX_AND_PREC(mstruct)
      ...
      for (size_t i2 = 0; i2 < SIZE; i2++) {
        if (eo.transform_trigonometric_functions && CHILD(i2).isPower() &&
            CHILD(i2)[0].isFunction() &&
            (CHILD(i2)[0].function()->id() == FUNCTION_ID_COS ||
             CHILD(i2)[0].function()->id() == FUNCTION_ID_SIN) &&
            CHILD(i2)[0].size() == 1 && CHILD(i2)[1].isNumber() &&
            CHILD(i2)[1].number().isTwo() && mstruct.size() > 0) {
          if (!FUNCTION_PROTECTED(eo, CHILD(i2)[0].function()->id() ==
                                              FUNCTION_ID_SIN
                                          ? FUNCTION_ID_COS
                                          : FUNCTION_ID_SIN)) {
            for (size_t i = 0; i < mstruct.size(); i++) {
              if (mstruct[i].isPower() && mstruct[i][0].isFunction() &&
                  mstruct[i][0].function()->id() ==
                      (CHILD(i2)[0].function()->id() == FUNCTION_ID_SIN
                           ? FUNCTION_ID_COS
                           : FUNCTION_ID_SIN) &&
                  mstruct[i][0].size() == 1 && mstruct[i][1].isNumber() &&
                  mstruct[i][1].number().isTwo() &&
                  CHILD(i2)[0][0] == mstruct[i][0][0]) {
                ...
                if (CHILD(i2)[0].function()->id() == FUNCTION_ID_SIN) {
                  madd = madd2;
                  madd2.calculateNegate(eo);
                } else {
                  madd = madd1;
                  madd1.calculateNegate(eo);
                }
          ...
          if (eo.protected_function != CHILD(i2)[0].function()) {
            ...
            if (mstruct[0].isNumber()) {
              if (CHILD(0).isNumber()) {
                if (mstruct.size() == SIZE - 1 &&
                    CHILD(0).number() == -mstruct[0].number()) {
                  b = true;
                  for (size_t i = 1; i < mstruct.size(); i++) {
                    if (!mstruct[i].equals(CHILD(i2 > i ? i : i + 1))) {
                      b = false;
                      break;
                    }
                  }
                ...
                b = true;
                for (size_t i = 1; i < mstruct.size(); i++) {
                  if (!mstruct[i].equals(CHILD(i2 >= i ? i - 1 : i))) {
                    b = false;
                    break;
                  }
                }
              ...
              b = true;
              for (size_t i = 0; i < mstruct.size(); i++) {
                if (!mstruct[i].equals(CHILD(i2 > i + 1 ? i + 1 : i + 2))) {
                  b = false;
                  break;
                }
              }
            ...
            if (b) {
              CHILD(i2)
              [0].setFunctionId(CHILD(i2)[0].function()->id() == FUNCTION_ID_COS
                                    ? FUNCTION_ID_SIN
                                    : FUNCTION_ID_COS);
              MERGE_APPROX_AND_PREC(mstruct)
        ...
        } else if (eo.transform_trigonometric_functions &&
                   CHILD(i2).isPower() && CHILD(i2)[0].isFunction() &&
                   (CHILD(i2)[0].function()->id() == FUNCTION_ID_COSH ||
                    CHILD(i2)[0].function()->id() == FUNCTION_ID_SINH) &&
                   CHILD(i2)[0].size() == 1 && CHILD(i2)[1].isNumber() &&
                   CHILD(i2)[1].number().isTwo() && mstruct.size() > 0) {
          if (!FUNCTION_PROTECTED(eo, CHILD(i2)[0].function()->id() ==
                                              FUNCTION_ID_SINH
            ...
            for (size_t i = 0; i < mstruct.size(); i++) {
              if (mstruct[i].isPower() && mstruct[i][0].isFunction() &&
                ...
                madd1.delChild(i2 + 1, true);
                madd2.delChild(i + 1, true);
                if (mstruct[i][0].function()->id() == FUNCTION_ID_SINH)
                  madd = madd1;
                else
                  madd = madd2;
                if (madd1.calculateAdd(madd2, eo)) {
          ...
          if (eo.protected_function != CHILD(i2)[0].function()) {
            if (CHILD(i2)[0].function()->id() == FUNCTION_ID_SINH &&
                mstruct.size() == SIZE - 1) {
          ...
          if (CHILD(i2).function()->id() == FUNCTION_ID_SIGNUM &&
              !FUNCTION_PROTECTED(eo, FUNCTION_ID_SIGNUM) &&
              CHILD(i2).size() == 2 && CHILD(i2)[0].isAddition() &&
              CHILD(i2)[0].size() == 2 && CHILD(i2)[0].representsReal(true)) {
            ...
            for (size_t im = 0; im < mstruct.size(); im++) {
              if (mstruct[im] == CHILD(i2)) {
                ...
                if ((m1 == CHILD(i2)[0][0] && m2 == CHILD(i2)[0][1]) ||
                    (m2 == CHILD(i2)[0][0] && m1 == CHILD(i2)[0][1])) {
                  SET_CHILD_MAP(i2)
          ...
          } else if (CHILD(i2).function()->id() == FUNCTION_ID_ASIN ||
                     CHILD(i2).function()->id() == FUNCTION_ID_ACOS) {
            CHILD(i2).setFunctionId(CHILD(i2).function()->id() ==
                                            FUNCTION_ID_ASIN
                                        ? FUNCTION_ID_ACOS
                                        : FUNCTION_ID_ASIN);
            if (equals(mstruct) && !FUNCTION_PROTECTED(eo, FUNCTION_ID_ASIN) &&
                !FUNCTION_PROTECTED(eo, FUNCTION_ID_ACOS)) {
              // asin(x)+acos(x)=pi/2
              delChild(i2 + 1, true);
              if (DEFAULT_RADIANS(eo.parse_options.angle_unit)) {
                calculateMultiply(CALCULATOR->getVariableById(VARIABLE_ID_PI),
                                  eo);
            ...
            CHILD(i2).setFunctionId(CHILD(i2).function()->id() ==
                     ...
                     CHILD(i2).function()->id() == FUNCTION_ID_COSH) {
            CHILD(i2).setFunctionId(CHILD(i2).function()->id() ==
                                            FUNCTION_ID_SINH
                                        ? FUNCTION_ID_COSH
                                        : FUNCTION_ID_SINH);
            if (equals(mstruct) && !FUNCTION_PROTECTED(eo, FUNCTION_ID_SINH) &&
                !FUNCTION_PROTECTED(eo, FUNCTION_ID_COSH)) {
              // sinh(x)+cosh(x)=e^x
              MathStructure *mexp = &CHILD(i2)[0];
              mexp->ref();
              delChild(i2 + 1, true);
              MathStructure *mmul =
                  new MathStructure(CALCULATOR->getVariableById(VARIABLE_ID_E));
      ...
      if (eo.transform_trigonometric_functions) {
        for (size_t i2 = 0; i2 < mstruct.size(); i2++) {
          if (mstruct[i2].isPower() && mstruct[i2][0].isFunction() &&
              (mstruct[i2][0].function()->id() == FUNCTION_ID_COS ||
               mstruct[i2][0].function()->id() == FUNCTION_ID_SIN) &&
              mstruct[i2][0].size() == 1 && mstruct[i2][1].isNumber() &&
              mstruct[i2][1].number().isTwo() && SIZE > 0) {
            // ay*sin(x)^2-ay=-ay*cos(x)^2, ay*cos(x)^2-ay=-ay*sin(x)^2
            if (eo.protected_function != mstruct[i2][0].function()) {
              bool b = false;
              if (CHILD(0).isNumber()) {
                if (mstruct[0].isNumber()) {
                  if (mstruct.size() - 1 == SIZE &&
                      CHILD(0).number() == -mstruct[0].number()) {
                    b = true;
                    for (size_t i = 1; i < SIZE; i++) {
                      if (!CHILD(i).equals(mstruct[i2 > i ? i : i + 1])) {
                        b = false;
                        break;
                      }
                    }
                  ...
                  b = true;
                  for (size_t i = 1; i < SIZE; i++) {
                    if (!CHILD(i).equals(mstruct[i2 >= i ? i - 1 : i])) {
                      b = false;
                      break;
                    }
                  }
                }
              } else if (mstruct.size() - 2 == SIZE &&
                         mstruct[0].isMinusOne()) {
                b = true;
                for (size_t i = 0; i < SIZE; i++) {
                  if (!CHILD(i).equals(mstruct[i2 > i + 1 ? i + 1 : i + 2])) {
                    b = false;
                    break;
                  }
                }
              ...
              if (b) {
                mstruct[i2][0].setFunctionId(mstruct[i2][0].function()->id() ==
                                                     FUNCTION_ID_COS
                                                 ? FUNCTION_ID_SIN
                                                 : FUNCTION_ID_COS);
          ...
          } else if (mstruct[i2].isPower() && mstruct[i2][0].isFunction() &&
                     (mstruct[i2][0].function()->id() == FUNCTION_ID_COSH ||
                      mstruct[i2][0].function()->id() == FUNCTION_ID_SINH) &&
                     mstruct[i2][0].size() == 1 && mstruct[i2][1].isNumber() &&
                     mstruct[i2][1].number().isTwo() && SIZE > 0) {
            if (eo.protected_function != mstruct[i2][0].function()) {
              if (mstruct[i2][0].function()->id() == FUNCTION_ID_SINH &&
                  mstruct.size() - 1 == SIZE) {
                // ay*sinh(x)^2+ay=ay*cosh(x)^2
                for (size_t i = 0; i < SIZE; i++) {
                  if (!CHILD(i).equals(mstruct[i2 > i ? i : i + 1]))
    ...
    case STRUCT_POWER: {
      ...
      if (eo.transform_trigonometric_functions && SIZE == 2 &&
          CHILD(0).isNumber() && mstruct[0].isFunction()) {
        if ((mstruct[0].function()->id() == FUNCTION_ID_COS ||
             mstruct[0].function()->id() == FUNCTION_ID_SIN) &&
            !FUNCTION_PROTECTED(eo,
                                mstruct[0].function()->id() == FUNCTION_ID_SIN
                                    ? FUNCTION_ID_COS
                                    : FUNCTION_ID_SIN) &&
            mstruct[0].size() == 1 && mstruct[1].isNumber() &&
            mstruct[1].number().isTwo()) {
          if (CHILD(1).isPower() && CHILD(1)[0].isFunction() &&
              CHILD(1)[0].function()->id() ==
                  (mstruct[0].function()->id() == FUNCTION_ID_SIN
                       ? FUNCTION_ID_COS
                       : FUNCTION_ID_SIN) &&
              CHILD(1)[0].size() == 1 && CHILD(1)[1].isNumber() &&
              CHILD(1)[1].number().isTwo() && mstruct[0][0] == CHILD(1)[0][0]) {
            // a*sin(x)^2+cos(x)^2=1+(a-1)*sin(x),
            // a*cos(x)^2+sin(x)^2=1+(a-1)*cos(x)
        ...
        } else if ((mstruct[0].function()->id() == FUNCTION_ID_COSH ||
                    mstruct[0].function()->id() == FUNCTION_ID_SINH) &&
                   !FUNCTION_PROTECTED(eo, mstruct[0].function()->id() ==
                                                   FUNCTION_ID_SINH
                                               ? FUNCTION_ID_COSH
                                               : FUNCTION_ID_SINH) &&
                   mstruct[0].size() == 1 && mstruct[1].isNumber() &&
          ...
          if (CHILD(1).isPower() && CHILD(1)[0].isFunction() &&
              ...
              CHILD(1)[0].size() == 1 && CHILD(1)[1].isNumber() &&
              CHILD(1)[1].number().isTwo() && mstruct[0][0] == CHILD(1)[0][0]) {
            // a*sinh(x)^2+cosh(x)^2=1+(a+1)*sinh(x),
            // a*cosh(x)^2+sinh(x)^2=(a+1)*cosh(x)-1
            if (CHILD(0).calculateAdd(m_one, eo)) {
              add(CHILD(1)[0].function()->id() == FUNCTION_ID_SINH
                      ? m_one
                      : m_minus_one);
              MERGE_APPROX_AND_PREC(mstruct)
              return 1;
    ...
    default: {
      ...
      if (eo.transform_trigonometric_functions && SIZE == 2 &&
          CHILD(1).isPower() && CHILD(1)[0].isFunction() &&
          CHILD(1)[0].function() != eo.protected_function &&
          CHILD(1)[1] == nr_two && CHILD(1)[0].size() == 1) {
        if (mstruct.isNumber() && CHILD(0).isNumber()) {
          if (CHILD(1)[0].function()->id() == FUNCTION_ID_SIN &&
              mstruct.number() == -CHILD(0).number()) {
            // a*sin(x)^2-a=a*cos(x)^2
            CHILD(1)[0].setFunctionId(FUNCTION_ID_COS);
            MERGE_APPROX_AND_PREC(mstruct)
            calculateNegate(eo, mparent, index_this);
            return 1;
          } else if (CHILD(1)[0].function()->id() == FUNCTION_ID_COS &&
                     mstruct.number() == -CHILD(0).number()) {
            ...
            calculateNegate(eo, mparent, index_this);
            return 1;
          } else if (CHILD(1)[0].function()->id() == FUNCTION_ID_COSH &&
                     mstruct.number() == -CHILD(0).number()) {
            // a*cosh(x)^2-a=a*sinh(x)^2
            MERGE_APPROX_AND_PREC(mstruct)
            CHILD(1)[0].setFunctionId(FUNCTION_ID_SINH);
            return 1;
          } else if (CHILD(1)[0].function()->id() == FUNCTION_ID_SINH &&
                     mstruct == CHILD(0)) {
      ...
      } else if (eo.transform_trigonometric_functions && SIZE == 3 &&
                 CHILD(0).isMinusOne()) {
        size_t i = 0;
        if (CHILD(1).isPower() && CHILD(1)[0].isFunction() &&
            (CHILD(1)[0].function()->id() == FUNCTION_ID_SIN ||
             CHILD(1)[0].function()->id() == FUNCTION_ID_COS ||
             CHILD(1)[0].function()->id() == FUNCTION_ID_COSH) &&
            CHILD(1)[0].function() != eo.protected_function &&
            CHILD(1)[1] == nr_two && CHILD(1)[0].size() == 1 &&
            CHILD(2) == mstruct)
          i = 1;
        if (CHILD(2).isPower() && CHILD(2)[0].isFunction() &&
            (CHILD(2)[0].function()->id() == FUNCTION_ID_SIN ||
             CHILD(2)[0].function()->id() == FUNCTION_ID_COS ||
             CHILD(2)[0].function()->id() == FUNCTION_ID_COSH) &&
            CHILD(2)[0].function() != eo.protected_function &&
            CHILD(2)[1] == nr_two && CHILD(2)[0].size() == 1 &&
            CHILD(1) == mstruct)
          i = 2;
        if (i > 0) {
      ...
      if (mstruct.isDateTime() ||
          (mstruct.isFunction() &&
           mstruct.function()->id() == FUNCTION_ID_SIGNUM &&
           !FUNCTION_PROTECTED(eo, FUNCTION_ID_SIGNUM))) {
        // try again with reversed order
        return 0;
      }
  ...
  case STRUCT_POWER: {
    if (CHILD(0).isFunction() &&
        (CHILD(0).function()->id() == FUNCTION_ID_COS ||
         CHILD(0).function()->id() == FUNCTION_ID_SIN) &&
        eo.protected_function != CHILD(0).function() && CHILD(0).size() == 1 &&
        CHILD(1).isNumber() && CHILD(1).number().isTwo()) {
      if (eo.transform_trigonometric_functions && mstruct.isMinusOne()) {
        // sin(x)^2-1=-cos(x)^2, cos(x)^2-1=-sin(x)^2
        CHILD(0).setFunctionId(CHILD(0).function()->id() == FUNCTION_ID_SIN
                                   ? FUNCTION_ID_COS
                                   : FUNCTION_ID_SIN);
        negate();
        MERGE_APPROX_AND_PREC(mstruct)
        return 1;
      } else if (mstruct.isPower() && mstruct[0].isFunction() &&
                 mstruct[0].function()->id() ==
                     (CHILD(0).function()->id() == FUNCTION_ID_SIN
                          ? FUNCTION_ID_COS
                          : FUNCTION_ID_SIN) &&
                 eo.protected_function != mstruct[0].function() &&
                 mstruct[0].size() == 1 && mstruct[1].isNumber() &&
                 mstruct[1].number().isTwo() && CHILD(0)[0] == mstruct[0][0]) {
        // cos(x)^2+sin(x)^2=1
        set(1, 1, 0, true);
        MERGE_APPROX_AND_PREC(mstruct)
        return 1;
      }
    } else if (eo.transform_trigonometric_functions && CHILD(0).isFunction() &&
               (CHILD(0).function()->id() == FUNCTION_ID_COSH ||
                CHILD(0).function()->id() == FUNCTION_ID_SINH) &&
               eo.protected_function != CHILD(0).function() &&
               CHILD(0).size() == 1 && CHILD(1).isNumber() &&
               CHILD(1).number().isTwo()) {
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_SIGNUM && mstruct.isMultiplication() &&
        !FUNCTION_PROTECTED(eo, FUNCTION_ID_SIGNUM)) {
      if ((SIZE == 1 || SIZE == 2) && CHILD(0).isAddition() &&
          CHILD(0).size() == 2 &&
          (CHILD(0)[0].isOne() || CHILD(0)[1].isOne()) &&
          CHILD(0).representsReal(true)) {
        ...
        for (size_t im = 0; im < mstruct.size(); im++) {
          if (mstruct[im] == *this) {
            ...
            if ((mstruct.size() == 1 &&
                 mstruct[0] == CHILD(0)[CHILD(0)[0].isOne() ? 1 : 0]) ||
                (mstruct.size() > 1 &&
                 mstruct == CHILD(0)[CHILD(0)[0].isOne() ? 1 : 0])) {
              mchild->unref();
              setFunctionId(FUNCTION_ID_ABS);
              if (SIZE == 2) {
                ERASE(1);
            ...
            mstruct.insertChild_nocopy(mchild, im + 1);
               ...
               ((o_function->id() == FUNCTION_ID_ASIN &&
                 mstruct.function()->id() == FUNCTION_ID_ACOS) ||
                (o_function->id() == FUNCTION_ID_ACOS &&
                 mstruct.function()->id() == FUNCTION_ID_ASIN)) &&
               !FUNCTION_PROTECTED(eo, FUNCTION_ID_ACOS) &&
               !FUNCTION_PROTECTED(eo, FUNCTION_ID_ASIN) && SIZE == 1 &&
               mstruct.size() == 1 && CHILD(0) == mstruct[0]) {
      // asin(x)+acos(x)=pi/2
      set_fraction_of_turn(*this, eo, 1, 4);
      MERGE_APPROX_AND_PREC(mstruct)
      return 1;
    } else if (mstruct.isFunction() &&
               ((o_function->id() == FUNCTION_ID_SINH &&
                 mstruct.function()->id() == FUNCTION_ID_COSH) ||
                (o_function->id() == FUNCTION_ID_COSH &&
                 mstruct.function()->id() == FUNCTION_ID_SINH)) &&
               !FUNCTION_PROTECTED(eo, FUNCTION_ID_COSH) &&
               !FUNCTION_PROTECTED(eo, FUNCTION_ID_SINH) && SIZE == 1 &&
               mstruct.size() == 1 && CHILD(0) == mstruct[0]) {
      // sinh(x)+cosh(x)=e^x
      ...
      return 1;
               ...
               o_function->id() == FUNCTION_ID_STRIP_UNITS &&
               mstruct.function()->id() == FUNCTION_ID_STRIP_UNITS &&
               mstruct.size() == 1 && SIZE == 1) {
    ...
    if (mstruct.isDateTime()) {
      ...
      if (o_datetime->add(*mstruct.datetime())) {
        ...
        return 1;
               ...
               mstruct[0].isMinusOne() && mstruct[1].isDateTime() &&
               (CALCULATOR->getUnitById(UNIT_ID_SECOND) ||
                CALCULATOR->getUnitById(UNIT_ID_DAY))) {
      if (CALCULATOR->getUnitById(UNIT_ID_DAY) &&
          !mstruct[1].datetime()->timeIsSet() && !o_datetime->timeIsSet()) {
      ...
      MERGE_APPROX_AND_PREC(mstruct)
      return 1;
    } else if (CALCULATOR->getUnitById(UNIT_ID_SECOND) &&
               ((mstruct.isUnit() &&
                 mstruct.unit()->baseUnit() ==
                     CALCULATOR->getUnitById(UNIT_ID_SECOND) &&
                 mstruct.unit()->baseExponent() == 1 &&
                 !mstruct.unit()->hasNonlinearRelationTo(
                     CALCULATOR->getUnitById(UNIT_ID_SECOND))) ||
                (mstruct.isMultiplication() && mstruct.size() == 2 &&
                 mstruct[0].isNumber() && mstruct[0].number().isReal() &&
                 !mstruct[0].number().isInterval() && mstruct[1].isUnit() &&
                 mstruct[1].unit()->baseUnit() ==
                     CALCULATOR->getUnitById(UNIT_ID_SECOND) &&
                 mstruct[1].unit()->baseExponent() == 1 &&
                 !mstruct[1].unit()->hasNonlinearRelationTo(
                     CALCULATOR->getUnitById(UNIT_ID_SECOND))))) {
      ...
      if (CALCULATOR->getUnitById(UNIT_ID_MONTH) &&
          u != CALCULATOR->getUnitById(UNIT_ID_YEAR) &&
          (u == CALCULATOR->getUnitById(UNIT_ID_MONTH) ||
           u->isChildOf(CALCULATOR->getUnitById(UNIT_ID_MONTH)))) {
        // date + a*month
      ...
      } else if (CALCULATOR->getUnitById(UNIT_ID_YEAR) &&
                 (u == CALCULATOR->getUnitById(UNIT_ID_YEAR) ||
                  u->isChildOf(CALCULATOR->getUnitById(UNIT_ID_YEAR)))) {
        // date + a*year
      ...
      } else if (CALCULATOR->getUnitById(UNIT_ID_DAY) &&
                 (u == CALCULATOR->getUnitById(UNIT_ID_DAY) ||
                  u->isChildOf(CALCULATOR->getUnitById(UNIT_ID_DAY)))) {
        // date + a*day
      ...
      } else if (CALCULATOR->getUnitById(UNIT_ID_HOUR) &&
                 (u == CALCULATOR->getUnitById(UNIT_ID_HOUR) ||
                  u->isChildOf(CALCULATOR->getUnitById(UNIT_ID_HOUR)))) {
        // date + a*hour
      ...
      } else if (CALCULATOR->getUnitById(UNIT_ID_MINUTE) &&
                 (u == CALCULATOR->getUnitById(UNIT_ID_MINUTE) ||
                  u->isChildOf(CALCULATOR->getUnitById(UNIT_ID_MINUTE)))) {
        // date + a*minute
...
bool reducable(const MathStructure &mnum, const MathStructure &mden,
               Number &nr) {
  switch (mnum.type()) {
  case STRUCT_NUMBER: {
  }
    ...
    for (size_t i = 0; i < mden.size() && reduce; i++) {
      ...
      case STRUCT_MULTIPLICATION: {
        ...
        for (size_t i2 = 0; i2 < mden[i].size(); i2++) {
          if (mnum == mden[i][i2]) {
            reduce = true;
            if (!nr.isOne() && !nr.isFraction())
              nr.set(1, 1, 0);
            break;
          } else if (mden[i][i2].isPower() && mden[i][i2][1].isNumber() &&
                     mden[i][i2][1].number().isReal() &&
                     mnum == mden[i][i2][0]) {
            if (!mden[i][i2][1].number().isPositive()) {
              break;
            }
            if (mden[i][i2][1].number().isLessThan(nr))
              nr = mden[i][i2][1].number();
            reduce = true;
            break;
          }
        }
      ...
      case STRUCT_POWER: {
        if (mden[i][1].isNumber() && mden[i][1].number().isReal() &&
            mnum == mden[i][0]) {
          if (!mden[i][1].number().isPositive()) {
            reduce = false;
            break;
          }
          if (mden[i][1].number().isLessThan(nr))
            nr = mden[i][1].number();
          break;
        }
      }
...
bool addablePower(const MathStructure &mstruct, const EvaluationOptions &eo) {
  if (mstruct[0].representsNonNegative(true))
    return true;
  if (mstruct[1].representsInteger())
    return true;
  // return eo.allow_complex && mstruct[0].representsNegative(true) &&
  // mstruct[1].isNumber() && mstruct[1].number().isRational() &&
  // mstruct[1].number().denominatorIsEven();
  return eo.allow_complex && mstruct[1].isNumber() &&
         mstruct[1].number().isRational() &&
         mstruct[1].number().denominatorIsEven();
}
...
int MathStructure::merge_multiplication(MathStructure &mstruct,
                                        const EvaluationOptions &eo,
                                        MathStructure *mparent,
                                        size_t index_this, size_t index_mstruct,
                                        bool reversed, bool do_append) {
  // test if two factors can be merged
  if (mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    // both factors are numbers try Number::multiply() (might never fail for
    // infinite values)
    Number nr(o_number);
    if (nr.multiply(mstruct.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || mstruct.number().isApproximate()) &&
        (eo.allow_complex || !nr.isComplex() || o_number.isComplex() ||
         mstruct.number().isComplex()) &&
        (eo.allow_infinite || !nr.includesInfinity() ||
  ...
  if (m_type == STRUCT_NUMBER && o_number.isInfinite()) {
    ...
    if (eo.approximation == APPROXIMATION_EXACT) {
      ...
      if (CALCULATOR->usesIntervalArithmetic()) {
        ...
        EvaluationOptions eo2 = eo;
        eo2.approximation = APPROXIMATION_APPROXIMATE;
        if (eo2.interval_calculation == INTERVAL_CALCULATION_NONE)
  ...
  // check if factors are numerator and denominator, and denominator is
  // polynomial
  const MathStructure *mnum = NULL, *mden = NULL;
  bool b_nonzero = false;
  if (eo.reduce_divisions) {
    if (!isNumber() && mstruct.isPower() && mstruct[0].isAddition() &&
        mstruct[0].size() > 1 && mstruct[1].isNumber() &&
        mstruct[1].number().isMinusOne()) {
      // second factor is denominator (exponent = -1)
      if ((!isPower() || !CHILD(1).hasNegativeSign()) && representsNumber() &&
          mstruct[0].representsNumber()) {
        if ((!eo.warn_about_denominators_assumed_nonzero &&
             eo.assume_denominators_nonzero &&
             !mstruct[0].representsZero(true)) ||
            mstruct[0].representsNonZero(true)) {
          // denominator is not zero
          b_nonzero = true;
        }
        if (b_nonzero || (eo.warn_about_denominators_assumed_nonzero &&
                          eo.assume_denominators_nonzero &&
                          !mstruct[0].representsZero(true))) {
          mnum = this;
          mden = &mstruct[0];
    ...
    } else if (!mstruct.isNumber() && isPower() && CHILD(0).isAddition() &&
               CHILD(0).size() > 1 && CHILD(1).isNumber() &&
               CHILD(1).number().isMinusOne()) {
      // first factor is denominator (exponent = -1)
      if ((!mstruct.isPower() || !mstruct[1].hasNegativeSign()) &&
          mstruct.representsNumber() && CHILD(0).representsNumber()) {
        if ((!eo.warn_about_denominators_assumed_nonzero &&
             eo.assume_denominators_nonzero &&
             !CHILD(0).representsZero(true)) ||
  ...
  if (mnum && mden && eo.reduce_divisions) {
    ...
    case STRUCT_MULTIPLICATION: {
      ...
      if (i_reducable < mnum->size()) {
        if (!b_nonzero && eo.warn_about_denominators_assumed_nonzero &&
            !warn_about_denominators_assumed_nonzero(*mden, eo))
          break;
        if (mnum == this) {
          mstruct.ref();
        ...
        if (b_pow) {
          ...
          if (nr == CHILD(0)[i_reducable][1].number()) {
            ...
            if (CHILD(0)[i_reducable][1].number().isOne()) {
              CHILD(0)
              [i_reducable].setToChild(1, true, &CHILD(0), i_reducable + 1);
            } else {
      ...
      if (reducable(*mnum, *mden, nr)) {
        ...
        if (mnum == this) {
          ...
          if (!nr.isOne()) {
            ...
            mparent->swapChildren(index_this + 1, index_mstruct + 1);
            reduce(mnum2, (*mparent)[index_this][0], nr, eo);
            (*mparent)[index_this].calculateRaiseExponent(eo, mparent,
                                                          index_this);
          } else {
            set_nocopy(mstruct, true);
            reduce(mnum2, CHILD(0), nr, eo);
  ...
  if (mstruct.isFunction() && eo.protected_function != mstruct.function()) {
    if (((mstruct.function()->id() == FUNCTION_ID_ABS && mstruct.size() == 1 &&
          mstruct[0].isFunction() &&
          mstruct[0].function()->id() == FUNCTION_ID_SIGNUM &&
          mstruct[0].size() == 2) ||
         (mstruct.function()->id() == FUNCTION_ID_SIGNUM &&
          mstruct.size() == 2 && mstruct[0].isFunction() &&
          mstruct[0].function()->id() == FUNCTION_ID_ABS &&
          mstruct[0].size() == 1)) &&
        (equals(mstruct[0][0]) ||
         (isFunction() && o_function->id() == FUNCTION_ID_ABS && SIZE == 1 &&
          CHILD(0) == mstruct[0][0]) ||
         (isPower() && CHILD(0) == mstruct[0][0]) ||
         (isPower() && CHILD(0).isFunction() &&
          CHILD(0).function()->id() == FUNCTION_ID_ABS &&
          CHILD(0).size() == 1 && CHILD(0)[0] == mstruct[0][0]))) {
      // sgn(abs(x))*x^y=x^y
      MERGE_APPROX_AND_PREC(mstruct)
      return 2;
    } else if (mstruct.function()->id() == FUNCTION_ID_SIGNUM &&
               mstruct.size() == 2) {
      if (equals(mstruct[0]) && representsReal(true)) {
        ...
        MERGE_APPROX_AND_PREC(mstruct)
        return 1;
      } else if (isPower() && CHILD(1).representsOdd() &&
                 mstruct[0] == CHILD(0) && CHILD(0).representsReal(true)) {
        // sgn(x)*x^3=abs(x)^3
        CHILD(0).transformById(FUNCTION_ID_ABS);
        MERGE_APPROX_AND_PREC(mstruct)
        return 1;
      }
    } else if (mstruct.function()->id() == FUNCTION_ID_ROOT &&
               VALID_ROOT(mstruct)) {
      if (equals(mstruct[0]) && mstruct[0].representsReal(true) &&
          mstruct[1].number().isOdd()) {
        ...
        return 1;
                 ...
                 CHILD(0) == mstruct[0] && CHILD(0).representsReal(true) &&
                 mstruct[1].number().isOdd()) {
        ...
        calculateMultiplyLast(eo);
        return 1;
      } else if (isPower() && CHILD(0).isFunction() &&
                 CHILD(0).function()->id() == FUNCTION_ID_ABS &&
                 CHILD(0).size() == 1 && CHILD(0)[0].equals(mstruct[0]) &&
                 CHILD(1).isNumber() && CHILD(1).number().isRational() &&
                 CHILD(1).number().isFraction() &&
                 CHILD(1).number().denominator() == mstruct[1].number() &&
                 CHILD(0).representsReal(true) &&
                 CHILD(1).number().numerator() == mstruct[1].number() - 1) {
        ...
        return 1;
               ...
               mstruct.function()->id() == FUNCTION_ID_SINC &&
               mstruct.size() == 1 && equals(mstruct[0]) &&
               mstruct.function()->getDefaultValue(2) != "pi") {
      // sinc(x)*x=sin(x)
      calculateMultiply(CALCULATOR->getRadUnit(), eo);
      transformById(FUNCTION_ID_SIN);
      if (eo.calculate_functions)
        calculateFunctions(eo, false);
      MERGE_APPROX_AND_PREC(mstruct)
      return 1;
    }
  }
  if (isZero()) {
    if (mstruct.isFunction()) {
      if ((mstruct.function()->id() == FUNCTION_ID_LOG ||
           mstruct.function()->id() == FUNCTION_ID_EXPINT) &&
          mstruct.size() == 1) {
        if (mstruct[0].representsNonZero() ||
            warn_about_assumed_not_value(mstruct[0], m_zero, eo)) {
          // 0*ln(x)=0 and 0*Ei(x)=0 if x is assumed non-zero
          MERGE_APPROX_AND_PREC(mstruct)
          return 2;
        }
      } else if (mstruct.function()->id() == FUNCTION_ID_LOGINT &&
                 mstruct.size() == 1) {
        if (mstruct.representsNumber(true) ||
            warn_about_assumed_not_value(mstruct[0], m_one, eo)) {
          // 0*li(x)=0 if x is assumed not one
          MERGE_APPROX_AND_PREC(mstruct)
          return 2;
        }
      } else if (mstruct.function()->id() == FUNCTION_ID_TAN &&
                 mstruct.size() == 1) {
        ...
        mstruct.setFunctionId(FUNCTION_ID_TAN);
               ...
               mstruct[1].representsNumber()) {
      if ((mstruct[0].function()->id() == FUNCTION_ID_LOG ||
           mstruct[0].function()->id() == FUNCTION_ID_EXPINT) &&
          mstruct[0].size() == 1) {
        if (mstruct[0][0].representsNonZero() ||
            warn_about_assumed_not_value(mstruct[0][0], m_zero, eo)) {
          // ln(x)^a*0=0 and Ei(x)*0=0 if x is assumed non-zero
          MERGE_APPROX_AND_PREC(mstruct)
          return 2;
        }
      } else if (mstruct[0].function()->id() == FUNCTION_ID_LOGINT &&
                 mstruct[0].size() == 1) {
        if (mstruct[0].representsNumber(true) ||
            warn_about_assumed_not_value(mstruct[0][0], m_one, eo)) {
    ...
    case STRUCT_VECTOR: {
      if (!isMatrix() && mstruct.isMatrix()) {
        ...
        if (SIZE != mstruct.size()) {
          CALCULATOR->error(
              true,
              _("The second matrix must have as many rows (was %s) as the "
                "first has columns (was %s) for matrix multiplication."),
              i2s(mstruct.size()).c_str(), i2s(SIZE).c_str(), NULL);
          return -1;
        }
        transform(STRUCT_VECTOR);
      }
      if (isMatrix()) {
          ...
          if (CALCULATOR->usesMatlabStyleMatrices()) {
            if (CHILD(0).size() != 1) {
              CALCULATOR->error(
                  ...
                  _("The second matrix must have as many rows (was %s) as the "
                    "first has columns (was %s) for matrix multiplication."),
                  i2s(1).c_str(), i2s(CHILD(0).size()).c_str(), NULL);
              return -1;
            }
            ...
            if (CHILD(0).size() != mstruct.size()) {
              CALCULATOR->error(
                  ...
                  _("The second matrix must have as many rows (was %s) as the "
                    "first has columns (was %s) for matrix multiplication."),
                  i2s(mstruct.size()).c_str(), i2s(CHILD(0).size()).c_str(),
                  NULL);
              return -1;
            }
        ...
        if (CHILD(0).size() != mstruct.size()) {
          CALCULATOR->error(
              ...
              _("The second matrix must have as many rows (was %s) as the "
                "first has columns (was %s) for matrix multiplication."),
              i2s(mstruct.size()).c_str(), i2s(CHILD(0).size()).c_str(), NULL);
          return -1;
        }
        MathStructure msave(*this);
        size_t r = SIZE;
        ...
        for (size_t index_r = 0; index_r < SIZE; index_r++) {
          for (size_t index_c = 0; index_c < CHILD(0).size(); index_c++) {
            for (size_t index = 0; index < msave[0].size(); index++) {
              ...
              CHILD(index_r)
              [index_c].calculateAdd(mtmp, eo, &CHILD(index_r), index_c);
            }
          }
        }
        ...
        CALCULATOR->error(true,
                          _("Please use the cross(), dot(), and hadamard() "
                            "functions for vector multiplication."),
                          NULL);
        // dot product of two vectors: [a1, a2, a3, ..]*[b1, b2, b3,
        // ...]=a1*b1+a2*b2+a3*b3+... dimension of the vectors must be equal
        /*if(SIZE == mstruct.size()) {
  ...
  case STRUCT_ADDITION: {
    ...
    case STRUCT_ADDITION: {
      ...
      if (eo.expand != 0 && SIZE < 1000 && mstruct.size() < 1000 &&
          (SIZE * mstruct.size() < (eo.expand == -1 ? 50 : 500))) {
        // avoid multiplication of polynomials with intervals (if factors might
        // be negitive)
        if (eo.expand > -2 ||
            (!containsInterval(true, false, false, eo.expand == -2 ? 1 : 0) &&
             !mstruct.containsInterval(true, false, false,
                                       eo.expand == -2 ? 1 : 0)) ||
            (representsNonNegative(true) &&
             mstruct.representsNonNegative(true))) {
          MathStructure msave(*this);
          CLEAR;
          for (size_t i = 0; i < mstruct.size(); i++) {
          ...
          return 1;
                   ...
                   (!containsInterval(true, false, false,
                                      eo.expand == -2 ? 1 : 0) ||
                    mstruct.representsNonNegative(true))) {
          return 0;
        }
    ...
    case STRUCT_POWER: {
      if (mstruct[1].isNumber() && *this == mstruct[0]) {
        ...
        if ((!eo.warn_about_denominators_assumed_nonzero &&
             eo.assume_denominators_nonzero && !representsZero(true)) ||
            (mstruct[1].isNumber() && mstruct[1].number().isReal() &&
             !mstruct[1].number().isMinusOne()) ||
            representsNonZero(true) || mstruct[1].representsPositive() ||
            (eo.warn_about_denominators_assumed_nonzero &&
             eo.assume_denominators_nonzero && !representsZero(true) &&
             warn_about_denominators_assumed_nonzero_or_positive(
                 *this, mstruct[1], eo))) {
          if (mparent) {
            mparent->swapChildren(index_this + 1, index_mstruct + 1);
      ...
      if (eo.expand == 0 && mstruct[0].isAddition())
        return -1;
      ...
      if (eo.combine_divisions && mstruct[1].hasNegativeSign()) {
        ...
        for (size_t i = 0; i < SIZE; i++) {
          if (CHILD(i).isOne())
            ...
            ret = CHILD(i).merge_multiplication(*mstruct2, eo, NULL, 0, 0,
                                                false, false);
          if (ret == 0) {
            ret = mstruct2->merge_multiplication(CHILD(i), eo, NULL, 0, 0, true,
                                                 false);
            if (ret >= 1) {
              mstruct2->ref();
          ...
          if (ret >= 1) {
            mstruct2->unref();
            if (i + 1 != SIZE)
              mstruct2 = new MathStructure(mstruct);
            merged[i] = true;
            merges++;
          } else {
            if (i + 1 == SIZE)
      ...
      if (eo.expand == 0 || (eo.expand < -1 &&
                             mstruct.containsInterval(
                                 true, false, false, eo.expand == -2 ? 1 : 0) &&
                             !representsNonNegative(true)))
        return -1;
    }
    case STRUCT_MULTIPLICATION: {
      if (do_append && (eo.expand == 0 ||
                        (eo.expand < -1 &&
                         mstruct.containsInterval(true, false, false,
                                                  eo.expand == -2 ? 1 : 0) &&
                         !representsNonNegative(true)))) {
  ...
  case STRUCT_MULTIPLICATION: {
    ...
    case STRUCT_ADDITION: {
      if (eo.expand == 0 ||
          containsType(STRUCT_DATETIME, false, true, false) > 0) {
        if (!do_append)
          return -1;
        APPEND_REF(&mstruct);
        return 1;
    ...
    default: {
      if (do_append) {
        ...
        for (size_t i = 0; i < SIZE; i++) {
          if (CALCULATOR->aborted())
            break;
          int ret = CHILD(i).merge_multiplication(mstruct, eo, NULL, 0, 0,
                                                  false, false);
          if (ret == 0) {
            ret = mstruct.merge_multiplication(CHILD(i), eo, NULL, 0, 0, true,
                                               false);
            if (ret >= 1) {
              if (ret == 2)
                ret = 3;
          ...
          if (ret >= 1) {
            if (ret != 2)
              calculateMultiplyIndex(i, eo, true, mparent, index_this);
            return 1;
          }
        }
      }
  ...
  case STRUCT_POWER: {
    ...
    case STRUCT_POWER: {
      if (CHILD(0).isFunction() &&
          CHILD(0).function()->id() == FUNCTION_ID_ABS &&
          mstruct[0].isFunction() &&
          mstruct[0].function()->id() == FUNCTION_ID_ROOT &&
          mstruct[1].isMinusOne() && CHILD(0).size() == 1 &&
          VALID_ROOT(mstruct[0]) && CHILD(0)[0].equals(mstruct[0][0]) &&
          CHILD(1).isNumber() && CHILD(1).number().isRational() &&
          CHILD(1).number().isFraction() &&
          CHILD(1).number().denominator() == mstruct[0][1].number() &&
          CHILD(0)[0].representsReal(true) &&
          CHILD(1).number().numerator() == -(mstruct[0][1].number() - 1)) {
      ...
      if (mstruct[0].isFunction() &&
          mstruct[0].function()->id() == FUNCTION_ID_ABS &&
          CHILD(0).isFunction() &&
          CHILD(0).function()->id() == FUNCTION_ID_ROOT &&
          CHILD(1).isMinusOne() && mstruct[0].size() == 1 &&
          VALID_ROOT(CHILD(0)) && CHILD(0)[0].equals(mstruct[0][0]) &&
          mstruct[1].isNumber() && mstruct[1].number().isRational() &&
          mstruct[1].number().isFraction() &&
          mstruct[1].number().denominator() == CHILD(0)[1].number() &&
          mstruct[0][0].representsReal(true) &&
          mstruct[1].number().numerator() == -(CHILD(0)[1].number() - 1)) {
        // root(x, 3)^-1*abs(x)^(-2/3)=1/x
        SET_CHILD_MAP(0)
        SET_CHILD_MAP(0)
        raise(m_minus_one);
      ...
      if (mstruct[0] == CHILD(0) ||
          (CHILD(0).isMultiplication() && CHILD(0).size() == 2 &&
           CHILD(0)[0].isMinusOne() && CHILD(0)[1] == mstruct[0] &&
           (mstruct[1].representsEven() || mstruct[1].representsOdd()))) {
        // x^a*x^b=x^(a+b)
        // (-x)^a*x^b=(-x)^(a+b) if b is even
        // (-x)^a*x^b=-(-x)^(a+b) if b is odd
        if (mstruct[0].isUnit() && mstruct[0].prefix()) {
          if (CHILD(0).isMultiplication())
            CHILD(0)[0].setPrefix(mstruct[0].prefix());
          else
            CHILD(0).setPrefix(mstruct[0].prefix());
        }
        bool b = eo.allow_complex || CHILD(0).representsNonNegative(true),
             b2 = true, b_warn = false;
        if (!b) {
          // if complex not allowed and base might be negative, exponents must
          // be integers
        ...
        if (b) {
          ...
          if (IS_REAL(mstruct[1]) && IS_REAL(CHILD(1))) {
            if (mstruct[1].number().isPositive() ==
          ...
          if (!b || b2test) {
            b = (!eo.warn_about_denominators_assumed_nonzero &&
                 eo.assume_denominators_nonzero &&
                 !CHILD(0).representsZero(true)) ||
                CHILD(0).representsNonZero(true) ||
                (CHILD(1).representsPositive() &&
                 mstruct[1].representsPositive()) ||
                (CHILD(1).representsNegative() &&
                 mstruct[1].representsNegative());
            if (!b && eo.warn_about_denominators_assumed_nonzero &&
                eo.assume_denominators_nonzero &&
                !CHILD(0).representsZero(true)) {
              b = true;
              b_warn = true;
        ...
        if (b) {
          if (IS_REAL(CHILD(1)) && IS_REAL(mstruct[1])) {
            ...
            if (b_warn &&
                !warn_about_denominators_assumed_nonzero(CHILD(0), eo))
              return -1;
            if (b2) {
              ...
              // a and b have different signs and a+b is not negative:
              // x^a/x^b=x^(a-b+1)/x
            ...
            if (CHILD(0).isNumber() && CHILD(0).number().isRational() &&
                !CHILD(0).number().isZero()) {
              ...
              if (CHILD(1).isMultiplication() && CHILD(1).size() > 0) {
                if (mstruct[1].isMultiplication() && mstruct[1].size() > 0) {
                  size_t i1 = 0, i2 = 0;
                  if (CHILD(1)[0].isNumber())
                    i1 = 1;
                  if (mstruct[1][0].isNumber())
                    i2 = 1;
                  ...
                  if (b1n) {
                    ...
                    b1n =
                        (!b1m && i1 == 1 && CHILD(1)[0].number().isRational() &&
                         CHILD(1)[0].number().isFraction());
                    b2m = (i2 == 1 && mstruct[1][0].number().isMinusOne());
                    b2n = (!b2m && i2 == 1 &&
                           mstruct[1][0].number().isRational() &&
                           mstruct[1][0].number().isFraction());
                  }
                } else if (CHILD(1).size() == 2 && CHILD(1)[0].isNumber() &&
                           CHILD(1)[0].number().isFraction() &&
                           CHILD(1)[0].number().isRational() &&
                           CHILD(1)[0].number().isNegative() &&
                           CHILD(1)[1] == mstruct[1]) {
                  b1n = true;
                }
              } else if (mstruct[1].isMultiplication() &&
                         mstruct[1].size() == 2 && mstruct[1][0].isNumber() &&
                         mstruct[1][0].number().isRational() &&
                         mstruct[1][0].number().isFraction() &&
                         mstruct[1][0].number().isNegative() &&
                         mstruct[1][1] == CHILD(1)) {
                b2n = true;
              }
              if (b1n && !b2m && !b2n) {
                if (cmp_num_abs_2_to_den(CHILD(1)[0].number()) < 0)
                  return -1;
              } else if (b2n && !b1m && !b1n) {
                if (cmp_num_abs_2_to_den(mstruct[1][0].number()) < 0)
                  return -1;
            ...
            if (mstruct2.calculateAdd(mstruct[1], eo)) {
              // test and warn for possible division by zero (test both base and
              // exponents)
              if (b_warn && !warn_about_denominators_assumed_nonzero_llgg(
                                CHILD(0), CHILD(1), mstruct[1], eo))
                return -1;
              CHILD(1) = mstruct2;
              calculateRaiseExponent(eo, mparent, index_this);
              return 1;
        ...
        if (!CHILD(0).isMultiplication() && !mstruct[0].isMultiplication() &&
            (mstruct[1].representsInteger() ||
             CHILD(0).representsPositive(true) ||
             mstruct[0].representsPositive(true))) {
          ...
          if (mstruct2.calculateMultiply(mstruct[0], eo)) {
            ...
            return 1;
                   ...
                   CHILD(1).representsInteger() && CHILD(0).isFunction() &&
                   mstruct[0].isFunction() &&
                   eo.protected_function != mstruct[0].function() &&
                   eo.protected_function != CHILD(0).function() &&
                   CHILD(0).size() == 1 && mstruct[0].size() == 1 &&
                   CHILD(0)[0] == mstruct[0][0]) {
          if ((CHILD(0).function()->id() == FUNCTION_ID_COS &&
               mstruct[0].function()->id() == FUNCTION_ID_SIN) ||
              (CHILD(0).function()->id() == FUNCTION_ID_SIN &&
               mstruct[0].function()->id() == FUNCTION_ID_COS) ||
              (CHILD(0).function()->id() == FUNCTION_ID_COSH &&
               mstruct[0].function()->id() == FUNCTION_ID_SINH) ||
              (CHILD(0).function()->id() == FUNCTION_ID_SINH &&
               mstruct[0].function()->id() == FUNCTION_ID_COSH)) {
            // cos(x)^n*sin(x)^n=sin(2x)^n/2^n
            if (CHILD(0).function()->id() == FUNCTION_ID_COSH)
              CHILD(0).setFunctionId(FUNCTION_ID_SINH);
            ...
            return 1;
                      ...
                      mstruct[0].function()->id() == FUNCTION_ID_COS) ||
                     (CHILD(0).function()->id() == FUNCTION_ID_COS &&
                      mstruct[0].function()->id() == FUNCTION_ID_TAN)) {
            ...
            return 1;
                      ...
                      mstruct[0].function()->id() == FUNCTION_ID_COSH) ||
                     (CHILD(0).function()->id() == FUNCTION_ID_COSH &&
                      mstruct[0].function()->id() == FUNCTION_ID_TANH)) {
      ...
      } else if (eo.transform_trigonometric_functions && CHILD(1).isInteger() &&
                 mstruct[1].isInteger() && CHILD(0).isFunction() &&
                 mstruct[0].isFunction() &&
                 eo.protected_function != mstruct[0].function() &&
                 eo.protected_function != CHILD(0).function() &&
                 mstruct[0].size() == 1 && CHILD(0).size() == 1 &&
                 CHILD(0)[0] == mstruct[0][0]) {
        if (CHILD(1).number().isNonNegative() !=
          ...
          if (CHILD(0).function()->id() == FUNCTION_ID_SIN) {
            if (mstruct[0].function()->id() == FUNCTION_ID_COS) {
              ...
              if (mstruct[1].number().isZero()) {
                ...
                MERGE_APPROX_AND_PREC(mstruct)
                return 1;
              } else if (mstruct[1].number().isPositive() ==
                         CHILD(1).number().isPositive()) {
                // n>m
          ...
          if ((CHILD(0).function()->id() == FUNCTION_ID_TAN &&
               mstruct[0].function()->id() == FUNCTION_ID_COS)) {
      ...
      } else if (mstruct[0].isMultiplication() && mstruct[0].size() == 2 &&
                 mstruct[0][0].isMinusOne() && mstruct[0][1] == CHILD(0) &&
                 CHILD(1).representsEven()) {
        return 0;
      }
      break;
    }
    case STRUCT_FUNCTION: {
      if (!FUNCTION_PROTECTED(eo, FUNCTION_ID_SIGNUM) &&
          mstruct.function()->id() == FUNCTION_ID_SIGNUM &&
          mstruct.size() == 2 && CHILD(0).isFunction() &&
          CHILD(0).function()->id() == FUNCTION_ID_ABS &&
          CHILD(0).size() == 1 && mstruct[0] == CHILD(0)[0] &&
          CHILD(1).isNumber() && CHILD(1).number().isRational() &&
          CHILD(1).number().numeratorIsOne() &&
          !CHILD(1).number().denominatorIsEven() &&
          CHILD(0)[0].representsReal(true)) {
      ...
      if (eo.transform_trigonometric_functions && CHILD(0).isFunction() &&
          CHILD(0).size() == 1 && mstruct.size() == 1 && CHILD(1).isInteger() &&
          CHILD(1).number().isNegative() &&
          eo.protected_function != mstruct.function() &&
          eo.protected_function != CHILD(0).function()) {
        if (CHILD(0).function()->id() == FUNCTION_ID_SIN) {
          if (mstruct.function()->id() == FUNCTION_ID_COS &&
              CHILD(0)[0] == mstruct[0]) {
          ...
          if (mstruct.function()->id() == FUNCTION_ID_COSH &&
              CHILD(0)[0] == mstruct[0]) {
            if (CHILD(1).number().isMinusOne()) {
            ...
            calculateMultiplyLast(eo);
            return 1;
          } else if (mstruct.function()->id() == FUNCTION_ID_TANH &&
                     CHILD(0)[0] == mstruct[0]) {
            if (CHILD(1).number().isMinusOne()) {
      ...
      if (mstruct.function()->id() == FUNCTION_ID_STRIP_UNITS &&
          mstruct.size() == 1) {
        if (m_type == STRUCT_POWER && CHILD(0).isVariable() &&
            CHILD(0).variable()->isKnown() &&
            mstruct[0].contains(CHILD(0), false) > 0) {
    ...
    default: {
      if (!mstruct.isNumber() && CHILD(1).isNumber() && CHILD(0) == mstruct) {
        ...
        if ((!eo.warn_about_denominators_assumed_nonzero &&
             eo.assume_denominators_nonzero &&
             !CHILD(0).representsZero(true)) ||
            (CHILD(1).isNumber() && CHILD(1).number().isReal() &&
             !CHILD(1).number().isMinusOne()) ||
            CHILD(0).representsNonZero(true) || CHILD(1).representsPositive() ||
            (eo.warn_about_denominators_assumed_nonzero &&
             eo.assume_denominators_nonzero && !CHILD(0).representsZero(true) &&
             warn_about_denominators_assumed_nonzero_or_positive(
                 CHILD(0), CHILD(1), eo))) {
          CHILD(1).number()++;
          MERGE_APPROX_AND_PREC(mstruct)
      ...
      if (mstruct.isZero() &&
          (!eo.keep_zero_units ||
           containsType(STRUCT_UNIT, false, true, true) <= 0 ||
           (CHILD(0).isUnit() && CHILD(0).unit() == CALCULATOR->getRadUnit()) ||
           (CHILD(0).isFunction() && CHILD(0).representsNumber(false))) &&
          !representsUndefined(true, true, !eo.assume_denominators_nonzero) &&
          representsNonMatrix()) {
        clear(true);
        MERGE_APPROX_AND_PREC(mstruct)
        return 1;
      }
      if (CHILD(0).isFunction() && mstruct.isZero() &&
          CHILD(1).representsNumber()) {
        if ((CHILD(0).function()->id() == FUNCTION_ID_LOG ||
             CHILD(0).function()->id() == FUNCTION_ID_EXPINT) &&
            SIZE == 1) {
          if (CHILD(0)[0].representsNonZero() ||
              warn_about_assumed_not_value(CHILD(0)[0], m_zero, eo)) {
            // ln(x)^a*0=0 and Ei(x)*0=0 if x is assumed non-zero
            clear(true);
            MERGE_APPROX_AND_PREC(mstruct)
            return 3;
          }
        } else if (CHILD(0).function()->id() == FUNCTION_ID_LOGINT &&
                   SIZE == 1) {
          if (CHILD(0).representsNumber(true) ||
              warn_about_assumed_not_value(CHILD(0)[0], m_one, eo)) {
            // li(x)^a*0=0 if x is assumed not one
            clear(true);
            MERGE_APPROX_AND_PREC(mstruct)
            return 3;
  ...
  case STRUCT_FUNCTION: {
    if (eo.protected_function != o_function) {
      if (((o_function->id() == FUNCTION_ID_ABS && SIZE == 1 &&
            CHILD(0).isFunction() &&
            CHILD(0).function()->id() == FUNCTION_ID_SIGNUM &&
            CHILD(0).size() == 2) ||
           (o_function->id() == FUNCTION_ID_SIGNUM && SIZE == 2 &&
            CHILD(0).isFunction() &&
            CHILD(0).function()->id() == FUNCTION_ID_ABS &&
            CHILD(0).size() == 1)) &&
          (CHILD(0)[0] == mstruct ||
           (mstruct.isFunction() &&
            mstruct.function()->id() == FUNCTION_ID_ABS &&
            mstruct.size() == 1 && CHILD(0)[0] == mstruct[0]) ||
           (mstruct.isPower() && mstruct[0] == CHILD(0)[0]) ||
           (mstruct.isPower() && mstruct[0].isFunction() &&
            mstruct[0].function()->id() == FUNCTION_ID_ABS &&
            mstruct[0].size() == 1 && CHILD(0)[0] == mstruct[0][0]))) {
        // sgn(abs(x))*x^y=x^y
        if (mparent) {
          mparent->swapChildren(index_this + 1, index_mstruct + 1);
        } else {
        ...
        if (mstruct.isFunction() &&
            mstruct.function()->id() == FUNCTION_ID_ABS &&
            mstruct.size() == 1 && mstruct[0] == CHILD(0) &&
            CHILD(0).representsReal(true)) {
          // abs(x)*abs(x)=x^2
          SET_CHILD_MAP(0)
          MERGE_APPROX_AND_PREC(mstruct)
          ...
          return 1;
                   ...
                   eo.protected_function != mstruct.function() &&
                   mstruct.function()->id() == FUNCTION_ID_SIGNUM &&
                   mstruct.size() == 2 && mstruct[0] == CHILD(0) &&
                   CHILD(0).representsScalar()) {
          // sgn(x)*abs(x)=x
          SET_CHILD_MAP(0)
          MERGE_APPROX_AND_PREC(mstruct)
          return 1;
        ...
        if (mstruct.isFunction() &&
          ...
          ERASE(1)
          return 1;
        } else if (mstruct.isPower() && mstruct[1].representsOdd() &&
                   mstruct[0] == CHILD(0) && CHILD(0).representsReal(true)) {
          // sgn(x)*x^3=abs(x)^3
          mstruct[0].transformById(FUNCTION_ID_ABS);
          if (mparent) {
        ...
        if (CHILD(0) == mstruct && CHILD(0).representsReal(true) &&
            CHILD(1).number().isOdd()) {
          // root(x, 3)*x=abs(x)^(1/3)*x
          CHILD(0).transformById(FUNCTION_ID_ABS);
          CHILD(1).number().recip();
          m_type = STRUCT_POWER;
          ...
          return 1;
                   ...
                   mstruct.function()->id() == FUNCTION_ID_ROOT &&
                   VALID_ROOT(mstruct) && CHILD(0) == mstruct[0] &&
                   CHILD(0).representsReal(true) && CHILD(1).number().isOdd() &&
                   mstruct[1].number().isOdd()) {
          ...
          return 1;
                   ...
                   mstruct[0].function()->id() == FUNCTION_ID_ABS &&
                   mstruct[0].size() == 1 && mstruct[0][0].equals(CHILD(0)) &&
                   mstruct[1].isNumber() && mstruct[1].number().isRational() &&
                   mstruct[1].number().isFraction() &&
                   mstruct[0].number().denominator() == CHILD(1).number() &&
                   CHILD(0).representsReal(true) &&
                   mstruct[1].number().numerator() == CHILD(1).number() - 1) {
          // root(x, 3)*abs(x)^(2/3)=x
          SET_CHILD_MAP(0)
          return 1;
        }
      } else if ((o_function->id() == FUNCTION_ID_LOG ||
                  o_function->id() == FUNCTION_ID_EXPINT) &&
                 SIZE == 1 && mstruct.isZero()) {
        // Ei(x)*0=0 if x is assumed non-zero
        if (CHILD(0).representsNonZero() ||
            warn_about_assumed_not_value(CHILD(0), m_zero, eo)) {
          clear(true);
          MERGE_APPROX_AND_PREC(mstruct)
          return 3;
        }
        ...
        if (representsNumber(true) ||
            warn_about_assumed_not_value(CHILD(0), m_one, eo)) {
          clear(true);
          MERGE_APPROX_AND_PREC(mstruct)
          return 3;
        }
        ...
        setFunctionId(FUNCTION_ID_TAN);
                 ...
                 o_function->id() == FUNCTION_ID_SINC && SIZE == 1 &&
                 CHILD(0) == mstruct &&
                 o_function->getDefaultValue(2) != "pi") {
        // sinc(x)*x=sin(x)
        ...
        MERGE_APPROX_AND_PREC(mstruct)
        return 1;
      } else if (eo.transform_trigonometric_functions && mstruct.isFunction() &&
                 mstruct.size() == 1 &&
                 eo.protected_function != mstruct.function()) {
        if (o_function->id() == FUNCTION_ID_TAN && SIZE == 1) {
          ...
          if (mstruct.function()->id() == FUNCTION_ID_TAN &&
              mstruct[0] == CHILD(0)) {
            // tan(x)*cos(x)=sin(x)
            setFunctionId(FUNCTION_ID_SIN);
            MERGE_APPROX_AND_PREC(mstruct)
            return 1;
          } else if (mstruct.function()->id() == FUNCTION_ID_SIN &&
                     mstruct[0] == CHILD(0)) {
          ...
          if (mstruct.function()->id() == FUNCTION_ID_TANH &&
              mstruct[0] == CHILD(0)) {
            // tanh(x)*cosh(x)=sinh(x)
            setFunctionId(FUNCTION_ID_SINH);
            MERGE_APPROX_AND_PREC(mstruct)
    ...
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1) {
      if (mstruct.isFunction() &&
        ...
        return 1;
      } else if (mstruct.isVariable() && mstruct.variable()->isKnown() &&
                 CHILD(0).contains(mstruct, false) > 0) {
      ...
      } else if (mstruct.isPower() && mstruct[0].isVariable() &&
                 mstruct[0].variable()->isKnown() &&
                 CHILD(0).contains(mstruct[0], false) > 0) {
  ...
  default: {
    ...
    case STRUCT_COMPARISON: {
      if (isComparison()) {
        // use logical and for multiplication of comparisons (logical and equals
        // multiplication)
        mstruct.ref();
    ...
    default: {
      if (mstruct.isFunction() &&
          mstruct.function()->id() == FUNCTION_ID_STRIP_UNITS &&
          mstruct.size() == 1) {
        if (m_type == STRUCT_VARIABLE && o_variable->isKnown() &&
            mstruct[0].contains(*this, false) > 0) {
      ...
      if (mstruct.isZero() &&
          (!eo.keep_zero_units ||
           containsType(STRUCT_UNIT, false, true, true) <= 0 ||
           (isUnit() && unit() == CALCULATOR->getRadUnit()) ||
           (isFunction() && representsNumber(false))) &&
          !representsUndefined(true, true, !eo.assume_denominators_nonzero) &&
          representsNonMatrix()) {
        clear(true);
        MERGE_APPROX_AND_PREC(mstruct)
      ...
      if (isZero() &&
          !mstruct.representsUndefined(true, true,
                                       !eo.assume_denominators_nonzero) &&
          (!eo.keep_zero_units ||
           mstruct.containsType(STRUCT_UNIT, false, true, true) <= 0 ||
           (mstruct.isUnit() && mstruct.unit() == CALCULATOR->getRadUnit())) &&
          mstruct.representsNonMatrix()) {
        MERGE_APPROX_AND_PREC(mstruct)
        return 2;
      }
      if (equals(mstruct)) {
        // x*x=x^2
        if (mstruct.isUnit() && mstruct.prefix())
          o_prefix = mstruct.prefix();
        raise_nocopy(new MathStructure(2, 1, 0));
        MERGE_APPROX_AND_PREC(mstruct)
        calculateRaiseExponent(eo, mparent, index_this);
        return 1;
...
bool test_if_numerator_not_too_large(Number &vb, Number &ve) {
  if (!vb.isRational())
    return false;
  if (!mpz_fits_slong_p(mpq_numref(ve.internalRational())))
    return false;
  long int exp = labs(mpz_get_si(mpq_numref(ve.internalRational())));
  if (vb.isRational()) {
    if ((long long int)exp *
                mpz_sizeinbase(mpq_numref(vb.internalRational()), 10) <=
            1000000LL &&
        (long long int)exp *
                mpz_sizeinbase(mpq_denref(vb.internalRational()), 10) <=
            1000000LL)
      return true;
  }
  return false;
      ...
      if (m1[0].number().isMinusOne()) {
        ...
        if (m2.isMultiplication() && m2.size() == m1.size() - 1) {
          for (size_t i = 1; i < m1.size(); i++) {
            if (!m1[i].equals(m2[i - 1], true, true))
              return false;
          }
          return true;
        }
      ...
      if (m2[0].number().isMinusOne()) {
        ...
        if (m1.isMultiplication() && m1.size() == m2.size() - 1) {
          for (size_t i = 1; i < m2.size(); i++) {
            if (!m2[i].equals(m1[i - 1], true, true))
              return false;
          }
          return true;
        }
...
int MathStructure::merge_power(MathStructure &mstruct,
                               const EvaluationOptions &eo,
                               MathStructure *mparent, size_t index_this,
                               size_t index_mstruct, bool) {
  // test if base and exponent can be merged
  if (mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    // base and exponent are numbers try Number::raise()
    Number nr(o_number);
    if (nr.raise(mstruct.number(),
        ...
        (eo.allow_infinite || !nr.includesInfinity() ||
         o_number.includesInfinity() || mstruct.number().includesInfinity())) {
      // Exponentiation succeeded without inappropriate change in approximation
      // status
      if (o_number == nr) {
        o_number = nr;
        numberUpdated();
    ...
    if (!o_number.isMinusOne() && !o_number.isOne() &&
        mstruct.number().isRational() && !mstruct.isInteger()) {
      if (o_number.isRational() && !o_number.isZero() &&
          mstruct.number().isFraction()) {
        if (mstruct.number().isNegative() &&
            cmp_num_abs_2_to_den(mstruct.number()) >= 0) {
          // a^(-b)=a^(-b+1)/a
          ...
          return 1;
        } else if (mstruct.number().isPositive() &&
                   cmp_num_abs_2_to_den(mstruct.number()) > 0) {
          // a^b=a^(b-1)*a
          Number nmul(o_number);
          mstruct.number()--;
      ...
      if (o_number.isNegative()) {
        ...
        if (!exp_num.isOne() && !exp_num.isMinusOne() &&
            o_number.isPositive() &&
            test_if_numerator_not_too_large(o_number, exp_num)) {
          // a^(n/d)=(a^n)^(1/d)
          nr = o_number;
          if (nr.raise(exp_num, eo.approximation < APPROXIMATION_APPROXIMATE) &&
              (eo.approximation >= APPROXIMATION_APPROXIMATE ||
               !nr.isApproximate() || o_number.isApproximate() ||
               mstruct.number().isApproximate()) &&
              (eo.allow_complex || !nr.isComplex() || o_number.isComplex() ||
               mstruct.number().isComplex()) &&
              (eo.allow_infinite || !nr.includesInfinity() ||
               o_number.includesInfinity() ||
               mstruct.number().includesInfinity())) {
            o_number = nr;
            numberUpdated();
        ...
        if (o_number.isPositive()) {
          ...
          if (eo.split_squares && o_number.isInteger() &&
              o_number.integerLength() < 100000L &&
              nr_root.isLessThanOrEqualTo(LARGEST_RAISED_PRIME_EXPONENT)) {
            // a^(1/d)=b^(1/d)*(a/b)^(1/d)
            // search list of primes raised by the exponent denominator for
            // value which can divide the base without remainder
            ...
            while (b) {
              ...
              if (overflow) {
                ...
                for (size_t i = 0;
                     root == 2 ? (i < NR_OF_SQUARE_PRIMES)
                               : (RAISED_PRIMES[root - 3][i] != 0);
                     i++) {
                  if (CALCULATOR->aborted())
                    break;
                  if (mpz_divisible_ui_p(
                          cval,
                          (unsigned long int)(root == 2
                                                  ? SQUARE_PRIMES[i]
                                                  : RAISED_PRIMES[root - 3]
                                                                 [i]))) {
                    nr *= PRIMES[i];
                    o_number /= (root == 2 ? SQUARE_PRIMES[i]
                                           : RAISED_PRIMES[root - 3][i]);
                    b = true;
                    break;
                  }
                }
                ...
                for (size_t i = 0;
                  ...
                  if (CALCULATOR->aborted())
                    break;
                  if ((root == 2 ? SQUARE_PRIMES[i]
                                 : RAISED_PRIMES[root - 3][i]) > val) {
                    break;
                  } else if (val % (root == 2 ? SQUARE_PRIMES[i]
                                              : RAISED_PRIMES[root - 3][i]) ==
                             0) {
                    nr *= PRIMES[i];
                    o_number /= (root == 2 ? SQUARE_PRIMES[i]
            ...
            if (!nr.isOne()) {
              ...
              if (!mstruct.number().numeratorIsOne()) {
                CHILD(0).calculateRaise(mstruct.number().numerator(), eo, this,
                                        0);
              }
              calculateMultiplyIndex(0, eo, true, mparent, index_this);
              return 1;
          ...
          if (eo.split_squares && nr_root != 2) {
            ...
            // test if denominator is divisible with prime number (each from
            // PRIMES array) and try calculating root
            for (size_t i = 1; i < NR_OF_PRIMES; i++) {
              if (nr_root.isLessThanOrEqualTo(PRIMES[i]))
                break;
    ...
    if (o_number.isMinusOne() && mstruct.number().isRational()) {
      if (mstruct.number().isInteger()) {
        // (-1)^n equals 1 if n is even, -1 if n is odd (is normally handled
        // above)
        if (mstruct.number().isEven())
          set(m_one, true);
        else
        ...
        if (eo.allow_complex && mstruct.number().denominatorIsTwo()) {
          // (-1)^(n/2) equals i if floor(n/2) is even and -i if floor(n/2) is
          // odd
          if (nr_floor.isEven())
            set(nr_one_i, true);
          else
          ...
          if (eo.allow_complex && mstruct.number().denominator() == 3) {
            ...
            if (nr_floor.isEven())
              ...
              calculateMultiply(nr_minus_i, eo);
            calculateMultiply(nr_half, eo);
            if (nr_floor.isEven() == mstruct.number().numeratorIsOne())
              calculateAdd(nr_half, eo);
            else
              calculateAdd(nr_minus_half, eo);
            MERGE_APPROX_AND_PREC(mstruct)
            return 1;
          } else if (eo.allow_complex && mstruct.number().denominator() == 4) {
            // (-1)^(n/4) equals (1+i)*1/sqrt(2) if floor(n/4) is even and
            // (-1-i)*1/sqrt(2) if floor(n/4) is odd
            if (nr_floor.isEven() == mstruct.number().numeratorIsOne())
              set(1, 1, 0, true);
            else
              set(-1, 1, 0, true);
            if (nr_floor.isEven())
              calculateAdd(nr_one_i, eo);
            else
              calculateAdd(nr_minus_i, eo);
            multiply(nr_two);
    ...
    if (o_number.isRational() && !o_number.isInteger() &&
        !o_number.numeratorIsOne() && mstruct.number().isRational()) {
      // (n/d)^a=n^a*1/d^a
    ...
    // If base numerator is larger than denominator, invert base and negate
    // exponent
    if (o_number.isRational() && !o_number.isInteger() && !o_number.isZero() &&
        ((o_number.isNegative() && o_number.isGreaterThan(nr_minus_one) &&
          mstruct.number().isInteger()) ||
         (o_number.isPositive() && o_number.isLessThan(nr_one)))) {
  ...
  if (m_type == STRUCT_NUMBER && o_number.isInfinite(false) &&
    ...
    if (mtest.representsNegative(false)) {
      ...
      MERGE_APPROX_AND_PREC(mstruct)
      return 1;
    } else if (mtest.representsNonZero(false) &&
               mtest.representsPositive(false)) {
      if (o_number.isMinusInfinity()) {
        if (mstruct.representsEven(false)) {
          // (-infinity)^a=infinity if a is even
          o_number.setPlusInfinity();
  ...
  case STRUCT_VECTOR: {
    if (mstruct.isNumber() && mstruct.number().isInteger()) {
      if (isMatrix()) {
        ...
        // requires equal number of columns and rows (columns of 1st matrix must
        // be equal to rows in 2nd matrix)
        if (matrixIsSquare()) {
  ...
  case STRUCT_ADDITION: {
    if (mstruct.isNumber() && mstruct.number().isInteger() &&
      ...
      if (eo.reduce_divisions && mstruct.number().isMinusOne()) {
        // 1/(a1+a2+...)
        if (SIZE == 2 && CHILD(0).isPower() && CHILD(0)[0].isNumber() &&
            CHILD(0)[1] == nr_half && CHILD(1).isNumber() &&
            !containsInterval()) {
          // 1/(sqrt(a)+b)
          Number nr(CHILD(1).number());
          if (nr.square() && nr.negate() && nr.add(CHILD(0)[0].number()) &&
              nr.recip()) {
        ...
        for (size_t i = 0; i < SIZE && !b_break; i++) {
          switch (CHILD(i).type()) {
          case STRUCT_NUMBER: {
            if (!CHILD(i).number().isRational() ||
                CHILD(i).number().numeratorIsGreaterThan(1000000L) ||
                CHILD(i).number().numeratorIsLessThan(-1000000L) ||
                CHILD(i).number().denominatorIsGreaterThan(1000L)) {
              bnum = 0;
              bden = 0;
              inegs = 0;
              b_break = true;
            }
            if (bden != 0 && !CHILD(i).number().isInteger())
              bden = 1;
            if (bnum != 0 && !CHILD(i).isZero()) {
              if (CHILD(i).number().numeratorIsOne() ||
                  CHILD(i).number().numeratorIsMinusOne())
                bnum = 0;
              else
                bnum = 1;
            }
            if (CHILD(i).number().hasNegativeSign()) {
              ...
              // (for predictable result when number of negative and positive
              // terms are equal)
              if (i == 0)
                inegs++;
          ...
          case STRUCT_MULTIPLICATION: {
            if (CHILD(i).size() > 0 && CHILD(i)[0].isNumber()) {
              if (!CHILD(i)[0].number().isRational() ||
                  CHILD(i)[0].number().numeratorIsGreaterThan(1000000L) ||
                  CHILD(i)[0].number().numeratorIsLessThan(-1000000L) ||
                  CHILD(i)[0].number().denominatorIsGreaterThan(1000L)) {
                bnum = 0;
                bden = 0;
                inegs = 0;
                b_break = true;
              }
              if (bden != 0 && !CHILD(i)[0].number().isInteger())
                bden = 1;
              if (bnum != 0 && !CHILD(i)[0].isZero()) {
                if (CHILD(i)[0].number().numeratorIsOne() ||
                    CHILD(i)[0].number().numeratorIsMinusOne())
                  bnum = 0;
                else
                  bnum = 1;
              }
        ...
        if (bnum < 0)
          bnum = 0;
        if (bnum || bden) {
          // denominator only contains rational multipliers (non-numerical
          // values excluded) if bnum is true, denonimator contains multiplier
          // with numerator > 1: determine the greater common divisor of
          // multiplier numerators if bden is true, denominator contains
          // non-integer multipliers: determine least common multiplier of
          // multiplier denominators
            ...
            case STRUCT_NUMBER: {
              if (CHILD(i).number().isInteger()) {
                if (bnum && !nr_num.isOne() && !CHILD(i).number().isZero()) {
                  if (nr_num.isZero())
                    nr_num = CHILD(i).number();
                  else
                    nr_num.gcd(CHILD(i).number());
                }
              } else {
                if (bnum && !nr_num.isOne() && !CHILD(i).number().isZero()) {
                  if (nr_num.isZero())
                    nr_num = CHILD(i).number().numerator();
                  else
                    nr_num.gcd(CHILD(i).number().numerator());
                }
                if (bden) {
                  nr_den.lcm(CHILD(i).number().denominator());
                  if (nr_den.isGreaterThan(1000000L))
                    nr_den.clear();
                }
              }
              break;
            ...
            case STRUCT_MULTIPLICATION: {
              if (CHILD(i).size() > 0 && CHILD(i)[0].isNumber()) {
                if (CHILD(i)[0].number().isInteger()) {
                  if (bnum && !nr_num.isOne() &&
                      !CHILD(i)[0].number().isZero()) {
                    if (nr_num.isZero())
                      nr_num = CHILD(i)[0].number();
                    else
                      nr_num.gcd(CHILD(i)[0].number());
                  }
                } else {
                  if (bnum && !nr_num.isOne() &&
                      !CHILD(i)[0].number().isZero()) {
                    if (nr_num.isZero())
                      nr_num = CHILD(i)[0].number().numerator();
                    else
                      nr_num.gcd(CHILD(i)[0].number().numerator());
                  }
                  if (bden) {
          ...
          if (!nr_den.isZero() && (!nr_den.isOne() || !nr_num.isOne())) {
            ...
            // if the number of negative terms is greater than the number of
            // positive terms, negate all terms
            for (size_t i = 0; i < SIZE; i++) {
              switch (CHILD(i).type()) {
              case STRUCT_NUMBER: {
                CHILD(i).number() *= nr;
        ...
        if (inegs > 0) {
          // if the number of negative terms is greater than the number of
          // positive terms: 1/(a1+a2+...)=-1/(-a1-a2-...) this makes it easier
          // the handle expressions such as 1/(a1-a2-a3+a4)+1/(-a1+a2+a3-a4)
          // (=0)
          for (size_t i = 0; i < SIZE; i++) {
            switch (CHILD(i).type()) {
            case STRUCT_NUMBER: {
          ...
          return 1;
                 ...
                 (eo.reduce_divisions || !mstruct.number().isMinusOne()) &&
                 (eo.expand > -2 || !containsInterval())) {
        // (a1+a2+a3...)^b
        bool b = true;
        ...
        if (SIZE > 1) {
          ...
          // if number of terms and exponent is small enough to allow reasonably
          // fast calculation
          if (eo.expand == -1) {
            // use more conservative values
            switch (SIZE) {
            case 4: {
              if (m.isGreaterThan(3)) {
                b = false;
              }
              break;
            }
            case 3: {
              if (m.isGreaterThan(4)) {
                b = false;
              }
              break;
            }
            ...
            if (b || i_pow > 300) {
              ...
              Number num_terms;
              if (num_terms.binomial(i_pow + (long int)SIZE - 1,
                                     (long int)SIZE - 1)) {
        ...
        if (b) {
          if (!representsNonMatrix()) {
            // use simple expansion for base which might be/inlcude matrix(es):
            // (a+b)^n=(a+b)(a+b)...
          ...
          // negative exponent: inverse after expansion (using absolute
          // exponent)
          if (neg)
            calculateInverse(eo);
          MERGE_APPROX_AND_PREC(mstruct)
    ...
    if (mstruct.representsInteger()) {
      ...
      for (size_t i = 0; i < SIZE;) {
        if (CHILD(i).representsNonNegative(true)) {
          ...
          ERASE(i);
        } else if (CHILD(i).isNumber() && CHILD(i).number().isNegative() &&
                   !CHILD(i).number().isMinusOne()) {
          // (-5)^z=5^z*(-1)^z
          CHILD(i).number().negate();
  ...
  case STRUCT_POWER: {
    // (x^y)^z
    if ((eo.allow_complex && CHILD(1).representsFraction()) ||
        (mstruct.representsInteger() &&
         (eo.allow_complex || CHILD(0).representsInteger())) ||
        representsNonNegative(true)) {
      ...
      if ((((!eo.assume_denominators_nonzero ||
            ...
            !CHILD(0).representsNonZero(true)) ||
           CHILD(0).isZero()) &&
          CHILD(1).representsNegative(true)) {
        // check that a is positive or x is non-zero
        if (!eo.assume_denominators_nonzero || CHILD(0).isZero() ||
            !warn_about_denominators_assumed_nonzero(CHILD(0), eo))
          break;
      }
      if (!CHILD(1).representsNonInteger() && !mstruct.representsInteger()) {
        if (CHILD(1).representsEven() && CHILD(0).representsReal(true)) {
          ...
          if (CHILD(0).representsNegative(true)) {
            ...
            CHILD(0).set(CALCULATOR->getFunctionById(FUNCTION_ID_ABS),
                         &mstruct_base, NULL);
          }
        } else if (!CHILD(1).representsOdd() &&
                   !CHILD(0).representsNonNegative(true)) {
    ...
    if (mstruct.isNumber() && CHILD(0).isVariable() &&
        CHILD(0).variable()->id() == VARIABLE_ID_E && CHILD(1).isNumber() &&
        CHILD(1).number().hasImaginaryPart() &&
        !CHILD(1).number().hasRealPart() && mstruct.number().isReal()) {
      // (e^(a*i))^b
      CALCULATOR->beginTemporaryEnableIntervalArithmetic();
      if (CALCULATOR->usesIntervalArithmetic()) {
        // calculate floor((a+pi)/pi/2) and make sure that upper and lower value
        // is equal
        CALCULATOR->beginTemporaryStopMessages();
  ...
  case STRUCT_VARIABLE: {
    if (o_variable->id() == VARIABLE_ID_E) {
      if (mstruct.isMultiplication() && mstruct.size() == 2 &&
          mstruct[0].isNumber()) {
        if (mstruct[1].isVariable() &&
            mstruct[1].variable()->id() == VARIABLE_ID_PI) {
          if (mstruct[0].number().isI() || mstruct[0].number().isMinusI()) {
            // e^(i*pi)=-1
            set(m_minus_one, true);
            return 1;
          } else if (mstruct[0].number().hasImaginaryPart() &&
                     !mstruct[0].number().hasRealPart() &&
                     mstruct[0].number().internalImaginary()->isRational()) {
            // e^(a*i*pi)=(-1)^(a)
            set(-1, 1, 0, true);
        ...
        } else if (mstruct[0].number().isI() && mstruct[1].isFunction() &&
                   mstruct[1].function()->id() == FUNCTION_ID_ATAN &&
                   mstruct[1].size() == 1 &&
                   !mstruct[1][0].containsUnknowns() &&
                   ((eo.expand != 0 && eo.expand > -2) ||
                    !mstruct[1][0].containsInterval(true, false, false,
                                                    eo.expand == -2 ? 1 : 0))) {
          ...
          return 1;
                 ...
                 mstruct.function()->id() == FUNCTION_ID_LOG &&
                 mstruct.size() == 1) {
        if (mstruct[0].representsNumber() &&
            (eo.allow_infinite || mstruct[0].representsNonZero())) {
          // e^ln(x)=x; x!=0
      ...
      if (o_function->id() == FUNCTION_ID_ABS && SIZE == 1) {
        if (mstruct.representsEven() && CHILD(0).representsReal(true)) {
          ...
          return 1;
                 ...
                 CHILD(0).representsReal(true) &&
                 ((CHILD(1).isZero() && mstruct.representsPositive()) ||
                  CHILD(1).isOne())) {
        ...
        if (mstruct.representsEven() && CHILD(0).representsReal(true) &&
          ...
          calculateRaiseExponent(eo);
          return 1;
        } else if (mstruct.isNumber() && mstruct.number().isInteger() &&
                   !mstruct.number().isMinusOne()) {
          if (mstruct == CHILD(1)) {
            // root(x, a)^a=x
            SET_CHILD_MAP(0)
            MERGE_APPROX_AND_PREC(mstruct)
  ...
  default_power_merge:
    ...
    if (mstruct.isAddition()) {
      ...
      if (!b) {
        ...
        for (size_t i = 0; i < mstruct.size(); i++) {
          if (!mstruct[i].representsInteger() &&
              (!bneg || !eo.allow_complex || !mstruct[i].isNumber() ||
               !mstruct[i].number().isRational() ||
               !mstruct[i].number().denominatorIsEven())) {
            b = false;
            break;
          }
        }
      ...
      if (b) {
        // try raising the base by each factor separately:
        // x^(a*b*c*...)=(x^a)^(b*c*...))
        MathStructure mthis(*this);
        for (size_t i = 0; i < mstruct.size(); i++) {
          if (i == 0)
            mthis.raise(mstruct[i]);
          // exponent factor must be real and, if base is zero, positive
          if (!mstruct[i].representsReal() ||
              (isZero() && !mstruct[i].representsPositive()))
            continue;
          if (i > 0)
            mthis[1] = mstruct[i];
          EvaluationOptions eo2 = eo;
          ...
          if (mthis.calculateRaiseExponent(eo2)) {
            if ((mthis.isPower() &&
                 ((m_type == STRUCT_FUNCTION &&
                   o_function->id() == FUNCTION_ID_ABS && SIZE == 1 &&
                   CHILD(0).equals(mthis[0], true, true)) ||
                  (is_negation(mthis[0], *this))))) {
              mthis = *this;
              mthis.raise(m_zero);
              continue;
            }
            bool b_calc = (m_type != STRUCT_NUMBER || !mstruct[i].isNumber() ||
                           mthis.isNumber());
            set(mthis);
            if (mstruct.size() == 2) {
              if (i == 0) {
            ...
            return 1;
               ...
               !mstruct.number().isInteger() &&
               !mstruct.number().numeratorIsOne() &&
               !mstruct.number().numeratorIsMinusOne()) {
      // x^(n/d)
      if (representsNonNegative(true) &&
          (m_type != STRUCT_FUNCTION || o_function->id() != FUNCTION_ID_ABS)) {
        // x>0
        if (isMultiplication() && SIZE == 2 && CHILD(0).isMinusOne() &&
            mstruct.number().numeratorIsEven()) {
          // (-x)^(n/d), n is even
          bool b;
          if (mstruct.number().isNegative()) {
            // n<0: test x^(-n)
            ...
            if (b && mtest.isPower() &&
                mtest[1] == -mstruct.number().numerator())
              b = false;
            if (!b)
              break;
            // (-x)^(-n/d)=(x^n)^(-1/d)
...
int MathStructure::merge_logical_and(MathStructure &mstruct,
                                     const EvaluationOptions &eo,
                                     MathStructure *mparent, size_t index_this,
                                     size_t index_mstruct, bool) {
  ...
  if (eo.test_comparisons && isLogicalOr()) {
    ...
    if (CHILD(0) == mstruct[0]) {
      ...
      case COMPARISON_RESULT_NOT_EQUAL: {
        if (ct_comp == COMPARISON_EQUALS && ct2 == COMPARISON_EQUALS) {
          ...
          MERGE_APPROX_AND_PREC(mstruct)
          return 1;
        } else if (ct_comp == COMPARISON_EQUALS &&
                   ct2 == COMPARISON_NOT_EQUALS) {
          MERGE_APPROX_AND_PREC(mstruct)
          return 2;
        } else if (ct_comp == COMPARISON_NOT_EQUALS &&
                   ct2 == COMPARISON_EQUALS) {
          if (mparent) {
            mparent->swapChildren(index_this + 1, index_mstruct + 1);
          } else {
            set_nocopy(mstruct, true);
      ...
      case COMPARISON_RESULT_EQUAL: {
        ...
        if (ct_comp == COMPARISON_NOT_EQUALS) {
          if (ct2 == COMPARISON_LESS || ct2 == COMPARISON_EQUALS_LESS) {
            ...
            if (ct2 == COMPARISON_LESS)
              return 3;
            return 1;
          } else if (ct2 == COMPARISON_GREATER ||
                     ct2 == COMPARISON_EQUALS_GREATER) {
          ...
          if (ct_comp == COMPARISON_EQUALS)
            return 2;
          ct_comp = COMPARISON_EQUALS;
          if (ct2 == COMPARISON_EQUALS)
            return 3;
          return 1;
        } else if ((ct_comp == COMPARISON_LESS ||
                    ct_comp == COMPARISON_EQUALS_LESS) &&
                   (ct2 == COMPARISON_LESS || ct2 == COMPARISON_EQUALS_LESS)) {
          if (ct_comp == COMPARISON_LESS)
            return 2;
          ct_comp = COMPARISON_LESS;
          if (ct2 == COMPARISON_LESS)
      ...
      case COMPARISON_RESULT_EQUAL_OR_GREATER: {
        ...
        case COMPARISON_EQUALS_GREATER: {
          ct1 = COMPARISON_EQUALS_LESS;
          break;
        }
        case COMPARISON_LESS: {
          ct1 = COMPARISON_GREATER;
          break;
        }
        case COMPARISON_EQUALS_LESS: {
          ct1 = COMPARISON_EQUALS_GREATER;
          break;
        }
        default: {
        }
      ...
      case COMPARISON_RESULT_EQUAL_OR_LESS: {
        ...
        case COMPARISON_LESS: {
          if (ct2 == COMPARISON_GREATER || ct2 == COMPARISON_EQUALS) {
            ...
            MERGE_APPROX_AND_PREC(mstruct)
            return 1;
          } else if (ct2 == COMPARISON_LESS || ct2 == COMPARISON_EQUALS_LESS ||
                     ct2 == COMPARISON_NOT_EQUALS) {
            MERGE_APPROX_AND_PREC(mstruct)
            return 2;
          }
      ...
      case COMPARISON_RESULT_LESS: {
        ...
        case COMPARISON_EQUALS: {
          ...
          case COMPARISON_GREATER: {
            MERGE_APPROX_AND_PREC(mstruct) clear(true);
            return 1;
          }
          case COMPARISON_NOT_EQUALS: {
          }
          ...
          case COMPARISON_LESS: {
            MERGE_APPROX_AND_PREC(mstruct) return 2;
          }
          default: {
          }
          }
      ...
      default: {
        if (!eo.test_comparisons)
          return -1;
        if (comparisonType() == COMPARISON_EQUALS &&
            !CHILD(1).contains(mstruct[0])) {
          mstruct.calculateReplace(CHILD(0), CHILD(1), eo);
          if (eo.isolate_x) {
            mstruct.isolate_x(eo, eo);
          ...
          return 1;
        } else if (mstruct.comparisonType() == COMPARISON_EQUALS &&
                   !mstruct[1].contains(CHILD(0))) {
    ...
    } else if (eo.test_comparisons && comparisonType() == COMPARISON_EQUALS &&
               !CHILD(0).isNumber() && !CHILD(0).containsInterval() &&
               (CHILD(1).isNumber() || eo.isolate_x) &&
               mstruct.contains(CHILD(0)) && !CHILD(1).contains(CHILD(0)) &&
               !CHILD(1).contains(mstruct[0])) {
      mstruct.calculateReplace(CHILD(0), CHILD(1), eo);
      if (eo.isolate_x) {
        mstruct.isolate_x(eo, eo);
      ...
      calculateLogicalAndLast(eo);
      return 1;
    } else if (eo.test_comparisons &&
               mstruct.comparisonType() == COMPARISON_EQUALS &&
               !mstruct[0].isNumber() && !mstruct[0].containsInterval() &&
               (mstruct[1].isNumber() || eo.isolate_x) &&
               contains(mstruct[0]) && !mstruct[1].contains(mstruct[0]) &&
               !mstruct[1].contains(CHILD(0))) {
      calculateReplace(mstruct[0], mstruct[1], eo);
      if (eo.isolate_x) {
        isolate_x(eo, eo);
...
int MathStructure::merge_logical_or(MathStructure &mstruct,
                                    const EvaluationOptions &eo,
                                    MathStructure *mparent, size_t index_this,
                                    size_t index_mstruct, bool) {
    ...
    if (CHILD(0) == mstruct[0]) {
      ...
      case COMPARISON_RESULT_EQUAL: {
        ...
        case COMPARISON_EQUALS: {
          ...
          case COMPARISON_NOT_EQUALS: {
            set(1, 1, 0, true);
            MERGE_APPROX_AND_PREC(mstruct) return 1;
          }
          case COMPARISON_EQUALS_LESS: {
          }
          case COMPARISON_EQUALS_GREATER: {
            ct_comp = ct2;
            MERGE_APPROX_AND_PREC(mstruct) return 3;
          }
          case COMPARISON_LESS: {
            ct_comp = COMPARISON_EQUALS_LESS;
            MERGE_APPROX_AND_PREC(mstruct) return 1;
      ...
      default: {
        return -1;
               ...
               comparisonType() == COMPARISON_NOT_EQUALS &&
               !CHILD(0).isNumber() && !CHILD(0).containsInterval() &&
               (CHILD(1).isNumber() || eo.isolate_x) &&
               mstruct.contains(CHILD(0)) && !CHILD(1).contains(CHILD(0)) &&
               !CHILD(1).contains(mstruct[0])) {
      ...
      return 1;
               ...
               mstruct.comparisonType() == COMPARISON_NOT_EQUALS &&
               !mstruct[0].isNumber() && !mstruct[0].containsInterval() &&
               (mstruct[1].isNumber() || eo.isolate_x) &&
               contains(mstruct[0]) && !mstruct[1].contains(mstruct[0]) &&
               !mstruct[1].contains(CHILD(0))) {
...
int MathStructure::merge_logical_xor(MathStructure &mstruct,
                                     const EvaluationOptions &eo,
                                     MathStructure *, size_t, size_t, bool) {
  ...
  if ((isLogicalNot() && CHILD(0) == mstruct) ||
      (mstruct.isLogicalNot() && equals(mstruct[0]))) {
    set(1, 1, 0, true);
    MERGE_APPROX_AND_PREC(mstruct)
    return 1;
  }
...
int MathStructure::merge_bitwise_and(MathStructure &mstruct,
                                     const EvaluationOptions &eo,
                                     MathStructure *, size_t, size_t, bool) {
  if (mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.bitAnd(mstruct.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || mstruct.number().isApproximate()) &&
        (eo.allow_complex || !nr.isComplex() || o_number.isComplex() ||
         mstruct.number().isComplex()) &&
  ...
  if (equals(mstruct, true, true) && representsScalar() &&
      mstruct.representsScalar()) {
    MERGE_APPROX_AND_PREC(mstruct)
    return 2;
  }
  ...
  if ((isBitwiseNot() || isLogicalNot()) && CHILD(0) == mstruct &&
      mstruct.representsScalar()) {
    clear(true);
    MERGE_APPROX_AND_PREC(mstruct)
    return 1;
  }
  if ((mstruct.isBitwiseNot() || mstruct.isLogicalNot()) &&
      equals(mstruct[0]) && representsScalar()) {
    clear(true);
    MERGE_APPROX_AND_PREC(mstruct)
    return 1;
  }
...
int MathStructure::merge_bitwise_or(MathStructure &mstruct,
                                    const EvaluationOptions &eo,
                                    MathStructure *, size_t, size_t, bool) {
  if (mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.bitOr(mstruct.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || mstruct.number().isApproximate()) &&
        (eo.allow_complex || !nr.isComplex() || o_number.isComplex() ||
         mstruct.number().isComplex()) &&
...
int MathStructure::merge_bitwise_xor(MathStructure &mstruct,
                                     const EvaluationOptions &eo,
                                     MathStructure *, size_t, size_t, bool) {
  if (mstruct.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.bitXor(mstruct.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || mstruct.number().isApproximate()) &&
        (eo.allow_complex || !nr.isComplex() || o_number.isComplex() ||
         mstruct.number().isComplex()) &&
...
#define MERGE_ALL(FUNC, TRY_LABEL)                                             \
  size_t i2, i3 = SIZE;                                                        \
  bool do_abort = false;                                                       \
...
bool fix_intervals(MathStructure &mstruct, const EvaluationOptions &eo,
                   bool *failed, long int min_precision, bool function_middle) {
  if (mstruct.type() == STRUCT_NUMBER) {
    if (eo.interval_calculation != INTERVAL_CALCULATION_NONE) {
      if (!mstruct.number().isInterval(false) &&
          mstruct.number().precision() >= 0 &&
          (CALCULATOR->usesIntervalArithmetic() ||
           mstruct.number().precision() <= PRECISION + 10)) {
  ...
  } else if (mstruct.type() == STRUCT_FUNCTION &&
             (mstruct.function()->id() == FUNCTION_ID_INTERVAL ||
              mstruct.function()->id() == FUNCTION_ID_UNCERTAINTY)) {
    if (eo.interval_calculation == INTERVAL_CALCULATION_NONE) {
      bool b = mstruct.calculateFunctions(eo, false);
      if (b) {
        fix_intervals(mstruct, eo, failed, function_middle);
        return true;
      } else if (function_middle && mstruct.type() == STRUCT_FUNCTION &&
                 mstruct.function()->id() == FUNCTION_ID_INTERVAL &&
                 mstruct.size() == 2) {
...
bool contains_zero_unit(const MathStructure &mstruct) {
  ...
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (contains_zero_unit(mstruct[i]))
      return true;
  }
  return false;
}
...
bool test_var_int(const MathStructure &mstruct, bool *v = NULL) {
  if (mstruct.isVariable() && (mstruct.variable()->id() == VARIABLE_ID_E ||
                               mstruct.variable()->id() == VARIABLE_ID_PI)) {
    if (!v)
      return true;
    if (*v)
      return false;
    ...
    if (!v) {
      if (mstruct.number().isInterval()) {
        ...
        if (nr_int.isInterval()) {
          ...
          nr_int.floor();
          nr_int2.ceil();
          return nr_int2 == nr_int + 1 &&
                 (mstruct.number() < nr_int || mstruct.number() > nr_int) &&
                 (mstruct.number() < nr_int2 || mstruct.number() > nr_int2);
        }
      ...
      if (mstruct.isApproximate()) {
        ...
        Number nr_c(nr_f);
        nr_c++;
        return comparison_is_not_equal(
                   mstruct.number().compareApproximately(nr_f)) &&
               comparison_is_not_equal(
                   mstruct.number().compareApproximately(nr_c));
      }
  ...
  if (mstruct.isMultiplication() || mstruct.isAddition() ||
      (mstruct.isPower() && mstruct[1].isInteger())) {
    bool v2 = false;
    if (!v)
      v = &v2;
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (!test_var_int(mstruct[i], v))
        return false;
    }
    if (*v)
      return true;
...
bool test_non_integer(const MathStructure &mstruct, const EvaluationOptions &) {
  if (test_var_int(mstruct))
    return true;
  if (!mstruct.isApproximate()) {
    if ((mstruct.isMultiplication() || mstruct.isAddition()) &&
        mstruct.size() >= 2 && mstruct[0].isNumber() &&
        mstruct[0].number().isReal() && !mstruct[0].number().isInterval() &&
        !mstruct[0].number().isInteger()) {
      for (size_t i = 1; i < mstruct.size(); i++) {
        if (!mstruct[i].representsInteger())
          return false;
      }
...
bool MathStructure::factorizeUnits() {
  switch (m_type) {
  case STRUCT_ADDITION: {
    if (containsType(STRUCT_DATETIME, false, true, false) > 0)
      return false;
    bool b = false;
    ...
    for (size_t i = 0; i < mstruct_units.size(); i++) {
      ...
      if (mstruct_units[i].isMultiplication()) {
        ...
        if (mstruct_units[i].size() == 0)
          mstruct_units[i].setUndefined();
        else if (mstruct_units[i].size() == 1)
          mstruct_units[i].setToChild(1);
        ...
        if (mstruct_new[i].size() == 0)
          mstruct_new[i].set(1, 1, 0);
        else if (mstruct_new[i].size() == 1)
          mstruct_new[i].setToChild(1);
...
int contains_temp_unit(const MathStructure &m, bool top = true) {
  if (m.isUnit() &&
      (m.unit() == CALCULATOR->getUnitById(UNIT_ID_KELVIN) ||
       m.unit()->containsRelativeTo(CALCULATOR->getUnitById(UNIT_ID_KELVIN)))) {
    return 1;
  } else if (m.isPower() && m[0].isUnit() &&
             (m[0].unit() == CALCULATOR->getUnitById(UNIT_ID_KELVIN) ||
              m[0].unit()->containsRelativeTo(
                  CALCULATOR->getUnitById(UNIT_ID_KELVIN)))) {
    return 2;
    ...
    for (size_t i = 0; i < m.size(); i++) {
      if (!i_ret) {
        ...
        if (i_ret == 2 || (i_ret && b_unit))
          return 2;
        if (!i_ret && !b_unit &&
            m[i].containsType(STRUCT_UNIT, false, false, false))
          b_unit = true;
      } else if (!b_unit &&
                 m[i].containsType(STRUCT_UNIT, false, false, false)) {
        return 2;
...
bool MathStructure::calculatesub(const EvaluationOptions &eo,
                                 const EvaluationOptions &feo, bool recursive,
                                 MathStructure *mparent, size_t index_this) {
  ...
  // the normal process here is the first calculate all children (if recursive)
  // and try merging every child with every other child

  switch (m_type) {
  case STRUCT_VARIABLE: {
    if (eo.calculate_variables && o_variable->isKnown()) {
      // replace variable with value of calculation is approximate or variable
      // is exact (variable is approximate if marked as approximate or value
      // contains interval)
      if ((eo.approximation == APPROXIMATION_APPROXIMATE ||
           (!o_variable->isApproximate() &&
            !((KnownVariable *)o_variable)
                 ->get()
                 .containsInterval(true, false, false, 0, true))) &&
          !((KnownVariable *)o_variable)->get().isAborted()) {
        ...
        // replace precision with interval and vice versa depending on interval
        // calculation mode (foremost relevant for INTERVAL_CALCULATION_NONE)
  ...
  case STRUCT_POWER: {
    ...
    if (eo.sync_units && CHILD(0).isUnit() &&
        CALCULATOR->getTemperatureCalculationMode() ==
            TEMPERATURE_CALCULATION_RELATIVE &&
        !CHILD(1).isOne()) {
      if (CHILD(0).unit() == CALCULATOR->getUnitById(UNIT_ID_CELSIUS) &&
          CALCULATOR->getUnitById(UNIT_ID_KELVIN))
        CHILD(0).setUnit(CALCULATOR->getUnitById(UNIT_ID_KELVIN));
      else if (CHILD(0).unit() == CALCULATOR->getUnitById(UNIT_ID_FAHRENHEIT) &&
               CALCULATOR->getUnitById(UNIT_ID_RANKINE))
        CHILD(0).setUnit(CALCULATOR->getUnitById(UNIT_ID_RANKINE));
    }
  ...
  case STRUCT_ADDITION: {
    ...
    if (eo.sync_units &&
        (syncUnits(false, &found_nonlinear_relations, true, feo) ||
         (found_nonlinear_relations && eo.sync_nonlinear_unit_relations))) {
      if (found_nonlinear_relations && eo.sync_nonlinear_unit_relations) {
        ...
        for (size_t i = 0; i < SIZE; i++) {
          ...
          if (temp_unit_found == 1)
            temp_unit_found++;
          if (!temp_unit_found && CALCULATOR->getTemperatureCalculationMode() ==
                                      TEMPERATURE_CALCULATION_RELATIVE) {
            temp_unit_found = contains_temp_unit(CHILD(i));
          }
          if (temp_unit_found > 1) {
            if (CALCULATOR->getUnitById(UNIT_ID_FAHRENHEIT) &&
                CALCULATOR->getUnitById(UNIT_ID_RANKINE))
              CHILD(i).replace(CALCULATOR->getUnitById(UNIT_ID_FAHRENHEIT),
                               CALCULATOR->getUnitById(UNIT_ID_RANKINE));
            if (CALCULATOR->getUnitById(UNIT_ID_CELSIUS) &&
                CALCULATOR->getUnitById(UNIT_ID_KELVIN))
              CHILD(i).replace(CALCULATOR->getUnitById(UNIT_ID_CELSIUS),
                               CALCULATOR->getUnitById(UNIT_ID_KELVIN));
          }
        }
        CHILDREN_UPDATED;
  ...
  case STRUCT_MULTIPLICATION: {
    ...
    if (eo.sync_units && CALCULATOR->getTemperatureCalculationMode() ==
                             TEMPERATURE_CALCULATION_RELATIVE) {
      int temp_unit_found = 0;
      ...
      if (temp_unit_found > 1) {
        replace(CALCULATOR->getUnitById(UNIT_ID_FAHRENHEIT),
                CALCULATOR->getUnitById(UNIT_ID_RANKINE));
        replace(CALCULATOR->getUnitById(UNIT_ID_CELSIUS),
                CALCULATOR->getUnitById(UNIT_ID_KELVIN));
      }
      if (temp_unit_found)
        CALCULATOR->beginTemporaryStopMessages();
    ...
    if (eo.sync_units &&
        syncUnits(eo.sync_nonlinear_unit_relations, NULL, true, feo)) {
      unformat(eo);
      MERGE_RECURSE
    }
        ...
        for (size_t i = 0; i < SIZE; i++) {
          if (CHILD(i).isNumber()) {
            if (CHILD(i).number().isInterval(false)) {
              ...
              nonintervals++;
              if (nonintervals == 2 && had_interval)
                break;
            }
          }
        }
        if (had_interval && nonintervals >= 2)
          evalSort(false);
      }
      ...
      try_multiply_matrix:
        ...
        if (i2 < i) {
          for (;; i2--) {
            ...
            if (r == 0) {
              SWAP_CHILDREN(i2, i);
              r = CHILD(i2).merge_multiplication(CHILD(i), eo, this, i2, i,
                                                 true);
              if (r < 1) {
                SWAP_CHILDREN(i2, i);
              }
            ...
            if (i2 == 0)
              break;
            if (b_matrix && !CHILD(i2).representsNonMatrix())
              break;
          }
        }
        ...
        for (i2 = i + 1; i2 < SIZE; i2++) {
          if (CALCULATOR->aborted())
            break;
          if (had_matrix && !CHILD(i2).representsNonMatrix())
            continue;
  ...
  case STRUCT_BITWISE_NOT: {
    ...
    case STRUCT_NUMBER: {
      ...
      if (nr.bitNot() &&
          (eo.approximation >= APPROXIMATION_APPROXIMATE ||
           !nr.isApproximate() || CHILD(0).number().isApproximate()) &&
          (eo.allow_complex || !nr.isComplex() ||
           CHILD(0).number().isComplex()) &&
          (eo.allow_infinite || !nr.includesInfinity() ||
           CHILD(0).number().includesInfinity())) {
        set(nr, true);
      }
      break;
    }
    ...
    case STRUCT_BITWISE_OR: {
      CHILD(0).setType(CHILD(0).isBitwiseOr() ? STRUCT_BITWISE_AND
                                              : STRUCT_BITWISE_OR);
  ...
  case STRUCT_LOGICAL_AND: {
    ...
    if (SIZE == 1) {
      if (CHILD(0).representsBoolean() ||
          (mparent && !mparent->isMultiplication() &&
           mparent->representsBoolean())) {
        setToChild(1, false, mparent, index_this + 1);
  ...
  case STRUCT_LOGICAL_NOT: {
    ...
    if (CHILD(0).representsNonZero()) {
      ...
      case COMPARISON_EQUALS: {
        CHILD(0).setComparisonType(COMPARISON_NOT_EQUALS);
        break;
      }
      case COMPARISON_NOT_EQUALS: {
        CHILD(0).setComparisonType(COMPARISON_EQUALS);
        break;
      }
      case COMPARISON_EQUALS_GREATER: {
        CHILD(0).setComparisonType(COMPARISON_LESS);
        break;
      }
      case COMPARISON_EQUALS_LESS: {
        CHILD(0).setComparisonType(COMPARISON_GREATER);
        break;
      }
      case COMPARISON_GREATER: {
        CHILD(0).setComparisonType(COMPARISON_EQUALS_LESS);
        break;
      }
      case COMPARISON_LESS: {
        CHILD(0).setComparisonType(COMPARISON_EQUALS_GREATER);
        break;
      }
      }
      setToChild(1, true);
      ...
      setToChild(1);
      setToChild(1);
      if (!representsBoolean() || (mparent && !mparent->isMultiplication() &&
                                   mparent->representsBoolean())) {
      ...
      CHILD(0).setType(CHILD(0).isLogicalOr() ? STRUCT_LOGICAL_AND
                                              : STRUCT_LOGICAL_OR);
      for (size_t i = 0; i < CHILD(0).size(); i++) {
  ...
  case STRUCT_COMPARISON: {
    EvaluationOptions eo2 = eo;
    if (eo2.assume_denominators_nonzero == 1)
      eo2.assume_denominators_nonzero = false;
    if (recursive) {
    ...
    if (CHILD(1).isZero() && !CHILD(0).isNumber() &&
        CHILD(0).representsBoolean()) {
      if (ct_comp == COMPARISON_EQUALS) {
        ...
        break;
                 ...
                 ct_comp == COMPARISON_LESS || ct_comp == COMPARISON_GREATER) {
        SET_CHILD_MAP(0)
        b = true;
        break;
      }
    ...
    if (CHILD(0).isMultiplication() && CHILD(1).isMultiplication()) {
      ...
      while (i1 < CHILD(0).size() && i2 < CHILD(1).size()) {
        if (CHILD(0)[i1] == CHILD(1)[i2] &&
            CHILD(0)[i1].representsPositive(true)) {
      ...
      if (CHILD(0).size() == 1)
        ...
        CHILD(0).set(1, 1, 0, true);
      if (CHILD(1).size() == 1)
        CHILD(1).setToChild(1, true);
      else if (CHILD(1).size() == 0)
        CHILD(1).set(1, 1, 0, true);
    }
    if (((CHILD(0).isNumber() ||
          (CHILD(0).isVariable() && !CHILD(0).variable()->isKnown() &&
           ((UnknownVariable *)CHILD(0).variable())->interval().isNumber())) &&
         (CHILD(1).isNumber() ||
          ((CHILD(1).isVariable() && !CHILD(1).variable()->isKnown() &&
            ((UnknownVariable *)CHILD(1).variable())
                ->interval()
                .isNumber())))) ||
        (CHILD(0).isDateTime() && CHILD(1).isDateTime())) {
      ComparisonResult cr;
      if (CHILD(0).isNumber()) {
        if (CHILD(1).isNumber())
          cr = CHILD(1).number().compareApproximately(CHILD(0).number());
        else
          cr = ((UnknownVariable *)CHILD(1).variable())
                   ->interval()
                   .number()
                   .compareApproximately(CHILD(0).number());
      } else if (CHILD(1).isNumber()) {
        cr = CHILD(1).number().compareApproximately(
            ((UnknownVariable *)CHILD(0).variable())->interval().number());
      } else if (CHILD(1).isVariable()) {
        cr = ((UnknownVariable *)CHILD(1).variable())
                 ->interval()
                 .number()
                 .compareApproximately(((UnknownVariable *)CHILD(0).variable())
                                           ->interval()
                                           .number());
      } else {
      ...
      case COMPARISON_LESS: {
        if (cr == COMPARISON_RESULT_LESS) {
          ...
          b = true;
                   ...
                   cr != COMPARISON_RESULT_NOT_EQUAL) {
          clear(true);
          b = true;
        }
        break;
    ...
    if ((CHILD(0).representsUndefined() &&
         !CHILD(1).representsUndefined(true, true, true)) ||
        (CHILD(1).representsUndefined() &&
         !CHILD(0).representsUndefined(true, true, true))) {
      if (ct_comp == COMPARISON_EQUALS) {
        clear(true);
        b = true;
        break;
    ...
    if ((ct_comp == COMPARISON_EQUALS_LESS || ct_comp == COMPARISON_GREATER) &&
        CHILD(1).isZero()) {
      if (CHILD(0).isLogicalNot() || CHILD(0).isLogicalAnd() ||
          CHILD(0).isLogicalOr() || CHILD(0).isLogicalXor() ||
          CHILD(0).isComparison()) {
        ...
        b = true;
                ...
                ct_comp == COMPARISON_LESS) &&
               CHILD(0).isZero()) {
      if (CHILD(1).isLogicalNot() || CHILD(1).isLogicalAnd() ||
          CHILD(1).isLogicalOr() || CHILD(1).isLogicalXor() ||
    ...
    if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
      if ((CHILD(0).representsReal(true) && CHILD(1).representsComplex(true)) ||
          (CHILD(1).representsReal(true) && CHILD(0).representsComplex(true))) {
        if (ct_comp == COMPARISON_EQUALS) {
          clear(true);
        } else {
          set(1, 1, 0, true);
        ...
        b = true;
      } else if ((CHILD(0).representsZero(true) &&
                  CHILD(1).representsZero(true))) {
        if (ct_comp != COMPARISON_EQUALS) {
          clear(true);
        } else {
          set(1, 1, 0, true);
        ...
        b = true;
      } else if (CHILD(0).isVariable() && !CHILD(0).variable()->isKnown() &&
                 CHILD(0).representsInteger() &&
                 test_non_integer(CHILD(1), eo)) {
        if (ct_comp == COMPARISON_EQUALS)
          clear(true);
        else
          set(1, 1, 0, true);
        b = true;
                  ...
                  ((CHILD(1).isNumber() && CHILD(1).number().isNonZero() &&
                    comparison_is_not_equal(
                        CHILD(1).number().compare(nr_one))) ||
                   CHILD(1).representsNegative())) ||
                 (CHILD(1).representsBoolean() &&
                  ((CHILD(0).isNumber() && CHILD(0).number().isNonZero() &&
                    comparison_is_not_equal(
                        CHILD(0).number().compare(nr_one))) ||
                   CHILD(0).representsNegative()))) {
        if (ct_comp == COMPARISON_EQUALS)
          clear(true);
        else
          set(1, 1, 0, true);
    ...
    if (b)
      break;
    if (CHILD(1).isNumber() && CHILD(0).isVariable() &&
        !CHILD(0).variable()->isKnown()) {
      Assumptions *ass =
          ((UnknownVariable *)CHILD(0).variable())->assumptions();
      if (ass && ass->min()) {
        bool b_inc = ass->includeEqualsMin();
        switch (ct_comp) {
        case COMPARISON_EQUALS: {
          if ((b_inc && CHILD(1).number() < *ass->min()) ||
              (!b_inc && CHILD(1).number() <= *ass->min())) {
            clear(true);
            b = true;
          }
          break;
        ...
        case COMPARISON_EQUALS_LESS: {
          if (b_inc && CHILD(1).number() == *ass->min()) {
            ct_comp = COMPARISON_EQUALS;
            b = true;
          } else if ((b_inc && CHILD(1).number() < *ass->min()) ||
                     (!b_inc && CHILD(1).number() <= *ass->min())) {
            clear(true);
            b = true;
          }
      ...
      if (ass && ass->max() && isComparison()) {
        ...
        switch (ct_comp) {
        case COMPARISON_EQUALS: {
          if ((b_inc && CHILD(1).number() > *ass->max()) ||
              (!b_inc && CHILD(1).number() >= *ass->max())) {
            clear(true);
            b = true;
          }
          break;
        ...
        case COMPARISON_EQUALS_GREATER: {
          if (b_inc && CHILD(1).number() == *ass->max()) {
            ct_comp = COMPARISON_EQUALS;
            b = true;
          } else if ((b_inc && CHILD(1).number() > *ass->max()) ||
                     (!b_inc && CHILD(1).number() >= *ass->max())) {
            clear(true);
            b = true;
          }
    ...
    if (eo.approximation == APPROXIMATION_EXACT && eo.test_comparisons > 0) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (b_ass == 0 || !CALCULATOR->usesIntervalArithmetic()) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          break;
        }
        ...
        if (!b_failed) {
          if (i == 0 && mtest[0].isAddition() && mtest[0].size() > 1 &&
              mtest[1].isZero()) {
            mtest[1] = mtest[0][0];
            mtest[1].negate();
          ...
          CALCULATOR->beginTemporaryStopMessages();
          if (b_ass == 2)
            mtest[0].calculateFunctions(eo3);
    ...
    if (incomp <= 0) {
      if (mtest_new &&
          (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS)) {
        ...
        if (isApproximate()) {
          ...
          if ((a_pos && b_npos) || (a_npos && b_pos) || (a_nneg && b_neg) ||
              (a_neg && b_nneg)) {
            incomp = 1;
          }
        }
  ...
  case STRUCT_FUNCTION: {
    // always calculate certain functions (introduced by operations outside of
    // functions)
    if (o_function->id() == FUNCTION_ID_ABS ||
        o_function->id() == FUNCTION_ID_ROOT ||
        o_function->id() == FUNCTION_ID_INTERVAL ||
        o_function->id() == FUNCTION_ID_UNCERTAINTY ||
        o_function->id() == FUNCTION_ID_SIGNUM ||
        o_function->id() == FUNCTION_ID_DIRAC ||
        o_function->id() == FUNCTION_ID_HEAVISIDE) {
      ...
      if (b) {
        ...
        for (size_t i = 0; i < SIZE; i++) {
          if (CHILD(i).calculatesub(eo2, feo2, true, this, i))
            b = true;
        }
        CHILDREN_UPDATED;
      }
      break;
    } else if (recursive && eo.expand > 0 &&
               (o_function->id() == FUNCTION_ID_NEWTON_RAPHSON ||
                o_function->id() == FUNCTION_ID_SECANT_METHOD)) {
      EvaluationOptions eo2 = eo;
      eo2.expand = -1;
  ...
  default: {
    if (recursive) {
      for (size_t i = 0; i < SIZE; i++) {
        if (CHILD(i).calculatesub(eo, feo, true, this, i))
          b = true;
      }
      CHILDREN_UPDATED;
    }
...
#define MERGE_INDEX(FUNC, TRY_LABEL)                                           \
  bool b = false;                                                              \
  TRY_LABEL:                                                                   \
...
bool MathStructure::calculateMergeIndex(size_t index,
                                        const EvaluationOptions &eo,
                                        const EvaluationOptions &feo,
                                        MathStructure *mparent,
                                        size_t index_this) {
  switch (m_type) {
  case STRUCT_MULTIPLICATION: {
...
bool MathStructure::calculateLogicalOrLast(const EvaluationOptions &eo,
                                           bool check_size,
                                           MathStructure *mparent,
                                           size_t index_this) {
  return calculateLogicalOrIndex(SIZE - 1, eo, check_size, mparent, index_this);
}
bool MathStructure::calculateLogicalOrIndex(size_t index,
                                            const EvaluationOptions &eo,
                                            bool check_size,
                                            MathStructure *mparent,
                                            size_t index_this) {
  ...
  if (index >= SIZE || !isLogicalOr()) {
    CALCULATOR->error(true, "calculateLogicalOrIndex() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
bool MathStructure::calculateLogicalOr(const MathStructure &mor,
                                       const EvaluationOptions &eo,
                                       MathStructure *mparent,
                                       size_t index_this) {
...
bool MathStructure::calculateLogicalXorLast(const EvaluationOptions &eo,
                                            MathStructure *mparent,
                                            size_t index_this) {

  if (!isLogicalXor()) {
    CALCULATOR->error(true, "calculateLogicalXorLast() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
bool MathStructure::calculateLogicalXor(const MathStructure &mxor,
                                        const EvaluationOptions &eo,
                                        MathStructure *mparent,
                                        size_t index_this) {
...
bool MathStructure::calculateLogicalAndLast(const EvaluationOptions &eo,
                                            bool check_size,
                                            MathStructure *mparent,
                                            size_t index_this) {
  return calculateLogicalAndIndex(SIZE - 1, eo, check_size, mparent,
                                  index_this);
}
bool MathStructure::calculateLogicalAndIndex(size_t index,
                                             const EvaluationOptions &eo,
                                             bool check_size,
                                             MathStructure *mparent,
                                             size_t index_this) {
  ...
  if (index >= SIZE || !isLogicalAnd()) {
    CALCULATOR->error(true, "calculateLogicalAndIndex() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
bool MathStructure::calculateLogicalAnd(const MathStructure &mand,
                                        const EvaluationOptions &eo,
                                        MathStructure *mparent,
                                        size_t index_this) {
...
bool MathStructure::calculateInverse(const EvaluationOptions &eo,
                                     MathStructure *mparent,
                                     size_t index_this) {
  return calculateRaise(m_minus_one, eo, mparent, index_this);
}
bool MathStructure::calculateNegate(const EvaluationOptions &eo,
                                    MathStructure *mparent, size_t index_this) {
  if (m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.negate() && (eo.approximation >= APPROXIMATION_APPROXIMATE ||
                        !nr.isApproximate() || o_number.isApproximate())) {
      o_number = nr;
      numberUpdated();
      return true;
...
bool MathStructure::calculateBitwiseNot(const EvaluationOptions &eo,
                                        MathStructure *mparent,
                                        size_t index_this) {
  transform(STRUCT_BITWISE_NOT);
  return calculatesub(eo, eo, false, mparent, index_this);
}
bool MathStructure::calculateLogicalNot(const EvaluationOptions &eo,
                                        MathStructure *mparent,
                                        size_t index_this) {
  transform(STRUCT_LOGICAL_NOT);
  return calculatesub(eo, eo, false, mparent, index_this);
}
bool MathStructure::calculateRaiseExponent(const EvaluationOptions &eo,
                                           MathStructure *mparent,
                                           size_t index_this) {
  if (!isPower()) {
    CALCULATOR->error(true, "calculateRaiseExponent() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
bool MathStructure::calculateRaise(const MathStructure &mexp,
                                   const EvaluationOptions &eo,
                                   MathStructure *mparent, size_t index_this) {
  if (mexp.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.raise(mexp.number(), eo.approximation < APPROXIMATION_APPROXIMATE) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || mexp.number().isApproximate()) &&
        (eo.allow_complex || !nr.isComplex() || o_number.isComplex() ||
         mexp.number().isComplex()) &&
        (eo.allow_infinite || !nr.includesInfinity() ||
         o_number.includesInfinity() || mexp.number().includesInfinity())) {
      o_number = nr;
      numberUpdated();
      return true;
    }
...
bool MathStructure::calculateBitwiseAndLast(const EvaluationOptions &eo,
                                            bool check_size,
                                            MathStructure *mparent,
                                            size_t index_this) {
  return calculateBitwiseAndIndex(SIZE - 1, eo, check_size, mparent,
                                  index_this);
}
bool MathStructure::calculateBitwiseAndIndex(size_t index,
                                             const EvaluationOptions &eo,
                                             bool check_size,
                                             MathStructure *mparent,
                                             size_t index_this) {
  ...
  if (index >= SIZE || !isBitwiseAnd()) {
    CALCULATOR->error(true, "calculateBitwiseAndIndex() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
bool MathStructure::calculateBitwiseAnd(const MathStructure &mand,
                                        const EvaluationOptions &eo,
                                        MathStructure *mparent,
                                        size_t index_this) {
...
bool MathStructure::calculateBitwiseOrLast(const EvaluationOptions &eo,
                                           bool check_size,
                                           MathStructure *mparent,
                                           size_t index_this) {
  return calculateBitwiseOrIndex(SIZE - 1, eo, check_size, mparent, index_this);
}
bool MathStructure::calculateBitwiseOrIndex(size_t index,
                                            const EvaluationOptions &eo,
                                            bool check_size,
                                            MathStructure *mparent,
                                            size_t index_this) {
  ...
  if (index >= SIZE || !isBitwiseOr()) {
    CALCULATOR->error(true, "calculateBitwiseOrIndex() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
bool MathStructure::calculateBitwiseOr(const MathStructure &mor,
                                       const EvaluationOptions &eo,
                                       MathStructure *mparent,
                                       size_t index_this) {
...
bool MathStructure::calculateBitwiseXorLast(const EvaluationOptions &eo,
                                            bool check_size,
                                            MathStructure *mparent,
                                            size_t index_this) {
  return calculateBitwiseXorIndex(SIZE - 1, eo, check_size, mparent,
                                  index_this);
}
bool MathStructure::calculateBitwiseXorIndex(size_t index,
                                             const EvaluationOptions &eo,
                                             bool check_size,
                                             MathStructure *mparent,
                                             size_t index_this) {
  ...
  if (index >= SIZE || !isBitwiseXor()) {
    CALCULATOR->error(true, "calculateBitwiseXorIndex() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
bool MathStructure::calculateBitwiseXor(const MathStructure &mxor,
                                        const EvaluationOptions &eo,
                                        MathStructure *mparent,
                                        size_t index_this) {
...
bool MathStructure::calculateMultiplyLast(const EvaluationOptions &eo,
                                          bool check_size,
                                          MathStructure *mparent,
                                          size_t index_this) {
  return calculateMultiplyIndex(SIZE - 1, eo, check_size, mparent, index_this);
}
bool MathStructure::calculateMultiplyIndex(size_t index,
                                           const EvaluationOptions &eo,
                                           bool check_size,
                                           MathStructure *mparent,
                                           size_t index_this) {
  ...
  if (index >= SIZE || !isMultiplication()) {
    CALCULATOR->error(true, "calculateMultiplyIndex() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
try_multiply_matrix_index:
  ...
  if (index > 0) {
    for (size_t i = index - 1;; i--) {
      ...
      if (r == 0) {
        SWAP_CHILDREN(i, index);
        r = CHILD(i).merge_multiplication(CHILD(index), eo, this, i, index,
                                          true);
        if (r < 1) {
          SWAP_CHILDREN(i, index);
        } else if (r == 2) {
      ...
      if (i == 0)
        break;
      if (b_matrix && !CHILD(i).representsNonMatrix())
        break;
    }
  }
  ...
  bool had_matrix = false;
  for (size_t i = index + 1; i < SIZE; i++) {
    if (had_matrix && !CHILD(i).representsNonMatrix())
      continue;
    if (CALCULATOR->aborted())
      break;
...
bool MathStructure::calculateMultiply(const MathStructure &mmul,
                                      const EvaluationOptions &eo,
                                      MathStructure *mparent,
                                      size_t index_this) {
  if (mmul.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.multiply(mmul.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || mmul.number().isApproximate()) &&
        (eo.allow_complex || !nr.isComplex() || o_number.isComplex() ||
         mmul.number().isComplex()) &&
        (eo.allow_infinite || !nr.includesInfinity() ||
...
bool MathStructure::calculateDivide(const MathStructure &mdiv,
                                    const EvaluationOptions &eo,
                                    MathStructure *mparent, size_t index_this) {
  if (mdiv.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.divide(mdiv.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || mdiv.number().isApproximate()) &&
        (eo.allow_complex || !nr.isComplex() || o_number.isComplex() ||
         mdiv.number().isComplex()) &&
        (eo.allow_infinite || !nr.includesInfinity() ||
...
bool MathStructure::calculateAddLast(const EvaluationOptions &eo,
                                     bool check_size, MathStructure *mparent,
                                     size_t index_this) {
  return calculateAddIndex(SIZE - 1, eo, check_size, mparent, index_this);
}
bool MathStructure::calculateAddIndex(size_t index, const EvaluationOptions &eo,
                                      bool check_size, MathStructure *mparent,
                                      size_t index_this) {

  if (index >= SIZE || !isAddition()) {
    CALCULATOR->error(true, "calculateAddIndex() error: %s. %s",
                      format_and_print(*this).c_str(),
                      _("This is a bug. Please report it."), NULL);
    return false;
  }
...
bool MathStructure::calculateAdd(const MathStructure &madd,
                                 const EvaluationOptions &eo,
                                 MathStructure *mparent, size_t index_this) {
  if (madd.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.add(madd.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || madd.number().isApproximate())) {
      o_number = nr;
      numberUpdated();
      return true;
    }
...
bool MathStructure::calculateSubtract(const MathStructure &msub,
                                      const EvaluationOptions &eo,
                                      MathStructure *mparent,
                                      size_t index_this) {
  if (msub.type() == STRUCT_NUMBER && m_type == STRUCT_NUMBER) {
    Number nr(o_number);
    if (nr.subtract(msub.number()) &&
        (eo.approximation >= APPROXIMATION_APPROXIMATE || !nr.isApproximate() ||
         o_number.isApproximate() || msub.number().isApproximate())) {
      o_number = nr;
      numberUpdated();
      return true;
    }
...
bool MathStructure::calculateFunctions(const EvaluationOptions &eo,
                                       bool recursive, bool do_unformat) {

  if (m_type == STRUCT_FUNCTION && o_function != eo.protected_function &&
      !b_protected && !CALCULATOR->aborted()) {
    ...
    // test if the number of arguments (children) is appropriate for the
    // function
    if (SIZE == 1 && o_function->minargs() > 1 &&
        !CHILD(0).representsScalar()) {
      if (!CHILD(0).isVector()) {
        MathStructure mtest(CHILD(0));
        CALCULATOR->beginTemporaryStopMessages();
        mtest.eval(eo);
        if (mtest.isVector() && mtest.size() >= (size_t)o_function->minargs() &&
            (o_function->maxargs() < 0 ||
             mtest.size() <= (size_t)o_function->maxargs())) {
      ...
      } else if (CHILD(0).size() >= (size_t)o_function->minargs() &&
                 (o_function->maxargs() < 0 ||
                  CHILD(0).size() <= (size_t)o_function->maxargs())) {
        CHILD(0).setType(STRUCT_FUNCTION);
        CHILD(0).setFunction(o_function);
        SET_CHILD_MAP(0)
      }
      if (SIZE >= (size_t)o_function->minargs()) {
        if (o_function->id() == FUNCTION_ID_LOGN)
          CALCULATOR->error(false,
                            _("log() with a single argument is considered "
                              "ambiguous. Please use ln() or log10() instead."),
                            NULL);
        while ((o_function->maxargs() > 0 &&
                SIZE < (size_t)o_function->maxargs()) ||
               (o_function->maxargs() < 0 &&
                !o_function->getDefaultValue(SIZE + 1).empty())) {
          Argument *arg = o_function->getArgumentDefinition(SIZE + 1);
          APPEND(m_zero)
          if (arg)
            arg->parse(&LAST, o_function->getDefaultValue(SIZE));
          else
            CALCULATOR->parse(&LAST, o_function->getDefaultValue(SIZE));
        }
      }
    }
    ...
    if (o_function->maxargs() > -1 && (long int)SIZE > o_function->maxargs()) {
      if (o_function->maxargs() == 1 && o_function->getArgumentDefinition(1) &&
          o_function->getArgumentDefinition(1)->handlesVector() &&
          o_function->getArgumentDefinition(1)->type() !=
              ARGUMENT_TYPE_VECTOR) {
        // for functions which takes exactly one argument: if there are
        // additional children calculate the function separately for each child
        // and place results in vector
        bool b = false;
        for (size_t i2 = 0; i2 < CHILD(0).size(); i2++) {
          CHILD(0)[i2].transform(o_function);
          if (CHILD(0)[i2].calculateFunctions(eo, recursive, do_unformat))
            b = true;
    ...
    for (size_t i = 0; i < SIZE; i++) {
      ...
      if (arg) {
        last_arg = arg;
        last_i = i;
        if (i > 0 && arg->type() == ARGUMENT_TYPE_SYMBOLIC &&
            (CHILD(i).isZero() || CHILD(i).isUndefined())) {
          // argument type is symbol and value is zero or undefined, search the
          // first argument/child for symbols
          CHILD(i).set(CHILD(0).find_x_var(), true);
          if (CHILD(i).isUndefined() && CHILD(0).isVariable() &&
              CHILD(0).variable()->isKnown())
            CHILD(i).set(
                ((KnownVariable *)CHILD(0).variable())->get().find_x_var(),
                true);
          ...
          if (CHILD(i).isUndefined()) {
            CALCULATOR->error(false, _("No unknown variable/symbol was found."),
                              NULL);
        ...
        if (!arg->test(CHILD(i), i + 1, o_function, eo)) {
          if (arg->handlesVector() && arg->type() != ARGUMENT_TYPE_VECTOR &&
              CHILD(i).isVector()) {
            ...
            for (size_t i2 = 0; i2 < CHILD(i).size(); i2++) {
              ...
              for (size_t i3 = 0; i3 < SIZE; i3++) {
                if (i3 < i)
                  CHILD(i)[i2].insertChild(CHILD(i3), i3 + 1);
                else if (i3 > i)
                  CHILD(i)[i2].addChild(CHILD(i3));
              }
              if (CHILD(i)[i2].calculateFunctions(eo, recursive, do_unformat))
                b = true;
        ...
        if (arg->handlesVector() &&
            (arg->type() != ARGUMENT_TYPE_VECTOR || CHILD(i).isMatrix())) {
          if (arg->type() == ARGUMENT_TYPE_VECTOR) {
            CHILD(i).transposeMatrix();
          } else if ((arg->tests() ||
                      (o_function->subtype() == SUBTYPE_USER_FUNCTION &&
                       CHILD(i).containsType(STRUCT_VECTOR, false, true,
                                             false) > 0)) &&
                     !CHILD(i).isVector() && !CHILD(i).representsScalar()) {
            CHILD(i).eval(eo);
            CHILD_UPDATED(i);
          }
          if (CHILD(i).isVector()) {
    ...
    if (last_arg && o_function->maxargs() < 0 &&
        last_i >= o_function->minargs()) {
      // use the last argument to test additional arguments (if number of
      // arguments is unlimited and last argument was above minimum number of
      // arguments)
    ...
    if (ret > 0) {
      ...
      while (mstruct->isVector() && mstruct->size() == 1)
        mstruct->setToChild(1, true);
      set_nocopy(*mstruct, true);
      if (recursive)
      ...
      if (ret < 0) {
        ...
        if (o_function->maxargs() > 0 && ret > o_function->maxargs()) {
          if (mstruct->isVector()) {
            if (do_unformat)
              mstruct->unformat(eo);
            for (size_t arg_i = 1; arg_i <= SIZE && arg_i <= mstruct->size();
                 arg_i++) {
...
int evalSortCompare(const MathStructure &mstruct1,
                    const MathStructure &mstruct2, const MathStructure &parent,
                    bool b_abs = false);
int evalSortCompare(const MathStructure &mstruct1,
                    const MathStructure &mstruct2, const MathStructure &parent,
                    bool b_abs) {
  if (parent.isMultiplication()) {
    if ((!mstruct1.representsNonMatrix() && !mstruct2.representsScalar()) ||
    ...
    if (mstruct1.isMultiplication() && mstruct1.size() > 0) {
      ...
      if (mstruct2.isMultiplication()) {
        ...
        for (size_t i = 0;; i++) {
          ...
          if (i + start >= mstruct1.size())
            return -1;
          i2 = evalSortCompare(mstruct1[i + start], mstruct2[i + start2],
                               parent, b_abs);
          if (i2 != 0)
            return i2;
        }
        ...
        if (i2 != 0)
          return i2;
        if (b_abs && start == 1 &&
            (mstruct1[0].isMinusOne() || mstruct1[0].isOne()))
          return 0;
        return 1;
      }
      ...
      if (mstruct1.isMultiplication()) {
        ...
        if (i2 != 0)
          return i2;
        if (b_abs && start == 1 &&
            (mstruct2[0].isMinusOne() || mstruct2[0].isOne()))
          return 0;
        return -1;
      }
    }
  ...
  case STRUCT_NUMBER: {
    ...
    if (b_abs) {
      ComparisonResult cmp =
          mstruct1.number().compareAbsolute(mstruct2.number());
      if (cmp == COMPARISON_RESULT_LESS ||
          cmp == COMPARISON_RESULT_EQUAL_OR_LESS ||
          cmp == COMPARISON_RESULT_OVERLAPPING_LESS ||
          cmp == COMPARISON_RESULT_CONTAINS)
        return -1;
      else if (cmp == COMPARISON_RESULT_GREATER ||
               cmp == COMPARISON_RESULT_EQUAL_OR_GREATER ||
               cmp == COMPARISON_RESULT_OVERLAPPING_GREATER ||
               cmp == COMPARISON_RESULT_CONTAINED)
        return 1;
      return 0;
    }
    if (!mstruct1.number().hasImaginaryPart() &&
      ...
      if (mstruct1.number().isFloatingPoint()) {
        ...
        if (mstruct1.number().isInterval()) {
          if (!mstruct2.number().isInterval())
            return 1;
        } else if (mstruct2.number().isInterval())
          return -1;
      } else if (mstruct2.number().isFloatingPoint())
        return -1;
  ...
  case STRUCT_VARIABLE: {
    ...
    if (mstruct1.variable() < mstruct2.variable())
      return -1;
    else if (mstruct1.variable() == mstruct2.variable())
      return 0;
    return 1;
  }
  case STRUCT_FUNCTION: {
    if (mstruct1.function()->id() != mstruct2.function()->id()) {
      if (mstruct1.function()->id() == 0)
        return 1;
      if (mstruct1.function()->id() < mstruct2.function()->id())
        return -1;
      return 1;
  ...
  default: {
    if (mstruct2.size() < mstruct1.size())
      return -1;
    else if (mstruct2.size() > mstruct1.size())
      return 1;
    int ie;
...
void MathStructure::evalSort(bool recursive, bool b_abs) {
  ...
  // if(m_type != STRUCT_ADDITION && m_type != STRUCT_MULTIPLICATION && m_type
  // != STRUCT_LOGICAL_AND && m_type != STRUCT_LOGICAL_OR && m_type !=
  // STRUCT_LOGICAL_XOR && m_type != STRUCT_BITWISE_AND && m_type !=
  // STRUCT_BITWISE_OR && m_type != STRUCT_BITWISE_XOR) return;
  if (m_type != STRUCT_ADDITION && m_type != STRUCT_MULTIPLICATION &&
      m_type != STRUCT_BITWISE_AND && m_type != STRUCT_BITWISE_OR &&
      m_type != STRUCT_BITWISE_XOR)
    return;
  ...
  for (size_t i = 0; i < SIZE; i++) {
    if (i == 0) {
      ...
      if (evalSortCompare(CHILD(i), *v_subs[sorted.back()], *this, b_abs) >=
          0) {
        sorted.push_back(v_order[i]);
        ...
        for (size_t i2 = sorted.size() - 2;; i2--) {
          if (evalSortCompare(CHILD(i), *v_subs[sorted[i2]], *this, b_abs) >=
              0) {
