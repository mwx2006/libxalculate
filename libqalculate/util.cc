...
string &remove_parenthesis(string &str) {
  if (str[0] == LEFT_PARENTHESIS_CH &&
      str[str.length() - 1] == RIGHT_PARENTHESIS_CH) {
...
string d2s(double value, int precision) {
  ...
  auto buffer = new char[precision + 21];
...
bool equalsIgnoreCase(const string &str1, const char *str2) {
  if (str1.empty() || strlen(str2) == 0)
    return false;
  for (size_t i1 = 0, i2 = 0; i1 < str1.length() || i2 < strlen(str2);
       i1++, i2++) {
    if (i1 >= str1.length() || i2 >= strlen(str2))
      return false;
    if (((signed char)str1[i1] < 0 && i1 + 1 < str1.length()) ||
        ((signed char)str2[i2] < 0 && i2 + 1 < strlen(str2))) {
      size_t iu1 = 1, iu2 = 1;
      if ((signed char)str1[i1] < 0) {
        while (iu1 + i1 < str1.length() && (signed char)str1[i1 + iu1] < 0) {
          iu1++;
...
string sub_suffix_html(const string &name) {
  ...
  if (b) {
    if (is_in(NUMBERS, name[name.length() - 1])) {
      ...
      while ((signed char)name[name.length() - i2] < 0 &&
             (unsigned char)name[name.length() - i2] < 0xC0 &&
             i2 < name.length()) {
        i2++;
      }
    }
...
string sub_suffix(const string &name, const string &tag_begin,
...
void parse_qalculate_version(string qalculate_version,
                             int *qalculate_version_numbers) {
  for (size_t i = 0; i < 3; i++) {
    ...
    qalculate_version = qalculate_version.substr(
        dot_i + 1, qalculate_version.length() - (dot_i + 1));
  }
}
...
string getLocalDir() {
  ...
  if (dirExists(str))
    return str;
  char path[MAX_PATH];
  SHGetFolderPathA(NULL, CSIDL_LOCAL_APPDATA | CSIDL_FLAG_CREATE, NULL, 0,
                   path);
  str = path;
  return str + "\\Qalculate";
#endif
...
string getGlobalDefinitionsDir() {
  ...
  if (getcwd(buffer, 500)) {
    string dir = buffer;
    if (dirExists(buildPath(dir, "libqalculate")) &&
        fileExists(buildPath(dir, "data", "functions.xml")))
      return buildPath(dir, "data");
    size_t i = dir.rfind("/");
    if (i != string::npos && i > 0 && i < dir.length() - 1)
      dir = dir.substr(0, i);
    if (dirExists(buildPath(dir, "libqalculate")) &&
        fileExists(buildPath(dir, "data", "functions.xml")))
      return buildPath(dir, "data");
...
bool dirExists(string dirpath) { return fileExists(dirpath); }
...
char *locale_from_utf8(const char *str) {
  ...
  if (!buffer)
    return NULL;
  size_t err =
      iconv(conv, (ICONV_CONST char **)&str, &inlength, &buffer, &outlength);
  if (err != (size_t)-1)
    err = iconv(conv, NULL, &inlength, &buffer, &outlength);
  iconv_close(conv);
  memset(buffer, 0, 4);
  if (err == (size_t)-1) {
    free(dest);
...
char *utf8_strdown(const char *str, int l) {
  ...
  if (U_SUCCESS(err)) {
    ...
    if (strcmp(buffer, "á") == 0 || strcmp(buffer, "à") == 0) {
      buffer[0] = 'a';
      buffer[1] = '\0';
    } else if (strcmp(buffer, "é") == 0 || strcmp(buffer, "è") == 0) {
      buffer[0] = 'e';
      buffer[1] = '\0';
    } else if (strcmp(buffer, "í") == 0 || strcmp(buffer, "ì") == 0) {
      buffer[0] = 'i';
      buffer[1] = '\0';
    } else if (strcmp(buffer, "ú") == 0 || strcmp(buffer, "ù") == 0) {
      buffer[0] = 'u';
      buffer[1] = '\0';
    } else if (strcmp(buffer, "ó") == 0 || strcmp(buffer, "ò") == 0 ||
               strcmp(buffer, "õ") == 0) {
      buffer[0] = 'o';
      buffer[1] = '\0';
    } else if (strcmp(buffer, "ñ") == 0) {
      buffer[0] = 'n';
      buffer[1] = '\0';
    }
    return buffer;
...
int checkAvailableVersion(const char *version_id, const char *current_version,
                          string *available_version, int timeout) {
#ifdef HAVE_LIBCURL
  string sbuffer;
  ...
  curl_easy_setopt(curl, CURLOPT_URL,
                   "https://qalculate.github.io/CURRENT_VERSIONS");
  ...
  string s_version;
  if (i2 == string::npos)
    s_version = sbuffer.substr(i + strlen(version_id) + 1);
  else
    s_version = sbuffer.substr(i + strlen(version_id) + 1,
                               i2 - (i + strlen(version_id) + 1));
  remove_blank_ends(s_version);
  if (s_version.empty())
    return -1;
  if (available_version)
    *available_version = s_version;
  if (s_version != current_version) {
    ...
    for (i = 0; i < version_parts_new.size(); i++) {
      if (i == version_parts_old.size() ||
          version_parts_new[i] > version_parts_old[i])
        return true;
      else if (version_parts_new[i] < version_parts_old[i])
        return false;
    }
  }
...
Thread::Thread()
    : running(false), m_thread(NULL), m_threadReadyEvent(NULL), m_threadID(0) {
...
bool Thread::start() {
  if (running)
    return true;
  running =
      pthread_create(&m_thread, &m_thread_attr, &Thread::doRun, this) == 0;
  return running;
}
