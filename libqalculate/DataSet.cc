/*
    ...
    Copyright (C) 2004-2006, 2016, 2019  Hanna Knutsson
...
const MathStructure *DataObject::getPropertyStruct(DataProperty *property) {
  ...
  for (size_t i = 0; i < properties.size(); i++) {
    if (properties[i] == property) {
      if (!m_properties[i])
        m_properties[i] =
            property->generateStruct(s_properties[i], a_properties[i]);
...
bool DataObject::isUserModified() const { return b_uchanged; }
void DataObject::setUserModified(bool user_modified) {
  b_uchanged = user_modified;
}

DataSet *DataObject::parentSet() const { return parent; }

DataProperty::DataProperty(DataSet *parent_set, string s_name, string s_title,
...
void DataProperty::setNameIsReference(size_t index, bool is_ref) {
  if (index > 0 && index <= name_is_ref.size())
    name_is_ref[index - 1] = is_ref;
}
bool DataProperty::nameIsReference(size_t index) const {
  if (index > 0 && index <= name_is_ref.size())
    return name_is_ref[index - 1];
  return false;
}
...
size_t DataProperty::hasName(const string &s_name) {
  for (size_t i = 0; i < names.size(); i++) {
    if (equalsIgnoreCase(s_name, names[i]))
      return i + 1;
  }
  return 0;
}
size_t DataProperty::countNames() const { return names.size(); }
...
const string &DataProperty::description() const { return sdescr; }
...
const string &DataProperty::getUnitString() const { return sunit; }
...
string DataProperty::getInputString(const string &valuestr) {
  string str;
  if (b_brackets && valuestr.length() > 1 && valuestr[0] == '[' &&
      valuestr[valuestr.length() - 1] == ']') {
    str = CALCULATOR->localizeExpression(
  ...
  if (!sunit.empty()) {
    ...
    str += cu.print(
        false, true, CALCULATOR->messagePrintOptions().use_unicode_signs,
        CALCULATOR->messagePrintOptions().can_display_unicode_string_function,
        CALCULATOR->messagePrintOptions().can_display_unicode_string_arg);
  }
  return str;
}
string DataProperty::getDisplayString(const string &valuestr) {
  if (!sunit.empty()) {
    ...
    str += " ";
    CompositeUnit cu("", "", "", sunit);
    return str + cu.print(false, true,
                          CALCULATOR->messagePrintOptions().use_unicode_signs,
...
MathStructure *DataProperty::generateStruct(const string &valuestr,
                                            int is_approximate) {
  ...
  case PROPERTY_NUMBER: {
    if (valuestr.length() > 1 && valuestr[0] == '[' &&
        valuestr[valuestr.length() - 1] == ']') {
      size_t i = valuestr.find(",");
      if (i != string::npos) {
        Number nr;
        nr.setInterval(
            Number(valuestr.substr(1, i - 1)),
            Number(valuestr.substr(i + 1, valuestr.length() - i - 2)));
        mstruct = new MathStructure(nr);
        break;
      } else if (b_brackets) {
        if (((b_approximate && is_approximate < 0) || is_approximate > 0) &&
            valuestr.find(SIGN_PLUSMINUS) == string::npos &&
            valuestr.find("+/-") == string::npos) {
          ParseOptions po;
          po.read_precision = ALWAYS_READ_PRECISION;
          mstruct = new MathStructure(
              Number(valuestr.substr(1, valuestr.length() - 2), po));
        } else {
          mstruct = new MathStructure(
              Number(valuestr.substr(1, valuestr.length() - 2)));
        }
        break;
      }
    }
  ...
  case PROPERTY_STRING: {
    if (b_brackets && valuestr.length() > 1 && valuestr[0] == '[' &&
        valuestr[valuestr.length() - 1] == ']')
      mstruct = new MathStructure(valuestr.substr(1, valuestr.length() - 2));
    else
...
void DataProperty::setCaseSensitive(bool is_case_sensitive) {
  b_case = is_case_sensitive;
}
bool DataProperty::isCaseSensitive() const { return b_case; }
void DataProperty::setUsesBrackets(bool uses_brackets) {
  b_brackets = uses_brackets;
}
bool DataProperty::usesBrackets() const { return b_brackets; }
void DataProperty::setApproximate(bool is_approximate) {
  b_approximate = is_approximate;
}
bool DataProperty::isApproximate() const { return b_approximate; }
void DataProperty::setPropertyType(PropertyType property_type) {
  ptype = property_type;
}
PropertyType DataProperty::propertyType() const { return ptype; }

bool DataProperty::isUserModified() const { return b_uchanged; }
void DataProperty::setUserModified(bool user_modified) {
  b_uchanged = user_modified;
}

DataSet *DataProperty::parentSet() const { return parent; }

DataSet::DataSet(string s_category, string s_name, string s_default_file,
                 string s_title, string s_description, bool is_local)
    : MathFunction(s_name, 1, 2, s_category, s_title, s_description) {
  b_local = is_local;
...
int DataSet::subtype() const { return SUBTYPE_DATA_SET; }

int DataSet::calculate(MathStructure &mstruct, const MathStructure &vargs,
                       const EvaluationOptions &) {
  DataObject *o = getObject(vargs[0]);
  if (!o) {
    CALCULATOR->error(true, _("Object %s not available in data set."),
                      vargs[0].symbol().c_str(), NULL);
    return 0;
  }
  if (equalsIgnoreCase(vargs[1].symbol(), string("info")) ||
      equalsIgnoreCase(vargs[1].symbol(), string(_("info")))) {
  ...
  if (!dp) {
    CALCULATOR->error(true, _("Property %s not available in data set."),
                      vargs[1].symbol().c_str(), NULL);
    return 0;
  }
  const MathStructure *pmstruct = o->getPropertyStruct(dp);
  if (!pmstruct) {
    CALCULATOR->error(true, _("Property %s not defined for object %s."),
                      vargs[1].symbol().c_str(), vargs[0].symbol().c_str(),
                      NULL);
    return 0;
  }
...
const string &DataSet::defaultProperty() const { return getDefaultValue(2); }

void DataSet::setCopyright(string s_copyright) {
  scopyright = s_copyright;
  setChanged(true);
}
const string &DataSet::copyright() const { return scopyright; }
void DataSet::setDefaultDataFile(string s_file) {
  sfile = s_file;
  setChanged(true);
}
const string &DataSet::defaultDataFile() const { return sfile; }
...
bool DataSet::loadObjects(const char *file_name, bool is_user_defs) {
  if (file_name) {
    ...
    if (i != sfile.length() - 1) {
      string filepath =
          buildPath(getLocalDataDir(), "definitions", "datasets",
                    sfile.substr(i + 1, sfile.length() - (i + 1)));
      if (loadObjects(filepath.c_str(), true)) {
        b = true;
      } else {
#ifndef _WIN32
        string filepath_old =
            buildPath(getOldLocalDir(), "definitions", "datasets",
                      sfile.substr(i + 1, sfile.length() - (i + 1)));
    ...
    bool b =
        !isLocal() &&
        loadObjects(buildPath(getGlobalDefinitionsDir(), sfile).c_str(), false);
    string filepath =
        buildPath(getLocalDataDir(), "definitions", "datasets", sfile);
    if (b && !fileExists(filepath))
      return true;
    if (loadObjects(filepath.c_str(), true)) {
      b = true;
    } else {
#ifndef _WIN32
      string filepath_old =
          buildPath(getOldLocalDir(), "definitions", "datasets", sfile);
      if (loadObjects(filepath_old.c_str(), true)) {
        b = true;
        recursiveMakeDir(getLocalDataDir());
        makeDir(buildPath(getLocalDataDir(), "definitions"));
  ...
  if (doc == NULL) {
    if (!is_user_defs) {
      CALCULATOR->error(true, _("Unable to load data objects in %s."),
                        file_name, NULL);
    }
    return false;
  }
  ...
  if (cur == NULL) {
    CALCULATOR->error(
        true, _("File not identified as Qalculate! definitions file: %s."),
        file_name, NULL);
    xmlFreeDoc(doc);
    return false;
  }
    ...
    if (!xmlStrcmp(cur->name, (const xmlChar *)"object")) {
      ...
      for (size_t i = 1; i <= countNames(); i++) {
        if (!xmlStrcmp(cur->name,
                       (const xmlChar *)properties[i]->getName(i).c_str())) {
          b = true;
          break;
        }
      }
        ...
        if (properties[i]->isKey()) {
          for (size_t i2 = 1; i2 <= properties[i]->countNames(); i2++) {
            if (properties[i]->getName(i2).find(' ') != string::npos) {
              ...
              XML_GET_STRING_FROM_PROP(cur, properties[i]->getName(i2).c_str(),
                                       str)
            }
            remove_blank_ends(str);
            if (!str.empty()) {
      ...
      if (is_user_defs && objects_before > 0) {
        for (size_t i = 0; i < p_refs.size(); i++) {
          for (size_t i2 = 0; i2 < objects_before; i2++) {
            if (s_refs[i] == objects[i2]->getProperty(p_refs[i]) ||
                s_refs[i] ==
                    objects[i2]->getNonlocalizedKeyProperty(p_refs[i])) {
              o = objects[i2];
              old_object = true;
              break;
            }
        ...
        for (size_t i = 0; i < properties.size(); i++) {
          for (size_t i2 = 1; i2 <= properties[i]->countNames(); i2++) {
            if (properties[i]->getName(i2).find(' ') != string::npos) {
              ...
              cmp = !xmlStrcmp(
                  child->name,
                  (const xmlChar *)properties[i]->getName(i2).c_str());
            }
            if (cmp) {
              ...
              if (value) {
                if (!xmlStrcmp(value, (const xmlChar *)"false")) {
                  i_approx = 0;
                           ...
                           !xmlStrcmp(value, (const xmlChar *)"true")) {
                  i_approx = 1;
                }
                xmlFree(value);
              } else {
              ...
              if (properties[i]->propertyType() == PROPERTY_STRING) {
                ...
                if (!lang && (ils < 0 || lang_status[ils] < 1 ||
                              properties[i]->isKey())) {
                  if (value) {
                    str = (char *)value;
                    remove_blank_ends(str);
                  ...
                  if (locale == (char *)lang) {
                    ...
                    o->setProperty(properties[i], str, i_approx);
                  } else if ((ils < 0 || lang_status[ils] < 1) &&
                             strlen((char *)lang) >= 2 &&
                             lang[0] == localebase[0] &&
                             lang[1] == localebase[1]) {
                    if (ils < 0) {
                ...
                unc_rel = false;
                uncertainty =
                    xmlGetProp(child, (xmlChar *)"relative_uncertainty");
                if (!uncertainty)
                ...
                if (uncertainty) {
                  if (unc_rel) {
                    str.insert(0, "(");
                    str.insert(
                        0, CALCULATOR->getFunctionById(FUNCTION_ID_UNCERTAINTY)
                               ->referenceName());
                    str += ", ";
        ...
        for (size_t i2 = 0; i2 < properties.size(); i2++) {
          if (properties[i2]->isKey()) {
            ...
            if (approx < 0 && !vstr->empty()) {
              xmlNewProp(newnode,
                         (xmlChar *)properties[i2]->getReferenceName().c_str(),
                         (xmlChar *)vstr->c_str());
            }
          }
        }
        ...
        if (save_global || approx >= 0 || !properties[i2]->isKey()) {
          if (!vstr->empty()) {
            if (properties[i2]->getReferenceName().find(' ') != string::npos) {
              if (save_global &&
                  properties[i2]->propertyType() == PROPERTY_STRING) {
                str = "_";
              } else {
                str = "";
              }
              ...
              gsub(" ", "_", str);
              newnode2 = xmlNewTextChild(newnode, NULL, (xmlChar *)str.c_str(),
                                         (xmlChar *)vstr->c_str());
            } else {
              if (save_global &&
                  properties[i2]->propertyType() == PROPERTY_STRING) {
                ...
                str += properties[i2]->getReferenceName();
                newnode2 =
                    xmlNewTextChild(newnode, NULL, (xmlChar *)str.c_str(),
                                    (xmlChar *)vstr->c_str());
              } else {
            ...
            if (approx >= 0) {
              xmlNewProp(newnode2, (xmlChar *)"approximate",
                         (xmlChar *)b2tf(approx > 0));
            }
          }
        }
      }
...
bool DataSet::objectsLoaded() const { return b_loaded || sfile.empty(); }
...
DataProperty *DataSet::getProperty(string property) {
  if (property.empty())
    return NULL;
  for (size_t i = 0; i < properties.size(); i++) {
    if (properties[i]->hasName(property))
      return properties[i];
  }
  return NULL;
}
    ...
    if (properties[i]->isKey()) {
      ...
      if (dp->isCaseSensitive()) {
        for (size_t i2 = 0; i2 < objects.size(); i2++) {
          if (object == objects[i2]->getProperty(dp) ||
              object == objects[i2]->getNonlocalizedKeyProperty(dp)) {
            return objects[i2];
          }
        }
      } else {
        for (size_t i2 = 0; i2 < objects.size(); i2++) {
          if (equalsIgnoreCase(object, objects[i2]->getProperty(dp)) ||
              equalsIgnoreCase(object,
                               objects[i2]->getNonlocalizedKeyProperty(dp))) {
            return objects[i2];
          }
        }
      }
...
DataObject *DataSet::getObject(const MathStructure &object) {
  if (object.isSymbolic())
    return getObject(object.symbol());
  if (!objectsLoaded())
    loadObjects();
  DataProperty *dp;
  for (size_t i = 0; i < properties.size(); i++) {
...
const MathStructure *DataSet::getObjectProperyStruct(string property,
                                                     string object) {
...
DataPropertyArgument::DataPropertyArgument(DataSet *data_set, string name_,
                                           bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {
  b_text = true;
  o_data = data_set;
}
DataPropertyArgument::DataPropertyArgument(const DataPropertyArgument *arg) {
  set(arg);
  b_text = true;
  o_data = arg->dataSet();
}
DataPropertyArgument::~DataPropertyArgument() {}
bool DataPropertyArgument::subtest(MathStructure &value,
                                   const EvaluationOptions &eo) const {
  if (!value.isSymbolic()) {
    value.eval(eo);
  }
  return value.isSymbolic() && o_data &&
         (o_data->getProperty(value.symbol()) ||
          equalsIgnoreCase(value.symbol(), string("info")) ||
          equalsIgnoreCase(value.symbol(), string(_("info"))));
}
int DataPropertyArgument::type() const { return ARGUMENT_TYPE_DATA_PROPERTY; }
Argument *DataPropertyArgument::copy() const {
  return new DataPropertyArgument(this);
}
...
DataObjectArgument::DataObjectArgument(DataSet *data_set, string name_,
                                       bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {
  b_text = true;
  o_data = data_set;
}
DataObjectArgument::DataObjectArgument(const DataObjectArgument *arg) {
  set(arg);
  b_text = true;
  o_data = arg->dataSet();
}
DataObjectArgument::~DataObjectArgument() {}
bool DataObjectArgument::subtest(MathStructure &value,
                                 const EvaluationOptions &eo) const {
  if (value.isSymbolic())
    return true;
  value.eval(eo);
  ...
  while (dp) {
    if (dp->isKey() &&
        (dp->propertyType() == PROPERTY_EXPRESSION ||
         (value.isNumber() && dp->propertyType() == PROPERTY_NUMBER))) {
      return true;
    }
    dp = o_data->getNextProperty(&it);
  }
  CALCULATOR->error(true,
                    _("Data set \"%s\" has no object key that supports the "
                      "provided argument type."),
                    o_data->title().c_str(), NULL);
  return false;
}
int DataObjectArgument::type() const { return ARGUMENT_TYPE_DATA_OBJECT; }
Argument *DataObjectArgument::copy() const {
  return new DataObjectArgument(this);
}
