...
/**
 * The Unit class both represents a base unit and is the base class for other
 * unit types. Base units are units defined as basis for other units. Meters and
 * seconds are typical base units.
 *
 * For base units, a name is all that is needed.
 * Base units do however normally have three different names defined for use in
 * expressions - abbreviation (ex. "m"), singular ("meter") and plural
 * ("meters").
 */
...
public:
  Unit(std::string cat_, std::string name_, std::string plural_ = "",
       std::string singular_ = "", std::string title_ = "",
       bool is_local = true, bool is_builtin = false, bool is_active = true);
  Unit();
   ...
   *
   * Equivalent to preferredName() for Unit and AliasUnit, but closer to
   * MathStructure::print() for CompositeUnit (prints out base expression).
   */
  virtual std::string print(
      bool plural_, bool short_, bool use_unicode = false,
      bool (*can_display_unicode_string_function)(const char *, void *) = NULL,
      void *can_display_unicode_string_arg = NULL) const;
  virtual std::string print(const PrintOptions &po, bool format = false,
                            int tagtype = 0, bool input = false,
                            bool plural = true) const;
  virtual const std::string &plural(
      bool return_singular_if_no_plural = true, bool use_unicode = false,
      bool (*can_display_unicode_string_function)(const char *, void *) = NULL,
      void *can_display_unicode_string_arg = NULL) const;
  virtual const std::string &singular(
      bool return_abbreviation_if_no_singular = true, bool use_unicode = false,
      bool (*can_display_unicode_string_function)(const char *, void *) = NULL,
      void *can_display_unicode_string_arg = NULL) const;
  virtual const std::string &abbreviation(
      bool return_singular_if_no_abbreviation = true, bool use_unicode = false,
      bool (*can_display_unicode_string_function)(const char *, void *) = NULL,
      void *can_display_unicode_string_arg = NULL) const;
  virtual bool isUsedByOtherUnits() const;
  virtual Unit *baseUnit() const;
  virtual MathStructure &convertToBaseUnit(MathStructure &mvalue,
                                           MathStructure &mexp) const;
  virtual MathStructure &convertFromBaseUnit(MathStructure &mvalue,
                                             MathStructure &mexp) const;
  ...
  /** Returns the subtype of the unit, corresponding to which subsubclass the
   * object belongs to.
   *
   * @returns ::UnitSubtype.
   */
  virtual int subtype() const;
  /** If specified unit is a base unit for this unit, directly or with other
   * units in between. Equivalent to u->isParentOf(this).
   */
  virtual bool isChildOf(Unit *u) const;
  /** If this unit is a base unit for specified unit, directly or with other
   * units in between. Equivalent to u->isChildOf(this).
   */
  virtual bool isParentOf(Unit *u) const;
  virtual bool hasNonlinearRelationTo(Unit *u) const;
  virtual bool
  hasApproximateRelationTo(Unit *u, bool check_variables = false,
  ...
  virtual bool hasApproximateRelationToBase(
 ...
 * For example, hour are defined as an alias unit that equals 60 minutes which
 * in turn is defined in relation to seconds.
 *
 * Alias units have an associated base unit, exponent and relation expression.
 * For more complex relations an inverse relation can also be specified for
 * conversion back from the base unit. The base unit must not necessarily be of
 * the base unit class and it is recommended that an alias unit is defined in
 * relation to the closest unit (ex. 1ft = 3 hands, 1 hand = 4 in, and 1 in =
 * 0.0254 m).
 *
 * The relation is usually just a number that tells how large quantity of the
 * base unit is needed to get the alias unit (alias unit = base unit *
 * relation). More complex units can specify the relation as a full-blown
 * expression where '\x' is replaced by the quantity of the base unit and
 * '\y' is the exponent.
 * For example, Degrees Celsius has the relation "\x + 273.15" and the inverse
 * relation "\x - 273.15" to the base unit Kelvin. For simple relations, the
 * reversion is automatic and ought not be defined separately.
 *
 * The precision property inherited from ExpressionItem defines the precision of
 * the relation.
 *
 * The exponent defines the exponential relation to the base unit, so that the
 * alias unit equals the base unit raised to the exponent. For simple unit
 * relations this gives: alias unit = relation * base unit^exponent.
 *
 * Alias units normally have three different names defined for use in
 * expressions - abbreviation (ex. "m"), singular ("meter") and plural
 * ("meters").
 */
...
public:
  AliasUnit(std::string cat_, std::string name_, std::string plural_,
            std::string singular_, std::string title_, Unit *alias,
            std::string relation = "1", int exp = 1, std::string inverse = "",
            bool is_local = true, bool is_builtin = false,
            bool is_active = true);
  ...
  virtual void setUncertainty(std::string standard_uncertainty,
                              bool is_relative = false);
  virtual MathStructure &convertToFirstBaseUnit(MathStructure &mvalue,
                                                MathStructure &mexp) const;
  virtual MathStructure &convertFromFirstBaseUnit(MathStructure &mvalue,
                                                  MathStructure &mexp) const;
  virtual MathStructure &convertToBaseUnit(MathStructure &mvalue,
                                           MathStructure &mexp) const;
  virtual MathStructure &convertFromBaseUnit(MathStructure &mvalue,
  ...
  virtual bool hasNonlinearRelationTo(Unit *u) const;
  virtual bool
  hasApproximateExpression(bool check_variables = false,
                           bool ignore_high_precision_intervals = false) const;
  virtual bool
  hasApproximateRelationTo(Unit *u, bool check_variables = false,
                           bool ignore_high_precision_intervals = false) const;
 ...
 * Composite units often have an alias unit associated with them, as they do not
 * have a reference name on their own. For example, a joule is defined as an
 * alias defined in relation to a composite unit defined as "Newton * meter".
 *
 * The names of composite units are only used to reference the unit in
 * definitions of other units. They can not be used in expressions.
 *
 * Composite units are defined as a composition of units.
 * The units, with prefixes and exponents, can either be added one by one with
 * add() or parsed from an expression (ex. "cm^3/g) with setBaseExpression().
 */
...
public:
  CompositeUnit(std::string cat_, std::string name_, std::string title_ = "",
                std::string base_expression_ = "", bool is_local = true,
   ...
   */
  virtual Unit *get(size_t index, int *exp = NULL,
                    Prefix **prefix = NULL) const;
  ...
  virtual std::string print(const PrintOptions &po, bool format = false,
                            ...
                            bool plural = true) const;
  virtual int subtype() const;
  /** If this unit contains a sub/base unit with a relation to the specified
   * unit.
   */
  virtual bool containsRelativeTo(Unit *u) const;
  virtual bool hasNonlinearRelationToBase() const;
  ...
  generateMathStructure(bool make_division = false,
                        bool set_null_prefixes = false) const;
