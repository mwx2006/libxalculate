...
void parse_and_precalculate_plot(string &expression, MathStructure &mstruct,
...
MathStructure
Calculator::expressionToPlotVector(string expression, const MathStructure &min,
                                   const MathStructure &max, int steps,
                                   MathStructure *x_vector, string x_var,
                                   const ParseOptions &po, int msecs) {
  return expressionToPlotVector(expression, min, max, steps, true, x_vector,
                                x_var, po, msecs);
}
MathStructure Calculator::expressionToPlotVector(
    string expression, const MathStructure &min, const MathStructure &max,
    int steps, bool separate_complex_part, MathStructure *x_vector,
    string x_var, const ParseOptions &po, int msecs) {
  ...
  generate_plotvector(mparse, x_mstruct, min, max, steps,
                      x_vector ? *x_vector : x_v, y_vector, eo);
  ...
  if (y_vector.size() == 0) {
    error(true,
          _("Unable to generate plot data with current min, max and sampling "
            "rate."),
          NULL);
  }
  return y_vector;
...
MathStructure Calculator::expressionToPlotVector(
    string expression, float min, float max, int steps, MathStructure *x_vector,
    string x_var, const ParseOptions &po, int msecs) {
  MathStructure min_mstruct(min), max_mstruct(max);
  ParseOptions po2 = po;
  po2.read_precision = DONT_READ_PRECISION;
  MathStructure y_vector(expressionToPlotVector(expression, min_mstruct,
                                                max_mstruct, steps, true,
                                                x_vector, x_var, po2, msecs));
  return y_vector;
}
MathStructure Calculator::expressionToPlotVector(
    string expression, const MathStructure &min, const MathStructure &max,
    const MathStructure &step, MathStructure *x_vector, string x_var,
    const ParseOptions &po, int msecs) {
  return expressionToPlotVector(expression, min, max, step, true, x_vector,
                                x_var, po, msecs);
}
MathStructure Calculator::expressionToPlotVector(
    string expression, const MathStructure &min, const MathStructure &max,
    const MathStructure &step, bool separate_complex_part,
    MathStructure *x_vector, string x_var, const ParseOptions &po, int msecs) {
  Variable *v = getActiveVariable(x_var);
  MathStructure x_mstruct;
  if (v)
    x_mstruct = v;
  ...
  MathStructure x_v;
  MathStructure y_vector;
  generate_plotvector(mparse, x_mstruct, min, max, step,
                      x_vector ? *x_vector : x_v, y_vector, eo);
  endTemporaryStopMessages();
  endTemporaryStopIntervalArithmetic();
  if (msecs > 0) {
  ...
  if (y_vector.size() == 0) {
    error(
        true,
        _("Unable to generate plot data with current min, max and step size."),
        NULL);
  }
  return y_vector;
}
MathStructure Calculator::expressionToPlotVector(
    string expression, float min, float max, float step,
    MathStructure *x_vector, string x_var, const ParseOptions &po, int msecs) {
  MathStructure min_mstruct(min), max_mstruct(max), step_mstruct(step);
  ParseOptions po2 = po;
  po2.read_precision = DONT_READ_PRECISION;
  MathStructure y_vector(expressionToPlotVector(expression, min_mstruct,
                                                max_mstruct, step_mstruct, true,
                                                x_vector, x_var, po2, msecs));
  return y_vector;
}
MathStructure Calculator::expressionToPlotVector(string expression,
  ...
  beginTemporaryStopMessages();
  MathStructure y_vector(
      mparse.generateVector(x_mstruct, x_vector, eo).eval(eo));
  endTemporaryStopMessages();
  endTemporaryStopIntervalArithmetic();
  if (msecs > 0) {
    if (aborted())
...
bool Calculator::invokeGnuplot(string commands, string commandline_extra,
                               bool persistent) {
  if (priv->persistent_plot)
    persistent = true;
  FILE *pipe = NULL;
  if (!b_gnuplot_open || !gnuplot_pipe || persistent ||
      commandline_extra != gnuplot_cmdline) {
    if (!persistent) {
      closeGnuplot();
    }
    ...
    if (!pipe) {
      error(true,
            _("Failed to invoke gnuplot. Make sure that you have gnuplot "
...
#else
#ifdef HAVE_BYO_GNUPLOT
bool qalc_invoke_gnuplot(vector<std::pair<string, string>>, string, string,
                         bool);
string qalc_gnuplot_data_dir();
#else
bool Calculator::invokeGnuplot(string, string, bool) { return false; }
#endif
#endif
...
bool Calculator::plotVectors(PlotParameters *param,
                             const vector<MathStructure> &y_vectors,
                             const vector<MathStructure> &x_vectors,
                             vector<PlotDataParameters *> &pdps,
                             bool persistent, int msecs) {
    ...
    if (!y_vectors[i].isUndefined()) {
      for (size_t i2 = 0; i2 < y_vectors[i].size() - 1; i2++) {
        if (y_vectors[i][i2].isNumber() &&
            y_vectors[i][i2].number().hasImaginaryPart()) {
          if (y_vectors[i][i2 + 1].isNumber() &&
              y_vectors[i][i2 + 1].number().hasImaginaryPart()) {
            ...
            for (i2 = 0; i2 < y_vectors[i].size(); i2++) {
              if (y_vectors[i][i2].isNumber()) {
                if (y_vectors[i][i2].number().hasImaginaryPart()) {
                  yim_vectors[i][i2].number() =
                      y_vectors[i][i2].number().imaginaryPart();
                }
              } else {
  ...
  if (param->filename.empty()) {
    ...
    if (param->filetype == PLOT_FILETYPE_AUTO) {
      ...
      if (i == string::npos) {
        ...
        error(false, _("No extension in file name. Saving as PNG image."),
              NULL);
      } else {
        string ext =
            param->filename.substr(i + 1, param->filename.length() - (i + 1));
        if (ext == "png") {
          param->filetype = PLOT_FILETYPE_PNG;
        } else if (ext == "ps") {
          ...
          param->filetype = PLOT_FILETYPE_PNG;
          error(false,
                _("Unknown extension in file name. Saving as PNG image."),
                NULL);
        }
      }
    }
  ...
  case PLOT_LEGEND_NONE: {
    plot += "set nokey\n";
    break;
  }
  case PLOT_LEGEND_TOP_LEFT: {
    plot += "set key top left\n";
    break;
  }
  case PLOT_LEGEND_TOP_RIGHT: {
    plot += "set key top right\n";
    break;
  }
  case PLOT_LEGEND_BOTTOM_LEFT: {
    plot += "set key bottom left\n";
    break;
  }
  case PLOT_LEGEND_BOTTOM_RIGHT: {
    plot += "set key bottom right\n";
    break;
  }
  case PLOT_LEGEND_BELOW: {
    plot += "set key below\n";
    break;
  }
  case PLOT_LEGEND_OUTSIDE: {
  ...
  for (size_t serie = 0; serie < y_vectors.size(); serie++) {
    if (y_vectors[serie].isUndefined()) {
      ...
      if (y_vectors[serie].size() > 0 && !y_vectors[serie][0].isNumber()) {
        ...
        } else if (y_vectors[serie][0].isMultiplication() &&
                   y_vectors[serie][0].size() >= 2 &&
                   y_vectors[serie][0][0].isNumber()) {
          munit[serie] = y_vectors[serie][0];
  ...
  if (!param->auto_y_min || !param->auto_y_max) {
    plot += "set yrange [";
    if (!param->auto_y_min)
      plot += d2s(param->y_min);
    plot += ":";
    if (!param->auto_y_max)
      plot += d2s(param->y_max);
    plot += "]";
    plot += "\n";
  }
  if (param->x_log) {
  ...
  for (size_t i = 0; i < y_vectors.size(); i++) {
    if (!y_vectors[i].isUndefined()) {
      ...
      if (y_vectors[i].size() * 2 > samples)
        samples = y_vectors[i].size() * 2;
    }
  }
  plot += "set samples ";
  plot += i2s(samples);
  ...
  for (size_t i_pre = 0; i_pre < y_vectors.size() * 2; i_pre++) {
    size_t i = i_pre / 2;
    if ((i_pre % 2 == 0 && !y_vectors[i].isUndefined()) ||
        (i_pre % 2 == 1 && !yim_vectors[i].isUndefined())) {
      if (file_index != 1) {
        plot += ",";
      }
      ...
      if (i < pdps.size()) {
        ...
        case PLOT_SMOOTHING_UNIQUE: {
          plot += " smooth unique";
          break;
        }
        case PLOT_SMOOTHING_CSPLINES: {
          plot += " smooth csplines";
          break;
        }
        case PLOT_SMOOTHING_BEZIER: {
          plot += " smooth bezier";
          break;
        }
        case PLOT_SMOOTHING_SBEZIER: {
          plot += " smooth sbezier";
          break;
        }
        default: {
        ...
        if (!pdps[i]->title.empty()) {
          ...
          if (i_pre % 2 == 1) {
            title += " : ";
            title += _("imaginary part");
          } else if (!yim_vectors[i].isUndefined()) {
            title += " : ";
            title += _("real part");
          }
          plot += " title \"";
        ...
        case PLOT_STYLE_HISTOGRAM: {
          plot += " with histeps";
          break;
        }
        case PLOT_STYLE_STEPS: {
          plot += " with steps";
          break;
        }
        case PLOT_STYLE_CANDLESTICKS: {
  ...
  for (size_t i_pre = 0; i_pre < y_vectors.size() * 2; i_pre++) {
    ...
    if ((i_pre % 2 == 0 && !y_vectors[serie].isUndefined()) ||
        (i_pre % 2 == 1 && !yim_vectors[serie].isUndefined())) {
      string filename = "gnuplot_data";
      filename += i2s(file_index);
      file_index++;
#ifdef HAVE_GNUPLOT_CALL
      ...
      if (!fdata) {
        error(true, _("Could not create temporary file %s"), filepath.c_str(),
              NULL);
        return false;
      }
#else
      ...
      if (msecs > 0)
        startControl(msecs);
      ComparisonResult ct1 = COMPARISON_RESULT_EQUAL,
                       ct2 = COMPARISON_RESULT_EQUAL;
      size_t last_index = string::npos, last_index2 = string::npos;
      bool check_continuous = pdps[serie]->test_continuous &&
                              (pdps[serie]->style == PLOT_STYLE_LINES ||
                               pdps[serie]->style == PLOT_STYLE_POINTS_LINES);
      bool prev_failed = false;
      const MathStructure *yprev = NULL;
      for (size_t i = 1;
           (i_pre % 2 == 0 && i <= y_vectors[serie].countChildren()) ||
           (i_pre % 2 == 1 && i <= yim_vectors[serie].countChildren());
           i++) {
        ...
        if (i_pre % 2 == 1)
          yvalue = yim_vectors[serie].getChild(i);
        else
          yvalue = y_vectors[serie].getChild(i);
        if (!yvalue->isNumber() && !munit[serie].isZero()) {
          if (yvalue->isMultiplication() && yvalue->size() >= 2 &&
              yvalue->getChild(1)->isNumber()) {
            bool b = false;
            if (yvalue->size() == 2 &&
                yvalue->getChild(2)->equals(munit[serie])) {
              b = true;
            } else if (yvalue->size() > 2 && munit[serie].isMultiplication() &&
                       munit[serie].size() == yvalue->size() - 1) {
              b = true;
        ...
        if (!yvalue->isNumber()) {
          ...
          b_imagzero_y =
              b_real || testComplexZero(&yvalue->number(),
                                        yvalue->number().internalImaginary());
          if (!b_imagzero_y) {
            invalid_nr = true;
          }
        }
        if (serie < x_vectors.size() && !x_vectors[serie].isUndefined() &&
            x_vectors[serie].countChildren() ==
                y_vectors[serie].countChildren()) {
          if (!x_vectors[serie].getChild(i)->isNumber()) {
            invalid_nr = true;
          } else if (!x_vectors[serie].getChild(i)->number().isReal()) {
            b_imagzero_x = testComplexZero(
                &x_vectors[serie].getChild(i)->number(),
                x_vectors[serie].getChild(i)->number().internalImaginary());
            if (!b_imagzero_x) {
              invalid_nr = true;
            }
          }
          if (!invalid_nr) {
            if (b_imagzero_x)
              plot_data +=
                  x_vectors[serie].getChild(i)->number().realPart().print(po);
            else
        ...
        if (!invalid_nr) {
          if (check_continuous && !prev_failed) {
            if (i == 1 || ct2 == COMPARISON_RESULT_UNKNOWN)
              ct = COMPARISON_RESULT_EQUAL;
            else
              ct = yprev->number().compare(yvalue->number());
            if ((ct == COMPARISON_RESULT_GREATER ||
                 ct == COMPARISON_RESULT_LESS) &&
                (ct1 == COMPARISON_RESULT_GREATER ||
                 ct1 == COMPARISON_RESULT_LESS) &&
                (ct2 == COMPARISON_RESULT_GREATER ||
                 ct2 == COMPARISON_RESULT_LESS) &&
                ct1 != ct2 && ct != ct2) {
          ...
          if (b_imagzero_y)
            plot_data += yvalue->number().realPart().print(po);
          else
            plot_data += yvalue->print(po);
          plot_data += "\n";
          prev_failed = false;
        } else if (!prev_failed) {
...
bool Calculator::gnuplotOpen() { return b_gnuplot_open && gnuplot_pipe; }
