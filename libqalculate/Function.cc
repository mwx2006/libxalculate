...
MathFunction::MathFunction(string name_, int argc_, int max_argc_, string cat_,
                           string title_, string descr_, bool is_active)
    : ExpressionItem(cat_, name_, title_, descr_, false, true, is_active) {
  priv = new MathFunction_p;
  // arc = min number of arguments, max_argc = max arguments (if max_argc=-1,
  // place additional arguments in vector)
  argc = argc_;
...
void MathFunction::set(const ExpressionItem *item) {
  if (item->type() == TYPE_FUNCTION) {
    ...
    for (int i = argc + 1; i <= max_argc || !f->getDefaultValue(i).empty();
         i++) {
...
int MathFunction::type() const { return TYPE_FUNCTION; }
int MathFunction::subtype() const { return SUBTYPE_FUNCTION; }
int MathFunction::id() const { return 0; }
...
void MathFunction::setExample(string new_example) { sexample = new_example; }
...
int MathFunction::args() const { return max_argc; }
int MathFunction::minargs() const { return argc; }
int MathFunction::maxargs() const { return max_argc; }
string MathFunction::condition() const { return scondition; }
...
bool MathFunction::testCondition(const MathStructure &vargs) {
  ...
  // create a temporary function from the condition expression (for handling av
  // arguments)
  CALCULATOR->beginTemporaryStopMessages();
  UserFunction test_function("", "CONDITION_TEST_FUNCTION", scondition, false,
                             argc, "", "",
                             max_argc < 0 && !default_values.empty() &&
                                     scondition.find("\\v") == string::npos &&
                                     scondition.find("\\w") == string::npos
                                 ? argc + default_values.size()
                                 : max_argc);
  MathStructure vargs2(vargs);
  if (test_function.maxargs() > 0 &&
      vargs2.size() > (size_t)test_function.maxargs())
    vargs2.resizeVector(test_function.maxargs(), m_zero);
  ...
  if (!mstruct.isNumber() || !mstruct.number().getBoolean()) {
    if (CALCULATOR->showArgumentErrors() && !CALCULATOR->aborted()) {
      CALCULATOR->error(true, _("%s() requires that %s"), name().c_str(),
                        printCondition().c_str(), NULL);
    }
    return false;
  }
  return true;
...
string MathFunction::printCondition() {
  if (scondition.empty())
    return scondition;
  // replace arguments (represented by \x, \y, \z, \a, etc.) with argument
  // definition names
  string str = scondition;
  string svar, argstr;
  Argument *arg;
  int i_args = maxargs();
  bool b_v = i_args < 0 && (default_values.empty() ||
                            scondition.find("\\v") != string::npos ||
                            scondition.find("\\w") != string::npos);
  if (b_v)
    i_args = minargs() + 2;
  else if (i_args < 0)
    i_args = minargs() + default_values.size();
...
int MathFunction::args(const string &argstr, MathStructure &vargs,
                       const ParseOptions &parseoptions) {
  // read arguments from expression (e.g. "52, 2" from expression "sin(52, 2)");
  // used in Calculator::parse()
  ...
  if (maxargs() > 0) {
    ...
    // if last argument is vector or maximum number of arguments is 1 and the
    // only argument allows vector
    last_is_vctr = arg && ((arg->type() == ARGUMENT_TYPE_VECTOR) ||
                           (maxargs() == 1 && arg->handlesVector()));
  }
    ...
    case COMMA_CH: {
      if (pars == 0 && !in_cit1 && !in_cit2) {
        ...
        if (itmp <= maxargs() || args() < 0 || !last_is_vctr) {
          ...
          // and maximum number of arguments is unlimited, use the last argument
          // definition
          if (!arg && itmp > argc && args() < 0 &&
              itmp > (int)last_argdef_index && (int)last_argdef_index > argc) {
          ...
          if (stmp.empty()) {
            if (arg) {
              ...
              if (arg->type() == ARGUMENT_TYPE_TEXT &&
                  getDefaultValue(itmp) == "\"\"")
                arg->parse(mstruct, "", po);
              else
          ...
          if (ignored) {
            ...
            CALCULATOR->error(false,
                              _n("Additional arguments for function %s() were "
                                 "ignored. Function can only use %s argument.",
                                 "Additional arguments for function %s() were "
                                 "ignored. Function can only use %s arguments.",
                                 maxargs()),
                              name().c_str(), i2s(maxargs()).c_str(), NULL);
          }
        } else if (last_is_vctr) {
          // if last argument is a vector, use additional arguments to fill the
          // vector
          if (!vctr_started) {
            if (!vargs[vargs.size() - 1].isVector() ||
                vargs[vargs.size() - 1].size() != 1) {
          ...
          if (stmp.empty()) {
            ...
            if (getArgumentDefinition(maxargs())->type() ==
                    ARGUMENT_TYPE_TEXT &&
                getDefaultValue(itmp) == "\"\"")
              getArgumentDefinition(maxargs())->parse(mstruct, "");
            else
              getArgumentDefinition(maxargs())->parse(mstruct,
                                                      getDefaultValue(itmp));
  ...
  if (!str.empty()) {
    ...
    if (itmp <= maxargs() || args() < 0 || !last_is_vctr) {
      ...
      remove_blank_ends(stmp);
      arg = getArgumentDefinition(itmp);
      if (!arg && itmp > argc && args() < 0 && itmp > (int)last_argdef_index &&
          (int)last_argdef_index > argc) {
        arg = priv->argdefs[last_argdef_index];
      }
      bool ignored = (itmp > maxargs() && args() >= 0);
      if (ignored)
      ...
      if (stmp.empty()) {
        MathStructure *mstruct = new MathStructure();
        if (getArgumentDefinition(maxargs())->type() == ARGUMENT_TYPE_TEXT &&
            getDefaultValue(itmp) == "\"\"")
          getArgumentDefinition(maxargs())->parse(mstruct, "");
        else
          getArgumentDefinition(maxargs())->parse(mstruct,
  ...
  if ((itmp < maxargs() && itmp >= minargs()) ||
      (maxargs() < 0 && itmp >= minargs() &&
       (size_t)itmp - minargs() < default_values.size() &&
       !default_values[itmp - minargs()].empty())) {
    int itmp2 = itmp;
    if (id() == FUNCTION_ID_LOGN)
      CALCULATOR->error(false,
                        _("log() with a single argument is considered "
                          "ambiguous. Please use ln() or log10() instead."),
                        NULL);
    while ((size_t)itmp2 - minargs() < default_values.size() &&
           (maxargs() > 0 || !default_values[itmp2 - minargs()].empty())) {
      arg = getArgumentDefinition(itmp2 + 1);
      MathStructure *mstruct = new MathStructure();
      if (arg->type() == ARGUMENT_TYPE_TEXT &&
          default_values[itmp2 - minargs()] == "\"\"")
        arg->parse(mstruct, "");
      else if (arg)
...
void MathFunction::clearArgumentDefinitions() {
  for (unordered_map<size_t, Argument *>::iterator it = priv->argdefs.begin();
       it != priv->argdefs.end(); ++it) {
    delete it->second;
  }
  priv->argdefs.clear();
  last_argdef_index = 0;
...
bool MathFunction::testArgumentCount(int itmp) {
  if (itmp >= minargs()) {
    if (itmp > maxargs() && maxargs() >= 0 &&
        (maxargs() > 1 || !getArgumentDefinition(1) ||
         !getArgumentDefinition(1)->handlesVector())) {
      CALCULATOR->error(false,
                        _n("Additional arguments for function %s() were "
                           "ignored. Function can only use %s argument.",
                           "Additional arguments for function %s() were "
  ...
  if (b) {
    CALCULATOR->error(
        true,
        _n("You need at least %s argument (%s) in function %s().",
           "You need at least %s arguments (%s) in function %s().", minargs()),
        i2s(minargs()).c_str(), str.c_str(), name().c_str(), NULL);
  } else {
    CALCULATOR->error(true,
                      _n("You need at least %s argument in function %s().",
                         "You need at least %s arguments in function %s().",
                         minargs()),
                      i2s(minargs()).c_str(), name().c_str(), NULL);
  }
  return false;
}
MathStructure
MathFunction::createFunctionMathStructureFromVArgs(const MathStructure &vargs) {
...
MathStructure
MathFunction::createFunctionMathStructureFromSVArgs(vector<string> &svargs) {
  MathStructure mstruct(this, NULL);
...
MathStructure MathFunction::calculate(const string &argv,
...
int MathFunction::parse(MathStructure &mstruct, const string &argv,
...
bool MathFunction::testArguments(MathStructure &vargs) {
  ...
  for (unordered_map<size_t, Argument *>::iterator it = priv->argdefs.begin();
    ...
    if (it->second && it->first > 0 && it->first <= vargs.size()) {
      // for symbols arguments with zero or undefined value, search the first
      // argument for a symbol
      if (it->second->type() == ARGUMENT_TYPE_SYMBOLIC &&
          (vargs[it->first - 1].isZero() ||
           vargs[it->first - 1].isUndefined())) {
        vargs[it->first - 1] = vargs[0].find_x_var();
        if (vargs[it->first - 1].isUndefined() && vargs[0].isVariable() &&
            vargs[0].variable()->isKnown())
          vargs[it->first - 1] =
              ((KnownVariable *)vargs[0].variable())->get().find_x_var();
        ...
        if (vargs[it->first - 1].isUndefined()) {
          vargs[it->first - 1].set(CALCULATOR->getVariableById(VARIABLE_ID_X),
                                   true);
          CALCULATOR->error(false, _("No unknown variable/symbol was found."),
                            NULL);
        }
      }
      if (!it->second->test(vargs[it->first - 1], it->first, this))
        return false;
    }
  }
  if (max_argc < 0 && (int)last > argc &&
      priv->argdefs.find(last) != priv->argdefs.end()) {
...
MathStructure MathFunction::calculate(MathStructure &vargs,
                                      const EvaluationOptions &eo) {
  int itmp = vargs.size();
  if (testArgumentCount(itmp)) {
    ...
    if (!testArguments(vargs) || (ret = calculate(mstruct, vargs, eo)) < 1) {
      if (ret < 0) {
        ...
        if (maxargs() > 0 && ret > maxargs()) {
          if (mstruct.isVector()) {
            for (size_t arg_i = 0;
                 arg_i < vargs.size() && arg_i < mstruct.size(); arg_i++) {
    ...
    if (precision() >= 0 &&
        (precision() < mstruct.precision() || mstruct.precision() < 1))
...
int MathFunction::calculate(MathStructure &, const MathStructure &,
...
void MathFunction::setDefaultValue(size_t arg_, string value_) {
  if ((int)arg_ > argc) {
    while (default_values.size() < arg_ - argc)
...
void MathFunction::appendDefaultValues(MathStructure &vargs) {
  if ((int)vargs.size() < minargs())
    return;
  while ((int)vargs.size() < maxargs() ||
         (maxargs() < 0 &&
          (size_t)vargs.size() - minargs() < default_values.size() &&
          !default_values[vargs.size() - minargs()].empty())) {
...
MathStructure MathFunction::produceVector(const MathStructure &vargs, int begin,
                                          int end) {
  if (begin < 1) {
    begin = minargs() + 1;
    if (begin < 1)
...
MathStructure MathFunction::produceArgumentsVector(const MathStructure &vargs,
                                                   int begin, int end) {
  if (begin < 1) {
    begin = minargs() + 1;
    if (begin < 1)
...
bool MathFunction::representsPositive(const MathStructure &, bool) const {
  return false;
}
bool MathFunction::representsNegative(const MathStructure &, bool) const {
  return false;
}
bool MathFunction::representsNonNegative(const MathStructure &vargs,
                                         bool allow_units) const {
  return representsPositive(vargs, allow_units);
}
bool MathFunction::representsNonPositive(const MathStructure &vargs,
                                         bool allow_units) const {
  return representsNegative(vargs, allow_units);
}
bool MathFunction::representsInteger(const MathStructure &vargs,
                                     bool allow_units) const {
  return representsBoolean(vargs) || representsEven(vargs, allow_units) ||
         representsOdd(vargs, allow_units);
}
bool MathFunction::representsNumber(const MathStructure &vargs,
                                    bool allow_units) const {
  return representsReal(vargs, allow_units) ||
         representsComplex(vargs, allow_units);
}
bool MathFunction::representsRational(const MathStructure &vargs,
                                      bool allow_units) const {
  return representsInteger(vargs, allow_units);
}
bool MathFunction::representsNonComplex(const MathStructure &vargs,
                                        bool allow_units) const {
  return representsReal(vargs, allow_units);
}
bool MathFunction::representsReal(const MathStructure &vargs,
                                  bool allow_units) const {
  return representsRational(vargs, allow_units);
}
bool MathFunction::representsComplex(const MathStructure &, bool) const {
  return false;
}
bool MathFunction::representsNonZero(const MathStructure &vargs,
                                     bool allow_units) const {
  return representsPositive(vargs, allow_units) ||
         representsNegative(vargs, allow_units);
}
bool MathFunction::representsEven(const MathStructure &, bool) const {
  return false;
}
bool MathFunction::representsOdd(const MathStructure &, bool) const {
  return false;
}
bool MathFunction::representsUndefined(const MathStructure &) const {
  return false;
}
bool MathFunction::representsBoolean(const MathStructure &) const {
  return false;
}
...
UserFunction::UserFunction(string cat_, string name_, string formula_,
                           bool is_local, int argc_, string title_,
                           string descr_, int max_argc_, bool is_active)
    : MathFunction(name_, argc_, max_argc_, cat_, title_, descr_, is_active) {
  b_local = is_local;
  b_builtin = false;
  setFormula(formula_, argc_, max_argc_);
  setChanged(false);
}
UserFunction::UserFunction(const UserFunction *function) { set(function); }
string UserFunction::formula() const { return sformula; }
string UserFunction::internalFormula() const { return sformula_calc; }
ExpressionItem *UserFunction::copy() const { return new UserFunction(this); }
void UserFunction::set(const ExpressionItem *item) {
  if (item->type() == TYPE_FUNCTION &&
      item->subtype() == SUBTYPE_USER_FUNCTION) {
    ...
    for (size_t i = 1; i <= ((UserFunction *)item)->countSubfunctions(); i++) {
      ...
      v_precalculate.push_back(
          ((UserFunction *)item)->subfunctionPrecalculated(i));
    }
    if (!v_subs.empty()) {
      bool b = hasChanged();
      setFormula(sformula);
...
int UserFunction::subtype() const { return SUBTYPE_USER_FUNCTION; }
...
bool replace_f_interval(MathStructure &mstruct, const EvaluationOptions &eo) {
  // replace interval() and uncertainty() with numbers with intervals, if
  // possible
  if (mstruct.isFunction() &&
      mstruct.function()->id() == FUNCTION_ID_INTERVAL && mstruct.size() == 2) {
    if (mstruct[0].isNumber() && mstruct[1].isNumber()) {
      ...
      if (create_interval(mstruct, m1, m2))
        return true;
      m1.eval(eo);
      m2.eval(eo);
      if (create_interval(mstruct, m1, m2))
    ...
    return false;
  } else if (eo.interval_calculation != INTERVAL_CALCULATION_NONE &&
             mstruct.isFunction() &&
    ...
    if (mstruct[0].isNumber() && mstruct[1].isNumber()) {
      ...
      if (mstruct[2].number().getBoolean()) {
        ...
        if (set_uncertainty(m1, m2, eo, false))
          return true;
        m1.eval(eo);
        m2.eval(eo);
        if (set_uncertainty(m1, m2, eo, true))
          return true;
        m1 = mstruct[0];
        m2 = mstruct[1];
...
int UserFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                            const EvaluationOptions &eo) {
  ParseOptions po;
  if (b_local)
    po.angle_unit = eo.parse_options.angle_unit;
  if (args() != 0) {
    ...
    for (int i = 0; i < i_args; i++) {
      ...
      if ((!arg || !arg->tests() || arg->type() == ARGUMENT_TYPE_FREE) &&
          mv->containsInterval(true, false, false, 0, true)) {
        size_t count = 0;
        for (size_t i3 = 0; i3 < 1 || (maxargs() < 0 && i3 < 3); i3++) {
          ...
          for (size_t i2 = 0; i2 < priv->v_subs_calc.size(); i2++) {
            pos = 0;
            size_t c2 = 0;
            while ((pos = priv->v_subs_calc[i2].find(svar, pos)) !=
                   string::npos) {
              pos += svar.length();
              c2++;
            }
    ...
    if (maxargs() < 0) {
      if (stmp.find("\\v") != string::npos) {
        v_id.push_back(
            CALCULATOR->addId(new MathStructure(produceVector(vargs)), true));
      ...
      if (stmp.find("\\w") != string::npos) {
        v_id.push_back(CALCULATOR->addId(
            new MathStructure(produceArgumentsVector(vargs)), true));
            ...
            if ((i2 = str.find(svar, i2)) != string::npos) {
              if (i2 != 0 && str[i2 - 1] == '\\') {
                ...
                if (subfunctionPrecalculated(i3 + 1)) {
                  ...
                  str.replace(i2, svar.size(),
                              string("(") + priv->v_subs_calc[i] + ")");
                }
              }
            } else {
              break;
  ...
  if (argc_ < 0) {
    ...
    before_find_in_set_formula:
      if (i < 24 && (i2 = new_formula.find(svar_o, i4)) != string::npos) {
        ...
        i3 = 0;
        if (new_formula.length() > i2 + 2 &&
            new_formula[i2 + 2] == ID_WRAP_LEFT_CH) {
          if ((i3 = new_formula.find(ID_WRAP_RIGHT_CH, i2 + 2)) !=
              string::npos) {
        ...
        while ((i2 = new_formula.find(svar_o, i2 + 1)) != string::npos) {
          if (i2 > 0 && new_formula[i2 - 1] == '\\') {
            ...
            if (i2 + 4 < new_formula.length() &&
                new_formula[i2 + 2] == ID_WRAP_LEFT_CH &&
                (i3 = new_formula.find(ID_WRAP_RIGHT_CH, i2 + 3)) !=
                    string::npos) {
        ...
        for (size_t sub_i = 0; sub_i < priv->v_subs_calc.size(); sub_i++) {
          i2 = 0;
          while ((i2 = priv->v_subs_calc[sub_i].find(svar_o, i2 + 1)) !=
                 string::npos) {
        ...
        before_find_in_vsubs_set_formula:
          if (i < 24 && (i2 = priv->v_subs_calc[sub_i].find(svar_o, i4)) !=
                            string::npos) {
            if (i2 > 0 && priv->v_subs_calc[sub_i][i2 - 1] == '\\') {
              i4 = i2 + 2;
            ...
            i3 = 0;
            if (priv->v_subs_calc[sub_i].length() > i2 + 2 &&
                priv->v_subs_calc[sub_i][i2 + 2] == ID_WRAP_LEFT_CH) {
              if ((i3 = priv->v_subs_calc[sub_i].find(
                       ID_WRAP_RIGHT_CH, i2 + 2)) != string::npos) {
            ...
            optionals = true;
            b = true;
          } else if ((i2 = priv->v_subs_calc[sub_i].find(svar, i5)) !=
                     string::npos) {
            if (i2 > 0 && priv->v_subs_calc[sub_i][i2 - 1] == '\\') {
              i5 = i2 + 2;
              goto before_find_in_vsubs_set_formula;
...
void UserFunction::delSubfunction(size_t index) {
  if (index > 0 && index <= v_subs.size()) {
    ...
    if (index <= priv->v_subs_calc.size())
      priv->v_subs_calc.erase(priv->v_subs_calc.begin() + (index - 1));
  }
...
void UserFunction::setSubfunctionPrecalculated(size_t index,
                                               bool precalculate) {
  if (index > 0 && index <= v_precalculate.size()) {
    setChanged(true);
...
size_t UserFunction::countSubfunctions() const { return v_subs.size(); }
...
Argument::~Argument() {}
Argument *Argument::copy() const { return new Argument(this); }
...
bool Argument::test(MathStructure &value, int index, MathFunction *f,
  ...
  if (!b) {
    if (b_error && (type() != ARGUMENT_TYPE_SYMBOLIC || !value.isUndefined())) {
      if (sname.empty()) {
        CALCULATOR->error(true, _("Argument %s in %s() must be %s."),
                          i2s(index).c_str(), f->name().c_str(),
                          printlong().c_str(), NULL);
      } else {
        CALCULATOR->error(true, _("Argument %s, %s, in %s() must be %s."),
                          i2s(index).c_str(), sname.c_str(), f->name().c_str(),
                          printlong().c_str(), NULL);
      }
    }
    return false;
...
void Argument::parse(MathStructure *mstruct, const string &str,
  ...
  if (b_text) {
    ...
    while (true) {
      size_t pars2 = 1;
      size_t i = pars;
      if (str.length() >= 2 + pars * 2 && str[pars] == LEFT_PARENTHESIS_CH &&
          str[str.length() - 1 - pars] == RIGHT_PARENTHESIS_CH) {
        while (true) {
    ...
    if (str.length() >= 2 + pars * 2) {
      if (str[pars] == ID_WRAP_LEFT_CH &&
          str[str.length() - 1 - pars] == ID_WRAP_RIGHT_CH &&
          str.find(ID_WRAP_RIGHT, pars + 1) == str.length() - 1 - pars) {
        CALCULATOR->parse(mstruct, str.substr(pars, str.length() - pars * 2),
                          po);
        if (mstruct->isDateTime() &&
            !mstruct->datetime()->parsed_string.empty())
          mstruct->set(mstruct->datetime()->parsed_string, false, true);
        return;
      }
      if (str[pars] == '\\' && str[str.length() - 1 - pars] == '\\') {
        CALCULATOR->parse(
            mstruct, str.substr(1 + pars, str.length() - 2 - pars * 2), po);
        if (mstruct->isDateTime() &&
            !mstruct->datetime()->parsed_string.empty())
          mstruct->set(mstruct->datetime()->parsed_string, false, true);
        return;
      ...
      if ((str[pars] == '\"' && str[str.length() - 1 - pars] == '\"') ||
          (str[pars] == '\'' && str[str.length() - 1 - pars] == '\'')) {
        size_t i = pars + 1;
    ...
    string str2;
    if (pars == 0 || (cits / 2) % 2 != 0)
      str2 = str;
    else
      str2 = str.substr(pars + (cits ? 1 : 0),
                        str.length() - pars * 2 - (cits ? 2 : 0));
    if (cits == 0)
      replace_internal_operators(str2);
    if ((cits / 2) % 2 == 0) {
      ...
      if (i != string::npos && i < str2.length() - 2) {
        ...
        while ((i = str2.find(ID_WRAP_LEFT, i)) != string::npos) {
          ...
          bool do_par = (i == 0 || i2 + 1 == str2.length() ||
                         str2[i - 1] != LEFT_PARENTHESIS_CH ||
                         str2[i2 + 1] != RIGHT_PARENTHESIS_CH);
          string str3;
          if (do_par)
            str3 = LEFT_PARENTHESIS_CH;
          if (!m_temp) {
            CALCULATOR->error(true, _("Internal id %s does not exist."),
                              i2s(id).c_str(), NULL);
            str3 += CALCULATOR->getVariableById(VARIABLE_ID_UNDEFINED)
                        ->preferredInputName(true, false, false, true)
                        .name;
          } else {
...
string Argument::name() const { return sname; }
...
string Argument::getCustomCondition() const { return scondition; }

bool Argument::zeroForbidden() const { return !b_zero; }
void Argument::setZeroForbidden(bool forbid_zero) { b_zero = !forbid_zero; }

bool Argument::tests() const { return b_test; }
void Argument::setTests(bool does_test) { b_test = does_test; }

bool Argument::alerts() const { return b_error; }
void Argument::setAlerts(bool does_error) { b_error = does_error; }
bool Argument::suggestsQuotes() const {
  return b_text || type() == ARGUMENT_TYPE_DATE;
}
int Argument::type() const { return ARGUMENT_TYPE_FREE; }
...
void Argument::setHandleVector(bool handle_vector) {
  b_handle_vector = handle_vector;
}
...
void Argument::setRationalPolynomial(bool rational_polynomial) {
  b_rational = rational_polynomial;
}

NumberArgument::NumberArgument(string name_, ArgumentMinMaxPreDefinition minmax,
                               bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {
  fmin = NULL;
  fmax = NULL;
  b_incl_min = true;
  b_incl_max = true;
...
bool NumberArgument::includeEqualsMin() const { return b_incl_min; }
const Number *NumberArgument::min() const { return fmin; }
...
bool NumberArgument::includeEqualsMax() const { return b_incl_max; }
const Number *NumberArgument::max() const { return fmax; }
bool NumberArgument::complexAllowed() const { return b_complex; }
void NumberArgument::setComplexAllowed(bool allow_complex) {
  b_complex = allow_complex;
}
bool NumberArgument::rationalNumber() const { return b_rational_number; }
void NumberArgument::setRationalNumber(bool rational_number) {
  b_rational_number = rational_number;
}
bool NumberArgument::subtest(MathStructure &value,
                             const EvaluationOptions &eo) const {
  if (!value.isNumber()) {
    value.eval(eo);
  }
  /*if(value.isMultiplication() && value.size() == 2 && value[0].isNumber() &&
  value[1].isUnit() && value[1].unit()->baseUnit() ==
  CALCULATOR->getRadUnit()->baseUnit()) { value /= CALCULATOR->getRadUnit();
          value.eval(eo);
  }*/
  if (!value.isNumber() ||
      (b_rational_number && !value.number().isRational())) {
    return false;
  }
  if (!b_complex && value.number().hasImaginaryPart()) {
    if (!value.number().imaginaryPartIsNonZero())
      value.number().clearImaginary();
    else
      return false;
  }
  if (fmin) {
    ComparisonResult cmpr = fmin->compare(value.number());
    if (!(cmpr == COMPARISON_RESULT_GREATER ||
          (b_incl_min && COMPARISON_IS_EQUAL_OR_GREATER(cmpr)))) {
      return false;
    }
  }
  if (fmax) {
    ComparisonResult cmpr = fmax->compare(value.number());
    if (!(cmpr == COMPARISON_RESULT_LESS ||
          (b_incl_max && COMPARISON_IS_EQUAL_OR_LESS(cmpr)))) {
      return false;
    }
  }
  return true;
}
int NumberArgument::type() const { return ARGUMENT_TYPE_NUMBER; }
Argument *NumberArgument::copy() const { return new NumberArgument(this); }
...
string NumberArgument::print() const { return _("number"); }
...
IntegerArgument::IntegerArgument(string name_,
                                 ArgumentMinMaxPreDefinition minmax,
                                 bool does_test, bool does_error,
...
void IntegerArgument::setIntegerType(IntegerType integer_type) {
  i_inttype = integer_type;
}
...
const Number *IntegerArgument::min() const { return imin; }
...
const Number *IntegerArgument::max() const { return imax; }
bool IntegerArgument::subtest(MathStructure &value,
                              const EvaluationOptions &eo) const {
  if (!value.isNumber()) {
    value.eval(eo);
  }
  if (!value.isNumber())
    return false;
  if (!value.number().isInteger() && value.number().isInterval() &&
      value.number().precision(true) > PRECISION + 10) {
    Number nr;
    if (value.number().getCentralInteger(nr))
      value.set(nr, true);
  }
...
int IntegerArgument::type() const { return ARGUMENT_TYPE_INTEGER; }
Argument *IntegerArgument::copy() const { return new IntegerArgument(this); }
...
string IntegerArgument::print() const { return _("integer"); }
...
SymbolicArgument::SymbolicArgument(string name_, bool does_test,
                                   bool does_error)
    : Argument(name_, does_test, does_error) {}
SymbolicArgument::SymbolicArgument(const SymbolicArgument *arg) { set(arg); }
SymbolicArgument::~SymbolicArgument() {}
bool SymbolicArgument::subtest(MathStructure &value,
                               const EvaluationOptions &eo) const {
  if (!value.isSymbolic() &&
      (!value.isVariable() || value.variable()->isKnown())) {
    value.eval(eo);
  }
...
string SymbolicArgument::subprintlong() const {
  return _("an unknown variable/symbol");
}

TextArgument::TextArgument(string name_, bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {
  b_text = true;
}
TextArgument::TextArgument(const TextArgument *arg) {
  set(arg);
  b_text = true;
}
TextArgument::~TextArgument() {}
bool TextArgument::subtest(MathStructure &value,
                           const EvaluationOptions &eo) const {
  if (!value.isSymbolic()) {
    value.eval(eo);
  }
...
DateArgument::DateArgument(string name_, bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {}
DateArgument::DateArgument(const DateArgument *arg) { set(arg); }
DateArgument::~DateArgument() {}
void DateArgument::parse(MathStructure *mstruct, const string &str,
                         const ParseOptions &po) const {
...
bool DateArgument::subtest(MathStructure &value,
                           const EvaluationOptions &eo) const {
  if (!value.isDateTime()) {
    value.eval(eo);
  }
...
string DateArgument::subprintlong() const {
  return string(_("a date")) + " (Y-M-D)";
}

VectorArgument::VectorArgument(string name_, bool does_test, bool allow_matrix,
                               bool does_error)
    : Argument(name_, does_test, does_error) {
...
bool VectorArgument::subtest(MathStructure &value,
                             const EvaluationOptions &eo) const {
  bool subvector = false;
  for (size_t i = 0; i < subargs.size(); i++) {
    if (subargs[i] && (subargs[i]->type() == ARGUMENT_TYPE_VECTOR ||
                       subargs[i]->type() == ARGUMENT_TYPE_MATRIX)) {
      subvector = true;
      break;
    }
  }
  if (subvector && value.isFunction() &&
      (value.function()->id() == FUNCTION_ID_HORZCAT ||
       value.function()->id() == FUNCTION_ID_VERTCAT)) {
  ...
  if (!value.isVector()) {
    if (isLastArgument() && eo.approximation != APPROXIMATION_EXACT &&
...
bool VectorArgument::reoccuringArguments() const { return b_argloop; }
void VectorArgument::setReoccuringArguments(bool reocc) { b_argloop = reocc; }
...
size_t VectorArgument::countArguments() const { return subargs.size(); }
...
MatrixArgument::MatrixArgument(string name_, bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {
  b_square = false;
}
...
bool MatrixArgument::subtest(MathStructure &value,
                             const EvaluationOptions &eo) const {
  value.eval(eo);
  if (!value.isMatrix()) {
    if (value.isVector() &&
        (value.size() == 0 || value[0].representsScalar())) {
...
ExpressionItemArgument::ExpressionItemArgument(string name_, bool does_test,
                                               bool does_error)
    : Argument(name_, does_test, does_error) {
  b_text = true;
}
ExpressionItemArgument::ExpressionItemArgument(
    const ExpressionItemArgument *arg) {
  set(arg);
  b_text = true;
}
ExpressionItemArgument::~ExpressionItemArgument() {}
bool ExpressionItemArgument::subtest(MathStructure &value,
                                     const EvaluationOptions &eo) const {
  if (!value.isSymbolic()) {
    value.eval(eo);
  }
...
int ExpressionItemArgument::type() const {
  return ARGUMENT_TYPE_EXPRESSION_ITEM;
}
Argument *ExpressionItemArgument::copy() const {
  return new ExpressionItemArgument(this);
}
string ExpressionItemArgument::print() const { return _("object"); }
string ExpressionItemArgument::subprintlong() const {
  return _("a valid function, unit or variable name");
}

FunctionArgument::FunctionArgument(string name_, bool does_test,
                                   bool does_error)
    : Argument(name_, does_test, does_error) {
  b_text = true;
}
...
bool FunctionArgument::subtest(MathStructure &value,
                               const EvaluationOptions &eo) const {
  if (!value.isSymbolic()) {
    value.eval(eo);
  }
...
string FunctionArgument::subprintlong() const {
  return _("a valid function name");
}

UnitArgument::UnitArgument(string name_, bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {
  b_text = true;
}
UnitArgument::UnitArgument(const UnitArgument *arg) {
  set(arg);
  b_text = true;
}
UnitArgument::~UnitArgument() {}
bool UnitArgument::subtest(MathStructure &value,
                           const EvaluationOptions &eo) const {
  if (!value.isSymbolic()) {
    value.eval(eo);
  }
...
VariableArgument::VariableArgument(string name_, bool does_test,
                                   bool does_error)
    : Argument(name_, does_test, does_error) {
  b_text = true;
}
VariableArgument::VariableArgument(const VariableArgument *arg) {
  set(arg);
  b_text = true;
}
VariableArgument::~VariableArgument() {}
bool VariableArgument::subtest(MathStructure &value,
                               const EvaluationOptions &eo) const {
  if (!value.isSymbolic()) {
    value.eval(eo);
  }
...
string VariableArgument::subprintlong() const {
  return _("a valid variable name");
}

FileArgument::FileArgument(string name_, bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {
  b_text = true;
}
FileArgument::FileArgument(const FileArgument *arg) {
  set(arg);
  b_text = true;
}
FileArgument::~FileArgument() {}
bool FileArgument::subtest(MathStructure &value,
                           const EvaluationOptions &eo) const {
  if (!value.isSymbolic()) {
    value.eval(eo);
  }
...
BooleanArgument::BooleanArgument(string name_, bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {}
BooleanArgument::BooleanArgument(const BooleanArgument *arg) { set(arg); }
BooleanArgument::~BooleanArgument() {}
bool BooleanArgument::subtest(MathStructure &value,
                              const EvaluationOptions &eo) const {
  if (!value.isNumber()) {
    value.eval(eo);
  }
...
AngleArgument::AngleArgument(string name_, bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {}
...
string AngleArgument::subprintlong() const {
  return _("an angle or a number (using the default angle unit)");
}
void AngleArgument::parse(MathStructure *mstruct, const string &str,
                          const ParseOptions &po) const {
  CALCULATOR->parse(mstruct, str, po);
  if (HAS_DEFAULT_ANGLE_UNIT(po.angle_unit)) {
    if (contains_angle_unit(*mstruct, po))
      return;
  }
...
ArgumentSet::ArgumentSet(string name_, bool does_test, bool does_error)
    : Argument(name_, does_test, does_error) {}
...
bool ArgumentSet::subtest(MathStructure &value,
                          const EvaluationOptions &eo) const {
  for (size_t i = 0; i < subargs.size(); i++) {
    if ((i == 0 || subargs[i]->type() != ARGUMENT_TYPE_MATRIX ||
         !subargs[i]->tests() || !value.representsScalar()) &&
        subargs[i]->test(value, 1, NULL, eo)) {
      return true;
    }
  }
  return false;
...
size_t ArgumentSet::countArguments() const { return subargs.size(); }
