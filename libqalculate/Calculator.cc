...
PrintOptions::PrintOptions()
    : min_exp(EXP_PRECISION), base(BASE_DECIMAL),
      base_display(BASE_DISPLAY_NONE), lower_case_numbers(false),
      lower_case_e(false), number_fraction_format(FRACTION_DECIMAL),
      indicate_infinite_series(false), show_ending_zeroes(true),
      abbreviate_names(true), use_reference_names(false),
      place_units_separately(true), use_unit_prefixes(true),
      use_prefixes_for_all_units(false), use_prefixes_for_currencies(false),
      use_all_prefixes(false), use_denominator_prefix(true),
      negative_exponents(false), short_multiplication(true),
      limit_implicit_multiplication(false), allow_non_usable(false),
      use_unicode_signs(false), multiplication_sign(MULTIPLICATION_SIGN_DOT),
      division_sign(DIVISION_SIGN_DIVISION_SLASH), spacious(true),
      excessive_parenthesis(false), halfexp_to_sqrt(true), min_decimals(0),
      max_decimals(-1), use_min_decimals(true), use_max_decimals(true),
      round_halfway_to_even(false), improve_division_multipliers(true),
      prefix(NULL), is_approximate(NULL),
      can_display_unicode_string_function(NULL),
      can_display_unicode_string_arg(NULL), hide_underscore_spaces(false),
      preserve_format(false), allow_factorization(false),
      spell_out_logical_operators(false), restrict_to_parent_precision(true),
      restrict_fraction_length(false), exp_to_root(false),
      preserve_precision(false), interval_display(INTERVAL_DISPLAY_INTERVAL),
      digit_grouping(DIGIT_GROUPING_NONE),
      date_time_format(DATE_TIME_FORMAT_ISO), time_zone(TIME_ZONE_LOCAL),
      custom_time_zone(0), twos_complement(true),
      hexadecimal_twos_complement(false), binary_bits(0) {}
...
const string &PrintOptions::decimalpoint() const {
  if (decimalpoint_sign.empty())
    return CALCULATOR->getDecimalPoint();
  return decimalpoint_sign;
}

InternalPrintStruct::InternalPrintStruct()
    : depth(0), power_depth(0), division_depth(0), wrap(false), num(NULL),
      den(NULL), re(NULL), im(NULL), exp(NULL), minus(NULL), exp_minus(NULL),
      parent_approximate(false), parent_precision(-1), iexp(NULL) {}

ParseOptions::ParseOptions()
    : variables_enabled(true), functions_enabled(true), unknowns_enabled(true),
      units_enabled(true), rpn(false), base(BASE_DECIMAL),
      limit_implicit_multiplication(false), read_precision(DONT_READ_PRECISION),
      dot_as_separator(false), brackets_as_parentheses(false),
      angle_unit(ANGLE_UNIT_NONE), unended_function(NULL),
      preserve_format(false), default_dataset(NULL),
      parsing_mode(PARSING_MODE_ADAPTIVE), twos_complement(false),
      hexadecimal_twos_complement(false) {}

EvaluationOptions::EvaluationOptions()
    : approximation(APPROXIMATION_TRY_EXACT), sync_units(true),
      sync_nonlinear_unit_relations(true), keep_prefixes(false),
      calculate_variables(true), calculate_functions(true),
      test_comparisons(true), isolate_x(true), expand(true),
      combine_divisions(false), reduce_divisions(true), allow_complex(true),
      allow_infinite(true), assume_denominators_nonzero(true),
      warn_about_denominators_assumed_nonzero(false), split_squares(true),
      keep_zero_units(true), auto_post_conversion(POST_CONVERSION_OPTIMAL),
      mixed_units_conversion(MIXED_UNITS_CONVERSION_DEFAULT),
      structuring(STRUCTURING_SIMPLIFY), isolate_var(NULL),
      do_polynomial_division(true), protected_function(NULL),
      complex_number_form(COMPLEX_NUMBER_FORM_RECTANGULAR),
      local_currency_conversion(true), transform_trigonometric_functions(true),
...
#define PRINT_TIME(x) gettimeofday(&tvtime, NULL); usecs2 = tvtime.tv_usec -
usecs + (tvtime.tv_sec - secs) * 1000000; printf("%s %li\n", x, usecs2); #define
PRINT_TIMEDIFF(x) gettimeofday(&tvtime, NULL); printf("%s %li\n", x,
tvtime.tv_usec - usecs + (tvtime.tv_sec - secs) * 1000000 - usecs2); usecs2 =
tvtime.tv_usec - usecs + (tvtime.tv_sec - secs) * 1000000; #define ADD_TIME1
gettimeofday(&tvtime, NULL); usecs2 = tvtime.tv_usec - usecs + (tvtime.tv_sec -
secs) * 1000000; #define ADD_TIME2 gettimeofday(&tvtime, NULL); usecs3 +=
tvtime.tv_usec - usecs + (tvtime.tv_sec - secs) * 1000000 - usecs2; */
...
string CalculatorMessage::message() const { return smessage; }
const char *CalculatorMessage::c_message() const { return smessage.c_str(); }
MessageType CalculatorMessage::type() const { return mtype; }
int CalculatorMessage::stage() const { return i_stage; }
int CalculatorMessage::category() const { return i_cat; }
...
void Calculator::addDefaultStringAlternative(string replacement,
                                             string standard) {
  default_signs.push_back(replacement);
  default_real_signs.push_back(standard);
}
bool Calculator::delDefaultStringAlternative(string replacement,
                                             string standard) {
...
MathStructure m_undefined, m_empty_vector, m_empty_matrix, m_zero, m_one,
    m_minus_one, m_one_i;
Number nr_zero, nr_one, nr_two, nr_three, nr_minus_one, nr_one_i, nr_minus_i,
    nr_half, nr_minus_half, nr_plus_inf, nr_minus_inf;
...
Calculator::Calculator() {
  ...
  exchange_rates_time[1] = (time_t)472368L * (time_t)3600;
  exchange_rates_time[2] = 0;
  priv->exchange_rates_time2[0] = (time_t)472368L * (time_t)3600;
  exchange_rates_check_time[0] = 0;
  exchange_rates_check_time[1] = (time_t)472368L * (time_t)3600;
  exchange_rates_check_time[2] = 0;
  priv->exchange_rates_check_time2[0] = (time_t)472368L * (time_t)3600;
  ...
  if ((local_digit_group_separator.length() == 1 &&
       (signed char)local_digit_group_separator[0] < 0) ||
      local_digit_group_separator == NBSP)
    local_digit_group_separator = SPACE;
  else if (local_digit_group_separator == NNBSP)
    local_digit_group_separator = THIN_SPACE;
  ...
  ILLEGAL_IN_NAMES =
      "\a\b" + DOT_S +
      RESERVED OPERATORS SEXADOT SPACES PARENTHESISS VECTOR_WRAPS COMMAS;
  ILLEGAL_IN_NAMES_MINUS_SPACE_STR =
      "\a\b" + DOT_S +
      RESERVED OPERATORS SEXADOT PARENTHESISS VECTOR_WRAPS COMMAS;
...
Calculator::Calculator(bool ignore_locale) {
  ...
  if (b_ignore_locale) {
    ...
    if (current_lc_monetary)
      saved_locale = strdup(current_lc_monetary);
    else
      saved_locale = NULL;
    setlocale(LC_ALL, "C");
#ifdef ENABLE_NLS
    ...
    if (n == 0) {
      ...
      DWORD n = 0;
      if (GetUserPreferredUILanguages(MUI_LANGUAGE_NAME, &nlang, NULL, &n)) {
        auto wlocale = new WCHAR[n];
        if (GetUserPreferredUILanguages(MUI_LANGUAGE_NAME, &nlang, wlocale,
                                        &n)) {
          string lang = utf8_encode(wlocale);
          gsub("-", "_", lang);
          if (lang.length() > 5)
  ...
  if ((local_digit_group_separator.length() == 1 &&
       (signed char)local_digit_group_separator[0] < 0) ||
      local_digit_group_separator == " ")
    local_digit_group_separator = " ";
  else if (local_digit_group_separator == " ")
    local_digit_group_separator = THIN_SPACE;
  local_digit_group_format = lc->grouping;
  remove_blank_ends(local_digit_group_format);
  default_dot_as_separator = (local_digit_group_separator == ".");
...
Calculator::~Calculator() {
  ...
  terminateThreads();
  clearRPNStack();
  for (unordered_map<Unit *, MathStructure *>::iterator it =
           priv->composite_unit_base.begin();
       it != priv->composite_unit_base.end(); ++it)
    it->second->unref();
  for (unordered_map<size_t, MathStructure *>::iterator it =
           priv->id_structs.begin();
       it != priv->id_structs.end(); ++it)
    it->second->unref();
#define REMOVE_EXPRESSION_ITEM(o)                                              \
  o->setRegistered(false);                                                     \
  o->destroy();
  for (size_t i = 0; i < functions.size(); i++) {
    REMOVE_EXPRESSION_ITEM(functions[i])
  }
  for (size_t i = 0; i < variables.size(); i++) {
    REMOVE_EXPRESSION_ITEM(variables[i])
  }
  for (size_t i = 0; i < units.size(); i++) {
    REMOVE_EXPRESSION_ITEM(units[i])
  }
  for (size_t i = 0; i < prefixes.size(); i++)
    delete prefixes[i];
  if (v_C) {
    REMOVE_EXPRESSION_ITEM(v_C)
  }
  if (decimal_null_prefix)
    delete decimal_null_prefix;
  if (binary_null_prefix)
    delete binary_null_prefix;
  if (default_assumptions)
    delete default_assumptions;
  if (saved_locale)
    free(saved_locale);
  delete priv;
...
Unit *Calculator::getGraUnit() {
  if (!u_gra)
    u_gra = getUnit("gra");
  if (!u_gra) {
    error(true, _("Gradians unit is missing. Creating one for this session."),
          NULL);
    u_gra = addUnit(new AliasUnit(_("Angle/Plane Angle"), "gra", "gradians",
                                  "gradian", "Gradian", getRadUnit(), "pi/200",
                                  1, "", false, true, true));
  }
  return u_gra;
}
Unit *Calculator::getRadUnit() {
  if (!u_rad)
    u_rad = getUnit("rad");
  if (!u_rad) {
    error(true, _("Radians unit is missing. Creating one for this session."),
          NULL);
    u_rad = addUnit(new Unit(_("Angle/Plane Angle"), "rad", "radians", "radian",
                             "Radian", false, true, true));
  }
  return u_rad;
}
Unit *Calculator::getDegUnit() {
  if (!u_deg)
    u_deg = getUnit("deg");
  if (!u_deg) {
    error(true, _("Degrees unit is missing. Creating one for this session."),
          NULL);
    u_deg = addUnit(new AliasUnit(_("Angle/Plane Angle"), "deg", "degrees",
                                  "degree", "Degree", getRadUnit(), "pi/180", 1,
                                  "", false, true, true));
  }
  return u_deg;
}
    ...
    if (l == 3 && pos[0] == '\xe2') {
      if (pos[1] == '\x80') {
        ...
        if (pos[2] == '\x89')
          return false;
        // quotation marks
        if (((signed char)pos[2] >= -104 && (signed char)pos[2] <= -97) ||
            (signed char)pos[2] == -70 || (signed char)pos[2] == -71)
          return false;
        // operator
        if (pos[2] == '\xa2')
          return false;
        ...
        // exponents
        if ((signed char)pos[2] == -80 ||
            ((signed char)pos[2] >= -76 && (signed char)pos[2] <= -69) ||
            (signed char)pos[2] == -67 || (signed char)pos[2] == -66)
          return false;
      } else if (pos[1] == '\x85') {
        // fractions
        if ((signed char)pos[2] >= -112 && (signed char)pos[2] <= -98)
          return false;
      } else if (pos[1] == '\x88') {
        // operators
        if (pos[2] == '\x95' || pos[2] == '\x99' || pos[2] == '\x92')
          return false;
      } else if (pos[1] == '\x89') {
        // inequalities
        if (pos[2] == '\xa0' || pos[2] == '\xa5' || pos[2] == '\xa4')
          return false;
      } else if (pos[1] == '\x8b') {
        // operator
        if (pos[2] == '\x85')
      ...
      if (pos[1] == '\xbc' && pos[2] == '\x8b')
        return false;
    } else if (l == 2 && pos[0] == '\xc2') {
      // exponents
      if ((signed char)pos[1] == -71 || (signed char)pos[1] == -77 ||
          (signed char)pos[1] == -78)
        return false;
      // fractions
      if ((signed char)pos[1] == -66 || (signed char)pos[1] == -67 ||
          (signed char)pos[1] == -68)
        return false;
      // operators
      if (pos[1] == '\xb1' || pos[1] == '\xb7')
        return false;
    } else if (l == 2 && pos[0] == '\xc3') {
      // operators
      if (pos[1] == '\x97' || pos[1] == '\xb7')
        return false;
    }
    pos += (l - 1);
  }
...
bool Calculator::showArgumentErrors() const { return b_argument_errors; }
...
int Calculator::endTemporaryStopMessages(
    int *message_count, int *warning_count,
    int release_messages_if_no_equal_or_greater_than_message_type) {
  ...
  if (release_messages_if_no_equal_or_greater_than_message_type >=
    ...
    if (release_messages_if_no_equal_or_greater_than_message_type ==
            MESSAGE_WARNING &&
        stopped_warnings_count[disable_errors_ref] > 0)
      release_messages = true;
    else if (release_messages_if_no_equal_or_greater_than_message_type ==
                 MESSAGE_INFORMATION &&
             stopped_messages_count[disable_errors_ref] > 0)
      release_messages = true;
  }
  if (message_count)
    *message_count = stopped_messages_count[disable_errors_ref];
  if (warning_count)
    *warning_count = stopped_warnings_count[disable_errors_ref];
  stopped_errors_count.pop_back();
  stopped_warnings_count.pop_back();
  stopped_messages_count.pop_back();
  if (release_messages)
    addMessages(&stopped_messages[disable_errors_ref]);
  stopped_messages.pop_back();
  return ret;
}
void Calculator::endTemporaryStopMessages(
    bool release_messages, vector<CalculatorMessage> *blocked_messages) {
  if (disable_errors_ref <= 0)
    return;
  disable_errors_ref--;
  stopped_errors_count.pop_back();
  ...
  stopped_messages_count.pop_back();
  if (blocked_messages)
    *blocked_messages = stopped_messages[disable_errors_ref];
  if (release_messages)
    addMessages(&stopped_messages[disable_errors_ref]);
  stopped_messages.pop_back();
}
void Calculator::addMessages(vector<CalculatorMessage> *message_vector) {
  for (size_t i3 = 0; i3 < message_vector->size(); i3++) {
    ...
    if (!dup_error) {
      if (disable_errors_ref > 0) {
        for (size_t i2 = 0; !dup_error && i2 < (size_t)disable_errors_ref;
             i2++) {
        ...
        if (!dup_error)
          stopped_messages[disable_errors_ref - 1].push_back(
              (*message_vector)[i3]);
      } else {
...
const PrintOptions &Calculator::messagePrintOptions() const {
  return message_printoptions;
}
void Calculator::setMessagePrintOptions(const PrintOptions &po) {
  message_printoptions = po;
}
...
bool Calculator::stillHasVariable(Variable *v) {
  for (vector<Variable *>::iterator it = deleted_variables.begin();
       it != deleted_variables.end(); ++it) {
    if (*it == v)
      return false;
  }
  return true;
...
bool Calculator::stillHasUnit(Unit *u) {
  for (vector<Unit *>::iterator it = deleted_units.begin();
       it != deleted_units.end(); ++it) {
    if (*it == u)
      return false;
  }
  return true;
...
bool Calculator::stillHasFunction(MathFunction *f) {
  for (vector<MathFunction *>::iterator it = deleted_functions.begin();
...
void Calculator::saveFunctionCalled() { b_save_called = true; }
...
bool equals_ignore_us(const string &str1, const string &str2, int ignore_us) {
  if (ignore_us == 0)
    return str1 == str2;
  if (str1.length() != str2.length() - ignore_us)
    return false;
  size_t iu = 0;
  for (size_t i = 0; i < str1.length(); i++) {
    if (ignore_us > 0 && str2[i + iu] == '_') {
      ignore_us--;
      iu++;
    }
    if (str1[i] != str2[i + iu])
...
bool equalsIgnoreCase(const string &str1, const string &str2, int ignore_us) {
  if (str1.empty() || str2.empty())
    return false;
  for (size_t i1 = 0, i2 = 0; i1 < str1.length() || i2 < str2.length();
       i1++, i2++) {
    if (ignore_us > 0 && str2[i2] == '_') {
      i2++;
      ignore_us--;
    }
...
ExpressionItem *Calculator::getActiveExpressionItem(ExpressionItem *item) {
  ...
  for (size_t i = 1; i <= item->countNames(); i++) {
    ExpressionItem *item2 = getActiveExpressionItem(
        item->getName(i).name, item, !item->getName(i).completion_only);
    if (item2) {
      return item2;
    }
  }
...
bool name_allows_underscore_removal(const string &name) {
  ...
  while (true) {
    if (i == string::npos) {
      break;
    } else if (i == name.length() - 1 || name[i - 1] == '_' ||
               (i == name.length() - 2 &&
                (name[name.length() - 1] < '0' ||
                 name[name.length() - 1] > '9') &&
                ((signed char)name[i - 1] >= 0 ||
                 CALCULATOR->getPrefix(name.substr(0, i))))) {
      i_us = 0;
      break;
    }
    i_us++;
...
ExpressionItem *Calculator::getActiveExpressionItem(string name_,
                                                    ExpressionItem *item,
                                                    bool ignore_us) {
  ExpressionItem *o = getActiveExpressionItem(name_, item);
  if (o || !ignore_us || !name_allows_underscore_removal(name_))
    return o;
  gsub("_", "", name_);
  ...
  if (l > UFV_LENGTHS) {
      ...
      if (ufvl_t[i] == 'f' || ufvl_t[i] == 'v' || ufvl_t[i] == 'u') {
        if (ufvl[i] != item) {
          const ExpressionName &ename =
              ((ExpressionItem *)ufvl[i])->getName(ufvl_i[i]);
          if ((ename.case_sensitive &&
               equals_ignore_us(name_, ename.name, priv->ufvl_us[i])) ||
              (!ename.case_sensitive &&
               equalsIgnoreCase(name_, ename.name, priv->ufvl_us[i]))) {
    ...
    for (size_t i2 = 1; i2 <= 3; i2++) {
      for (size_t i = 0; i < ufv[i2][l].size(); i++) {
        if (ufv[i2][l][i] != item) {
          const ExpressionName &ename =
              ((ExpressionItem *)ufv[i2][l][i])->getName(ufv_i[i2][l][i]);
          if ((ename.case_sensitive &&
               equals_ignore_us(name_, ename.name, priv->ufv_us[i2][l][i])) ||
              (!ename.case_sensitive &&
               equalsIgnoreCase(name_, ename.name, priv->ufv_us[i2][l][i]))) {
...
ExpressionItem *Calculator::getInactiveExpressionItem(string name,
                                                      ExpressionItem *item) {
  if (name.empty())
    return NULL;
  for (size_t index = 0; index < variables.size(); index++) {
    if (variables[index] != item && !variables[index]->isActive() &&
        variables[index]->hasName(name)) {
      return variables[index];
    }
  }
  for (size_t index = 0; index < functions.size(); index++) {
    if (functions[index] != item && !functions[index]->isActive() &&
        functions[index]->hasName(name)) {
...
ExpressionItem *Calculator::getExpressionItem(string name,
                                              ExpressionItem *item) {
  if (name.empty())
    return NULL;
...
Assumptions *Calculator::defaultAssumptions() { return default_assumptions; }
...
DecimalPrefix *Calculator::getNearestDecimalPrefix(int exp10, int exp) const {
  ...
  while ((exp < 0 && i >= 0) ||
         (exp >= 0 && i < (int)decimal_prefixes.size())) {
    if (decimal_prefixes[i]->exponent(exp) == exp10) {
      return decimal_prefixes[i];
    } else if (decimal_prefixes[i]->exponent(exp) > exp10) {
      if (i == 0) {
        return decimal_prefixes[i];
      } else if (exp10 - decimal_prefixes[i - 1]->exponent(exp) <
                 decimal_prefixes[i]->exponent(exp) - exp10) {
...
DecimalPrefix *Calculator::getOptimalDecimalPrefix(int exp10, int exp,
                                                   bool all_prefixes) const {
  if (decimal_prefixes.size() <= 0 || exp10 == 0)
    return NULL;
  int i = 0;
  if (exp < 0) {
    i = decimal_prefixes.size() - 1;
  ...
  while ((exp < 0 && i >= 0) ||
    ...
    if (all_prefixes || (p->exponent() % 3 == 0 && p->exponent() >= -24 &&
                         p->exponent() <= 24)) {
      ...
      if (p->exponent(exp) == exp10) {
        if (p == decimal_null_prefix)
          return NULL;
        return p;
      } else if (p->exponent(exp) > exp10) {
        if ((exp < 0 && (i == (int)decimal_prefixes.size() - 1 ||
                         (!all_prefixes && p->exponent() == 24))) ||
            (exp >= 0 && (i == 0 || (!all_prefixes && p->exponent() == -24)))) {
          if (p == decimal_null_prefix)
            return NULL;
          return p;
        }
...
DecimalPrefix *Calculator::getOptimalDecimalPrefix(const Number &exp10,
                                                   const Number &exp,
                                                   bool all_prefixes) const {
  if (decimal_prefixes.size() <= 0 || exp10.isZero())
    return NULL;
  int i = 0;
  ComparisonResult c;
  if (exp.isNegative()) {
  ...
  while ((exp.isNegative() && i >= 0) ||
         (!exp.isNegative() && i < (int)decimal_prefixes.size())) {
    p = decimal_prefixes[i];
    if (all_prefixes || (p->exponent() % 3 == 0 && p->exponent() >= -24 &&
                         p->exponent() <= 24)) {
      if (p_prev && (p_prev->exponent() >= 0) != (p->exponent() >= 0) &&
      ...
      if (c == COMPARISON_RESULT_EQUAL) {
        ...
        if ((exp.isNegative() && (i == (int)decimal_prefixes.size() - 1 ||
                                  (!all_prefixes && p->exponent() == 24))) ||
            (!exp.isNegative() &&
             (i == 0 || (!all_prefixes && p->exponent() == -24)))) {
          if (p == decimal_null_prefix)
            return NULL;
          return p;
        }
...
void Calculator::setTemperatureCalculationMode(
    TemperatureCalculationMode temperature_calculation_mode) {
...
int Calculator::usesBinaryPrefixes() const { return priv->use_binary_prefixes; }
...
BinaryPrefix *Calculator::getNearestBinaryPrefix(int exp2, int exp) const {
  ...
  while ((exp < 0 && i >= 0) || (exp >= 0 && i < (int)binary_prefixes.size())) {
    if (binary_prefixes[i]->exponent(exp) == exp2) {
      ...
      if (i == 0) {
        return binary_prefixes[i];
      } else if (exp2 - binary_prefixes[i - 1]->exponent(exp) <
                 binary_prefixes[i]->exponent(exp) - exp2) {
...
BinaryPrefix *Calculator::getOptimalBinaryPrefix(int exp2, int exp) const {
  if (binary_prefixes.size() <= 0 || exp2 == 0)
    return NULL;
  int i = -1;
  if (exp < 0) {
    i = binary_prefixes.size() - 1;
  ...
  int exp2_1, exp2_2;
  while ((exp < 0 && i >= -1) ||
         (exp >= 0 && i < (int)binary_prefixes.size())) {
    if (i >= 0)
      p = binary_prefixes[i];
    else
...
BinaryPrefix *Calculator::getOptimalBinaryPrefix(const Number &exp2,
  ...
  if (binary_prefixes.size() <= 0 || exp2.isZero())
    return NULL;
  int i = -1;
  ComparisonResult c;
  if (exp.isNegative()) {
  ...
  while ((exp.isNegative() && i >= -1) ||
         (!exp.isNegative() && i < (int)binary_prefixes.size())) {
    if (i >= 0)
      p = binary_prefixes[i];
    else
      p = binary_null_prefix;
    ...
    if (c == COMPARISON_RESULT_EQUAL) {
      ...
      if ((exp.isNegative() && i == (int)binary_prefixes.size() - 1) ||
          (!exp.isNegative() && i == 0)) {
        if (p == binary_null_prefix)
          return NULL;
        return p;
...
Prefix *Calculator::addPrefix(Prefix *p) {
  if (p->type() == PREFIX_DECIMAL) {
    if (decimal_prefixes.empty() ||
        ((DecimalPrefix *)p)->exponent() >
            decimal_prefixes[decimal_prefixes.size() - 1]->exponent()) {
      decimal_prefixes.push_back((DecimalPrefix *)p);
    } else {
      size_t i = decimal_prefixes.size() - 1;
      while (i > 0 && ((DecimalPrefix *)p)->exponent() <
                          decimal_prefixes[i - 1]->exponent())
        i--;
      decimal_prefixes.insert(decimal_prefixes.begin() + i, (DecimalPrefix *)p);
    }
  } else if (p->type() == PREFIX_BINARY) {
    if (binary_prefixes.empty() ||
        ((BinaryPrefix *)p)->exponent() >
            binary_prefixes[binary_prefixes.size() - 1]->exponent()) {
      binary_prefixes.push_back((BinaryPrefix *)p);
    } else {
      size_t i = binary_prefixes.size() - 1;
      while (i > 0 && ((BinaryPrefix *)p)->exponent() <
                          binary_prefixes[i - 1]->exponent())
        i--;
  ...
  for (size_t i2 = 1; i2 <= p->countNames(); i2++) {
    ...
    if (l2 > UFV_LENGTHS) {
      ...
      for (vector<void *>::iterator it = ufvl.begin();; ++it) {
        l = 0;
        if (it != ufvl.end()) {
          if (ufvl_t[i] == 'v' || ufvl_t[i] == 'f' || ufvl_t[i] == 'u')
            l = ((ExpressionItem *)(*it))->getName(ufvl_i[i]).name.length();
          else if (ufvl_t[i] == 'p' || ufvl_t[i] == 'P')
            l = ((Prefix *)(*it))->getName(ufvl_i[i]).name.length();
          l -= priv->ufvl_us[i];
        }
        if (it == ufvl.end()) {
...
void Calculator::setPrecision(int precision) {
  ...
  if (PRECISION_TO_BITS(precision) > (double)MPFR_PREC_MAX - 1000L) {
    if (BITS_TO_PRECISION(MPFR_PREC_MAX) > INT_MAX)
      i_precision = INT_MAX;
    else
      i_precision = (int)BITS_TO_PRECISION(MPFR_PREC_MAX - 1000L);
...
int Calculator::getPrecision() const { return i_precision; }
void Calculator::useIntervalArithmetic(bool use_interval_arithmetic) {
  b_interval = use_interval_arithmetic;
}
bool Calculator::usesIntervalArithmetic() const {
  return i_start_interval > 0 || (b_interval && i_stop_interval <= 0);
}
void Calculator::beginTemporaryStopIntervalArithmetic() { i_stop_interval++; }
void Calculator::endTemporaryStopIntervalArithmetic() { i_stop_interval--; }
void Calculator::beginTemporaryEnableIntervalArithmetic() {
  i_start_interval++;
}
void Calculator::endTemporaryEnableIntervalArithmetic() { i_start_interval--; }

bool Calculator::usesMatlabStyleMatrices() const {
  return priv->matlab_matrices;
}
void Calculator::useMatlabStyleMatrices(bool use_matlab_style_matrices) {
  priv->matlab_matrices = use_matlab_style_matrices;
}

bool Calculator::conciseUncertaintyInputEnabled() const {
  return priv->concise_uncertainty_input;
}
void Calculator::setConciseUncertaintyInputEnabled(
    bool enable_concise_uncertainty_input) {
  priv->concise_uncertainty_input = enable_concise_uncertainty_input;
}

long int Calculator::fixedDenominator() const {
  return priv->fixed_denominator;
}
void Calculator::setFixedDenominator(long int fixed_denominator) {
  if (fixed_denominator > 1)
    priv->fixed_denominator = fixed_denominator;
}

void Calculator::setCustomAngleUnit(Unit *u) {
  if (u)
    u->ref();
  if (priv->custom_angle_unit)
    priv->custom_angle_unit->unref();
  priv->custom_angle_unit = u;
}
Unit *Calculator::customAngleUnit() { return priv->custom_angle_unit; }
...
void Calculator::setCustomInputBase(Number nr) {
  ...
  if (!nr.isReal()) {
    ...
    if (priv->custom_input_base_i < 2)
      priv->custom_input_base_i = 2;
  }
}
void Calculator::setCustomOutputBase(Number nr) {
  priv->custom_output_base = nr;
}
const Number &Calculator::customInputBase() const {
  return priv->custom_input_base;
}
const Number &Calculator::customOutputBase() const {
  return priv->custom_output_base;
}
...
string Calculator::localToString(bool include_spaces) const {
  if (include_spaces)
    return string(SPACE) + string(_("to")) + SPACE;
  else
    return _("to");
}
...
bool Calculator::getIgnoreLocale() { return b_ignore_locale; }
...
void Calculator::addBuiltinVariables() {
  ...
  v_i = (KnownVariable *)addVariable(new KnownVariable(
      "", "i", mstruct, "Imaginary i (sqrt(-1))", false, true));
  mstruct.number().setPlusInfinity();
  v_pinf = (KnownVariable *)addVariable(new KnownVariable(
      "", "plus_infinity", mstruct, "+Infinity", false, true));
  mstruct.number().setMinusInfinity();
  v_minf = (KnownVariable *)addVariable(new KnownVariable(
      "", "minus_infinity", mstruct, "-Infinity", false, true));
  mstruct.setUndefined();
  v_undef = (KnownVariable *)addVariable(
      new KnownVariable("", "undefined", mstruct, "Undefined", false, true));
  ...
  v_percent = (KnownVariable *)addVariable(new KnownVariable(
      "", "percent", MathStructure(1, 1, -2), "Percent", false, true));
  v_percent->addName("%");
  v_permille = (KnownVariable *)addVariable(new KnownVariable(
      "", "permille", MathStructure(1, 1, -3), "Per Mille", false, true));
  v_permyriad = (KnownVariable *)addVariable(new KnownVariable(
      "", "permyriad", MathStructure(1, 1, -4), "Per Myriad", false, true));
  v_x = (UnknownVariable *)addVariable(
      new UnknownVariable("", "x", "", false, false));
  v_y = (UnknownVariable *)addVariable(
      new UnknownVariable("", "y", "", false, false));
  v_z = (UnknownVariable *)addVariable(
      new UnknownVariable("", "z", "", false, false));
  v_C = new UnknownVariable("", "C", "", false, true);
  v_C->setAssumptions(new Assumptions());
  v_n = (UnknownVariable *)addVariable(
      new UnknownVariable("", "n", "", false, true));
...
void Calculator::addBuiltinUnits() {
  ...
  u_btc = addUnit(new AliasUnit(_("Currency"), "BTC", "bitcoins", "bitcoin",
                                "Bitcoins", u_euro, "34189.1", 1, "", false,
                                true, true));
  ...
  priv->u_byn =
      addUnit(new AliasUnit(_("Currency"), "BYN", "", "", "Belarusian Ruble",
                            u_euro, "1/3.60681", 1, "", false, true, true));
  priv->u_byn->setHidden(true);
  priv->u_byn->setApproximate();
  priv->u_byn->setPrecision(-2);
  priv->u_byn->setChanged(false);
  Unit *u = addUnit(new AliasUnit(_("Currency"), "BYR", "", "",
                                  "Belarusian Ruble p. (obsolete)", priv->u_byn,
                                  "0.0001", 1, "", false, true, true));
  u->setHidden(true);
  u->setChanged(false);
  priv->u_kelvin = NULL;
  priv->u_rankine = NULL;
...
bool Calculator::variableUnitsEnabled() const { return b_var_units; }

void Calculator::error(bool critical, int message_category,
                       const char *TEMPLATE, ...) {
  va_list ap;
  va_start(ap, TEMPLATE);
  message(critical ? MESSAGE_ERROR : MESSAGE_WARNING, message_category,
          TEMPLATE, ap);
  va_end(ap);
}
void Calculator::error(bool critical, const char *TEMPLATE, ...) {
  va_list ap;
  va_start(ap, TEMPLATE);
  message(critical ? MESSAGE_ERROR : MESSAGE_WARNING, MESSAGE_CATEGORY_NONE,
          TEMPLATE, ap);
  va_end(ap);
}
void Calculator::message(MessageType mtype, int message_category,
                         const char *TEMPLATE, ...) {
  va_list ap;
  va_start(ap, TEMPLATE);
...
void Calculator::message(MessageType mtype, int message_category,
  ...
  while (true) {
    i = error_str.find("%", i);
    if (i == string::npos || i + 1 == error_str.length())
      break;
  ...
  if (!dup_error) {
    if (disable_errors_ref > 0)
      stopped_messages[disable_errors_ref - 1].push_back(
          CalculatorMessage(error_str, mtype, message_category, current_stage));
    else
      messages.push_back(
          CalculatorMessage(error_str, mtype, message_category, current_stage));
  }
...
void Calculator::clearMessages() { messages.clear(); }
void Calculator::cleanMessages(const MathStructure &mstruct,
                               size_t first_message) {
  if (first_message > 0)
    first_message--;
  if (messages.size() <= first_message)
    return;
...
Unit *Calculator::getUnitById(int id) const {
  ...
  case UNIT_ID_KELVIN: {
    return priv->u_kelvin;
  }
  case UNIT_ID_RANKINE: {
    return priv->u_rankine;
  }
  case UNIT_ID_CELSIUS: {
    return priv->u_celsius;
  }
  case UNIT_ID_FAHRENHEIT: {
    return priv->u_fahrenheit;
  }
  case UNIT_ID_BYN: {
    return priv->u_byn;
  }
...
Unit *Calculator::getActiveUnit(string name_) {
  ...
  if (l > UFV_LENGTHS) {
    ...
    for (size_t i = 0; i < ufv[2][l].size(); i++) {
      const ExpressionName &ename =
          ((ExpressionItem *)ufv[2][l][i])->getName(ufv_i[2][l][i]);
      if ((ename.case_sensitive &&
           equals_ignore_us(name_, ename.name, priv->ufv_us[2][l][i])) ||
          (!ename.case_sensitive &&
           equalsIgnoreCase(name_, ename.name, priv->ufv_us[2][l][i]))) {
...
Unit *Calculator::getLocalCurrency() {
  if (priv->local_currency)
    return priv->local_currency;
  struct lconv *lc = localeconv();
  if (lc) {
    ...
    if (!local_currency.empty()) {
      if (local_currency.length() > 3)
        local_currency = local_currency.substr(0, 3);
      return getActiveUnit(local_currency);
    }
  }
  return NULL;
}
void Calculator::setLocalCurrency(Unit *u) { priv->local_currency = u; }
Unit *Calculator::getCompositeUnit(string internal_name_) {
  if (internal_name_.empty())
    return NULL;
  for (size_t i = 0; i < units.size(); i++) {
    if (units[i]->subtype() == SUBTYPE_COMPOSITE_UNIT &&
        units[i]->hasName(internal_name_)) {
      return units[i];
    }
  }
  return NULL;
...
Variable *Calculator::addVariable(Variable *v, bool force, bool check_names) {
  ...
  if (!v->isLocal() && variables.size() > 0 &&
      variables[variables.size() - 1]->isLocal()) {
...
void Calculator::expressionItemActivated(ExpressionItem *item) {
  if (item->type() == TYPE_FUNCTION) {
    for (size_t i = 1; i <= item->countNames(); i++) {
      ExpressionItem *item2 = getActiveFunction(
          item->getName(i).name, !item->getName(i).completion_only);
      if (item2)
        item2->setActive(false);
    }
  } else {
    for (size_t i = 1; i <= item->countNames(); i++) {
      ExpressionItem *item2 = getActiveVariable(
          item->getName(i).name, !item->getName(i).completion_only);
      if (item2)
        item2->setActive(false);
      item2 = getActiveUnit(item->getName(i).name,
                            !item->getName(i).completion_only);
      if (item2)
        item2->setActive(false);
    }
  }
...
void Calculator::expressionItemDeleted(ExpressionItem *item) {
  switch (item->type()) {
  case TYPE_VARIABLE: {
    for (vector<Variable *>::iterator it = variables.begin();
         it != variables.end(); ++it) {
      if (*it == item) {
        variables.erase(it);
  ...
  case TYPE_FUNCTION: {
    for (vector<MathFunction *>::iterator it = functions.begin();
         it != functions.end(); ++it) {
      if (*it == item) {
        functions.erase(it);
    ...
    if (item->subtype() == SUBTYPE_DATA_SET) {
      for (vector<DataSet *>::iterator it = data_sets.begin();
           it != data_sets.end(); ++it) {
        if (*it == item) {
          data_sets.erase(it);
          break;
  ...
  for (size_t i2 = 1; i2 <= item->countNames(); i2++) {
    if (item->type() == TYPE_VARIABLE || item->type() == TYPE_UNIT) {
      for (size_t i = 0; i < variables.size(); i++) {
        if (!variables[i]->isLocal() && !variables[i]->isActive() &&
            variables[i]->hasName(item->getName(i2).name,
                                  item->getName(i2).case_sensitive)) {
          bool b = true;
          for (size_t i3 = 1; i3 <= variables[i]->countNames(); i3++) {
            if (getActiveVariable(variables[i]->getName(i3).name,
                                  !variables[i]->getName(i3).completion_only) ||
                getActiveUnit(variables[i]->getName(i3).name,
                              !variables[i]->getName(i3).completion_only)) {
              b = false;
              break;
            }
          ...
          if (b)
            variables[i]->setActive(true);
        }
      }
      for (size_t i = 0; i < units.size(); i++) {
        if (!units[i]->isLocal() && !units[i]->isActive() &&
            units[i]->hasName(item->getName(i2).name,
                              item->getName(i2).case_sensitive)) {
          bool b = true;
          for (size_t i3 = 1; i3 <= units[i]->countNames(); i3++) {
            if (getActiveVariable(units[i]->getName(i3).name,
                                  !units[i]->getName(i3).completion_only) ||
                getActiveUnit(units[i]->getName(i3).name,
                              !units[i]->getName(i3).completion_only)) {
              b = false;
              break;
            }
      ...
      for (size_t i = 0; i < functions.size(); i++) {
        if (!functions[i]->isLocal() && !functions[i]->isActive() &&
            functions[i]->hasName(item->getName(i2).name,
                                  item->getName(i2).case_sensitive)) {
          bool b = true;
          for (size_t i3 = 1; i3 <= functions[i]->countNames(); i3++) {
            if (getActiveFunction(functions[i]->getName(i3).name,
                                  !functions[i]->getName(i3).completion_only)) {
              b = false;
              break;
            }
          }
...
void Calculator::nameChanged(ExpressionItem *item, bool new_item) {
  if (!item->isActive() || item->countNames() == 0)
    return;
  if (item->type() == TYPE_UNIT &&
      ((Unit *)item)->subtype() == SUBTYPE_COMPOSITE_UNIT) {
    return;
  }
  size_t l2;
  if (!new_item)
  ...
  for (size_t i2 = 1; i2 <= item->countNames(); i2++) {
    ...
    while (true) {
      if (l2 > UFV_LENGTHS) {
        ...
        for (vector<void *>::iterator it = ufvl.begin();; ++it) {
          if (it != ufvl.end()) {
            if (ufvl_t[i] == 'f' || ufvl_t[i] == 'u' || ufvl_t[i] == 'v')
              l = ((ExpressionItem *)(*it))->getName(ufvl_i[i]).name.length();
            else if (ufvl_t[i] == 'p' || ufvl_t[i] == 'P')
              l = ((Prefix *)(*it))->getName(ufvl_i[i]).name.length();
            l -= priv->ufvl_us[i];
          ...
          if (it == ufvl.end()) {
            ...
            if (l < l2 || (i_us == 0 && priv->ufvl_us[i] > 0) ||
                ((i_us == 0 || priv->ufvl_us[i] > 0) &&
                 ((item->type() == TYPE_VARIABLE && l == l2 &&
                   ufvl_t[i] == 'v') ||
                  (item->type() == TYPE_FUNCTION && l == l2 &&
                   (ufvl_t[i] != 'p' && ufvl_t[i] != 'P')) ||
                  (item->type() == TYPE_UNIT && l == l2 &&
                   (ufvl_t[i] != 'p' && ufvl_t[i] != 'P' &&
                    ufvl_t[i] != 'f'))))) {
              ufvl.insert(it, (void *)item);
              switch (item->type()) {
              case TYPE_VARIABLE: {
                ufvl_t.insert(ufvl_t.begin() + i, 'v');
                break;
              }
              case TYPE_FUNCTION: {
                ufvl_t.insert(ufvl_t.begin() + i, 'f');
                break;
              }
              case TYPE_UNIT: {
                ufvl_t.insert(ufvl_t.begin() + i, 'u');
                break;
              }
              }
        ...
        if (i_us == 0) {
          size_t i = 0;
          for (vector<size_t>::reverse_iterator it =
                   priv->ufv_us[itype][l2].rbegin();
               ; ++it) {
            i++;
            if (it == priv->ufv_us[itype][l2].rend()) {
              ...
              priv->ufv_us[itype][l2].insert(priv->ufv_us[itype][l2].begin(),
                                             0);
              break;
            }
            if ((*it) == 0) {
              if (it == priv->ufv_us[itype][l2].rbegin()) {
                ...
                priv->ufv_us[itype][l2].insert(
                    priv->ufv_us[itype][l2].end() - i, 0);
              }
              break;
            }
          }
...
Variable *Calculator::getVariableById(int id) const {
  ...
  case VARIABLE_ID_EULER: {
    return v_euler;
  }
  case VARIABLE_ID_CATALAN: {
    return v_catalan;
  }
  case VARIABLE_ID_I: {
    return v_i;
  }
  case VARIABLE_ID_PLUS_INFINITY: {
    return v_pinf;
  }
  case VARIABLE_ID_MINUS_INFINITY: {
    return v_minf;
  }
  case VARIABLE_ID_X: {
    return v_x;
  ...
  case VARIABLE_ID_PERCENT: {
    return v_percent;
  }
  case VARIABLE_ID_PERMILLE: {
    return v_permille;
  }
  case VARIABLE_ID_PERMYRIAD: {
    return v_permyriad;
  }
  case VARIABLE_ID_UNDEFINED: {
    return v_undef;
  }
  }
  unordered_map<int, Variable *>::iterator it = priv->id_variables.find(id);
  if (it == priv->id_variables.end())
    return NULL;
  return it->second;
}
...
Variable *Calculator::getActiveVariable(string name_) {
  ...
  if (l > UFV_LENGTHS) {
    ...
    for (size_t i = 0; i < ufv[3][l].size(); i++) {
      const ExpressionName &ename =
          ((ExpressionItem *)ufv[3][l][i])->getName(ufv_i[3][l][i]);
      if ((ename.case_sensitive &&
           equals_ignore_us(name_, ename.name, priv->ufv_us[3][l][i])) ||
          (!ename.case_sensitive &&
           equalsIgnoreCase(name_, ename.name, priv->ufv_us[3][l][i]))) {
...
ExpressionItem *Calculator::addExpressionItem(ExpressionItem *item,
                                              bool force) {
  switch (item->type()) {
  case TYPE_VARIABLE: {
  ...
  case TYPE_FUNCTION: {
    if (item->subtype() == SUBTYPE_DATA_SET)
      return addDataSet((DataSet *)item, force);
    else
...
MathFunction *Calculator::addFunction(MathFunction *f, bool force,
  ...
  if (!f->isLocal() && functions.size() > 0 &&
      functions[functions.size() - 1]->isLocal()) {
...
MathFunction *Calculator::getFunctionById(int id) const {
  switch (id) {
  case FUNCTION_ID_SIN: {
    return f_sin;
  }
  case FUNCTION_ID_COS: {
    return f_cos;
  }
  case FUNCTION_ID_TAN: {
    return f_tan;
  }
  case FUNCTION_ID_ASIN: {
    return f_asin;
  }
  case FUNCTION_ID_ACOS: {
    return f_acos;
  }
  case FUNCTION_ID_ATAN: {
    return f_atan;
  }
  case FUNCTION_ID_SINH: {
    return f_sinh;
  }
  case FUNCTION_ID_COSH: {
    return f_cosh;
  }
  case FUNCTION_ID_TANH: {
    return f_tanh;
  }
  case FUNCTION_ID_ASINH: {
    return f_asinh;
  }
  case FUNCTION_ID_ACOSH: {
    return f_acosh;
  }
  case FUNCTION_ID_ATANH: {
    return f_atanh;
  }
  case FUNCTION_ID_SINC: {
    return f_sinc;
  }
  case FUNCTION_ID_LOG: {
    return f_ln;
  }
  case FUNCTION_ID_LOGN: {
    return f_logn;
  }
  case FUNCTION_ID_SQRT: {
    return f_sqrt;
  }
  case FUNCTION_ID_CBRT: {
    return f_cbrt;
  }
  case FUNCTION_ID_ROOT: {
    return f_root;
  }
  case FUNCTION_ID_CIS: {
    return priv->f_cis;
  }
  case FUNCTION_ID_ABS: {
    return f_abs;
  }
  case FUNCTION_ID_SIGNUM: {
    return f_signum;
  }
  case FUNCTION_ID_POLYLOG: {
    return f_Li;
  }
  case FUNCTION_ID_GAMMA: {
    return f_gamma;
  }
  case FUNCTION_ID_ERF: {
    return f_erf;
  }
  case FUNCTION_ID_ERFC: {
    return f_erfc;
  }
  case FUNCTION_ID_ERFI: {
    return priv->f_erfi;
  }
  case FUNCTION_ID_STRIP_UNITS: {
    return f_stripunits;
  }
  case FUNCTION_ID_UNCERTAINTY: {
    return f_uncertainty;
  }
  case FUNCTION_ID_INTERVAL: {
    return f_interval;
  }
  case FUNCTION_ID_FACTORIAL: {
    return f_factorial;
  }
  case FUNCTION_ID_DOUBLE_FACTORIAL: {
    return f_factorial2;
  }
  case FUNCTION_ID_I_GAMMA: {
    return f_igamma;
  }
  case FUNCTION_ID_SININT: {
    return f_Si;
  }
  case FUNCTION_ID_COSINT: {
    return f_Ci;
  }
  case FUNCTION_ID_SINHINT: {
    return f_Shi;
  }
  case FUNCTION_ID_COSHINT: {
    return f_Chi;
  }
  case FUNCTION_ID_LOGINT: {
    return f_li;
  }
  case FUNCTION_ID_EXPINT: {
    return f_Ei;
  }
  case FUNCTION_ID_FRESNEL_S: {
    return priv->f_fresnels;
  }
  case FUNCTION_ID_FRESNEL_C: {
    return priv->f_fresnelc;
  }
  case FUNCTION_ID_RE: {
    return f_re;
  }
  ...
  case FUNCTION_ID_ARG: {
    return f_arg;
  }
  case FUNCTION_ID_INTEGRATE: {
    return f_integrate;
  }
  case FUNCTION_ID_DIFFERENTIATE: {
    return f_diff;
  }
  case FUNCTION_ID_BETA: {
    return f_beta;
  }
  case FUNCTION_ID_BESSELY: {
    return f_bessely;
  }
  case FUNCTION_ID_BESSELJ: {
    return f_besselj;
  }
  case FUNCTION_ID_HEAVISIDE: {
    return f_heaviside;
  }
  case FUNCTION_ID_DIRAC: {
    return f_dirac;
  }
  case FUNCTION_ID_DIGAMMA: {
    return f_digamma;
  }
  case FUNCTION_ID_AIRY: {
    return f_airy;
  }
  case FUNCTION_ID_ZETA: {
    return f_zeta;
  }
  case FUNCTION_ID_LAMBERT_W: {
    return f_lambert_w;
  }
  case FUNCTION_ID_IF: {
    return f_if;
  }
  case FUNCTION_ID_SHIFT: {
    return f_shift;
  }
  case FUNCTION_ID_XOR: {
    return f_xor;
  }
  case FUNCTION_ID_WARNING: {
    return f_warning;
  }
  case FUNCTION_ID_MESSAGE: {
    return f_message;
  }
  case FUNCTION_ID_PLOT: {
    return f_plot;
  }
  case FUNCTION_ID_SAVE: {
    return f_save;
  }
  case FUNCTION_ID_CONCATENATE: {
    return f_concatenate;
  }
  case FUNCTION_ID_SECANT_METHOD: {
    return priv->f_secant;
  }
  case FUNCTION_ID_NEWTON_RAPHSON: {
    return priv->f_newton;
  }
  case FUNCTION_ID_RAND: {
    return f_rand;
  }
  }
  unordered_map<int, MathFunction *>::iterator it = priv->id_functions.find(id);
  if (it == priv->id_functions.end())
    return NULL;
  return it->second;
}
...
MathFunction *Calculator::getActiveFunction(string name_) {
  ...
  if (l > UFV_LENGTHS) {
    ...
    for (size_t i = 0; i < ufv[1][l].size(); i++) {
      const ExpressionName &ename =
          ((ExpressionItem *)ufv[1][l][i])->getName(ufv_i[1][l][i]);
      if ((ename.case_sensitive &&
           equals_ignore_us(name_, ename.name, priv->ufv_us[1][l][i])) ||
          (!ename.case_sensitive &&
           equalsIgnoreCase(name_, ename.name, priv->ufv_us[1][l][i]))) {
...
bool Calculator::variableNameIsValid(const string &name_) {
  return !name_.empty() &&
         name_.find_first_of(ILLEGAL_IN_NAMES) == string::npos &&
         is_not_in(NUMBERS, name_[0]);
}
...
bool Calculator::unitNameIsValid(const string &name_) {
  return !name_.empty() &&
         name_.find_first_of(ILLEGAL_IN_UNITNAMES) == string::npos;
}
bool Calculator::variableNameIsValid(const char *name_) {
  if (strlen(name_) == 0)
    return false;
  if (is_in(NUMBERS, name_[0]))
    return false;
...
#define VERSION_BEFORE(i1, i2, i3)                                             \
  (version_numbers[0] < i1 ||                                                  \
   (version_numbers[0] == i1 &&                                                \
    (version_numbers[1] < i2 ||                                                \
     (version_numbers[1] == i2 && version_numbers[2] < i3))))
bool Calculator::variableNameIsValid(const string &name_,
...
bool Calculator::unitNameIsValid(const string &name_, int version_numbers[3],
                                 bool is_user_defs) {
  return unitNameIsValid(name_.c_str(), version_numbers, is_user_defs);
}
bool Calculator::variableNameIsValid(const char *name_, int version_numbers[3],
                                     bool is_user_defs) {
  if (strlen(name_) == 0)
    return false;
  if (is_in(NUMBERS, name_[0]))
  ...
  for (size_t i = 0; name_[i] != '\0'; i++) {
    if (is_in(ILLEGAL_IN_NAMES, name_[i])) {
      if (is_user_defs && VERSION_BEFORE(0, 8, 1) &&
          name_[i] == BITWISE_NOT_CH) {
        b = true;
      } else {
        return false;
      }
  ...
  if (b) {
    error(true,
          _("\"%s\" is not allowed in names anymore. Please change the name of "
            "\"%s\", or the variable will be lost."),
          BITWISE_NOT, name_, NULL);
  }
  return true;
}
bool Calculator::functionNameIsValid(const char *name_, int version_numbers[3],
                                     bool is_user_defs) {
  if (strlen(name_) == 0)
    return false;
  if (is_in(NUMBERS, name_[0]))
  ...
  if (b) {
    error(true,
          _("\"%s\" is not allowed in names anymore. Please change the name "
            "\"%s\", or the function will be lost."),
          BITWISE_NOT, name_, NULL);
  }
  return true;
}
bool Calculator::unitNameIsValid(const char *name_, int version_numbers[3],
                                 bool is_user_defs) {
  if (strlen(name_) == 0)
    return false;
  bool b = false;
...
bool Calculator::nameTaken(string name, ExpressionItem *object) {
  ...
  if (object) {
    switch (object->type()) {
    case TYPE_VARIABLE: {
      return variableNameTaken(name, (Variable *)object);
    }
    case TYPE_UNIT: {
      return unitNameTaken(name, (Unit *)object);
    }
    case TYPE_FUNCTION: {
      return functionNameTaken(name, (MathFunction *)object);
    }
    }
  } else {
...
string Calculator::getName(string name, ExpressionItem *object, bool force,
                           bool always_append) {
  ...
  if (i2 > 1 && !always_append) {
    error(false, _("Name \"%s\" is in use. Replacing with \"%s\"."),
          name.c_str(), stmp.c_str(), NULL);
  }
  return stmp;
}
