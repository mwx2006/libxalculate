...
bool factorize_fix_root_power(MathStructure &m) {
  if (!m[0].isFunction() || m[0].function()->id() != FUNCTION_ID_ROOT ||
      !VALID_ROOT(m[0]))
    return false;
  if (m[1].isNumber() && m[1].number().isInteger() &&
      !m[1].number().isMinusOne()) {
    if (m[1] == m[0][1]) {
      // root(x, a)^a=x
      m.setToChild(1, true);
      m.setToChild(1, true);
...
bool sqrfree_differentiate(const MathStructure &mpoly,
                           const MathStructure &x_var, MathStructure &mdiff,
  ...
  case STRUCT_ADDITION: {
    ...
    for (size_t i = 0; i < mpoly.size(); i++) {
      ...
      if (!sqrfree_differentiate(mpoly[i], x_var, mdiff[i], eo))
        return false;
    }
...
bool fix_root_pow(MathStructure &m, const MathStructure &xvar,
                  const EvaluationOptions &eo) {
...
bool sqrfree_yun(const MathStructure &a, const MathStructure &xvar,
                 MathStructure &factors, const EvaluationOptions &eo) {
  MathStructure w(a);
  MathStructure z;
...
bool sqrfree_simple(const MathStructure &a, const MathStructure &xvar,
                    MathStructure &factors, const EvaluationOptions &eo) {
  MathStructure w(a);
  while (true) {
    MathStructure z, zmod;
    if (!sqrfree_differentiate(w, xvar, z, eo))
      return false;
    ...
    MathStructure mgcd;
    if (!MathStructure::gcd(w, z, mgcd, eo))
      return false;
    ...
    MathStructure tmp(w);
    if (!MathStructure::polynomialQuotient(tmp, mgcd, xvar, w, eo))
      return false;
    if (!sqrfree_simple(mgcd, xvar, factors, eo))
      return false;
  }
  return true;
}
...
void multiply_lcm(const MathStructure &e, const Number &lcm,
                  MathStructure &mmul, const EvaluationOptions &eo) {
...
bool sqrfree(MathStructure &mpoly, const vector<MathStructure> &symbols,
             const EvaluationOptions &eo) {
  ...
  if (symbols.size() > 1) {
    for (size_t i = 1; i < symbols.size(); i++) {
      if (mpoly.degree(symbols[symbol_index])
              .isGreaterThan(mpoly.degree(symbols[i])))
        symbol_index = i;
    }
  }
  ...
  if (!sqrfree_yun(tmp, xvar, factors, eo2)) {
    if (var)
      tmp.replace(var, symbols[symbol_index]);
    factors.clearVector();
  ...
  if (var) {
    mpoly.replace(var, symbols[symbol_index]);
    var->destroy();
  }
  ...
  if (newsymbols.size() > 0) {
    for (size_t i = 0; i < factors.size(); i++) {
      if (!sqrfree(factors[i], newsymbols, eo))
        return false;
    }
  }
  ...
  for (size_t i = 0; i < factors.size(); i++) {
    ...
    if (!factors[i].isOne()) {
      if (mpoly.isOne()) {
        mpoly = factors[i];
        if (i != 0)
          mpoly.raise(MathStructure((long int)i + 1, 1L, 0L));
      } else {
  ...
  if (CALCULATOR->aborted())
    return false;
  if (mpoly.isZero()) {
    CALCULATOR->error(true, "mpoly is zero: %s. %s",
                      format_and_print(tmp).c_str(),
  ...
  if (CALCULATOR->aborted())
    return false;
  if (mquo.isZero()) {
    // CALCULATOR->error(true, "quo is zero: %s. %s",
    // format_and_print(tmp).c_str(), _("This is a bug. Please report it."),
    // NULL);
    return false;
  }
  if (newsymbols.size() > 0) {
    if (!sqrfree(mquo, newsymbols, eo))
      return false;
  }
  if (!mquo.isOne()) {
...
bool gather_factors(const MathStructure &mstruct, const MathStructure &x_var,
                    MathStructure &madd, MathStructure &mmul,
                    MathStructure &mexp, bool mexp_as_x2 = false) {
  madd.clear();
  if (mexp_as_x2)
    mexp = m_zero;
  else
  ...
  } else if (!mexp_as_x2 && mstruct.isPower() && mstruct[1].isInteger() &&
             mstruct[0] == x_var) {
    if (mstruct[0] == x_var) {
      mexp = mstruct[1];
      mmul = m_one;
    ...
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (!b_x && mstruct[i] == x_var) {
        ...
        i_x = i;
                 ...
                 mstruct[i][1].isInteger() && mstruct[i][0] == x_var) {
        b_x = true;
        i_x = i;
        mexp = mstruct[i][1];
...
bool factorize_find_multiplier(const MathStructure &mstruct,
                               MathStructure &mnew,
                               MathStructure &factor_mstruct, bool only_units) {
  ...
  case STRUCT_ADDITION: {
    ...
    if (mstruct.size() > 0) {
      ...
      while (true) {
        ...
        if (!cur_mstruct->containsInterval(true) && !cur_mstruct->isNumber() &&
            (!only_units || cur_mstruct->isUnit_exp())) {
          const MathStructure *exp = NULL;
          const MathStructure *bas;
          if (cur_mstruct->isPower() && IS_REAL((*cur_mstruct)[1]) &&
              !(*cur_mstruct)[0].isNumber()) {
          ...
          for (size_t i2 = 1; i2 < mstruct.size(); i2++) {
            ...
            while (true) {
              ...
              if (cmp_mstruct->equals(*bas)) {
                ...
                b = true;
                break;
              } else if (cmp_mstruct->isPower() && IS_REAL((*cmp_mstruct)[1]) &&
                         cmp_mstruct->base()->equals(*bas)) {
                if (exp) {
                  if (cmp_mstruct->exponent()->number().isLessThan(
                          exp->number())) {
            ...
            if (b) {
              ...
              while (true) {
                ...
                if (cmp_mstruct->equals(factor_mstruct.last())) {
                  ...
                  b = true;
                  break;
                } else if (cmp_mstruct->isPower() &&
                           IS_REAL((*cmp_mstruct)[1]) &&
                           cmp_mstruct->base()->equals(factor_mstruct.last())) {
                  if (exp) {
                    if (cmp_mstruct->exponent()->number().isLessThan(
                            exp->number())) {
                      exp = cmp_mstruct->exponent();
      ...
      while (true) {
        ...
        for (size_t i2 = 0; i2 < mnew.size(); i2++) {
          ...
          case STRUCT_MULTIPLICATION: {
            bool b = true;
            if (mfactor->isNumber() &&
                (mnew[i2].size() < 1 || !mnew[i2][0].isNumber())) {
              mnew[i2].insertChild(MathStructure(1, 1, 0), 1);
            }
                ...
                if (!IS_REAL(mnew[i2][i3][1])) {
                  if (mfactor->equals(mnew[i2][i3])) {
                    ...
                    b = false;
                           ...
                           mfactor->base()->equals(mnew[i2][i3][0])) {
                  if (mfactor->equals(mnew[i2][i3])) {
                    mnew[i2].delChild(i3 + 1);
                  } else {
...
bool polynomial_divide_integers(const vector<Number> &vnum,
                                const vector<Number> &vden,
                                vector<Number> &vquotient) {
  ...
  while (numdeg >= dendeg) {
    ...
    numdeg -= dendeg;
    if (!numcoeff.isIntegerDivisible(dencoeff))
      break;
    numcoeff /= dencoeff;
...
bool combination_factorize_is_complicated(MathStructure &m) {
  if (m.isPower()) {
    return combination_factorize_is_complicated(m[0]) ||
           combination_factorize_is_complicated(m[1]);
  }
  return m.size() > 0;
}
  ...
  case STRUCT_ADDITION: {
    ...
    for (size_t i = 0; i < mstruct_units.size(); i++) {
      if (mstruct_units[i].isMultiplication()) {
        for (size_t i2 = 0; i2 < mstruct_units[i].size();) {
          if (!mstruct_units[i][i2].isPower() ||
              !mstruct_units[i][i2][1].hasNegativeSign()) {
        ...
        for (size_t i2 = 0; i2 < mstruct_new[i].size();) {
          if (mstruct_new[i][i2].isPower() &&
              mstruct_new[i][i2][1].hasNegativeSign()) {
      ...
      } else if (mstruct_new[i].isPower() &&
                 mstruct_new[i][1].hasNegativeSign()) {
        mstruct_new[i].set(1, 1, 0);
      } else {
        mstruct_units[i].setUndefined();
      }
    ...
    if (mstruct.isAddition()) {
      ...
      for (size_t i = 0; i < mstruct_units.size(); i++) {
        if (mstruct_units[i].isMultiplication()) {
          for (size_t i2 = 0; i2 < mstruct_units[i].size();) {
            if (!mstruct_units[i][i2].containsType(STRUCT_UNIT, true) &&
                !mstruct_units[i][i2].containsUnknowns()) {
              mstruct_units[i].delChild(i2 + 1);
            } else {
              i2++;
            }
          ...
          if (mstruct_units[i].size() == 0)
            ...
            mstruct_units[i].setToChild(1);
          for (size_t i2 = 0; i2 < mstruct_new[i].size();) {
            if (mstruct_new[i][i2].containsType(STRUCT_UNIT, true) ||
                mstruct_new[i][i2].containsUnknowns()) {
              mstruct_new[i].delChild(i2 + 1);
            } else {
              i2++;
            }
          ...
          if (mstruct_new[i].size() == 0)
            ...
            mstruct_new[i].setToChild(1);
        } else if (mstruct_units[i].containsType(STRUCT_UNIT, true) ||
                   mstruct_units[i].containsUnknowns()) {
          mstruct_new[i].set(1, 1, 0);
        } else {
          mstruct_units[i].setUndefined();
        }
...
bool MathStructure::factorize(const EvaluationOptions &eo_pre, bool unfactorize,
                              int term_combination_levels, int max_msecs,
                              bool only_integers, int recursive,
                              struct timeval *endtime_p,
                              const MathStructure &force_factorization,
                              bool complete_square, bool only_sqrfree,
  ...
  if (unfactorize) {
    ...
    if (mdiv->size() > 0) {
      bool b_ret = false;
      if (SIZE == 1 && recursive) {
        b_ret = CHILD(0).factorize(eo, false, term_combination_levels, 0,
                                   only_integers, recursive, endtime_p,
                                   force_factorization, complete_square,
                                   only_sqrfree, max_factor_degree);
      } else if (SIZE > 1) {
        b_ret = factorize(eo, false, term_combination_levels, 0, only_integers,
                          recursive, endtime_p, force_factorization,
                          complete_square, only_sqrfree, max_factor_degree);
      }
      if (mdiv->size() > 1) {
        // 5/y + x/y + z = (5 + x)/y + z
      ...
      if (recursive) {
        for (; index < SIZE; index++) {
          b_ret = CHILD(index).factorize(eo, false, term_combination_levels, 0,
                                         only_integers, recursive, endtime_p,
                                         force_factorization, complete_square,
                                         only_sqrfree, max_factor_degree) ||
                  b_ret;
  ...
  if (term_combination_levels >= -1 && isAddition() && isRationalPolynomial()) {
    ...
    if (sqrfree(msqrfree, eo)) {
      if ((!only_sqrfree || msqrfree.isPower()) && !equals(msqrfree) &&
          (!msqrfree.isMultiplication() || msqrfree.size() != 2 ||
           (!(msqrfree[0].isNumber() && msqrfree[1].isAddition()) &&
            !(msqrfree[1].isNumber() && msqrfree[0].isAddition())))) {
        ...
        if (!b_equal) {
          ...
          if (CALCULATOR->aborted())
            return false;
          CALCULATOR->error(true, "factorized result is wrong: %s != %s. %s",
                            format_and_print(msqrfree).c_str(),
          ...
          if (!isAddition()) {
              ...
              for (size_t i = 1; i < SIZE; i++) {
                if (CHILD(i).isNumber()) {
                  ...
                  delChild(i);
                           ...
                           CHILD(i)[0].isMultiplication() &&
                           CHILD(i)[0].size() >= 2 &&
                           CHILD(i)[0][0].isNumber() &&
                           CHILD(i)[0][0].number().isRational() &&
                           !CHILD(i)[0][0].number().isInteger() &&
                           CHILD(i)[1].isInteger()) {
            ...
            if (isMultiplication() && SIZE >= 2 && CHILD(0).isNumber() &&
                CHILD(0).number().isRational() &&
                !CHILD(0).number().isInteger()) {
                ...
                if (CHILD(i).isAddition()) {
                  ...
                  for (size_t i2 = 0; i2 < CHILD(i).size(); i2++) {
                    if (CHILD(i)[i2].isNumber()) {
                      if (!CHILD(i)[i2].number().isIntegerDivisible(den)) {
                        b = false;
                        break;
                      }
                    } else if (CHILD(i)[i2].isMultiplication() &&
                               CHILD(i)[i2][0].isNumber()) {
                      if (!CHILD(i)[i2][0].number().isIntegerDivisible(den)) {
                        b = false;
                        break;
                      }
                    } else {
            ...
            if (isMultiplication()) {
              for (size_t i = 0; i < SIZE; i++) {
                if (CHILD(i).isPower() && CHILD(i)[1].isInteger()) {
                  if (CHILD(i)[0].isAddition()) {
                    ...
                    for (size_t i2 = 0; i2 < CHILD(i)[0].size(); i2++) {
                      if ((!CHILD(i)[0][i2].isNumber() ||
                           !CHILD(i)[0][i2].number().isNegative()) &&
                          (!CHILD(i)[0][i2].isMultiplication() ||
                           CHILD(i)[0][i2].size() < 2 ||
                           !CHILD(i)[0][i2][0].isNumber() ||
                           !CHILD(i)[0][i2][0].number().isNegative())) {
                        b = false;
                        break;
                      }
                    }
                  ...
                  } else if (CHILD(i)[0].isMultiplication() &&
                             CHILD(i)[0].size() >= 2 &&
                             CHILD(i)[0][0].isNumber() &&
                             CHILD(i)[0][0].number().isNegative()) {
                  ...
                  for (size_t i2 = 0; i2 < CHILD(i).size(); i2++) {
                    if ((!CHILD(i)[i2].isNumber() ||
                         !CHILD(i)[i2].number().isNegative()) &&
                        (!CHILD(i)[i2].isMultiplication() ||
                         CHILD(i)[i2].size() < 2 ||
                         !CHILD(i)[i2][0].isNumber() ||
                         !CHILD(i)[i2][0].number().isNegative())) {
                      b = false;
                      break;
                    }
                  }
            ...
            if (isPower() && CHILD(1).isInteger()) {
              if (CHILD(0).isAddition()) {
                ...
                for (size_t i2 = 0; i2 < CHILD(0).size(); i2++) {
                  if ((!CHILD(0)[i2].isNumber() ||
                       !CHILD(0)[i2].number().isNegative()) &&
                      (!CHILD(0)[i2].isMultiplication() ||
                       CHILD(0)[i2].size() < 2 || !CHILD(0)[i2][0].isNumber() ||
                       !CHILD(0)[i2][0].number().isNegative())) {
                    b = false;
                    break;
                  }
                }
              ...
              } else if (CHILD(0).isMultiplication() && CHILD(0).size() >= 2 &&
                         CHILD(0)[0].isNumber() &&
                         CHILD(0)[0].number().isNegative()) {
          ...
          evalSort(true);
          factorize(eo, false, term_combination_levels, 0, only_integers,
                    recursive, endtime_p, force_factorization, complete_square,
                    only_sqrfree, max_factor_degree);
          return true;
        }
      }
  ...
  case STRUCT_ADDITION: {
    ...
    if (term_combination_levels >= -1 && !only_sqrfree &&
      ...
      if (SIZE <= 3 && SIZE > 1) {
        ...
        Number nr2(1, 1);
        if (CHILD(0).isPower() && CHILD(0)[0].size() == 0 &&
            CHILD(0)[1].isNumber() && CHILD(0)[1].number().isTwo()) {
          xvar = &CHILD(0)[0];
        } else if (CHILD(0).isMultiplication() && CHILD(0).size() == 2 &&
                   CHILD(0)[0].isNumber()) {
          if (CHILD(0)[1].isPower()) {
            if (CHILD(0)[1][0].size() == 0 && CHILD(0)[1][1].isNumber() &&
                CHILD(0)[1][1].number().isTwo()) {
              xvar = &CHILD(0)[1][0];
          ...
          if (factorable) {
            ...
            if (factorable) {
              if (!only_integers) {
                ...
                mstructb24.number().raise(nr_half);
                if ((!sqrtb24ac.isApproximate() &&
                     mstructb24.number().isApproximate()) ||
                    (sqrtb24ac.isInteger() &&
                     !mstructb24.number().isInteger())) {
                  factorable = false;
                }
              }
            }
      ...
      if (factorize_find_multiplier(*this, mnew, *factor_mstruct) &&
        ...
        mnew.factorize(eo, false, term_combination_levels, 0, only_integers,
                       recursive, endtime_p, force_factorization,
                       complete_square, only_sqrfree, max_factor_degree);
        factor_mstruct->factorize(eo, false, term_combination_levels, 0,
                                  only_integers, recursive, endtime_p,
                                  force_factorization, complete_square,
                                  only_sqrfree, max_factor_degree);
        clear(true);
      ...
      if (SIZE > 1 && CHILD(SIZE - 1).isNumber() &&
          CHILD(SIZE - 1).number().isInteger() && max_factor_degree != 0) {
        MathStructure *xvar = NULL;
        Number qnr(1, 1);
        int degree = 1;
        bool overflow = false;
        int qcof = 1;
        if (CHILD(0).isPower() && !CHILD(0)[0].isNumber() &&
            CHILD(0)[0].size() == 0 && CHILD(0)[1].isNumber() &&
            CHILD(0)[1].number().isInteger() &&
            CHILD(0)[1].number().isPositive()) {
          xvar = &CHILD(0)[0];
          degree = CHILD(0)[1].number().intValue(&overflow);
        } else if (CHILD(0).isMultiplication() && CHILD(0).size() == 2 &&
                   CHILD(0)[0].isNumber() && CHILD(0)[0].number().isInteger()) {
          if (CHILD(0)[1].isPower()) {
            if (CHILD(0)[1][0].size() == 0 && !CHILD(0)[1][0].isNumber() &&
                CHILD(0)[1][1].isNumber() &&
                CHILD(0)[1][1].number().isInteger() &&
                CHILD(0)[1][1].number().isPositive()) {
              xvar = &CHILD(0)[1][0];
        ...
        if (xvar && !overflow && degree <= 1000 && degree > 2 && qcof != 0 &&
            pcof != 0) {
          bool b = true, b2 = true;
          for (size_t i = 1; b && i < SIZE - 1; i++) {
            ...
            case STRUCT_POWER: {
              if (!CHILD(i)[1].isNumber() || !xvar->equals(CHILD(i)[0]) ||
                  !CHILD(i)[1].number().isInteger() ||
                  !CHILD(i)[1].number().isPositive()) {
                b = false;
              }
              break;
            }
            case STRUCT_MULTIPLICATION: {
              if (!(CHILD(i).size() == 2) || !CHILD(i)[0].isNumber()) {
                ...
                if (!CHILD(i)[1][1].isNumber() ||
                    !xvar->equals(CHILD(i)[1][0]) ||
                    !CHILD(i)[1][1].number().isInteger() ||
                    !CHILD(i)[1][1].number().isPositive()) {
                  b = false;
                }
              } else if (!xvar->equals(CHILD(i)[1])) {
                b = false;
              }
              if (b && b2 && !CHILD(i)[0].isInteger())
                b2 = false;
              break;
            }
            default: {
          ...
          if (b) {
            ...
            for (size_t i = 0; b && i < SIZE - 1; i++) {
              ...
              case STRUCT_POWER: {
                ...
                if (curdeg == prevdeg || curdeg > degree ||
                    (prevdeg > 0 && curdeg > prevdeg) || overflow) {
                  b = false;
                } else {
            ...
            if (zeroes.size() > 0) {
              ...
              mleft.factorize(eo, false, 0, 0, only_integers, recursive,
                              endtime_p, force_factorization, complete_square,
                              only_sqrfree, max_factor_degree);
              ...
              if (mleft.isMultiplication()) {
                ...
                } else if (CHILD(0).isMultiplication() && CHILD(0).size() > 0 &&
                           CHILD(0)[0].isNumber()) {
              ...
              for (size_t i = 0; i < SIZE - 1; i++) {
                ...
                while (mcur && i + 1 < SIZE) {
                  if (CHILD(i + 1).isPower()) {
                    if (CHILD(i + 1)[0].isAddition() &&
                        CHILD(i + 1)[1].isNumber() &&
                        mcur->equals(CHILD(i + 1)[0])) {
                      dupspow += CHILD(i + 1)[1].number();
                    } else {
                      mcur = NULL;
                    }
                  } else if (CHILD(i + 1).isAddition() &&
                             mcur->equals(CHILD(i + 1))) {
                    dupspow++;
                  } else {
                    mcur = NULL;
                  }
          ...
          if (b && b2 && (max_factor_degree < 0 || max_factor_degree >= 2) &&
              degree > 3 && degree < 50) {
            // Kronecker method
            vector<Number> vnum;
            ...
            for (size_t i = 0; b && i < SIZE; i++) {
              ...
              case STRUCT_POWER: {
                if (CHILD(i)[0] == *xvar && CHILD(i)[1].isInteger()) {
                  ...
                  if (curdeg < 0 || overflow || curdeg > degree)
                    b = false;
                  else
                    vnum[curdeg] += 1;
                } else {
              ...
              case STRUCT_MULTIPLICATION: {
                if (CHILD(i).size() == 2 && CHILD(i)[0].isInteger()) {
                  ...
                  if (!overflow && CHILD(i)[1].isPower() &&
                      CHILD(i)[1][0] == *xvar && CHILD(i)[1][1].isInteger()) {
            ...
            if (b && lcoeff != 0) {
              degree /= 2;
              if (max_factor_degree > 0 && degree > max_factor_degree)
                degree = max_factor_degree;
              for (int i = 0; i <= degree; i++) {
                if (CALCULATOR->aborted())
                  return false;
                MathStructure mcalc(*this);
                mcalc.calculateReplace(
                    *xvar, Number((i / 2 + i % 2) * (i % 2 == 0 ? -1 : 1), 1),
                    eo);
            ...
            if (b) {
              ...
              for (int i = 2; i < vs[0] / 3 && i < 1000; i++) {
                if (vs[0] % i == 0)
                  factors0.push_back(i);
              }
              if (vs[0] % 3 == 0)
                factors0.push_back(vs[0] / 3);
              if (vs[0] % 2 == 0)
                factors0.push_back(vs[0] / 2);
              factors0.push_back(vs[0]);
              for (int i = 2; i < lcoeff / 3 && i < 1000; i++) {
                if (lcoeff % i == 0)
                  factorsl.push_back(i);
              }
              factorsl.push_back(1);
              if (lcoeff % 3 == 0)
                factorsl.push_back(lcoeff / 3);
              if (lcoeff % 2 == 0)
                factorsl.push_back(lcoeff / 2);
              factorsl.push_back(lcoeff);

              long long int cmax =
                  500000LL / (factors0.size() * factorsl.size());
              if (term_combination_levels != 0)
                cmax *= 10;
              if (degree >= 2 && cmax > 10) {
                vector<Number> vden;
                vector<Number> vquo;
                ...
                for (size_t i = 0; i < factors0.size() * 2; i++) {
                  ...
                  for (size_t i2 = 0; i2 < factorsl.size(); i2++) {
                    ...
                    if (c1min < -cmax / 2)
                      c1min = -cmax / 2;
                    for (long int c1 = c1min; c1 <= c1max && c1 <= cmax / 2;
                         c1++) {
                      long int v1 = ::labs(c2 + c1 + c0);
                      long int v2 = ::labs(c2 - c1 + c0);
                      if (v1 != 0 && v2 != 0 && v1 <= vs[1] && v2 <= vs[2] &&
                          (c1 != 0 || c2 != 0) && vs[1] % v1 == 0 &&
                          vs[2] % v2 == 0) {
                        vden[0] = c0;
                        vden[1] = c1;
                        ...
                        if (polynomial_divide_integers(vnum, vden, vquo)) {
                          ...
                          if (c2 != 0) {
                            ...
                            if (c2 == 1) {
                              ...
                              mterm->addChild_nocopy(
                                  new MathStructure(c2, 1L, 0L));
                          ...
                          MathStructure mquo;
                          if (mtest.size() > 1 &&
                              polynomialDivide(mthis, mtest, mquo, eo, false)) {
                            mquo.factorize(eo, false, 0, 0, only_integers,
                                           recursive, endtime_p,
                                           force_factorization, complete_square,
                                           only_sqrfree, max_factor_degree);
                            set(mquo, true);
                ...
                for (size_t i = 0; i < factors0.size() * 2; i++) {
                  ...
                  for (size_t i2 = 0; i2 < factorsl.size(); i2++) {
                    ...
                    for (long int c2p = 0; c2p <= c2totalmax; c2p++) {
                      ...
                      long int c1max = t1max / 2 + t1max % 2,
                               c1min = -t1max / 2 - t1max % 2;
                          ...
                          if (polynomial_divide_integers(vnum, vden, vquo)) {
                            ...
                            for (int i2 = i_d; i2 >= 2; i2--) {
                              if (vc[i2] != 0) {
                                ...
                                mpow->setType(STRUCT_POWER);
                                mpow->addChild(*xvar);
                                mpow->addChild_nocopy(
                                    new MathStructure(i2, 1, 0));
                                if (vc[i2] == 1) {
                                  mtest.addChild_nocopy(mpow);
                                } else {
                                  MathStructure *mterm = new MathStructure();
                            ...
                            if (vc[1] == 1) {
                              ...
                              mterm->setType(STRUCT_MULTIPLICATION);
                              mterm->addChild_nocopy(
                                  new MathStructure(vc[1], 1L, 0L));
                              mterm->addChild(*xvar);
                              mtest.addChild_nocopy(mterm);
                            }
                            mtest.addChild_nocopy(
                                new MathStructure(vc[0], 1L, 0L));
                            MathStructure mthis(*this);
                            MathStructure mquo;
                            if (mtest.size() > 1 &&
                                polynomialDivide(mthis, mtest, mquo, eo,
                                                 false)) {
                              mquo.factorize(eo, false, 0, 0, only_integers,
                                             recursive, endtime_p,
                                             force_factorization,
                                             complete_square, only_sqrfree,
                                             max_factor_degree);
                              free(vc);
                              set(mquo, true);
                              multiply(mtest, true);
        ...
        for (size_t i = 0; i < SIZE && b; i++) {
          ...
          if (CHILD(i).isInteger() && CHILD(i).number().integerLength() < 100) {
            ...
            if (CHILD(i)[0].isInteger() &&
                CHILD(i).number().integerLength() < 100) {
              if (i == 0)
                nr1 = CHILD(i)[0].number();
              else
                nr2 = CHILD(i)[0].number();
              i2++;
            }
            for (; i2 < CHILD(i).size(); i2++) {
              if (!CHILD(i)[i2].isPower() || !CHILD(i)[i2][1].isInteger() ||
                  !CHILD(i)[i2][1].number().isPositive() ||
                  !CHILD(i)[i2][1].number().isEven() ||
                  CHILD(i)[1].number().integerLength() >= 100 ||
                  !CHILD(i)[i2][0].representsNonMatrix()) {
                b = false;
                break;
              }
            }
          } else if (CHILD(i).isPower() && CHILD(i)[1].isNumber() &&
                     CHILD(i)[1].number().isInteger() &&
                     CHILD(i)[1].number().isPositive() &&
                     CHILD(i)[1].number().isEven() &&
                     CHILD(i)[1].number().integerLength() < 100 &&
                     CHILD(i)[0].representsNonMatrix()) {
            b_nonnum = true;
            b = true;
          }
        }
        ...
        if (b) {
          ...
          for (size_t i = 0; i < SIZE; i++) {
            if (CHILD(i).isNumber()) {
              ...
              if (CHILD(i)[0].isOne())
                CHILD(i).delChild(1, true);
              if ((*mmul)[i][0].isOne())
                (*mmul)[i].delChild(1, true);
            } else if (CHILD(i).isPower()) {
          ...
          if (recursive) {
            factorize(eo, false, term_combination_levels, 0, only_integers,
                      recursive, endtime_p, force_factorization,
                      complete_square, only_sqrfree, max_factor_degree);
            mmul->factorize(eo, false, term_combination_levels, 0,
                            only_integers, recursive, endtime_p,
                            force_factorization, complete_square, only_sqrfree,
                            max_factor_degree);
          }
      ...
      if (max_factor_degree != 0 && SIZE == 2 && CHILD(0).isPower() &&
          CHILD(0)[1].isNumber() && CHILD(0)[1].number() == 3 &&
          CHILD(1).isMultiplication() && CHILD(1).size() == 2 &&
          CHILD(1)[0].isMinusOne() && CHILD(1)[1].isPower() &&
          CHILD(1)[1][1].isNumber() && CHILD(1)[1][1].number() == 3) {
        if (CHILD(0)[0].representsNonMatrix() &&
            CHILD(1)[1][0].representsNonMatrix()) {
      ...
      //-x^3+y^3=(-x+y)(x^2+xy+y^2)
      if (max_factor_degree != 0 && SIZE == 2 && CHILD(1).isPower() &&
          CHILD(1)[1].isNumber() && CHILD(1)[1].number() == 3 &&
          CHILD(0).isMultiplication() && CHILD(0).size() == 2 &&
          CHILD(0)[0].isMinusOne() && CHILD(0)[1].isPower() &&
          CHILD(0)[1][1].isNumber() && CHILD(0)[1][1].number() == 3) {
        if (CHILD(1)[0].representsNonMatrix() &&
            CHILD(0)[1][0].representsNonMatrix()) {
          MathStructure *m2 = new MathStructure(*this);
      ...
      if (max_factor_degree != 0 && !only_integers &&
          !force_factorization.isUndefined() && SIZE >= 2) {
        MathStructure mexp, madd, mmul;
        if (gather_factors(*this, force_factorization, madd, mmul, mexp) &&
            !madd.isZero() && !mmul.isZero() && mexp.isInteger() &&
            mexp.number().isGreaterThan(nr_two)) {
          if (!mmul.isOne())
            madd.calculateDivide(mmul, eo);
          bool overflow = false;
          ...
          if (!overflow) {
            if (n % 4 == 0) {
              ...
              for (int i = 1; i < n - 1; i++) {
                ...
                if (i % 2 == 1)
                  mterm->calculateMultiply(m_minus_one, eo);
      ...
      for (size_t i2 = 0; i2 < SIZE; i2++) {
        if ((!CHILD(i2).isNumber() || !CHILD(i2).number().isNegative()) &&
            (!CHILD(i2).isMultiplication() || CHILD(i2).size() < 2 ||
             !CHILD(i2)[0].isNumber() || !CHILD(i2)[0].number().isNegative())) {
          b = false;
          break;
        }
      }
              ...
              for (size_t i3 = i + 1; i3 < SIZE; i3++) {
                if (CHILD(i3).isMultiplication() && CHILD(i3).size() > 1) {
                  for (size_t i4 = 0; i4 < CHILD(i3).size(); i4++) {
                    if (CHILD(i3)[i4].isAddition() &&
                        CHILD(i3)[i4] == CHILD(i)[i2]) {
                      ...
                      delChild(i3 + 1, true);
                      evalSort(true);
                      factorize(eo, false, term_combination_levels, 0,
                                only_integers, recursive, endtime_p,
                                force_factorization, complete_square,
                                only_sqrfree, max_factor_degree);
                      return true;
    ...
    // complete the square
    if (max_factor_degree != 0 &&
        (term_combination_levels != 0 || complete_square)) {
      if (only_integers) {
        if (SIZE <= 3 && SIZE > 1) {
          MathStructure *xvar = NULL;
          Number nr2(1, 1);
        ...
        if (!force_factorization.isUndefined()) {
          ...
          if (CHILD(0).isPower() && CHILD(0)[0].size() == 0 &&
            ...
            for (size_t i2 = 0; i2 < CHILD(0).size(); i2++) {
              if (CHILD(0).isPower() && CHILD(0)[i2][0].size() == 0 &&
                  CHILD(0)[i2][1].isNumber() &&
                  CHILD(0)[i2][1].number().isTwo()) {
                xvar = &CHILD(0)[0];
              }
            }
          }
        }
        if (xvar && gather_factors(*this, *xvar, m0, m1, m2, true) &&
            !m1.isZero() && !m2.isZero()) {
          MathStructure *mx = new MathStructure(*xvar);
          set(m1, true);
          calculateMultiply(nr_half, eo);
          if (!m2.isOne())
            calculateDivide(m2, eo);
          add_nocopy(mx);
          calculateAddLast(eo);
          raise(nr_two);
          if (!m2.isOne())
          ...
          if (!m0.isZero())
            add(m0);
          if (recursive) {
            CHILD(0).factorize(eo, false, 0, 0, only_integers, recursive,
                               endtime_p, force_factorization, complete_square,
                               only_sqrfree, max_factor_degree);
            CHILD(1).factorize(eo, false, 0, 0, only_integers, recursive,
                               endtime_p, force_factorization, complete_square,
                               only_sqrfree, max_factor_degree);
            CHILDREN_UPDATED
          }
    ...
    if (SIZE > 2 && term_combination_levels > 0) {
          ...
          for (size_t i2 = 0; i2 < SIZE; i2++) {
            if (CHILD(i2).isMultiplication()) {
              for (size_t i3 = 0; i3 < CHILD(i2).size(); i3++) {
                if (CHILD(i2)[i3].isPower() && CHILD(i2)[i3][1].isNumber() &&
                    CHILD(i2)[i3][1].number().isRational() &&
                    CHILD(i2)[i3][1].number().isNegative() &&
                    CHILD(i2)[i3][0] == syms[i]) {
                  if (min_pow_i.isZero() ||
                      CHILD(i2)[i3][1].number() > min_pow_i)
                    min_pow_i = CHILD(i2)[i3][1].number();
                  count++;
                  break;
                }
              }
            } else if (CHILD(i2).isPower() && CHILD(i2)[1].isNumber() &&
                       CHILD(i2)[1].number().isRational() &&
                       CHILD(i2)[1].number().isNegative() &&
                       CHILD(i2)[0] == syms[i]) {
              if (min_pow_i.isZero() || CHILD(i2)[1].number() > min_pow_i)
                min_pow_i = CHILD(i2)[1].number();
              count++;
            }
          }
            ...
            for (size_t i2 = 0; i2 < SIZE; i2++) {
              if (CHILD(i2).isMultiplication()) {
                for (size_t i3 = 0; i3 < CHILD(i2).size(); i3++) {
                  if (CHILD(i2)[i3].isPower() && CHILD(i2)[i3][1].isNumber() &&
                      CHILD(i2)[i3][1].number().isRational() &&
                      CHILD(i2)[i3][1].number().isPositive() &&
                      CHILD(i2)[i3][0] == syms[i]) {
                    if (min_pow_i.isZero() ||
                        CHILD(i2)[i3][1].number() < min_pow_i)
                      min_pow_i = CHILD(i2)[i3][1].number();
                    ...
                    if (min_pow_i.isZero() || min_pow_i > 1)
                      min_pow_i = 1;
                    count++;
                    break;
                  }
            ...
            for (size_t i3 = 0; i3 < CHILD(i2).size(); i3++) {
              if (CHILD(i2)[i3].isPower() && CHILD(i2)[i3][1].isNumber() &&
                  ...
                  (min_pow.isPositive()
                       ...
                       : CHILD(i2)[i3][1].number().isNegative()) &&
                  CHILD(i2)[i3][0] == syms[i]) {
                if (CHILD(i2)[i3][1] == min_pow)
                  CHILD(i2).delChild(i3 + 1, true);
                else if (CHILD(i2)[i3][1] == min_pow + 1)
                  CHILD(i2)[i3].setToChild(1, true);
                else {
                ...
                break;
                     ...
                     (min_pow.isPositive()
                          ...
                          : CHILD(i2)[1].number().isNegative()) &&
                     CHILD(i2)[0] == syms[i]) {
            if (CHILD(i2)[1] == min_pow)
              CHILD(i2).set(1, 1, 0, true);
            else if (CHILD(i2)[1] == min_pow + 1)
              CHILD(i2).setToChild(1, true);
            else {
      ...
      if (LAST.isNumber()) {
        ...
        mdel->ref();
        delChild(SIZE, true);
        b = factorize(eo, false, term_combination_levels - 1, 0, only_integers,
                      recursive, endtime_p, force_factorization,
                      complete_square, only_sqrfree, max_factor_degree);
      ...
      for (size_t i = 0; !b && i < SIZE; i++) {
        ...
        if (mdel->isMultiplication()) {
          for (size_t i2 = 0; i2 < mdel->size(); i2++) {
            if ((*mdel)[i2].isPower() && (*mdel)[i2][0].containsUnknowns()) {
              if (contains((*mdel)[i2][0], false, false, false) > 0) {
                b = false;
                break;
              }
            } else if ((*mdel)[i2].containsUnknowns()) {
              if (contains((*mdel)[i2], false, false, false) > 0) {
                b = false;
                break;
              }
            }
        ...
        if (b) {
          b = factorize(eo, false, term_combination_levels - 1, 0,
                        only_integers, recursive, endtime_p,
                        force_factorization, complete_square, only_sqrfree,
                        max_factor_degree);
          if (recursive)
            mdel->factorize(eo, false, term_combination_levels, 0,
                            only_integers, recursive, endtime_p,
                            force_factorization, complete_square, only_sqrfree,
                            max_factor_degree);
          add_nocopy(mdel, true);
          ...
          if (CHILD(i2).isMultiplication()) {
            for (size_t i3 = 0; i3 < CHILD(i2).size(); i3++) {
              if (CHILD(i2)[i3].isPower() && CHILD(i2)[i3][1].isNumber() &&
                  ...
                  CHILD(i2)[i3][1].number().isNegative() &&
                  CHILD(i2)[i3][0] == syms[i]) {
                if (min_pow_i.isZero() || CHILD(i2)[i3][1].number() > min_pow_i)
                  min_pow_i = CHILD(i2)[i3][1].number();
                count++;
                break;
              }
      ...
      } else if (term_combination_levels > 0 &&
                 term_combination_levels - 1 < max_run_index) {
      ...
      do {
        ...
        if (endtime_p && endtime_p->tv_sec > 0) {
          ...
          if (curtime.tv_sec > endtime_p->tv_sec ||
              (curtime.tv_sec == endtime_p->tv_sec &&
               curtime.tv_usec > endtime_p->tv_usec)) {
#else
          struct timespec curtime;
          clock_gettime(CLOCK_MONOTONIC, &curtime);
          if (curtime.tv_sec > endtime_p->tv_sec ||
              (curtime.tv_sec == endtime_p->tv_sec &&
               curtime.tv_nsec / 1000 > endtime_p->tv_usec)) {
#endif
            CALCULATOR->error(
                false,
                _("Because of time constraints only a limited number of "
                  "combinations of terms were tried during factorization. "
                  "Repeat factorization to try other random combinations."),
                NULL);
            break;
          }
        }
        ...
        if (mtest.factorize(eo, false, run_index == 0 ? 0 : -1 - run_index, 0,
                            only_integers, false, endtime_p,
                            force_factorization, complete_square, only_sqrfree,
                            max_factor_degree)) {
          bool b =
              best_index < 0 || (mbest.isAddition() && !mtest.isAddition());
          if (!b && (mtest.isAddition() == mbest.isAddition())) {
            ...
            if (!b && (!mtest.isAddition() || mtest.size() == mbest.size())) {
              size_t c1 = mtest.countTotalChildren() +
                          CHILD(index).countTotalChildren();
              size_t c2 = mbest.countTotalChildren() +
                          CHILD(best_index).countTotalChildren();
              b = (c1 < c2);
              if (c1 == c2) {
                b = (count_powers(mtest) + count_powers(CHILD(index))) <
                    (count_powers(mbest) + count_powers(CHILD(best_index)));
              }
            }
          }
          if (b) {
      ...
      if (best_index >= 0) {
        ...
        if (term_combination_levels >= -1 && (run_index > 0 || recursive)) {
          factorize(eo, false, term_combination_levels, 0, only_integers, true,
                    endtime_p, force_factorization, complete_square,
                    only_sqrfree, max_factor_degree);
        }
        return true;
  ...
  default: {
    ...
    if (isComparison()) {
      ...
      for (size_t i = 0; i < SIZE; i++) {
        if (CHILD(i).factorize(eo2, false, term_combination_levels, 0,
                               only_integers, recursive, endtime_p,
                               force_factorization, complete_square,
                               only_sqrfree, max_factor_degree)) {
          CHILD_UPDATED(i);
      ...
      for (size_t i = 0; i < SIZE; i++) {
        if (CHILD(i).factorize(eo, false, term_combination_levels, 0,
                               only_integers, recursive, endtime_p,
                               force_factorization, complete_square,
                               only_sqrfree, max_factor_degree)) {
          CHILD_UPDATED(i);
