/*
    ...
    Copyright (C) 2004-2006, 2016  Hanna Knutsson
...
public:
   ...
   * @param is_approximate If the value is approximate. 1 for approximate, 0 for
   * exact, -1 for property default.
   */
  void setProperty(DataProperty *property, std::string s_value,
                   int is_approximate = -1);
  /** Set an untranslated value for a key property. Used when a text value is
   * translated, but the original value still is needed as a reference key.
   *
   * @param property Property to set (must belong to parent data set).
   * @param s_vale Value for the property.
   */
  ...
  /** Returns parsed value for a property. Parses the text string value if not
   * parsed before
   *
   ...
   *
   * @param property Property to read.
   * @param[out] is_approximate If the value is approximate. Is set to 1 for
   * approximate, 0 for exact, -1 for property default, if not NULL.
   * @returns Unparsed value or empty string if property value is not set.
   */
  const std::string &getProperty(DataProperty *property,
                                 int *is_approximate = NULL);
  /** Returns unparsed untranslated value for a key property. Used when a text
   * value is translated, but the original value still is needed as a reference
   * key.
   *
   * @param property Property to read.
   * @returns Unparsed untranslated value or empty string if property value is
   * not set.
   */
  const std::string &getNonlocalizedKeyProperty(DataProperty *property);
  /** Returns value for a property in a format suitable for use in expressions
   * with unit appended.
   *
   * @param property Property to read.
   * @returns Value in input format or empty string if property value is not
   * set.
   */
  std::string getPropertyInputString(DataProperty *property);
  /** Returns value for a property in a format suitable for display with unit
   * appended.
   *
   * @param property Property to read.
   * @returns Value in display format or empty string if property value is not
   * set.
   */
  std::string getPropertyDisplayString(DataProperty *property);

  /** If the object has been modified by the end user (if setUserModified() has
   * been called).
   *
...
public:
   ...
   */
  DataProperty(DataSet *parent_set, std::string s_name = "",
               std::string s_title = "", std::string s_description = "");
  ...
  MathStructure *generateStruct(const std::string &valuestr,
                                int is_approximate = -1);
...
/// A data set.
/** This is a simple database class for storage of many grouped values, when
 * ordinary variables is not practical.
 *
 * A data set consists of properties and objects, with values for the
 * properties. Qalculate! includes for example a "Planets" data set with
 * properties such as name, mass, speed and density, and an object for each
 * planet in solar system.
 *
 * A data set is also mathemtical function, dataset(object, property), which
 * retrieves values for objects and properties. Data sets can be saved and
 * loaded from a XML file.
 */
...
public:
  DataSet(std::string s_category = "", std::string s_name = "",
          std::string s_default_file = "", std::string s_title = "",
          std::string s_description = "", bool is_local = true);
  ...
  const MathStructure *getObjectProperyStruct(std::string property,
                                              std::string object);
  const std::string &getObjectProperty(std::string property,
                                       std::string object);
  std::string getObjectPropertyInputString(std::string property,
                                           std::string object);
  std::string getObjectPropertyDisplayString(std::string property,
                                             std::string object);
...
public:
  DataPropertyArgument(DataSet *data_set, std::string name_ = "",
...
public:
  DataObjectArgument(DataSet *data_set, std::string name_ = "",
                     bool does_test = true, bool does_error = true);
