...
/**
 * A mathematical function, subclassed from MathFunction, should at least
 * reimplement calculate(MathStructure&, const MathStructure&, const
 * EvaluationOptions&) and copy(), and preferably also the represents*
...
protected:
  ...
  virtual MathStructure
  createFunctionMathStructureFromVArgs(const MathStructure &vargs);
  virtual MathStructure
  createFunctionMathStructureFromSVArgs(std::vector<std::string> &svargs);
  std::string scondition;
  std::string sexample;

public:
  MathFunction(std::string name_, int argc_, int max_argc_ = 0,
               std::string cat_ = "", std::string title_ = "",
               std::string descr_ = "", bool is_active = true);
  ...
  /** Returns the subtype of the mathematical function,  corresponding to which
   * subsubclass the object belongs to.
   *
  ...
  std::string example(bool raw_format = false,
                      std::string name_string = "") const;
  void setExample(std::string new_example);

  bool testArgumentCount(int itmp);
  virtual MathStructure
  calculate(const std::string &eq,
            const EvaluationOptions &eo = default_evaluation_options);
  virtual MathStructure parse(const std::string &eq,
                              const ParseOptions &po = default_parse_options);
  virtual int parse(MathStructure &mstruct, const std::string &eq,
                    const ParseOptions &po = default_parse_options);
  virtual MathStructure
  calculate(MathStructure &vargs,
            const EvaluationOptions &eo = default_evaluation_options);
  /**
   * The main function for subclasses to reimplement.
   * Calculates a value from arguments in vargs and puts it in mstruct.
   *
   * This function expects the number of arguments to be equal to the maximum
   * number of arguments, and checked by the argument definitions.
   *
   * If the return value is negative, then argument -(return value) has been
   * evaluated in mstruct. If -(return value) is greater than max arguments,
   * then mstruct is a std::vector of evaluated argument values.
   *
   * @param[out] mstruct Structure that is set with the result of the
   * calculation.
   ...
   */
  virtual int calculate(MathStructure &mstruct, const MathStructure &vargs,
  ...
  /** Returns the maximum number of arguments that the function accepts or -1 if
   ...
   */
  int maxargs() const;
  /** Parses arguments from a text string and places them in a std::vector. The
   * text string should be a comma separated list of arguments.
   *
   ...
   */
  int args(const std::string &str, MathStructure &vargs,
           const ParseOptions &po = default_parse_options);
   ...
   * @returns The argument definition for the index or NULL if no the argument
   * was not defined for the index
   */
  ...
  MathStructure produceVector(const MathStructure &vargs, int begin = -1,
                              int end = -1);
  MathStructure produceArgumentsVector(const MathStructure &vargs,
...
/**
 * User functions are functions defined using expression strings, representing
 * mathematical formulas.
 *
 * The expression/formula of a function is basically a normal expression with
 * placeholders for arguments. These placeholders consists of a backslash and a
 * letter — x, y, z for the 1st, 2nd and 3rd arguments and a to u for argument 4
 * to 24. They are replaced by entered arguments when a function is calculated.
 * The placeholders naturally also decide the number of arguments that a
 * function requires. For example the function for triangle area ("base * height
 * / 2") has the name triangle and the formula "(\x*\y)/2", which gives that
 * "triangle(2, 3)" equals "(2*3) / 2" and returns "3" as result. An argument
 * can be used more than one time and all arguments must not necessarily be in
 * order in the formula.
 *
 * Additionally, optional arguments can be put in the formula with upper-case
 * (X, Y, Z, ...) instead of lower-case letters (x, y, z, ...). The default
 * value can be put in brackets after the letter (ex. "\X{2}"). The default
 * value may be omitted and is then zero. All additional arguments after an
 * optional argument must also be optional.
 *
 ...
 * ...). Even though it would be quite meaningless, the formula for triangle
 * function could for example have a subfunction "\x*\y" and the formula "\1/2".
...
public:
  UserFunction(std::string cat_, std::string name_, std::string formula_,
               bool is_local = true, int argc_ = -1, std::string title_ = "",
               std::string descr_ = "", int max_argc_ = 0,
               bool is_active = true);
...
/// A mathematical function argument definition with free value and base class
...
protected:
  ...
  bool b_zero, b_test, b_matrix, b_text, b_error, b_rational, b_last,
      b_handle_vector;
  /** This function is called from Argument::test() and performs validation
   * specific to the argument definition type. Should be reimplemented by all
   * subclasses.
   *
  ...
  /** This function is called from Argument::printlong() and returns description
   * specific the argument definition type. Should be reimplemented by all
   * subclasses. For example IntegerArgument::subprintlong() might return "an
   * integer" and Argument::printlong() might append " that fulfills the
   * condition: even(\x)".
   *
...
public:
   ...
   */
  Argument(std::string name_ = "", bool does_test = true,
   ...
   * @returns true if the value is valid for the argument definition.
   */
  bool test(MathStructure &value, int index, MathFunction *f,
   ...
   * @param po Parse options.
   */
  virtual void parse(MathStructure *mstruct, const std::string &str,
                     const ParseOptions &po = default_parse_options) const;
  ...
  /** If the value for the argument will be tested. If not, the argument only
   * works as an suggestion and any value is allowed.
   *
  ...
  /** If an error message will be presented to the user if the value for the
  ...
  /** Returns the type of the argument, corresponding to which subclass the
   * object belongs to.
   *
...
/** These arguments allows numerical values. The value can be restricted to real
 * or rational numbers (defaults to allow all numbers, including complex), and a
 * max and/or min value.
 */
...
public:
  NumberArgument(std::string name_ = "",
                 ArgumentMinMaxPreDefinition minmax = ARGUMENT_MIN_MAX_NONE,
                 bool does_test = true, bool does_error = true);
...
/** These arguments allows numerical integer values. The value can be restricted
 * to a max and/or min value.
 */
...
public:
  IntegerArgument(std::string name_ = "",
                  ArgumentMinMaxPreDefinition minmax = ARGUMENT_MIN_MAX_NONE,
                  bool does_test = true, bool does_error = true,
                  IntegerType integer_type = INTEGER_TYPE_NONE);
...
public:
  SymbolicArgument(std::string name_ = "", bool does_test = true,
                   bool does_error = true);
...
/// A text argument.
/** Accepts text (symbolic) structures. Argument values are parsed as text,
 * unless surrounded by back slashes (which are then removed). Surrounding
 * Parentheses and first quotation marks are removed.
 */
...
public:
  TextArgument(std::string name_ = "", bool does_test = true,
               bool does_error = true);
...
public:
  DateArgument(std::string name_ = "", bool does_test = true,
               bool does_error = true);
...
public:
  VectorArgument(std::string name_ = "", bool does_test = true,
                 bool allow_matrix = false, bool does_error = true);
...
public:
  MatrixArgument(std::string name_ = "", bool does_test = true,
                 bool does_error = true);
...
public:
  ExpressionItemArgument(std::string name_ = "", bool does_test = true,
                         bool does_error = true);
...
public:
  FunctionArgument(std::string name_ = "", bool does_test = true,
                   bool does_error = true);
...
public:
  BooleanArgument(std::string name_ = "", bool does_test = true,
                  bool does_error = true);
...
public:
  UnitArgument(std::string name_ = "", bool does_test = true,
               bool does_error = true);
...
public:
  AngleArgument(std::string name_ = "", bool does_test = true,
                bool does_error = true);
...
public:
  VariableArgument(std::string name_ = "", bool does_test = true,
                   bool does_error = true);
...
public:
  FileArgument(std::string name_ = "", bool does_test = true,
               bool does_error = true);
...
/** This is used when several different type of arguments shall be accepted by a
 * function.
 */
...
public:
  ArgumentSet(std::string name_ = "", bool does_test = true,
              bool does_error = true);
