...
bool combine_powers(MathStructure &m, const MathStructure &x_var,
                    const EvaluationOptions &eo, const EvaluationOptions &feo) {
  bool b_ret = false;
  ...
  for (size_t i = 0; i < m.size() - 1; i++) {
    if (m[i].isPower() && !m[i][0].contains(x_var, true) &&
        m[i][1].contains(x_var, true)) {
      for (size_t i2 = i + 1; i2 < m.size(); i2++) {
        // a^(f(x))*b^(g(x))=e^(f(x)/ln(a)+g(x)/ln(b))
        if (m[i2].isPower() && !m[i2][0].contains(x_var, true) &&
            m[i2][1].contains(x_var, true)) {
          if (m[i2][0] != m[i][0]) {
            if (!m[i2][0].isVariable() ||
                m[i2][0].variable()->id() != VARIABLE_ID_E) {
            ...
            if (!m[i][0].isVariable() ||
                m[i][0].variable()->id() != VARIABLE_ID_E) {
...
bool isUnit_multi(const MathStructure &mstruct) {
  ...
  for (size_t i = 0; i < mstruct.size(); i++) {
    if ((i > 0 || !mstruct[i].isNumber()) && !mstruct[i].isUnit_exp())
      return false;
  }
  return true;
}

MathStructure *find_mvar(MathStructure &m, const MathStructure &x_var,
...
bool get_multiplier(const MathStructure &mstruct, const MathStructure &xvar,
                    MathStructure &mcoeff, size_t exclude_i = (size_t)-1) {
      ...
      for (size_t i2 = 0; i2 < mcur->size(); i2++) {
        if ((*mcur)[i2] == xvar) {
          ...
          if (mcoeff.isZero()) {
            ...
            for (size_t i3 = 0; i3 < mcur->size(); i3++) {
              if (i3 != i2) {
                if (mcoeff[mcoeff.size() - 1].isZero())
                  mcoeff[mcoeff.size() - 1] = (*mcur)[i3];
                else
                  mcoeff[mcoeff.size() - 1].multiply((*mcur)[i3], true);
              }
            }
          }
          break;
        } else if (xvar.isMultiplication() && xvar.size() > 0 &&
                   (*mcur)[i2] == xvar[0]) {
          if (mcur->size() - i2 < xvar.size())
            break;
          b = true;
          for (size_t i3 = 1; i3 < xvar.size(); i3++) {
            if ((*mcur)[i2 + i3] != xvar[i3]) {
              b = false;
              break;
            }
          }
...
bool get_power(const MathStructure &mstruct, const MathStructure &xvar,
               MathStructure &mpow) {
  if (mstruct == xvar) {
    mpow = m_one;
    return true;
  ...
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (get_power(mstruct[i], xvar, mpow))
      return true;
  }
  return false;
}
const MathStructure *get_power_term(const MathStructure &mstruct,
                                    const MathStructure &xvar) {
  if (mstruct == xvar) {
    return &mstruct;
  }
  if (mstruct.isPower() && mstruct[0] == xvar) {
...
int test_comparisons(const MathStructure &msave, MathStructure &mthis,
                     const MathStructure &x_var, const EvaluationOptions &eo,
                     bool sub, int alt) {
  if (mthis.isComparison() && mthis[0] == x_var) {
    if (mthis.comparisonType() != COMPARISON_EQUALS &&
        mthis.comparisonType() != COMPARISON_NOT_EQUALS)
      return 1;
    MathStructure mtest;
    ...
    if (mtest.isComparison()) {
      ...
      if (CALCULATOR->usesIntervalArithmetic()) {
        ...
        if (mtest2.isNumber()) {
          if (mtest.comparisonType() == COMPARISON_LESS ||
              mtest.comparisonType() == COMPARISON_EQUALS_LESS) {
            if (!mtest2.number().hasImaginaryPart() &&
                mtest2.number().lowerEndPoint().isNonPositive())
              return 1;
          } else if (mtest.comparisonType() == COMPARISON_GREATER ||
                     mtest.comparisonType() == COMPARISON_EQUALS_GREATER) {
            if (!mtest2.number().hasImaginaryPart() &&
                mtest2.number().upperEndPoint().isNonNegative())
              return 1;
          } else if (mtest.comparisonType() == COMPARISON_NOT_EQUALS) {
            if (!mtest2.number().isNonZero())
              return 1;
            else if (mtest2.number().isInterval() ||
                     mtest2.number().isRational()) {
              mthis = m_one;
              return 0;
            }
          } else {
          ...
          if (mtest.comparisonType() == COMPARISON_GREATER ||
              mtest.comparisonType() == COMPARISON_EQUALS_GREATER)
            return 1;
          else if (mtest.comparisonType() == COMPARISON_EQUALS) {
            mthis.clear();
            return 0;
          } else if (mtest.comparisonType() == COMPARISON_NOT_EQUALS) {
            mthis = m_one;
          ...
          if (mtest.comparisonType() == COMPARISON_LESS ||
              mtest.comparisonType() == COMPARISON_EQUALS_LESS) {
            if (mtest2[0].isNumber() &&
                !mtest2[0].number().hasImaginaryPart() &&
                mtest2[0].number().lowerEndPoint().isNonPositive())
              return 1;
          } else if (mtest.comparisonType() == COMPARISON_GREATER ||
                     mtest.comparisonType() == COMPARISON_EQUALS_GREATER) {
            if (!mtest2[0].isNumber() ||
            ...
            if (!mtest2[0].number().isNonZero())
              return 1;
            else if (mtest2[0].number().isInterval() ||
                     mtest2[0].number().isRational()) {
              mthis = m_one;
              return 0;
            }
          } else {
      ...
      if (mtest.isNumber()) {
        if (mtest.number().getBoolean() == 1) {
          ...
          if (mtest.isApproximate() &&
              mtest.comparisonType() != COMPARISON_EQUALS &&
              mtest.comparisonType() != COMPARISON_NOT_EQUALS)
            return -1;
          if (mtest.comparisonType() == COMPARISON_EQUALS)
            mthis.clear();
          else if (mtest.comparisonType() == COMPARISON_NOT_EQUALS)
            mthis = m_one;
          return 0;
...
int newton_raphson(const MathStructure &mstruct, MathStructure &x_value,
                   const MathStructure &x_var, const EvaluationOptions &eo) {
  if (mstruct == x_var) {
    x_value = m_zero;
    return 1;
  ...
  if (CALCULATOR->aborted() || !mstruct.isAddition() || mstruct.size() > 200)
    return -1;
  if (mstruct.size() == 2) {
    if (mstruct[1] == x_var && mstruct[0].isNumber() &&
        mstruct[0].number().isReal()) {
      x_value = mstruct[0];
      x_value.number().negate();
      return 1;
    }
    if (mstruct[0] == x_var && mstruct[1].isNumber() &&
        mstruct[1].number().isReal()) {
      x_value = mstruct[1];
      x_value.number().negate();
      return 1;
    }
  ...
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (mstruct[i] != x_var) {
      ...
      case STRUCT_MULTIPLICATION: {
        if (mstruct[i].size() == 2 && mstruct[i][0].isNumber() &&
            (mstruct[i][1] == x_var ||
             (mstruct[i][1].isPower() && mstruct[i][1][0] == x_var &&
              mstruct[i][1][1].isNumber() &&
              mstruct[i][1][1].number().isInteger() &&
              mstruct[i][1][1].number().isPositive()))) {
          break;
        }
        return -1;
      }
      case STRUCT_POWER: {
        if (mstruct[i][0] == x_var && mstruct[i][1].isNumber() &&
            mstruct[i][1].number().isInteger() &&
            mstruct[i][1].number().isPositive()) {
          break;
        }
      }
      default: {
  ...
  if (!mdiff.differentiate(x_var, eo))
    return -1;
  ...
  for (int i = 0; i < 100 + PRECISION + ideg * 2; i++) {
    ...
    if (mguess.number().hasImaginaryPart())
      mguess.number().internalImaginary()->setToFloatingPoint();
    ...
    if (nrdiv.isLessThan(nr_target_high) &&
        nrdiv.isGreaterThan(nr_target_low)) {
      if (CALCULATOR->usesIntervalArithmetic()) {
        if (!x_value.number().setInterval(mguess.number(), mtest.number()))
          return -1;
      } else {
        x_value = mtest;
        if (x_value.number().precision() < 0 ||
            x_value.number().precision() > PRECISION + 10)
...
int find_interval_precision(const MathStructure &mstruct) {
  ...
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (iv_prec > -1) {
      if (find_interval_precision(mstruct[i]) > -1)
        return 0;
    } else {
...
MathStructure *find_abs_sgn(MathStructure &mstruct,
  ...
  case STRUCT_FUNCTION: {
    if (((mstruct.function()->id() == FUNCTION_ID_ABS && mstruct.size() == 1) ||
         (mstruct.function()->id() == FUNCTION_ID_SIGNUM &&
          mstruct.size() == 2)) &&
        mstruct[0].contains(x_var, false) &&
        mstruct[0].representsNonComplex()) {
      return &mstruct;
    }
    break;
  }
...
bool fix_n_multiple(MathStructure &mstruct, const EvaluationOptions &eo,
                    const EvaluationOptions &feo, const MathStructure &x_var) {
  bool b_ret = false;
  if (mstruct.isComparison()) {
    if (mstruct.comparisonType() == COMPARISON_EQUALS && x_var.isVariable() &&
        !x_var.variable()->isKnown() &&
        !((UnknownVariable *)x_var.variable())->interval().isUndefined() &&
        mstruct[1].contains(CALCULATOR->getVariableById(VARIABLE_ID_N))) {
      ...
      if (CALCULATOR->usesIntervalArithmetic()) {
        ...
        if (mtest.isolate_x(eo2, feo2,
                            CALCULATOR->getVariableById(VARIABLE_ID_N))) {
          if (CALCULATOR->endTemporaryStopMessages() == 0) {
            if (mtest.isZero()) {
              mstruct.clear(true);
              b_ret = true;
            } else if (mtest.isComparison() &&
                       mtest.comparisonType() == COMPARISON_EQUALS &&
                       mtest[0].isVariable() &&
                       mtest[0].variable() ==
                           CALCULATOR->getVariableById(VARIABLE_ID_N) &&
              ...
              if (mtest[1].number().isInteger()) {
                mstruct.calculateReplace(
                    CALCULATOR->getVariableById(VARIABLE_ID_N), mtest[1], eo);
                b_ret = true;
                ...
                if (mtest[1].number().getCentralInteger(nr_int, &b_multiple)) {
                  mstruct.calculateReplace(
                      CALCULATOR->getVariableById(VARIABLE_ID_N), nr_int, eo);
                  b_ret = true;
                } else if (!b_multiple) {
                  mstruct.clear(true);
                  b_ret = true;
    ...
    if (b_ret)
      mstruct.calculatesub(eo, feo, false);
  }
  return b_ret;
}

bool MathStructure::isolate_x_sub(const EvaluationOptions &eo,
  ...
  case STRUCT_ADDITION: {
    ...
    if (CHILD(0).size() >= 2 &&
      ...
      for (size_t i = 0; i < CHILD(0).size(); i++) {
        if (CHILD(0)[i] == x_var ||
            (CHILD(0)[i].isPower() && CHILD(0)[i][0] == x_var)) {
          b = false;
          ...
          for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
            if (CHILD(0)[i][i2] == x_var ||
                (CHILD(0)[i][i2].isPower() && CHILD(0)[i][i2][0] == x_var)) {
              b = false;
              const MathStructure *mexp = &m_one;
              if (CHILD(0)[i][i2] != x_var)
                mexp = &CHILD(0)[i][i2][1];
      ...
      if (b) {
        ...
        if (a_iv >= 0 && (c_iv < 0 || c_iv > a_iv) &&
            CHILD(1).representsNonZero()) {
          // x=(-2c)/(b+/-sqrt(b^2-4ac))
          ...
          if (c_iv >= 0 && c_iv <= PRECISION) {
            ...
            stop_iv = true;
            if (mstruct_b.isNumber() && mstruct_b.number().isNonZero() &&
                !mstruct_b.number().hasImaginaryPart()) {
              mstruct_bl = mstruct_b.number().lowerEndPoint();
              mstruct_bu = mstruct_b.number().upperEndPoint();
              stop_iv = false;
            } else if (is_units_with_multiplier(mstruct_b) &&
                       mstruct_b[0].number().isNonZero() &&
                       !mstruct_b[0].number().hasImaginaryPart()) {
          ...
          if (b) {
            ...
            if (stop_iv) {
              ...
              CALCULATOR->error(false,
                                _("Interval arithmetic was disabled during "
                                  "calculation of %s."),
                                format_and_print(mbak).c_str(), NULL);
          ...
          if (!mstruct_a.representsNonZero()) {
            ...
            mtest->transform(ct == COMPARISON_NOT_EQUALS
                                 ? COMPARISON_EQUALS
                                 : COMPARISON_NOT_EQUALS,
                             m_zero);
            mtest->calculatesub(eo2, eo, false);
            transform_nocopy(ct == COMPARISON_NOT_EQUALS ? STRUCT_LOGICAL_OR
                                                         : STRUCT_LOGICAL_AND,
                             mtest);
            calculatesub(eo2, eo, false);
          }
          ...
          mchild2->isolate_x_sub(eo, eo2, x_var, morig);
          transform_nocopy(ct == COMPARISON_NOT_EQUALS ? STRUCT_LOGICAL_AND
                                                       : STRUCT_LOGICAL_OR,
                           mchild2);
          if (!mstruct_b.representsNonZero() &&
              !mstruct_a.representsNonZero()) {
            ...
            mchild3a->transform_nocopy(ct == COMPARISON_NOT_EQUALS
                                           ? STRUCT_LOGICAL_OR
                                           : STRUCT_LOGICAL_AND,
                                       mchild3b);
            mchild3a->calculatesub(eo2, eo, false);
            add_nocopy(mchild3a,
                       ct == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_AND
    ...
    // a*b^(dx)+cx=y =>
    // (y*d*log(b)-c*lambertw(a*b^(y*d/c)*log(b)/c))/(c*d*log(b))
    if ((ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) &&
        ...
        if (CHILD(0)[i].isMultiplication()) {
          for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
            if (CHILD(0)[i][i2].isPower() &&
                CHILD(0)[i][i2][1].contains(x_var) &&
                !CHILD(0)[i][i2][0].contains(x_var)) {
              mvar = &CHILD(0)[i][i2][1];
          ...
          if (mvar)
            break;
        } else if (CHILD(0)[i].isPower() && CHILD(0)[i][1].contains(x_var) &&
                   !CHILD(0)[i][0].contains(x_var)) {
          mvar = &CHILD(0)[i][1];
          m_b_p = &CHILD(0)[i][0];
          i_px = i;
          break;
      ...
      if (mvar) {
        ...
        if ((mvar->isAddition() || mvar->isMultiplication()) &&
            m_b.representsPositive()) {
          ...
          if (mvar2) {
            ...
            if (get_multiplier(*mvar, *mvar2, m_b_exp) &&
                m_b.representsReal() && !m_b.contains(x_var)) {
              m_b ^= m_b_exp;
              mvar = mvar2;
            } else {
              mvar = NULL;
        ...
        MathStructure m_c;
        if (mvar && get_multiplier(CHILD(0), *mvar, m_c, i_px) &&
            m_c.representsNonZero() && m_c.contains(x_var) == 0 &&
            m_b.representsNonZero()) {
          MathStructure mlogb(CALCULATOR->getFunctionById(FUNCTION_ID_LOG),
                              &m_b, NULL);
          if (mlogb.calculateFunctions(eo))
            mlogb.calculatesub(eo2, eo, true);
          ...
          if (marg) {
            if (!CHILD(1).isZero()) {
              ...
              if (!m_c.isOne())
                marg->last().last().calculateDivide(m_c, eo2);
              marg->last().calculateRaiseExponent(eo2);
              marg->calculateMultiplyLast(eo2);
            }
            if (!m_c.isOne())
              marg->calculateDivide(m_c, eo2);
          }
          if (marg && mvar->representsNonComplex() &&
              m_b.representsPositive()) {
            ...
            if (!marg->representsNonNegative()) {
              ...
              if (marg2->calculateFunctions(eo))
                marg2->calculatesub(eo2, eo, true);
              marg2->calculateNegate(eo2);
              if (CHILD(1).isZero()) {
                marg2->calculateDivide(mlogb, eo2);
              } else {
                if (!m_c.isOne())
                  marg2->calculateMultiply(m_c, eo2);
                marg2->add(CHILD(1));
                marg2->last().calculateMultiply(mlogb, eo);
                marg2->calculateAddLast(eo2);
                marg2->calculateDivide(mlogb, eo2);
                if (!m_c.isOne())
                  marg2->calculateDivide(m_c, eo2);
              }
              mreq1->transform(ct_comp == COMPARISON_NOT_EQUALS
                                   ? COMPARISON_LESS
              ...
              mreq2->transform(ct_comp == COMPARISON_EQUALS
                                   ? COMPARISON_LESS
                                   : COMPARISON_EQUALS_GREATER,
                               m_zero);
              ...
              marg2->add(*mreq1, ct_comp == COMPARISON_NOT_EQUALS
                                     ? OPERATION_LOGICAL_OR
                                     : OPERATION_LOGICAL_AND);
              marg2->add_nocopy(mreq2,
            ...
            if (CHILD(1).isZero()) {
              ...
              if (!m_c.isOne())
                marg->calculateMultiply(m_c, eo2);
              marg->add(CHILD(1));
            ...
            isolate_x_sub(eo, eo2, x_var, morig);
            if (mreq1) {
              add_nocopy(mreq1, ct_comp == COMPARISON_NOT_EQUALS
                                    ? OPERATION_LOGICAL_OR
                                    : OPERATION_LOGICAL_AND);
              calculatesub(eo2, eo, false);
            }
            if (marg2) {
              add_nocopy(marg2, ct_comp == COMPARISON_EQUALS
                                    ? OPERATION_LOGICAL_OR
                                    : OPERATION_LOGICAL_AND);
              calculatesub(eo2, eo, false);
            }
      ...
      if (i_px == 0) {
        ...
        for (size_t i = 0; i < CHILD(0).size() && b; i++) {
          ...
          if (CHILD(0)[i].isMultiplication()) {
            for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
              if (!b && CHILD(0)[i][i2].isPower() &&
                  CHILD(0)[i][i2][1].contains(x_var) &&
                  !CHILD(0)[i][i2][0].contains(x_var)) {
                if (mvar2) {
                  if (mvar1->equals(CHILD(0)[i][i2]) ||
                      mvar2->equals(CHILD(0)[i][i2])) {
                    b = true;
                  }
                } else if (mvar1) {
                  if (!mvar1->equals(CHILD(0)[i][i2])) {
        ...
        if (b && mvar2 && mvar1->base()->representsPositive()) {
          ...
          } else if (mvar1->base()->isNumber() && mvar2->base()->isNumber() &&
                     !mvar1->base()->number().isInterval() &&
                     !mvar2->base()->number().isInterval()) {
            if (mvar1->base()->number() < mvar2->base()->number()) {
              if (mvar2->base()->number() == (mvar1->base()->number() ^ 2)) {
                b_two = true;
              } else if (mvar2->base()->number() ==
                         (mvar1->base()->number() ^ 3)) {
                b_three = true;
              } else if (mvar2->base()->number() ==
                         (mvar1->base()->number() ^ Number(3, 2))) {
                b_three = true;
                b_two = true;
              } else if (mvar1->base()->number() ==
                         (mvar2->base()->number() ^ -1)) {
                b_m_one = true;
                MathStructure *mtmp = mvar2;
                mvar2 = mvar1;
          ...
          if (b_two || b_three || b_m_one) {
            ...
            if (b_three && b_two) {
              if (mvar1->base()->equals(*mvar2->base()))
                mv[1].calculateMultiply(nr_half, eo2);
              else
                mv[0].number().sqrt();
            }
            MathStructure mv2(*mvar2);
            UnknownVariable *var =
                new UnknownVariable("", format_and_print(mv));
            var->setInterval(mv);
      ...
      if (i_px == 0) {
        ...
        for (size_t i = 0; i < CHILD(0).size(); i++) {
          if (CHILD(0)[i].isMultiplication() && CHILD(0)[i].size() == 2 &&
              CHILD(0)[i][0].isNumber() && CHILD(0)[i][0].number().isReal() &&
              CHILD(0)[i][1].isPower() &&
              ((i == 0 && CHILD(0)[i][1][1].contains(x_var)) ||
               (i > 0 && CHILD(0)[i][1][1] == *mvar)) &&
              CHILD(0)[i][1][0].isNumber() &&
              CHILD(0)[i][1][0].number().isPositive()) {
            if (CHILD(0)[i][0].number().isPositive()) {
              ...
              if (!i_pos)
                pos_first = false;
              i_neg++;
              if (!b_mul && !CHILD(0)[i][0].number().isMinusOne())
                b_mul = true;
            } else {
              mvar = NULL;
              break;
            ...
            if (i == 0) {
              if (!CHILD(0)[i][1][1].representsReal())
                break;
              mvar = &CHILD(0)[i][1][1];
            }
            ...
            for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
              if (!b && CHILD(0)[i][i2].isPower() &&
                  ((i == 0 && CHILD(0)[i][i2][1].contains(x_var)) ||
                   (i > 0 && CHILD(0)[i][i2][1] == *mvar)) &&
                  CHILD(0)[i][i2][0].isNumber() &&
                  CHILD(0)[i][i2][0].number().isPositive()) {
                i_x = i2;
                b = true;
              } else if (CHILD(0)[i][i2].contains(x_var)) {
                b = false;
            ...
            if (b && i == 0) {
              if (!CHILD(0)[i][i_x][1].representsReal())
                break;
            ...
            if (!b) {
              ...
              break;
                     ...
                     ((i == 0 && CHILD(0)[i][1].contains(x_var)) ||
                      (i > 0 && CHILD(0)[i][1] == *mvar)) &&
                     CHILD(0)[i][0].isNumber() &&
                     CHILD(0)[i][0].number().isPositive()) {
            if (!i_neg)
              neg_first = false;
            if (i_neg)
              pos_first = false;
        ...
        if (mvar) {
          if ((pos_first && !i_neg && CHILD(1).representsNonPositive()) ||
              (neg_first && !i_pos && CHILD(1).representsNonNegative())) {
            if (ct_comp == COMPARISON_EQUALS) {
              clear(true);
            } else {
              set(1, 1, 0, true);
          ...
          if ((neg_first || pos_first) && b_mulnum && CHILD(1) == mulsum &&
              (CHILD(1).isZero() ||
               ((pos_first && CHILD(1).representsNonNegative()) ||
                (neg_first && CHILD(1).representsNonPositive())))) {
            CHILD(1).clear();
          ...
          if ((neg_first || pos_first) && b_mulnum && CHILD(1) == basesum &&
              ((CHILD(1).representsNonNegative() && pos_first) ||
               (CHILD(1).representsNonPositive() && neg_first))) {
            CHILD(1).set(1, 1, 0);
            if (!mvar->equals(x_var)) {
              MathStructure mx(*mvar);
              CHILD(0) = mx;
          ...
          if (eo2.expand && eo.approximation != APPROXIMATION_EXACT &&
              (neg_first || pos_first) && b_mulnum) {
            MathFunction *f =
                CALCULATOR->getFunctionById(FUNCTION_ID_NEWTON_RAPHSON);
            if (f) {
              UnknownVariable *var = NULL;
              MathStructure mx;
              if (!mvar->equals(x_var)) {
                var = new UnknownVariable("", string(LEFT_PARENTHESIS) +
                                                  format_and_print(*mvar) +
                                                  RIGHT_PARENTHESIS);
              ...
              if ((neg_first && i_pos && !CHILD(1).representsNonPositive()) ||
                  (pos_first && i_neg && !CHILD(1).representsNonNegative())) {
                MathStructure mdiff(CHILD(0));
                if (var)
                  mdiff.replace(*mvar, mx);
                if (mdiff.differentiate(mx, eo)) {
                  ...
                  if (b) {
                    ...
                    if (b) {
                      ...
                      if ((cr == COMPARISON_RESULT_GREATER && neg_first) ||
                          (cr == COMPARISON_RESULT_LESS && pos_first)) {
                        if (ct_comp == COMPARISON_EQUALS) {
                          clear(true);
                        } else {
                          set(1, 1, 0, true);
                  ...
                  if (b) {
                    ...
                    if (msolve1.isNumber()) {
                      ...
                      if (msolve2.isNumber() &&
                        ...
                        add_nocopy(malt, ct_comp == COMPARISON_EQUALS
                                             ? OPERATION_LOGICAL_OR
                                             : OPERATION_LOGICAL_AND);
                        calculatesub(eo2, eo, false);
                        if (var)
      ...
      if (CHILD(0).containsFunctionId(FUNCTION_ID_LOG)) {
        ...
        for (size_t i = 0; i < CHILD(0).size(); i++) {
          if (CHILD(0)[i].isMultiplication()) {
            ...
            for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
              if (CHILD(0)[i][i2].contains(x_var)) {
                if (!mln2 && CHILD(0)[i][i2].isFunction() &&
                    CHILD(0)[i][i2].function()->id() == FUNCTION_ID_LOG &&
                    CHILD(0)[i][i2].size() == 1) {
                  mln2 = &CHILD(0)[i][i2];
                } else {
                  mln2 = NULL;
          ...
          } else if (!mln && CHILD(0)[i].isFunction() &&
                     CHILD(0)[i].function()->id() == FUNCTION_ID_LOG &&
                     CHILD(0)[i].size() == 1) {
            mln = &CHILD(0)[i];
          } else if (!mvar) {
            mvar = &CHILD(0)[i];
        ...
        if (mvar && mln) {
          ...
          if (!mx.isOne()) {
            ...
            if (!mx.isOne() && mmul1.representsNonZero() &&
                mmul2.representsNonZero() && !mmul2.contains(x_var)) {
              MathStructure *marg;
              if (!CHILD(1).isZero()) {
                marg = new MathStructure(
                    CALCULATOR->getVariableById(VARIABLE_ID_E));
                marg->calculateRaise(CHILD(1), eo2);
                if (!mmul1.isOne())
                  marg->calculateMultiply(mmul1, eo2);
              } else {
                marg = new MathStructure(mmul1);
              }
              if (!mmul2.isOne())
                marg->calculateDivide(mmul2, eo2);
              MathStructure *mreq1 = NULL;
              ...
              if (!mmul1.isOne())
                CHILD(1).calculateDivide(mmul1, eo2);
              CHILDREN_UPDATED
              isolate_x_sub(eo, eo2, x_var, morig);
              if (mreq1) {
                add_nocopy(mreq1, ct_comp == COMPARISON_NOT_EQUALS
      ...
      if (CHILD(0).containsFunctionId(FUNCTION_ID_COSH) &&
          CHILD(0).containsFunctionId(FUNCTION_ID_SINH)) {
        // a*cosh(x) + b*cosh(x)
        ...
        for (size_t i = 0; i < CHILD(0).size(); i++) {
          if (CHILD(0)[i].isMultiplication()) {
            ...
            for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
              if (CHILD(0)[i][i2].contains(x_var)) {
                if (!b_found && CHILD(0)[i][i2].isFunction() &&
                    (CHILD(0)[i][i2].function()->id() == FUNCTION_ID_COSH ||
                     CHILD(0)[i][i2].function()->id() == FUNCTION_ID_SINH) &&
                    CHILD(0)[i][i2].size() == 1 &&
                    (!marg || marg->equals(CHILD(0)[i][i2][0]))) {
                  if (!marg)
                    marg = &CHILD(0)[i][i2][0];
                  if (CHILD(0)[i][i2].function()->id() == FUNCTION_ID_COSH)
                    m_cosh = &CHILD(0)[i][i2];
                  else
                    m_sinh = &CHILD(0)[i][i2];
                  b_found = true;
            ...
            if (!b_found) {
              ...
              break;
                     ...
                     (CHILD(0)[i].function()->id() == FUNCTION_ID_COSH ||
                      CHILD(0)[i].function()->id() == FUNCTION_ID_SINH) &&
                     CHILD(0)[i].size() == 1 &&
                     (!marg || marg->equals(CHILD(0)[i][0]))) {
            if (!marg)
              marg = &CHILD(0)[i][0];
            if (CHILD(0)[i].function()->id() == FUNCTION_ID_COSH)
              m_cosh = &CHILD(0)[i];
            else
              m_sinh = &CHILD(0)[i];
          } else {
    ...
    if (eo2.expand &&
        (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) &&
        CHILD(0).size() <= 10 && CHILD(0).size() > 1) {
      int i_root = 0;
      size_t root_index = 0;
      for (size_t i = 0; i < CHILD(0).size(); i++) {
        if (CALCULATOR->aborted())
        ...
        if (CHILD(0)[i].isPower()) {
          if (CHILD(0)[i][1].isNumber() && !CHILD(0)[i][1].isInteger() &&
              CHILD(0)[i][1].number().numeratorIsOne() &&
              CHILD(0)[i][1].number().denominatorIsLessThan(10)) {
            if (i_root) {
          ...
          } else if (!CHILD(0)[i][1].isNumber() ||
                     !CHILD(0)[i][1].number().isInteger()) {
            i_root = 0;
            break;
          }
        } else if (CHILD(0)[i].isFunction() &&
                   CHILD(0)[i].function()->id() == FUNCTION_ID_ROOT) {
          if (VALID_ROOT(CHILD(0)[i]) &&
              CHILD(0)[i][1].number().isLessThan(10)) {
            if (i_root) {
          ...
          for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
            if (CHILD(0)[i][i2].contains(x_var)) {
              if (CHILD(0)[i][i2].isPower()) {
                if (CHILD(0)[i][i2][1].isNumber() &&
                    !CHILD(0)[i][i2][1].number().isInteger() &&
                    CHILD(0)[i][i2][1].number().numeratorIsOne() &&
                    CHILD(0)[i][i2][1].number().denominatorIsLessThan(10)) {
                  if (i_root) {
                    if (i_root !=
                        CHILD(0)[i][i2][1].number().denominator().intValue()) {
                      i_root = 0;
                      b_break = true;
                      break;
                    }
                ...
                } else if (!CHILD(0)[i][i2][1].isNumber() ||
                           !CHILD(0)[i][i2][1].number().isInteger()) {
                  i_root = 0;
                  b_break = true;
                  break;
                }
              } else if (CHILD(0)[i][i2].isFunction() &&
                         CHILD(0)[i][i2].function()->id() == FUNCTION_ID_ROOT) {
                if (VALID_ROOT(CHILD(0)[i][i2]) &&
                    CHILD(0)[i][i2][1].number().isLessThan(10)) {
                  if (i_root) {
      ...
      if (i_root) {
        ...
        if (mtest[0].calculateRaise(i_root, eo2) &&
            mtest[1].calculateRaise(i_root, eo2)) {
          mtest.childrenUpdated();
          if (eo2.expand && morig) {
            MathStructure mtest2(mtest[0]);
            mtest2.calculateSubtract(mtest[1], eo2);
            if (mtest2.factorize(eo2, false, false, 0, false, false, NULL,
                                 m_undefined, false, false, 3) &&
                mtest2.isMultiplication()) {
          ...
          if (i_root && mtest.isolate_x(eo2, eo, x_var)) {
            if ((mtest.isLogicalAnd() || mtest.isLogicalOr() ||
                 mtest.isComparison()) &&
                test_comparisons(*this, mtest, x_var, eo, false,
                                 eo2.expand ? 1 : 2) < 0) {
              if (eo2.expand) {
                CALCULATOR->endTemporaryStopMessages(true);
                add(mtest, ct_comp == COMPARISON_EQUALS ? OPERATION_LOGICAL_AND
                                                        : OPERATION_LOGICAL_OR);
                calculatesub(eo2, eo, false);
                return true;
              }
    ...
    for (size_t i = 0; i < CHILD(0).size() && !b_multiple_div; i++) {
      ...
      if (CHILD(0)[i].isMultiplication()) {
        for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
          if (CHILD(0)[i][i2].isPower() && CHILD(0)[i][i2][1].isNumber() &&
              CHILD(0)[i][i2][1].number().isReal() &&
              CHILD(0)[i][i2][1].number().isNegative() &&
              CHILD(0)[i][i2][0].contains(x_var)) {
            if (!mdiv.isUndefined())
              b_multiple_div = true;
            else
            ...
            break;
                 ...
                 CHILD(0)[i][1].number().isReal() &&
                 CHILD(0)[i][1].number().isNegative()) {
        if (!mdiv.isUndefined())
          b_multiple_div = true;
        else
          mdiv = CHILD(0)[i];
    ...
    if (!mdiv.isUndefined() && countTotalChildren() > 200)
      mdiv.setUndefined();
    if (!mdiv.isUndefined() && b_multiple_div && mdiv.containsInterval()) {
      ...
      if (!failed && isolate_x_sub(eo, eo3, x_var, morig)) {
        ...
        if ((CALCULATOR->usesIntervalArithmetic() &&
            ...
            mbak.containsInterval())
          CALCULATOR->error(
              false,
              _("Interval arithmetic was disabled during calculation of %s."),
              format_and_print(mbak).c_str(), NULL);
        fix_intervals(*this, eo2);
        return true;
      }
        ...
        if (!b2 || !mdiv.containsInterval()) {
          if (CHILD(0)[i3].isPower() && CHILD(0)[i3].equals(mdiv, true)) {
            ...
            for (size_t i4 = 0; i4 < CHILD(0)[i3].size(); i4++) {
              if (CHILD(0)[i3][i4].isPower() &&
                  CHILD(0)[i3][i4].equals(mdiv, true)) {
                b = true;
                CHILD(0)[i3].delChild(i4 + 1);
                if (CHILD(0)[i3].size() == 0)
                  CHILD(0)[i3].set(1, 1, 0, true);
                else if (CHILD(0)[i3].size() == 1)
                  CHILD(0)[i3].setToChild(1, true);
                break;
              }
            }
          }
      ...
      if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
        ...
        CALCULATOR->beginTemporaryStopMessages();
        isolate_x(eo, eo2, x_var);
        if (!strict_check || eo2.expand ||
            ((m_type != STRUCT_COMPARISON || CHILD(0) == x_var ||
              !CHILD(0).isAddition() || CHILD(0).size() < 3) &&
             ((m_type != STRUCT_LOGICAL_AND && m_type != STRUCT_LOGICAL_OR) ||
              SIZE == 0 || !CHILD(0).isComparison() || CHILD(0)[0] == x_var ||
              !CHILD(0)[0].isAddition() || CHILD(0)[0].size() < 3))) {
          if (!mdiv[0].representsNonZero(true)) {
            MathStructure *mtest = new MathStructure(mdiv[0]);
            mtest->add(m_zero,
                       not_equals ? OPERATION_EQUALS : OPERATION_NOT_EQUALS);
            mtest->isolate_x_sub(eo, eo2, x_var);
            add_nocopy(mtest, not_equals ? OPERATION_LOGICAL_OR
                                         : OPERATION_LOGICAL_AND);
            calculatesub(eo2, eo, false);
          }
          CALCULATOR->endTemporaryStopMessages(true);
    ...
    for (size_t i = 0; i < CHILD(0).size(); i++) {
      ...
      if (!mbase.isZero() && CHILD(0)[i] == mbase) {
                 ...
                 CHILD(0)[i][1].number().isPositive() &&
                 CHILD(0)[i][1].number().isRational()) {
        if (CHILD(0)[i][0].isFunction() &&
            CHILD(0)[i][0].function()->id() == FUNCTION_ID_ROOT &&
            VALID_ROOT(CHILD(0)[i][0])) {
          if (mbase.isZero())
            mbase = CHILD(0)[i][0][0];
          else if (mbase != CHILD(0)[i][0][0]) {
            b = false;
            break;
          }
          if (!nr_root.isZero()) {
        ...
        for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
          if (!mbase.isZero() && CHILD(0)[i][i2] == mbase) {
          } else if (CHILD(0)[i][i2].isPower() &&
                     CHILD(0)[i][i2][1].isNumber() &&
                     CHILD(0)[i][i2][1].number().isPositive() &&
                     CHILD(0)[i][i2][1].number().isRational()) {
            if (CHILD(0)[i][i2][0].isFunction() &&
                CHILD(0)[i][i2][0].function()->id() == FUNCTION_ID_ROOT &&
                VALID_ROOT(CHILD(0)[i][i2][0])) {
              if (mbase.isZero())
                mbase = CHILD(0)[i][i2][0][0];
              else if (mbase != CHILD(0)[i][i2][0][0]) {
                b = false;
                break;
              }
              if (!nr_root.isZero()) {
              ...
              if (mbase.isZero())
                mbase = CHILD(0)[i][i2][0];
              else if (mbase != CHILD(0)[i][i2][0]) {
                b = false;
                break;
              }
            ...
            b_f_root = true;
          } else if (CHILD(0)[i][i2].isNumber() &&
                     !CHILD(0)[i][i2].number().isZero()) {
    ...
    if (b && !mbase.isZero() && nr_root.isLessThan(100)) {
      ...
      if (!nr_root.isZero()) {
        if (b_f_root) {
          ...
          mvar.set(CALCULATOR->getFunctionById(FUNCTION_ID_ROOT), &mbase,
                   &mroot, NULL);
        } else {
          ...
          if (CHILD(0)[i] == mbase) {
            ...
            for (size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
              if (CHILD(0)[i][i2] == mbase) {
                ...
                if (CHILD(0)[i][i2][0].isFunction() &&
                    CHILD(0)[i][i2][0].function()->id() == FUNCTION_ID_ROOT &&
                    CHILD(0)[i][i2][0] != mbase) {
                  CHILD(0)[i][i2][1].number().multiply(nr_root);
                  CHILD(0)
                  [i][i2][1].number().divide(CHILD(0)[i][i2][0][1].number());
                  if (CHILD(0)[i][i2][1].isOne())
                    CHILD(0)[i][i2] = var;
                  else
                    CHILD(0)[i][i2][0] = var;
    ...
    if (!morig || !equals(*morig)) {
      ...
      b = eo2.do_polynomial_division &&
          (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) &&
          do_simplification(mtest[0], eo, true, true, false, false, true);
      if (b && mtest[0].isMultiplication() && mtest[0].size() == 2 &&
          mtest[0][1].isPower() && mtest[0][1][1].representsNegative()) {
        ...
        if (mtest.isolate_x_sub(eo, eo2, x_var, this)) {
          mreq.setComparisonType(ct_comp == COMPARISON_EQUALS
                                     ? COMPARISON_NOT_EQUALS
                                     : COMPARISON_EQUALS);
          ...
          mtest.add(mreq, ct_comp == COMPARISON_EQUALS ? OPERATION_LOGICAL_AND
                                                       : OPERATION_LOGICAL_OR);
          mtest.calculatesub(eo2, eo, false);
          set_nocopy(mtest);
          return true;
        ...
        if (mtest[0].factorize(eo2, false, false, 0, false, false, NULL,
                               m_undefined, false, false, 3) &&
            !(mtest[0].isMultiplication() && mtest[0].size() == 2 &&
              ((mtest[0][0].isNumber() && mtest[0][1].isAddition()) ||
               mtest[0][0] == CHILD(1) || mtest[0][1] == CHILD(1)))) {
          mtest.childUpdated(1);
          if (mtest.isolate_x_sub(eo, eo2, x_var, this)) {
            set_nocopy(mtest);
            return true;
    ...
    if (CHILD(0).size() >= 2 &&
      ...
      for (size_t i = 0; i < CHILD(0).size(); i++) {
        if (CALCULATOR->aborted())
          return false;
        if (CHILD(0)[i].isPower() && CHILD(0)[i][0] == x_var &&
            CHILD(0)[i][1].isNumber() && CHILD(0)[i][1].number() == 3) {
          if (cbpow)
            mstruct_a.add(m_one, true);
          else {
            cbpow = true;
        ...
        } else if (CHILD(0)[i].isPower() && CHILD(0)[i][0] == x_var &&
                   CHILD(0)[i][1].isNumber() && CHILD(0)[i][1].number() == 2) {
          if (sqpow)
            mstruct_b.add(m_one, true);
          else {
            sqpow = true;
            ...
            if (CHILD(0)[i][i2] == x_var) {
              if (nopow) {
                ...
                mstruct_c.delChild(i2 + 1, true);
                       ...
                       CHILD(0)[i][i2][0] == x_var &&
                       CHILD(0)[i][i2][1].isNumber() &&
                       CHILD(0)[i][i2][1].number() == 2) {
              if (sqpow) {
                MathStructure *madd = new MathStructure(CHILD(0)[i]);
                madd->delChild(i2 + 1, true);
                mstruct_b.add_nocopy(madd, true);
      ...
      cube_eq_stop_iv:
        ...
        if (failed) {
          ...
          int b_zero = -1;
          int b_real = -1;
          if (mdelta.representsNonZero(true))
            b_zero = 0;
          else if (mdelta.representsZero(true))
            b_zero = 1;
          ...
          if (b_real < 0 && mdelta.representsComplex(true))
            b_real = -2;
          if (b_real == -1)
            b_zero = -1;

          if (b_real > 0 && (!mstruct_a.representsNonComplex() ||
                             !mstruct_b.representsNonComplex() ||
                             !mstruct_c.representsNonComplex() ||
                             !mstruct_d.representsNonComplex()))
            b_real = -2;
          ...
          if (b_zero >= 0) {
            ...
            if (mdelta0.representsNonZero(true))
              b0_zero = 0;
            else if (mdelta0.representsZero(true))
              b0_zero = 1;
            else {
              ...
              if (mdelta1.representsNegative(true)) {
                ...
                if (mdelta1.representsZero(true)) {
                  ...
                  if (stop_iv) {
                    CALCULATOR->endTemporaryStopIntervalArithmetic();
                    CALCULATOR->error(false,
                                      _("Interval arithmetic was disabled "
                                        "during calculation of %s."),
                                      format_and_print(mbak).c_str(), NULL);
                    fix_intervals(*this, eo2);
                  } else if (containsInterval(true, false, false, -2, false)) {
                    set(mbak);
          ...
          if (b_zero == 0 && b0_zero == 0) {
            ...
            if (b_real == 0 && x_var.representsReal(true)) {
              if (!mC.representsComplex(true)) {
                ...
                if (mC.calculateFunctions(eo))
                  mC.calculatesub(eo3, eo, true);
                // x = -1/(3a)*(b + C + delta0/C)
                CHILD(0) = x_var;
                CHILD(1).set(-1, 3, 0);
                CHILD(1).calculateDivide(mstruct_a, eo3);
              ...
              if (b_real == 1) {
                if ((*malt1)[1].isNumber()) {
                ...
                } else if ((*malt1)[1].isMultiplication() &&
                           (*malt1)[1][0].isNumber()) {
                  bool b = true;
                  ...
                  if (b)
                    (*malt1)[1][0].number().clearImaginary();
                }
                if ((*malt2)[1].isNumber()) {
                  (*malt2)[1].number().clearImaginary();
                } else if ((*malt2)[1].isMultiplication() &&
                           (*malt2)[1][0].isNumber()) {
                  bool b = true;
                  ...
                  if (b)
                    (*malt2)[1][0].number().clearImaginary();
                }
                if ((*malt3)[1].isNumber()) {
                  (*malt3)[1].number().clearImaginary();
                } else if ((*malt3)[1].isMultiplication() &&
                           (*malt3)[1][0].isNumber()) {
                  bool b = true;
                  ...
                  if (b)
                    (*malt3)[1][0].number().clearImaginary();
                }
              }

              if (b_real < 1 || !x_var.representsReal(true) ||
                  (!(*malt1)[1].representsComplex(true) &&
                   !(*malt2)[1].representsComplex(true) &&
                   !(*malt3)[1].representsComplex(true))) {
    ...
    if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
      ...
      if (m && m->function()->id() == FUNCTION_ID_ABS) {
        ...
        mcheck->add(m_zero, cmp_type == COMPARISON_NOT_EQUALS
                                ? OPERATION_EQUALS_GREATER
                                : OPERATION_LESS);
        mcheck->isolate_x_sub(eo, eo2, x_var);
        malt->isolate_x_sub(eo, eo2, x_var);
        malt->add_nocopy(mcheck,
                         cmp_type == COMPARISON_NOT_EQUALS
                             ? OPERATION_LOGICAL_OR
                             : OPERATION_LOGICAL_AND,
                         true);
        malt->calculatesub(eo2, eo, false);
        ...
        add_nocopy(mcheck,
                   cmp_type == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_OR
                                                     : OPERATION_LOGICAL_AND,
                   true);
        calculatesub(eo2, eo, false);
        add_nocopy(malt,
                   cmp_type == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_AND
                                                     : OPERATION_LOGICAL_OR,
                   true);
        calculatesub(eo2, eo, false);
        ...
        mcheck->add(m_zero,
                    cmp_type == COMPARISON_NOT_EQUALS
                        ? (mabs[1].isMinusOne() ? OPERATION_GREATER
                                                : OPERATION_EQUALS_GREATER)
                        : (mabs[1].isMinusOne() ? OPERATION_EQUALS_LESS
                                                : OPERATION_LESS));
        mcheck->isolate_x_sub(eo, eo2, x_var);
        malt->isolate_x_sub(eo, eo2, x_var);
        malt->add_nocopy(mcheck,
        ...
        CHILD_UPDATED(0)
        mcheck->add(m_zero, cmp_type == COMPARISON_NOT_EQUALS
                                ? (mabs[1].isOne() ? OPERATION_LESS
                                                   : OPERATION_EQUALS_LESS)
                                : (mabs[1].isOne() ? OPERATION_EQUALS_GREATER
                                                   : OPERATION_GREATER));
        mcheck->isolate_x_sub(eo, eo2, x_var);
        isolate_x_sub(eo, eo2, x_var);
        add_nocopy(mcheck,
    ...
    if ((ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) &&
        CHILD(1).isNumber() && eo.approximation != APPROXIMATION_EXACT &&
        !x_var.representsComplex(true)) {
      ...
      if (failed) {
        ...
        while ((ret = newton_raphson(mtest, x_value, x_var, eo3)) > 0) {
          if (isNumber()) {
            ...
            if (isComparison())
              transform(mbak.comparisonType() == COMPARISON_NOT_EQUALS
                            ? STRUCT_LOGICAL_AND
                            : STRUCT_LOGICAL_OR);
          ...
          MathStructure mrem;
          if (!polynomial_long_division(mtestcopy, mdiv, x_var, mtest, mrem,
                                        eo3, false, true) ||
              !mrem.isNumber()) {
            ret = -1;
            break;
          }
          if (!mtest.contains(x_var))
            break;
        }
        CALCULATOR->endTemporaryStopIntervalArithmetic();
        if (ret < 0) {
          ...
          if (!x_var.representsReal())
            CALCULATOR->error(
                false, _("Not all complex roots were calculated for %s."),
                format_and_print(mbak).c_str(), NULL);
          if ((CALCULATOR->usesIntervalArithmetic() &&
               eo.approximation != APPROXIMATION_EXACT) ||
              mbak.containsInterval())
    ...
    if ((!morig || !equals(*morig)) && !CHILD(1).isZero()) {
      ...
      if (mtest[0].factorize(eo2, false, false, 0, false, false, NULL,
                             m_undefined, false, false, 3) &&
          !(mtest[0].isMultiplication() && mtest[0].size() == 2 &&
            (mtest[0][0].isNumber() || mtest[0][0] == CHILD(1) ||
             mtest[0][1] == CHILD(1)))) {
        mtest.childUpdated(1);
        if (mtest.isolate_x_sub(eo, eo2, x_var, this)) {
          set_nocopy(mtest);
  ...
  case STRUCT_MULTIPLICATION: {
      ...
      if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
        ...
        for (size_t i = 0; i < CHILD(0).size(); i++) {
          if (CALCULATOR->aborted())
            return false;
          if (CHILD(0)[i].isPower() &&
              !CHILD(0)[i][1].representsNonNegative()) {
        ...
        for (size_t i = 0; i < SIZE;) {
          ...
          if (checktype[i] > 0) {
            ...
            if (checktype[i] == 2) {
              ...
              if (ct_comp == COMPARISON_NOT_EQUALS)
                mexpcheck->add(m_zero, OPERATION_LESS);
              else
                mexpcheck->add(m_zero, OPERATION_EQUALS_GREATER);
              mexpcheck->isolate_x_sub(eo, eo2, x_var);
              if (ct_comp == COMPARISON_NOT_EQUALS)
                mexpcheck->add_nocopy(mcheck, OPERATION_LOGICAL_AND, true);
              else
                mexpcheck->add_nocopy(mcheck, OPERATION_LOGICAL_OR, true);
              mexpcheck->calculatesub(eo2, eo, false);
              mcheck = mexpcheck;
            }
            if (mcheckpowers) {
              if (ct_comp == COMPARISON_NOT_EQUALS)
                mcheckpowers->add_nocopy(mcheck, OPERATION_LOGICAL_OR, true);
              else
                mcheckpowers->add_nocopy(mcheck, OPERATION_LOGICAL_AND, true);
            } else {
              mcheckpowers = mcheck;
            }
          }
        ...
        if (mcheckpowers) {
          ...
          if (ct_comp == COMPARISON_NOT_EQUALS)
            add_nocopy(mcheckpowers, OPERATION_LOGICAL_OR, true);
          else
            add_nocopy(mcheckpowers, OPERATION_LOGICAL_AND, true);
        ...
        if (checktype1 != 1 && (ct_comp == COMPARISON_EQUALS_LESS ||
                                ct_comp == COMPARISON_EQUALS_GREATER)) {
        ...
        if (checktype2 != 1 && (ct_comp == COMPARISON_EQUALS_LESS ||
                                ct_comp == COMPARISON_EQUALS_GREATER)) {
    ...
    for (size_t i = 0; i < CHILD(0).size(); i++) {
      if (CALCULATOR->aborted())
        return false;
      if (CHILD(0)[i].isPower() && CHILD(0)[i][1].isNumber() &&
          CHILD(0)[i][1].number().isNegative() &&
          CHILD(0)[i][1].number().isReal()) {
        MathStructure *mtest;
        ...
        if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
          isolate_x_sub(eo, eo2, x_var, morig);
          mtest->add(m_zero, cmp_type == COMPARISON_NOT_EQUALS
                                 ? OPERATION_EQUALS
                                 : OPERATION_NOT_EQUALS);
          mtest->isolate_x_sub(eo, eo2, x_var);
          add_nocopy(mtest,
    ...
    if (CHILD(1).isZero())
      ...
      zero1 = 2;
    MathStructure *mcheckmulti = NULL, *mtryzero = NULL, *mchecknegative = NULL,
    ...
    for (size_t i = 0; i < CHILD(0).size(); i++) {
      ...
      if (!CHILD(0)[i].contains(x_var)) {
        bool nonzero = false;
        if (CHILD(0)[i].isPower() &&
            !CHILD(0)[i][1].representsNonNegative(true) &&
            !CHILD(0)[i][0].representsNonZero(true)) {
          ...
          if (CHILD(0)[i][1].representsNegative(true)) {
            ...
            if (ct_comp == COMPARISON_NOT_EQUALS)
              mcheckor->add(m_zero, OPERATION_EQUALS_LESS);
            else
              mcheckor->add(m_zero, OPERATION_EQUALS_GREATER);
            mcheckor->isolate_x(eo2, eo);
            if (ct_comp == COMPARISON_NOT_EQUALS)
              mcheck->add_nocopy(mcheckor, OPERATION_LOGICAL_AND);
            else
              mcheck->add_nocopy(mcheckor, OPERATION_LOGICAL_OR);
            mcheck->calculatesub(eo2, eo, false);
          }
          if (mchecknonzeropow) {
            if (ct_comp == COMPARISON_NOT_EQUALS)
              mchecknonzeropow->add_nocopy(mcheck, OPERATION_LOGICAL_OR);
            else
              mchecknonzeropow->add_nocopy(mcheck, OPERATION_LOGICAL_AND);
          } else {
            mchecknonzeropow = mcheck;
          }
        }
        if (!nonzero && !CHILD(0)[i].representsNonZero(true)) {
          if (zero1 != 1 && (ct_comp == COMPARISON_EQUALS ||
                             ct_comp == COMPARISON_NOT_EQUALS)) {
            ...
            if (mcheckmulti) {
              if (ct_comp == COMPARISON_NOT_EQUALS)
                mcheckmulti->add_nocopy(mcheck, OPERATION_LOGICAL_OR);
              else
                mcheckmulti->add_nocopy(mcheck, OPERATION_LOGICAL_AND);
            } else {
              mcheckmulti = mcheck;
            }
          }
          if (ct_comp != COMPARISON_EQUALS &&
            ...
            if (!mtryzero) {
              ...
              case COMPARISON_EQUALS_GREATER: {
                mcheck->add(m_zero, OPERATION_EQUALS_LESS);
                break;
              }
              default: {
              }
            ...
            if (zero1 == 2) {
              ...
              if (ct_comp == COMPARISON_NOT_EQUALS)
                mcheck2->add(m_zero, OPERATION_NOT_EQUALS);
              else
                mcheck2->add(m_zero, OPERATION_EQUALS);
              mcheck2->isolate_x(eo2, eo);
              if (ct_comp == COMPARISON_NOT_EQUALS)
                mcheck2->add_nocopy(mcheck, OPERATION_LOGICAL_OR);
              else
                mcheck2->add_nocopy(mcheck, OPERATION_LOGICAL_AND);
              mcheck2->calculatesub(eo2, eo, false);
              mcheck = mcheck2;
            }
            if (mtryzero) {
              if (ct_comp == COMPARISON_NOT_EQUALS)
                ...
                mtryzero->add_nocopy(mcheck, OPERATION_LOGICAL_OR);
            } else {
              mtryzero = mcheck;
            }
          }
    ...
    if (combine_powers(*this, x_var, eo2, eo) &&
        m_type != STRUCT_MULTIPLICATION)
      return isolate_x_sub(eo, eo2, x_var, morig);
    if (!b &&
        (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) &&
        CHILD(0).size() >= 2) {
      ...
      MathStructure m_c;
      for (size_t i = 0; i < CHILD(0).size(); i++) {
        if (CHILD(0)[i].isPower() && CHILD(0)[i][1].contains(x_var) &&
            !CHILD(0)[i][0].contains(x_var)) {
      ...
      if (mvar) {
        ...
        if (found_mvar) {
          MathStructure mln(CALCULATOR->getFunctionById(FUNCTION_ID_LOG),
                            &CHILD(0)[i_px][0], NULL);
          if (mln.calculateFunctions(eo))
            mln.calculatesub(eo2, eo, true);
          MathStructure *marg = NULL;
          if (mln.representsNonZero()) {
            marg = new MathStructure(mln);
            if (comparison_might_be_equal(m_c.compare(m_zero))) {
              marg->unref();
              marg = NULL;
            } else if (!m_c.isOne())
              marg->calculateMultiply(m_c, eo2);
          ...
          if (marg) {
            ...
            if (!mmul.isOne())
              marg->calculateDivide(mmul, eo2);
          }
          if (marg && !m_a.isZero()) {
            marg->multiply(CHILD(0)[i_px][0]);
            marg->last().raise(m_a);
            if (!m_c.isOne())
              marg->last().last().calculateMultiply(m_c, eo2);
            marg->last().calculateRaiseExponent(eo2);
            marg->calculateMultiplyLast(eo2);
          }
          if (marg && mvar->representsNonComplex() &&
              CHILD(0)[i_px][0].representsPositive()) {
            if (mcheckmulti)
              mcheckmulti->unref();
            if (mtryzero)
              mcheckmulti->unref();
            ...
            if (!marg2 && !marg->representsNonNegative()) {
              ...
              if (marg2->calculateFunctions(eo))
                marg2->calculatesub(eo2, eo, true);
              if (!m_a.isZero())
                marg2->calculateSubtract(m_a, eo2);
              if (!m_c.isOne())
                marg2->calculateDivide(m_c, eo2);
      ...
      if (CHILD(0).size() == 2) {
        ...
        bool b_swap = false;
        if (CHILD(0)[0].isFunction() &&
            CHILD(0)[0].function()->id() == FUNCTION_ID_LOG &&
            CHILD(0)[0].size() == 1) {
          CHILD(0).swapChildren(1, 2);
          b_swap = true;
        }
        if (CHILD(0)[1].isFunction() &&
            CHILD(0)[1].function()->id() == FUNCTION_ID_LOG &&
            CHILD(0)[1].size() == 1) {
          MathStructure mmul(1, 1, 0), mexp(1, 1, 0);
          mvar = NULL;
          if (CHILD(0)[0] == CHILD(0)[1][0]) {
            mvar = &CHILD(0)[0];
          } else if (CHILD(0)[0].isPower() &&
                     CHILD(0)[0][0] == CHILD(0)[1][0]) {
            mexp = CHILD(0)[0][1];
            mvar = &CHILD(0)[0][0];
          } else {
            if (get_multiplier(CHILD(0)[1][0], CHILD(0)[0], mmul)) {
              mvar = &CHILD(0)[0];
                       ...
                       get_multiplier(CHILD(0)[1][0], CHILD(0)[0][0], mmul)) {
              mexp = CHILD(0)[0][1];
              mvar = &CHILD(0)[0][0];
            }
          }
          if (mvar && mexp.isInteger() &&
              (mexp.number().isOne() || mexp.number().isTwo()) &&
              mmul.representsNonZero() && !mmul.contains(x_var)) {
            if (mcheckmulti)
              mcheckmulti->unref();
            if (mtryzero)
              mcheckmulti->unref();
            ...
            if (mexp.isOne() || mmul.isOne()) {
              ...
              if (!mmul.isOne())
                marg->calculateMultiply(mmul, eo2);
              else if (!mexp.isOne())
            ...
            if (mvar->representsNonComplex()) {
              ...
              if (!marg->representsPositive()) {
                ...
                if (marg2->calculateFunctions(eo))
                  marg2->calculatesub(eo2, eo, true);
                if (!mexp.isOne())
                  marg2->calculateDivide(mexp, eo2);
                marg2->transform(STRUCT_POWER);
                marg2->insertChild(CALCULATOR->getVariableById(VARIABLE_ID_E),
                                   1);
                (*marg2)[0].calculatesub(eo2, eo, true);
                marg2->calculateRaiseExponent(eo2);
                if (!mmul.isOne())
                  marg2->calculateDivide(mmul, eo2);
                mreq1->transform(ct_comp == COMPARISON_NOT_EQUALS
                                     ? COMPARISON_LESS
                                     : COMPARISON_EQUALS_GREATER,
                                 CALCULATOR->getVariableById(VARIABLE_ID_E));
              ...
              if (!mmul.isOne())
                CHILD(1).calculateDivide(mmul, eo2);
              CHILDREN_UPDATED
              isolate_x_sub(eo, eo2, x_var, morig);
              if (mreq1) {
                add_nocopy(mreq1, ct_comp == COMPARISON_NOT_EQUALS
              ...
              if (marg3->calculateFunctions(eo))
                marg3->calculatesub(eo2, eo, true);
              if (!mexp.isOne())
                marg3->calculateDivide(mexp, eo2);
              ...
              if (!mmul.isOne())
                marg3->calculateDivide(mmul, eo2);
              marg2->transform(ct_comp, *mvar);
              marg2->swapChildren(1, 2);
              marg2->isolate_x_sub(eo, eo2, x_var, morig);
              ...
              add_nocopy(marg2, ct_comp == COMPARISON_EQUALS
                                    ...
                                    : OPERATION_LOGICAL_AND);
              add_nocopy(marg3,
                         ct_comp == COMPARISON_EQUALS ? OPERATION_LOGICAL_OR
                                                      : OPERATION_LOGICAL_AND,
                         true);
              calculatesub(eo2, eo, false);
            }
    ...
    if (b) {
      ...
      if (mchecknegative) {
        ...
        switch (ct_comp) {
        case COMPARISON_LESS: {
          mneg->setComparisonType(COMPARISON_GREATER);
          break;
        }
        case COMPARISON_GREATER: {
          mneg->setComparisonType(COMPARISON_LESS);
          break;
        }
        case COMPARISON_EQUALS_LESS: {
          mneg->setComparisonType(COMPARISON_EQUALS_GREATER);
          break;
        }
        case COMPARISON_EQUALS_GREATER: {
          mneg->setComparisonType(COMPARISON_EQUALS_LESS);
          break;
        }
        default: {
        }
    ...
    if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
      ...
      for (size_t i = 0; i < CHILD(0).size(); i++) {
        ...
        if (CHILD(0)[i].isPower() && CHILD(0)[i][1].isNumber() &&
          ...
          if (!CHILD(0)[i][1].number().isInteger()) {
            if (nrexp.isZero())
              nrexp = CHILD(0)[i][1].number().denominator();
            else
              nrexp.lcm(CHILD(0)[i][1].number().denominator());
          }
        } else if (CHILD(0)[i].isFunction() &&
                   CHILD(0)[i].function()->id() == FUNCTION_ID_ROOT &&
                   VALID_ROOT(CHILD(0)[i])) {
          if (nrexp.isZero())
            nrexp = CHILD(0)[i][1].number();
          else
            nrexp.lcm(CHILD(0)[i][1].number());
        } else if (CHILD(0)[i] != x_var &&
                   !(CHILD(0)[i].isFunction() &&
                     CHILD(0)[i].function()->id() == FUNCTION_ID_ABS &&
                     CHILD(0)[i].size() == 1 &&
                     CHILD(0)[i][0].representsReal(true))) {
          nrexp.clear();
          break;
        }
      }
    ...
    if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
      ...
      if (m && m->function()->id() == FUNCTION_ID_ABS) {
        ...
        if (!mabs[1].isOne() && !mabs[1].isMinusOne()) {
          ...
          (*malt0)[0].calculatesub(eo2, eo, true);
          mcheck = new MathStructure(mabs[0]);
          mcheck->add(mabs[1], cmp_type == COMPARISON_NOT_EQUALS
                                   ? OPERATION_NOT_EQUALS
                                   : OPERATION_EQUALS);
          mcheck->isolate_x_sub(eo, eo2, x_var);
          malt0->isolate_x_sub(eo, eo2, x_var);
  ...
  case STRUCT_POWER: {
    if (CHILD(0)[0].contains(x_var)) {
      if (CHILD(0)[1].contains(x_var)) {
        if ((ct_comp == COMPARISON_EQUALS ||
          ...
          if (!mvar || !get_multiplier(CHILD(0)[1], *mvar, mmul) ||
              mmul.contains(x_var) ||
              comparison_might_be_equal(mmul.compare(m_zero)))
            return false;
          MathStructure mexp(1, 1, 0);
          if (mvar->isPower() && *mvar != CHILD(0)[0])
            mexp = (*mvar)[1];
          if (mmul.isOne() && mexp.isOne() && CHILD(1).isNumber() &&
              CHILD(1).number().isRational() &&
              CHILD(0)[0].representsNonComplex()) {
            ...
            if (CHILD(1).number().isInteger()) {
              if (CHILD(1).number().isOne()) {
                ...
                if (nr.ln() && nr.lambertW() && nr.exp() && nr.round()) {
                  ...
                  if (!CALCULATOR->endTemporaryStopMessages() &&
                      !nrtest.isApproximate() && CHILD(1).number() == nrtest) {
                    CHILD(1).set(nr, true);
                    CHILD(0).setToChild(1, true);
                    return true;
                  }
              ...
              if (nr_den == 4) {
                ...
                CALCULATOR->beginTemporaryStopMessages();
                // n=-round(e^(lambertw(ln(b))))
                if (nr_den.ln() && nr_den.lambertW() && nr_den.exp() &&
                    nr_den.round() && nr_den.negate()) {
              ...
              if (nr_den == 27) {
                ...
                // n=-round(e^(lambertw(ln(b))))
                if (nr_den.ln() && nr_den.lambertW() && nr_den.exp() &&
                    nr_den.negate() && nr_den.round()) {
                  nr_den.setApproximate(false);
                  Number nrtest(nr_den);
                  nrtest.raise(nr_den, true);
                  // if b=n^n, x=n
                  if (!CALCULATOR->endTemporaryStopMessages() &&
                      !nrtest.isApproximate()) {
          ...
          } else if (mmul.isOne() && mexp.isMinusOne() &&
                     CHILD(0)[0].representsNonComplex() &&
                     (CHILD(1).isOne() || CHILD(1).isMinusOne())) {
            CHILD(0).setToChild(1, true);
            return true;
          }
          // if(!mexp.representsPositive()) return false;
          MathStructure *marg = new MathStructure(
              CALCULATOR->getFunctionById(FUNCTION_ID_LOG), &CHILD(1), NULL);
          if (marg->calculateFunctions(eo))
            marg->calculatesub(eo2, eo, true);
          if (CHILD(0)[0].representsNonComplex() &&
              CHILD(1).compare(m_zero) == COMPARISON_RESULT_LESS) {
            // x is real, b > 0:
            ...
            // x=e^lambertw(ln(b)+2i*pi*n/a) || x=e^lambertw(ln(b)+2i*pi*n/a,
            // -1) || x=e^lambertw(ln(b)+2i*pi*n/a, 1)
            marg->add(nr_one_i);
            marg->last().multiply(nr_two);
            marg->last().multiply(CALCULATOR->getVariableById(VARIABLE_ID_PI),
                                  true);
            marg->last().multiply(CALCULATOR->getVariableById(VARIABLE_ID_N),
                                  true);
            marg->evalSort(true);
            MathStructure *mreq2 = NULL;
            ...
            if (!CHILD(1).isNumber() || !CHILD(1).number().isFraction()) {
              mreq1 = new MathStructure(*marg);
              mreq1->transform(ct_comp == COMPARISON_EQUALS
                                   ? COMPARISON_NOT_EQUALS
                                   : COMPARISON_EQUALS,
                               m_zero);
              mreq1->isolate_x(eo2, eo);
            ...
            mreq3a->transform(ct_comp == COMPARISON_EQUALS
                                  ? COMPARISON_GREATER
                                  : COMPARISON_EQUALS_LESS,
                              CALCULATOR->getVariableById(VARIABLE_ID_PI));
            mreq3a->last().negate();
            ...
            mreq3b->transform(ct_comp == COMPARISON_NOT_EQUALS
                                  ? COMPARISON_GREATER
                                  : COMPARISON_EQUALS_LESS,
                              CALCULATOR->getVariableById(VARIABLE_ID_PI));
            ...
            marg2->isolate_x_sub(eo, eo2, x_var, morig);
            marg2->add(*mreq3a,
                       ct_comp == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_OR
                                                        : OPERATION_LOGICAL_AND,
                       true);
            if (mreq1)
              marg2->add(*mreq1,
        ...
        if ((ct_comp == COMPARISON_EQUALS ||
          ...
          UnknownVariable *var = new UnknownVariable(
              "", string(LEFT_PARENTHESIS) + format_and_print(mvar) +
                      RIGHT_PARENTHESIS);
          var->setInterval(mvar);
        ...
        if (b_neg && CHILD(1).isZero()) {
          if (ct_comp == COMPARISON_EQUALS ||
            ...
            isolate_x_sub(eo, eo2, x_var, morig);
            malt->isolate_x_sub(eo, eo2, x_var, morig);
            add_nocopy(malt, ct_comp == COMPARISON_NOT_EQUALS
                                 ? OPERATION_LOGICAL_AND
                                 : OPERATION_LOGICAL_OR);
          ...
          return true;
        } else if (b_neg && ct_comp != COMPARISON_EQUALS &&
                   ct_comp != COMPARISON_NOT_EQUALS) {
        ...
        if (CHILD(0)[1].number().isMinusOne()) {
          ...
          if (CHILD(0)[1].number().isEven()) {
            if (!CHILD(1).representsNonNegative(true)) {
              ...
              if (b_real && (CHILD(1).representsNegative(true) ||
                             CHILD(1).representsComplex(true))) {
                if (ct_comp == COMPARISON_EQUALS) {
                  clear(true);
          ...
          if (CHILD(0)[1].number().isTwo()) {
            ...
            if (b_neg)
              CHILD(1)[1].number().negate();
            CHILD(1).calculateRaiseExponent(eo2);
            CHILDREN_UPDATED
          } else if (!b_real && CHILD(0)[1].number() == 4 &&
                     (ct_comp == COMPARISON_EQUALS ||
                      ct_comp == COMPARISON_NOT_EQUALS)) {
            CHILD(1).raise(CHILD(0)[1].number());
            CHILD(1)[1].number().recip();
            ...
            isolate_x_sub(eo, eo2, x_var, morig);
            add_nocopy(malt1, ct_comp == COMPARISON_NOT_EQUALS ||
                                      ct_comp == COMPARISON_LESS ||
                                      ct_comp == COMPARISON_EQUALS_LESS
                                  ? OPERATION_LOGICAL_AND
                                  : OPERATION_LOGICAL_OR);
            ...
            calculatesub(eo2, eo, false);
            b_set = true;
          } else if (!b_real && (ct_comp == COMPARISON_EQUALS ||
                                 ct_comp == COMPARISON_NOT_EQUALS)) {
            if (CHILD(0)[1].number() > 20)
              return false;
            ...
            if (CHILD(1).representsNegative(true)) {
              ...
              if (CHILD(1).isNumber() && !CHILD(1).number().hasRealPart()) {
                if (CHILD(1).number().imaginaryPartIsNegative())
                  marg_pi.set(-1, 2, 0);
                else
                  marg_pi.set(1, 2, 0);
              } else {
                marg.set(CALCULATOR->getFunctionById(FUNCTION_ID_ARG),
                         &CHILD(1), NULL);
            ...
            minv.number().recip();
            MathStructure mmul(CALCULATOR->getFunctionById(FUNCTION_ID_ABS),
                               &CHILD(1), NULL);
            ...
            while (nr_i.isLessThan(mdeg.number())) {
              ...
              if (!marg_pi.isZero()) {
                if (nexp.isZero())
                  nexp = marg_pi;
                else if (!nexp.add(marg_pi)) {
                  set(mbak);
                  return false;
                }
                if (!nexp.divide(mdeg.number())) {
                  set(mbak);
                  return false;
                }
                if (nexp.isInteger()) {
              ...
              if (b_neg)
                mroot.calculateRaise(m_minus_one, eo2);
              if (nr_i.isZero()) {
                CHILD(0).setToChild(1);
                CHILD(1) = mroot;
                isolate_x_sub(eo, eo2, x_var, morig);
                ...
                malt->add(mroot, mbak.comparisonType() == COMPARISON_NOT_EQUALS
                                     ? OPERATION_NOT_EQUALS
                                     : OPERATION_EQUALS);
                malt->isolate_x_sub(eo, eo2, x_var, morig);
                add_nocopy(malt,
                           mbak.comparisonType() == COMPARISON_NOT_EQUALS
                               ? OPERATION_LOGICAL_AND
                               : OPERATION_LOGICAL_OR,
                           true);
              }
            ...
            if (CHILD(1).calculateFunctions(eo))
          ...
          if (!b_set) {
            if (CHILD(0)[1].number().isEven()) {
              ...
              mneg->isolate_x_sub(eo, eo2, x_var, morig);
              isolate_x_sub(eo, eo2, x_var, morig);
              add_nocopy(mneg, ct_comp == COMPARISON_NOT_EQUALS ||
                                       ct_comp == COMPARISON_LESS ||
                                       ct_comp == COMPARISON_EQUALS_LESS
                                   ? OPERATION_LOGICAL_AND
                                   : OPERATION_LOGICAL_OR);
          ...
          if (warn_complex)
            CALCULATOR->error(
                false,
                _("Only one or two of the roots were calculated for %s."),
                format_and_print(mbak).c_str(), NULL);
        } else {
          ...
          if (!CHILD(1).representsNonNegative(true)) {
            if (ct_comp != COMPARISON_EQUALS &&
              ...
              if (CHILD(1).representsNonComplex(true)) {
                ...
                mposcheck->add(m_zero, ct_comp == COMPARISON_NOT_EQUALS
                                           ? OPERATION_LESS
                                           : OPERATION_EQUALS_GREATER);
                mposcheck->isolate_x_sub(eo, eo2, x_var);
              } else {
          ...
          if (b_test) {
            ...
            add_nocopy(mposcheck, ct_comp == COMPARISON_NOT_EQUALS
                                      ? OPERATION_LOGICAL_OR
                                      : OPERATION_LOGICAL_AND);
            calculatesub(eo2, eo, false);
          }
        ...
        if (b_neg && !b_nonzero) {
          ...
          mtest->add(m_zero, (mbak.comparisonType() == COMPARISON_NOT_EQUALS)
                                 ? OPERATION_EQUALS
                                 : OPERATION_NOT_EQUALS);
          mtest->isolate_x_sub(eo, eo2, x_var);
          add_nocopy(mtest, (mbak.comparisonType() == COMPARISON_NOT_EQUALS)
                                ? OPERATION_LOGICAL_OR
                                : OPERATION_LOGICAL_AND);
          calculatesub(eo2, eo, false);
          if (mbak.comparisonType() != COMPARISON_NOT_EQUALS &&
              mbak.comparisonType() != COMPARISON_EQUALS) {
            ...
            if (mbak[0][1].representsInteger() && mbak[0][1].representsEven()) {
              ...
              malt->add(m_zero, (ct_comp == COMPARISON_EQUALS_GREATER ||
                                 ct_comp == COMPARISON_GREATER)
                                    ? OPERATION_LESS
                                    : OPERATION_GREATER);
            }
    ...
    } else if (CHILD(0)[1].contains(x_var) && CHILD(0)[0].representsNumber() &&
               (!CHILD(0)[1].representsNonComplex() ||
                (CHILD(0)[0].representsNonNegative() ||
                 (CHILD(0)[0].isNumber() && CHILD(0)[0].number().isNegative() &&
                  CHILD(1).isNumber())))) {
      // a^x=b => x=log(b, a)
      ...
      if ((ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) &&
        ...
        if (!CHILD(1).representsNonZero()) {
          ...
          if (!m0->representsNonComplex(true)) {
            ...
            if (m0->calculateFunctions(eo))
              m0->calculatesub(eo2, eo, true);
          }
          m0->transform(ct_comp == COMPARISON_EQUALS ? COMPARISON_GREATER
                                                     : COMPARISON_EQUALS_LESS,
                        m_zero);
          m0->isolate_x_sub(eo, eo2, x_var, morig);
          if (mtest2) {
            m0->add_nocopy(mtest2, ct_comp == COMPARISON_EQUALS
                                       ? OPERATION_LOGICAL_AND
                                       : OPERATION_LOGICAL_OR);
          ...
          m0->add_nocopy(mtest3, ct_comp == COMPARISON_EQUALS
                                     ? OPERATION_LOGICAL_AND
                                     : OPERATION_LOGICAL_OR);
          m0->calculatesub(eo2, eo, false);
        }
        mtest = new MathStructure(CHILD(0)[1]);
        mtest->transform(ct_comp == COMPARISON_EQUALS ? COMPARISON_NOT_EQUALS
                                                      : COMPARISON_EQUALS,
                         m_zero);
        mtest->isolate_x_sub(eo, eo2, x_var, morig);
        if (!CHILD(0)[0].isZero()) {
          MathStructure *mtest_b = new MathStructure(CHILD(0)[0]);
          mtest_b->transform(ct_comp == COMPARISON_EQUALS
                                 ? COMPARISON_NOT_EQUALS
                                 : COMPARISON_EQUALS,
                             m_zero);
          mtest_b->isolate_x_sub(eo, eo2, x_var, morig);
          mtest->add_nocopy(mtest_b, ct_comp == COMPARISON_EQUALS
                                         ? OPERATION_LOGICAL_OR
                                         : OPERATION_LOGICAL_AND);
          mtest->calculatesub(eo2, eo, false);
        }
      ...
      if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) {
        ...
        if (cr1 == COMPARISON_RESULT_EQUAL) {
          ...
          if (cr2 == COMPARISON_RESULT_EQUAL) {
            ...
            m1->add_nocopy(m1b, ct_comp == COMPARISON_EQUALS
                                    ? OPERATION_LOGICAL_OR
                                    : OPERATION_LOGICAL_AND);
      ...
      if ((ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS)) {
        if (CHILD(0)[0].isMinusOne()) {
          ...
          isolate_x_sub(eo, eo2, x_var, morig);
          if (m1) {
            add_nocopy(m1, ct == COMPARISON_EQUALS ? OPERATION_LOGICAL_OR
                                                   : OPERATION_LOGICAL_AND);
            calculatesub(eo2, eo, false);
          }
          if (mtest) {
            add_nocopy(mtest, ct == COMPARISON_NOT_EQUALS
                                  ? OPERATION_LOGICAL_OR
                                  : OPERATION_LOGICAL_AND);
            calculatesub(eo2, eo, false);
          }
      ...
      if (ct_comp != COMPARISON_EQUALS && ct_comp != COMPARISON_NOT_EQUALS) {
        if (CHILD(0)[0].isNumber() && CHILD(0)[0].number().isReal() &&
          ...
          switch (ct_comp) {
          case COMPARISON_LESS: {
            b_gz = CHILD(1).representsPositive();
            b_clear = !b_gz && CHILD(1).representsNonPositive();
            break;
          }
          case COMPARISON_GREATER: {
            b_gz = CHILD(1).representsNegative();
            b_clear = !b_gz && CHILD(1).representsNonNegative();
            break;
          }
          case COMPARISON_EQUALS_LESS: {
            b_gz = CHILD(1).representsNonNegative();
            b_clear = !b_gz && CHILD(1).representsNegative();
            break;
          }
          case COMPARISON_EQUALS_GREATER: {
            b_gz = CHILD(1).representsNonPositive();
            b_clear = !b_gz && CHILD(1).representsPositive();
            break;
          }
          default: {
          }
          ...
          if (b_clear) {
            ...
            if (!CHILD(0).representsNonComplex(true)) {
              ...
              if (CHILD(0).calculateFunctions(eo))
                CHILD(0).calculatesub(eo2, eo, true);
            }
            CHILDREN_UPDATED
            isolate_x_sub(eo, eo2, x_var, morig);
      ...
      if (CHILD(0)[1].representsNonComplex()) {
        if (CHILD(0)[0].representsNegative()) {
          MathStructure mtest2(CALCULATOR->getFunctionById(FUNCTION_ID_LOGN),
                               &msave, &CHILD(0)[0], NULL);
          mtest2[1].calculateNegate(eo2);
          mtest2.childUpdated(2);
          if (mtest2.calculateFunctions(eo))
            mtest2.calculatesub(eo2, eo, true);
          if (mtest)
            mtest->unref();
          if (m0)
            m0->unref();
          ...
          CHILD(1).set(CALCULATOR->getFunctionById(FUNCTION_ID_LOGN), &msave,
                       &CHILD(0)[0], NULL);
        ...
        CHILD(1).set(CALCULATOR->getFunctionById(FUNCTION_ID_LOG), &msave,
                     NULL);
        if (!msave.isZero()) {
          CHILD(1) += nr_one_i;
          CHILD(1)[1] *= nr_two;
          CHILD(1)
          [1].multiply(CALCULATOR->getVariableById(VARIABLE_ID_PI), true);
          CHILD(1)
          [1].multiply(CALCULATOR->getVariableById(VARIABLE_ID_N), true);
        }
        CHILD(1).divide_nocopy(new MathStructure(
            CALCULATOR->getFunctionById(FUNCTION_ID_LOG), &CHILD(0)[0], NULL));
      ...
      if (COMPARISON_MIGHT_BE_EQUAL(cr1)) {
        ...
        mn1->setComparisonType(ct == COMPARISON_EQUALS ? COMPARISON_NOT_EQUALS
                                                       : COMPARISON_EQUALS);
      ...
      isolate_x_sub(eo, eo2, x_var, morig);
      if (mn1) {
        add_nocopy(mn1, ct == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_OR
                                                    : OPERATION_LOGICAL_AND);
        calculatesub(eo2, eo, false);
      }
      if (m1) {
      ...
      if (mtest) {
        add_nocopy(mtest, ct == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_OR
                                                      : OPERATION_LOGICAL_AND);
        calculatesub(eo2, eo, false);
        if (m0) {
          add_nocopy(m0, ct == COMPARISON_EQUALS ? OPERATION_LOGICAL_OR
                                                 : OPERATION_LOGICAL_AND);
          calculatesub(eo2, eo, false);
        }
      }
  ...
  case STRUCT_FUNCTION: {
    if (CHILD(0).function()->id() == FUNCTION_ID_ROOT && VALID_ROOT(CHILD(0))) {
      if (CHILD(0)[0].contains(x_var)) {
        MathStructure *mposcheck = NULL;
        bool b_test = false;
        if (CHILD(0)[1].number().numeratorIsEven() &&
            !CHILD(1).representsNonNegative(true)) {
          if (ct_comp != COMPARISON_EQUALS &&
              ct_comp != COMPARISON_NOT_EQUALS) {
            if (CHILD(1).representsNegative(true)) {
              if (ct_comp == COMPARISON_EQUALS_LESS ||
        ...
        if ((ct_comp == COMPARISON_EQUALS_LESS || ct_comp == COMPARISON_LESS) &&
            CHILD(0)[1].number().numeratorIsEven()) {
          mposcheck = new MathStructure(CHILD(0)[0]);
          mposcheck->add(m_zero, OPERATION_EQUALS_GREATER);
          mposcheck->isolate_x_sub(eo, eo2, x_var);
        }
    ...
    } else if (CHILD(0).function()->id() == FUNCTION_ID_LOG &&
               CHILD(0).size() == 1) {
      if (CHILD(0)[0].contains(x_var)) {
    ...
    } else if (CHILD(0).function()->id() == FUNCTION_ID_LAMBERT_W &&
               (CHILD(0).size() == 1 ||
                (CHILD(0).size() == 2 && CHILD(0)[1].isInteger())) &&
               ((ct_comp == COMPARISON_EQUALS ||
                 ct_comp == COMPARISON_NOT_EQUALS) ||
                (CHILD(0).size() == 1 || CHILD(0)[1].isZero() ||
                 CHILD(0)[1].isMinusOne()))) {
      if (CHILD(0)[0].contains(x_var)) {
        CHILDREN_UPDATED;
        bool b_test = true;
        if (ct_comp != COMPARISON_EQUALS && ct_comp != COMPARISON_NOT_EQUALS) {
            ...
            if (cr == COMPARISON_RESULT_LESS) {
              ...
              if (ct_comp == COMPARISON_EQUALS_GREATER)
                ct_comp = COMPARISON_EQUALS;
                       ...
                       cr != COMPARISON_RESULT_EQUAL_OR_GREATER) {
              return false;
            }
          } else {
            ComparisonResult cr = CHILD(1).compare(nr_minus_one);
            if (cr == COMPARISON_RESULT_GREATER) {
              ...
              if (ct_comp == COMPARISON_EQUALS_LESS)
                ct_comp = COMPARISON_EQUALS;
                       ...
                       cr != COMPARISON_RESULT_EQUAL_OR_LESS) {
              return false;
            }
          }
        ...
        if ((ct_comp == COMPARISON_LESS || ct_comp == COMPARISON_EQUALS_LESS) &&
          ...
          mand->add(MathStructure(CALCULATOR->getVariableById(VARIABLE_ID_E)),
                    CHILD(0).size() == 2 && CHILD(0)[1].isMinusOne()
                        ? OPERATION_EQUALS_LESS
                        : OPERATION_EQUALS_GREATER);
          (*mand)[1].inverse();
          (*mand)[1].negate();
        ...
        if (!b_test ||
            (!mtest.isLogicalAnd() && !mtest.isLogicalOr() &&
             !mtest.isComparison()) ||
            test_comparisons(*this, mtest, x_var, eo, false,
                             eo2.expand ? 1 : 2) >= 0) {
          set(mtest);
    ...
    } else if (CHILD(0).function()->id() == FUNCTION_ID_LOGN &&
               CHILD(0).size() == 2) {
      if (CHILD(0)[0].contains(x_var)) {
        MathStructure msave(CHILD(1));
        CHILD(1) = CHILD(0)[1];
    ...
    } else if ((CHILD(0).function()->id() == FUNCTION_ID_ERF ||
                CHILD(0).function()->id() == FUNCTION_ID_ERFC ||
                CHILD(0).function()->id() == FUNCTION_ID_ERFI) &&
               CHILD(0).size() == 1) {
      if (CHILD(0)[0].contains(x_var) &&
          (ct_comp == COMPARISON_EQUALS ||
           (CHILD(0)[0].representsReal() &&
            CHILD(0).function()->id() != FUNCTION_ID_ERFI)) &&
          CHILD(1).isNumber()) {
        Number nr(CHILD(1).number());
        if (CHILD(0).function()->id() == FUNCTION_ID_ERFC) {
          nr--;
          nr.negate();
        } else if (CHILD(0).function()->id() == FUNCTION_ID_ERFI) {
          nr /= nr_minus_i;
        }
        if (nr.isFraction()) {
          if (CHILD(0).function()->id() != FUNCTION_ID_ERF)
            CHILD(1).set(nr, true);
    ...
    } else if (CHILD(0).function()->id() == FUNCTION_ID_ERFINV &&
               CHILD(0).size() == 1) {
      ...
      if (!b_zero && !b_one) {
        if (f->id() == FUNCTION_ID_SIN)
          CHILD(1).transformById(FUNCTION_ID_ASIN);
        else if (f->id() == FUNCTION_ID_COS)
          CHILD(1).transformById(FUNCTION_ID_ACOS);
        else
          CHILD(1).transformById(FUNCTION_ID_ATAN);
      ...
      if (!b_one && !b_zero &&
          (f->id() == FUNCTION_ID_SIN || f->id() == FUNCTION_ID_COS)) {
        CHILD(1)[1] *= 2;
        MathStructure *malt = new MathStructure(*this);
        (*malt)[1][0].negate();
        if (f->id() == FUNCTION_ID_SIN) {
        ...
        malt->isolate_x_sub(eo, eo2, x_var, morig);
        if (ct_comp == COMPARISON_NOT_EQUALS)
          add_nocopy(malt, OPERATION_LOGICAL_AND);
        else
          add_nocopy(malt, OPERATION_LOGICAL_OR);
        calculatesub(eo2, eo, false);
      } else {
      ...
      if (CHILD(0).representsNonComplex()) {
        ...
        CHILD(1).last() *= nr_two;
        (*malt)[1] += CALCULATOR->getVariableById(VARIABLE_ID_N);
        (*malt)[1].last() *= CALCULATOR->getVariableById(VARIABLE_ID_PI);
        (*malt)[1].last() *= nr_two;
        CHILD(1) *= nr_one_i;
        (*malt)[1] *= nr_one_i;
        CHILD(1).calculatesub(eo2, eo, true);
      ...
      return true;
    } else if (CHILD(0).function()->id() == FUNCTION_ID_ASIN &&
               (ct_comp == COMPARISON_NOT_EQUALS ||
                ct_comp == COMPARISON_EQUALS)) {
      ...
      if (m1.calculateFunctions(eo))
        m1.calculatesub(eo2, eo, true);
      m2.transformById(FUNCTION_ID_IM);
      if (m2.calculateFunctions(eo))
        m2.calculatesub(eo2, eo, true);
      ...
      if (DEFAULT_RADIANS(eo.parse_options.angle_unit)) {
        mreq1->transform(ct_comp == COMPARISON_NOT_EQUALS
                             ...
                             : COMPARISON_EQUALS_LESS,
                         CALCULATOR->getVariableById(VARIABLE_ID_PI));
        mreq2->transform(ct_comp == COMPARISON_NOT_EQUALS
                             ? COMPARISON_LESS
                             : COMPARISON_EQUALS_GREATER,
                         CALCULATOR->getVariableById(VARIABLE_ID_PI));
        mreq1->last() *= nr_half;
        ...
        if (mreq3) {
          mreq3->transform(ct_comp == COMPARISON_NOT_EQUALS
                               ? COMPARISON_EQUALS
                               : COMPARISON_NOT_EQUALS,
                           CALCULATOR->getVariableById(VARIABLE_ID_PI));
        ...
        if (mreq5) {
          mreq5->transform(ct_comp == COMPARISON_NOT_EQUALS
                               ? COMPARISON_EQUALS
                               : COMPARISON_NOT_EQUALS,
                           CALCULATOR->getVariableById(VARIABLE_ID_PI));
          mreq5->last() *= nr_half;
      ...
      if (mreq4)
        mreq4->transform(ct_comp == COMPARISON_NOT_EQUALS
                             ? COMPARISON_LESS
                             : COMPARISON_EQUALS_GREATER,
                         m_zero);
      if (mreq6)
        mreq6->transform(ct_comp == COMPARISON_NOT_EQUALS
                             ? COMPARISON_GREATER
                             : COMPARISON_EQUALS_LESS,
                         m_zero);
      mreq1->isolate_x(eo2, eo);
      ...
      if (mreq3) {
        ...
        mreq3->add_nocopy(mreq4, ct_comp == COMPARISON_NOT_EQUALS
                                     ? OPERATION_LOGICAL_AND
                                     : OPERATION_LOGICAL_OR);
      ...
      if (mreq5) {
        ...
        mreq5->add_nocopy(mreq6, ct_comp == COMPARISON_NOT_EQUALS
                                     ? OPERATION_LOGICAL_AND
                                     : OPERATION_LOGICAL_OR);
      ...
      if (NO_DEFAULT_ANGLE_UNIT(eo.parse_options.angle_unit))
        convert(CALCULATOR->getRadUnit());
      calculatesub(eo2, eo, false);
      return true;
    } else if (CHILD(0).function()->id() == FUNCTION_ID_ACOS &&
               (ct_comp == COMPARISON_NOT_EQUALS ||
                ct_comp == COMPARISON_EQUALS)) {
      MathStructure m1(CHILD(1));
      CHILD(0).setToChild(1, true);
      ...
      calculatesub(eo2, eo, false);
      return true;
    } else if (CHILD(0).function()->id() == FUNCTION_ID_ATAN &&
               (ct_comp == COMPARISON_NOT_EQUALS ||
                ct_comp == COMPARISON_EQUALS)) {
      MathStructure m1(CHILD(1));
      CHILD(0).setToChild(1, true);
      ...
      calculatesub(eo2, eo, false);
      return true;
    } else if (CHILD(0).function()->id() == FUNCTION_ID_ASINH &&
               (ct_comp == COMPARISON_NOT_EQUALS ||
                ct_comp == COMPARISON_EQUALS)) {
      ...
      if (m2.calculateFunctions(eo))
        m2.calculatesub(eo2, eo, true);
      if (!m2.isZero()) {
        MathStructure *mreq1 = NULL, *mreq2 = NULL, *mreq3 = NULL,
                      *mreq4 = NULL, *mreq5 = NULL, *mreq6 = NULL;
        m1.transformById(FUNCTION_ID_RE);
        if (m1.calculateFunctions(eo))
          m1.calculatesub(eo2, eo, true);
        ...
        if (m1.isZero()) {
          ...
          mreq3->transform(ct_comp,
                           CALCULATOR->getVariableById(VARIABLE_ID_PI));
          mreq4->transform(ct_comp,
                           CALCULATOR->getVariableById(VARIABLE_ID_PI));
          mreq3->last() *= nr_half;
        ...
        if (mreq3) {
          ...
          mreq5->isolate_x(eo2, eo);
          mreq6->isolate_x(eo2, eo);
          mreq3->add_nocopy(mreq5, ct_comp == COMPARISON_NOT_EQUALS
                                       ? OPERATION_LOGICAL_OR
                                       : OPERATION_LOGICAL_AND);
          mreq3->calculatesub(eo2, eo, false);
          mreq4->add_nocopy(mreq6, ct_comp == COMPARISON_NOT_EQUALS
                                       ? OPERATION_LOGICAL_OR
                                       : OPERATION_LOGICAL_AND);
          mreq4->calculatesub(eo2, eo, false);
          mreq1->add_nocopy(mreq3, ct_comp == COMPARISON_NOT_EQUALS
                                       ? OPERATION_LOGICAL_AND
                                       : OPERATION_LOGICAL_OR);
          mreq1->calculatesub(eo2, eo, false);
          mreq2->add_nocopy(mreq4, ct_comp == COMPARISON_NOT_EQUALS
                                       ? OPERATION_LOGICAL_AND
                                       : OPERATION_LOGICAL_OR);
      ...
      calculatesub(eo2, eo, false);
      return true;
    } else if (CHILD(0).function()->id() == FUNCTION_ID_ACOSH &&
               (ct_comp == COMPARISON_NOT_EQUALS ||
                ct_comp == COMPARISON_EQUALS)) {
      MathStructure m1(CHILD(1)), m2(CHILD(1));
      CHILD(0).setToChild(1, true);
      ...
      if (m2.isZero()) {
        ...
        mreq1->add_nocopy(mreq3, ct_comp == COMPARISON_NOT_EQUALS
                                     ? OPERATION_LOGICAL_OR
                                     : OPERATION_LOGICAL_AND);
        mreq1->add_nocopy(mreq4,
                          ct_comp == COMPARISON_NOT_EQUALS
                              ? OPERATION_LOGICAL_OR
                              : OPERATION_LOGICAL_AND,
                          true);
        mreq1->calculatesub(eo2, eo, false);
        mreq2->add_nocopy(mreq5, ct_comp == COMPARISON_NOT_EQUALS
                                     ? OPERATION_LOGICAL_OR
                                     : OPERATION_LOGICAL_AND);
        mreq2->add_nocopy(mreq6,
                          ct_comp == COMPARISON_NOT_EQUALS
                              ? OPERATION_LOGICAL_OR
                              : OPERATION_LOGICAL_AND,
                          true);
        mreq2->calculatesub(eo2, eo, false);
        mreq1->add_nocopy(mreq2, ct_comp == COMPARISON_NOT_EQUALS
                                     ? OPERATION_LOGICAL_AND
                                     : OPERATION_LOGICAL_OR);
        mreq1->calculatesub(eo2, eo, false);
        add_nocopy(mreq1, ct_comp == COMPARISON_NOT_EQUALS
      ...
      return true;
    } else if (CHILD(0).function()->id() == FUNCTION_ID_ATANH &&
               (ct_comp == COMPARISON_NOT_EQUALS ||
                ct_comp == COMPARISON_EQUALS)) {
      MathStructure m1(CHILD(1)), m2(CHILD(1));
      CHILD(0).setToChild(1, true);
      ...
      return true;
    } else if (CHILD(0).function()->id() == FUNCTION_ID_ABS &&
               CHILD(0).size() == 1) {
      if (CHILD(0)[0].contains(x_var)) {
        if (CHILD(1).representsComplex() &&
          ...
          if (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_LESS ||
              ct_comp == COMPARISON_EQUALS_LESS)
            clear(true);
          else if (ct_comp == COMPARISON_NOT_EQUALS ||
                   ct_comp == COMPARISON_GREATER ||
    ...
    } else if (CHILD(0).function()->id() == FUNCTION_ID_SIGNUM &&
               CHILD(0).size() == 2) {
      if (CHILD(0)[0].contains(x_var) &&
          CHILD(0)[0].representsNonComplex(true)) {
        ...
        if (CHILD(1).isNumber() && !CHILD(1).number().isInterval(false)) {
          if (CHILD(1).number().isOne()) {
            if (ct_comp == COMPARISON_EQUALS ||
                ct_comp == COMPARISON_EQUALS_GREATER)
              ct_comp = (CHILD(0)[1].isOne() ? COMPARISON_EQUALS_GREATER
                                             : COMPARISON_GREATER);
            else if (ct_comp == COMPARISON_NOT_EQUALS ||
                     ct_comp == COMPARISON_LESS)
              ct_comp = (CHILD(0)[1].isOne() ? COMPARISON_LESS
                                             : COMPARISON_EQUALS_LESS);
            else if (ct_comp == COMPARISON_GREATER) {
              clear(true);
              return 1;
            ...
            if (ct_comp == COMPARISON_EQUALS ||
                ct_comp == COMPARISON_EQUALS_LESS)
              ct_comp = (CHILD(0)[1].isMinusOne() ? COMPARISON_EQUALS_LESS
                                                  : COMPARISON_LESS);
            else if (ct_comp == COMPARISON_NOT_EQUALS ||
                     ct_comp == COMPARISON_GREATER)
          ...
          if (CHILD(0)[1].isZero() || CHILD(0)[1].isOne() ||
              CHILD(0)[1].isMinusOne()) {
            if (CHILD(1).number().isPositive()) {
              ...
              if (ct_comp == COMPARISON_GREATER ||
                  ct_comp == COMPARISON_EQUALS_GREATER)
                ct_comp =
                    (CHILD(0)[1].isMinusOne() ? COMPARISON_GREATER
                                              : COMPARISON_EQUALS_GREATER);
              else
                ct_comp = (CHILD(0)[1].isMinusOne() ? COMPARISON_EQUALS_LESS
                                                    : COMPARISON_LESS);
...
bool contains_unsolved_equals(const MathStructure &mstruct,
                              const MathStructure &x_var) {
  if (mstruct.isComparison()) {
    return mstruct.comparisonType() == COMPARISON_EQUALS &&
           mstruct[0] != x_var && mstruct[1] != x_var &&
           mstruct.contains(x_var);
  }
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (contains_unsolved_equals(mstruct[i], x_var))
      return true;
  }
  return false;
}

bool sync_sine(MathStructure &mstruct, const EvaluationOptions &eo,
               const MathStructure &x_var, bool use_cos, bool b_hyp = false,
               const MathStructure &mstruct_parent = m_undefined) {
  if (!mstruct_parent.isUndefined() && mstruct.isFunction() &&
      mstruct.function()->id() ==
          (b_hyp ? FUNCTION_ID_SINH : FUNCTION_ID_SIN) &&
      mstruct.size() == 1 && mstruct[0].contains(x_var)) {
    ...
    if (b) {
      ...
      return true;
             ...
             mstruct[1].isNumber() && mstruct[1].number().isEven() &&
             mstruct[0].size() == 1) {
    ...
    if (mstruct[0].function()->id() ==
            (b_hyp ? FUNCTION_ID_TANH : FUNCTION_ID_TAN) &&
        mstruct[0][0].contains(x_var)) {
    ...
    if (mstruct[0].function()->id() ==
            (use_cos ? (b_hyp ? FUNCTION_ID_SINH : FUNCTION_ID_SIN)
                     : (b_hyp ? FUNCTION_ID_COSH : FUNCTION_ID_COS)) &&
        mstruct[0][0].contains(x_var)) {
      mstruct[0].setFunctionId(
          use_cos ? (b_hyp ? FUNCTION_ID_COSH : FUNCTION_ID_COS)
                  : (b_hyp ? FUNCTION_ID_SINH : FUNCTION_ID_SIN));
...
bool sync_trigonometric_functions(MathStructure &mstruct,
                                  ...
                                  bool use_cos = false) {
  bool b_ret = false;
  if (sync_sine(mstruct, eo, x_var, use_cos))
    b_ret = true;
  if (sync_sine(mstruct, eo, x_var, use_cos, true))
    b_ret = true;
  return b_ret;
}

bool MathStructure::isolate_x(const EvaluationOptions &eo,
                              const MathStructure &x_varp, bool check_result) {
...
bool MathStructure::isolate_x(const EvaluationOptions &eo,
  ...
  if (isProtected())
    return false;
  if (!isComparison()) {
    if (m_type == STRUCT_FUNCTION && o_function->id() == FUNCTION_ID_D_SOLVE)
      return false;
    bool b = false;
  ...
  if (x_var.isUndefined()) {
    ...
    if (eo.isolate_var && contains(*eo.isolate_var))
      x_var2 = eo.isolate_var;
    else
      x_var2 = &find_x_var();
    if (x_var2->isUndefined())
      return false;
    x_var = *x_var2;
  }

  if (CHILD(0) == x_var && !CHILD(1).contains(x_var))
    return true;
  if (!CHILD(1).isZero()) {
    CHILD(0).calculateSubtract(CHILD(1), eo);
    CHILD(1).clear(true);
  ...
  if (eo.expand > 0)
    simplify_functions(*this, eo, feo, x_var);
  ...
  if (eo.expand > 0 && contains_unsolved_equals(*this, x_var)) {
    ...
    if (sync_trigonometric_functions(mtest, eo3, x_var, false)) {
      ...
      if (eo.do_polynomial_division)
        do_simplification(mtest, eo3, true,
                          eo.structuring == STRUCTURING_NONE ||
                              eo.structuring == STRUCTURING_FACTORIZE,
                          false, true, true);
      if (CALCULATOR->aborted())
        return !check_result && b;
      if (mtest.isComparison() && mtest.isolate_x_sub(feo, eo2, x_var) &&
          !contains_unsolved_equals(mtest, x_var)) {
        set(mtest);
        b = true;
        do_cos = false;
