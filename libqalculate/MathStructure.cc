...
void MathStructure::mergePrecision(const MathStructure &o) {
...
size_t MathStructure::refcount() const { return i_ref; }
...
MathStructure::MathStructure() { init(); }
MathStructure::MathStructure(const MathStructure &o) {
  ...
  case STRUCT_FUNCTION: {
    ...
    if (o_function)
      o.function()->ref();
    if (o.functionValue())
      function_value = new MathStructure(*o.functionValue());
    break;
  }
...
void MathStructure::set(long int num, long int den, long int exp10,
...
bool MathStructure::operator==(const MathStructure &o) const {
  return equals(o);
}
...
const Number &MathStructure::number() const { return o_number; }
Number &MathStructure::number() { return o_number; }
...
void MathStructure::childUpdated(size_t index, bool recursive) {
  if (index > SIZE || index < 1)
    return;
  if (recursive)
    CHILD(index - 1).childrenUpdated(true);
  MERGE_APPROX_AND_PREC(CHILD(index - 1))
}
void MathStructure::childrenUpdated(bool recursive) {
  for (size_t i = 0; i < SIZE; i++) {
    if (recursive)
      CHILD(i).childrenUpdated(true);
    MERGE_APPROX_AND_PREC(CHILD(i))
  }
}
const string &MathStructure::symbol() const { return s_sym; }
const QalculateDateTime *MathStructure::datetime() const { return o_datetime; }
QalculateDateTime *MathStructure::datetime() { return o_datetime; }
ComparisonType MathStructure::comparisonType() const { return ct_comp; }
...
void MathStructure::setType(StructureType mtype) {
  ...
  if (m_type != STRUCT_VARIABLE && o_variable) {
    o_variable->unref();
    o_variable = NULL;
  }
  if (m_type != STRUCT_UNIT && o_unit) {
    o_unit->unref();
    o_unit = NULL;
    o_prefix = NULL;
  }
  if (m_type != STRUCT_DATETIME && o_datetime) {
    delete o_datetime;
    o_datetime = NULL;
  }
}
Unit *MathStructure::unit() const { return o_unit; }
Unit *MathStructure::unit_exp_unit() const {
  if (isUnit())
    return o_unit;
  if (isPower() && CHILD(0).isUnit())
    return CHILD(0).unit();
  return NULL;
}
Prefix *MathStructure::prefix() const { return o_prefix; }
Prefix *MathStructure::unit_exp_prefix() const {
  if (isUnit())
    return o_prefix;
  if (isPower() && CHILD(0).isUnit())
    return CHILD(0).prefix();
  return NULL;
}
void MathStructure::setPrefix(Prefix *p) { o_prefix = p; }
bool MathStructure::isPlural() const { return b_plural; }
...
MathFunction *MathStructure::function() const { return o_function; }
Variable *MathStructure::variable() const { return o_variable; }

bool MathStructure::isAddition() const { return m_type == STRUCT_ADDITION; }
bool MathStructure::isMultiplication() const {
  return m_type == STRUCT_MULTIPLICATION;
}
...
bool MathStructure::isEmptySymbol() const {
  return m_type == STRUCT_SYMBOLIC && s_sym.empty();
}
bool MathStructure::isVector() const { return m_type == STRUCT_VECTOR; }
bool MathStructure::isMatrix() const {
  if (m_type != STRUCT_VECTOR || SIZE < 1)
    return false;
  for (size_t i = 0; i < SIZE; i++) {
    if (!CHILD(i).isVector() ||
        (i > 0 && CHILD(i).size() != CHILD(i - 1).size()))
      return false;
  }
  return true;
}
...
bool MathStructure::isUnit_exp() const {
  return m_type == STRUCT_UNIT || (m_type == STRUCT_POWER && CHILD(0).isUnit());
}
bool MathStructure::isUnknown() const {
  return m_type == STRUCT_SYMBOLIC ||
         (m_type == STRUCT_VARIABLE && o_variable && !o_variable->isKnown());
}
bool MathStructure::isUnknown_exp() const {
  return isUnknown() || (m_type == STRUCT_POWER && CHILD(0).isUnknown());
}
bool MathStructure::isNumber_exp() const {
  return m_type == STRUCT_NUMBER ||
         (m_type == STRUCT_POWER && CHILD(0).isNumber());
}
...
bool MathStructure::isLogicalAnd() const {
  return m_type == STRUCT_LOGICAL_AND;
}
bool MathStructure::isLogicalOr() const { return m_type == STRUCT_LOGICAL_OR; }
bool MathStructure::isLogicalXor() const {
  return m_type == STRUCT_LOGICAL_XOR;
}
bool MathStructure::isLogicalNot() const {
  return m_type == STRUCT_LOGICAL_NOT;
}
bool MathStructure::isBitwiseAnd() const {
  return m_type == STRUCT_BITWISE_AND;
}
bool MathStructure::isBitwiseOr() const { return m_type == STRUCT_BITWISE_OR; }
bool MathStructure::isBitwiseXor() const {
  return m_type == STRUCT_BITWISE_XOR;
}
bool MathStructure::isBitwiseNot() const {
  return m_type == STRUCT_BITWISE_NOT;
}
...
bool MathStructure::isInfinity() const {
  return m_type == STRUCT_NUMBER && o_number.isInfinite(true);
}
bool MathStructure::isUndefined() const {
  return m_type == STRUCT_UNDEFINED ||
         (m_type == STRUCT_NUMBER && o_number.isUndefined()) ||
         (m_type == STRUCT_VARIABLE &&
          o_variable == CALCULATOR->getVariableById(VARIABLE_ID_UNDEFINED));
}
bool MathStructure::isInteger() const {
  return m_type == STRUCT_NUMBER && o_number.isInteger();
}
bool MathStructure::isInfinite(bool ignore_imag) const {
  return m_type == STRUCT_NUMBER && o_number.isInfinite(ignore_imag);
}
bool MathStructure::isNumber() const { return m_type == STRUCT_NUMBER; }
bool MathStructure::isZero() const {
  return m_type == STRUCT_NUMBER && o_number.isZero();
}
bool MathStructure::isApproximatelyZero() const {
  return m_type == STRUCT_NUMBER && !o_number.isNonZero();
}
bool MathStructure::isOne() const {
  return m_type == STRUCT_NUMBER && o_number.isOne();
}
bool MathStructure::isMinusOne() const {
  return m_type == STRUCT_NUMBER && o_number.isMinusOne();
}

bool MathStructure::hasNegativeSign() const {
  return (m_type == STRUCT_NUMBER && o_number.hasNegativeSign()) ||
         m_type == STRUCT_NEGATE ||
         (m_type == STRUCT_MULTIPLICATION && SIZE > 0 &&
          CHILD(0).hasNegativeSign());
}

bool MathStructure::representsBoolean() const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isOne() || o_number.isZero();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsBoolean();
  }
  case STRUCT_SYMBOLIC: {
    return CALCULATOR->defaultAssumptions()->isBoolean();
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsBoolean()) ||
           o_function->representsBoolean(*this);
  }
  case STRUCT_MULTIPLICATION: {
    for (size_t i = 0; i < SIZE; i++) {
      if (!CHILD(i).representsBoolean())
        return false;
    }
    return true;
  }
...
bool MathStructure::representsNumber(bool allow_units) const {
  ...
  case STRUCT_VARIABLE: {
    return o_variable->representsNumber(allow_units);
  }
  case STRUCT_SYMBOLIC: {
    return CALCULATOR->defaultAssumptions()->isNumber();
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsNumber(allow_units)) ||
           o_function->representsNumber(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return allow_units;
  }
  ...
  case STRUCT_POWER: {
    if (!CHILD(0).representsNonZero(allow_units) &&
        !CHILD(1).representsPositive(allow_units))
      return false;
  }
  ...
  case STRUCT_MULTIPLICATION: {
    for (size_t i = 0; i < SIZE; i++) {
      if (!CHILD(i).representsNumber(allow_units))
        return false;
    }
    return true;
  }
...
bool MathStructure::representsInteger(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isInteger();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsInteger(allow_units);
  }
  case STRUCT_SYMBOLIC: {
    return CALCULATOR->defaultAssumptions()->isInteger();
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsInteger(allow_units)) ||
           o_function->representsInteger(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return allow_units;
  }
  ...
  case STRUCT_MULTIPLICATION: {
    for (size_t i = 0; i < SIZE; i++) {
      if (!CHILD(i).representsInteger(allow_units))
        return false;
    }
    return true;
  }
  case STRUCT_POWER: {
    return CHILD(0).representsInteger(allow_units) &&
           CHILD(1).representsInteger(false) &&
           CHILD(1).representsPositive(false);
  }
  default: {
    return false;
  }
...
bool MathStructure::representsNonInteger(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isNonInteger();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsNonInteger(allow_units);
  }
  case STRUCT_FUNCTION: {
    return (function_value &&
            function_value->representsNonInteger(allow_units));
  }
  case STRUCT_UNIT: {
    return false;
  }
...
bool MathStructure::representsFraction(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isFraction();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsFraction(allow_units);
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsFraction(allow_units));
  }
  case STRUCT_UNIT: {
    return false;
  }
...
bool MathStructure::representsPositive(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isPositive();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsPositive(allow_units);
  }
  case STRUCT_SYMBOLIC: {
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsPositive(true);
    return (function_value &&
            function_value->representsPositive(allow_units)) ||
           o_function->representsPositive(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return allow_units;
  }
  case STRUCT_ADDITION: {
    for (size_t i = 0; i < SIZE; i++) {
      if (!CHILD(i).representsPositive(allow_units))
        return false;
    }
    return true;
  }
  ...
  case STRUCT_POWER: {
    return (CHILD(0).representsPositive(allow_units) &&
            CHILD(1).representsReal(false)) ||
           (CHILD(0).representsNonZero(allow_units) &&
            CHILD(0).representsReal(allow_units) &&
            CHILD(1).representsEven(false) &&
            CHILD(1).representsInteger(false));
  }
  default: {
    return false;
...
bool MathStructure::representsNegative(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isNegative();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsNegative(allow_units);
  }
  case STRUCT_SYMBOLIC: {
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsNegative(true);
    return (function_value &&
            function_value->representsNegative(allow_units)) ||
           o_function->representsNegative(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return false;
  }
  ...
  case STRUCT_POWER: {
    return CHILD(1).representsInteger(false) && CHILD(1).representsOdd(false) &&
           CHILD(0).representsNegative(allow_units);
  }
  default: {
    return false;
  }
...
bool MathStructure::representsNonNegative(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isNonNegative();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsNonNegative(allow_units);
  }
  case STRUCT_SYMBOLIC: {
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsNonNegative(true);
    return (function_value &&
            function_value->representsNonNegative(allow_units)) ||
           o_function->representsNonNegative(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return allow_units;
  }
  case STRUCT_ADDITION: {
    for (size_t i = 0; i < SIZE; i++) {
      if (!CHILD(i).representsNonNegative(allow_units))
        return false;
    }
    return true;
  }
  ...
  case STRUCT_POWER: {
    return (CHILD(0).isZero() && CHILD(1).representsNonNegative(false)) ||
           (CHILD(0).representsNonNegative(allow_units) &&
            CHILD(1).representsReal(false)) ||
           (CHILD(0).representsReal(allow_units) &&
            CHILD(1).representsEven(false) &&
            CHILD(1).representsInteger(false));
...
bool MathStructure::representsNonPositive(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isNonPositive();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsNonPositive(allow_units);
  }
  case STRUCT_SYMBOLIC: {
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsNonPositive(true);
    return (function_value &&
            function_value->representsNonPositive(allow_units)) ||
           o_function->representsNonPositive(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return false;
  }
  case STRUCT_ADDITION: {
    for (size_t i = 0; i < SIZE; i++) {
      if (!CHILD(i).representsNonPositive(allow_units))
        return false;
    }
    return true;
  }
  ...
  case STRUCT_POWER: {
    return (CHILD(0).isZero() && CHILD(1).representsPositive(false)) ||
           representsNegative(allow_units);
  }
  default: {
    return false;
  }
...
bool MathStructure::representsRational(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isRational();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsRational(allow_units);
  }
  case STRUCT_SYMBOLIC: {
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsRational(true);
    return (function_value &&
            function_value->representsRational(allow_units)) ||
           o_function->representsRational(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return false;
  }
  ...
  case STRUCT_POWER: {
    return CHILD(1).representsInteger(false) &&
           CHILD(0).representsRational(allow_units) &&
           (CHILD(0).representsPositive(allow_units) ||
            (CHILD(0).representsNegative(allow_units) &&
             CHILD(1).representsEven(false) &&
             CHILD(1).representsPositive(false)));
  }
...
bool MathStructure::representsReal(bool allow_units) const {
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsReal(true);
    return (function_value && function_value->representsReal(allow_units)) ||
           o_function->representsReal(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return allow_units;
  }
...
bool MathStructure::representsNonComplex(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return !o_number.hasImaginaryPart();
  }
  case STRUCT_VARIABLE: {
    if (o_variable->isKnown())
      return ((KnownVariable *)o_variable)
          ->get()
          .representsNonComplex(allow_units);
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsNonComplex(true);
    return (function_value &&
            function_value->representsNonComplex(allow_units)) ||
           o_function->representsNonComplex(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return allow_units;
  }
...
bool MathStructure::representsComplex(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.imaginaryPartIsNonZero();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsComplex(allow_units);
  }
  case STRUCT_SYMBOLIC: {
    return CALCULATOR->defaultAssumptions()->isComplex();
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsComplex(allow_units)) ||
           o_function->representsComplex(*this, allow_units);
  }
  case STRUCT_ADDITION: {
    bool c = false;
    for (size_t i = 0; i < SIZE; i++) {
    ...
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).representsComplex(allow_units)) {
        ...
        c = true;
                 ...
                 !CHILD(i).representsNonZero(allow_units)) {
        return false;
      }
    }
    return c;
  ...
  case STRUCT_POWER: {
    return CHILD(1).isNumber() && CHILD(1).number().isRational() &&
           !CHILD(1).number().isInteger() && CHILD(0).representsNegative();
  }
  default: {
    return false;
  }
...
bool MathStructure::representsNonZero(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isNonZero();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsNonZero(allow_units);
  }
  case STRUCT_SYMBOLIC: {
    return CALCULATOR->defaultAssumptions()->isNonZero();
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsNonZero(allow_units)) ||
           o_function->representsNonZero(*this, allow_units);
  }
  case STRUCT_UNIT: {
    return allow_units;
  }
  ...
  case STRUCT_POWER: {
    return (CHILD(0).representsNonZero(allow_units) &&
            CHILD(1).representsNumber(true)) ||
           (((!CHILD(0).isApproximatelyZero() &&
              CHILD(1).representsNonPositive()) ||
             CHILD(1).representsNegative()) &&
            CHILD(0).representsNumber(allow_units) &&
            CHILD(1).representsNumber(true));
  }
  default: {
    return false;
...
bool MathStructure::representsZero(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isZero();
  }
  case STRUCT_VARIABLE: {
    return o_variable->isKnown() &&
           !o_variable->representsNonZero(allow_units) &&
           ((KnownVariable *)o_variable)->get().representsZero();
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsZero(allow_units));
  }
  case STRUCT_ADDITION: {
    for (size_t i = 0; i < SIZE; i++) {
    ...
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).representsZero(allow_units)) {
        for (size_t i2 = 0; i2 < SIZE; i2++) {
          if (i2 != i && CHILD(i2).representsUndefined(true, true, true))
            return false;
        }
        return true;
      }
  ...
  case STRUCT_POWER: {
    return CHILD(0).representsZero(allow_units) &&
           CHILD(1).representsPositive(allow_units);
  }
  default: {
    return false;
...
bool MathStructure::representsApproximatelyZero(bool allow_units) const {
  ...
  case STRUCT_VARIABLE: {
    return o_variable->isKnown() &&
           !o_variable->representsNonZero(allow_units) &&
           ((KnownVariable *)o_variable)->get().representsApproximatelyZero();
  }
  case STRUCT_FUNCTION: {
    return (function_value &&
            function_value->representsApproximatelyZero(allow_units));
  }
  case STRUCT_ADDITION: {
    for (size_t i = 0; i < SIZE; i++) {
  ...
  case STRUCT_POWER: {
    return CHILD(0).representsApproximatelyZero(allow_units) &&
           CHILD(1).representsPositive(allow_units);
  }
  default: {
    return false;
...
bool MathStructure::representsEven(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isEven();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsEven(allow_units);
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsEven(allow_units)) ||
           o_function->representsEven(*this, allow_units);
  }
  default: {
    return false;
  }
...
bool MathStructure::representsOdd(bool allow_units) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    return o_number.isOdd();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsOdd(allow_units);
  }
  case STRUCT_FUNCTION: {
    return (function_value && function_value->representsOdd(allow_units)) ||
           o_function->representsOdd(*this, allow_units);
  }
  default: {
    return false;
  }
...
bool MathStructure::representsUndefined(bool include_childs,
                                        bool include_infinite,
                                        bool be_strict) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    if (include_infinite) {
      return o_number.includesInfinity();
  ...
  case STRUCT_VARIABLE: {
    return o_variable->representsUndefined(include_childs, include_infinite,
                                           be_strict);
  }
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsUndefined(include_childs, include_infinite,
                                          be_strict);
    return (function_value &&
            function_value->representsUndefined(include_childs,
                                                include_infinite, be_strict)) ||
           o_function->representsUndefined(*this);
  }
  case STRUCT_POWER: {
    if (be_strict) {
      if ((!CHILD(0).representsNonZero(true) &&
           !CHILD(1).representsNonNegative(false)) ||
          (CHILD(0).isInfinity() && !CHILD(1).representsNonZero(true))) {
        return true;
      }
    } else {
      if ((CHILD(0).representsZero(true) &&
           CHILD(1).representsNegative(false)) ||
          (CHILD(0).isInfinity() && CHILD(1).representsZero(true))) {
        return true;
      }
    }
  }
  default: {
    if (include_childs) {
      for (size_t i = 0; i < SIZE; i++) {
        if (CHILD(i).representsUndefined(include_childs, include_infinite,
                                         be_strict))
          return true;
      }
    }
...
bool MathStructure::representsNonMatrix() const {
      ...
      if (CHILD(i).isVariable() && CHILD(i).variable()->isKnown()) {
        if (((KnownVariable *)CHILD(i).variable())->get().isVector()) {
          if (n == 0) {
            ...
            if (n == 0)
              return true;
          } else if (((KnownVariable *)CHILD(i).variable())->get().size() !=
                     n) {
            return true;
          }
  ...
  case STRUCT_POWER: {
    return CHILD(0).representsNonMatrix();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsNonMatrix();
  }
  case STRUCT_SYMBOLIC: {
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsNonMatrix();
    return (function_value && function_value->representsNonMatrix()) ||
           o_function->representsNonMatrix(*this);
  }
  case STRUCT_INVERSE: {
  }
  case STRUCT_NEGATE: {
...
bool MathStructure::representsScalar() const {
  ...
  case STRUCT_POWER: {
    return CHILD(0).representsScalar();
  }
  case STRUCT_VARIABLE: {
    return o_variable->representsScalar();
  }
  case STRUCT_SYMBOLIC: {
    return CALCULATOR->defaultAssumptions()->isNonMatrix();
  }
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
      return CHILD(0).representsScalar();
    return (function_value && function_value->representsScalar()) ||
           o_function->representsScalar(*this);
  }
  case STRUCT_INVERSE: {
  }
  case STRUCT_NEGATE: {
...
void MathStructure::setApproximate(bool is_approx, bool recursive) {
  ...
  if (recursive) {
    if (m_type == STRUCT_NUMBER) {
      ...
      if (i_precision < 0 || i_precision > o_number.precision())
        i_precision = o_number.precision();
    }
    for (size_t i = 0; i < SIZE; i++) {
...
bool MathStructure::isApproximate() const { return b_approx; }

int MathStructure::precision() const { return i_precision; }
...
void MathStructure::add(const MathStructure &o, MathOperation op, bool append) {
  ...
  case OPERATION_EQUALS_LESS: {
    ...
    } else if (append && m_type == STRUCT_LOGICAL_AND &&
               LAST.type() == STRUCT_COMPARISON) {
...
void MathStructure::raise(const Number &o) { transform(STRUCT_POWER, o); }
...
void MathStructure::raise(int i) { transform(STRUCT_POWER, i); }
...
void MathStructure::raise(Variable *v) { transform(STRUCT_POWER, v); }
...
void MathStructure::raise(Unit *u) { transform(STRUCT_POWER, u); }
...
void MathStructure::raise(string sym) { transform(STRUCT_POWER, sym); }
void MathStructure::add_nocopy(MathStructure *o, MathOperation op,
                               bool append) {
  switch (op) {
  case OPERATION_ADD: {
    add_nocopy(o, append);
...
void MathStructure::setLogicalNot() { transform(STRUCT_LOGICAL_NOT); }
void MathStructure::setBitwiseNot() { transform(STRUCT_BITWISE_NOT); }

bool MathStructure::equals(const MathStructure &o, bool allow_interval,
  ...
  case STRUCT_SYMBOLIC: {
    return s_sym == o.symbol();
  }
  case STRUCT_DATETIME: {
    return *o_datetime == *o.datetime();
  }
  case STRUCT_NUMBER: {
    return o_number.equals(o.number(), allow_interval, allow_infinite);
  }
  case STRUCT_VARIABLE: {
    return o_variable == o.variable();
  }
  case STRUCT_UNIT: {
    Prefix *p1 =
        (o_prefix == NULL || o_prefix == CALCULATOR->getDecimalNullPrefix() ||
         o_prefix == CALCULATOR->getBinaryNullPrefix())
            ? NULL
            : o_prefix;
    Prefix *p2 = (o.prefix() == NULL ||
                  o.prefix() == CALCULATOR->getDecimalNullPrefix() ||
                  o.prefix() == CALCULATOR->getBinaryNullPrefix())
                     ? NULL
                     : o.prefix();
    return o_unit == o.unit() && p1 == p2;
  }
  case STRUCT_COMPARISON: {
    if (ct_comp != o.comparisonType())
      return false;
    break;
  }
  case STRUCT_FUNCTION: {
    if (o_function != o.function() || o_function->args() == 0)
      return false;
    break;
  }
  case STRUCT_LOGICAL_OR: {
  ...
  case STRUCT_LOGICAL_AND: {
    if (SIZE < 1)
      return false;
    if (SIZE == 2) {
      return (CHILD(0) == o[0] && CHILD(1) == o[1]) ||
             (CHILD(0) == o[1] && CHILD(1) == o[0]);
    }
  ...
  if (SIZE < 1)
    return true;
  for (size_t i = 0; i < SIZE; i++) {
    if (!CHILD(i).equals(o[i], allow_interval))
      return false;
  }
  return true;
}
bool MathStructure::equals(const Number &o, bool allow_interval,
                           bool allow_infinite) const {
  if (m_type != STRUCT_NUMBER)
    return false;
...
bool MathStructure::equals(Variable *v) const {
  if (m_type != STRUCT_VARIABLE)
    return false;
  return o_variable == v;
}
bool MathStructure::equals(string sym) const {
  if (m_type != STRUCT_SYMBOLIC)
    return false;
  return s_sym == sym;
}

bool remove_rad_unit_cf(MathStructure &m) {
  if (m.isFunction() && m.containsType(STRUCT_UNIT, false, true, true) <= 0)
    return false;
  for (size_t i = 0; i < m.size(); i++) {
    if (!remove_rad_unit_cf(m[i]))
      return false;
  }
  return true;
}
bool remove_rad_unit(MathStructure &m, const EvaluationOptions &eo, bool top) {
  if (top && !remove_rad_unit_cf(m))
    return false;
  if (m.isUnit()) {
    ...
    bool b = false;
    for (size_t i = 0; i < m.size(); i++) {
      if (remove_rad_unit(m[i], eo, false))
        b = true;
    }
    if (b) {
...
bool contains_unknown_possibly_with_unit(MathStructure &m) {
  if (m.isUnknown())
    return m.containsRepresentativeOfType(STRUCT_UNIT, true, true) != 0;
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_unknown_possibly_with_unit(m[i]))
      return true;
  }
  return false;
}
int compare_check_incompability(MathStructure *mtest) {
  if (contains_unknown_possibly_with_unit(*mtest))
    return -1;
  int incomp = 0;
  ...
  for (size_t i = 0; i < mtest->size(); i++) {
    ...
    } else if ((*mtest)[i].containsRepresentativeOfType(STRUCT_UNIT, true,
                                                        true) == 0) {
      not_unit_term_count++;
  ...
  if (b_not_number && unit_term_count > 0) {
    ...
    if ((long int)mtest->size() - (unit_term_count + not_unit_term_count) >=
        unit_term_count - compat_count + (not_unit_term_count > 0)) {
      incomp = -1;
...
bool replace_interval_unknowns(MathStructure &m, bool do_assumptions) {
  if (m.isVariable() && !m.variable()->isKnown()) {
    if (!((UnknownVariable *)m.variable())->interval().isUndefined()) {
      ...
      if (ass && ((ass->sign() != ASSUMPTION_SIGN_UNKNOWN &&
                   ass->sign() != ASSUMPTION_SIGN_NONZERO) ||
                  ass->min() || ass->max())) {
        Number nr_intval;
        if (ass->min()) {
          if (ass->max())
            nr_intval.setInterval(*ass->min(), *ass->max());
          else
            nr_intval.setInterval(*ass->min(), nr_plus_inf);
        } else if (ass->max()) {
          nr_intval.setInterval(nr_minus_inf, *ass->max());
        } else if (ass->sign() == ASSUMPTION_SIGN_NEGATIVE ||
                   ass->sign() == ASSUMPTION_SIGN_NONPOSITIVE) {
  ...
  bool b = false;
  for (size_t i = 0; i < m.size(); i++) {
    if (replace_interval_unknowns(m[i], do_assumptions))
      b = true;
  }
  return b;
}
...
bool MathStructure::mergeInterval(const MathStructure &o, bool set_to_overlap) {
  ...
  if (isMultiplication() && SIZE > 1 && CHILD(0).isNumber()) {
    if (o.isMultiplication() && o.size() > 1) {
      if (SIZE == o.size() + (o[0].isNumber() ? 0 : 1)) {
        ...
        for (size_t i = 1; i < SIZE; i++) {
          if (!CHILD(i).equals(o[0].isNumber() ? o[i] : o[i - 1]) ||
              !CHILD(i).representsNonNegative(true)) {
            b = false;
            break;
          }
        }
        if (b && o[0].isNumber()) {
          if (!CHILD(0).number().mergeInterval(o[0].number(), set_to_overlap))
            return false;
        } else if (b) {
          if (!CHILD(0).number().mergeInterval(nr_one, set_to_overlap))
            return false;
        }
    ...
    } else if (SIZE == 2 && o.equals(CHILD(1)) &&
               o.representsNonNegative(true)) {
      if (!CHILD(0).number().mergeInterval(nr_one, set_to_overlap))
        return false;
      CHILD(0).numberUpdated();
      CHILD_UPDATED(0);
  ...
  } else if (o.isMultiplication() && o.size() == 2 && o[0].isNumber() &&
             equals(o[1]) && representsNonNegative(true)) {
    Number nr(1, 1);
    if (!nr.mergeInterval(o[0].number(), set_to_overlap))
      return false;
...
ComparisonResult MathStructure::compare(const MathStructure &o) const {
  ...
  if (equals(o))
    return COMPARISON_RESULT_EQUAL;
  if (o.isZero()) {
    if (representsPositive(true))
      return COMPARISON_RESULT_LESS;
    if (representsNegative(true))
      return COMPARISON_RESULT_GREATER;
  }
  if (isMultiplication() && SIZE > 1 && CHILD(0).isNumber()) {
    if (o.isMultiplication() && o.size() > 1) {
      if (SIZE == o.size() + (o[0].isNumber() ? 0 : 1)) {
        ...
        for (size_t i = 1; i < SIZE; i++) {
          if (!CHILD(i).equals(o[0].isNumber() ? o[i] : o[i - 1]) ||
              !CHILD(i).representsPositive(true)) {
            b = false;
            break;
          }
        }
        if (b && o[0].isNumber())
          return CHILD(0).number().compare(o[0].number());
        else if (b)
      ...
      return CHILD(0).number().compare(nr_one);
             ...
             equals(o[1]) && representsPositive(true)) {
    return nr_one.compare(o[0].number());
  }
  if (o.representsReal(true) && representsComplex(true))
    return COMPARISON_RESULT_NOT_EQUAL;
  if (representsReal(true) && o.representsComplex(true))
    return COMPARISON_RESULT_NOT_EQUAL;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (incomp == 0) {
      ...
      if (mtest.representsNonZero(true))
        return COMPARISON_RESULT_NOT_EQUAL;
      if (mtest.representsNonPositive(true))
...
ComparisonResult
MathStructure::compareApproximately(const MathStructure &o,
                                    const EvaluationOptions &eo2) const {
  if (isNumber() && o.isNumber()) {
  ...
  if (isMultiplication() && SIZE > 1 && CHILD(0).isNumber()) {
    if (o.isMultiplication() && o.size() > 1) {
      if (SIZE == o.size() + (o[0].isNumber() ? 0 : 1)) {
        ...
        if (b && o[0].isNumber())
          return CHILD(0).number().compareApproximately(o[0].number());
        else if (b)
  ...
  if (o.representsZero(true) && representsZero(true))
    return COMPARISON_RESULT_EQUAL;
  if (o.representsReal(true) && representsComplex(true))
    return COMPARISON_RESULT_NOT_EQUAL;
  if (representsReal(true) && o.representsComplex(true))
  ...
  for (int i = 0; i < 2; i++) {
    ...
    CALCULATOR->endTemporaryStopMessages();
    if (mtest.equals(mtest2))
      return COMPARISON_RESULT_EQUAL;
    if (mtest.representsZero(true) && mtest2.representsZero(true))
      return COMPARISON_RESULT_EQUAL;
    if (mtest.isNumber() && mtest2.isNumber()) {
      if (mtest2.isApproximate() && o.isAddition() && o.size() > 1 &&
          mtest.isZero() && !mtest2.isZero()) {
        CALCULATOR->beginTemporaryStopMessages();
        mtest = *this;
        mtest.subtract(o[0]);
        mtest2 = o;
      ...
      } else if (mtest.isApproximate() && isAddition() && SIZE > 1 &&
                 mtest2.isZero() && !mtest.isZero()) {
        CALCULATOR->beginTemporaryStopMessages();
        mtest2 = o;
    ...
    if (mtest2.isZero() && mtest.isMultiplication() && mtest.size() > 0 &&
        mtest[0].isNumber()) {
      bool b = true;
      for (size_t i = 1; i < SIZE; i++) {
      ...
      if (b)
        return CHILD(0).number().compareApproximately(o.number());
    }
    if (!mtest2.isZero()) {
      CALCULATOR->beginTemporaryStopMessages();
...
bool MathStructure::containsOpaqueContents() const {
  if (isFunction())
    return true;
  if (isUnit() && o_unit->subtype() != SUBTYPE_BASE_UNIT)
    return true;
  if (isVariable() && o_variable->isKnown())
    return true;
  for (size_t i = 0; i < SIZE; i++) {
    if (CHILD(i).containsOpaqueContents())
      return true;
  }
  return false;
}
bool MathStructure::containsAdditionPower() const {
  if (m_type == STRUCT_POWER && CHILD(0).isAddition())
    return true;
  for (size_t i = 0; i < SIZE; i++) {
    if (CHILD(i).containsAdditionPower())
      return true;
  }
  return false;
}
...
size_t MathStructure::countTotalChildren(bool count_function_as_one) const {
  if ((m_type == STRUCT_FUNCTION && count_function_as_one) || SIZE == 0)
    return 1;
  size_t count = 0;
  for (size_t i = 0; i < SIZE; i++) {
...
void MathStructure::addChild(const MathStructure &o) { APPEND(o); }
void MathStructure::addChild_nocopy(MathStructure *o) { APPEND_POINTER(o); }
...
void MathStructure::setChild_nocopy(MathStructure *o, size_t index,
  ...
  if (index > 0 && index <= SIZE) {
    ...
    if (merge_precision) {
      if (!o->isApproximate() && o_prev->isApproximate())
        o->setApproximate(true);
      if (o_prev->precision() >= 0 &&
          (o->precision() < 0 || o_prev->precision() < o->precision()))
        o->setPrecision(o_prev->precision());
    }
    o_prev->unref();
...
size_t MathStructure::countChildren() const { return SIZE; }
size_t MathStructure::size() const { return SIZE; }
...
StructureType MathStructure::type() const { return m_type; }

int contains_angle_unit(const MathStructure &m, const ParseOptions &po,
                        int check_functions) {
  if (m.isUnit() &&
      m.unit()->baseUnit() == CALCULATOR->getRadUnit()->baseUnit() &&
      m.unit()->baseExponent() == 1)
    return 1;
  if (m.isVariable() && m.variable()->isKnown())
    return contains_angle_unit(((KnownVariable *)m.variable())->get(), po,
                               check_functions);
  if (m.isFunction()) {
    if (m.function()->id() == FUNCTION_ID_ASIN ||
        m.function()->id() == FUNCTION_ID_ACOS ||
        m.function()->id() == FUNCTION_ID_ATAN ||
        m.function()->id() == FUNCTION_ID_ATAN2 ||
        m.function()->id() == FUNCTION_ID_ARG) {
      if (NO_DEFAULT_ANGLE_UNIT(po.angle_unit))
        return 1;
      return 0;
    }
    if (!check_functions)
      return 0;
    if (m.containsType(STRUCT_UNIT, false, true, true) == 0)
      return 0;
    if (check_functions > 1 && m.size() == 0)
      return -1;
  }
  int ret = 0;
  for (size_t i = 0; i < m.size(); i++) {
...
int MathStructure::contains(const MathStructure &mstruct, bool structural_only,
                            bool check_variables, bool check_functions,
                            bool loose_equals) const {
  if (mstruct.isUnit() && mstruct.prefix() == NULL && m_type == STRUCT_UNIT)
    return mstruct.unit() == o_unit;
  if (equals(mstruct, loose_equals, loose_equals))
    return 1;
  if (structural_only) {
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).contains(mstruct, structural_only, check_variables,
                            check_functions, loose_equals))
        return 1;
    }
    if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown()) {
      return ((KnownVariable *)o_variable)
          ->get()
          .contains(mstruct, structural_only, check_variables, check_functions,
                    loose_equals);
    } else if (m_type == STRUCT_FUNCTION && check_functions) {
      if (function_value) {
        return function_value->contains(mstruct, structural_only,
                                        check_variables, check_functions,
                                        loose_equals);
      }
    }
  } else {
    ...
    if (m_type != STRUCT_FUNCTION) {
      for (size_t i = 0; i < SIZE; i++) {
        int retval =
            CHILD(i).contains(mstruct, structural_only, check_variables,
                              check_functions, loose_equals);
        if (retval == 1)
          return 1;
        else if (retval < 0)
...
size_t MathStructure::countOccurrences(const MathStructure &mstruct,
                                       bool check_variables) const {
  if (mstruct.isUnit() && mstruct.prefix() == NULL && m_type == STRUCT_UNIT &&
      mstruct.unit() == o_unit)
    return 1;
  ...
  if (check_variables && m_type == STRUCT_VARIABLE && o_variable->isKnown())
    return ((KnownVariable *)o_variable)->get().countOccurrences(mstruct, true);
  size_t i_occ = 0;
  for (size_t i = 0; i < SIZE; i++) {
...
int MathStructure::containsFunction(MathFunction *f, bool structural_only,
                                    bool check_variables,
                                    bool check_functions) const {
  if (m_type == STRUCT_FUNCTION && o_function == f)
    return 1;
  if (structural_only) {
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).containsFunction(f, structural_only, check_variables,
                                    check_functions))
        return 1;
    }
    if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown()) {
      return ((KnownVariable *)o_variable)
          ->get()
          .containsFunction(f, structural_only, check_variables,
                            check_functions);
    } else if (m_type == STRUCT_FUNCTION && check_functions) {
      if (function_value) {
        return function_value->containsFunction(
            f, structural_only, check_variables, check_functions);
      }
    }
  } else {
    int ret = 0;
    if (m_type != STRUCT_FUNCTION) {
      for (size_t i = 0; i < SIZE; i++) {
        int retval = CHILD(i).containsFunction(
            f, structural_only, check_variables, check_functions);
        if (retval == 1)
          return 1;
        else if (retval < 0)
...
int MathStructure::containsFunctionId(int id, bool structural_only,
                                      bool check_variables,
                                      bool check_functions) const {
  if (m_type == STRUCT_FUNCTION && o_function->id() == id)
    return 1;
  if (structural_only) {
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).containsFunctionId(id, structural_only, check_variables,
                                      check_functions))
        return 1;
    }
    if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown()) {
      return ((KnownVariable *)o_variable)
          ->get()
          .containsFunctionId(id, structural_only, check_variables,
                              check_functions);
    } else if (m_type == STRUCT_FUNCTION && check_functions) {
      if (function_value) {
        return function_value->containsFunctionId(
            id, structural_only, check_variables, check_functions);
      }
    }
  } else {
    int ret = 0;
    if (m_type != STRUCT_FUNCTION) {
      for (size_t i = 0; i < SIZE; i++) {
        int retval = CHILD(i).containsFunctionId(
            id, structural_only, check_variables, check_functions);
        if (retval == 1)
          return 1;
        else if (retval < 0)
...
int contains_interval_var(const MathStructure &m, bool structural_only,
  ...
  if (m.type() == STRUCT_NUMBER) {
    if (m.number().isInterval(false)) {
      if (ignore_high_precision_interval != 0) {
        if (m.number().precision(true) >
            (ignore_high_precision_interval < 0
                 ? (ignore_high_precision_interval == -1
                        ? PRECISION + 29
                        : PRECISION - ignore_high_precision_interval)
  ...
  if (m.type() == STRUCT_FUNCTION &&
      (m.function()->id() == FUNCTION_ID_INTERVAL ||
       m.function()->id() == FUNCTION_ID_UNCERTAINTY))
    return include_interval_function;
  if (structural_only) {
    for (size_t i = 0; i < m.size(); i++) {
      if (contains_interval_var(m[i], structural_only, check_variables,
                                check_functions, ignore_high_precision_interval,
                                include_interval_function))
        return 1;
    }
    if (m.type() == STRUCT_VARIABLE && check_variables &&
        m.variable()->isKnown()) {
      ...
      if (m.functionValue()) {
        return contains_interval_var(*m.functionValue(), structural_only,
                                     check_variables, check_functions,
                                     ignore_high_precision_interval,
                                     include_interval_function);
      }
    }
  } else {
    ...
    if (m.type() != STRUCT_FUNCTION) {
      for (size_t i = 0; i < m.size(); i++) {
        int retval = contains_interval_var(
...
int MathStructure::containsInterval(bool structural_only, bool check_variables,
                                    bool check_functions,
                                    int ignore_high_precision_interval,
                                    bool include_interval_function) const {
  if (m_type == STRUCT_NUMBER && o_number.isInterval(false)) {
    if (ignore_high_precision_interval != 0) {
      if (o_number.precision(true) >
          (ignore_high_precision_interval < 0
               ? (ignore_high_precision_interval == -1
                      ? PRECISION + 29
                      : PRECISION - ignore_high_precision_interval)
  ...
  if (structural_only) {
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).containsInterval(
              structural_only, check_variables, check_functions,
              ignore_high_precision_interval, include_interval_function))
        return 1;
    }
    if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown()) {
      ...
      return contains_interval_var(
          ((KnownVariable *)o_variable)->get(), structural_only,
          check_variables, check_functions, ignore_high_precision_interval,
          include_interval_function);
    } else if (m_type == STRUCT_FUNCTION && check_functions) {
      if (function_value) {
        return function_value->containsInterval(
            structural_only, check_variables, check_functions,
            ignore_high_precision_interval, include_interval_function);
      }
    }
    ...
    if (m_type != STRUCT_FUNCTION) {
      for (size_t i = 0; i < SIZE; i++) {
        int retval = CHILD(i).containsInterval(
            structural_only, check_variables, check_functions,
            ignore_high_precision_interval, include_interval_function);
        if (retval == 1)
          return 1;
...
int MathStructure::containsInfinity(bool structural_only, bool check_variables,
                                    bool check_functions) const {
  ...
  if (structural_only) {
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).containsInfinity(structural_only, check_variables,
                                    check_functions))
        return 1;
    }
    if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown()) {
      return ((KnownVariable *)o_variable)
          ->get()
          .containsInfinity(structural_only, check_variables, check_functions);
    } else if (m_type == STRUCT_FUNCTION && check_functions) {
      if (function_value) {
        return function_value->containsInfinity(
            structural_only, check_variables, check_functions);
      }
    }
  } else {
    ...
    if (m_type != STRUCT_FUNCTION) {
      for (size_t i = 0; i < SIZE; i++) {
        int retval = CHILD(i).containsInfinity(structural_only, check_variables,
                                               check_functions);
        if (retval == 1)
          return 1;
        else if (retval < 0)
...
int MathStructure::containsRepresentativeOf(const MathStructure &mstruct,
                                            bool check_variables,
                                            bool check_functions) const {
  if (equals(mstruct))
    return 1;
  ...
  if (m_type != STRUCT_FUNCTION) {
    for (size_t i = 0; i < SIZE; i++) {
      int retval = CHILD(i).containsRepresentativeOf(mstruct, check_variables,
                                                     check_functions);
      if (retval == 1)
        return 1;
      else if (retval < 0)
  ...
  if (m_type == STRUCT_VARIABLE && check_variables) {
    if (o_variable->isKnown())
      return ((KnownVariable *)o_variable)
          ->get()
          .containsRepresentativeOf(mstruct, check_variables, check_functions);
    else
      return ((UnknownVariable *)o_variable)
          ->interval()
          .containsRepresentativeOf(mstruct, check_variables, check_functions);
  } else if (m_type == STRUCT_FUNCTION && check_functions) {
    if (function_value) {
      return function_value->containsRepresentativeOf(mstruct, check_variables,
                                                      check_functions);
    }
    if (!mstruct.isNumber() &&
        (o_function->isBuiltin() || representsNumber())) {
      for (size_t i = 0; i < SIZE; i++) {
        int retval = CHILD(i).containsRepresentativeOf(mstruct, check_variables,
                                                       check_functions);
        if (retval != 0)
...
int MathStructure::containsType(StructureType mtype, bool structural_only,
                                bool check_variables,
                                bool check_functions) const {
  if (m_type == mtype)
    return 1;
  if (structural_only) {
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).containsType(mtype, true, check_variables, check_functions))
        return 1;
    }
    if (check_variables && m_type == STRUCT_VARIABLE && o_variable->isKnown()) {
      return ((KnownVariable *)o_variable)
          ->get()
          .containsType(mtype, false, check_variables, check_functions);
    } else if (check_functions && m_type == STRUCT_FUNCTION) {
      if (function_value) {
        return function_value->containsType(mtype, false, check_variables,
                                            check_functions);
      }
    }
    return 0;
    ...
    if (m_type != STRUCT_FUNCTION) {
      for (size_t i = 0; i < SIZE; i++) {
        int retval = CHILD(i).containsType(mtype, false, check_variables,
                                           check_functions);
        if (retval == 1)
          return 1;
        else if (retval < 0)
    ...
    if (check_variables && m_type == STRUCT_VARIABLE) {
      if (o_variable->isKnown())
        return ((KnownVariable *)o_variable)
            ->get()
            .containsType(mtype, false, check_variables, check_functions);
      else if (!((UnknownVariable *)o_variable)->interval().isUndefined())
        return ((UnknownVariable *)o_variable)
            ->interval()
            .containsType(mtype, false, check_variables, check_functions);
      ...
      if (mtype == STRUCT_UNIT) {
        if (o_function->id() == FUNCTION_ID_STRIP_UNITS)
          return 0;
        if (o_function->subtype() == SUBTYPE_USER_FUNCTION ||
            o_function->subtype() == SUBTYPE_DATA_SET ||
            o_function->id() == FUNCTION_ID_REGISTER ||
            o_function->id() == FUNCTION_ID_STACK ||
            o_function->id() == FUNCTION_ID_LOAD)
          return -1;
        // (NO_DEFAULT_ANGLE_UNIT(eo.parse_options.angle_unit) &&
        // (o_function->id() == FUNCTION_ID_ASIN || o_function->id() ==
        // FUNCTION_ID_ACOS || o_function->id() == FUNCTION_ID_ATAN ||
        // o_function->id() == FUNCTION_ID_RADIANS_TO_DEFAULT_ANGLE_UNIT ||
        // o_function->id() == FUNCTION_ID_ARG || o_function->id() ==
        // FUNCTION_ID_ATAN2))
        if (o_function->id() == FUNCTION_ID_LOG ||
            o_function->id() == FUNCTION_ID_LOGN ||
            o_function->id() == FUNCTION_ID_GAMMA ||
            o_function->id() == FUNCTION_ID_BETA ||
            o_function->id() == FUNCTION_ID_FACTORIAL ||
            o_function->id() == FUNCTION_ID_BESSELJ ||
            o_function->id() == FUNCTION_ID_BESSELY ||
            o_function->id() == FUNCTION_ID_ERF ||
            o_function->id() == FUNCTION_ID_ERFI ||
            o_function->id() == FUNCTION_ID_ERFC ||
            o_function->id() == FUNCTION_ID_LOGINT ||
            o_function->id() == FUNCTION_ID_POLYLOG ||
            o_function->id() == FUNCTION_ID_EXPINT ||
            o_function->id() == FUNCTION_ID_SININT ||
            o_function->id() == FUNCTION_ID_COSINT ||
            o_function->id() == FUNCTION_ID_SINHINT ||
            o_function->id() == FUNCTION_ID_COSHINT ||
            o_function->id() == FUNCTION_ID_FRESNEL_C ||
            o_function->id() == FUNCTION_ID_FRESNEL_S ||
            o_function->id() == FUNCTION_ID_SIGNUM ||
            o_function->id() == FUNCTION_ID_HEAVISIDE ||
            o_function->id() == FUNCTION_ID_LAMBERT_W ||
            o_function->id() == FUNCTION_ID_SINC ||
            o_function->id() == FUNCTION_ID_SIN ||
            o_function->id() == FUNCTION_ID_COS ||
            o_function->id() == FUNCTION_ID_TAN ||
            o_function->id() == FUNCTION_ID_SINH ||
            o_function->id() == FUNCTION_ID_COSH ||
            o_function->id() == FUNCTION_ID_TANH ||
            o_function->id() == FUNCTION_ID_ASINH ||
            o_function->id() == FUNCTION_ID_ACOSH ||
            o_function->id() == FUNCTION_ID_ATANH ||
            o_function->id() == FUNCTION_ID_ASIN ||
            o_function->id() == FUNCTION_ID_ACOS ||
            o_function->id() == FUNCTION_ID_ATAN)
          return 0;
        int ret = 0;
        for (size_t i = 0; i < SIZE; i++) {
          int ret_i = CHILD(i).containsType(mtype, false, check_variables,
                                            check_functions);
          if (ret_i > 0)
            return ret_i;
          else if (ret_i < 0)
...
int MathStructure::containsRepresentativeOfType(StructureType mtype,
                                                bool check_variables,
                                                bool check_functions) const {
  if (m_type == mtype)
    return 1;
  ...
  if (m_type != STRUCT_FUNCTION) {
    for (size_t i = 0; i < SIZE; i++) {
      int retval = CHILD(i).containsRepresentativeOfType(mtype, check_variables,
                                                         check_functions);
      if (retval == 1)
        return 1;
      else if (retval < 0)
  ...
  if (check_variables && m_type == STRUCT_VARIABLE && o_variable->isKnown()) {
    return ((KnownVariable *)o_variable)
        ->get()
        .containsRepresentativeOfType(mtype, check_variables, check_functions);
  } else if (check_functions && m_type == STRUCT_FUNCTION) {
    if (function_value) {
      return function_value->containsRepresentativeOfType(
          mtype, check_variables, check_functions);
    }
  }
  if (m_type == STRUCT_SYMBOLIC || m_type == STRUCT_VARIABLE ||
      m_type == STRUCT_FUNCTION || m_type == STRUCT_ABORTED) {
...
bool MathStructure::containsUnknowns() const {
  if (isUnknown())
    return true;
  for (size_t i = 0; i < SIZE; i++) {
    if (CHILD(i).containsUnknowns())
      return true;
  }
  return false;
}
bool MathStructure::containsDivision() const {
  if (m_type == STRUCT_DIVISION || m_type == STRUCT_INVERSE ||
      (m_type == STRUCT_POWER && CHILD(1).hasNegativeSign()))
    return true;
  for (size_t i = 0; i < SIZE; i++) {
    if (CHILD(i).containsDivision())
      return true;
  }
  return false;
}
...
void MathStructure::findAllUnknowns(MathStructure &unknowns_vector) {
  if (!unknowns_vector.isVector())
    unknowns_vector.clearVector();
  switch (m_type) {
  case STRUCT_VARIABLE: {
    if (o_variable->isKnown()) {
      break;
...
bool MathStructure::replace(const MathStructure &mfrom,
                            const MathStructure &mto, bool once_only,
                            bool exclude_function_arguments) {
  return replace(mfrom, mto, once_only, exclude_function_arguments, false);
}
bool MathStructure::replace(const MathStructure &mfrom,
                            const MathStructure &mto, bool once_only,
                            bool exclude_function_arguments,
                            bool replace_in_variables) {
  if (b_protected)
    b_protected = false;
  ...
  if (replace_in_variables && m_type == STRUCT_VARIABLE &&
      o_variable->isKnown()) {
    if (((KnownVariable *)o_variable)
            ->get()
            .contains(mfrom, !exclude_function_arguments, true, false, true) >
        0) {
      MathStructure m(((KnownVariable *)o_variable)->get());
      if (!m.isAborted() &&
          m.replace(mfrom, mto, once_only, exclude_function_arguments, true)) {
        if (!o_variable->isRegistered()) {
          Variable *v =
              CALCULATOR->getActiveVariable(o_variable->referenceName());
          if (v->isKnown() &&
              ((KnownVariable *)v)->get().equals(m, true, true)) {
            set(v);
            return true;
          }
        }
        KnownVariable *var =
            new KnownVariable("", o_variable->referenceName(), m);
        set(var);
        var->destroy();
        return true;
      }
  ...
  if (mfrom.size() > 0 && mfrom.type() == m_type && SIZE > mfrom.size() &&
      (mfrom.isAddition() || mfrom.isMultiplication() || mfrom.isLogicalAnd() ||
       mfrom.isLogicalOr())) {
    bool b = true;
    size_t i2 = 0;
    for (size_t i = 0; i < mfrom.size(); i++) {
      b = false;
      for (; i2 < SIZE; i2++) {
        if (CHILD(i2).equals(mfrom[i], true, true)) {
          b = true;
          break;
        }
      }
    ...
    if (b) {
      ...
      if (SIZE == 1)
        ...
        replace(mfrom, mto, once_only, exclude_function_arguments,
                replace_in_variables);
      if (mfrom.isAddition())
        add(mto);
      else if (mfrom.isMultiplication())
        multiply(mto);
      else if (mfrom.isLogicalAnd())
        transform(STRUCT_LOGICAL_AND, mto);
      else if (mfrom.isLogicalOr())
        transform(STRUCT_LOGICAL_OR, mto);
      return true;
    }
  }
  if (exclude_function_arguments && m_type == STRUCT_FUNCTION)
    return false;
  bool b = false;
  for (size_t i = 0; i < SIZE; i++) {
...
bool MathStructure::calculateReplace(const MathStructure &mfrom,
                                     const MathStructure &mto,
  ...
  if (mfrom.size() > 0 && mfrom.type() == m_type && SIZE > mfrom.size() &&
    ...
    if (b) {
      ...
      if (SIZE == 1)
        ...
        calculateReplace(mfrom, mto, eo, exclude_function_arguments);
      if (mfrom.isAddition())
        add(mto);
      else if (mfrom.isMultiplication())
        multiply(mto);
      ...
      if (eo.calculate_functions && m_type == STRUCT_FUNCTION)
...
bool MathStructure::replace(const MathStructure &mfrom1,
                            const MathStructure &mto1,
                            const MathStructure &mfrom2,
                            const MathStructure &mto2) {
...
bool MathStructure::removeType(StructureType mtype) {
  ...
  if (m_type == STRUCT_MULTIPLICATION) {
    ...
    if (m_type == STRUCT_FUNCTION) {
      if (mtype != STRUCT_UNIT || (o_function->id() != FUNCTION_ID_SQRT &&
                                   o_function->id() != FUNCTION_ID_ROOT &&
                                   o_function->id() != FUNCTION_ID_CBRT))
        return b;
    }
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).removeType(mtype)) {
  ...
  for (size_t i = 0; i < SIZE; i++) {
    ...
    if (mstruct->isVariable()) {
      if (!((UnknownVariable *)mstruct->variable())->interval().isUndefined()) {
        if (x_mstruct->isUndefined())
          x_mstruct = mstruct;
      } else if (mstruct->variable() ==
                 CALCULATOR->getVariableById(VARIABLE_ID_X)) {
        return *mstruct;
      } else if (!x_mstruct->isVariable()) {
        x_mstruct = mstruct;
      } else if (mstruct->variable() ==
                 CALCULATOR->getVariableById(VARIABLE_ID_Y)) {
        x_mstruct = mstruct;
      } else if (mstruct->variable() ==
                     CALCULATOR->getVariableById(VARIABLE_ID_Z) &&
                 x_mstruct->variable() !=
                     CALCULATOR->getVariableById(VARIABLE_ID_Y)) {
        x_mstruct = mstruct;
      }
    } else if (mstruct->isSymbolic()) {
      if (!x_mstruct->isVariable() &&
          (!x_mstruct->isSymbolic() ||
           x_mstruct->symbol() > mstruct->symbol())) {
        x_mstruct = mstruct;
      }
    }
  }
...
bool flattenMultiplication(MathStructure &mstruct, bool recursive) {
  ...
  if (recursive) {
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (flattenMultiplication(mstruct[i], true))
        retval = true;
    }
  }
