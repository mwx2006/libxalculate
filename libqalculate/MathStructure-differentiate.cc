...
bool function_differentiable(MathFunction *o_function) {
  return (o_function->id() == FUNCTION_ID_SQRT ||
          o_function->id() == FUNCTION_ID_ROOT ||
          o_function->id() == FUNCTION_ID_CBRT ||
          o_function->id() == FUNCTION_ID_LOG ||
          o_function->id() == FUNCTION_ID_LOGN ||
          o_function->id() == FUNCTION_ID_ARG ||
          o_function->id() == FUNCTION_ID_GAMMA ||
          o_function->id() == FUNCTION_ID_I_GAMMA ||
          o_function->id() == FUNCTION_ID_BETA ||
          o_function->id() == FUNCTION_ID_INCOMPLETE_BETA ||
          o_function->id() == FUNCTION_ID_ABS ||
...
bool is_differentiable(const MathStructure &m) {
  if (m.isFunction() && !function_differentiable(m.function()))
    return false;
  for (size_t i = 0; i < m.size(); i++) {
    if (!is_differentiable(m[i]))
      return false;
  }
  return true;
}
...
bool MathStructure::differentiate(const MathStructure &x_var,
  ...
  case STRUCT_LOGICAL_AND: {
    ...
    if (b) {
      ...
      if (mtest.differentiate(x_var, eo) &&
          mtest.containsFunctionId(FUNCTION_ID_DIFFERENTIATE, true) <= 0) {
        set(mtest);
        break;
      }
    }
  ...
  case STRUCT_COMPARISON: {
    if (ct_comp == COMPARISON_GREATER || ct_comp == COMPARISON_EQUALS_GREATER ||
        ct_comp == COMPARISON_LESS || ct_comp == COMPARISON_EQUALS_LESS) {
      // f(x)>=0 = sgn(heaviside(f)) :
      // sgn(heaviside(f))'=f'*2*dirac(f)*dirac(heaviside(f)) f(x)<0 =
      // sgn(-heaviside(f)) :
      // sgn(heaviside(-f))'=f'*-2*dirac(f)*dirac(heaviside(-f)) f(x)<=0 =
      // sgn(heaviside(-f)) :
      // sgn(-heaviside(f))'=f'*-2*dirac(f)*dirac(heaviside(f)) f(x)>0 =
      // sgn(-heaviside(-f)) :
      // sgn(-heaviside(-f))'=f'*2*dirac(f)*dirac(heaviside(-f))
  ...
  case STRUCT_POWER: {
    ...
    if (x_in_base && !x_in_exp) {
      ...
      if (!CHILD(1).isNumber() || !CHILD(1).number().add(-1))
        CHILD(1) += m_minus_one;
      ...
      if (!CHILD(0).isVariable() ||
        ...
        MathStructure *mstruct = new MathStructure(
            CALCULATOR->getFunctionById(FUNCTION_ID_LOG), &CHILD(0), NULL);
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_SQRT && SIZE == 1) {
    ...
    } else if ((o_function->id() == FUNCTION_ID_LOG && SIZE == 1) ||
               (o_function->id() == FUNCTION_ID_LOGN && SIZE == 2 &&
                CHILD(1).isVariable() &&
                CHILD(1).variable()->id() == VARIABLE_ID_E)) {
      if (CHILD(0) == x_var) {
        // ln(x)'=1/x
        setToChild(1, true);
        inverse();
    ...
    } else if (o_function->id() == FUNCTION_ID_INCOMPLETE_BETA && SIZE == 3 &&
               CHILD(0).containsRepresentativeOf(x_var, true, true) != 0 &&
               CHILD(1).containsRepresentativeOf(x_var, true, true) == 0 &&
               CHILD(2).containsRepresentativeOf(x_var, true, true) == 0) {
      ...
      mstruct.differentiate(x_var, eo);
      multiply(mstruct);
    } else if (o_function->id() == FUNCTION_ID_I_GAMMA && SIZE == 2 &&
               CHILD(0).containsRepresentativeOf(x_var, true, true) == 0) {
      ...
      mstruct.differentiate(x_var, eo);
      multiply(mstruct);
    } else if (o_function->id() == FUNCTION_ID_BESSELJ && SIZE == 2 &&
               CHILD(0).isInteger()) {
      ...
      multiply(mstruct);
      multiply(nr_half);
    } else if (o_function->id() == FUNCTION_ID_BESSELY && SIZE == 2 &&
               CHILD(0).isInteger()) {
      ...
      MathStructure mdiff(CHILD(0));
      if (o_function->getDefaultValue(2) == "pi")
        CHILD(0) /= CALCULATOR->getVariableById(VARIABLE_ID_PI);
      mdiff.differentiate(x_var, eo);
      multiply(mdiff);
      ...
      if (o_function->getDefaultValue(2) == "pi")
        CHILD(0) *= CALCULATOR->getVariableById(VARIABLE_ID_PI);
      MathStructure m_cos(CALCULATOR->getFunctionById(FUNCTION_ID_COS),
                          &CHILD(0), NULL);
      m_cos[0].multiply(CALCULATOR->getRadUnit());
      m_cos.divide(CHILD(0));
      MathStructure m_sin(CALCULATOR->getFunctionById(FUNCTION_ID_SIN),
                          &CHILD(0), NULL);
    ...
    } else if (o_function->id() == FUNCTION_ID_INTEGRATE && SIZE >= 4 &&
               CHILD(3) == x_var && CHILD(1).isUndefined() &&
               CHILD(2).isUndefined()) {
      // integrate(f,undefined,undefined,x)'=f
      SET_CHILD_MAP(0);
    } else if (o_function->id() == FUNCTION_ID_DIFFERENTIATE &&
               (SIZE == 3 || (SIZE == 4 && CHILD(3).isUndefined())) &&
               CHILD(1) == x_var) {
      // diff(f,x,a)'=diff(f,x,a+1)
      CHILD(2) += m_one;
    } else if (o_function->id() == FUNCTION_ID_DIFFERENTIATE && SIZE == 2 &&
               CHILD(1) == x_var) {
      // diff(f,x)'=diff(f,x,2)
      addChild(MathStructure(2, 1, 0));
    } else if (o_function->id() == FUNCTION_ID_DIFFERENTIATE && SIZE == 1 &&
               x_var == CALCULATOR->getVariableById(VARIABLE_ID_X)) {
      // diff(f)'=diff(f,x,2)
      addChild(x_var);
      addChild(MathStructure(2, 1, 0));
    } else {
  ...
  case STRUCT_MULTIPLICATION: {
    ...
    size_t idiv = 0;
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).isPower() && CHILD(i)[1].isNumber() &&
          CHILD(i)[1].number().isNegative()) {
        if (idiv == 0) {
