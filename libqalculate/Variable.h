...
/** Assumptions have a type and a sign. The type describes the type of the value
 * -- if it represents a number or something else, and what type of number is
 * represented. The sign restricts the signedness of a number. The sign
 * generally only applies the assumptions representing a number. The assumption
 * class also includes max and min values, which however are not used anywhere
 * yet.
 */
class Assumptions {
...
public:
  Variable(std::string cat_, std::string name_, std::string title_ = "",
  ...
  /** Returns the subtype of the variable, corresponding to which subsubclass
   ...
   *
   * @returns true if the variable is of class KnownVariable, false if
   * UnknownVariable.
   */
...
public:
   ...
   */
  UnknownVariable(std::string cat_, std::string name_, std::string title_ = "",
                  bool is_local = true, bool is_builtin = false,
                  bool is_active = true);
...
/** Known variables have an associated value. The value can be a simple number
 * or a full mathematical expression. The known variable class is used both for
 * variable values and constants.
 *
 * The value can be provided as an expression in the form of a text string or as
 * a mathematical value in the form of an object of the MathStructure class. The
 * text string is parsed when needed, which saves time when loading many
 * variable definitions which might not be used, at least not immediately.
 */
...
public:
   ...
   * @param is_active If the variable is active and can be used in expressions.
   */
  KnownVariable(std::string cat_, std::string name_, const MathStructure &o,
                std::string title_ = "", bool is_local = true,
   ...
   * @param is_active If the variable is active and can be used in expressions.
   */
  KnownVariable(std::string cat_, std::string name_, std::string expression_,
                std::string title_ = "", bool is_local = true,
                bool is_builtin = false, bool is_active = true);
  ...
  /** Returns the variable's string expression or an empty string if it has not
   * got an expression.
   *
  ...
  /** Returns the value of the variable. If no value is set or parsed and an
   * expression is set, the expression is parsed and resulting value returned.
   *
...
/// Abstract base class for variables with a value which is recalculated when
...
public:
  DynamicVariable(std::string cat_, std::string name_, std::string title_ = "",
                  bool is_local = false, bool is_builtin = true,
                  bool is_active = true);
   ...
   *
   * @returns Precision of the calculated value or zero if the value has not yet
   * been calculated.
   */
