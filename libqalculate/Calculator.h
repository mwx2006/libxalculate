 ...
 *
 * libqalculate is a math library for expression evaluation with units,
 *variables and functions support and CAS functionality.
 *
 * The main parts of the library is the almighty Calculator class, the
 *MathStructure class for mathematical expressions and classes for objects in an
 *expression, mostly of the class Numbers and sub classes of ExpressionItem.
 *
 * A simple application using libqalculate need only create a calculator object,
 *perhaps load definitions (functions, variables, units, etc.), and calculate
 *(and output) an expression as follows: \code new Calculator();
 * CALCULATOR->loadExchangeRates();
 * CALCULATOR->loadGlobalDefinitions();
 * CALCULATOR->loadLocalDefinitions();
 * cout << CALCULATOR->calculateAndPrint("1 + 1", 2000) << endl;\endcode
 * In the above example, the calculation is terminated after two seconds (2000
 *ms), if it is not finished before then. Applications using localized numbers
 *should first call Calculator::unlocalizeExpression() on the expression.
 *
 * A less simple application might calculate and output the expression
 *separately. \code EvaluationOptions eo; MathStructure result;
 * CALCULATOR->calculate(&mstruct, unlocalizeExpression("1 + 1"), 2000,
 *eo);\endcode
 *
 * More complex usage mainly involves manipulating objects of the MathStructure
 *class directly.
 *
 ...
 * Alternatively MathStructure::format() followed by MathStructure::print() can
 *be used, without the possibility to specify a time limit.
 *
 * Central to the flexibility of libqalculate is the many options passed to
 *evaluating and display functions with EvaluationOptions and PrintOptions.
 *
...
struct PlotParameters {
  ...
  std::string filename;
  /// The image type to save as. Set to PLOT_FILETYPE_AUTO to guess from file
  /// extension.
  ...
  bool color;
  /// If the minimum y-axis value shall be set automatically (otherwise uses
  /// y_min). Default: true
  bool auto_y_min;
  /// If the minimum x-axis value shall be set automatically (otherwise uses
  /// x_min). Default: true
  bool auto_x_min;
  /// If the maximum y-axis value shall be set automatically (otherwise uses
  /// y_max). Default: true
  bool auto_y_max;
  /// If the maximum x-axis value shall be set automatically (otherwise uses
  /// x_max). Default: true
  bool auto_x_max;
...
/// A message with information to the user. Primarily used for errors and
/// warnings.
...
public:
  CalculatorMessage(std::string message_, MessageType type_ = MESSAGE_WARNING,
                    int cat_ = MESSAGE_CATEGORY_NONE,
                    int stage_ = MESSAGE_STAGE_UNSET);
...
/** The calculator class is responsible for loading functions, variables and
 * units, and keeping track of them, as well as parsing expressions and much
 * more. A calculator object must be created before any other Qalculate! class
 * is used. There should never be more than one calculator object, accessed with
 * CALCULATOR.
 *
 * A simple application using libqalculate need only create a calculator object,
 * perhaps load definitions (functions, variables, units, etc.) and use the
 * calculate function as follows: \code new Calculator();
 * CALCULATOR->loadGlobalDefinitions();
 * CALCULATOR->loadLocalDefinitions();
...
protected:
  ...
  std::string NAME_NUMBER_PRE_S, NAME_NUMBER_PRE_STR, DOT_STR, DOT_S, COMMA_S,
      COMMA_STR, ILLEGAL_IN_NAMES, ILLEGAL_IN_UNITNAMES,
  ...
  std::string per_str, times_str, plus_str, minus_str, and_str, AND_str, or_str,
      OR_str, XOR_str;
  size_t per_str_len, times_str_len, plus_str_len, minus_str_len, and_str_len,
      AND_str_len, or_str_len, OR_str_len, XOR_str_len;

  std::vector<MathStructure *> rpn_stack;

  bool calculateRPN(MathStructure *mstruct, int command, size_t index,
                    int msecs, const EvaluationOptions &eo,
                    int function_arguments = 0);
  bool calculateRPN(std::string str, int command, size_t index, int msecs,
                    const EvaluationOptions &eo, MathStructure *parsed_struct,
                    MathStructure *to_struct, bool make_to_division,
                    int function_arguments = 0);

public:
  KnownVariable *v_pi, *v_e, *v_euler, *v_catalan, *v_i, *v_pinf, *v_minf,
      *v_undef, *v_precision;
  ...
  MathFunction *f_vector, *f_sort, *f_rank, *f_limits, *f_component,
      *f_dimension, *f_merge_vectors;
  MathFunction *f_matrix, *f_matrix_to_vector, *f_area, *f_rows, *f_columns,
      *f_row, *f_column, *f_elements, *f_element, *f_transpose, *f_identity,
      *f_determinant, *f_permanent, *f_adjoint, *f_cofactor, *f_inverse,
      *f_magnitude, *f_hadamard, *f_entrywise;
  ...
  MathFunction *f_abs, *f_gcd, *f_lcm, *f_signum, *f_heaviside, *f_dirac,
      *f_round, *f_floor, *f_ceil, *f_trunc, *f_int, *f_frac, *f_rem, *f_mod;
  MathFunction *f_polynomial_unit, *f_polynomial_primpart,
      *f_polynomial_content, *f_coeff, *f_lcoeff, *f_tcoeff, *f_degree,
      *f_ldegree;
  ...
  MathFunction *f_sin, *f_cos, *f_tan, *f_asin, *f_acos, *f_atan, *f_sinh,
      *f_cosh, *f_tanh, *f_asinh, *f_acosh, *f_atanh, *f_atan2, *f_sinc,
      *f_radians_to_default_angle_unit;
  MathFunction *f_zeta, *f_gamma, *f_digamma, *f_beta, *f_airy, *f_besselj,
      *f_bessely, *f_erf, *f_erfc;
  MathFunction *f_total, *f_percentile, *f_min, *f_max, *f_mode, *f_rand;
  MathFunction *f_date, *f_datetime, *f_timevalue, *f_timestamp, *f_stamptodate,
      *f_days, *f_yearfrac, *f_week, *f_weekday, *f_month, *f_day, *f_year,
      *f_yearday, *f_time, *f_add_days, *f_add_months, *f_add_years;
  ...
  MathFunction *f_genvector, *f_for, *f_sum, *f_product, *f_process,
      *f_process_matrix, *f_csum, *f_if, *f_is_number, *f_is_real,
      *f_is_rational, *f_is_integer, *f_represents_number, *f_represents_real,
      *f_represents_rational, *f_represents_integer, *f_function, *f_select;
  MathFunction *f_diff, *f_integrate, *f_solve, *f_multisolve, *f_dsolve,
      *f_limit;
  MathFunction *f_li, *f_Li, *f_Ei, *f_Si, *f_Ci, *f_Shi, *f_Chi, *f_igamma;
  MathFunction *f_error, *f_warning, *f_message, *f_save, *f_load, *f_export,
      *f_title;
  MathFunction *f_register, *f_stack;
  MathFunction *f_plot;

  Unit *u_rad, *u_gra, *u_deg, *u_euro, *u_btc, *u_second, *u_minute, *u_hour,
  ...
  //@{
  /** Calculates an expression. The expression should be unlocalized first with
   * unlocalizeExpression(). This function starts the calculation in a separate
   * thread and will return when the calculation has started unless a maximum
   * time has been specified. The calculation can then be stopped with abort().
   *
   * @param[out] mstruct Math structure to fill with the result.
   * @param str Expression.
   * @param msecs The maximum time for the calculation in milliseconds. If msecs
   * <= 0 the time will be unlimited.
   * @param eo Options for the evaluation and parsing of the expression.
   * @param[out] parsed_struct NULL or a math structure to fill with the result
   * of the parsing of the expression.
   * @param[out] to_struct NULL or a math structure to fill with unit expression
   * parsed after "to". If expression does not contain a "to" string, and
   * to_struct is a unit or a symbol (a unit expression string), to_struct will
   * be used instead.
   * @param make_to_division If true, the expression after "to" will be
   * interpreted as a unit expression to convert the result to.
   * @returns true if the calculation was successfully started (and finished if
   * msecs > 0).
   */
  bool calculate(MathStructure *mstruct, std::string str, int msecs,
                 const EvaluationOptions &eo = default_user_evaluation_options,
                 MathStructure *parsed_struct = NULL,
                 MathStructure *to_struct = NULL, bool make_to_division = true);
  /** Calculates an expression. The expression should be unlocalized first with
   ...
   * This function starts the calculation in a separate thread and will return
   * when the calculation has started unless a maximum time has been specified.
   ...
   * @returns The result of the calculation.
   */
  bool calculate(MathStructure *mstruct, int msecs,
                 const EvaluationOptions &eo = default_user_evaluation_options,
                 std::string to_str = "");
  /** Calculates a parsed value.
   *
   ...
   */
  MathStructure
  calculate(const MathStructure &mstruct,
            const EvaluationOptions &eo = default_user_evaluation_options,
            std::string to_str = "");
  /** Calculates an expression.and outputs the result to a text string. The
   * expression should be unlocalized first with unlocalizeExpression().
   *
   * Unlike other functions for expression evaluation this function handles
   * ending "to"-commands, in addition to unit conversion, such "to hexadecimal"
   * or to "fractions", similar to the qalc application.
   *
   *
   * @param str Expression.
   * @param msecs The maximum time for the calculation in milliseconds. If msecs
   ...
   * \since 2.6.0
   */
  std::string calculateAndPrint(
      std::string str, int msecs = 10000,
      const EvaluationOptions &eo = default_user_evaluation_options,
      const PrintOptions &po = default_print_options);
  std::string calculateAndPrint(std::string str, int msecs,
                                const EvaluationOptions &eo,
                                const PrintOptions &po,
                                std::string *parsed_expression);
  std::string calculateAndPrint(
      std::string str, int msecs, const EvaluationOptions &eo,
      const PrintOptions &po, AutomaticFractionFormat auto_fraction,
      AutomaticApproximation auto_approx = AUTOMATIC_APPROXIMATION_OFF,
      std::string *parsed_expression = NULL, int max_length = -1,
  ...
  std::string calculateAndPrint(std::string str, int msecs,
                                ...
                                AutomaticApproximation auto_approx,
                                std::string *parsed_expression, int max_length,
                                bool *result_is_comparison, bool format,
                                int colorize = 0, int tagtype = TAG_TYPE_HTML);
  int testCondition(std::string expression);
  //@}

  /** @name Functions for printing expressions with the option to set maximum
   * time or abort. */
  //@{
  /** Calls MathStructure::format(po) and MathStructure::print(po). The process
   * is aborted after msecs milliseconds.
   */
  std::string print(const MathStructure &mstruct, int milliseconds = 100000,
                    const PrintOptions &po = default_print_options);
  /** @name Functions for printing expressions with the option to set maximum
   * time or abort. */
  //@{
  /** Calls MathStructure::format(po) and MathStructure::print(po). The process
   * is aborted after msecs milliseconds. \since 4.0.0
   */
  std::string print(const MathStructure &mstruct, int milliseconds,
                    const PrintOptions &po, bool format, int colorize = 0,
                    int tagtype = TAG_TYPE_HTML);
  /// Deprecated: use print() instead
  std::string
  printMathStructureTimeOut(const MathStructure &mstruct,
                            int milliseconds = 100000,
                            const PrintOptions &op = default_print_options);

  /** Called before calculation, formatting or printing of a MathStructure
   * (Calculator::calculate(), without maximum time, MathStructure::eval(),
   * MathStructure::format() and MathStructure::print(), etc.) or printing of a
   * Number (using Number::print) to be able to abort the process. Always use
   * Calculator::stopControl() after finishing.
   *
   * @param milliseconds The maximum time for the process in milliseconds. If
   * msecs <= 0 the time will be unlimited (stop with abort()).
   */
  void startControl(int milliseconds = 0);
  /** Always call this function after Calculator::startControl() after
   * formatting, printing or calculation has finished.
   */
  void stopControl(void);
  /** Abort formatting, printing or evaluation (after startControl() has been
   * called). This function will normally be called from a thread that checks
   * for user input or other conditions.
   *
  ...
  void clearBuffers();
  /** Terminate calculation and print threads if started. Do not use to
   ...
   * msecs > 0).
   */
  bool calculateRPNRegister(
      size_t index, int msecs,
      const EvaluationOptions &eo = default_user_evaluation_options);
  /** Applies a mathematical operation to the first and second value on the RPN
   * stack. The the second value is changed with input from the first value. For
   * example, with OPERATION_SUBTRACT the first value is subtracted from the
   * second. The first value on the stack is removed. If not enough registers is
   * available, then zeros are added. This function starts the calculation in a
   * separate thread and will return when the calculation has started unless a
   * maximum time has been specified. The calculation can then be stopped with
   * abort().
   *
   * @param op Operation.
   * @param msecs The maximum time for the calculation in milliseconds. If msecs
   * <= 0 the time will be unlimited.
   * @param eo Options for the evaluation and parsing of the expression.
   * @param[out] parsed_struct NULL or a math structure to fill with the
   * unevaluated result.
   * @returns true if the calculation was successfully started (and finished if
   * msecs > 0).
   */
  bool
  calculateRPN(MathOperation op, int msecs,
               const EvaluationOptions &eo = default_user_evaluation_options,
               MathStructure *parsed_struct = NULL);
  /** Applies a mathematical operation to the first value on the RPN stack. The
   * value is set as the first argument of the function. If no register is
   * available, then zero is added. This function starts the calculation in a
   * separate thread and will return when the calculation has started unless a
   * maximum time has been specified. The calculation can then be stopped with
   * abort().
   *
   ...
   */
  bool
  calculateRPN(MathFunction *f, int msecs,
               const EvaluationOptions &eo = default_user_evaluation_options,
               MathStructure *parsed_struct = NULL);
   ...
   * msecs > 0).
   */
  bool calculateRPNBitwiseNot(
      int msecs, const EvaluationOptions &eo = default_user_evaluation_options,
      MathStructure *parsed_struct = NULL);
   ...
   * example, with OPERATION_SUBTRACT the first value is subtracted from the
   * second. The first value on the stack is removed. If not enough registers is
   * available, then zeros are added.
   *
   * @param op Operation.
   * @param eo Options for the evaluation and parsing of the expression.
   * @param[out] parsed_struct NULL or a math structure to fill with the
   ...
   */
  MathStructure *
  calculateRPN(MathOperation op,
               const EvaluationOptions &eo = default_user_evaluation_options,
               MathStructure *parsed_struct = NULL);
  /** Applies a mathematical operation to the first value on the RPN stack. The
   * value is set as the first argument of the function. If no register is
   * available, then zero is added.
   *
   * @param f Mathematical function.
   * @param eo Options for the evaluation and parsing of the expression.
   * @param[out] parsed_struct NULL or a math structure to fill with the
   ...
   * @returns The first value on the stack.
   */
  MathStructure *calculateRPNBitwiseNot(
      const EvaluationOptions &eo = default_user_evaluation_options,
      MathStructure *parsed_struct = NULL);
  /** Applies logical not to the first value on the RPN stack.
   * If no register is available, then zero is added.
   ...
   * @returns The first value on the stack.
   */
  MathStructure *calculateRPNLogicalNot(
      const EvaluationOptions &eo = default_user_evaluation_options,
      MathStructure *parsed_struct = NULL);
   ...
   */
  bool
  RPNStackEnter(MathStructure *mstruct, int msecs,
                const EvaluationOptions &eo = default_user_evaluation_options);
  /** Calculates an expression and adds the result first on the RPN stack. The
   * expression should be unlocalized first with unlocalizeExpression(). This
   * function starts the calculation in a separate thread and will return when
   * the calculation has started unless a maximum time has been specified. The
   * calculation can then be stopped with abort().
   *
   * @param str Expression.
   * @param msecs The maximum time for the calculation in milliseconds. If msecs
   * <= 0 the time will be unlimited.
   ...
   */
  bool
  RPNStackEnter(std::string str, int msecs,
                const EvaluationOptions &eo = default_user_evaluation_options,
                MathStructure *parsed_struct = NULL,
                MathStructure *to_struct = NULL, bool make_to_division = true);
   ...
   *
   * @param mstruct Value.
   * @param eval If true, the the mathematical structure will be evaluated
   * first.
   */
  void
  RPNStackEnter(MathStructure *mstruct, bool eval = false,
                const EvaluationOptions &eo = default_user_evaluation_options);
  /** Calculates an expression adds the result first on the RPN stack. The
   * expression should be unlocalized first with unlocalizeExpression().
   *
   * @param str Expression.
   * @param eo Options for the evaluation and parsing of the expression.
   ...
   */
  void
  RPNStackEnter(std::string str,
                const EvaluationOptions &eo = default_user_evaluation_options,
                MathStructure *parsed_struct = NULL,
                MathStructure *to_struct = NULL, bool make_to_division = true);
  bool
  setRPNRegister(size_t index, MathStructure *mstruct, int msecs,
                 const EvaluationOptions &eo = default_user_evaluation_options);
  bool
  setRPNRegister(size_t index, std::string str, int msecs,
                 const EvaluationOptions &eo = default_user_evaluation_options,
                 MathStructure *parsed_struct = NULL,
                 MathStructure *to_struct = NULL, bool make_to_division = true);
  void
  setRPNRegister(size_t index, MathStructure *mstruct, bool eval = false,
                 const EvaluationOptions &eo = default_user_evaluation_options);
  void
  setRPNRegister(size_t index, std::string str,
                 const EvaluationOptions &eo = default_user_evaluation_options,
                 MathStructure *parsed_struct = NULL,
                 MathStructure *to_struct = NULL, bool make_to_division = true);
  ...
  //@{
  /** Returns a localized expressions. Affects decimal signs and argument
   * separators.
   *
   ...
   */
  std::string
  localizeExpression(std::string str,
                     const ParseOptions &po = default_parse_options) const;
  /** Returns an unlocalized expressions. Affects decimal signs and argument
   * separators.
   *
   ...
   */
  std::string
  unlocalizeExpression(std::string str,
                       const ParseOptions &po = default_parse_options) const;
   ...
   * @param eo Options for the evaluation and parsing of the expression (nothing
   * will be done if units are not enabled).
   * @returns true if " to " was found and the expression split.
   */
  bool separateToExpression(std::string &str, std::string &to_str,
                            const EvaluationOptions &eo,
                            bool keep_modifiers = false,
                            bool allow_empty_from = false) const;
  bool hasToExpression(const std::string &str,
                       bool allow_empty_from = false) const;
  bool hasToExpression(const std::string &str, bool allow_empty_from,
                       const EvaluationOptions &eo) const;

  /// Split an expression string after and before " where ".
  bool separateWhereExpression(std::string &str, std::string &where_str,
                               const EvaluationOptions &eo) const;
  bool hasWhereExpression(const std::string &str,
                          const EvaluationOptions &eo) const;

  std::string parseComments(std::string &str,
                            const ParseOptions &po = default_parse_options,
                            bool *double_tag = NULL);
  void parseSigns(std::string &str,
                  bool convert_to_internal_representation = false) const;
   ...
   */
  MathStructure parse(std::string str,
                      const ParseOptions &po = default_parse_options);
  void parse(MathStructure *mstruct, std::string str,
             const ParseOptions &po = default_parse_options);
  bool parseNumber(MathStructure *mstruct, std::string str,
                   const ParseOptions &po = default_parse_options);
  bool parseOperators(MathStructure *mstruct, std::string str,
                      const ParseOptions &po = default_parse_options);
  bool parseAdd(std::string &str, MathStructure *mstruct,
                const ParseOptions &po, MathOperation s, bool append = true);
  bool parseAdd(std::string &str, MathStructure *mstruct,
   ...
   * @param[out] units NULL or a math structure to fill with the parsed unit
   * expression (or set to undefined if no units were found).
   * @returns Converted value.
   */
  MathStructure convert(const MathStructure &mstruct, std::string composite_,
                        const EvaluationOptions &eo, MathStructure *units,
                        bool transform_orig,
                        MathStructure *parsed_struct = NULL);
  MathStructure
  convert(const MathStructure &mstruct, std::string composite_,
          const EvaluationOptions &eo = default_user_evaluation_options,
   ...
   * @returns Converted value.
   */
  MathStructure convert(const MathStructure &mstruct, Unit *to_unit,
                        const EvaluationOptions &eo, bool always_convert,
                        bool convert_to_mixed_units, bool transform_orig,
                        MathStructure *parsed_struct = NULL);
  MathStructure
  convert(const MathStructure &mstruct, Unit *to_unit,
          const EvaluationOptions &eo = default_user_evaluation_options,
          bool always_convert = true, bool convert_to_mixed_units = true);
  MathStructure
  convert(const MathStructure &mstruct, KnownVariable *to_var,
          const EvaluationOptions &eo = default_user_evaluation_options);
  MathStructure
  convert(double value, Unit *from_unit, Unit *to_unit,
          const EvaluationOptions &eo = default_user_evaluation_options);
  MathStructure
  convert(std::string str, Unit *from_unit, Unit *to_unit, int milliseconds,
          const EvaluationOptions &eo = default_user_evaluation_options);
  /** Deprecated: use convert() */
  MathStructure
  convertTimeOut(std::string str, Unit *from_unit, Unit *to_unit,
                 int milliseconds,
                 const EvaluationOptions &eo = default_user_evaluation_options);
  MathStructure
  convert(std::string str, Unit *from_unit, Unit *to_unit,
          const EvaluationOptions &eo = default_user_evaluation_options);
  MathStructure convertToBaseUnits(
  ...
  Unit *getOptimalUnit(Unit *u, bool allow_only_div = false,
                       bool convert_to_local_currency = true);
  /** Deprecated: use convertToOptimalUnit() */
  MathStructure convertToBestUnit(
      const MathStructure &mstruct,
      const EvaluationOptions &eo = default_user_evaluation_options,
      bool convert_to_si_units = true);
  MathStructure convertToOptimalUnit(
      const MathStructure &mstruct,
      const EvaluationOptions &eo = default_user_evaluation_options,
      bool convert_to_si_units = true);
  MathStructure convertToCompositeUnit(
      const MathStructure &mstruct, CompositeUnit *cu,
      const EvaluationOptions &eo = default_user_evaluation_options,
      bool always_convert = true);
  MathStructure convertToMixedUnits(
      const MathStructure &mstruct,
      const EvaluationOptions &eo = default_user_evaluation_options);
  //@}

  void setTemperatureCalculationMode(
   ...
   */
  //@{
  /** Set assumptions for objects without own assumptions (unknown variables and
   * symbols).
   */
  void setDefaultAssumptions(Assumptions *ass);
  /** Returns the default assumptions for objects without own assumptions
   * (unknown variables and symbols).
   */
  ...
  //@{
  /** Returns a decimal prefix with exactly the provided value, that fulfils the
   * condition prefix->exponent(exp) == exp10.
   *
  ...
  /** Returns a binary prefix with exactly the provided value, that fulfils the
   * condition prefix->exponent(exp) == exp2.
   *
  ...
  /** Returns a prefix with exactly the provided value, that fulfils the
   * condition prefix->value(exp) == o.
   *
   ...
   * @param exp10 Base-10 exponent of the value.
   * @param exp The exponent of the unit.
   * @param all_prefixes If false, prefixes which is not a multiple of thousand
   * (centi, deci, deca, hecto) will be skipped.
   * @returns A prefix or NULL if the unit should be left without prefix.
   */
  DecimalPrefix *getOptimalDecimalPrefix(int exp10, int exp = 1,
   ...
   * @returns A prefix or NULL if the unit should be left without prefix.
   */
  DecimalPrefix *getOptimalDecimalPrefix(const Number &exp10, const Number &exp,
                                         bool all_prefixes = true) const;
   ...
   * @returns A prefix or NULL if the unit should be left without prefix.
   */
  BinaryPrefix *getOptimalBinaryPrefix(const Number &exp2,
                                       const Number &exp) const;
  /** Controls if binary, instead of decimal, prefixes will be used by default.
   * 1 = use binary prefixes for information units, 2 = use binary prefixes for
   * all units.
   */
  ...
  /** @name Functions for managing functions, variables, units, prefixes and
   * data sets. */
  //@{
  ...
  /** Checks if a pointer points to a variable that still exists in the
   * calculator. As opposed to hasFunction(), this function only checks if the
   * mathematical function has been deleted.
   */
  bool stillHasVariable(Variable *v);
  /** Checks if a pointer points to a unit that still exists in the calculator.
   * As opposed to hasUnit(), this function only checks if the unit has been
   * deleted.
   */
  bool stillHasUnit(Unit *u);
  /** Checks if a pointer points to a mathematical function that still exists in
   * the calculator. As opposed to hasFunction(), this function only checks if
   * the mathematical function has been deleted.
   */
  bool stillHasFunction(MathFunction *f);
  void saveFunctionCalled();
  bool checkSaveFunctionCalled();
  ExpressionItem *getActiveExpressionItem(std::string name,
                                          ExpressionItem *item = NULL);
  ExpressionItem *getActiveExpressionItem(std::string name,
                                          ExpressionItem *item, bool ignore_us);
  ExpressionItem *getInactiveExpressionItem(std::string name,
                                            ExpressionItem *item = NULL);
  ExpressionItem *getActiveExpressionItem(ExpressionItem *item);
  ExpressionItem *getExpressionItem(std::string name,
                                    ExpressionItem *item = NULL);
  ...
  /** Returns prefix for an index (starting at zero). All prefixes can be
   * traversed by starting at index zero and increasing the index until NULL is
   * returned.
   *
  ...
  /** Returns variable for an index (starting at zero). All variables can be
   * traversed by starting at index zero and increasing the index until NULL is
   * returned.
   *
  ...
  /** Returns unit for an index (starting at zero). All units can be traversed
   * by starting at index zero and increasing the index until NULL is returned.
   *
   * @param index Index of unit.
   * @returns Unit for index or NULL if not found.
   */
  Unit *getUnit(size_t index) const;
  /** Returns function for an index (starting at zero). All functions can be
   * traversed by starting at index zero and increasing the index until NULL is
   * returned.
   *
  ...
  void reset();
  /** Adds builtin variables. Called automatically when the calculator is
   * created. */
  void addBuiltinVariables();
  /** Adds builtin functions. Called automatically when the calculator is
   * created. */
  ...
  /** @name Functions for testing validity of functions, variable and unit
   * names. */
  //@{
  ...
  bool variableNameIsValid(const char *name_, int version_numbers[3],
                           bool is_user_defs);
  bool variableNameIsValid(const std::string &name_, int version_numbers[3],
                           bool is_user_defs);
  ...
  bool functionNameIsValid(const char *name_, int version_numbers[3],
                           bool is_user_defs);
  bool functionNameIsValid(const std::string &name_, int version_numbers[3],
                           bool is_user_defs);
  ...
  bool unitNameIsValid(const char *name_, int version_numbers[3],
                       bool is_user_defs);
  bool unitNameIsValid(const std::string &name_, int version_numbers[3],
                       bool is_user_defs);
  bool utf8_pos_is_valid_in_name(char *pos);
  std::string convertToValidUnitName(std::string name_);
  /** Checks if a name is used by another object which is not allowed to have
   * the same name.
   *
   * @param name Name.
  ...
  std::string getName(std::string name = "", ExpressionItem *object = NULL,
                      bool force = false, bool always_append = false);
  //@}
  ...
  int endTemporaryStopMessages(
      int *message_count = NULL, int *warning_count = NULL,
      int release_messages_if_no_equal_or_greater_than_message_type = -1);
  void endTemporaryStopMessages(
      bool release_messages,
      std::vector<CalculatorMessage> *blocked_messages = NULL);
  ...
  /** @name Functions for loading and saving definitions (variables, functions,
   * units, etc.). */
  //@{
  /** Load all standard global (system wide) definitions from the global data
   * directory ($PREFIX/share/qalculate).
   *
   * @returns true if the definitions were successfully loaded.
   */
  bool loadGlobalDefinitions();
  /** Load global (system wide) definitions from a file in the global data
   * directory ($PREFIX/share/qalculate).
   *
  ...
  /** Load local, user specific, definitions from the local definitions
   * directory (~/.qalculate/definitions). All files in the directory and in the
   * datasets subdirectory are loaded.
   *
   * @returns true if the definitions were successfully loaded.
   */
   ...
   * @param is_user_defs true if the definitions are local, false if they are
   * global.
   * @returns true if the definitions were successfully loaded.
   */
  ...
  void saveVariables(void *xmldoc, bool save_global = false,
                     bool save_only_temp = false);
  void saveUnits(void *xmldoc, bool save_global = false,
                 bool save_only_temp = false);
  void saveFunctions(void *xmldoc, bool save_global = false,
                     bool save_only_temp = false);
  //@}

  /** @name Functions for CSV file import/export. */
  //@{
  bool importCSV(MathStructure &mstruct, const char *file_name,
                 int first_row = 1, std::string delimiter = ",",
                 std::vector<std::string> *headers = NULL);
  bool importCSV(const char *file_name, int first_row = 1, bool headers = true,
                 std::string delimiter = ",", bool to_matrix = false,
                 std::string name = "", std::string title = "",
                 std::string category = "");
  bool exportCSV(const MathStructure &mstruct, const char *file_name,
                 std::string delimiter = ",");
  //@}

  /** @name Functions for exchange rates. */
  /** Checks if able to downloading exchange rates from the Internet (using
   * libcurl).
   *
  ...
  /// Deprecated: gvfs is not needed anymore.
  bool hasGnomeVFS();
  /** Load exchange rates. Use before loadGlobalCurrencies() or
   ...
   * Multiple exchange rates sources might be used. Iterate over these, using
   * the index parameter, until an empty string is returned.
   *
  ...
  /** Download current exchange rates from the Internet to local disc with
   ...
   *
   * @param n_days How old in days exchange rates may be before exchange rates
   * need updating
   * @param force_check If exchange rates date should be checked again even if
   * found outdated within n_days before
   * @param send_warning If the standard exchange rates warning should be sent.
   * @returns false if exchange.rates need updating
   */
  bool checkExchangeRatesDate(unsigned int n_days = 7, bool force_check = false,
                              bool send_warning = false, int n = -1);
  ...
  /// Check if exchange rates has been used since resetExchangeRatesUsed() was
  /// last called
   ...
   */
  bool canPlot();
  MathStructure expressionToPlotVector(
      std::string expression, const MathStructure &min,
      const MathStructure &max, int steps, bool separate_complex_part,
      MathStructure *x_vector = NULL, std::string x_var = "\\x",
      const ParseOptions &po = default_parse_options, int msecs = 5000);
  MathStructure expressionToPlotVector(
      std::string expression, const MathStructure &min,
      const MathStructure &max, int steps, MathStructure *x_vector = NULL,
      std::string x_var = "\\x", const ParseOptions &po = default_parse_options,
      int msecs = 5000);
  MathStructure expressionToPlotVector(
      std::string expression, float min, float max, int steps,
      MathStructure *x_vector = NULL, std::string x_var = "\\x",
      const ParseOptions &po = default_parse_options, int msecs = 5000);
  MathStructure expressionToPlotVector(
      std::string expression, const MathStructure &min,
      const MathStructure &max, const MathStructure &step,
      bool separate_complex_part, MathStructure *x_vector = NULL,
      std::string x_var = "\\x", const ParseOptions &po = default_parse_options,
      int msecs = 5000);
  MathStructure expressionToPlotVector(
      std::string expression, const MathStructure &min,
      ...
      const ParseOptions &po = default_parse_options, int msecs = 5000);
  MathStructure expressionToPlotVector(
      std::string expression, float min, float max, float step,
      MathStructure *x_vector = NULL, std::string x_var = "\\x",
      const ParseOptions &po = default_parse_options, int msecs = 5000);
  MathStructure
  expressionToPlotVector(std::string expression, const MathStructure &x_vector,
                         std::string x_var = "\\x",
                         const ParseOptions &po = default_parse_options,
                         int msecs = 5000);
  bool plotVectors(PlotParameters *param,
                   const std::vector<MathStructure> &y_vectors,
                   const std::vector<MathStructure> &x_vectors,
                   std::vector<PlotDataParameters *> &pdps,
                   bool persistent = false, int msecs = 5000);
  void forcePersistentPlot(bool persistent = true);
  bool invokeGnuplot(std::string commands, std::string commandline_extra = "",
  ...
  /** Set if interval should be produced for approximate functions and
   * irrational numbers. This does not affect calculation of lower precision
   * explicit intervals (uncertainty propagation).
   *
   * @param use_interval_arithmetic Set true to activate, or false to
   * deactivate, interval arithmetic.
   */
  ...
  /** Sets argument separator and decimal sign from the current locale. Mainly
   * for internal use. */
  void setLocale();
   ...
   * To use comma as an ignored separator in numbers, must be invoked with
   * comma_as_separator = true, to change the default function argument
   * separator to semicolon, in addition to using
  ...
  void unsetLocale();
  /** Returns the translated text string used in expressions for converting to a
   * specific unit expression (ex "5 meters to feet.*/
  ...
  void addDefaultStringAlternative(std::string replacement,
                                   std::string standard);
  bool delDefaultStringAlternative(std::string replacement,
                                   std::string standard);
  //@}
   ...
   * @param persistent If false the values will be removed from storage when
   * retrieved with getId().
   * @returns Storage id.
   */
  size_t addId(MathStructure *mstruct, bool persistent = false);
  /** Stores a function value with arguments parsed from a text string using
   * Function::parse(), with an associated id. Mainly for internal use.
   *
   * @param f Mathematical function.
   * @param str Arguments.
   * @param po Parse options.
   ...
   * @returns Storage id.
   */
  size_t parseAddId(MathFunction *f, const std::string &str,
                    const ParseOptions &po, bool persistent = false);
  size_t parseAddIdAppend(MathFunction *f, const MathStructure &append_mstruct,
                          const std::string &str, const ParseOptions &po,
                          bool persistent = false);
  size_t parseAddVectorId(const std::string &str, const ParseOptions &po,
                          bool persistent = false);
  ...
  /** Removes and unreferences (value->unref() will be called) a value from
   * storage. Mainly for internal use.
   *
   * @param id Storage id.
   */
