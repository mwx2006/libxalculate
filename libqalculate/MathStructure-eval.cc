...
#define IS_VAR_EXP(x, ofa)                                                     \
  ((x.isVariable() && x.variable()->isKnown() &&                               \
    (!ofa || x.variable()->title() == "\b")) ||                                \
   (x.isPower() && x[0].isVariable() && x[0].variable()->isKnown() &&          \
    (!ofa || x[0].variable()->title() == "\b")))
  ...
  if (deg2) {
    ...
    for (size_t i2 = 0; i2 < mstruct.size();) {
      ...
      if (mstruct[i2] == mvar) {
        a_struct.set(1, 1, 0);
        b = true;
      } else if (mstruct[i2].isPower() && mstruct[i2][0] == mvar &&
                 mstruct[i2][1].isNumber() && mstruct[i2][1].number().isTwo()) {
        b_struct.set(1, 1, 0);
        b = true;
        ...
        for (size_t i3 = 0; i3 < mstruct[i2].size(); i3++) {
          if (mstruct[i2][i3] == mvar) {
            ...
            b = true;
            break;
          } else if (mstruct[i2][i3].isPower() && mstruct[i2][i3][0] == mvar &&
                     mstruct[i2][i3][1].isNumber() &&
                     mstruct[i2][i3][1].number().isTwo()) {
            b_struct = mstruct[i2];
    ...
    if (b_struct == a_struct) {
      if (a_struct.isMultiplication() && a_struct.size() == 1)
        mul_struct = a_struct[0];
      else
        mul_struct = a_struct;
      a_struct.set(1, 1, 0);
      b_struct.set(1, 1, 0);
      ...
      for (size_t i = 0; i < a_struct.size();) {
        ...
        for (size_t i2 = i3; i2 < b_struct.size(); i2++) {
          if (a_struct[i] == b_struct[i2]) {
            i3 = i2;
            if (mul_struct.isOne())
              mul_struct = a_struct[i];
            else
              mul_struct.multiply(a_struct[i], true);
    ...
    if (a_struct.isMultiplication() && a_struct.size() == 0)
      a_struct.set(1, 1, 0);
    else if (a_struct.isMultiplication() && a_struct.size() == 1)
      a_struct.setToChild(1);
    if (b_struct.isMultiplication() && b_struct.size() == 0)
      b_struct.set(1, 1, 0);
    else if (b_struct.isMultiplication() && b_struct.size() == 1)
      b_struct.setToChild(1);
    if (!b_struct.isOne()) {
...
bool var_contains_interval(const MathStructure &mstruct) {
  ...
  if (mstruct.isVariable() && mstruct.variable()->isKnown())
    return var_contains_interval(((KnownVariable *)mstruct.variable())->get());
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (var_contains_interval(mstruct[i]))
      return true;
  }
  return false;
}
  ...
  if (mstruct.type() == STRUCT_ADDITION) {
      ...
      if (mstruct[i].isMultiplication()) {
          ...
          if (IS_VAR_EXP(mstruct[i][i2], ofa)) {
            ...
            for (size_t i3 = 0; i3 < variables.size(); i3++) {
              if (variables[i3] == mstruct[i][i2]) {
                ...
                b_found = true;
                if (term_sgn[i3] == 1 &&
                    !mstruct[i].representsNonNegative(true))
                  term_sgn[i3] = 0;
                else if (term_sgn[i3] == -1 &&
                         !mstruct[i].representsNonPositive(true))
                  term_sgn[i3] = 0;
                break;
              }
            }
            if (!b_found) {
              ...
              if (mstruct[i].representsNonNegative(true))
                term_sgn.push_back(1);
              else if (mstruct[i].representsNonPositive(true))
                term_sgn.push_back(-1);
              else
                term_sgn.push_back(0);
            }
          }
        }
        ...
        for (size_t i3 = 0; i3 < variables.size(); i3++) {
          if (variables[i3] == mstruct[i]) {
            variable_count[i3]++;
            b_found = true;
            if (term_sgn[i3] == 1 && !mstruct[i].representsNonNegative(true))
              term_sgn[i3] = 0;
            else if (term_sgn[i3] == -1 &&
                     !mstruct[i].representsNonPositive(true))
              term_sgn[i3] = 0;
    ...
    for (size_t i = 0; i < variables.size();) {
      ...
      if (!var_contains_interval(variables[i])) {
        ...
        b_erase = true;
        if (!variables[i].isPower() ||
            (variables[i][1].isNumber() && variables[i][1].number().isTwo())) {
          for (size_t i2 = i + 1; i2 < variables.size(); i2++) {
            if ((variables[i].isPower() && !variables[i2].isPower() &&
                 variables[i][0] == variables[i2]) ||
                (!variables[i].isPower() && variables[i2].isPower() &&
                 variables[i2][0] == variables[i] &&
                 variables[i2][1].isNumber() &&
                 variables[i2][1].number().isTwo())) {
              bool b_erase2 = false;
    ...
    for (size_t i = 0; i < variables.size(); i++) {
      ...
      Number nr;
      if (variables[i].isPower())
        v_ms = &((KnownVariable *)variables[i][0].variable())->get();
      else
        v_ms = &((KnownVariable *)variables[i].variable())->get();
      if (v_ms->isNumber())
        nr = v_ms->number();
      else if (v_ms->isMultiplication() && v_ms->size() > 0 &&
               (*v_ms)[0].isNumber())
        nr = (v_ms)[0].number();
      else {
      ...
      if (nr.isInterval()) {
        ...
        if (variables[i].isPower() && variables[i][1].isNumber() &&
            variables[i][1].number().isReal())
          u_candidate.raise(variables[i][1].number());
...
bool contains_duplicate_interval_variables_eq(const MathStructure &mstruct,
                                              const MathStructure &xvar,
                                              vector<KnownVariable *> &vars) {
  if (mstruct.isVariable() && mstruct.variable()->isKnown() &&
      ((KnownVariable *)mstruct.variable())
          ->get()
          .containsInterval(false, true, false, false)) {
  ...
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (contains_duplicate_interval_variables_eq(mstruct[i], xvar, vars))
      return true;
  }
  return false;
}
bool fix_eqs(MathStructure &m, const EvaluationOptions &eo) {
  ...
  if (m.isComparison()) {
    ...
    if (eo.isolate_var && m.contains(*eo.isolate_var))
      x_var = eo.isolate_var;
    else
      x_var = &m.find_x_var();
    if (!x_var->isUndefined()) {
      ...
      if (contains_duplicate_interval_variables_eq(m, *x_var, vars)) {
        ...
        vars.clear();
        if (m[0].containsType(STRUCT_ADDITION) &&
            contains_duplicate_interval_variables_eq(m[0], *x_var, vars))
          m[0].factorize(eo, false, false, 0, false, 1, NULL, m_undefined,
                         false, false, 3);
        return true;
      }
    }
...
/*
        ...
        Functions for handling of interval variables using interval arithmetic.
   Calculates the derivative of the expression for each interval variable,
   searches 	points where the derivative is zero, and calculates the
   expression separately for appropriate subintervals.

*/

void find_interval_variables(const MathStructure &mstruct,
                             vector<KnownVariable *> &vars,
                             vector<int> &v_count, vector<int> &v_prec) {
  if (mstruct.isVariable() && mstruct.variable()->isKnown()) {
    KnownVariable *v = (KnownVariable *)mstruct.variable();
    ...
    if (mv.isNumber()) {
      if (mv.number().isInterval())
        var_prec = mv.number().precision(1);
      else if (CALCULATOR->usesIntervalArithmetic() &&
               mv.number().precision() >= 0)
        var_prec = mv.number().precision();
    } else if (mv.isMultiplication()) {
      for (size_t i = 0; i < mv.size(); i++) {
        if (mv[i].isNumber()) {
          if (mv[i].number().isInterval()) {
            var_prec = mv[i].number().precision(1);
            break;
          } else if (mv[i].number().precision() >= 0) {
            var_prec = mv[i].number().precision();
            break;
          }
        }
      }
...
bool contains_undefined(
    ...
    bool calc = false, const MathStructure &x_var = m_zero,
    const MathStructure &m_intval = nr_zero) {
  if (m.isPower() && (m[1].representsNegative() ||
                      (m[1].isNumber() && !m[1].number().isNonNegative()))) {
    if (calc) {
      m[0].replace(x_var, m_intval, true);
      m[0].calculatesub(eo, eo, true);
    }
    if (contains_not_nonzero(m[0]))
      return true;
  }
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_undefined(m[i], eo, calc, x_var, m_intval))
      return true;
  }
  return false;
}

// search for values where the expression is non-zero, using intervals of
// diminishing width
bool find_interval_zeroes(const MathStructure &mstruct, MathStructure &malts,
                          const MathStructure &mvar, const Number &nr_intval,
                          const EvaluationOptions &eo, int depth,
                          const Number &nr_prec, int orig_prec = 0,
                          int is_real = -1, int undef_depth = 0) {
  if (CALCULATOR->aborted())
    return false;
  if (depth == 0)
    orig_prec = nr_intval.precision(1);
  MathStructure mtest(mstruct);
  mtest.replace(mvar, nr_intval);
  mtest.eval(eo);
  if (is_real < 0)
    is_real = mtest.representsNonComplex(true);
  ComparisonResult cmp;
  if (is_real == 0) {
    MathStructure m_re(CALCULATOR->getFunctionById(FUNCTION_ID_RE), &mtest,
                       NULL);
    m_re.calculateFunctions(eo);
    cmp = m_re.compare(m_zero);
    MathStructure m_im(CALCULATOR->getFunctionById(FUNCTION_ID_IM), &mtest,
                       NULL);
    m_im.calculateFunctions(eo);
    ComparisonResult cmp2 = m_im.compare(m_zero);
    if (COMPARISON_IS_NOT_EQUAL(cmp) || cmp2 == COMPARISON_RESULT_EQUAL ||
        cmp == COMPARISON_RESULT_UNKNOWN)
      cmp = cmp2;
  } else {
  ...
  if (COMPARISON_IS_NOT_EQUAL(cmp)) {
    return true;
             ...
             (undef_depth <= 5 && contains_undefined(mtest))) {
    if (cmp == COMPARISON_RESULT_EQUAL ||
        (nr_intval.precision(1) >
             (orig_prec > PRECISION ? orig_prec + 5 : PRECISION + 5) ||
         (!nr_intval.isNonZero() &&
          nr_intval.uncertainty().isLessThan(nr_prec)))) {
      if (cmp == COMPARISON_RESULT_EQUAL && depth <= 3)
        return false;
      if (malts.size() > 0 &&
          (cmp = malts.last().compare(nr_intval)) !=
              COMPARISON_RESULT_UNKNOWN &&
          cmp != COMPARISON_RESULT_EQUAL && COMPARISON_MIGHT_BE_EQUAL(cmp)) {
        malts.last().number().setInterval(malts.last().number(), nr_intval);
        if (malts.last().number().precision(1) <
            (orig_prec > PRECISION ? orig_prec + 3 : PRECISION + 3)) {
          return false;
        }
      } else {
    ...
    for (size_t i = 0; i < splits.size(); i++) {
      if (!find_interval_zeroes(
              mstruct, malts, mvar, splits[i], eo, depth + 1, nr_prec,
              orig_prec, is_real,
              cmp == COMPARISON_RESULT_UNKNOWN ? undef_depth + 1 : 0))
        return false;
    }
    return true;
  }
...
bool contains_interval_variable(const MathStructure &m, int i_type = 0) {
  if (i_type == 0 && m.isVariable() &&
      m.containsInterval(true, true, false, 1, false))
    return true;
  else if (i_type == 1 && m.containsInterval(true, false, false, 1, true))
    return true;
  else if (i_type == 2 && m.containsInterval(true, true, false, 1, true))
    return true;
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_interval_variable(m[i]))
      return true;
  }
  return false;
}
bool calculate_differentiable_functions(MathStructure &m,
                                        const EvaluationOptions &eo,
                                        bool recursive = true,
                                        bool do_unformat = true) {
  if (m.isFunction() && m.function() != eo.protected_function &&
      function_differentiable(m.function())) {
  ...
  if (recursive) {
    for (size_t i = 0; i < m.size(); i++) {
      if (CALCULATOR->aborted())
        break;
      if (calculate_differentiable_functions(m[i], eo, recursive,
                                             do_unformat)) {
        m.childUpdated(i + 1);
        b = true;
      }
...
bool calculate_nondifferentiable_functions(MathStructure &m,
                                           const EvaluationOptions &eo,
  ...
  if (m.isFunction() && m.function() != eo.protected_function) {
    if ((i_type <= 0 &&
         (!function_differentiable(m.function()) ||
          (m.function()->id() == FUNCTION_ID_INCOMPLETE_BETA &&
           (m.size() != 3 ||
            m[1].containsInterval(true, false, false, 1, true) ||
            m[2].containsInterval(true, false, false, 1, true))) ||
          (m.function()->id() == FUNCTION_ID_I_GAMMA &&
           (m.size() != 2 ||
            m[1].containsInterval(true, false, false, 1, true))))) ||
        (i_type >= 0 && !contains_interval_variable(m, i_type))) {
      if (m.calculateFunctions(eo, false, do_unformat)) {
        if (recursive)
          calculate_nondifferentiable_functions(m, eo, recursive, do_unformat,
                                                i_type);
        return true;
      } else {
        return false;
      ...
      if (eo.approximation == APPROXIMATION_APPROXIMATE &&
          !m.containsUnknowns())
        eo3.approximation = APPROXIMATION_EXACT_VARIABLES;
      else
  ...
  if (recursive) {
    for (size_t i = 0; i < m.size(); i++) {
      if (CALCULATOR->aborted())
        break;
      if (calculate_nondifferentiable_functions(m[i], eo, recursive,
                                                do_unformat, i_type)) {
        m.childUpdated(i + 1);
        b = true;
      }
...
void remove_nonzero_mul(MathStructure &msolve, const MathStructure &u_var,
                        const EvaluationOptions &eo) {
  if (!msolve.isMultiplication())
    return;
...
void solve_intervals2(MathStructure &mstruct, vector<KnownVariable *> vars,
  ...
  if (vars.size() > 0) {
    ...
    if (eo.calculate_functions)
      calculate_differentiable_functions(mstruct, eo);
    ...
    if (!msolve.differentiate(u_var, eo) ||
        msolve.countTotalChildren(false) > 10000 ||
        contains_diff_for(msolve, u_var) || CALCULATOR->aborted()) {
      b = false;
    }
    ...
    if (b) {
      ...
      // test if derivative is non-zero, replacing each occurrence of the
      // variable with independent intervals
      ...
      if (mtest.countTotalChildren(false) < 100) {
        if (mtest.representsNonComplex(true)) {
          ...
          if (!COMPARISON_IS_EQUAL_OR_GREATER(cmp) &&
              !COMPARISON_IS_EQUAL_OR_LESS(cmp))
            b = false;
        } else {
          MathStructure m_re(CALCULATOR->getFunctionById(FUNCTION_ID_RE),
                             &mtest, NULL);
          m_re.calculateFunctions(eo);
          ...
          if (!COMPARISON_IS_EQUAL_OR_GREATER(cmp) &&
            ...
            MathStructure m_im(CALCULATOR->getFunctionById(FUNCTION_ID_IM),
                               &mtest, NULL);
            m_im.calculateFunctions(eo);
      ...
      if (!b) {
        ...
        eo.approximation = APPROXIMATION_APPROXIMATE;
        eo.expand = eo_pre.expand;
        msolve.factorize(eo, false, false, 0, false, true, NULL, m_undefined,
                         false, false, 1);
        remove_nonzero_mul(msolve, u_var, eo);
        if (msolve.isZero()) {
        } else if (contains_undefined(msolve) ||
                   msolve.countTotalChildren(false) > 1000 ||
                   msolve.containsInterval(true, true, false, 1, true)) {
          b = false;
        } else {
          MathStructure mtest(mstruct);
          ...
          if (contains_undefined(mtest, eo, true, u_var, mv)) {
            ...
            b = find_interval_zeroes(msolve, malts, u_var, nr_intval, eo, 0,
                                     nr_prec);
          }
        }
        eo.expand = false;
    ...
    if (!b) {
      CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
                        _("Interval potentially calculated wide."), NULL);
...
KnownVariable *fix_find_interval_variable(MathStructure &mstruct) {
  if (mstruct.isVariable() && mstruct.variable()->isKnown()) {
    ...
    if (contains_interval_variable(m))
...
bool replace_intervals(MathStructure &m) {
  if (m.isNumber()) {
    int var_prec = 0;
    if (m.number().isInterval())
      var_prec = m.number().precision(1);
    else if (CALCULATOR->usesIntervalArithmetic() &&
             m.number().precision() >= 0)
      var_prec = m.number().precision();
    if (var_prec <= PRECISION + 10) {
...
void solve_intervals(MathStructure &mstruct, const EvaluationOptions &eo,
  ...
  for (size_t i = 0; i < v_count.size();) {
    if (v_count[i] < 2 || (feo.approximation == APPROXIMATION_EXACT &&
                           vars[i]->title() != "\b")) {
...
void find_interval_create_var(const Number &nr, MathStructure &m,
                              MathStructure &unc, MathStructure &unc2,
                              KnownVariable **v, KnownVariable **v2) {
  if (nr.hasImaginaryPart() && nr.internalImaginary()->isInterval()) {
    if (nr.hasRealPart() && nr.isInterval(false)) {
      ...
      *v = new KnownVariable("", string("(") + format_and_print(nmid) + ")",
                             nmid);
      (*v)->setApproximate(false);
      *v2 = new KnownVariable("", string("(") + format_and_print(nmid2) + ")",
                              nmid2);
    ...
    nmid.intervalToMidValue();
    *v =
        new KnownVariable("", string("(") + format_and_print(nmid) + ")", nmid);
    (*v)->setApproximate(false);
    m.set(*v);
    (*v)->destroy();
  }
...
KnownVariable *find_interval_replace_var(MathStructure &m, MathStructure &unc,
                                         MathStructure &unc2,
                                         KnownVariable **v2,
                                         const EvaluationOptions &eo,
                                         MathStructure *mnew, Variable **prev_v,
                                         bool &b_failed) {
  if (eo.approximation != APPROXIMATION_EXACT_VARIABLES &&
      eo.calculate_variables && m.isVariable() && m.variable()->isKnown() &&
      (eo.approximation != APPROXIMATION_EXACT ||
       m.variable()->title() == "\b")) {
    const MathStructure &mvar = ((KnownVariable *)m.variable())->get();
    if (!mvar.containsInterval(true, true, false, 1, true))
      return NULL;
    if (mvar.isNumber()) {
      m.variable()->ref();
      *prev_v = m.variable();
      ...
      *mnew = m;
    ...
    } else if (mvar.isFunction() &&
               mvar.function()->id() == FUNCTION_ID_INTERVAL &&
               mvar.size() == 2 &&
               !mvar[0].containsInterval(true, true, false, 1, true) &&
               !mvar[1].containsInterval(true, true, false, 1, true)) {
      if (mvar[0].isAddition() && mvar[0].size() == 2 && mvar[1].isAddition() &&
          mvar[1].size() == 2) {
        const MathStructure *mmid = NULL, *munc = NULL;
        if (mvar[0][0].equals(mvar[1][0])) {
          mmid = &mvar[0][0];
          if (mvar[0][1].isNegate() && mvar[0][1][0].equals(mvar[1][1]))
            munc = &mvar[1][1];
          if (mvar[1][1].isNegate() && mvar[1][1][0].equals(mvar[0][1]))
            munc = &mvar[0][1];
        } else if (mvar[0][1].equals(mvar[1][1])) {
          mmid = &mvar[0][1];
          if (mvar[0][0].isNegate() && mvar[0][0][0].equals(mvar[1][0]))
            munc = &mvar[1][0];
          if (mvar[1][0].isNegate() && mvar[1][0][0].equals(mvar[0][0]))
            munc = &mvar[0][0];
        } else if (mvar[0][0].equals(mvar[1][1])) {
          mmid = &mvar[0][0];
          if (mvar[0][1].isNegate() && mvar[0][1][0].equals(mvar[1][0]))
            munc = &mvar[1][0];
          if (mvar[1][0].isNegate() && mvar[1][0][0].equals(mvar[0][1]))
            munc = &mvar[0][1];
        } else if (mvar[0][1].equals(mvar[1][0])) {
          mmid = &mvar[0][0];
          if (mvar[0][0].isNegate() && mvar[0][0][0].equals(mvar[1][1]))
            munc = &mvar[1][1];
          if (mvar[1][1].isNegate() && mvar[1][1][0].equals(mvar[0][0]))
            munc = &mvar[0][0];
        }
        if (mmid && munc) {
          unc = *munc;
          ...
          KnownVariable *v = new KnownVariable(
              "", string("(") + format_and_print(*mmid) + ")", mmid2);
          m.set(v);
          v->destroy();
          return v;
        }
      ...
      mmid *= nr_half;
      KnownVariable *v = new KnownVariable(
          "", string("(") + format_and_print(mmid) + ")", mmid);
      m.variable()->ref();
      *prev_v = m.variable();
      m.set(v);
      *mnew = m;
      ...
      return v;
               ...
               mvar.function()->id() == FUNCTION_ID_UNCERTAINTY &&
               mvar.size() == 3 && mvar[2].isNumber() &&
               !mvar[0].containsInterval(true, true, false, 1, true) &&
               !mvar[1].containsInterval(true, true, false, 1, true)) {
      ...
      KnownVariable *v = new KnownVariable(
          "", string("(") + format_and_print(mvar[0]) + ")", mvar[0]);
      m.variable()->ref();
      *prev_v = m.variable();
      m.set(v);
      *mnew = m;
    ...
    b_failed = true;
  } else if (m.isNumber() && m.number().isInterval(false) &&
             m.number().precision(true) <= PRECISION + 10) {
    KnownVariable *v = NULL;
    find_interval_create_var(m.number(), m, unc, unc2, &v, v2);
    return v;
  } else if (m.isFunction() && m.function()->id() == FUNCTION_ID_INTERVAL &&
             m.size() == 2 &&
             !m[0].containsInterval(true, true, false, 1, true) &&
             !m[1].containsInterval(true, true, false, 1, true)) {
    if (m[0].isAddition() && m[0].size() == 2 && m[1].isAddition() &&
        m[1].size() == 2) {
      MathStructure *mmid = NULL, *munc = NULL;
      if (m[0][0].equals(m[1][0])) {
        mmid = &m[0][0];
        if (m[0][1].isNegate() && m[0][1][0].equals(m[1][1]))
          munc = &m[1][1];
        if (m[1][1].isNegate() && m[1][1][0].equals(m[0][1]))
          munc = &m[0][1];
      } else if (m[0][1].equals(m[1][1])) {
        mmid = &m[0][1];
        if (m[0][0].isNegate() && m[0][0][0].equals(m[1][0]))
          munc = &m[1][0];
        if (m[1][0].isNegate() && m[1][0][0].equals(m[0][0]))
          munc = &m[0][0];
      } else if (m[0][0].equals(m[1][1])) {
        mmid = &m[0][0];
        if (m[0][1].isNegate() && m[0][1][0].equals(m[1][0]))
          munc = &m[1][0];
        if (m[1][0].isNegate() && m[1][0][0].equals(m[0][1]))
          munc = &m[0][1];
      } else if (m[0][1].equals(m[1][0])) {
        mmid = &m[0][0];
        if (m[0][0].isNegate() && m[0][0][0].equals(m[1][1]))
          munc = &m[1][1];
        if (m[1][1].isNegate() && m[1][1][0].equals(m[0][0]))
          munc = &m[0][0];
      }
      if (mmid && munc) {
        unc = *munc;
    ...
    mmid *= nr_half;
    KnownVariable *v =
        new KnownVariable("", string("(") + format_and_print(mmid) + ")", mmid);
    m.set(v);
    v->destroy();
    return v;
  } else if (m.isFunction() && m.function()->id() == FUNCTION_ID_UNCERTAINTY &&
             m.size() == 3 && m[2].isNumber() &&
             !m[0].containsInterval(true, true, false, 1, true) &&
             !m[1].containsInterval(true, true, false, 1, true)) {
    ...
    KnownVariable *v =
        new KnownVariable("", string("(") + format_and_print(m[0]) + ")", m[0]);
    m.set(v);
    v->destroy();
    return v;
  }
  for (size_t i = 0; i < m.size(); i++) {
    KnownVariable *v = find_interval_replace_var(m[i], unc, unc2, v2, eo, mnew,
                                                 prev_v, b_failed);
    if (b_failed)
      return NULL;
    if (v)
...
bool find_interval_replace_var_nr(MathStructure &m) {
  if ((m.isNumber() && m.number().isInterval(false) &&
       m.number().precision(true) <= PRECISION + 10) ||
      (m.isFunction() && m.function()->id() == FUNCTION_ID_INTERVAL &&
       m.size() == 2) ||
      (m.isFunction() && m.function()->id() == FUNCTION_ID_UNCERTAINTY &&
       m.size() == 3)) {
    KnownVariable *v =
        new KnownVariable("", string("(") + format_and_print(m) + ")", m);
    m.set(v);
    v->destroy();
    return true;
  }
  bool b = false;
  for (size_t i = 0; i < m.size(); i++) {
    if (find_interval_replace_var_nr(m[i]))
      b = true;
  }
  return b;
}

bool replace_variables_with_interval(MathStructure &m,
                                     const EvaluationOptions &eo,
                                     bool in_nounit = false,
                                     bool only_argument_vars = false) {
  if (m.isVariable() && m.variable()->isKnown() &&
      (!only_argument_vars || m.variable()->title() == "\b")) {
    const MathStructure &mvar = ((KnownVariable *)m.variable())->get();
    if (!mvar.containsInterval(true, true, false, 1, true))
      return false;
    if (mvar.isNumber()) {
  ...
  for (size_t i = 0; i < m.size(); i++) {
    if (replace_variables_with_interval(m[i], eo, in_nounit,
                                        only_argument_vars))
      b = true;
  }
  return b;
...
bool contains_diff_for(const MathStructure &m, const MathStructure &x_var) {
  if (m.isFunction() && m.function() &&
      m.function()->id() == FUNCTION_ID_DIFFERENTIATE && m.size() >= 2 &&
      m[1] == x_var)
    return true;
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_diff_for(m[i], x_var))
      return true;
  }
  return false;
}

MathStructure calculate_uncertainty(MathStructure &m,
                                    const EvaluationOptions &eo,
                                    bool &b_failed) {
  vector<KnownVariable *> vars;
  ...
  if (eo.approximation != APPROXIMATION_EXACT_VARIABLES &&
      eo.calculate_variables)
    replace_variables_with_interval(m, eo, false,
                                    eo.approximation == APPROXIMATION_EXACT);
  while (true) {
    Variable *prev_v = NULL;
    MathStructure mnew;
    KnownVariable *v2 = NULL;
    KnownVariable *v = find_interval_replace_var(m, unc, unc2, &v2, eo, &mnew,
                                                 &prev_v, b_failed);
    if (!v)
      break;
    if (!mnew.isZero()) {
  ...
  m.unformat(eo);
  if (eo.sync_units && eo.approximation != APPROXIMATION_EXACT)
    sync_approximate_units(m, eo, &vars, &uncs, true);
  if (b_failed || vars.empty())
    return m_zero;
  MathStructure munc;
  ...
  for (size_t i = 0; i < vars.size(); i++) {
    ...
    if (!mdiff->differentiate(muv, eo) || contains_diff_for(*mdiff, muv) ||
        CALCULATOR->aborted()) {
      b_failed = true;
      return m_zero;
    }
    ...
    if (!mdiff->representsNonComplex(true)) {
      ...
      if (!munc_i) {
        ...
        munc_i->add_nocopy(mdiff_i, true);
    }
    mdiff->raise(nr_two);
    mdiff->multiply(uncs[i]);
    mdiff->last().raise(nr_two);
    if (munc.isZero()) {
      munc.set_nocopy(*mdiff);
      mdiff->unref();
    } else
      munc.add_nocopy(mdiff, true);
  }
  uv->destroy();
  munc.raise(nr_half);
  if (munc_i) {
...
Variable *find_interval_replace_var_comp(MathStructure &m,
                                         const EvaluationOptions &eo,
                                         Variable **v) {
  if (eo.approximation != APPROXIMATION_EXACT &&
      eo.approximation != APPROXIMATION_EXACT_VARIABLES &&
      eo.calculate_variables && m.isVariable() && m.variable()->isKnown() &&
      ((KnownVariable *)m.variable())
          ->get()
          .containsInterval(true, true, false, 1, true)) {
...
bool eval_comparison_sides(MathStructure &m, const EvaluationOptions &eo) {
  if (m.isComparison()) {
    ...
    if (!m[0].isUnknown()) {
      ...
      CALCULATOR->beginTemporaryStopMessages();
      m[0].eval(eo);
      if (m[0].containsFunctionId(FUNCTION_ID_UNCERTAINTY) &&
      ...
      CALCULATOR->beginTemporaryStopMessages();
      m[1].eval(eo);
      if (m[1].containsFunctionId(FUNCTION_ID_UNCERTAINTY) &&
    ...
    bool ret = true;
    for (size_t i = 0; i < m.size(); i++) {
      if (!eval_comparison_sides(m[i], eo))
        ret = false;
    }
    m.childrenUpdated();
    m.calculatesub(eo, eo, false);
...
bool separate_unit_vars(MathStructure &m, const EvaluationOptions &eo,
                        bool only_approximate, bool dry_run) {
  if (m.isVariable() && m.variable()->isKnown()) {
    const MathStructure &mvar = ((KnownVariable *)m.variable())->get();
    if (mvar.isMultiplication()) {
      ...
      for (size_t i = 0; i < mvar.size(); i++) {
        if (is_unit_multiexp(mvar[i])) {
          if (!b)
            b = !only_approximate ||
                contains_approximate_relation_to_base(mvar[i], true);
...
// Functions for handling of function arguments with intervals replaced by
// temporary variables
int contains_interval_var(const MathStructure &m, bool structural_only,
                          bool check_variables, bool check_functions,
                          int ignore_high_precision_interval,
                          bool include_interval_function);
int contains_function_interval(const MathStructure &m,
                               bool structural_only = true,
                               bool check_variables = false,
                               bool check_functions = false,
                               int ignore_high_precision_interval = 0,
                               bool include_interval_function = false) {
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_function_interval(
            m[i], structural_only, check_variables, check_functions,
            ignore_high_precision_interval, include_interval_function))
      return true;
  }
  if (m.isVariable() && m.variable()->isKnown() &&
      m.variable()->title() == "\b") {
    if (ignore_high_precision_interval == 0)
      return true;
    return contains_interval_var(
        ((KnownVariable *)m.variable())->get(), structural_only,
        check_variables, check_functions, ignore_high_precision_interval,
        include_interval_function);
  }
  return false;
}
bool replace_function_vars(MathStructure &m) {
  for (size_t i = 0; i < m.size(); i++) {
    if (replace_function_vars(m[i]))
      return true;
  }
  if (m.isVariable() && m.variable()->isKnown() &&
      m.variable()->title() == "\b") {
...
bool simplify_functions(MathStructure &mstruct, const EvaluationOptions &eo,
                        const EvaluationOptions &feo,
                        const MathStructure &x_var) {
  if (!mstruct.isAddition()) {
    ...
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (CALCULATOR->aborted())
        break;
      if (simplify_functions(mstruct[i], eo, feo, x_var)) {
        b = true;
        mstruct.childUpdated(i + 1);
      }
    }
  ...
  if (mstruct.containsFunctionId(FUNCTION_ID_SIN, false, false, false) > 0 &&
      mstruct.containsFunctionId(FUNCTION_ID_COS, false, false, false)) {
    if (x_var.isUndefined()) {
      // a*(sin(x)+cos(x))=a*sqrt(2)*sin(x+pi/4)
      bool b_ret = false;
      for (size_t i = 0; i < mstruct.size(); i++) {
        if (mstruct[i].isFunction() && mstruct[i].size() == 1 &&
            mstruct[i].function()->id() == FUNCTION_ID_SIN) {
          for (size_t i2 = 0; i2 < mstruct.size(); i2++) {
            if (i != i2 && mstruct[i2].isFunction() &&
                mstruct[i2].size() == 1 &&
                mstruct[i2].function()->id() == FUNCTION_ID_COS &&
                mstruct[i][0] == mstruct[i2][0]) {
          ...
          for (size_t i3 = 0; i3 < mstruct[i].size(); i3++) {
            if (mstruct[i][i3].isFunction() && mstruct[i][i3].size() == 1 &&
                mstruct[i][i3].function()->id() == FUNCTION_ID_SIN) {
      ...
      for (size_t i = 0; i < mstruct.size(); i++) {
        if (mstruct[i].isFunction() && mstruct[i].size() == 1 &&
            (mstruct[i].function()->id() == FUNCTION_ID_SIN ||
             mstruct[i].function()->id() == FUNCTION_ID_COS) &&
            mstruct[i][0].contains(x_var)) {
          marg = &mstruct[i][0];
          b_cos = mstruct[i].function()->id() == FUNCTION_ID_COS;
        } else if (mstruct[i].isMultiplication()) {
          for (size_t i2 = 0; i2 < mstruct[i].size(); i2++) {
            if (!marg && mstruct[i][i2].isFunction() &&
                mstruct[i][i2].size() == 1 &&
                (mstruct[i][i2].function()->id() == FUNCTION_ID_SIN ||
                 mstruct[i][i2].function()->id() == FUNCTION_ID_COS) &&
                mstruct[i][i2][0].contains(x_var)) {
              marg = &mstruct[i][i2][0];
        ...
        if (marg) {
          ...
          for (size_t i3 = i + 1; i3 < mstruct.size(); i3++) {
            if (mstruct[i3].isFunction() && mstruct[i3].size() == 1 &&
                mstruct[i3].function()->id() ==
                    (b_cos ? FUNCTION_ID_SIN : FUNCTION_ID_COS) &&
                mstruct[i3][0] == *marg) {
              b = true;
            } else if (mstruct[i3].isMultiplication()) {
              bool b2 = false;
              for (size_t i2 = 0; i2 < mstruct[i3].size(); i2++) {
                if (!b2 && mstruct[i3][i2].isFunction() &&
                    mstruct[i3][i2].size() == 1 &&
                    (mstruct[i3][i2].function()->id() == FUNCTION_ID_SIN ||
                     mstruct[i3][i2].function()->id() == FUNCTION_ID_COS) &&
                    mstruct[i3][i2][0] == *marg) {
                  if ((mstruct[i3][i2].function()->id() == FUNCTION_ID_SIN) ==
                      b_cos)
                    b = true;
                  b2 = true;
        ...
        if (marg) {
          ...
          for (size_t i3 = i; i3 < mstruct.size();) {
            bool b = false;
            if (mstruct[i3].isFunction() && mstruct[i3].size() == 1 &&
                mstruct[i3].function()->id() == FUNCTION_ID_SIN &&
                mstruct[i3][0] == *marg) {
              if (m_a.isZero())
                m_a = m_one;
              else
                m_a.add(m_one, true);
              b = true;
            } else if (mstruct[i3].isFunction() && mstruct[i3].size() == 1 &&
                       mstruct[i3].function()->id() == FUNCTION_ID_COS &&
                       mstruct[i3][0] == *marg) {
              if (m_b.isZero())
                m_b = m_one;
              else
              ...
              for (size_t i2 = 0; i2 < mstruct[i3].size(); i2++) {
                if (mstruct[i3][i2].isFunction() &&
                    mstruct[i3][i2].size() == 1 &&
                    mstruct[i3][i2].function()->id() == FUNCTION_ID_SIN &&
                    mstruct[i3][i2][0] == *marg) {
                  mstruct[i3].delChild(i2 + 1, true);
                  if (m_a.isZero())
                    m_a.set_nocopy(mstruct[i3]);
                  else {
                    mstruct[i3].ref();
                    m_a.add_nocopy(&mstruct[i3], true);
                  }
                  b = true;
                  break;
                } else if (mstruct[i3][i2].isFunction() &&
                           mstruct[i3][i2].size() == 1 &&
                           mstruct[i3][i2].function()->id() ==
                               FUNCTION_ID_COS &&
                           mstruct[i3][i2][0] == *marg) {
                  mstruct[i3].delChild(i2 + 1, true);
                  if (m_b.isZero())
                    m_b.set_nocopy(mstruct[i3]);
                  else {
                    mstruct[i3].ref();
                    m_b.add_nocopy(&mstruct[i3], true);
                  }
                  b = true;
                  break;
                }
          ...
          MathStructure *m_sin = new MathStructure(
              CALCULATOR->getFunctionById(FUNCTION_ID_SIN), NULL);
          m_sin->addChild_nocopy(marg);
          m_b.calculateDivide(m_a, eo);
          MathStructure *m_atan = new MathStructure(
              CALCULATOR->getFunctionById(FUNCTION_ID_ATAN), &m_b, NULL);
          if (m_atan->calculateFunctions(feo))
            m_atan->calculatesub(eo, feo, true);
          if (HAS_DEFAULT_ANGLE_UNIT(eo.parse_options.angle_unit))
            m_atan->calculateMultiply(CALCULATOR->getRadUnit(), eo);
          ...
          if (mstruct.size() == 0) {
            mstruct.set_nocopy(*m_sin);
            m_sin->unref();
          } else
            mstruct.insertChild_nocopy(m_sin, i + 1);
...
bool simplify_ln(MathStructure &mstruct) {
  bool b_ret = false;
  for (size_t i = 0; i < mstruct.size(); i++) {
    if (simplify_ln(mstruct[i]))
      b_ret = true;
  }
  if (mstruct.isAddition()) {
    size_t i_ln = (size_t)-1, i_ln_m = (size_t)-1;
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (mstruct[i].isFunction() &&
          mstruct[i].function()->id() == FUNCTION_ID_LOG &&
          mstruct[i].size() == 1 && mstruct[i][0].isNumber() &&
          mstruct[i][0].number().isReal() &&
          mstruct[i][0].number().isNonZero()) {
        if (i_ln == (size_t)-1) {
          i_ln = i;
        } else {
          bool b = true;
          if (mstruct[i_ln].isMultiplication()) {
            if (mstruct[i_ln][1][0].number().raise(mstruct[i_ln][0].number(),
                                                   true)) {
              mstruct[i_ln].setToChild(2, true);
              b_ret = true;
            } else
              b = false;
          }
          if (b &&
              (!mstruct[i][0].number().isNegative() ||
               !mstruct[i_ln][0].number().isNegative()) &&
              mstruct[i_ln][0].number().multiply(mstruct[i][0].number())) {
            ...
            b_ret = true;
                 ...
                 mstruct[i][1].isFunction() &&
                 mstruct[i][1].function()->id() == FUNCTION_ID_LOG &&
                 mstruct[i][1].size() == 1 && mstruct[i][1][0].isNumber() &&
                 mstruct[i][1][0].number().isReal() &&
                 mstruct[i][1][0].number().isNonZero() &&
                 mstruct[i][0].isInteger() &&
                 mstruct[i][0].number().isLessThan(1000) &&
                 mstruct[i][0].number().isGreaterThan(-1000)) {
        if (mstruct[i][0].number().isPositive()) {
          if (i_ln == (size_t)-1) {
            ...
            bool b = true;
            if (mstruct[i_ln].isMultiplication()) {
              if (mstruct[i_ln][1][0].number().raise(
                      mstruct[i_ln][0].number())) {
                mstruct[i_ln].setToChild(2, true);
                b_ret = true;
              } else
                b = false;
            ...
            if (b &&
                mstruct[i][1][0].number().raise(mstruct[i][0].number(), true)) {
              if ((!mstruct[i][1][0].number().isNegative() ||
                   !mstruct[i_ln][0].number().isNegative()) &&
                  mstruct[i_ln][0].number().multiply(
                      mstruct[i][1][0].number())) {
                mstruct.delChild(i + 1);
                i--;
              } else {
          ...
          if (i_ln_m == (size_t)-1) {
            ...
            if (!b && mstruct[i_ln_m][1][0].number().raise(
                          -mstruct[i_ln_m][0].number())) {
              mstruct[i_ln_m][0].set(m_minus_one, true);
              b_ret = true;
              b = true;
            }
            bool b_m1 = b && mstruct[i][0].number().isMinusOne();
            if (b && (b_m1 || mstruct[i][1][0].number().raise(
                                  -mstruct[i][0].number(), true))) {
              if ((!mstruct[i][1][0].number().isNegative() ||
                   !mstruct[i_ln_m][1][0].number().isNegative()) &&
                  mstruct[i_ln_m][1][0].number().multiply(
                      mstruct[i][1][0].number())) {
                mstruct.delChild(i + 1);
                b_ret = true;
                i--;
...
bool simplify_roots(MathStructure &mstruct, const EvaluationOptions &eo) {
  ...
  if (mstruct.isMultiplication() && mstruct.size() >= 2 &&
      mstruct[0].isNumber() && mstruct[0].number().isRational()) {
    for (size_t i = 1; i < mstruct.size(); i++) {
      if (mstruct[i].isPower() && mstruct[i][1].isNumber() &&
          !mstruct[i][1].number().includesInfinity() &&
          mstruct[i][0].isNumber() && mstruct[i][0].number().isRational() &&
          !mstruct[i][0].number().isZero()) {
        if (mstruct[0].number().denominator() ==
                mstruct[i][0].number().numerator() &&
            mstruct[0].number().numerator() ==
                mstruct[i][0].number().denominator()) {
          // (n/m)^a*m/n=(n/m)^(a-1)
          mstruct[i][1].number()--;
          mstruct.childUpdated(i + 1);
          ...
          b_ret = true;
          break;
        } else if (mstruct[i][1].number().isNegative() &&
                   mstruct[0].number().isIntegerDivisible(
                       mstruct[i][0].number())) {
        ...
        } else if (mstruct[i][1].number().isPositive() &&
                   !mstruct[0].number().isInteger() &&
                   mstruct[0].number().denominator().isIntegerDivisible(
                       mstruct[i][0].number())) {
      ...
      if (mstruct[i].isPower()) {
        for (size_t i2 = i + 1; i2 < mstruct.size();) {
          if (mstruct[i2].isPower() && mstruct[i][0] == mstruct[i2][0]) {
            if (!eo.allow_complex &&
                !mstruct[i][0].representsNonNegative(true) &&
                (!mstruct[i][1].representsInteger() ||
                 !mstruct[i2][1].representsInteger())) {
              break;
            }
...
bool MathStructure::complexToExponentialForm(const EvaluationOptions &eo) {
  if (m_type == STRUCT_NUMBER && o_number.hasImaginaryPart()) {
    ...
    MathStructure mabs(CALCULATOR->getFunctionById(FUNCTION_ID_ABS), this,
                       NULL);
    MathStructure marg(CALCULATOR->getFunctionById(FUNCTION_ID_ARG), this,
                       NULL);
    marg *= nr_one_i;
    marg.eval(eo2);
  ...
  for (size_t i = 0; i < SIZE; i++) {
    if (CHILD(i).complexToExponentialForm(eo)) {
      b = true;
      CHILD_UPDATED(i);
    }
  }
...
bool MathStructure::complexToPolarForm(const EvaluationOptions &eo) {
  if (m_type == STRUCT_NUMBER && o_number.hasImaginaryPart()) {
    ...
    if (u)
      marg.multiply(u, true);
    set(marg, true);
    transformById(FUNCTION_ID_SIN);
    multiply(nr_one_i);
    add_nocopy(new MathStructure(CALCULATOR->getFunctionById(FUNCTION_ID_COS),
                                 &marg, NULL));
    if (!mabs.isOne())
      multiply(mabs);
    evalSort(true);
    return true;
  } else if (m_type == STRUCT_POWER && CHILD(0).isVariable() &&
             CHILD(0).variable()->id() == VARIABLE_ID_E &&
             CHILD(1).isNumber() && CHILD(1).number().hasImaginaryPart() &&
             !CHILD(1).number().hasRealPart()) {
    MathStructure marg(CHILD(1).number().imaginaryPart());
    EvaluationOptions eo2 = eo;
    eo2.complex_number_form = COMPLEX_NUMBER_FORM_RECTANGULAR;
    switch (eo2.parse_options.angle_unit) {
    case ANGLE_UNIT_DEGREES: {
      marg.calculateMultiply(Number(180, 1), eo2);
      marg.calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI), eo2);
      marg.multiply(CALCULATOR->getDegUnit(), true);
      break;
    }
    case ANGLE_UNIT_GRADIANS: {
      marg.calculateMultiply(Number(200, 1), eo2);
      marg.calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI), eo2);
      marg.multiply(CALCULATOR->getGraUnit(), true);
      break;
    }
    case ANGLE_UNIT_RADIANS: {
      marg.multiply(CALCULATOR->getRadUnit(), true);
      break;
    }
    case ANGLE_UNIT_CUSTOM: {
      if (CALCULATOR->customAngleUnit()) {
        marg.calculateMultiply(angle_units_in_turn(eo2, 1, 2), eo2);
        marg.calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI), eo2);
        marg.multiply(CALCULATOR->customAngleUnit(), true);
      }
      break;
    }
    default: {
    ...
    evalSort(true);
    return true;
  } else if (m_type == STRUCT_MULTIPLICATION && SIZE == 2 &&
             CHILD(1).isPower() && CHILD(1)[0].isVariable() &&
             CHILD(1)[0].variable()->id() == VARIABLE_ID_E &&
             CHILD(1)[1].isNumber() &&
             CHILD(1)[1].number().hasImaginaryPart() &&
             !CHILD(1)[1].number().hasRealPart() && CHILD(0).isNumber() &&
             !CHILD(0).number().hasImaginaryPart()) {
    ...
    CHILD(1).add_nocopy(new MathStructure(
        CALCULATOR->getFunctionById(FUNCTION_ID_COS), &marg, NULL));
    CHILD_UPDATED(1)
    evalSort(true);
    return true;
  } else if (representsReal(true)) {
  ...
  if (m_type == STRUCT_POWER || m_type == STRUCT_FUNCTION)
    return false;
  bool b = false;
  for (size_t i = 0; i < SIZE; i++) {
    if (CHILD(i).complexToPolarForm(eo)) {
      b = true;
      CHILD_UPDATED(i);
    }
  }
...
bool MathStructure::complexToCisForm(const EvaluationOptions &eo) {
  if (m_type == STRUCT_NUMBER && o_number.hasImaginaryPart()) {
    ...
    switch (eo2.parse_options.angle_unit) {
    case ANGLE_UNIT_RADIANS: {
      multiply(CALCULATOR->getRadUnit(), true);
      break;
    }
    case ANGLE_UNIT_DEGREES: {
      calculateMultiply(Number(180, 1), eo2);
      calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI), eo2);
      multiply(CALCULATOR->getDegUnit(), true);
      break;
    }
    case ANGLE_UNIT_GRADIANS: {
      calculateMultiply(Number(200, 1), eo2);
      calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI), eo2);
      multiply(CALCULATOR->getGraUnit(), true);
      break;
    }
    case ANGLE_UNIT_CUSTOM: {
      if (CALCULATOR->customAngleUnit()) {
        calculateMultiply(angle_units_in_turn(eo2, 1, 2), eo2);
        calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI), eo2);
        multiply(CALCULATOR->customAngleUnit(), true);
      }
      break;
    }
    default: {
    ...
    case ANGLE_UNIT_DEGREES: {
      CHILD(1).calculateMultiply(Number(180, 1), eo2);
      CHILD(1).calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI),
                               eo2);
      CHILD(1).multiply(CALCULATOR->getDegUnit(), true);
      break;
    }
    case ANGLE_UNIT_GRADIANS: {
      CHILD(1).calculateMultiply(Number(200, 1), eo2);
      CHILD(1).calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI),
                               eo2);
      CHILD(1).multiply(CALCULATOR->getGraUnit(), true);
      break;
    }
    case ANGLE_UNIT_CUSTOM: {
      if (CALCULATOR->customAngleUnit()) {
        CHILD(1).calculateMultiply(angle_units_in_turn(eo2, 1, 2), eo2);
        CHILD(1).calculateDivide(CALCULATOR->getVariableById(VARIABLE_ID_PI),
                                 eo2);
        CHILD(1).multiply(CALCULATOR->customAngleUnit(), true);
      }
      break;
    }
    default: {
  ...
  if (b && SIZE >= 2 && isMultiplication() && i_cis < SIZE &&
      CHILD(i_cis).isMultiplication() && CHILD(i_cis).size() == 2 &&
      CHILD(i_cis).last().isFunction() &&
      CHILD(i_cis).last().function()->id() == FUNCTION_ID_CIS &&
      CHILD(i_cis)[0].isOne()) {
...
bool convert_to_default_angle_unit(MathStructure &m,
                                   const EvaluationOptions &eo) {
  bool b = false;
  for (size_t i = 0; i < m.size(); i++) {
    if (convert_to_default_angle_unit(m[i], eo))
      b = true;
    if (m.isFunction() && m.function()->getArgumentDefinition(i + 1) &&
        m.function()->getArgumentDefinition(i + 1)->type() ==
            ARGUMENT_TYPE_ANGLE) {
      Unit *u = NULL;
      if (eo.parse_options.angle_unit == ANGLE_UNIT_DEGREES)
        u = CALCULATOR->getDegUnit();
      else if (eo.parse_options.angle_unit == ANGLE_UNIT_GRADIANS)
        u = CALCULATOR->getGraUnit();
      else if (eo.parse_options.angle_unit == ANGLE_UNIT_CUSTOM)
        u = CALCULATOR->customAngleUnit();
...
bool remove_add_zero_unit(MathStructure &m) {
  if (m.isAddition() && m.size() > 1) {
    ...
    for (size_t i = 0; i < m.size(); i++) {
      if (m[i].isMultiplication() && m[i].size() > 1 && m[i][0].isZero() &&
          !m[i].isApproximate()) {
        b = true;
      } else {
        b2 = true;
      ...
      if (m[i].isMultiplication() && m[i].size() > 1 && m[i][0].isZero() &&
        ...
        for (size_t i2 = 1; i2 < m[i].size(); i2++) {
          if (!m[i][i2].isUnit_exp() ||
              (m[i][i2].isPower() &&
               m[i][i2][0].unit()->hasNonlinearRelationToBase()) ||
              (m[i][i2].isUnit() &&
               m[i][i2].unit()->hasNonlinearRelationToBase())) {
            b2 = false;
            break;
          }
        }
...
Unit *find_log_unit(const MathStructure &m, bool toplevel = true) {
  if (!toplevel && m.isUnit() && m.unit()->subtype() == SUBTYPE_ALIAS_UNIT &&
      ((AliasUnit *)m.unit())->hasNonlinearExpression() &&
      (((AliasUnit *)m.unit())->expression().find("log") != string::npos ||
       ((AliasUnit *)m.unit())->inverseExpression().find("log") !=
           string::npos ||
       ((AliasUnit *)m.unit())->expression().find("ln") != string::npos ||
       ((AliasUnit *)m.unit())->inverseExpression().find("ln") !=
           string::npos)) {
  ...
  if (m.isMultiplication() && toplevel && m.last().isUnit()) {
    ...
    if (u) {
      for (size_t i = 0; i < m.size() - 1; i++) {
        if (m[i].containsType(STRUCT_UNIT, true))
          return u;
      }
      return NULL;
    }
    ...
    if (mvar.contains(u, false, true, true)) {
      if (mvar.isMultiplication()) {
        ...
        for (size_t i = 0; i < mvar.size(); i++) {
          if (is_unit_multiexp(mvar[i])) {
            b = true;
          } else if (mvar[i].containsType(STRUCT_UNIT, false, true, true) !=
                     0) {
            b = false;
            break;
          }
      ...
      if (eo.calculate_variables &&
          ((eo.approximation != APPROXIMATION_EXACT &&
            eo.approximation != APPROXIMATION_EXACT_VARIABLES) ||
           (!m.variable()->isApproximate() &&
            !mvar.containsInterval(true, false, false, 0, true)))) {
        m.set(mvar);
        m.unformat(eo);
        separate_unit(m, u, eo);
        return true;
...
void separate_unit2(MathStructure &m, Unit *u, const EvaluationOptions &eo) {
  if (m.isMultiplication()) {
    ...
    for (size_t i = 0; i < m.size(); i++) {
      ...
      if (m[i].isUnit_exp()) {
        if (i_unit < 1 && ((m[i].isUnit() && m[i].unit() == u) ||
                           (m[i].isPower() && m[i][0].unit() == u))) {
          if (i_unit == i - 1) {
            m[i].multiply(m_one);
        ...
        i_unit = i;
                 ...
                 m[i].containsType(STRUCT_UNIT, false, true, true)) {
...
void convert_log_units(MathStructure &m, const EvaluationOptions &eo) {
  while (true) {
    ...
    if (!m.convert(u, true, NULL, false, eo))
      break;
    CALCULATOR->error(
        false, "Log-based units were converted before calculation.", NULL);
  }
}

Unit *contains_temperature_unit(const MathStructure &m, bool only_cf = true,
                                Unit *u_prev = NULL) {
  if (m.isUnit()) {
    if (!only_cf) {
      if (m.unit() != u_prev &&
          (m.unit() == CALCULATOR->getUnitById(UNIT_ID_KELVIN) ||
           m.unit()->containsRelativeTo(
               CALCULATOR->getUnitById(UNIT_ID_KELVIN))))
        return m.unit();
    } else if (m.unit() == CALCULATOR->getUnitById(UNIT_ID_CELSIUS) ||
               m.unit() == CALCULATOR->getUnitById(UNIT_ID_FAHRENHEIT)) {
      return m.unit();
    }
  }
  if (m.isVariable() && m.variable()->isKnown()) {
    return contains_temperature_unit(((KnownVariable *)m.variable())->get(),
                                     only_cf, u_prev);
  }
  if (m.isFunction() && m.function()->id() == FUNCTION_ID_STRIP_UNITS)
    return NULL;
...
void separate_temperature_units2(MathStructure &m,
                                 const EvaluationOptions &eo) {
  if (m.isMultiplication()) {
    size_t i_unit = m.size();
    for (size_t i = 0; i < m.size(); i++) {
      ...
      if (m[i].isUnit_exp()) {
        if (i_unit < 1 &&
            ((m[i].isUnit() && m[i].unit()->baseUnit() ==
                                   CALCULATOR->getUnitById(UNIT_ID_KELVIN)) ||
             (m[i].isPower() && m[i][0].unit()->baseUnit() ==
                                    CALCULATOR->getUnitById(UNIT_ID_KELVIN)))) {
          if (i_unit == i - 1) {
            m[i].multiply(m_one);
            m[i].swapChildren(1, 2);
...
void convert_temperature_units(MathStructure &m, const EvaluationOptions &eo) {
  if (CALCULATOR->getTemperatureCalculationMode() ==
  ...
  if (!contains_temperature_unit(m, false, u)) {
    ...
    if (mp->isMultiplication() && mp->size() > 0 && mp->last().isUnit_exp()) {
      ...
      for (size_t i = 0; i < mp->size() - 1; i++) {
        if (contains_temperature_unit((*mp)[i], true)) {
          b = true;
          break;
        }
      }
...
bool warn_ratio_units(MathStructure &m, bool top_level = true) {
  if (!top_level && m.isUnit() &&
      ((m.unit()->subtype() == SUBTYPE_BASE_UNIT &&
        m.unit()->referenceName() == "Np") ||
       (m.unit()->subtype() == SUBTYPE_ALIAS_UNIT &&
        ((AliasUnit *)m.unit())->baseUnit()->referenceName() == "Np"))) {
    CALCULATOR->error(true,
                      _("Logarithmic ratio units are treated as other units "
                        "and the result might not be as expected."),
                      NULL);
    return true;
  }
...
bool try_isolate_x(MathStructure &mstruct, EvaluationOptions &eo3,
                   const EvaluationOptions &eo) {
  if (mstruct.isProtected())
    return false;
  if (mstruct.isComparison()) {
    CALCULATOR->beginTemporaryStopMessages();
    MathStructure mtest(mstruct);
    ...
    if (eo.isolate_var)
      ...
      x_var2 = &mstruct.find_x_var();
    if (x_var2->isUndefined() ||
        (mtest[0] == *x_var2 && !mtest[1].contains(*x_var2))) {
      CALCULATOR->endTemporaryStopMessages();
      return false;
    }
    ...
    bool b = false;
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (try_isolate_x(mstruct[i], eo3, eo))
        b = true;
    }
    return b;
  }
...
bool MathStructure::structure(StructuringMode structuring,
                              const EvaluationOptions &eo, bool restore_first) {
  switch (structuring) {
  ...
  case STRUCTURING_FACTORIZE: {
    return factorize(eo, restore_first, 3, 0, true, 2, NULL, m_undefined, true,
                     false, -1);
  }
  default: {
...
bool merge_uncertainty(MathStructure &m, MathStructure &munc,
                       const EvaluationOptions &eo3) {
  bool b_failed = true;
  if (munc.isVector() && m.isVector() && munc.size() == m.size()) {
    b_failed = false;
    for (size_t i = 0; i < m.size(); i++) {
      if (!merge_uncertainty(m[i], munc[i], eo3))
        b_failed = true;
    }
    m.childrenUpdated();
    return !b_failed;
  }
  if (munc.isFunction() && munc.function()->id() == FUNCTION_ID_ABS &&
      munc.size() == 1) {
    munc.setToChild(1);
  }
...
test_munc:
  if (munc.isNumber()) {
      ...
      for (size_t i = 0; i < m.size(); i++) {
        if (m[i].isNumber()) {
          ...
          break;
               ...
               m.last().isUnit() &&
               m.last().unit() == CALCULATOR->getRadUnit()) {
    ...
    if (munc.isMultiplication()) {
      if (munc.size() == 2) {
        if (m.isMultiplication() && m[0].isNumber() &&
            (munc[1] == m[1] || (munc[1].isFunction() &&
                                 munc[1].function()->id() == FUNCTION_ID_ABS &&
                                 munc[1].size() == 1 && m[1] == munc[1][0]))) {
          m[0].number().setUncertainty(munc[0].number());
          m[0].numberUpdated();
          m.childUpdated(1);
          b_failed = false;
        } else if (m.equals(munc[1]) ||
                   (munc[1].isFunction() &&
                    munc[1].function()->id() == FUNCTION_ID_ABS &&
                    munc[1].size() == 1 && m.equals(munc[1][0]))) {
        ...
        if (m[0].isNumber())
          i2++;
        if (m.size() + 1 - i2 != munc.size() && m.last().isUnit() &&
            m.last().unit() == CALCULATOR->getRadUnit()) {
        ...
        if (m.size() + 1 - i2 == munc.size()) {
          ...
          for (size_t i = 1; i < munc.size(); i++, i2++) {
            if (!munc[i].equals(m[i2]) &&
                !(munc[i].isFunction() &&
                  munc[i].function()->id() == FUNCTION_ID_ABS &&
                  munc[i].size() == 1 && m[i2] == munc[i][0])) {
              b = false;
              break;
            }
          }
  ...
  if (b_failed && one_prepended && munc.isMultiplication() && munc[0].isOne())
    munc.delChild(1, true);
  return !b_failed;
}

bool separate_vector_vars(MathStructure &m, const EvaluationOptions &eo,
                          vector<KnownVariable *> &vars,
                          vector<MathStructure> &values) {
  if (m.isVariable() && m.variable()->isKnown() &&
      (!m.variable()->isApproximate() ||
      ...
      if (!b) {
        ...
        for (size_t i = 0; i < mvar.size(); i++) {
          if (mvar[i].containsInterval(true, false, false, 1, true)) {
            KnownVariable *v = new KnownVariable(
                "", string("(") + format_and_print(mvar[i]) + ")", mvar[i]);
            m.addChild(v);
            v->ref();
            v->destroy();
          } else {
...
#define FORMAT_COMPLEX_NUMBERS                                                 \
  if (eo.complex_number_form == COMPLEX_NUMBER_FORM_EXPONENTIAL)               \
    complexToExponentialForm(eo);                                              \
  else if (eo.complex_number_form == COMPLEX_NUMBER_FORM_POLAR)                \
    complexToPolarForm(eo);                                                    \
  else if (eo.complex_number_form == COMPLEX_NUMBER_FORM_CIS)                  \
    complexToCisForm(eo);

void replace_aborted_variables(MathStructure &m) {
  if (m.isVariable() && m.variable()->isKnown() &&
      !m.variable()->isRegistered() &&
      m.variable()->referenceName().find(CALCULATOR->abortedMessage())) {
...
MathStructure &MathStructure::eval(const EvaluationOptions &eo) {
  ...
  if (eo.structuring != STRUCTURING_NONE)
    warn_ratio_units(*this);

  unformat(eo);

  if (CALCULATOR->aborted() || m_type == STRUCT_UNDEFINED ||
      m_type == STRUCT_ABORTED || m_type == STRUCT_DATETIME ||
      m_type == STRUCT_UNIT || m_type == STRUCT_SYMBOLIC ||
      (m_type == STRUCT_VARIABLE && !o_variable->isKnown()))
    return *this;
  ...
  if (eo.interval_calculation == INTERVAL_CALCULATION_INTERVAL_ARITHMETIC) {
    if (eo.calculate_functions)
      calculate_nondifferentiable_functions(*this, feo, true, true, 0);
    if (((eo.approximation != APPROXIMATION_EXACT &&
          eo.approximation != APPROXIMATION_EXACT_VARIABLES &&
          eo.calculate_variables) &&
         containsInterval(true, true, false, 1, true)) ||
        (eo.sync_units && eo.approximation != APPROXIMATION_EXACT_VARIABLES &&
         eo.approximation != APPROXIMATION_EXACT &&
         sync_approximate_units(*this, eo)) ||
        (eo.approximation == APPROXIMATION_EXACT &&
         contains_function_interval(*this, true, true, false, 1, true))) {
      EvaluationOptions eo3 = eo2;
      eo3.split_squares = false;
      eo3.assume_denominators_nonzero = false;
      if (eo.approximation == APPROXIMATION_APPROXIMATE && !containsUnknowns())
        eo3.approximation = APPROXIMATION_EXACT_VARIABLES;
      else
        eo3.approximation = APPROXIMATION_EXACT;
      vector<KnownVariable *> vars;
      ...
      while (eo.sync_units &&
             (separate_unit_vars(*this, feo, true) ||
              sync_approximate_units(*this, feo, &vars, &uncs, false))) {
        calculatesub(eo3, eo3);
      }
      eo3.approximation = APPROXIMATION_APPROXIMATE;
      if (eo.sync_units) {
    ...
    if (eo.calculate_functions)
      calculate_nondifferentiable_functions(*this, feo, true, true, -1);
    if (!isNumber() &&
        (((eo.approximation != APPROXIMATION_EXACT &&
           eo.approximation != APPROXIMATION_EXACT_VARIABLES &&
           eo.calculate_variables) &&
          containsInterval(true, true, false, 1, true)) ||
         containsInterval(true, false, false, 1, true) ||
         (eo.sync_units && eo.approximation != APPROXIMATION_EXACT &&
          sync_approximate_units(*this, eo) &&
          containsInterval(true, true, false, 1, true)) ||
         (eo.approximation == APPROXIMATION_EXACT &&
          contains_function_interval(*this, true, true, false, 1, true)))) {

      // calculate non-differentiable functions (not handled by variance
      // formula) and functions without uncertainties
      if (eo.calculate_functions)
      ...
      if (containsType(STRUCT_COMPARISON)) {
        ...
        // Handle comparisons by exact calculation followed by calculation of
        // each side of comparisons separately

        EvaluationOptions eo3 = eo;
        eo3.approximation = APPROXIMATION_EXACT;
        eo3.interval_calculation =
        ...
        for (size_t i = 0; i < vars.size(); i++) {
          if (vars[i]->isKnown())
            replace(vars[i], ((KnownVariable *)vars[i])->get());
          else
            replace(vars[i], ((UnknownVariable *)vars[i])->interval());
          vars[i]->destroy();
        }
        ...
        if (eval_comparison_sides(*this, feo)) {
          ...
          CALCULATOR->error(
              false,
              _("Calculation of uncertainty propagation partially failed "
                "(using interval arithmetic instead when necessary)."),
              NULL);
          ...
          for (size_t i = 0; i < SIZE; i++) {
            if (CHILD(i).isVector()) {
              ...
              for (size_t i2 = 0; i2 < CHILD(i).size(); i2++) {
                munc[i].addChild(
                    calculate_uncertainty(CHILD(i)[i2], eo, b_failed));
                if (b_failed)
                  break;
              }
            } else {
            ...
            for (size_t i = 0; i < mtest.size(); i++) {
              if (mtest[i].isVector()) {
                ...
                for (size_t i2 = 0; i2 < mtest[i].size(); i2++) {
                  munc[i].addChild(
                      calculate_uncertainty(mtest[i][i2], eo, b_failed));
                  if (b_failed)
                    break;
                }
              } else {
        ...
        if (!b_failed && !munc.isZero()) {
          ...
          eo3.interval_calculation =
              INTERVAL_CALCULATION_SIMPLE_INTERVAL_ARITHMETIC;
          if (eo3.approximation == APPROXIMATION_TRY_EXACT)
            eo3.approximation = APPROXIMATION_APPROXIMATE;
          munc.eval(eo3);
        ...
        if (b_failed) {
          ...
          CALCULATOR->error(false,
                            _("Calculation of uncertainty propagation failed "
                              "(using interval arithmetic instead)."),
                            NULL);
          EvaluationOptions eo3 = eo;
  ...
  if (eo2.interval_calculation == INTERVAL_CALCULATION_INTERVAL_ARITHMETIC ||
      eo2.interval_calculation == INTERVAL_CALCULATION_VARIANCE_FORMULA)
  ...
  if (eo2.approximation == APPROXIMATION_TRY_EXACT ||
      (eo2.approximation == APPROXIMATION_APPROXIMATE &&
       (containsUnknowns() || containsInterval(false, true, false, 0)))) {
    EvaluationOptions eo3 = eo2;
    if (eo.approximation == APPROXIMATION_APPROXIMATE && !containsUnknowns())
      eo3.approximation = APPROXIMATION_EXACT_VARIABLES;
    else
    ...
    if (eo.interval_calculation < INTERVAL_CALCULATION_INTERVAL_ARITHMETIC &&
        eo.expand && eo.expand >= -1 &&
        !containsType(STRUCT_COMPARISON, true, true, true)) {
      unformat(eo);
      eo3.expand = -1;
      calculatesub(eo3, feo);
    }
    eo3.approximation = APPROXIMATION_APPROXIMATE;
    if (eo2.interval_calculation >= INTERVAL_CALCULATION_INTERVAL_ARITHMETIC &&
        containsInterval(false, true, false, -1))
      factorize_variables(*this, eo3);
    if (containsType(STRUCT_COMPARISON) &&
        containsInterval(false, true, false, false)) {
      eo3.approximation = APPROXIMATION_EXACT;
    ...
    eo2.approximation = APPROXIMATION_APPROXIMATE;
  } else if (eo2.approximation == APPROXIMATION_EXACT &&
             contains_function_interval(*this, false, true, false, -1)) {
    EvaluationOptions eo3 = eo2;
    eo3.split_squares = false;
    eo3.assume_denominators_nonzero = false;
    calculatesub(eo3, feo);
    eo3.approximation = APPROXIMATION_APPROXIMATE;
    if (eo2.interval_calculation >= INTERVAL_CALCULATION_INTERVAL_ARITHMETIC)
      factorize_variables(*this, eo3, true);
  }
  if (eo2.approximation == APPROXIMATION_EXACT)
    replace_function_vars(*this);
  ...
  if (m_type == STRUCT_UNDEFINED || m_type == STRUCT_ABORTED ||
      m_type == STRUCT_DATETIME || m_type == STRUCT_UNIT ||
  ...
  // Try isolate x after expanding expression and perform simplification using
  // gcd and polynomial division
  if (eo.expand != 0 ||
      (eo.test_comparisons && containsType(STRUCT_COMPARISON))) {
    ...
    if (b) {
      ...
      if (eo.do_polynomial_division)
        do_simplification(*this, eo2, true,
      ...
      if (eo2.isolate_x) {
        ...
        if (isolate_x(eo2, feo)) {
          ...
          if (eo.assume_denominators_nonzero)
            eo2.assume_denominators_nonzero = 2;
          calculatesub(eo2, feo);
          if (containsType(STRUCT_ADDITION, false) == 1 &&
  ...
  // Final attempt to isolate x, by assuming denominators is non zero and
  // testing validity afterwards
  if (eo2.isolate_x && containsType(STRUCT_COMPARISON) &&
      eo2.assume_denominators_nonzero) {
    eo2.assume_denominators_nonzero = 2;
  ...
  if (eo.structuring != STRUCTURING_NONE) {
    ...
    if (!DEFAULT_RADIANS(eo.parse_options.angle_unit))
      convert_to_default_angle_unit(*this, eo);
    simplify_ln(*this);
    simplify_roots(*this, eo);
