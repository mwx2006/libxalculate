...
int BitXorFunction::calculate(MathStructure &mstruct,
  ...
  if (vargs[0].isNumber() && vargs[1].isNumber()) {
    ...
    if (mstruct.size() < vargs[i1].size())
      return 0;
    size_t i = 0;
    for (; i < vargs[i2].size(); i++) {
      mstruct[i].set(CALCULATOR->getFunctionById(FUNCTION_ID_XOR),
                     &vargs[i1][i], &vargs[i2][0], NULL);
    }
...
int XorFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
...
int ShiftFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                             const EvaluationOptions &eo) {
  if (vargs.size() >= 3 && !vargs[2].number().getBoolean() &&
      vargs[1].number().isNegative()) {
    Number nr(vargs[0].number());
    Number nr_div(vargs[1].number());
    if (!nr_div.negate() || !nr_div.exp2() || !nr.divide(nr_div) || !nr.trunc())
      return false;
    mstruct.set(nr);
    return 1;
  }
...
int CircularShiftFunction::calculate(MathStructure &mstruct,
                                     const MathStructure &vargs,
                                     const EvaluationOptions &eo) {
...
int BitCmpFunction::calculate(MathStructure &mstruct,
                              const MathStructure &vargs,
                              const EvaluationOptions &eo) {
  Number nr(vargs[0].number());
  if (vargs.size() >= 3 && vargs[2].number().getBoolean()) {
...
bool IFFunction::representsScalar(const MathStructure &vargs) const {
  return vargs.size() >= 3 && vargs[1].representsScalar() &&
         vargs[2].representsScalar();
}
bool IFFunction::representsNonMatrix(const MathStructure &vargs) const {
  return vargs.size() >= 3 && vargs[1].representsNonMatrix() &&
         vargs[2].representsNonMatrix();
}
int IFFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                          const EvaluationOptions &eo) {
  if (vargs[0].isNumber()) {
  ...
  if (mstruct.isVector() ||
    ...
    for (size_t i = 0; i < mstruct.size(); i++) {
      ...
      if (result > 0) {
        if (!m2.isVector() &&
            (!m2.isFunction() ||
             (m2.function()->id() != FUNCTION_ID_HORZCAT &&
              m2.function()->id() != FUNCTION_ID_VERTCAT)) &&
            !m2.representsScalar())
          m2.eval(eo);
        if ((m2.isVector() || (m2.isFunction() &&
                               (m2.function()->id() == FUNCTION_ID_HORZCAT ||
                                m2.function()->id() == FUNCTION_ID_VERTCAT))) &&
            m2.size() > 0) {
          if (i >= m2.size())
            mstruct = m2[i % m2.size()];
          else
            mstruct = m2[i];
        } else {
          mstruct = m2;
...
bool calculate_replace2(MathStructure &m, const MathStructure &mfrom1,
                        const MathStructure &mto1, const MathStructure &mfrom2,
  ...
  if (b) {
    ...
    if (eo.calculate_functions && m.type() == STRUCT_FUNCTION)
      m.calculateFunctions(eo, false);
  }
  return b;
}
bool calculate_userfunctions2(MathStructure &m, const MathStructure &x_mstruct,
                              const MathStructure &x_mstruct2,
                              const EvaluationOptions &eo) {
  bool b_ret = false;
  for (size_t i = 0; i < m.size(); i++) {
  ...
  if (m.isFunction()) {
    if (!m.contains(x_mstruct, true) && !m.contains(x_mstruct2, true) &&
...
int ForFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
                           const EvaluationOptions &eo) {
  ...
  if (eo.interval_calculation == INTERVAL_CALCULATION_VARIANCE_FORMULA ||
    ...
    while (true) {
      ...
      if (!uv)
        break;
      if (v)
        mstruct.replace(v, uv);
      vars.push_back(uv);
    }
  }
  mstruct.eval(eo);
  MathStructure m5(vargs[5]);
  if (vargs[5].isComparison() &&
      vargs[5].comparisonType() == COMPARISON_EQUALS && vargs[5][0] == vargs[6])
    m5 = vargs[5][1];
  else
    m5 = vargs[5];
  ...
  while (true) {
    ...
    mstruct = mupdate;
    if (vargs[3].isComparison() &&
        vargs[3].comparisonType() == COMPARISON_EQUALS &&
        vargs[3][0] == vargs[1])
      mcount = vargs[3][1];
    else
      mcount = vargs[3];
