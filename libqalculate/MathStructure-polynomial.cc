...
void MathStructure::polynomialContent(const MathStructure &xvar,
  ...
  for (Number i(ldeg); i.isLessThanOrEqualTo(deg); i++) {
    ...
    mtmp = mcontent;
    if (!MathStructure::gcd(coeff, mtmp, mcontent, eo, NULL, NULL, false))
      mcontent.set(1, 1, 0);
    if (mcontent.isOne())
      break;
  }
...
void MathStructure::polynomialPrimpart(const MathStructure &xvar,
                                       MathStructure &mprim,
                                       const EvaluationOptions &eo) const {
  if (isZero()) {
    mprim.clear();
...
void MathStructure::polynomialUnitContentPrimpart(
    const MathStructure &xvar, int &munit, MathStructure &mcontent,
    MathStructure &mprim, const EvaluationOptions &eo) const {
  ...
  if (mcontent.isNumber()) {
    ...
    if (munit == -1) {
      ...
      MathStructure::polynomialQuotient(*this, mcontent, xvar, mprim, eo,
                                        false);
    }
  }
}
  ...
  for (size_t i = 0;; i++) {
    ...
    if ((*mcur) == xvar) {
      ...
      for (size_t i2 = 0; i2 < mcur->size(); i2++) {
        ...
        } else if ((*mcur)[i2].isPower() && (*mcur)[i2][0] == xvar &&
                   (*mcur)[i2][1].isNumber()) {
...
void MathStructure::lcoefficient(const MathStructure &xvar,
                                 MathStructure &mcoeff) const {
  coefficient(xvar, degree(xvar), mcoeff);
}
void MathStructure::tcoefficient(const MathStructure &xvar,
                                 MathStructure &mcoeff) const {
  coefficient(xvar, ldegree(xvar), mcoeff);
}
void MathStructure::coefficient(const MathStructure &xvar, const Number &pownr,
                                MathStructure &mcoeff) const {
  const MathStructure *mcur = NULL;
  mcoeff.clear();
  for (size_t i = 0;; i++) {
...
bool MathStructure::polynomialQuotient(const MathStructure &mnum,
  ...
  if (check_args &&
      (!mnum.isRationalPolynomial() || !mden.isRationalPolynomial())) {
    return false;
  }
  ...
  for (size_t i = 0; numdeg.isGreaterThanOrEqualTo(dendeg); i++) {
    ...
    if (numcoeff == dencoeff) {
      ...
      if (dencoeff.isNumber()) {
        ...
        if (!MathStructure::polynomialDivide(mcopy, dencoeff, numcoeff, eo2,
                                             false)) {
          return false;
        }
      }
    ...
    if (mquotient.isZero())
      ...
      mquotient.calculateAdd(numcoeff, eo2);
...
bool get_first_symbol(const MathStructure &mpoly, MathStructure &xvar) {
  if (IS_A_SYMBOL(mpoly) || mpoly.isUnit()) {
    ...
    for (size_t i = 0; i < mpoly.size(); i++) {
      if (get_first_symbol(mpoly[i], xvar))
        return true;
    }
  } else if (mpoly.isPower()) {
...
bool MathStructure::polynomialDivide(const MathStructure &mnum,
                                     const MathStructure &mden,
                                     MathStructure &mquotient,
                                     const EvaluationOptions &eo,
                                     bool check_args) {
  ...
  MathStructure xvar;
  if (!get_first_symbol(mnum, xvar) && !get_first_symbol(mden, xvar))
    return false;
  ...
  for (size_t i = 0; numdeg.isGreaterThanOrEqualTo(dendeg); i++) {
    ...
    if (numcoeff == dencoeff) {
      ...
      if (dencoeff.isNumber()) {
        ...
        MathStructure mcopy(numcoeff);
        if ((mcopy.equals(mnum, true, true) &&
             dencoeff.equals(mden, true, true)) ||
            !MathStructure::polynomialDivide(mcopy, dencoeff, numcoeff, eo2,
                                             false)) {
          return false;
        }
      }
    ...
    if (mquotient.isZero())
      ...
      mquotient.add(numcoeff, true);
    if (numcoeff.isAddition() && mden.isAddition() &&
        numcoeff.size() * mden.size() >= (eo.expand == -1 ? 50 : 500))
      return false;
    numcoeff.calculateMultiply(mden, eo2);
    mrem.calculateSubtract(numcoeff, eo2);
    if (mrem.isZero()) {
...
bool divide_in_z(const MathStructure &mnum, const MathStructure &mden,
  ...
  if (mden.isPower()) {
    ...
    for (Number ni(mden[1].number()); ni.isPositive(); ni--) {
      if (!divide_in_z(qbar, mden[0], mquotient, sym_stats, var_i, eo))
        return false;
      qbar = mquotient;
    }
    return true;
  ...
  if (mden.isMultiplication()) {
    ...
    for (size_t i = 0; i < mden.size(); i++) {
      ...
      if (!divide_in_z(qbar, mden[i], mquotient, sym_stats2, 0, eo))
        return false;
      qbar = mquotient;
    }
    return true;
  ...
  Number numdeg = mnum.degree(xvar);
  Number dendeg = mden.degree(xvar);
  if (dendeg.isGreaterThan(numdeg))
    return false;
  MathStructure dencoeff;
  MathStructure mrem(mnum);
  mden.coefficient(xvar, dendeg, dencoeff);
  ...
  while (numdeg.isGreaterThanOrEqualTo(dendeg)) {
    ...
    mrem.coefficient(xvar, numdeg, numcoeff);
    MathStructure term;
    if (!divide_in_z(numcoeff, dencoeff, term, sym_stats, var_i + 1, eo))
      break;
    numdeg -= dendeg;
    ...
    if (term.isAddition() && mden.isAddition() &&
        term.size() * mden.size() >= (eo.expand == -1 ? 50 : 500))
      return false;
...
bool prem(const MathStructure &mnum, const MathStructure &mden,
          ...
          const EvaluationOptions &eo, bool check_args) {
  ...
  while (rdeg.isGreaterThanOrEqualTo(bdeg) && !mrem.isZero()) {
    ...
    if (rdeg == 0) {
      ...
      if (mrem.isAddition() && blcoeff.isAddition() &&
          mrem.size() * blcoeff.size() >= (eo.expand == -1 ? 50 : 500)) {
        mrem.clear();
        return false;
      }
...
void polynomial_smod(const MathStructure &mpoly, const Number &xi,
                     ...
                     MathStructure *mparent, size_t index_smod) {
  if (mpoly.isNumber()) {
    msmod = mpoly;
...
bool contains_zerointerval_multiplier(MathStructure &mstruct) {
  if (mstruct.isAddition()) {
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (contains_zerointerval_multiplier(mstruct[i]))
        return true;
    }
...
bool polynomial_long_division(const MathStructure &mnum,
  ...
  if (check_args && (!mnum.isRationalPolynomial(true, true) ||
                     !mden.isRationalPolynomial(true, true))) {
    return false;
  }
  ...
  if (xvar.isZero()) {
    ...
    for (size_t i = 0; i < symsd.size(); i++) {
      ...
      if (polynomial_long_division(mrem, mden, symsd[i], mquotient2, mrem2, eo,
                                   false, for_newtonraphson) &&
          !mquotient2.isZero()) {
        mrem = mrem2;
        if (mquotient.isZero())
          mquotient = mquotient2;
        else
          mquotient.calculateAdd(mquotient2, eo);
      }
      if (CALCULATOR->aborted()) {
        mrem = mnum;
        mquotient.clear();
  ...
  for (size_t i = 0; numdeg.isGreaterThanOrEqualTo(dendeg); i++) {
    ...
    numcoeff.calculateMultiply(mden, eo2);
    mrem.calculateSubtract(numcoeff, eo2);
    if (contains_zerointerval_multiplier(mquotient)) {
      mrem = mnum;
      mquotient.clear();
      return false;
    }
    if (mrem.isZero() || (for_newtonraphson && mrem.isNumber()))
      break;
    if (contains_zerointerval_multiplier(mrem)) {
      mrem = mnum;
      mquotient.clear();
      return false;
    }
...
bool MathStructure::isRationalPolynomial(
    bool allow_non_rational_coefficient,
    bool allow_interval_coefficient) const {
  switch (m_type) {
  case STRUCT_NUMBER: {
    if (allow_interval_coefficient)
      return o_number.isReal() && o_number.isNonZero();
    if (allow_non_rational_coefficient)
      return o_number.isReal() && !o_number.isInterval() &&
             o_number.isNonZero();
    return o_number.isRational() && !o_number.isZero();
  }
  case STRUCT_MULTIPLICATION: {
    for (size_t i = 0; i < SIZE; i++) {
      if (CHILD(i).isAddition() || CHILD(i).isMultiplication() ||
          !CHILD(i).isRationalPolynomial(allow_non_rational_coefficient,
                                         allow_interval_coefficient)) {
        return false;
      }
    }
    return true;
  ...
  case STRUCT_POWER: {
    return CHILD(1).isInteger() && CHILD(1).number().isNonNegative() &&
           !CHILD(1).number().isOne() && CHILD(1).number() < 1000 &&
           !CHILD(0).isNumber() && !CHILD(0).isMultiplication() &&
           !CHILD(0).isAddition() && !CHILD(0).isPower() &&
           CHILD(0).isRationalPolynomial(allow_non_rational_coefficient,
                                         allow_interval_coefficient);
  }
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_UNCERTAINTY ||
        o_function->id() == FUNCTION_ID_INTERVAL || containsInterval() ||
        containsInfinity())
      return false;
  }
  case STRUCT_UNIT: {
