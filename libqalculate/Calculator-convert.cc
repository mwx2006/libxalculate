...
MathStructure Calculator::convertToMixedUnits(const MathStructure &mstruct,
  ...
  if (mstruct[1].isUnit() &&
      (!mstruct[1].prefix() || mstruct[1].prefix() == decimal_null_prefix) &&
      mstruct[0].isNumber()) {
    ...
    bool accept_obsolete = (u->subtype() == SUBTYPE_ALIAS_UNIT &&
                            abs(((AliasUnit *)u)->mixWithBase()) > 1);
    Unit *original_u = u;
    ...
    while (muc > MIXED_UNITS_CONVERSION_DOWNWARDS && nr.isGreaterThan(nr_one)) {
      ...
      if (u->subtype() == SUBTYPE_ALIAS_UNIT &&
          ...
          ((AliasUnit *)u)->firstBaseUnit()->subtype() !=
              SUBTYPE_COMPOSITE_UNIT) {
        size_t idiv = ((AliasUnit *)u)->expression().find("1" DIVISION);
        if (idiv == 0 &&
            ((AliasUnit *)u)->expression().find_first_not_of(NUMBERS, 2) ==
                string::npos) {
          if ((((AliasUnit *)u)->mixWithBase() > 0 ||
               (((AliasUnit *)u)->mixWithBase() == 0 &&
                (muc == MIXED_UNITS_CONVERSION_FORCE_INTEGER ||
                 muc == MIXED_UNITS_CONVERSION_FORCE_ALL))) &&
              (((AliasUnit *)u)->mixWithBaseMinimum() <= 1 ||
               nr.isGreaterThanOrEqualTo(
                   ((AliasUnit *)u)->mixWithBaseMinimum()))) {
            ...
            if (!mstruct_nr.isNumber() || !m_exp.isOne() ||
                !mstruct_nr.number().isLessThan(nr) ||
                !mstruct_nr.number().isGreaterThanOrEqualTo(nr_one)) {
              best_u = NULL;
            } else {
      ...
      for (size_t i = 0; i < units.size(); i++) {
        Unit *ui = units[i];
        if (ui->subtype() == SUBTYPE_ALIAS_UNIT &&
            ((AliasUnit *)ui)->firstBaseUnit() == u &&
            ((AliasUnit *)ui)->firstBaseExponent() == 1) {
          AliasUnit *aui = (AliasUnit *)ui;
          int priority_i = aui->mixWithBase();
          if (((priority_i > 0 && (!best_u || priority_i <= best_priority)) ||
               (best_priority == 0 && priority_i == 0 &&
                ((muc == MIXED_UNITS_CONVERSION_FORCE_INTEGER &&
                  aui->expression().find_first_not_of(NUMBERS) ==
                      string::npos) ||
                 muc == MIXED_UNITS_CONVERSION_FORCE_ALL))) &&
              (aui->mixWithBaseMinimum() <= 1 ||
               nr.isGreaterThanOrEqualTo(aui->mixWithBaseMinimum()))) {
            MathStructure mstruct_nr(nr);
            MathStructure m_exp(m_one);
            ...
            if (mstruct_nr.isNumber() && m_exp.isOne() &&
                mstruct_nr.number().isLessThan(nr) &&
                mstruct_nr.number().isGreaterThanOrEqualTo(nr_one) &&
                (!best_u || mstruct_nr.number().isLessThan(best_nr))) {
              best_u = ui;
              best_nr = mstruct_nr.number();
      ...
      if (!best_u)
        break;
      if (best_priority != 0 && muc > MIXED_UNITS_CONVERSION_DEFAULT)
    ...
    if (u != original_u) {
      if (negated)
        last_nonobsolete_nr.negate();
      mstruct_new[0].set(last_nonobsolete_nr);
      mstruct_new[1].set(u, p);
    }
    while ((u->subtype() == SUBTYPE_BASE_UNIT ||
            (u->subtype() == SUBTYPE_ALIAS_UNIT &&
             ((AliasUnit *)u)->firstBaseUnit()->subtype() !=
                 SUBTYPE_COMPOSITE_UNIT &&
             ((AliasUnit *)u)->firstBaseExponent() == 1 &&
             (((AliasUnit *)u)->mixWithBase() != 0 ||
              muc == MIXED_UNITS_CONVERSION_FORCE_ALL ||
              muc == MIXED_UNITS_CONVERSION_FORCE_INTEGER))) &&
           !nr.isInteger() && nr.isNonZero()) {
      Number int_nr(nr);
      int_nr.intervalToMidValue();
      int_nr.trunc();
      if (muc == MIXED_UNITS_CONVERSION_DOWNWARDS_KEEP && int_nr.isZero())
        break;
      nr -= int_nr;
      bool b = false;
      Number best_nr;
      ...
      if (u->subtype() == SUBTYPE_ALIAS_UNIT &&
          (muc == MIXED_UNITS_CONVERSION_FORCE_ALL ||
           (((AliasUnit *)u)->expression().find_first_not_of(NUMBERS) ==
            string::npos))) {
        MathStructure mstruct_nr(nr);
        MathStructure m_exp(m_one);
        ((AliasUnit *)u)->convertToFirstBaseUnit(mstruct_nr, m_exp);
        mstruct_nr.eval(eo);
        while (!accept_obsolete &&
               ((AliasUnit *)u)->firstBaseUnit()->subtype() ==
                   SUBTYPE_ALIAS_UNIT &&
               abs(((AliasUnit *)((AliasUnit *)u)->firstBaseUnit())
                       ->mixWithBase()) > 1) {
          u = ((AliasUnit *)u)->firstBaseUnit();
          if (((AliasUnit *)u)->firstBaseExponent() == 1 &&
              (((AliasUnit *)u)->mixWithBase() != 0 ||
               muc == MIXED_UNITS_CONVERSION_FORCE_ALL ||
               (muc == MIXED_UNITS_CONVERSION_FORCE_INTEGER &&
                ((AliasUnit *)u)->expression().find_first_not_of(NUMBERS) ==
      ...
      if (!b) {
        ...
        for (size_t i = 0; i < units.size(); i++) {
          ...
          if (ui->subtype() == SUBTYPE_ALIAS_UNIT &&
            ...
            AliasUnit *aui = (AliasUnit *)ui;
            int priority_i = aui->mixWithBase();
            if (aui->expression().find("1" DIVISION) == 0 &&
                aui->expression().find_first_not_of(NUMBERS DIVISION, 2) ==
                    string::npos &&
                ((priority_i > 0 && (!best_u || priority_i <= best_priority)) ||
                 (best_priority == 0 && priority_i == 0 &&
                  (muc == MIXED_UNITS_CONVERSION_FORCE_INTEGER ||
                   muc == MIXED_UNITS_CONVERSION_FORCE_ALL)))) {
              MathStructure mstruct_nr(nr);
              ...
              mstruct_nr.eval(eo);
              if (mstruct_nr.isNumber() && m_exp.isOne() &&
                  mstruct_nr.number().isGreaterThan(nr) &&
                  mstruct_nr.number().isGreaterThanOrEqualTo(nr_one) &&
                  (!best_u || non_int ||
                   mstruct_nr.number().isLessThan(best_nr))) {
                non_int = false;
                best_u = ui;
                best_nr = mstruct_nr.number();
                best_priority = priority_i;
      ...
      if (int_nr.isZero()) {
        if (mstruct_new.isAddition())
          mstruct_new[mstruct_new.size() - 1].set(mstruct_term);
        else
          mstruct_new.set(mstruct_term);
      } else {
        if (negated)
          int_nr.negate();
        if (mstruct_new.isAddition())
          mstruct_new[mstruct_new.size() - 1][0].set(int_nr);
        else
          mstruct_new[0].set(int_nr);
...
MathStructure Calculator::convertTimeOut(string str, Unit *from_unit,
                                         Unit *to_unit, int msecs,
                                         const EvaluationOptions &eo) {
  ...
  tmp_tostruct = NULL;
  bool b_parse = false;
  if (!calculate_thread->write(b_parse)) {
    calculate_thread->cancel();
    return mstruct;
  }
  void *x = (void *)&mstruct;
  if (!calculate_thread->write(x)) {
    calculate_thread->cancel();
    return mstruct;
  }
...
MathStructure Calculator::convert(const MathStructure &mstruct,
                                  KnownVariable *to_var,
                                  const EvaluationOptions &eo) {
  if (mstruct.contains(to_var, true) > 0)
    return mstruct;
  size_t n_messages = messages.size();
  if (!to_var->unit().empty() && to_var->isExpression()) {
    ...
    if (b > 0 || (b < 0 && b_var_units)) {
      ...
      if (!CALCULATOR->endTemporaryStopMessages() && cu.countUnits() > 0) {
        AliasUnit au("", "temporary_alias_convert", "", "", "", &cu,
                     to_var->expression());
        bool unc_rel = false;
        if (!to_var->uncertainty(&unc_rel).empty())
          au.setUncertainty(to_var->uncertainty(), unc_rel);
        ...
        if (mstruct_new.contains(&au)) {
          ...
          if (b_var_units || !mstruct_new.containsType(STRUCT_UNIT, true))
            return mstruct_new;
        }
      }
    } else if (b_var_units) {
  ...
  mstruct_new /= to_var->get();
  mstruct_new.eval(eo);
  if ((eo.auto_post_conversion == POST_CONVERSION_OPTIMAL ||
       eo.auto_post_conversion == POST_CONVERSION_OPTIMAL_SI) &&
      mstruct_new.containsType(STRUCT_UNIT, true)) {
...
void fix_to_struct(MathStructure &m) {
  if (m.isPower() && m[0].isUnit()) {
    ...
    if (m[0].prefix() == NULL && m[0].unit()->defaultPrefix() != 0) {
      m[0].setPrefix(
          CALCULATOR->getExactDecimalPrefix(m[0].unit()->defaultPrefix()));
    }
  } else if (m.isUnit()) {
    ...
    for (size_t i = 0; i < m.size();) {
      if (m[i].isUnit()) {
        ...
        if (m[i].prefix() == NULL && m[i].unit()->defaultPrefix() != 0) {
          m[i].setPrefix(
              CALCULATOR->getExactDecimalPrefix(m[i].unit()->defaultPrefix()));
        }
        i++;
        ...
        if (m[i][0].prefix() == NULL && m[i][0].unit()->defaultPrefix() != 0) {
          m[i][0].setPrefix(CALCULATOR->getExactDecimalPrefix(
              m[i][0].unit()->defaultPrefix()));
        }
        i++;
      } else {
        m.delChild(i + 1);
...
MathStructure Calculator::convert(const MathStructure &mstruct, Unit *to_unit,
                                  const EvaluationOptions &eo,
                                  bool always_convert,
                                  bool convert_to_mixed_units) {
  return convert(mstruct, to_unit, eo, always_convert, convert_to_mixed_units,
                 false, NULL);
}
Unit *find_ounce(const MathStructure &m) {
  if (m.isUnit() && m.unit()->referenceName() == "oz")
    return m.unit();
  for (size_t i = 0; i < m.size(); i++) {
...
bool contains_angle_returning_function(const MathStructure &m) {
  if (m.isFunction() &&
      (m.function()->id() == FUNCTION_ID_ATAN ||
       ...
       m.function()->id() == FUNCTION_ID_ASIN ||
       m.function()->id() == FUNCTION_ID_ARG ||
       m.function()->id() == FUNCTION_ID_ATAN2 ||
       m.function()->id() == FUNCTION_ID_RADIANS_TO_DEFAULT_ANGLE_UNIT))
    return true;
  if (m.isFunction() && m.function()->subtype() == SUBTYPE_USER_FUNCTION) {
    UserFunction *f = (UserFunction *)m.function();
    if (f->formula().find("arctan") != string::npos ||
        f->formula().find("arccos") != string::npos ||
        f->formula().find("arcsin") != string::npos ||
        f->formula().find("atan(") != string::npos ||
        f->formula().find("acos(") != string::npos ||
        f->formula().find("asin(") != string::npos)
      return true;
  }
  if (m.isVariable() && m.variable()->isKnown())
    return contains_angle_returning_function(
        ((KnownVariable *)m.variable())->get());
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_angle_returning_function(m[i]))
      return true;
  }
  return false;
}

void contains_angle_ratio_b(const MathStructure &m, bool &num, bool &den,
                            bool in_den) {
  ...
  if (num && den)
    return;
  if (m.isPower()) {
    if (m[1].representsNegative())
      in_den = !in_den;
    contains_angle_ratio_b(m[0], num, den, in_den);
  } else {
    for (size_t i = 0; i < m.size(); i++) {
      if ((i == 0 && m.isInverse()) || (i == 1 && m.isDivision()))
        in_den = !in_den;
...
MathStructure
get_units_for_parsed_expression(const MathStructure *parsed_struct,
                                Unit *to_unit, const EvaluationOptions &eo,
                                const MathStructure *mstruct = NULL) {
  CompositeUnit *cu = NULL;
  if (to_unit->subtype() == SUBTYPE_COMPOSITE_UNIT)
    cu = (CompositeUnit *)to_unit;
  if (cu && cu->countUnits() == 0)
    return m_zero;
  if ((mstruct && mstruct->containsType(STRUCT_UNIT, true)) ||
      (!mstruct && parsed_struct->containsType(STRUCT_UNIT, false, true, true)))
    return m_zero;
  int exp1, exp2;
  bool b_ratio =
      cu && cu->countUnits() == 2 &&
      cu->get(1, &exp1)->baseUnit() == cu->get(2, &exp2)->baseUnit() &&
      exp1 == -exp2;
  bool b_angle = to_unit->baseUnit() == CALCULATOR->getRadUnit() ||
                 (b_ratio && cu->get(1)->baseUnit()->referenceName() == "m" &&
                  (exp1 == 1 || exp2 == 1));
  for (size_t i = 1; !b_angle && cu && i <= cu->countUnits(); i++) {
    if (cu->get(i)->baseUnit() == CALCULATOR->getRadUnit())
      b_angle = true;
  }
  if (!b_ratio || b_angle) {
    if (mstruct) {
      ...
      if (!(ct == 0 ||
            (b_angle && (ct < 0 || (parsed_struct &&
                                    !contains_angle_ratio(*parsed_struct))))))
        return m_zero;
    }
    EvaluationOptions eo2 = eo;
    if (eo.approximation == APPROXIMATION_EXACT)
    ...
    munit.unformat();
    if (b_angle && (b_ratio || (!cu && to_unit->baseExponent() == 1) ||
                    (cu && cu->get(1, &exp1) && exp1 == 1))) {
      munit = default_angle_unit(eo, true);
    } else {
      munit = CALCULATOR->convertToOptimalUnit(munit, eo2, true);
      if (b_angle && !DEFAULT_RADIANS(eo.parse_options.angle_unit))
        munit.replace(CALCULATOR->getRadUnit(), default_angle_unit(eo, true));
      munit.unformat();
    }
    fix_to_struct(munit);
    return munit;
...
bool contains_part_of_unit(const MathStructure &m, Unit *u) {
  if (u->subtype() == SUBTYPE_COMPOSITE_UNIT) {
    for (size_t i = 1; i <= ((CompositeUnit *)u)->countUnits(); i++) {
      if (contains_part_of_unit(m, ((CompositeUnit *)u)->get(i)))
        return true;
    }
    return false;
  }
  if (m.isUnit()) {
    ...
    if (m.unit()->subtype() == SUBTYPE_COMPOSITE_UNIT) {
      for (size_t i = 1; i <= ((CompositeUnit *)m.unit())->countUnits(); i++) {
        if (((CompositeUnit *)m.unit())->get(i) == u)
          return true;
      }
    }
  }
  for (size_t i = 0; i < m.size(); i++) {
    if (contains_part_of_unit(m[i], u))
      return true;
  }
  return false;
}
...
void replace_hz(MathStructure &m) {
  if (m.isUnit() && m.unit()->subtype() == SUBTYPE_ALIAS_UNIT &&
      ((AliasUnit *)m.unit())->firstBaseExponent() == -1 &&
      ((AliasUnit *)m.unit())->expression() == "1") {
...
void test_convert(MathStructure &mstruct_new, Unit *to_unit, long int &n,
                  bool b_pos, EvaluationOptions &eo2) {
  if (n <= 0 ||
      (eo2.auto_post_conversion != POST_CONVERSION_OPTIMAL &&
       eo2.auto_post_conversion != POST_CONVERSION_OPTIMAL_SI) ||
      CALCULATOR->aborted())
    return;
  AutoPostConversion pc = eo2.auto_post_conversion;
  eo2.auto_post_conversion = POST_CONVERSION_NONE;
  MathStructure mtest = CALCULATOR->convertToOptimalUnit(
      mstruct_new, eo2, pc == POST_CONVERSION_OPTIMAL_SI);
  long int ntest = count_unit_powers(mtest);
  if (!contains_part_of_unit(mtest, to_unit) &&
      (pc == POST_CONVERSION_OPTIMAL_SI || ntest < n)) {
    mstruct_new = mtest;
    if (b_pos)
      replace_hz(mstruct_new);
    n = ntest;
  }
  if (b_pos && n > 1) {
...
bool angle_convert(MathStructure &m, Unit *u, const EvaluationOptions &eo) {
  if (m.isFunction() &&
      ((m.function()->getArgumentDefinition(1) &&
        m.function()->getArgumentDefinition(1)->type() ==
            ARGUMENT_TYPE_ANGLE) ||
       m.function()->id() == FUNCTION_ID_CIS) &&
      m.size() >= 1) {
...
MathStructure Calculator::convert(const MathStructure &mstruct, Unit *to_unit,
  ...
  bool b_ratio =
      ...
      cu->get(1, &exp1)->baseUnit() == cu->get(2, &exp2)->baseUnit() &&
      exp1 == -exp2;
  bool b_angle = to_unit->baseUnit() == getRadUnit() ||
                 (b_ratio && cu->get(1)->baseUnit()->referenceName() == "m" &&
                  (exp1 == 1 || exp2 == 1));
  for (size_t i = 1; !b_angle && cu && i <= cu->countUnits(); i++) {
    if (cu->get(i)->baseUnit() == getRadUnit())
      b_angle = true;
  }
  if ((!b_ratio || b_angle) && !mstruct.containsType(STRUCT_UNIT, true)) {
    ...
    if (transform_orig &&
      ...
      if (b_angle && (b_ratio || (!cu && to_unit->baseExponent() == 1) ||
        ...
        if (b_angle && !DEFAULT_RADIANS(eo.parse_options.angle_unit))
          munit.replace(getRadUnit(), default_angle_unit(eo, true));
        munit.unformat();
      }
      fix_to_struct(munit);
      if (!munit.isZero()) {
        ...
        if (munit.isMultiplication() && munit.size() >= 2) {
          if (munit[0].isOne())
            munit.delChild(1, true);
          else if (munit[1].isOne())
            munit.delChild(2, true);
        }
        if (parsed_struct)
          parsed_struct->multiply(munit, true);
        return convert(mstruct_new, to_unit, eo, always_convert,
                       convert_to_mixed_units, false, NULL);
      }
    }
    if (!b_angle)
      return mstruct;
  ...
  MathStructure mstruct_new(mstruct);
  size_t n_messages = messages.size();
  if (to_unit->hasNonlinearRelationTo(to_unit->baseUnit()) &&
      to_unit->baseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT) {
    mstruct_new =
        convert(mstruct, to_unit->baseUnit(), eo, always_convert,
                convert_to_mixed_units, transform_orig, parsed_struct);
    mstruct_new.calculateDivide(
        ((CompositeUnit *)to_unit->baseUnit())
            ->generateMathStructure(false, eo.keep_prefixes),
        eo);
    to_unit->convertFromBaseUnit(mstruct_new);
    mstruct_new.eval(eo);
    mstruct_new.multiply(
        MathStructure(to_unit, eo.keep_prefixes ? decimal_null_prefix : NULL));
    EvaluationOptions eo2 = eo;
    eo2.sync_units = false;
  ...
  // bool b_simple = !cu && (to_unit->subtype() != SUBTYPE_ALIAS_UNIT ||
  // (((AliasUnit*) to_unit)->baseUnit()->subtype() != SUBTYPE_COMPOSITE_UNIT &&
  // ((AliasUnit*) to_unit)->baseExponent() == 1));
  ...
  if (mstruct_new.isAddition()) {
    ...
    if (!b_changed && !mstruct_new.equals(mstruct, true, true))
      b_changed = true;
  }

  if (mstruct_new.size() > 0 && !mstruct_new.isPower() &&
      !mstruct_new.isUnit() && !mstruct_new.isMultiplication()) {
    if (mstruct_new.size() > 0) {
      for (size_t i = 0; i < mstruct_new.size(); i++) {
        if (aborted())
          return mstruct;
        if (!mstruct_new.isFunction() ||
            !mstruct_new.function()->getArgumentDefinition(i + 1) ||
            mstruct_new.function()->getArgumentDefinition(i + 1)->type() ==
                ARGUMENT_TYPE_FREE ||
            (!mstruct_new.function()->getArgumentDefinition(i + 1)->tests() &&
             mstruct_new.function()->getArgumentDefinition(i + 1)->type() !=
                 ARGUMENT_TYPE_ANGLE)) {
          mstruct_new[i] = convert(mstruct_new[i], to_unit, eo, false,
                                   convert_to_mixed_units, false, NULL);
          if (!b_changed && !mstruct_new.equals(mstruct[i], true, true))
            b_changed = true;
        }
      }
      if (b_changed && !b_ratio && !mstruct.isFunction()) {
        ...
        mstruct_new.eval(eo2);
        if (b_angle)
          angle_convert(mstruct_new, to_unit, eo);
        cleanMessages(mstruct, n_messages + 1);
      }
      return mstruct_new;
    }
    ...
    if (transform_orig) {
        ...
        if (bu->subtype() == SUBTYPE_COMPOSITE_UNIT) {
          ...
          if (u1_type == 0) {
            for (size_t i = 1; i <= cu2->countUnits(); i++) {
              if (to_unit == cu2 && cu2->get(i)->referenceName() == "oz" &&
                  !to_unit->isRegistered()) {
                u1_type = 6;
                break;
              }
            }
            for (size_t i = 1; i <= cu2->countUnits(); i++) {
              ...
              exp *= bu->baseExponent();
              bu = bu->baseUnit();
              if ((u1_type == 0 || u1_type == 6) && exp % 3 == 0 &&
                  bu->referenceName() == "m") {
                if (u1_type == 6) {
                  u1_type = 0;
                  break;
      ...
      if (u1_type == 5) {
        ...
        } else if (mstruct_new.isMultiplication() && mstruct_new.size() >= 2 &&
                   mstruct_new.last().isUnit_exp() &&
                   !mstruct_new[mstruct_new.size() - 2].isUnit_exp()) {
          mstruct_u = &mstruct_new.last();
        }
        if (mstruct_u && (!mstruct_u->isPower() ||
                          (u1_type == 6 && (*mstruct_u)[1].isInteger()) ||
                          (u1_type == 1 && (*mstruct_u)[1].isMinusOne()))) {
          ...
          if (mstruct_u->isPower()) {
            if (u1_type == 6)
              exp *= (*mstruct_u)[1].number().intValue();
            else
              exp = -exp;
          }
          bu = bu->baseUnit();
          ...
          if (exp == 1) {
            if (bu->subtype() == SUBTYPE_COMPOSITE_UNIT) {
              ...
              } else if (u1_type != 1 && cu2->countUnits() == 2 &&
                         cu2->referenceName() == "N_m") {
                u2_type = 1;
              }
          ...
          if (u1_type == 1) {
            ...
            if (u1_type == 2)
              v = getActiveVariable("J_to_K");
            else if (u1_type == 3)
              v = getActiveVariable("J_to_m");
            else if (u1_type == 4)
              v = getActiveVariable("J_to_Hz");
          ...
          if (v) {
            mstruct_new *= v;
            if (parsed_struct)
              parsed_struct->multiply(v, true);
          }
        }
      }
    }
    if (mstruct_new.convert(to_unit, true, NULL, false, eo2,
                            eo.keep_prefixes ? decimal_null_prefix : NULL)) {
      b = true;
    } else if (b_ratio) {
      int exp;
      if (cu->get(1, &exp)->baseUnit()->referenceName() == "m") {
        ...
        if (u) {
          ...
          if (mstruct_new.contains(u, false, false, false))
            mstruct_new.replace(u, m_one, false, false);
        }
      }
      b = true;
    } else if (b_angle && angle_convert(mstruct_new, to_unit, eo)) {
      return mstruct_new;
    } else if (to_unit->baseUnit() == getRadUnit() &&
               mstruct_new.contains(to_unit, true) < 1) {
      mstruct_new.multiply(getRadUnit(), true);
      if (to_unit->baseExponent() != 1)
        mstruct_new.last().raise(to_unit->baseExponent());
      b = mstruct_new.convert(to_unit, true, NULL, false, eo2,
                              eo.keep_prefixes ? decimal_null_prefix : NULL) ||
          always_convert;
      ...
      if (to_unit->subtype() == SUBTYPE_ALIAS_UNIT &&
          ((AliasUnit *)to_unit)->baseUnit()->subtype() ==
              SUBTYPE_COMPOSITE_UNIT) {
      ...
      if (cu2) {
        ...
        case STRUCT_MULTIPLICATION: {
          ...
          for (size_t i = 1; i <= mstruct_new.countChildren(); i++) {
            ...
            if (mstruct_new.getChild(i)->isUnit() &&
                cu2->containsRelativeTo(mstruct_new.getChild(i)->unit())) {
              b = true;
            }
            if (mstruct_new.getChild(i)->isPower() &&
                mstruct_new.getChild(i)->base()->isUnit() &&
                cu2->containsRelativeTo(
                    mstruct_new.getChild(i)->base()->unit())) {
              b = true;
            }
          }
          break;
        ...
        case STRUCT_POWER: {
          if (mstruct_new.base()->isUnit() &&
              cu2->containsRelativeTo(mstruct_new.base()->unit())) {
            b = true;
          }
          break;
        }
      ...
      if (to_unit != priv->u_celsius && to_unit != priv->u_fahrenheit) {
        ...
        if (cu || to_unit->baseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT) {
          ...
          if (cu2)
            ...
            cu2 = (CompositeUnit *)to_unit->baseUnit();
          int exp2 = 1;
          for (size_t i = 1; i <= cu2->countUnits(); i++) {
            cu2->get(i, &exp2);
            if (exp2 < 0)
        ...
        if (cu) {
          MathStructure *mstruct_cu = new MathStructure(
              cu->generateMathStructure(false, eo.keep_prefixes));
          Prefix *p = NULL;
          size_t i = 1;
          ...
          if (exp != 1) {
            if (mstruct_cu->isPower())
              (*mstruct_cu)[1].number() *= exp;
            else
              mstruct_cu->raise(exp);
          }
          mstruct_new.multiply_nocopy(mstruct_cu);
        } else {
          mstruct_new.multiply_nocopy(new MathStructure(
              to_unit, eo.keep_prefixes ? decimal_null_prefix : NULL));
          if (exp != 1)
            mstruct_new.last().raise(exp);
        }
...
Unit *Calculator::findMatchingUnit(const MathStructure &mstruct) {
  switch (mstruct.type()) {
  case STRUCT_POWER: {
    if (mstruct.base()->isUnit() && mstruct.exponent()->isNumber() &&
        mstruct.exponent()->number().isInteger() &&
        mstruct.exponent()->number() < 10 &&
        mstruct.exponent()->number() > -10) {
      ...
      for (size_t i = 0; i < units.size(); i++) {
        Unit *u = units[i];
        if (u->subtype() == SUBTYPE_ALIAS_UNIT && u->baseUnit() == u_base &&
            ((AliasUnit *)u)->baseExponent() == exp) {
          return u;
        } else if (u->subtype() == SUBTYPE_COMPOSITE_UNIT &&
                   ((CompositeUnit *)u)->countUnits() == 1 &&
                   ((CompositeUnit *)u)->get(1, &exp2)->baseUnit() == u_base &&
                   exp == ((CompositeUnit *)u)->get(1)->baseExponent(exp2)) {
          return u;
        }
      }
    ...
    for (size_t i = 1; i <= mstruct.countChildren(); i++) {
      if (mstruct.getChild(i)->isUnit()) {
        cu->add(mstruct.getChild(i)->unit()->baseUnit(),
                mstruct.getChild(i)->unit()->baseExponent());
      } else if (mstruct.getChild(i)->isPower() &&
                 mstruct.getChild(i)->base()->isUnit() &&
                 mstruct.getChild(i)->exponent()->isNumber() &&
                 mstruct.getChild(i)->exponent()->number().isInteger()) {
        cu->add(mstruct.getChild(i)->base()->unit()->baseUnit(),
                mstruct.getChild(i)->exponent()->number().intValue());
      }
    }
    if (cu->countUnits() == 1) {
      int exp = 1, exp2 = 1;
  ...
  default: {
    for (size_t i = 0; i < mstruct.size(); i++) {
      if (aborted())
        return NULL;
      if (!mstruct.isFunction() ||
          !mstruct.function()->getArgumentDefinition(i + 1) ||
          mstruct.function()->getArgumentDefinition(i + 1)->type() !=
              ARGUMENT_TYPE_ANGLE) {
...
Unit *Calculator::getBestUnit(Unit *u, bool allow_only_div,
                              bool convert_to_local_currency) {
  return getOptimalUnit(u, allow_only_div, convert_to_local_currency);
}
Unit *Calculator::getOptimalUnit(Unit *u, bool allow_only_div,
                                 bool convert_to_local_currency) {
  switch (u->subtype()) {
  case SUBTYPE_BASE_UNIT: {
    if (convert_to_local_currency && u->isCurrency()) {
      ...
      if (u_local_currency)
        return u_local_currency;
    }
    return u;
  }
  case SUBTYPE_ALIAS_UNIT: {
    AliasUnit *au = (AliasUnit *)u;
    if (au->baseExponent() == 1 &&
        au->baseUnit()->subtype() == SUBTYPE_BASE_UNIT) {
      if (au->isCurrency()) {
        if (!convert_to_local_currency)
          return u;
        Unit *u_local_currency = getLocalCurrency();
        if (u_local_currency)
          return u_local_currency;
      ...
      return (Unit *)au->baseUnit();
               ...
               (au->firstBaseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT ||
                au->firstBaseExponent() != 1)) {
      return u;
    } else {
  ...
  case SUBTYPE_COMPOSITE_UNIT: {
    ...
    if (cu->countUnits() > 1) {
      for (size_t i = 0; i < units.size(); i++) {
        if (units[i]->subtype() == SUBTYPE_COMPOSITE_UNIT &&
            !units[i]->isHidden() && units[i]->isSIUnit()) {
          CompositeUnit *cu2 = (CompositeUnit *)units[i];
          if (cu == cu2 && !cu2->isHidden()) {
            points = max_points - 1;
          } else if (!cu2->isHidden() && cu2->isSIUnit() &&
                     cu2->countUnits() == cu->countUnits()) {
            bool b_match = true;
            for (size_t i2 = 1; i2 <= cu->countUnits(); i2++) {
              int exp2;
        ...
        if (units[i]->subtype() == SUBTYPE_COMPOSITE_UNIT &&
          ...
          if (cu2->countUnits() <= cu->countUnits()) {
            for (size_t i2 = 1; i2 <= cu2->countUnits(); i2++) {
              if (cu2->get(i2)->baseUnit()->subtype() ==
                      SUBTYPE_COMPOSITE_UNIT ||
                  cu2->get(i2)->baseExponent() != 1) {
                MathStructure *cu_mstruct = NULL;
                unordered_map<Unit *, MathStructure *>::iterator it =
                    priv->composite_unit_base.find(cu2);
                if (!cu2->hasChanged() &&
                    it != priv->composite_unit_base.end()) {
                  cu_mstruct = it->second;
                } else {
                  if (it != priv->composite_unit_base.end())
                    it->second->unref();
                ...
                if (cu_mstruct->size() == cu->countUnits()) {
                  ...
                  for (size_t i3 = 1; i3 <= cu->countUnits(); i3++) {
                    ...
                    if ((!b_exp &&
                         (!i3_m->isUnit() ||
                          cu->get(i3, &exp) != i3_m->unit() || exp != 1)) ||
                        (b_exp &&
                         (cu->get(i3, &exp) != i3_m->getChild(1)->unit() ||
                          exp == 1 || !i3_m->getChild(2)->equals(exp)))) {
                      b_match = false;
                      break;
                    }
                  }
      ...
      int exp2 = 1;
      // Fix units for G
      if (cu->countUnits() == 3 && cu->get(1, &exp2)->referenceName() == "m" &&
          exp2 == 3 && cu->get(2, &exp2)->referenceName() == "g" &&
          exp2 == -1 && cu->get(3, &exp2)->referenceName() == "s" &&
          exp2 == -2) {
        points = max_points - 1;
      }
    }
    ...
    for (size_t i = 0; i < units.size(); i++) {
      u2 = units[i];
      if (u2->subtype() == SUBTYPE_BASE_UNIT &&
          (points == 0 || (points == 1 && minus))) {
        for (size_t i2 = 1; i2 <= cu->countUnits(); i2++) {
          if (cu->get(i2, &exp)->baseUnit() == u2 &&
              !cu->get(i2)->hasNonlinearRelationTo(u2)) {
            points = 1;
            best_u = u2;
        ...
        if ((b_exp != 1 || bu->subtype() == SUBTYPE_COMPOSITE_UNIT) &&
          ...
          } else if (au->firstBaseExponent() != 1 ||
                     au->firstBaseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT) {
            MathStructure *cu_mstruct = NULL;
            unordered_map<Unit *, MathStructure *>::iterator it =
                priv->composite_unit_base.find(bu);
            if (!bu->hasChanged() && it != priv->composite_unit_base.end()) {
              cu_mstruct = it->second;
            } else {
              if (it != priv->composite_unit_base.end())
                it->second->unref();
              cu_mstruct = new MathStructure(
                  ((CompositeUnit *)bu)->generateMathStructure());
              if (b_exp != 1) {
                if (cu_mstruct->isMultiplication()) {
                  for (size_t i2 = 0; i2 < cu_mstruct->size(); i2++) {
                    if ((*cu_mstruct)[i2].isPower())
                      (*cu_mstruct)[i2][1].number() *= b_exp;
                    else
                      (*cu_mstruct)[i2].raise(b_exp);
                  }
            ...
            if (cu_mstruct->isMultiplication()) {
              for (size_t i2 = 1; i2 <= cu_mstruct->countChildren(); i2++) {
                ...
                if (cu_mstruct->getChild(i2)->isUnit()) {
                  ...
                  b_exp = 1;
                } else if (cu_mstruct->getChild(i2)->isPower() &&
                           cu_mstruct->getChild(i2)->base()->isUnit() &&
                           cu_mstruct->getChild(i2)->exponent()->isNumber() &&
                           cu_mstruct->getChild(i2)
                               ->exponent()
                               ->number()
                               .isInteger()) {
                  bu = cu_mstruct->getChild(i2)->base()->unit();
                  b_exp =
                      cu_mstruct->getChild(i2)->exponent()->number().intValue();
                }
                if (bu) {
                  bool b = false;
                  for (size_t i3 = 1; i3 <= cu->countUnits(); i3++) {
                    if (cu->get(i3, &exp) == bu) {
                      ...
                      if (exp >= b_exp) {
                        if (m)
                          new_points_m += exp - (exp - b_exp);
                        else
                          new_points += exp - (exp - b_exp);
                      } else {
                        if (m)
                          new_points_m += exp - (b_exp - exp);
                        else
              ...
              if (new_points > points && new_points >= new_points_m) {
                ...
                points = new_points;
                best_u = au;
              } else if (new_points_m > points ||
                         (new_points_m == points && minus)) {
                minus = true;
                points = new_points_m;
                best_u = au;
              }
    ...
    if (points > 1 && points < max_points - 1) {
      CompositeUnit *cu_new =
          new CompositeUnit("", "temporary_composite_convert");
      ...
      CompositeUnit *cu2 =
          new CompositeUnit("", "temporary_composite_convert_to_optimal_unit");
      bool b = false;
      for (size_t i = 1; i <= cu_mstruct.countChildren(); i++) {
        if (cu_mstruct.getChild(i)->isUnit()) {
          b = true;
          cu2->add(cu_mstruct.getChild(i)->unit());
        } else if (cu_mstruct.getChild(i)->isPower() &&
                   cu_mstruct.getChild(i)->base()->isUnit() &&
                   cu_mstruct.getChild(i)->exponent()->isNumber() &&
                   cu_mstruct.getChild(i)->exponent()->number().isInteger()) {
          if (cu_mstruct.getChild(i)->exponent()->number().isGreaterThan(10) ||
              cu_mstruct.getChild(i)->exponent()->number().isLessThan(-10)) {
            if (aborted() ||
                cu_mstruct.getChild(i)->exponent()->number().isGreaterThan(
                    1000) ||
                cu_mstruct.getChild(i)->exponent()->number().isLessThan(
                    -1000)) {
              b = false;
              break;
            }
          ...
          b = true;
          cu2->add(cu_mstruct.getChild(i)->base()->unit(),
                   cu_mstruct.getChild(i)->exponent()->number().intValue());
        }
      }
      if (b) {
...
#define CONVERT_MULTISUB(m)                                                    \
  ...
  for (size_t i = 1; i <= m.countChildren(); i++) {                            \
    if (m.getChild(i)->size() > 0 && !aborted() &&                             \
        m.getChild(i)->containsType(STRUCT_UNIT, true) &&                      \
        (!m.getChild(i)->isPower() || !m.getChild(i)->base()->isUnit() ||      \
         !m.getChild(i)->exponent()->isNumber() ||                             \
         !m.getChild(i)->exponent()->number().isRational())) {                 \
      m[i - 1] = convertToOptimalUnit(m[i - 1], eo, convert_to_si_units);      \
      if (!m[i - 1].equals(mstruct[i - 1], true, true))                        \
        child_updated = true;                                                  \
    }                                                                          \
  }                                                                            \
  if (child_updated)                                                           \
...
#define CONVERT_MULTISUB_N(m)                                                  \
  ...
  for (size_t i = 1; i <= m.countChildren(); i++) {                            \
    if (m.getChild(i)->size() > 0 && !aborted() &&                             \
      ...
      m[i - 1] = convertToOptimalUnit(m[i - 1], eo, convert_to_si_units);      \
      m.childUpdated(i);                                                       \
      if (!m[i - 1].equals(m_i_old, true, true))                               \
        child_updated = true;                                                  \
    }                                                                          \
  }                                                                            \
  if (child_updated)                                                           \
...
bool replace_prefixes(MathStructure &m, const EvaluationOptions &eo) {
  if (m.isUnit() && m.prefix()) {
    if (m.prefix() == CALCULATOR->getDecimalNullPrefix() ||
        m.prefix() == CALCULATOR->getBinaryNullPrefix()) {
      m.unformat(eo);
      return false;
    } else {
      m.unformat(eo);
  ...
  bool b = false;
  for (size_t i = 0; i < m.size(); i++) {
    if (replace_prefixes(m[i], eo))
      b = true;
  }
  if (b && (m.isMultiplication() || m.isPower()))
...
MathStructure Calculator::convertToBestUnit(const MathStructure &mstruct,
                                            const EvaluationOptions &eo,
                                            bool convert_to_si_units) {
  return convertToOptimalUnit(mstruct, eo, convert_to_si_units);
}
MathStructure Calculator::convertToOptimalUnit(const MathStructure &mstruct,
                                               const EvaluationOptions &eo,
                                               bool convert_to_si_units) {
  EvaluationOptions eo2 = eo;
  // eo2.calculate_functions = false;
  ...
  case STRUCT_POWER: {
    if (mstruct.base()->isUnit() && mstruct.exponent()->isNumber() &&
        mstruct.exponent()->number().isRational() &&
        !mstruct.exponent()->number().isZero()) {
      MathStructure mstruct_new(mstruct);
      int old_points = 0;
      bool overflow = false;
      if (mstruct_new.exponent()->isInteger())
        old_points = mstruct_new.exponent()->number().intValue(&overflow);
      else
        old_points =
            mstruct_new.exponent()->number().numerator().intValue(&overflow) +
            mstruct_new.exponent()->number().denominator().intValue() *
                (mstruct_new.exponent()->number().isNegative() ? -1 : 1);
      if (overflow)
        return mstruct_new;
      ...
      if (mstruct_new.base()->unit()->baseUnit()->subtype() ==
          SUBTYPE_COMPOSITE_UNIT) {
        ...
        mstruct_new =
            convertToOptimalUnit(mstruct_new, eo, convert_to_si_units);
        if (mstruct_new.equals(mstruct, true, true))
          return mstruct_new;
      } else {
        CompositeUnit *cu = new CompositeUnit(
            "", "temporary_composite_convert_to_optimal_unit");
        cu->add(mstruct_new.base()->unit(),
                mstruct_new.exponent()->number().numerator().intValue());
      ...
      if (mstruct_new.isMultiplication()) {
        for (size_t i = 1; i <= mstruct_new.countChildren(); i++) {
          if (mstruct_new.getChild(i)->isUnit()) {
            if (new_is_si_units && !mstruct_new.getChild(i)->unit()->isSIUnit())
              new_is_si_units = false;
            is_currency = mstruct_new.getChild(i)->unit()->isCurrency();
            new_points++;
            new_minus = false;
          } else if (mstruct_new.getChild(i)->isPower() &&
                     mstruct_new.getChild(i)->base()->isUnit() &&
                     mstruct_new.getChild(i)->exponent()->isNumber() &&
                     mstruct_new.getChild(i)
                         ->exponent()
                         ->number()
                         .isRational()) {
            int points = 0;
            if (mstruct_new.getChild(i)->exponent()->isInteger())
              points = mstruct_new.getChild(i)->exponent()->number().intValue();
            else
              points = mstruct_new.getChild(i)
                           ->exponent()
                           ->number()
                           .numerator()
                           .intValue() +
                       mstruct_new.getChild(i)
                           ...
                           (mstruct_new.getChild(i)
                                ...
                                : 1);
            if (new_is_si_units &&
                !mstruct_new.getChild(i)->base()->unit()->isSIUnit())
              new_is_si_units = false;
      ...
      } else if (mstruct_new.isPower() && mstruct_new.base()->isUnit() &&
                 mstruct_new.exponent()->isNumber() &&
                 mstruct_new.exponent()->number().isRational()) {
        int points = 0;
        if (mstruct_new.exponent()->isInteger())
          points = mstruct_new.exponent()->number().intValue();
        else
          points = mstruct_new.exponent()->number().numerator().intValue() +
                   mstruct_new.exponent()->number().denominator().intValue() *
                       (mstruct_new.exponent()->number().isNegative() ? -1 : 1);
        if (new_is_si_units && !mstruct_new.base()->unit()->isSIUnit())
          new_is_si_units = false;
      ...
      if ((new_points > old_points &&
           (!convert_to_si_units || is_si_units || !new_is_si_units)) ||
          (new_points == old_points && (new_minus || !old_minus) &&
           (!is_currency || !eo.local_currency_conversion) &&
           (!convert_to_si_units || !new_is_si_units)))
        return mstruct;
      return mstruct_new;
    }
  }
  ...
  case STRUCT_ADDITION: {
    if ((mstruct.isFunction() &&
        ...
        !mstruct.containsType(STRUCT_UNIT, true))
      return mstruct;
    MathStructure mstruct_new(mstruct);
    bool b = false;
    if (mstruct.isFunction() &&
      ...
      for (size_t i = 0; i < mstruct_new.size(); i++) {
        if (aborted())
          return mstruct;
        mstruct_new[i] =
            convertToOptimalUnit(mstruct_new[i], eo3, convert_to_si_units);
        if (!b && !mstruct_new[i].equals(mstruct[i], true, true))
          b = true;
      }
    } else {
      for (size_t i = 0; i < mstruct_new.size(); i++) {
        ...
        if (!mstruct_new.isFunction() ||
            ...
            mstruct_new.function()->getArgumentDefinition(i + 1)->type() !=
                ARGUMENT_TYPE_ANGLE) {
          mstruct_new[i] =
              convertToOptimalUnit(mstruct_new[i], eo, convert_to_si_units);
          if (!b && !mstruct_new[i].equals(mstruct[i], true, true))
            b = true;
        }
      }
        ...
        for (size_t i = 0; i < 3 && b_torque; i++) {
          if (mstruct_new[i].isMultiplication()) {
            ...
            for (size_t i2 = 0; i2 < mstruct_new[i].size(); i2++) {
              if (!b_torque && mstruct_new[i][i2].isUnit() &&
                  ((u_joule && mstruct_new[i][i2].unit() == u_joule) ||
                   (!u_joule &&
                    mstruct_new[i][i2].unit()->referenceName() == "J"))) {
                b_torque = true;
                if (!u_joule)
                  u_joule = mstruct_new[i][i2].unit();
              } else if (mstruct_new[i][i2].containsType(STRUCT_UNIT, true)) {
                b_torque = false;
                break;
              }
            }
          } else if (mstruct_new[i].isUnit() &&
                     ((u_joule && mstruct_new[i].unit() == u_joule) ||
                      (!u_joule &&
                       mstruct_new[i].unit()->referenceName() == "J"))) {
            if (!u_joule)
              u_joule = mstruct_new[i].unit();
          } else {
            b_torque = false;
          }
        }
  ...
  case STRUCT_UNIT: {
    if ((!mstruct.unit()->isCurrency() || !eo.local_currency_conversion) &&
        (!convert_to_si_units || mstruct.unit()->isSIUnit()))
      return mstruct;
    Unit *u =
        getOptimalUnit(mstruct.unit(), false, eo.local_currency_conversion);
    if (u != mstruct.unit()) {
      if ((u->isSIUnit() ||
           (u->isCurrency() && eo.local_currency_conversion)) &&
          (eo.approximation != APPROXIMATION_EXACT ||
           !mstruct.unit()->hasApproximateRelationTo(u, true))) {
  ...
  case STRUCT_MULTIPLICATION: {
    ...
    for (size_t i = 1; i <= mstruct_old.countChildren(); i++) {
      ...
      if (mstruct_old.getChild(i)->isUnit()) {
        if (is_si_units && !mstruct_old.getChild(i)->unit()->isSIUnit())
          is_si_units = false;
        is_currency = mstruct_old.getChild(i)->unit()->isCurrency();
        old_points++;
        old_minus = false;
      } else if (mstruct_old.getChild(i)->isPower() &&
                 mstruct_old.getChild(i)->base()->isUnit() &&
                 mstruct_old.getChild(i)->exponent()->isNumber() &&
                 mstruct_old.getChild(i)->exponent()->number().isRational()) {
        int points = 0;
        if (mstruct_old.getChild(i)->exponent()->number().isInteger())
          points = mstruct_old.getChild(i)->exponent()->number().intValue();
        else
          points =
              mstruct_old.getChild(i)
                  ->exponent()
                  ->number()
                  .numerator()
                  .intValue() +
              mstruct_old.getChild(i)
                      ...
                      .denominator()
                      .intValue() *
                  (mstruct_old.getChild(i)->exponent()->number().isNegative()
                       ? -1
                       : 1);
        ;
        if (is_si_units && !mstruct_old.getChild(i)->base()->unit()->isSIUnit())
          is_si_units = false;
    ...
    if ((!is_currency || !eo.local_currency_conversion) &&
        (!convert_to_si_units || is_si_units) && old_points <= 1 &&
        !old_minus) {
    ...
    if (mstruct_new.type() != STRUCT_MULTIPLICATION) {
      if (!mstruct_new.containsInterval(true, true, false, 1, true) &&
          !aborted())
        mstruct_new =
            convertToOptimalUnit(mstruct_new, eo, convert_to_si_units);
    } else {
      ...
      for (size_t i = 1; i <= mstruct_new.countChildren(); i++) {
        ...
        if (mstruct_new.getChild(i)->isUnit()) {
          ...
          cu->add(mstruct_new.getChild(i)->unit());
                   ...
                   mstruct_new.getChild(i)->base()->isUnit() &&
                   mstruct_new.getChild(i)->exponent()->isNumber() &&
                   mstruct_new.getChild(i)->exponent()->number().isInteger()) {
          b = true;
          cu->add(mstruct_new.getChild(i)->base()->unit(),
                  mstruct_new.getChild(i)->exponent()->number().intValue());
        }
      }
    ...
    if (((eo.approximation == APPROXIMATION_EXACT &&
          !mstruct_old.isApproximate()) &&
         (mstruct_new.isApproximate() ||
          (mstruct_old.containsInterval(true, true, false, 0, true) <= 0 &&
           mstruct_new.containsInterval(true, true, false, 0, true) > 0))) ||
        mstruct_new.equals(mstruct_old, true, true)) {
      CONVERT_MULTISUB(mstruct_old)
      return mstruct_old;
    }
    int new_points = 0;
      ...
      for (size_t i = 1; i <= mstruct_new.countChildren(); i++) {
        ...
        if (mstruct_new.getChild(i)->isUnit()) {
          ...
          new_minus = false;
                   ...
                   mstruct_new.getChild(i)->base()->isUnit() &&
                   mstruct_new.getChild(i)->exponent()->isNumber() &&
                   mstruct_new.getChild(i)->exponent()->number().isRational()) {
          int points = 0;
          if (mstruct_new.getChild(i)->exponent()->number().isInteger())
            points = mstruct_new.getChild(i)->exponent()->number().intValue();
          else
            points =
                mstruct_new.getChild(i)
                ...
                mstruct_new.getChild(i)
                        ...
                        .denominator()
                        .intValue() *
                    (mstruct_new.getChild(i)->exponent()->number().isNegative()
                         ? -1
                         : 1);
          if (new_is_si_units &&
              !mstruct_new.getChild(i)->base()->unit()->isSIUnit())
            new_is_si_units = false;
          new_is_currency =
              mstruct_new.getChild(i)->base()->unit()->isCurrency();
          if (points < 0) {
            new_points -= points;
          } else {
            new_points += points;
            new_minus = false;
               ...
               mstruct_new.exponent()->number().isRational()) {
      int points = 0;
      if (mstruct_new.exponent()->number().isInteger())
        points = mstruct_new.exponent()->number().intValue();
      else
        points = mstruct_new.exponent()->number().numerator().intValue() +
                 mstruct_new.exponent()->number().denominator().intValue() *
    ...
    if (new_points == 0 ||
        (new_points > old_points &&
         (!convert_to_si_units || is_si_units || !new_is_si_units)) ||
        (new_points == old_points && (new_minus || !old_minus) &&
         (!new_is_currency || !eo.local_currency_conversion) &&
         (!convert_to_si_units || !new_is_si_units))) {
      CONVERT_MULTISUB(mstruct_old)
      return mstruct_old;
    }
...
MathStructure Calculator::convertToCompositeUnit(const MathStructure &mstruct,
                                                 CompositeUnit *cu,
                                                 const EvaluationOptions &eo,
                                                 bool always_convert) {
  return convert(mstruct, cu, eo, always_convert);
}
MathStructure Calculator::convert(const MathStructure &mstruct_to_convert,
                                  string str2, const EvaluationOptions &eo,
...
void set_null_prefixes(MathStructure &m) {
  if (!m.isUnit() || !m.prefix())
    m.setPrefix(CALCULATOR->decimal_null_prefix);
  for (size_t i = 0; i < m.size(); i++) {
...
MathStructure Calculator::convert(const MathStructure &mstruct_to_convert,
                                  string str2, const EvaluationOptions &eo,
                                  MathStructure *to_struct, bool transform_orig,
                                  MathStructure *parsed_struct) {
  if (to_struct)
    to_struct->setUndefined();
  remove_blank_ends(str2);
  if (str2.empty())
    return mstruct_to_convert;
  current_stage = MESSAGE_STAGE_CONVERSION;
  int do_prefix = 0;
  if (str2.length() > 1 && str2[1] == '?' &&
      (str2[0] == 'b' || str2[0] == 'a' || str2[0] == 'd')) {
    do_prefix = 2;
  } else if (str2[0] == '?') {
    do_prefix = 1;
  }
  ...
  if (str2.find(SIGN_MINUS) == 0)
    str2.replace(0, strlen(SIGN_MINUS), "-");
  if (str2[0] == '-')
    eo2.mixed_units_conversion = MIXED_UNITS_CONVERSION_NONE;
  else if (str2[0] == '+')
    eo2.mixed_units_conversion = MIXED_UNITS_CONVERSION_FORCE_INTEGER;
  else if (eo2.mixed_units_conversion != MIXED_UNITS_CONVERSION_NONE)
    eo2.mixed_units_conversion = MIXED_UNITS_CONVERSION_DOWNWARDS_KEEP;
  if (do_prefix || str2[0] == '0' || str2[0] == '+' || str2[0] == '-') {
    str2 = str2.substr(do_prefix > 1 ? 2 : 1,
                       str2.length() - (do_prefix > 1 ? 2 : 1));
    remove_blank_ends(str2);
    if (str2.empty()) {
  ...
  if (!u)
    u = getCompositeUnit(str2);
  Variable *v = NULL;
  if (!u)
    v = getActiveVariable(str2);
  if (!u && !v) {
    for (size_t i = 0; i < signs.size(); i++) {
      if (str2 == signs[i]) {
        u = getUnit(real_signs[i]);
        if (!u)
          v = getActiveVariable(real_signs[i]);
        break;
      }
    }
  }
  ...
  if (v) {
    ...
    if (!u) {
      if (v->referenceName() == "bohr_radius")
        u = CALCULATOR->getActiveUnit("bohr_unit");
      else if (v->referenceName() == "elementary_charge")
        u = CALCULATOR->getActiveUnit("e_unit");
      else if (v->referenceName() == "electron_mass")
        u = CALCULATOR->getActiveUnit("electron_unit");
    }
    if (u)
      v = NULL;
  }
  ...
  if (u) {
    ...
    beginTemporaryStopMessages();
    CompositeUnit cu(
        "", eo.parse_options.limit_implicit_multiplication ? "01" : "00", "",
        str2);
    ...
    if (cu.countUnits() == 2 && cu.get(1)->referenceName() == "g" &&
        cu.get(2)->referenceName() == "m" && str2.substr(0, 2) == "kg") {
      int exp = 1;
      Prefix *p = NULL;
      if (cu.get(1, &exp, &p) && exp == 1 && p && p->value() == 1000 &&
          cu.get(2, &exp, &p) && exp == -2) {
        Unit *u = getUnit("pond");
        if (u) {
          MathStructure mtest(convertToBaseUnits(mstruct_to_convert, eo));
          mtest.sort();
          if (mtest.isMultiplication() && mtest.size() >= 3 &&
              mtest[mtest.size() - 3].isUnit() &&
              mtest[mtest.size() - 3].unit()->referenceName() == "g" &&
              mtest[mtest.size() - 2].isPower() &&
              mtest[mtest.size() - 2][1].isMinusOne() &&
              mtest[mtest.size() - 2][0].isUnit() &&
              mtest[mtest.size() - 2][0].unit()->referenceName() == "m" &&
              mtest[mtest.size() - 1].isPower() &&
              mtest[mtest.size() - 1][1] == Number(-2, 1) &&
              mtest[mtest.size() - 1][0].isUnit() &&
              mtest[mtest.size() - 1][0].unit()->referenceName() == "s") {
    ...
    if (to_struct) {
      if (!munits.isUndefined())
        to_struct->set(munits);
      else
        to_struct->set(cu.generateMathStructure(true));
    }
    if (cu.countUnits() > 0) {
      if (cu.countUnits() == 1) {
        Prefix *p = NULL;
  ...
  if (!b)
    return mstruct_to_convert;
  if (!v && eo2.mixed_units_conversion != MIXED_UNITS_CONVERSION_NONE)
    mstruct.set(convertToMixedUnits(mstruct, eo2));
  current_stage = MESSAGE_STAGE_UNSET;
  return mstruct;
}
