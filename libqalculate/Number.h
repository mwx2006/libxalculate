 ...
 * Has arbitrary precision (uses Calculator::precision()) and infinitely large
 * rational numbers. Implimented using GNU MP and MPFR.
 */
class Number {
...
protected:
  ...
  bool testFloatResult(bool allow_infinite_result = true, int error_level = 1,
...
public:
  ...
  void set(long int numerator, long int denominator = 1, long int exp_10 = 0,
           bool keep_precision = false, bool keep_imag = false);
  ...
  bool setInterval(const Number &nr_lower, const Number &nr_upper,
                   bool keep_precision = false);

  void setInternal(const mpz_t &mpz_value, bool keep_precision = false,
                   bool keep_imag = false);
  void setInternal(mpz_srcptr mpz_value, bool keep_precision = false,
                   bool keep_imag = false);
  void setInternal(const mpq_t &mpq_value, bool keep_precision = false,
                   bool keep_imag = false);
  void setInternal(const mpz_t &mpz_num, const mpz_t &mpz_den,
                   bool keep_precision = false, bool keep_imag = false);
  void setInternal(const mpfr_t &mpfr_value, bool merge_precision = false,
                   bool keep_imag = false);

  void setImaginaryPart(const Number &o);
  void setImaginaryPart(long int numerator, long int denominator = 1,
                        long int exp_10 = 0);
  void set(const Number &o, bool merge_precision = false,
           bool keep_imag = false);
  ...
  bool getCentralInteger(Number &nr_int, bool *b_multiple = NULL,
                         std::vector<Number> *v = NULL) const;
  ...
  /**
   * Converts a number to an integer. If the number does not represent an
   * integer it will rounded using round().
   *
   * @param[out] overflow If overflow is non-null it will be set to true if the
   * number was to large to fit the return type.
   ...
   *
   * @return Precision of the number or -1 if the number is exact or the
   * precision has not been set.
   */
  ...
  /** Returns the real part of the number if it is complex, or a copy if it is
   * real.
   *
   * @return true if the real part of a complex number.
   */
  Number realPart() const;
  /** Returns the imaginary part as real number of the number if it is complex,
   * or zero if it is real.
   *
  ...
  bool equals(const Number &o, bool allow_interval = false,
              bool allow_infinite = false) const;
  ...
  ComparisonResult compareAbsolute(const Number &o,
                                   bool ignore_imag = false) const;
  ComparisonResult compare(long int i) const;
  ComparisonResult
  compareApproximately(const Number &o,
                       int prec = EQUALS_PRECISION_LOWEST) const;
   ...
   * @param try_exact If an exact solution should be tried first (might be
   * slow).
   * @return true if the operation was successful.
   */
  ...
  /** Set the number to e, the base of natural logarithm, calculated with the
   * current default precision.
   */
  void e(bool use_cached_number = true);
  /** Set the number to pi, Archimede's constant, calculated with the current
   * default precision.
   */
  void pi();
  /** Set the number to Catalan's constant, calculated with the current default
   * precision.
   */
  void catalan();
  /** Set the number to Euler's constant, calculated with the current default
   * precision.
   */
  void euler();
  /** Set the number to Riemann's zeta with the number as integral point. The
   * number must be an integer greater than one.
   *
  ...
  std::string printNumerator(int base = 10, bool display_sign = true,
                             BaseDisplay base_display = BASE_DISPLAY_NORMAL,
                             bool lower_case = false) const;
  std::string printDenominator(int base = 10, bool display_sign = true,
                               BaseDisplay base_display = BASE_DISPLAY_NORMAL,
                               bool lower_case = false) const;
  std::string
  printImaginaryNumerator(int base = 10, bool display_sign = true,
                          BaseDisplay base_display = BASE_DISPLAY_NORMAL,
                          bool lower_case = false) const;
  std::string
  printImaginaryDenominator(int base = 10, bool display_sign = true,
                            BaseDisplay base_display = BASE_DISPLAY_NORMAL,
                            bool lower_case = false) const;
...
int from_float(Number &nr, std::string sbin, unsigned int bits,
               unsigned int expbits = 0);
std::string to_float(Number nr, unsigned int bits, unsigned int expbits = 0,
                     bool *approx = NULL);
int from_float(Number &nr, std::string sbin, unsigned int bits,
               unsigned int expbits, unsigned int sgnpos);
std::string to_float(Number nr, unsigned int bits, unsigned int expbits,
                     unsigned int sgnpos, bool *approx = NULL);
