...
#define NUMBER_BIT_PRECISION                                                   \
  (n_type == NUMBER_TYPE_FLOAT ? mpfr_get_prec(fl_value) : BIT_PRECISION)
...
#define PRINT_MPFR(x, y)                                                       \
  mpfr_out_str(stdout, y, 0, x, MPFR_RNDU);                                    \
  cout << endl;
#define PRINT_MPZ(x, y)                                                        \
  mpz_out_str(stdout, y, x);                                                   \
  cout << endl;
#define PRINT_MPQ(x, y)                                                        \
  mpq_out_str(stdout, y, x);                                                   \
  cout << endl;

#define CREATE_INTERVAL                                                        \
  (CALCULATOR ? CALCULATOR->usesIntervalArithmetic() : true)

#define INTERVAL_FLOOR(x)                                                      \
  x.floor();                                                                   \
  if (x.isInterval()) {                                                        \
    x = x.lowerEndPoint();                                                     \
    x.floor();                                                                 \
  }
#define INTERVAL_CEIL(x)                                                       \
  x.ceil();                                                                    \
  if (x.isInterval()) {                                                        \
    x = x.upperEndPoint();                                                     \
    x.ceil();                                                                  \
...
#define TRUNCATE                                                               \
  (po.custom_time_zone == TZ_TRUNCATE ||                                       \
   po.custom_time_zone == TZ_TRUNCATE + TZ_DOZENAL)
#define DOZENAL                                                                \
  (po.base == BASE_DUODECIMAL &&                                               \
   (po.custom_time_zone == TZ_DOZENAL ||                                       \
    po.custom_time_zone == TZ_TRUNCATE + TZ_DOZENAL))
...
long int integer_log(const mpfr_t &v, int base, bool roundup = false) {
  ...
  if (cmp < 0) {
    ...
    while (true) {
      ...
      if (cmp == 0 || (roundup && cmp < 0))
        break;
      else if (!roundup && cmp < 0) {
        i++;
        break;
      }
      i--;
...
void insert_thousands_separator(string &str, const PrintOptions &po) {
  if (po.digit_grouping != DIGIT_GROUPING_NONE &&
      (po.digit_grouping != DIGIT_GROUPING_LOCALE ||
       !CALCULATOR->local_digit_group_separator.empty() ||
       CALCULATOR->getIgnoreLocale())) {
    size_t group_size = 3, i_format = 0;
    if (po.digit_grouping == DIGIT_GROUPING_LOCALE &&
        CALCULATOR->local_digit_group_format.size() > i_format) {
      if (CALCULATOR->local_digit_group_format[i_format] == CHAR_MAX)
        return;
      if ((signed char)CALCULATOR->local_digit_group_format[i_format] > 0)
        group_size = CALCULATOR->local_digit_group_format[i_format];
    }
    ...
    if (i_deci != string::npos) {
      if (po.digit_grouping != DIGIT_GROUPING_LOCALE &&
          i_deci + po.decimalpoint().length() < str.length() - 4 &&
        ...
        if (do_thin_space == -1) {
          if (po.use_unicode_signs &&
              (!po.can_display_unicode_string_function ||
               (*po.can_display_unicode_string_function)(
                   THIN_SPACE, po.can_display_unicode_string_arg)))
            do_thin_space = 1;
          else
            do_thin_space = 0;
#ifdef _WIN32
          // do not use thin space on Windows < 10
          if (do_thin_space != 0 && !IsWindows10OrGreater())
            do_thin_space = 0;
#endif
        }
        while (i < str.length()) {
    ...
    if (po.digit_grouping == DIGIT_GROUPING_LOCALE || i > group_size + 1) {
      while (i > group_size) {
        ...
        if (po.digit_grouping != DIGIT_GROUPING_LOCALE ||
            CALCULATOR->local_digit_group_separator.empty() ||
            (nobreak && CALCULATOR->local_digit_group_separator == THIN_SPACE &&
             (!po.can_display_unicode_string_function ||
              (*po.can_display_unicode_string_function)(
                  NNBSP, po.can_display_unicode_string_arg)))) {
          if (do_thin_space == -1) {
            if ((po.digit_grouping == DIGIT_GROUPING_LOCALE &&
                 !CALCULATOR->local_digit_group_separator.empty()) ||
        ...
        if (po.digit_grouping == DIGIT_GROUPING_LOCALE &&
            CALCULATOR->local_digit_group_format.size() > i_format + 1) {
          i_format++;
          if (CALCULATOR->local_digit_group_format[i_format] == CHAR_MAX)
            break;
          if ((signed char)CALCULATOR->local_digit_group_format[i_format] > 0)
...
string format_number_string(string cl_str, int base, BaseDisplay base_display,
                            bool show_neg, bool format_base_two = true,
                            const PrintOptions &po = default_print_options) {
  if (format_base_two && (base == 2 || (base == 16 && po.binary_bits >= 8)) &&
      base_display != BASE_DISPLAY_NONE) {
    // use an appropriate number of bits (digits) for binary and hexadecimal
    // numbers
    ...
    if (base == 2 && (base_display == BASE_DISPLAY_NORMAL ||
                      base_display == BASE_DISPLAY_SUFFIX)) {
...
string printMPZ(mpz_ptr integ_pre, int base = 10, bool display_sign = true,
                bool lower_case = false, bool negative_base = false) {
  ...
  if (base == BASE_ROMAN_NUMERALS) {
    if (sign != 0 && mpz_cmpabs_ui(integ_pre, 10000) == -1) {
      ...
      CALCULATOR->error(false,
                        _("Cannot display numbers greater than 9999 or less "
                          "than -9999 as roman numerals."),
                        NULL);
    }
    base = 10;
  }
...
string printMPZ(mpz_srcptr integ_pre, int base = 10, bool display_sign = true,
                bool lower_case = false, bool negative_base = false) {
  mpz_t integ;
  mpz_init_set(integ, integ_pre);
...
void Number::set(string number, const ParseOptions &po) {
  ...
  if (po.base != BASE_UNICODE &&
    ...
    if (pm_index != string::npos) {
      ...
      number = number.substr(pm_index + (number[pm_index] == '+'
                                             ? strlen("+/-")
                                             : strlen(SIGN_PLUSMINUS)));
      if (!number.empty()) {
  ...
  if (po.base == BASE_BINARY_DECIMAL) {
    ...
    for (size_t i = 0; i < number.size(); i++) {
      ...
      if (!b_on && number[number.size() - i - 1] != '0') {
        ...
        while ((signed char)number[number.size() - i - 1] < 0 &&
               (unsigned char)number[number.size() - i - 1] < 0xC0 &&
               i + 1 < number.length()) {
          i++;
          str_char += number[number.size() - i - 1];
        }
        CALCULATOR->error(true,
                          _("Character \'%s\' was ignored in the number \"%s\" "
                            "with base %s."),
                          str_char.c_str(), number.c_str(), "2", NULL);
      }
      if (b_on) {
        if (i % 4 == 0)
          digit += 1;
  ...
  if (po.base == BASE_BIJECTIVE_26) {
    ...
    for (size_t i = 0; i < str.length();) {
      if (!(str[i] >= 'a' && str[i] <= 'z') &&
        ...
        while (i + n < str.length() && (signed char)str[i + n] < 0 &&
               (unsigned char)str[i + n] < 0xC0) {
          n++;
        }
        CALCULATOR->error(true,
                          _("Character \'%s\' was ignored in the number \"%s\" "
                            "in bijective base-26."),
                          str.substr(i, n).c_str(), number.c_str(), NULL);
        str.erase(i, n);
      } else {
        i++;
      }
    }
    // ABC...=1*26^(n-1)+2*26^(n-2)+3*26^(n-3)... + c_n*26^0 (n = number of
    // digits)
  ...
  if (po.base < BASE_CUSTOM ||
      (po.base == BASE_CUSTOM && (!CALCULATOR->customInputBase().isInteger() ||
                                  CALCULATOR->customInputBase() < 2 ||
                                  CALCULATOR->customInputBase() > 62))) {
    ...
    case BASE_SUPER_GOLDEN_RATIO: {
      // supergolden ratio =
      // (1+cbrt((29+3*sqrt(93))/2)+cbrt((29-3*sqrt(93))/2))/3 a=3*sqrt(93)
      base.set(93);
      base.sqrt();
      base.multiply(3);
      // b=cbrt((29-a)/2)
    ...
    default: {
      base = CALCULATOR->customInputBase();
    }
    }
    ...
    if (abs_base <= 62) {
      ...
      i_dot = number.length();
      // iterate the string and place the value of each recognized digit in a
      // vector (base <= abs(62) uses digits 0-9, a-z, A-Z)
      for (size_t i = 0; i < number.length(); i++) {
        ...
        if (number[i] >= '0' && number[i] <= '9') {
          ...
          // include whole Unicode character in error message
          while (i + 1 < number.length() && (signed char)number[i + 1] < 0 &&
                 (unsigned char)number[i + 1] < 0xC0) {
            i++;
            str_char += number[i];
          }
          CALCULATOR->error(true,
                            _("Character \'%s\' was ignored in the number "
                              "\"%s\" with base %s."),
                            str_char.c_str(), number.c_str(),
                            base.print().c_str(), NULL);
        }
        if (c >= 0) {
          if (abs_base <= c && !abs_base.isFraction()) {
            // digit value is higher than allowed by the base: show a warning,
            // but use anyway
            if (!error_digits.empty())
              error_digits += ", ";
      ...
      // iterate the string and place the value of each recognized digit in a
      // vector bases with absolute value over 62 uses Unicode characters as
      // digits with the Unicode code as digit value
      for (size_t i = 0; i < number.length(); i++) {
        size_t i_prev = i;
        long int c = (unsigned char)number[i];
        bool b_esc = false;
        if (number[i] == '\\' && i < number.length() - 1) {
          // backslash is used for specifying the digit value at the current
          // position directly (escaped value)
          i++;
          Number nrd;
          if (is_in(NUMBERS, number[i])) {
            // value begins with a decimal digit (0-9): read as decimal number
            // to the next non-decimal digit
            ...
            i = i2 - 1;
            b_esc = true;
          } else if (number[i] == 'x' && i < number.length() - 1 &&
                     is_in(NUMBERS "ABCDEFabcdef", number[i + 1])) {
            // value begins with a x, follow by a hexadecimal digit (0-9, a-z,
            // A-Z): read as hexadecimal number to the next non-hexadecimal
            // digit
            i++;
          ...
          if (digits.empty() && number[i] == (char)-30 &&
              i + 3 < number.length() && number[i + 1] == (char)-120 &&
              number[i + 2] == (char)-110) {
            ...
            b_minus = !b_minus;
            b_esc = true;
          } else if (i_dot == number.size() &&
                     (number[i] == CALCULATOR->getDecimalPoint()[0] ||
                      (!po.dot_as_separator && number[i] == '.'))) {
            // \.: read as decimal separator
            i_dot = digits.size();
            b_esc = true;
          } else if (b_esc) {
            // digit value found, check if value is allowed by base and show a
            // warning if not (use anyway)
        ...
        if (!b_esc) {
          // not an escaped value: read the whole Unicode character and
          // calculate the Unicode code
          if ((c & 0x80) != 0) {
            if (c < 0xe0) {
              i++;
              if (i >= number.length())
              ...
              if (i + 1 >= number.length())
                return;
              c = (((c & 0xf) << 12) |
                   ((((unsigned char)number[i]) & 0x3f) << 6) |
                   (((unsigned char)number[i + 1]) & 0x3f));
              i++;
            } else {
              i++;
              if (i + 2 >= number.length())
                return;
              c = ((c & 7) << 18) |
                  ((((unsigned char)number[i]) & 0x3f) << 12) |
                  ((((unsigned char)number[i + 1]) & 0x3f) << 6) |
                  (((unsigned char)number[i + 2]) & 0x3f);
              i += 2;
            }
          }
          if (abs_base.isLessThanOrEqualTo(c)) {
            // digit value is higher than allowed by base: show a warning, but
            // use anyway
            if (!error_digits.empty())
              error_digits += ", ";
            error_digits += number.substr(i_prev, i - i_prev + 1);
    ...
    if (!error_digits.empty())
      CALCULATOR->error(
          false, _("Digits (%s) are higher than expected for number base."),
          error_digits.c_str(), NULL);
    clear();
    if (i_dot > digits.size())
      i_dot = digits.size();
    Number nr_mul;
    // value=d_0*base^(n-1)+d_1*base^(n-2)+d_2*base^(n-3)... + d_(n - 1) (n =
    // number of digits, d_i=digit from left to right)
    ...
    // if number string begins with an odd number of minus symbols, negate the
    // value
    if (b_minus)
      negate();
    return;
  ...
  if (po.base == BASE_ROMAN_NUMERALS) {
    ...
    for (size_t i = 0; i < number.length(); i++) {
      switch (number[i]) {
      case 'I': {
        if (i > 0 && i == number.length() - 1 && number[i - 1] != 'i' &&
            number[i - 1] != 'j' &&
            number.find_first_of("jvxlcdm") != string::npos &&
            number.find_first_of("IJVXLCDM") == i) {
          cur.set(2);
          CALCULATOR->error(false,
                            _("Assuming the unusual practice of letting a last "
                              "capital I mean 2 in a roman numeral."),
                            NULL);
          break;
        }
      }
      ...
      case 'c': {
        if (rev_c) {
          ...
          if (i2 != string::npos && number[i2] == '|' &&
              i2 + (i2 - i) < number.length() && number[i2 + (i2 - i)] == ')') {
            bool b = true;
      ...
      case '|': {
        if (large) {
          ...
          break;
                   ...
                   number.find("|", i + 2) != string::npos) {
          cur.clear();
          large = true;
          break;
        }
        CALCULATOR->error(true, _("Error in roman numerals: %s."),
                          number_bak.c_str(), NULL);
        break;
      }
      case '(': {
        if (!rev_c) {
      ...
      default: {
        cur.clear();
        CALCULATOR->error(true, _("Unknown roman numeral: %c."), number[i],
                          NULL);
      }
      }
      if (!cur.isZero()) {
        ...
        if (i > 0 && numbers[i].equals(numbers[i - 1])) {
          ...
          if (rep > 3 && numbers[i].isLessThan(1000)) {
            error = true;
                     ...
                     (numbers[i].equals(5) || numbers[i].equals(50) ||
                      numbers[i].equals(500))) {
            error = true;
          }
        } else {
          rep = 1;
    ...
    if (error && number.find('|') == string::npos) {
      ...
      CALCULATOR->error(false,
                        _("Errors in roman numerals: \"%s\". Interpreted as "
                          "%s, which should be written as %s."),
                        number.c_str(), nr.print().c_str(),
                        nr.print(pro).c_str(), NULL);
    }
    values.clear();
    numbers.clear();
    set(nr);
  ...
  int base = po.base;
  if (base == BASE_CUSTOM)
    base = CALCULATOR->customInputBase().intValue();
  ...
  if (po.base == 16 && number.length() >= 2 && number[0] == '0' &&
      (number[1] == 'x' || number[1] == 'X')) {
    number = number.substr(2, number.length() - 2);
  } else if (po.base == 8 && number.length() >= 2 && number[0] == '0' &&
             (number[1] == 'o' || number[1] == 'O')) {
    number = number.substr(2, number.length() - 2);
  } else if (po.base == 8 && number.length() > 1 && number[0] == '0' &&
             number[1] != '.') {
    number.erase(number.begin());
  } else if (po.base == 2 && number.length() >= 2 && number[0] == '0' &&
             (number[1] == 'b' || number[1] == 'B')) {
    number = number.substr(2, number.length() - 2);
  } else if (po.base == 12 && number.length() >= 2 && number[0] == '0' &&
             (number[1] == 'd' || number[1] == 'D')) {
    number = number.substr(2, number.length() - 2);
  }

  // determine if value is negative for numbers using binary or hexadecimal
  // complement representation (number that begins with 1 or 8 is negative)
  bool b_twos = false;
#define TEST_TWOS                                                              \
  b_twos = (po.twos_complement && po.base == 2 && number[index] == '1') ||     \
           (po.hexadecimal_twos_complement && po.base == 16 &&                 \
            (number[index] == '8' || number[index] == '9' ||                   \
  ...
  if (b_twos && po.base == 2 && po.twos_complement > 1) {                      \
    ...
    for (size_t i = index + 1; i < number.size(); i++) {                       \
      if (number[i] == '0' || number[i] == '1')                                \
        n++;                                                                   \
      else if (number[i] == 'E' || number[i] == 'e' || number[i] == '.')       \
        break;                                                                 \
    }                                                                          \
  ...
  for (size_t index = 0; index < number.size(); index++) {
    if (number[index] >= '0' && ((base >= 10 && number[index] <= '9') ||
                                 (base < 10 && number[index] < '0' + base))) {
      ...
      if (number[index] != (b_twos ? '0' + (base - 1) : '0')) {
        // for negative numbers using complement representation, digit value =
        // base - digit - 1 (e.g. 0=1, 1=0 in binary base)
        mpz_add_ui(num, num,
                   b_twos
                       ? (unsigned long int)(base - 1) - (number[index] - '0')
                       : (unsigned long int)number[index] - '0');
      }
      if (in_decimals) {
    ...
    } else if (po.base == BASE_DUODECIMAL &&
               (number[index] == 'X' || number[index] == 'E' ||
                number[index] == 'x' || number[index] == 'e')) {
      // duo decimal numbers uses X and E instead of A and B
      mpz_mul_si(num, num, base);
      mpz_add_ui(num, num,
                 (number[index] == 'E' || number[index] == 'e') ? 11L : 10L);
      if (in_decimals) {
        mpz_mul_si(den, den, base);
      }
      numbers_started = true;
    } else if (base > 10 && number[index] >= 'a' &&
               number[index] < 'a' + base - (base > 36 ? 36 : 10)) {
      if (!numbers_started && !in_decimals) {
        TEST_TWOS
      }
      mpz_mul_si(num, num, base);
      if (!b_twos || (number[index] != 'a' + (base - (base > 36 ? 37 : 11)))) {
        // for negative numbers using complement representation, digit value =
        // base - digit - 1 (e.g. F=0, 0=15 (F) in hexadecimal base) for bases
        // over 36 digits are case sensitive
        mpz_add_ui(num, num,
                   b_twos ? (unsigned long int)(base - 1) -
                                (number[index] - 'a' + (base > 36 ? 36 : 10))
                          : (unsigned long int)number[index] - 'a' +
                                (base > 36 ? 36 : 10));
      }
      if (in_decimals) {
        mpz_mul_si(den, den, base);
      ...
      if (!b_twos || (number[index] != 'A' + (base - 11))) {
        mpz_add_ui(num, num,
                   b_twos ? (unsigned long int)(base - 1) -
                                (number[index] - 'A' + 10)
                          : (unsigned long int)number[index] - 'A' + 10);
      }
      if (in_decimals) {
        mpz_mul_si(den, den, base);
      }
      numbers_started = true;
               ...
               (number[index] == 'E' || number[index] == 'e') && base <= 10 &&
      ...
      if (exp_minus) {
        ...
        if (i_unc > 0)
          mpz_mul(mpq_denref(unc), mpq_denref(unc), e_den);
        mpz_clear(e_den);
      } else {
        ...
        if (i_unc > 0)
          mpz_mul(mpq_numref(unc), mpq_numref(unc), e_num);
        mpz_clear(e_num);
      }
      break;
    } else if (number[index] == '.') {
      if (in_decimals)
        CALCULATOR->error(false, _("Misplaced decimal separator ignored"),
                          NULL);
      else
        in_decimals = true;
    } else if (number[index] == ':') {
      ...
      if (in_decimals) {
        ...
        CALCULATOR->error(
            true,
            _("\':\' in decimal number ignored (decimal point detected)."),
            NULL);
      } else {
      ...
      minus = !minus;
               ...
               (CALCULATOR && number[index] == 'j' &&
                CALCULATOR->getVariableById(VARIABLE_ID_I)->hasName("j"))) {
      // i or j (if imaginary i variable has j name) found: number is imaginary
      b_cplx = true;
    } else if (base == 10 && number[index] == '(' &&
               index <= number.length() - 2) {
      // two digits in parentheses at the end of number speicifies the
      // uncertainty of the previous two digits
      ...
      if (i_unc > 0) {
        ...
        mpz_set_ui(mpq_numref(unc), i_unc);
               ...
               (number[index] != '_' || !numbers_started)) {
      string str_char = number.substr(index, 1);
      // unrecognized digit: read whole Unicode character and show error
      while (index + 1 < number.length() &&
             (signed char)number[index + 1] < 0 &&
             (unsigned char)number[index + 1] < 0xC0) {
        index++;
        str_char += number[index];
      }
      CALCULATOR->error(
          true,
          _("Character \'%s\' was ignored in the number \"%s\" with base %s."),
          str_char.c_str(), number.c_str(), i2s(base).c_str(), NULL);
    }
  }
  if (b_twos) {
  ...
  if (i_unc <= 0 &&
      (po.read_precision == ALWAYS_READ_PRECISION ||
       (in_decimals && po.read_precision == READ_PRECISION_WHEN_DECIMALS))) {

    // read precision: uncertainty = value of last digit / 2
    // (e.g. 22.0=22.0+/-0.05) upper end point = ((num * 2) + 1)/(den * 2) lower
    // end point = ((num * 2) - 1)/(den * 2)
    ...
    mpz_add_ui(num, num, 1);
    if (minus)
      mpz_neg(mpq_numref(rv1), num);
    else
      mpz_set(mpq_numref(rv1), num);
    mpz_set(mpq_denref(rv1), den);
    mpq_canonicalize(rv1);

    mpz_sub_ui(num, num, 2);
    if (minus)
      mpz_neg(mpq_numref(rv2), num);
    else
      mpz_set(mpq_numref(rv2), num);
    ...
    // avoid rounding issues when displaying the significant digits of the
    // number
    for (int i = 0; i < 3; i++) {
      mpfr_nextbelow(fu_value);
      mpfr_nextabove(fl_value);
    }
...
void Number::set(long int numerator, long int denominator, long int exp_10,
                 bool keep_precision, bool keep_imag) {
  if (!keep_precision) {
    b_approx = false;
    i_precision = -1;
  }
...
bool Number::setInterval(const Number &nr_lower, const Number &nr_upper,
                         bool keep_precision) {
  ...
  if (!nr_l.setToFloatingPoint() || !nr_u.setToFloatingPoint())
    return false;
  ...
  if (mpfr_cmp(nr_l.internalUpperFloat(), nr_u.internalUpperFloat()) > 0)
    mpfr_set(fu_value, nr_l.internalUpperFloat(), MPFR_RNDU);
  else
    mpfr_set(fu_value, nr_u.internalUpperFloat(), MPFR_RNDU);
  if (mpfr_cmp(nr_l.internalLowerFloat(), nr_u.internalLowerFloat()) > 0)
    mpfr_set(fl_value, nr_u.internalLowerFloat(), MPFR_RNDD);
  else
    mpfr_set(fl_value, nr_l.internalLowerFloat(), MPFR_RNDD);
  ...
  if (!b_imag && (nr_l.hasImaginaryPart() || nr_u.hasImaginaryPart())) {
    ...
    i_value->setInterval(nr_l.imaginaryPart(), nr_u.imaginaryPart(),
                         keep_precision);
    setPrecisionAndApproximateFrom(*i_value);
  }
...
void Number::setInternal(mpz_srcptr mpz_value, bool keep_precision,
                         bool keep_imag) {
  if (!keep_precision) {
    b_approx = false;
    i_precision = -1;
...
void Number::setInternal(const mpz_t &mpz_value, bool keep_precision,
                         bool keep_imag) {
  if (!keep_precision) {
    b_approx = false;
    i_precision = -1;
...
void Number::setInternal(const mpq_t &mpq_value, bool keep_precision,
                         bool keep_imag) {
  if (!keep_precision) {
    b_approx = false;
    i_precision = -1;
...
void Number::setInternal(const mpfr_t &mpfr_value, bool merge_precision,
                         bool keep_imag) {
  if (mpfr_inf_p(mpfr_value)) {
    if (mpfr_sgn(mpfr_value) > 0) {
      setPlusInfinity(merge_precision, keep_imag);
      return;
    }
    if (mpfr_sgn(mpfr_value) < 0) {
      setMinusInfinity(merge_precision, keep_imag);
      return;
    }
  }
  b_approx = true;
...
void Number::setImaginaryPart(long int numerator, long int denominator,
                              long int exp_10) {
  if (!i_value) {
    i_value = new Number();
    i_value->markAsImaginaryPart();
...
void Number::set(const Number &o, bool merge_precision, bool keep_imag) {
  ...
  if (o.isApproximate())
    b_approx = true;
  if (i_precision < 0 || o.precision() < i_precision)
...
const mpq_t &Number::internalRational() const { return r_value; }
const mpfr_t &Number::internalUpperFloat() const { return fu_value; }
const mpfr_t &Number::internalLowerFloat() const { return fl_value; }
mpq_t &Number::internalRational() { return r_value; }
mpfr_t &Number::internalUpperFloat() { return fu_value; }
mpfr_t &Number::internalLowerFloat() { return fl_value; }
Number *Number::internalImaginary() const { return i_value; }
void Number::markAsImaginaryPart(bool is_imag) { b_imag = is_imag; }
const NumberType &Number::internalType() const { return n_type; }
...
void Number::precisionToInterval() {
  if (hasImaginaryPart())
    i_value->precisionToInterval();
  if (i_precision >= 0 && !isInfinite(true) && !isInterval()) {
    if (!setToFloatingPoint())
      return;
    mpfr_clear_flags();
    mpfr_t f_log;
...
bool Number::intervalToPrecision(long int min_precision) {
  if (n_type == NUMBER_TYPE_FLOAT && !mpfr_equal_p(fl_value, fu_value)) {
    ...
    if (mpfr_zero_p(f_diff)) {
      mpfr_clears(f_diff, f_mid, NULL);
      return false;
    }
    long int i_prec = integer_log(f_diff, 10) + 1;
    if (i_prec < min_precision || testErrors(0)) {
      mpfr_clears(f_diff, f_mid, NULL);
      return false;
    }
    if (i_value && !i_value->intervalToPrecision()) {
      mpfr_clears(f_diff, f_mid, NULL);
      return false;
    }
    if (i_precision < 0 || i_prec < i_precision)
      i_precision = i_prec;
    mpfr_set(fl_value, f_mid, MPFR_RNDN);
    mpfr_set(fu_value, f_mid, MPFR_RNDN);
    mpfr_clears(f_diff, f_mid, NULL);
    b_approx = true;
  } else if (i_value && !i_value->intervalToPrecision())
    return false;
  return true;
}
void Number::intervalToMidValue(bool increase_precision_if_close) {
  if (i_value)
    i_value->intervalToMidValue();
  if (n_type == NUMBER_TYPE_FLOAT && !mpfr_equal_p(fl_value, fu_value)) {
    if (mpfr_inf_p(fl_value) || mpfr_inf_p(fu_value)) {
      if (mpfr_inf_p(fl_value) && mpfr_inf_p(fu_value) &&
          mpfr_sgn(fl_value) != mpfr_sgn(fu_value))
        clearReal();
      else if (mpfr_inf_p(fl_value))
        mpfr_set(fu_value, fl_value, MPFR_RNDN);
      else
...
void Number::splitInterval(unsigned int nr_of_parts, vector<Number> &v) const {
  if (n_type == NUMBER_TYPE_FLOAT && isReal()) {
    if (nr_of_parts == 2) {
      ...
      mpfr_inits2(mpfr_get_prec(fl_value), value_diff, lower_value, upper_value,
                  value_add, NULL);
      ...
      for (unsigned int i = 1; i <= nr_of_parts; i++) {
        ...
        if (mpfr_cmp(upper_value, fu_value) > 0)
          mpfr_set(upper_value, fu_value, MPFR_RNDU);
        v.push_back(*this);
...
bool Number::getCentralInteger(Number &nr_int, bool *b_multiple,
  ...
  if (cmp == 0) {
    ...
    nr_int.setInternal((__mpz_struct *const)z_int);
    if (b_multiple)
      *b_multiple = false;
    if (v) {
      mpfr_t f_prec;
...
bool Number::mergeInterval(const Number &o, bool set_to_overlap) {
  ...
  if (isRational()) {
    ...
    if (mpfr_cmp(fl_value, o.internalUpperFloat()) > 0 ||
        mpfr_cmp(fu_value, o.internalLowerFloat()) < 0) {
      return false;
    } else {
      if (mpfr_cmp(fl_value, o.internalLowerFloat()) < 0)
        mpfr_set(fl_value, o.internalLowerFloat(), MPFR_RNDD);
      if (mpfr_cmp(fu_value, o.internalUpperFloat()) > 0)
        mpfr_set(fu_value, o.internalUpperFloat(), MPFR_RNDU);
    }
  } else {
    if (mpfr_cmp(fl_value, o.internalLowerFloat()) > 0)
...
void Number::setUncertainty(const Number &o, bool to_precision) {
  ...
  if (o.hasImaginaryPart()) {
    ...
    setPrecisionAndApproximateFrom(*i_value);
    if (o.hasRealPart())
      setUncertainty(o.realPart(), to_precision);
    return;
  }
  if (o.isInfinite()) {
    if (n_type != NUMBER_TYPE_FLOAT) {
...
Number Number::uncertainty() const {
  ...
  if (i_value)
    nr.setImaginaryPart(i_value->uncertainty());
  return nr;
}
...
double Number::floatValue() const {
  if (n_type == NUMBER_TYPE_RATIONAL) {
    ...
    return mpfr_get_d(fu_value, MPFR_RNDN) / 2.0 +
           mpfr_get_d(fl_value, MPFR_RNDN) / 2.0;
  }
  return 0.0;
}
int Number::intValue(bool *overflow) const {
  ...
  if (n_type == NUMBER_TYPE_RATIONAL) {
    if (mpz_fits_sint_p(mpq_numref(r_value)) == 0) {
      if (overflow)
        *overflow = true;
      if (mpz_sgn(mpq_numref(r_value)) == -1)
        return INT_MIN;
      return INT_MAX;
    }
...
bool Number::isApproximate() const { return b_approx; }
bool Number::isFloatingPoint() const { return (n_type == NUMBER_TYPE_FLOAT); }
bool Number::isInterval(bool ignore_imag) const {
  return (n_type == NUMBER_TYPE_FLOAT && !mpfr_equal_p(fl_value, fu_value)) ||
         (!ignore_imag && i_value && i_value->isInterval());
}
...
int Number::precision(int calculate_from_interval) const {
  if (calculate_from_interval < 0) {
    ...
    if (i_precision < 0 || iv_prec < i_precision)
      return iv_prec;
    ...
    if (n_type == NUMBER_TYPE_FLOAT && !mpfr_equal_p(fl_value, fu_value)) {
      ...
      if (i_value && i_prec != 0) {
        ...
        if (imag_prec >= 0 && (i_prec < 0 || imag_prec < i_prec))
          i_prec = imag_prec;
      }
      mpfr_clears(f_diff, f_mid, NULL);
      return i_prec;
...
bool Number::includesInfinity(bool ignore_imag) const {
  return n_type >= NUMBER_TYPE_PLUS_INFINITY ||
         (n_type == NUMBER_TYPE_FLOAT &&
          (mpfr_inf_p(fl_value) || mpfr_inf_p(fu_value))) ||
         (!ignore_imag && i_value && i_value->includesInfinity());
}
bool Number::includesPlusInfinity() const {
  return n_type == NUMBER_TYPE_PLUS_INFINITY ||
         (n_type == NUMBER_TYPE_FLOAT &&
          (mpfr_inf_p(fu_value) && mpfr_sgn(fu_value) >= 0));
}
bool Number::includesMinusInfinity() const {
  return n_type == NUMBER_TYPE_MINUS_INFINITY ||
         (n_type == NUMBER_TYPE_FLOAT &&
          (mpfr_inf_p(fl_value) && mpfr_sgn(fl_value) < 0));
}
...
Number Number::complexNumerator() const {
  Number num;
  if (hasImaginaryPart())
    num.setInternal(mpq_numref(i_value->internalRational()));
  return num;
}
Number Number::complexDenominator() const {
  Number den(1, 0);
  if (hasImaginaryPart())
    den.setInternal(mpq_denref(i_value->internalRational()));
  return den;
}
...
Number Number::operator*(long int i) const {
  Number o2(*this);
  o2.multiply(i);
  return o2;
}
...
bool Number::bitAnd(const Number &o) {
  if (!o.isInteger() || !isInteger())
    return false;
  mpz_and(mpq_numref(r_value), mpq_numref(r_value),
          mpq_numref(o.internalRational()));
  setPrecisionAndApproximateFrom(o);
  return true;
}
bool Number::bitOr(const Number &o) {
  if (!o.isInteger() || !isInteger())
    return false;
  mpz_ior(mpq_numref(r_value), mpq_numref(r_value),
          mpq_numref(o.internalRational()));
  setPrecisionAndApproximateFrom(o);
  return true;
}
bool Number::bitXor(const Number &o) {
  if (!o.isInteger() || !isInteger())
    return false;
  mpz_xor(mpq_numref(r_value), mpq_numref(r_value),
          mpq_numref(o.internalRational()));
  setPrecisionAndApproximateFrom(o);
  return true;
}
...
bool Number::shiftLeft(const Number &o) {
  if (!o.isInteger() || !isInteger() || o.isNegative())
    return false;
  bool overflow = false;
...
bool Number::shiftRight(const Number &o) {
  ...
  if (overflow)
    return false;
  mpz_fdiv_q_2exp(mpq_numref(r_value), mpq_numref(r_value),
                  (unsigned long int)y);
  setPrecisionAndApproximateFrom(o);
  return true;
}
bool Number::shift(const Number &o) {
  ...
  if (y < 0)
    ...
    mpz_mul_2exp(mpq_numref(r_value), mpq_numref(r_value),
                 (unsigned long int)y);
  setPrecisionAndApproximateFrom(o);
  return true;
}
...
bool Number::hasRealPart() const {
  if (isInfinite(true))
    return true;
  if (n_type == NUMBER_TYPE_RATIONAL)
    return mpq_sgn(r_value) != 0;
  return !mpfr_zero_p(fu_value) || !mpfr_zero_p(fl_value);
}
bool Number::hasImaginaryPart() const { return i_value && !i_value->isZero(); }
bool Number::testErrors(int error_level) const {
  if (mpfr_underflow_p()) {
    if (error_level)
      CALCULATOR->error(error_level > 1, _("Floating point underflow"), NULL);
    return true;
  }
  if (mpfr_overflow_p()) {
    if (error_level)
      CALCULATOR->error(error_level > 1, _("Floating point overflow"), NULL);
    return true;
  }
  if (mpfr_divby0_p()) {
    if (error_level)
      CALCULATOR->error(error_level > 1,
                        _("Floating point division by zero exception"), NULL);
    return true;
  }
  if (mpfr_nanflag_p()) {
    if (error_level)
      CALCULATOR->error(error_level > 1,
                        _("Floating point not a number exception"), NULL);
    return true;
  }
  if (mpfr_erangeflag_p()) {
    if (error_level)
      CALCULATOR->error(error_level > 1, _("Floating point range exception"),
                        NULL);
    return true;
  }
  return false;
...
bool testComplexZero(const Number *this_nr, const Number *i_nr) {
  if (!i_nr)
    return false;
  if (!this_nr->isInfinite(true) && !i_nr->isInfinite(true) &&
      !i_nr->isZero() && !this_nr->isZero()) {
    if (i_nr->isFloatingPoint() &&
        (!i_nr->isInterval() || !i_nr->isNonZero())) {
      mpfr_t thisf, testf;
      mpfr_inits2(BIT_PRECISION - 10, thisf, testf, NULL);
      bool b = true, b2 = false;
      if (!this_nr->isInterval() ||
          (!mpfr_zero_p(this_nr->internalLowerFloat()) &&
           !mpfr_inf_p(this_nr->internalLowerFloat()))) {
        b2 = true;
      ...
      if (b && this_nr->isInterval() &&
          !mpfr_zero_p(this_nr->internalUpperFloat()) &&
...
bool testComplex(Number *this_nr, Number *i_nr) {
  // remove imaginary or real part if zero using the magnitude and (internal)
  // precision of the opposite part (e.g. 1e100+1e-100i=1e100) this might be
  // problematic in some situations, but generally makes life easier...
  if (!i_nr)
    return false;
  if (!this_nr->isInfinite(true) && !i_nr->isInfinite(true) &&
      !i_nr->isZero() && !this_nr->isZero()) {
    ...
    if (this_nr->isFloatingPoint() &&
        (!this_nr->isInterval() || !this_nr->realPartIsNonZero())) {
      mpfr_t thisf, testf;
      mpfr_inits2(BIT_PRECISION - 10, thisf, testf, NULL);
      bool b = true, b2 = false;
      if (!this_nr->isInterval() || (!mpfr_zero_p(i_nr->internalLowerFloat()) &&
                                     !mpfr_inf_p(i_nr->internalLowerFloat()))) {
        b2 = true;
        if (i_nr->isFloatingPoint()) {
      ...
      if (b && i_nr->isInterval() && !mpfr_zero_p(i_nr->internalUpperFloat()) &&
          !mpfr_inf_p(i_nr->internalUpperFloat())) {
        b2 = true;
...
bool Number::testFloatResult(bool allow_infinite_result, int error_level,
                             bool test_integer) {
  // test calculated floating point value and show mpfr errors (show as errors
  // if error_level > 1, show as warnings if error_level = 1, do not generate
  // message if error_level is zero)
  if (mpfr_underflow_p()) {
    if (error_level)
      CALCULATOR->error(error_level > 1, _("Floating point underflow"), NULL);
    return false;
  ...
  if (mpfr_nan_p(fu_value) || mpfr_nan_p(fl_value))
    return false;
  if (mpfr_nanflag_p()) {
    if (error_level)
      CALCULATOR->error(error_level > 1,
  ...
  if (mpfr_inexflag_p()) {
    ...
    b_approx = true;
    if (!CREATE_INTERVAL && !isInterval() &&
        (i_precision < 0 || i_precision > FROM_BIT_PRECISION(BIT_PRECISION)))
      i_precision = FROM_BIT_PRECISION(BIT_PRECISION);
  }
  mpfr_clear_flags();
  if (mpfr_inf_p(fl_value) && mpfr_inf_p(fu_value) &&
      mpfr_sgn(fl_value) == mpfr_sgn(fu_value)) {
    // lower and upper value is infinite with same sign: set number type
    // accordingly
...
void Number::setPrecisionAndApproximateFrom(const Number &o) {
  if (o.precision() >= 0 && (i_precision < 0 || o.precision() < i_precision))
    i_precision = o.precision();
  if (o.isApproximate())
    b_approx = true;
}

bool Number::isComplex() const { return i_value && i_value->isNonZero(); }
...
bool Number::isInteger(IntegerType integer_type) const {
  if (n_type != NUMBER_TYPE_RATIONAL || hasImaginaryPart())
    return false;
  if (mpz_cmp_ui(mpq_denref(r_value), 1) != 0)
    return false;
  switch (integer_type) {
  case INTEGER_TYPE_NONE: {
    return true;
  }
  case INTEGER_TYPE_SIZE: {
  }
  case INTEGER_TYPE_UINT: {
    return mpz_fits_uint_p(mpq_numref(r_value)) != 0;
  }
  case INTEGER_TYPE_SINT: {
    return mpz_fits_sint_p(mpq_numref(r_value)) != 0;
  }
  case INTEGER_TYPE_ULONG: {
    return mpz_fits_ulong_p(mpq_numref(r_value)) != 0;
  }
  case INTEGER_TYPE_SLONG: {
    return mpz_fits_slong_p(mpq_numref(r_value)) != 0;
  }
  }
  return true;
}
...
bool Number::isZero() const {
  ...
  if (n_type == NUMBER_TYPE_FLOAT)
    return mpfr_zero_p(fu_value) && mpfr_zero_p(fl_value);
  else if (n_type == NUMBER_TYPE_RATIONAL)
    return mpz_sgn(mpq_numref(r_value)) == 0;
  return false;
}
bool Number::isNonInteger() const {
  ...
  bool b_ret =
      mpfr_equal_p(fu_test, fl_test) && !mpfr_equal_p(fl_test, fl_value);
  mpfr_clears(fu_test, fl_test, NULL);
  return b_ret;
}
bool Number::isNonZero() const {
  if (i_value && i_value->isNonZero())
    return true;
  if (n_type == NUMBER_TYPE_FLOAT)
    return !mpfr_zero_p(fu_value) && mpfr_sgn(fu_value) == mpfr_sgn(fl_value);
  else if (n_type == NUMBER_TYPE_RATIONAL)
    return mpz_sgn(mpq_numref(r_value)) != 0;
  return true;
}
bool Number::isOne() const {
  if (!isReal())
    return false;
  if (n_type == NUMBER_TYPE_FLOAT) {
    return mpfr_cmp_ui(fu_value, 1) == 0 && mpfr_cmp_ui(fl_value, 1) == 0;
  }
...
bool Number::isTwo() const {
  if (!isReal())
    return false;
  if (n_type == NUMBER_TYPE_FLOAT) {
    return mpfr_cmp_ui(fu_value, 2) == 0 && mpfr_cmp_ui(fl_value, 2) == 0;
  }
  return mpq_cmp_si(r_value, 2, 1) == 0;
}
bool Number::isI() const {
  if (!i_value || !i_value->isOne())
    return false;
  if (n_type == NUMBER_TYPE_FLOAT)
    return mpfr_zero_p(fu_value) && mpfr_zero_p(fl_value);
  else if (n_type == NUMBER_TYPE_RATIONAL)
...
bool Number::isMinusOne() const {
  if (!isReal())
    return false;
  if (n_type == NUMBER_TYPE_FLOAT) {
    return mpfr_cmp_si(fu_value, -1) == 0 && mpfr_cmp_si(fl_value, -1) == 0;
  }
  return mpq_cmp_si(r_value, -1, 1) == 0;
}
bool Number::isMinusI() const {
  if (!i_value || !i_value->isMinusOne())
    return false;
  if (n_type == NUMBER_TYPE_FLOAT)
    return mpfr_zero_p(fu_value) && mpfr_zero_p(fl_value);
  else if (n_type == NUMBER_TYPE_RATIONAL)
...
bool Number::hasNegativeSign() const {
  if (hasRealPart())
    return realPartIsNegative();
  return imaginaryPartIsNegative();
}
bool Number::hasPositiveSign() const {
  if (hasRealPart())
    return realPartIsPositive();
  return imaginaryPartIsPositive();
}
bool Number::equalsZero() const { return isZero(); }
bool Number::equals(const Number &o, bool allow_interval,
                    bool allow_infinite) const {
  if (!allow_infinite && (includesInfinity() || o.includesInfinity()))
    return false;
  if (o.hasImaginaryPart()) {
    if (!i_value || !i_value->equals(*o.internalImaginary(), allow_interval,
                                     allow_infinite))
      return false;
  ...
  if (o.isFloatingPoint() && n_type != NUMBER_TYPE_FLOAT) {
    return mpfr_cmp_q(o.internalLowerFloat(), r_value) == 0 &&
           mpfr_cmp_q(o.internalUpperFloat(), r_value) == 0;
  } else if (n_type == NUMBER_TYPE_FLOAT) {
    if (o.isFloatingPoint())
      return (allow_interval || mpfr_equal_p(fu_value, fl_value)) &&
             mpfr_equal_p(fl_value, o.internalLowerFloat()) &&
             mpfr_equal_p(fu_value, o.internalUpperFloat());
    else
      return mpfr_cmp_q(fu_value, o.internalRational()) == 0 &&
             mpfr_cmp_q(fl_value, o.internalRational()) == 0;
  }
...
bool Number::equals(long int i) const {
  if (hasImaginaryPart())
    return false;
  if (n_type == NUMBER_TYPE_FLOAT)
    return mpfr_cmp_si(fl_value, i) == 0 && mpfr_cmp_si(fu_value, i) == 0;
  else if (n_type == NUMBER_TYPE_RATIONAL)
    return mpq_cmp_si(r_value, i, 1) == 0;
  return false;
}
int Number::equalsApproximately(const Number &o, int prec) const {
  if (includesInfinity() || o.includesInfinity())
    return false;
  if (o.isInterval() && !isInterval())
    return o.equalsApproximately(*this, prec);
  if (equals(o))
    return true;
  int b = 1;
  if (o.hasImaginaryPart()) {
  ...
  if (prec == EQUALS_PRECISION_LOWEST) {
    prec = i_precision;
    if (o.precision() >= 0 && (prec < 0 || o.precision() < prec))
      prec = o.precision();
  } else if (prec == EQUALS_PRECISION_HIGHEST) {
    prec = i_precision;
    if (o.precision() >= 0 && (o.precision() < 0 || o.precision() > prec))
      prec = o.precision();
  }
  if (prec < 0) {
    if (isInterval()) {
      if (o.isFloatingPoint()) {
        if (mpfr_cmp(fl_value, o.internalUpperFloat()) > 0)
          return 0;
        if (mpfr_cmp(fu_value, o.internalLowerFloat()) < 0)
          return 0;
      } else {
        if (mpfr_cmp_q(fl_value, o.internalRational()) > 0 ||
            mpfr_cmp_q(fu_value, o.internalRational()) < 0)
          return 0;
      }
    } else if (o.isInterval()) {
      if (isFloatingPoint()) {
        if (mpfr_cmp(o.internalLowerFloat(), fu_value) > 0)
          return 0;
        if (mpfr_cmp(o.internalUpperFloat(), fl_value) < 0)
          return 0;
      } else {
        if (mpfr_cmp_q(o.internalLowerFloat(), r_value) > 0 ||
            mpfr_cmp_q(o.internalUpperFloat(), r_value) < 0)
          return 0;
      }
    }
  ...
  if (prec_choosen || isApproximate() || o.isApproximate()) {
    ...
    if (isInterval()) {
      if (o.isInterval()) {
        ...
        if (mpfr_equal_p(test1, test2) && mpfr_equal_p(test2, test3) &&
            mpfr_equal_p(test3, test4))
          b2 = 1;
        else if (mpfr_cmp(test1, test4) > 0 || mpfr_cmp(test2, test3) < 0)
          b2 = 0;
        else
          b2 = -1;
        ...
        if (mpfr_equal_p(test1, test2) && mpfr_equal_p(test2, test3))
          b2 = 1;
        else if (mpfr_cmp(test3, test1) < 0 || mpfr_cmp(test3, test2) > 0)
          b2 = 0;
        else
          b2 = -1;
...
ComparisonResult Number::compare(long int i) const {
  return compare(Number(i, 1));
}
ComparisonResult Number::compare(const Number &o, bool ignore_imag) const {
  if (isPlusInfinity()) {
    if ((!ignore_imag && o.hasImaginaryPart()) || o.includesPlusInfinity())
      return COMPARISON_RESULT_UNKNOWN;
    else
      return COMPARISON_RESULT_LESS;
  }
  if (isMinusInfinity()) {
    if ((!ignore_imag && o.hasImaginaryPart()) || o.includesMinusInfinity())
      return COMPARISON_RESULT_UNKNOWN;
    else
      return COMPARISON_RESULT_GREATER;
  }
  if (o.isPlusInfinity()) {
    if ((!ignore_imag && hasImaginaryPart()) || includesPlusInfinity())
      return COMPARISON_RESULT_UNKNOWN;
    return COMPARISON_RESULT_GREATER;
  }
  if (o.isMinusInfinity()) {
    if ((!ignore_imag && hasImaginaryPart()) || includesMinusInfinity())
      return COMPARISON_RESULT_UNKNOWN;
    return COMPARISON_RESULT_LESS;
  }
  if (!ignore_imag && equals(o))
  ...
  if (ignore_imag || (!hasImaginaryPart() && !o.hasImaginaryPart())) {
    ...
    if (o.isFloatingPoint() && n_type != NUMBER_TYPE_FLOAT) {
      ...
      if (i != i2)
        return COMPARISON_RESULT_CONTAINS;
    } else if (n_type == NUMBER_TYPE_FLOAT) {
      if (o.isFloatingPoint()) {
        ...
        if (i != i2 && i2 <= 0 && i >= 0) {
          ...
          if (i > 0) {
            if (i2 <= 0)
              return COMPARISON_RESULT_CONTAINED;
            else
              return COMPARISON_RESULT_OVERLAPPING_GREATER;
          } else if (i < 0) {
            if (i2 >= 0)
              return COMPARISON_RESULT_CONTAINS;
            else
              return COMPARISON_RESULT_OVERLAPPING_LESS;
          } else {
            if (i2 == 0)
              return COMPARISON_RESULT_EQUAL_LIMITS;
            else if (i2 > 0)
              return COMPARISON_RESULT_CONTAINS;
            else
              return COMPARISON_RESULT_CONTAINED;
    ...
    if (i2 == 0 || i == 0) {
      if (i == 0)
        i = i2;
      if (i > 0)
        return COMPARISON_RESULT_EQUAL_OR_GREATER;
      else if (i < 0)
        return COMPARISON_RESULT_EQUAL_OR_LESS;
    } else if (i2 != i) {
      return COMPARISON_RESULT_UNKNOWN;
    }
    if (i == 0)
...
ComparisonResult Number::compareAbsolute(const Number &o,
                                         bool ignore_imag) const {
  if (isPositive()) {
    if (o.isPositive())
      return compare(o, ignore_imag);
    Number nr(o);
    nr.negate();
...
ComparisonResult Number::compareApproximately(const Number &o, int prec) const {
  if (isPlusInfinity()) {
    if (o.hasImaginaryPart() || o.includesPlusInfinity())
      return COMPARISON_RESULT_UNKNOWN;
    else
      return COMPARISON_RESULT_LESS;
  }
  if (isMinusInfinity()) {
    if (o.hasImaginaryPart() || o.includesMinusInfinity())
      return COMPARISON_RESULT_UNKNOWN;
    else
      return COMPARISON_RESULT_GREATER;
  }
  if (o.isPlusInfinity()) {
    if (hasImaginaryPart() || includesPlusInfinity())
      return COMPARISON_RESULT_UNKNOWN;
    return COMPARISON_RESULT_GREATER;
  }
  if (o.isMinusInfinity()) {
    if (hasImaginaryPart() || includesMinusInfinity())
      return COMPARISON_RESULT_UNKNOWN;
    return COMPARISON_RESULT_LESS;
  }
...
bool Number::isGreaterThan(const Number &o) const {
  if (n_type == NUMBER_TYPE_MINUS_INFINITY || o.isPlusInfinity())
    return false;
  if (o.isMinusInfinity())
    return true;
  ...
  if (o.isFloatingPoint() && n_type != NUMBER_TYPE_FLOAT) {
    ...
    if (o.isFloatingPoint())
      return mpfr_greater_p(fl_value, o.internalUpperFloat());
    else
      return mpfr_cmp_q(fl_value, o.internalRational()) > 0;
  }
  return mpq_cmp(r_value, o.internalRational()) > 0;
}
bool Number::isLessThan(const Number &o) const {
  if (o.isMinusInfinity() || n_type == NUMBER_TYPE_PLUS_INFINITY)
    return false;
  if (n_type == NUMBER_TYPE_MINUS_INFINITY || o.isPlusInfinity())
    return true;
  if (hasImaginaryPart() || o.hasImaginaryPart())
  ...
  if (o.isFloatingPoint() && n_type != NUMBER_TYPE_FLOAT) {
    ...
    if (o.isFloatingPoint())
      return mpfr_less_p(fu_value, o.internalLowerFloat());
    else
      return mpfr_cmp_q(fu_value, o.internalRational()) < 0;
  }
  return mpq_cmp(r_value, o.internalRational()) < 0;
...
bool Number::isGreaterThanOrEqualTo(const Number &o) const {
  ...
  if (!hasImaginaryPart() && !o.hasImaginaryPart()) {
    if (o.isFloatingPoint() && n_type != NUMBER_TYPE_FLOAT) {
      ...
      if (o.isFloatingPoint())
        return mpfr_greaterequal_p(fl_value, o.internalUpperFloat());
      else
        return mpfr_cmp_q(fl_value, o.internalRational()) >= 0;
    }
    return mpq_cmp(r_value, o.internalRational()) >= 0;
...
bool Number::isLessThanOrEqualTo(const Number &o) const {
  ...
  if (!hasImaginaryPart() && !o.hasImaginaryPart()) {
    if (o.isFloatingPoint() && n_type != NUMBER_TYPE_FLOAT) {
      ...
      if (o.isFloatingPoint())
        return mpfr_lessequal_p(fu_value, o.internalLowerFloat());
      else
        return mpfr_cmp_q(fu_value, o.internalRational()) <= 0;
    }
    return mpq_cmp(r_value, o.internalRational()) <= 0;
...
bool Number::denominatorIsEven() const {
  return !hasImaginaryPart() && n_type == NUMBER_TYPE_RATIONAL &&
         mpz_even_p(mpq_denref(r_value));
}
bool Number::denominatorIsTwo() const {
  return !hasImaginaryPart() && n_type == NUMBER_TYPE_RATIONAL &&
         mpz_cmp_si(mpq_denref(r_value), 2) == 0;
}
...
bool Number::numeratorIsOne() const {
  return !hasImaginaryPart() && n_type == NUMBER_TYPE_RATIONAL &&
         mpz_cmp_si(mpq_numref(r_value), 1) == 0;
}
...
int Number::integerLength() const {
  if (isInteger())
    return mpz_sizeinbase(mpq_numref(r_value), 2);
  return 0;
}
...
bool Number::add(const Number &o) {
  if (n_type == NUMBER_TYPE_RATIONAL && o.realPartIsRational()) {
    if (o.hasImaginaryPart()) {
      if (!i_value) {
        i_value = new Number(*o.internalImaginary());
        i_value->markAsImaginaryPart();
      } else if (!i_value->add(*o.internalImaginary()))
        return false;
      setPrecisionAndApproximateFrom(*i_value);
    }
    mpq_add(r_value, r_value, o.internalRational());
  ...
  if (includesMinusInfinity() && o.includesPlusInfinity())
    return false;
  if (includesPlusInfinity() && o.includesMinusInfinity())
    return false;
  if (isInfinite(true)) {
    setPrecisionAndApproximateFrom(o);
    return true;
  ...
  if (o.isFloatingPoint() || n_type == NUMBER_TYPE_FLOAT) {
    ...
    if (n_type != NUMBER_TYPE_FLOAT) {
      ...
      if (o.isFloatingPoint()) {
        ...
        if (mpfr_get_exp(fu_value) > 1000000L &&
            mpz_cmp_ui(mpq_denref(o.internalRational()), 1) != 0) {
          Number nr_o(o);
          nr_o.clearImaginary();
...
bool Number::add(long int i) {
  ...
  if (n_type == NUMBER_TYPE_FLOAT) {
    ...
    if (i < 0)
      mpz_submul_ui(mpq_numref(r_value), mpq_denref(r_value),
                    (unsigned int)(-i));
    else
      mpz_addmul_ui(mpq_numref(r_value), mpq_denref(r_value), (unsigned int)i);
  }
  return true;
}
...
bool Number::subtract(const Number &o) {
  if (n_type == NUMBER_TYPE_RATIONAL && o.realPartIsRational()) {
    if (o.hasImaginaryPart()) {
      ...
      if (!i_value->subtract(*o.internalImaginary()))
        return false;
      setPrecisionAndApproximateFrom(*i_value);
    }
    mpq_sub(r_value, r_value, o.internalRational());
  ...
  Number nr_bak(*this);
  if (includesPlusInfinity() && o.includesPlusInfinity())
    return false;
  if (includesMinusInfinity() && o.includesMinusInfinity())
    return false;
  if (o.hasImaginaryPart()) {
    if (!i_value) {
      i_value = new Number();
...
bool Number::subtract(long int i) {
  ...
  if (n_type == NUMBER_TYPE_FLOAT) {
    ...
    if (i < 0)
      mpz_addmul_ui(mpq_numref(r_value), mpq_denref(r_value),
                    (unsigned int)(-i));
    else
      mpz_submul_ui(mpq_numref(r_value), mpq_denref(r_value), (unsigned int)i);
  }
  return true;
}
...
bool Number::multiply(const Number &o) {
  if (o.hasImaginaryPart()) {
    if (o.hasRealPart()) {
      ...
      if (hasImaginaryPart()) {
        if (hasRealPart()) {
          ...
          nr_copy.set(*i_value);
          if (!nr_real.multiply(o.realPart()) ||
              !nr_copy.multiply(o.imaginaryPart()) || !nr_copy.negate() ||
              !nr_real.add(nr_copy))
            return false;
          Number nr_imag(*i_value);
          nr_copy.set(*this, false, true);
          if (!nr_copy.multiply(o.imaginaryPart()) ||
              !nr_imag.multiply(o.realPart()) || !nr_imag.add(nr_copy))
            return false;
    ...
    if (!multiply(*o.internalImaginary()))
      return false;
    if (!i_value) {
      i_value = new Number();
      i_value->markAsImaginaryPart();
  ...
  if (o.includesInfinity() && !isNonZero())
    return false;
  if (includesInfinity() && !o.isNonZero())
    return false;
  if (n_type == NUMBER_TYPE_MINUS_INFINITY ||
      n_type == NUMBER_TYPE_PLUS_INFINITY) {
    if (hasImaginaryPart()) {
      if (!i_value->multiply(o))
        return false;
      setPrecisionAndApproximateFrom(*i_value);
    }
    if (o.isNegative()) {
  ...
  if (!o.isFloatingPoint() && n_type == NUMBER_TYPE_RATIONAL &&
      (mpz_sizeinbase(mpq_numref(r_value), 10) +
               mpz_sizeinbase(mpq_numref(o.internalRational()), 10) >=
           1000000LL ||
       mpz_sizeinbase(mpq_denref(r_value), 10) +
               mpz_sizeinbase(mpq_denref(o.internalRational()), 10) >=
           1000000LL)) {
    if (!setToFloatingPoint())
      return false;
  }
...
bool Number::multiply(long int i) {
  if (i == 0 && includesInfinity())
    return false;
  if (n_type == NUMBER_TYPE_MINUS_INFINITY ||
      n_type == NUMBER_TYPE_PLUS_INFINITY) {
    if (hasImaginaryPart()) {
      if (!i_value->multiply(i))
        return false;
      setPrecisionAndApproximateFrom(*i_value);
    }
    if (i < 0) {
...
bool Number::divide(const Number &o) {
  if (isInfinite() || o.isInfinite() || o.hasImaginaryPart() ||
      o.isFloatingPoint() || n_type == NUMBER_TYPE_FLOAT) {
    Number oinv(o);
    if (!oinv.recip())
      return false;
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      if (isInterval(false)) {
        ...
        if (!setToFloatingPoint())
          return false;
        if (!i_value->setToFloatingPoint())
          return false;
        mpfr_t abs_il, abs_iu, absm_iu, absm_il, abs_rl, abs_ru, absm_ru,
            absm_rl, ftmp1, ftmp2, fu_tmp, fl_tmp;
        mpfr_inits2(BIT_PRECISION, abs_il, abs_iu, absm_iu, absm_il, abs_rl,
                    abs_ru, absm_ru, absm_rl, ftmp1, ftmp2, fu_tmp, fl_tmp,
                    NULL);

        if (mpfr_sgn(fl_value) != mpfr_sgn(fu_value))
          mpfr_set_zero(abs_rl, 0);
        else if (mpfr_cmpabs(fu_value, fl_value) < 0)
          mpfr_abs(abs_rl, fu_value, MPFR_RNDD);
        else
          mpfr_abs(abs_rl, fl_value, MPFR_RNDD);
        if (mpfr_cmpabs(fl_value, fu_value) > 0)
          mpfr_abs(abs_ru, fl_value, MPFR_RNDU);
        else
          mpfr_abs(abs_ru, fu_value, MPFR_RNDU);
        mpfr_neg(absm_ru, abs_ru, MPFR_RNDD);
        mpfr_neg(absm_rl, abs_rl, MPFR_RNDD);

        if (mpfr_sgn(i_value->internalLowerFloat()) !=
            mpfr_sgn(i_value->internalUpperFloat()))
          mpfr_set_zero(abs_il, 0);
        else if (mpfr_cmpabs(i_value->internalUpperFloat(),
                             i_value->internalLowerFloat()) < 0)
          mpfr_abs(abs_il, i_value->internalUpperFloat(), MPFR_RNDD);
        else
          mpfr_abs(abs_il, i_value->internalLowerFloat(), MPFR_RNDD);
        if (mpfr_cmpabs(i_value->internalLowerFloat(),
                        i_value->internalUpperFloat()) > 0)
          mpfr_abs(abs_iu, i_value->internalLowerFloat(), MPFR_RNDU);
        else
          mpfr_abs(abs_iu, i_value->internalUpperFloat(), MPFR_RNDU);
        ...
        mpfr_clears(abs_il, abs_iu, absm_iu, absm_il, abs_rl, abs_ru, absm_ru,
                    absm_rl, ftmp1, ftmp2, fu_tmp, fl_tmp, NULL);
        ...
        if (!den1.square() || !num_i.negate() || !den2.square() ||
            !den1.add(den2) || !num_r.divide(den1) || !num_i.divide(den1))
          return false;
        set(num_r);
    ...
    if (!i_value->recip() || !i_value->negate())
      return false;
    setPrecisionAndApproximateFrom(*i_value);
    return true;
  }
...
bool Number::raise(const Number &o, bool try_exact) {
  if (o.isTwo()) {
    if (!try_exact && n_type == NUMBER_TYPE_RATIONAL &&
        (mpz_sizeinbase(mpq_numref(r_value), 10) >= 1000 ||
         mpz_sizeinbase(mpq_numref(r_value), 10) >= 1000)) {
      if (!setToFloatingPoint())
        return false;
    }
    if (!try_exact && hasImaginaryPart() && i_value->isRational() &&
        (mpz_sizeinbase(mpq_numref(i_value->internalRational()), 10) >= 1000 ||
         mpz_sizeinbase(mpq_numref(i_value->internalRational()), 10) >= 1000)) {
      if (!i_value->setToFloatingPoint())
        return false;
    }
  ...
  if ((includesInfinity(true) || o.includesInfinity(true)) &&
      !hasImaginaryPart() && !o.hasImaginaryPart()) {
    if (isInfinite(false)) {
      if (o.isNegative()) {
        clear(true);
        return true;
    ...
    if (o.includesMinusInfinity()) {
      if (o.isFloatingPoint() && o.includesPlusInfinity())
        return false;
      if (!isNonZero()) {
        return false;
      } else if (isNegative()) {
        ...
        if (hasRealPart())
          return false;
        if (!o.isFloatingPoint() && !i_value->raise(o))
          return false;
  ...
  if (isZero()) {
    if (o.isZero()) {
      ...
      CALCULATOR->error(false, _("The result of 0^i is possibly undefined"),
                        NULL);
    }
    return true;
  }
  ...
  if (o.isZero()) {
    if (hasImaginaryPart() && i_value->includesInfinity())
      return false;
    set(1, 1, 0, false);
    setPrecisionAndApproximateFrom(o);
    return true;
  ...
  if (o.hasImaginaryPart()) {
    ...
    Number a2b2(nr_a);
    Number b2(nr_b);
    if (!a2b2.square() || !b2.square() || !a2b2.add(b2))
      return false;
    if (!nr_c.isZero()) {
      Number chalf(nr_c);
      a2b2c2 = a2b2;
      if (!chalf.multiply(nr_half) || !a2b2c2.raise(chalf))
        return false;
    }
    Number nr_arg(nr_b);
    if (!nr_arg.atan2(nr_a, true))
      return false;
    Number eraised, nexp(nr_d);
    eraised.e();
    if (!nexp.negate() || !nexp.multiply(nr_arg) || !eraised.raise(nexp, false))
      return false;

    if (!nr_arg.multiply(nr_c) || !nr_d.multiply(nr_half) || !a2b2.ln() ||
        !nr_d.multiply(a2b2) || !nr_arg.add(nr_d))
      return false;
    Number nr_cos(nr_arg);
    Number nr_sin(nr_arg);
    if (!nr_cos.cos() || !nr_sin.sin() || !nr_sin.multiply(nr_one_i) ||
        !nr_cos.add(nr_sin))
      return false;
    if (!eraised.multiply(a2b2c2) || !eraised.multiply(nr_cos))
      return false;
    if ((hasImaginaryPart() || o.hasRealPart()) && eraised.isInterval(false) &&
        eraised.precision(1) <= PRECISION + 20)
      CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
                        _("Interval calculated wide."), NULL);
    set(eraised);
    setPrecisionAndApproximateFrom(o);
    return true;
  }
  if (hasImaginaryPart()) {
    if (o.isNegative()) {
      ...
      if (!ninv.recip() || !opos.negate() || !ninv.raise(opos, try_exact))
        return false;
      set(ninv);
      return true;
    } // else if(!o.isNonNegative()) return false;
    if (hasRealPart() || !o.isInteger()) {
      if (try_exact && !isFloatingPoint() && !i_value->isFloatingPoint() &&
          o.isInteger() && o.isPositive() && o.isLessThan(100)) {
        int i = o.intValue();
        Number nr_init(*this);
        while (i > 1) {
          if (i > 5 && CALCULATOR->aborted())
            return false;
          if (!multiply(nr_init)) {
            set(nr_init);
            return false;
      ...
      nbase.e();
      if (!nexp.ln() || !nexp.multiply(o) || !nbase.raise(nexp, false))
        return false;
      set(nbase);
      return true;
    }
    if (!i_value->raise(o, try_exact))
      return false;
    Number ibneg(o);
    if (!ibneg.iquo(2))
      return false;
    if (!ibneg.isEven() && !i_value->negate())
      return false;
    if (o.isEven()) {
  ...
  if (n_type == NUMBER_TYPE_RATIONAL && !o.isFloatingPoint()) {
    bool success = false;
    if (mpz_fits_slong_p(mpq_numref(o.internalRational())) != 0 &&
        mpz_fits_ulong_p(mpq_denref(o.internalRational())) != 0) {
      ...
      if (length2 > length1)
        length1 = length2;
      if ((i_root <= 2 || mpq_sgn(r_value) > 0) &&
          ((!try_exact && i_root <= 3 &&
            (long long int)labs(i_pow) * length1 < 1000) ||
           (try_exact && (long long int)labs(i_pow) * length1 < 1000000LL &&
            i_root < 1000000L))) {
        ...
        if (i_root != 1) {
          ...
          if (mpz_cmp_ui(mpq_denref(r_test), 1) == 0) {
            if (i_pow != 1)
              mpz_pow_ui(mpq_numref(r_test), mpq_numref(r_test),
                         (unsigned long int)i_pow);
            if (i_root % 2 == 0 && mpq_sgn(r_test) < 0) {
              ...
              if (i_root == 2) {
                ...
                success =
                    mpz_root(mpq_numref(r_test), mpq_numref(r_test), i_root);
                complex_result = true;
              }
            } else {
              success =
            ...
            if (i_pow != 1) {
              mpz_pow_ui(mpq_numref(r_test), mpq_numref(r_test),
                         (unsigned long int)i_pow);
              mpz_pow_ui(mpq_denref(r_test), mpq_denref(r_test),
                         (unsigned long int)i_pow);
            }
            if (i_root % 2 == 0 && mpq_sgn(r_test) < 0) {
              if (b_imag) {
              ...
              if (i_root == 2) {
                ...
                success =
                    mpz_root(mpq_numref(r_test), mpq_numref(r_test), i_root) &&
                    mpz_root(mpq_denref(r_test), mpq_denref(r_test), i_root);
                complex_result = true;
              }
            } else {
              success =
          ...
          if (i_pow != 1) {
            mpz_pow_ui(mpq_numref(r_value), mpq_numref(r_value),
                       (unsigned long int)i_pow);
            if (mpz_cmp_ui(mpq_denref(r_value), 1) != 0)
              mpz_pow_ui(mpq_denref(r_value), mpq_denref(r_value),
                         (unsigned long int)i_pow);
          }
          success = true;
        } else {
  ...
  if (o.isFloatingPoint()) {
    int sgn_ol = mpfr_sgn(o.internalLowerFloat()),
        sgn_ou = mpfr_sgn(o.internalUpperFloat());
    ...
    if (sgn_l < 0) {
      ...
      if (!isInterval() && !o.isInterval()) {
        ...
        mpfr_inits2(BIT_PRECISION, f_value1, f_value2, f_value3, f_value4,
                    NULL);
        ...
        if (mpfr_cmp(f_value2, f_value1) > 0) {
          if (mpfr_cmp(f_value3, f_value2) > 0) {
            if (mpfr_cmp(f_value4, f_value3) > 0)
              i_upper = 4;
            else
              i_upper = 3;
          } else if (mpfr_cmp(f_value4, f_value2) > 0)
            i_upper = 4;
          else
            i_upper = 2;
          ...
          if (mpfr_cmp(f_value4, f_value3) > 0)
            ...
            i_upper = 3;
        } else if (mpfr_cmp(f_value4, f_value1) > 0)
          i_upper = 4;
        ...
        switch (i_upper) {
        case 1: {
          mpfr_pow(f_value1, fu_value, o.internalUpperFloat(), MPFR_RNDU);
          break;
        }
        case 2: {
          mpfr_pow(f_value1, fl_value, o.internalLowerFloat(), MPFR_RNDU);
          break;
        }
        case 3: {
          mpfr_pow(f_value1, fu_value, o.internalLowerFloat(), MPFR_RNDU);
          break;
        }
        case 4: {
          mpfr_pow(f_value1, fl_value, o.internalUpperFloat(), MPFR_RNDU);
          break;
        }
        }
        switch (i_lower) {
        case 1: {
          mpfr_pow(fl_value, fu_value, o.internalUpperFloat(), MPFR_RNDD);
          break;
        }
        case 2: {
          mpfr_pow(fl_value, fl_value, o.internalLowerFloat(), MPFR_RNDD);
        ...
        case 3: {
          mpfr_pow(fl_value, fu_value, o.internalLowerFloat(), MPFR_RNDD);
          break;
        }
        case 4: {
          mpfr_pow(fl_value, fl_value, o.internalUpperFloat(), MPFR_RNDD);
          break;
        }
        }
        ...
        if (sgn_ou != sgn_ol) {
          if (mpfr_cmp_ui(fl_value, 1) > 0)
            mpfr_set_ui(fl_value, 1, MPFR_RNDD);
          else if (mpfr_cmp_ui(fu_value, 1) < 0)
    ...
    if (o.isInteger() && o.integerLength() < 1000000L) {
      if (!CREATE_INTERVAL && !isInterval()) {
        mpfr_pow_z(fl_value, fl_value, mpq_numref(o.internalRational()),
                   MPFR_RNDN);
        mpfr_set(fu_value, fl_value, MPFR_RNDN);
      } else if (o.isEven() && sgn_l < 0 && sgn_u >= 0) {
        if (mpfr_cmpabs(fu_value, fl_value) < 0) {
          mpfr_pow_z(fu_value, fl_value, mpq_numref(o.internalRational()),
                     MPFR_RNDU);
        } else {
          mpfr_pow_z(fu_value, fu_value, mpq_numref(o.internalRational()),
                     MPFR_RNDU);
        }
      ...
      if (sgn_l < 0) {
        ...
        if (o.numeratorIsOne() &&
            mpz_fits_ulong_p(mpq_denref(o.internalRational()))) {
          unsigned long int i_root =
              mpz_get_ui(mpq_denref(o.internalRational()));
          if (!CREATE_INTERVAL && !isInterval()) {
          ...
          if (mpfr_equal_p(fu_value, fl_value) ||
            ...
            switch (i_upper) {
            case 1: {
              mpfr_pow(f_value1, fu_value, f_pow_u, MPFR_RNDU);
              break;
            }
            case 2: {
              mpfr_pow(f_value1, fl_value, f_pow_l, MPFR_RNDU);
              break;
            }
            case 3: {
              mpfr_pow(f_value1, fu_value, f_pow_l, MPFR_RNDU);
              break;
            }
            case 4: {
              mpfr_pow(f_value1, fl_value, f_pow_u, MPFR_RNDU);
              break;
            }
            }
            switch (i_lower) {
            case 1: {
              mpfr_pow(fl_value, fu_value, f_pow_u, MPFR_RNDD);
              break;
            }
            case 2: {
              mpfr_pow(fl_value, fl_value, f_pow_l, MPFR_RNDD);
            ...
            case 3: {
              mpfr_pow(fl_value, fu_value, f_pow_l, MPFR_RNDD);
              break;
            }
            case 4: {
              mpfr_pow(fl_value, fl_value, f_pow_u, MPFR_RNDD);
  ...
  if (try_complex) {
    ...
    if (sgn_l < 0 && sgn_u >= 0) {
      ...
      if (!nr_neg.raise(o) || (b_imag && nr_neg.hasImaginaryPart()))
        return false;
      Number nr_pos(upperEndPoint());
      if (!nr_pos.raise(o))
        return false;
      if (!nr_neg.setInterval(nr_zero, nr_neg) ||
          !nr_pos.setInterval(nr_zero, nr_pos))
        return false;
      if (!setInterval(nr_neg, nr_pos))
        return false;
    } else {
      Number nbase, nexp(*this);
      nbase.e();
      ...
      if (!nexp.multiply(o))
        return false;
      if (!nbase.raise(nexp, false))
        return false;
      if (b_imag && nbase.hasImaginaryPart())
        return false;
      set(nbase);
    }
    return true;
...
bool Number::sqrt() {
  if (hasImaginaryPart())
    return raise(Number(1, 2, 0), true);
  if (isNegative()) {
    if (b_imag)
      return false;
    if (!i_value) {
  ...
  if (n_type == NUMBER_TYPE_RATIONAL) {
    if (mpz_perfect_square_p(mpq_numref(r_value)) &&
        mpz_perfect_square_p(mpq_denref(r_value))) {
...
bool Number::cbrt() {
  if (hasImaginaryPart())
    return raise(Number(1, 3, 0), true);
  if (isOne() || isMinusOne() || isZero())
    return true;
  Number nr_bak(*this);
  if (n_type == NUMBER_TYPE_RATIONAL) {
    if (mpz_root(mpq_numref(r_value), mpq_numref(r_value), 3) &&
        mpz_root(mpq_denref(r_value), mpq_denref(r_value), 3)) {
      return true;
    }
    set(nr_bak);
  }
...
bool Number::root(const Number &o) {
  if (!o.isInteger() || !o.isPositive() || hasImaginaryPart() ||
      (o.isEven() && !isNonNegative()))
    return false;
  if (isOne() || o.isOne() || isZero() || isPlusInfinity())
    return true;
  if (o.isTwo())
    return sqrt();
  /*if(o.isEven() && (!isReal() || isNegative())) {
          ...
          while(o_odd_factor.isEven() && !o_odd_factor.isTwo()) {
                  if(!o_odd_factor.multiply(nr_half) ||
  !o_even_factor.multiply(2)) return false; if(CALCULATOR->aborted()) return
  false;
          }
  ...
  if (!mpz_fits_ulong_p(mpq_numref(o.internalRational()))) {
    ...
    if (n_type == NUMBER_TYPE_RATIONAL) {
      if (mpz_root(mpq_numref(r_value), mpq_numref(r_value), i_root) &&
          mpz_root(mpq_denref(r_value), mpq_denref(r_value), i_root)) {
        return true;
      }
      set(nr_bak);
      if (!setToFloatingPoint())
...
bool Number::allroots(const Number &o, vector<Number> &roots) {
  ...
  if (!nr_re.square() || !nr_im.square() || !nr_re.add(nr_im) ||
      !nr_re.sqrt() || !nr_re.raise(o_inv))
    return false;
  while (nr_i.isLessThan(o)) {
    if (CALCULATOR->aborted())
      return false;
    Number nr(nr_pi2);
    if (!nr.multiply(nr_i) || !nr.add(nr_arg) || !nr.multiply(nr_one_i) ||
        !nr.multiply(o_inv) || !nr.exp() || !nr.multiply(nr_re))
      return false;
    roots.push_back(nr);
    nr_i++;
  }
...
bool Number::square() {
  ...
  if (hasImaginaryPart()) {
    if (!hasRealPart()) {
      if (i_value->isFloatingPoint() &&
          (CREATE_INTERVAL || i_value->isInterval())) {
        Number nr_bak(*this);
        if (!i_value->setToFloatingPoint())
          return false;
    ...
    } else if ((n_type == NUMBER_TYPE_FLOAT || i_value->isFloatingPoint()) &&
               (CREATE_INTERVAL || isInterval(false))) {
      Number nr_bak(*this);
      if (!setToFloatingPoint())
        return false;
      if (!i_value->setToFloatingPoint()) {
      ...
      if (mpfr_sgn(i_value->internalLowerFloat()) < 0 &&
          mpfr_sgn(i_value->internalUpperFloat()) > 0) {
        if (mpfr_cmpabs(i_value->internalLowerFloat(),
                        i_value->internalUpperFloat()) > 0) {
      ...
      } else if (mpfr_cmpabs(i_value->internalLowerFloat(),
                             i_value->internalUpperFloat()) > 0) {
        mpfr_sqr(f_tmp, i_value->internalLowerFloat(), MPFR_RNDU);
        mpfr_sub(f_rl, f_rl, f_tmp, MPFR_RNDD);
      ...
      bool neg_rl = mpfr_sgn(fl_value) < 0, neg_ru = mpfr_sgn(fu_value) < 0,
           neg_il = mpfr_sgn(i_value->internalLowerFloat()) < 0,
           neg_iu = mpfr_sgn(i_value->internalUpperFloat()) < 0;
      if (neg_rl && !neg_ru && !neg_il) {
        ...
        if (mpfr_cmp(f_tmp, f_il) < 0)
          mpfr_swap(f_il, f_tmp);
        mpfr_mul(f_iu, fu_value, i_value->internalUpperFloat(), MPFR_RNDU);
        mpfr_mul(f_tmp, fl_value, i_value->internalLowerFloat(), MPFR_RNDU);
        if (mpfr_cmp(f_tmp, f_iu) > 0)
          mpfr_swap(f_iu, f_tmp);
      } else if (neg_rl && neg_ru && neg_il && neg_iu) {
        mpfr_mul(f_iu, fl_value, i_value->internalLowerFloat(), MPFR_RNDU);
  ...
  if (n_type == NUMBER_TYPE_RATIONAL &&
      (mpz_sizeinbase(mpq_numref(r_value), 10) >= 500000LL ||
       mpz_sizeinbase(mpq_numref(r_value), 10) >= 500000LL)) {
    if (!setToFloatingPoint())
      return false;
  }
  if (n_type == NUMBER_TYPE_RATIONAL) {
    ...
    if (!CREATE_INTERVAL && !isInterval()) {
      ...
      if (mpfr_sgn(fu_value) < 0) {
        ...
        if (mpfr_cmpabs(fu_value, fl_value) < 0)
          mpfr_sqr(fu_value, fl_value, MPFR_RNDU);
        else
...
void Number::setNegative(bool is_negative) {
  ...
  case NUMBER_TYPE_RATIONAL: {
    if (is_negative != (mpq_sgn(r_value) < 0))
      mpq_neg(r_value, r_value);
    break;
  }
  case NUMBER_TYPE_FLOAT: {
...
bool Number::round(bool halfway_to_even) {
  if (includesInfinity() || hasImaginaryPart())
    return false;
  if (n_type == NUMBER_TYPE_RATIONAL) {
    if (!isInteger()) {
      mpz_t i_rem;
      ...
      mpz_fdiv_qr(mpq_numref(r_value), i_rem, mpq_numref(r_value),
                  mpq_denref(r_value));
      mpz_set_ui(mpq_denref(r_value), 1);
      if (mpz_sgn(i_rem) == 0 &&
          (!halfway_to_even || mpz_odd_p(mpq_numref(r_value)))) {
        if (halfway_to_even)
          mpz_sub(mpq_numref(r_value), mpq_numref(r_value),
                  mpq_denref(r_value));
        else if (mpz_sgn(mpq_numref(r_value)) <= 0)
          mpz_sub(mpq_numref(r_value), mpq_numref(r_value),
                  mpq_denref(r_value));
      }
      mpz_clear(i_rem);
...
bool Number::floor() {
  if (isInfinite() || hasImaginaryPart())
    return false;
...
bool Number::mod(const Number &o) {
  ...
  if (isRational() && o.isRational()) {
    if (isInteger() && o.isInteger()) {
      mpz_fdiv_r(mpq_numref(r_value), mpq_numref(r_value),
                 mpq_numref(o.internalRational()));
    } else {
      mpq_div(r_value, r_value, o.internalRational());
...
bool Number::rem(const Number &o) {
  ...
  if (isRational() && o.isRational()) {
    if (isInteger() && o.isInteger()) {
      mpz_tdiv_r(mpq_numref(r_value), mpq_numref(r_value),
                 mpq_numref(o.internalRational()));
    } else {
      mpq_div(r_value, r_value, o.internalRational());
      mpz_tdiv_r(mpq_numref(r_value), mpq_numref(r_value), mpq_denref(r_value));
...
bool Number::irem(const Number &o, Number &q) {
  ...
  if (!isInteger() || !o.isInteger())
    return false;
  q.set(1, 0);
  mpz_tdiv_qr(mpq_numref(q.internalRational()), mpq_numref(r_value),
              mpq_numref(r_value), mpq_numref(o.internalRational()));
  return true;
}
bool Number::iquo(const Number &o) {
  ...
  if (!isInteger() || !o.isInteger())
    return false;
  mpz_tdiv_q(mpq_numref(r_value), mpq_numref(r_value),
             mpq_numref(o.internalRational()));
  return true;
}
...
bool Number::iquo(const Number &o, Number &r) {
  ...
  if (!isInteger() || !o.isInteger())
    return false;
  r.set(1, 0);
  mpz_tdiv_qr(mpq_numref(r_value), mpq_numref(r.internalRational()),
              mpq_numref(r_value), mpq_numref(o.internalRational()));
  return true;
}
...
bool Number::isqrt() {
  if (isInteger()) {
    if (mpz_sgn(mpq_numref(r_value)) < 0)
      return false;
    mpz_sqrt(mpq_numref(r_value), mpq_numref(r_value));
    return true;
  }
...
void Number::toBoolean() { setTrue(isNonZero()); }
...
void Number::setFalse() { setTrue(false); }
void Number::setLogicalNot() { setTrue(!isNonZero()); }

void Number::e(bool use_cached_number) {
  if (use_cached_number) {
    if (nr_e.isZero() || CREATE_INTERVAL != nr_e.isInterval() ||
        mpfr_get_prec(nr_e.internalLowerFloat()) < BIT_PRECISION) {
      nr_e.e(false);
    }
    set(nr_e);
  } else {
    if (n_type != NUMBER_TYPE_FLOAT) {
      ...
      if (mpfr_get_prec(fu_value) < BIT_PRECISION)
        mpfr_set_prec(fu_value, BIT_PRECISION);
      if (mpfr_get_prec(fl_value) < BIT_PRECISION)
...
bool Number::zeta() {
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    // zeta(x) is decreases when x increases in intervals (-2, 1) and (1,
    // infinity)
    ...
    // unable to detect zeta(x) interval errors reliably for intervals larger
    // than 1 (for x < -2)
    ...
    if (b_iverror)
      CALCULATOR->error(
          false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
          _("%s() lacks proper support interval arithmetic."),
          CALCULATOR->getFunctionById(FUNCTION_ID_ZETA)->name().c_str(), NULL);
  }
...
bool Number::zeta(const Number &o) {
  if (o.isOne())
    return zeta();
  if (o.includesInfinity() || !isGreaterThan(1) || !o.isPositive())
    return false;
  if (isPlusInfinity()) {
    set(1, 1, 0, true);
    return true;
  ...
  if (o.isInterval()) {
    ...
    if (!nr_l.zeta(nr_lo) || !nr_u.zeta(nr_uo))
      return false;
    setInterval(nr_l, nr_u);
    return true;
  }
  ...
  mpfr_set_zero(v, 0);
  mpfr_set_zero(n, 0);
  mpfr_set_si(wprec, -BIT_PRECISION + (mpfr_cmp_ui(x, 10) > 0 ? 10 : 70),
              MPFR_RNDN);
    ...
    if (mpfr_cmp(yprev, wprec) < 0) {
      ...
      if (CREATE_INTERVAL) {
        if (mpfr_zero_p(yprev))
          mpfr_set(yprev, wprec, MPFR_RNDN);
...
bool Number::bernoulli() {
  ...
  if (isGreaterThan(498)) {
    ...
    Number nr_zeta(*this);
    if (!nr_zeta.negate() || !nr_zeta.add(1) || !nr_zeta.zeta() ||
        !nr_zeta.multiply(*this) || !nr_zeta.negate())
      return false;
    set(nr_zeta);
    return true;
  }
  ...
  case 18: {
    set(43867L, 798, 0, true);
    return true;
  }
  case 22: {
    set(854513L, 138, 0, true);
    return true;
  }
  }
  if (m % 2 == 1) {
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (mpfr_sgn(fl_value) > 0) {
      if (mpfr_cmp_d(fl_value, 1.5) < 0) {
        ...
        mpfr_set_str(f_gamma_minx,
                     "1."
                     "461632144968362341262659542325721328468196204006446351295"
                     "98840859878644035380181024307499273372559",
                     10, MPFR_RNDN);
        if (mpfr_cmp(fl_value, f_gamma_minx) < 0) {
          if (mpfr_cmp(fu_value, f_gamma_minx) < 0) {
            ...
            if (mpfr_cmp(fl_value, fu_value) > 0)
              mpfr_swap(fl_value, fu_value);
            mpfr_set_str(fl_value,
                         "0."
                         "88560319441088870027881590058258873320795153366990344"
                         "887120016587513622741739634666479828021420359",
                         10, MPFR_RNDD);
          }
        } else {
          mpfr_gamma(fu_value, fu_value, MPFR_RNDU);
      ...
      // The value is an opposite sides of a negative integer (gamma(x) is
      // undefined for negative integers)
      if (!mpfr_equal_p(fu_test, fl_test) || mpfr_equal_p(fl_test, fl_value)) {
        set(nr_bak);
        return false;
      }
      ...
      if (b_iverror)
        CALCULATOR->error(
            false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
            _("%s() lacks proper support interval arithmetic."),
            CALCULATOR->getFunctionById(FUNCTION_ID_GAMMA)->name().c_str(),
            NULL);
    }
  }
  if (!testFloatResult()) {
...
bool Number::erf() {
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      if (isInterval(false)) {
        ...
        if (i_value->isInterval()) {
          ...
          if (isInterval(true)) {
            ...
            if (!nr_l.erf() || !nr_u.erf() || !nr_l2.erf() || !nr_u2.erf())
              return false;
        ...
        if (precision(1) <= PRECISION + 20)
          CALCULATOR->error(
              false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
              _("%s() lacks proper support interval arithmetic."),
              CALCULATOR->getFunctionById(FUNCTION_ID_ERF)->name().c_str(),
              NULL);
        setPrecisionAndApproximateFrom(nr_l);
        setPrecisionAndApproximateFrom(nr_u);
      ...
      if (v.internalImaginary())
        yprevi.set(*v.internalImaginary());
      for (int i = 0;; i++) {
        den = k;
        den *= 2;
        xpow = k;
        ...
        xpow++;
        num = x;
        if (i > PRECISION * 100 || CALCULATOR->aborted() || !kfac.multiply(k) ||
            !den.multiply(kfac) || !den.add(kfac) || !num.raise(xpow) ||
            !num.divide(den) || (k.isOdd() && !num.negate()) || !v.add(num) ||
            v.includesInfinity()) {
          CALCULATOR->setPrecision(prec_bak);
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          return false;
        ...
        if (v.internalImaginary())
          ...
          yprevi.clear();
        if (!ytest.subtract(yprev) ||
            (yprev.isNonZero() && !ytest.divide(yprev)) || !ytest.abs() ||
            !ytesti.subtract(yprevi) ||
            (yprevi.isNonZero() && !ytesti.divide(yprevi)) || !ytesti.abs()) {
          CALCULATOR->setPrecision(prec_bak);
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          return false;
        }
        if ((ytest < wprec || (!ytest.isNonZero() && ytest < wprec2)) &&
            (ytesti < wprec || (!ytesti.isNonZero() && ytesti < wprec2))) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          if (ytest.isZero()) {
            ytest++;
            ytest.setToFloatingPoint();
            mpfr_nextabove(ytest.internalUpperFloat());
            ytest--;
          }
          if (ytesti.isZero()) {
            ytesti++;
            ytesti.setToFloatingPoint();
            mpfr_nextabove(ytesti.internalUpperFloat());
            ytesti--;
          }
...
bool Number::erfi() {
  ...
  if (isInterval()) {
    ...
    nr_l.setInternal(fl_value);
    nr_u.setInternal(fu_value);
    if (!nr_l.erfi() || !nr_u.erfi())
      return false;
    setInterval(nr_l, nr_u);
    return true;
  }
  if (isGreaterThan(1000) || isLessThan(-1000))
    return false;
  Number nr_bak(*this);
  // erfi(x)=2/sqrt(pi)*sum(x^(1+2k)/(k!+2k*k!),0,infinity,k)
  mpfr_clear_flags();
  mpfr_t k, kfac, xpow, x, num, den, yprev, wprec, v;
  mpfr_inits2(BIT_PRECISION * 2, k, kfac, xpow, x, num, den, yprev, wprec, v,
              NULL);
  if (n_type == NUMBER_TYPE_FLOAT) {
    mpfr_set(x, fl_value, MPFR_RNDN);
  } else {
...
bool Number::erfinv() {
  ...
  if (isZero())
    return true;
  if (!isLessThanOrEqualTo(1) || !isGreaterThanOrEqualTo(-1))
    return false;
  ...
  for (size_t i = 0; i < 2; i++) {
    if (i == 0)
      mpfr_set(x, fl_value, MPFR_RNDD);
    else
      mpfr_set(x, fu_value, MPFR_RNDU);
    if (mpfr_cmp_ui(x, 1) >= 0) {
      ...
      if (i == 0)
        mpfr_set(fl_value, r, MPFR_RNDD);
      else
        mpfr_set(fu_value, r, MPFR_RNDU);
    }
  }
  mpfr_clears(f_test, r, r_div, r2_exp, sqrt_pi, x2, x, NULL);
  if (!testFloatResult()) {
...
bool Number::airy() {
  if (!isReal())
    return false;
  if (!isLessThanOrEqualTo(500) || !isGreaterThanOrEqualTo(-500))
    return false;
  Number nr_bak(*this);
  if (!setToFloatingPoint())
    return false;
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    mpfr_sub(fl_test, fu_value, fl_value, MPFR_RNDU);
    bool b_iverror =
        (mpfr_cmp_si(fl_value, -30) < 0 && mpfr_cmp_d(fl_test, 0.1) > 0) ||
        mpfr_cmp_d(fl_test, 0.5) > 0;
    mpfr_set(fu_test, fu_value, MPFR_RNDN);
    mpfr_set(fl_test, fl_value, MPFR_RNDN);
    ...
    if (b_iverror)
      CALCULATOR->error(
          false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
          _("%s() lacks proper support interval arithmetic."),
          CALCULATOR->getFunctionById(FUNCTION_ID_AIRY)->name().c_str(), NULL);
  }
  if (!testFloatResult()) {
    set(nr_bak);
    return false;
...
bool Number::besselj(const Number &o) {
  if (hasImaginaryPart() || !o.isInteger())
    return false;
  if (isZero()) {
    if (o.isZero())
      set(1, 1, 0, true);
  ...
  if (!mpz_fits_slong_p(mpq_numref(o.internalRational())))
    return false;
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    mpfr_init2(fl_test, mpfr_get_prec(fl_value));
    mpfr_sub(fl_test, fu_value, fl_value, MPFR_RNDU);
    // besselj(n, x) changing direction after every increase by approximately 3
    // of x
    ...
    if (b_iverror)
      CALCULATOR->error(
          false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
          _("%s() lacks proper support interval arithmetic."),
          CALCULATOR->getFunctionById(FUNCTION_ID_BESSELJ)->name().c_str(),
          NULL);
  }
  if (!testFloatResult()) {
    set(nr_bak);
...
bool Number::bessely(const Number &o) {
  if (hasImaginaryPart() || !isNonNegative() || !o.isInteger() || isZero())
    return false;
  if (isPlusInfinity()) {
    clear(true);
    return true;
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    mpfr_init2(fl_test, mpfr_get_prec(fl_value));
    mpfr_sub(fl_test, fu_value, fl_value, MPFR_RNDU);
    // bessely(n, x) changing direction after every increase by approximately 3
    // of x
    bool b_iverror = mpfr_cmp_d(fl_test, 3) > 0;
    mpfr_set(fu_test, fu_value, MPFR_RNDN);
    mpfr_set(fl_test, fl_value, MPFR_RNDN);
    ...
    if (b_iverror)
      CALCULATOR->error(
          false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
          _("%s() lacks proper support interval arithmetic."),
          CALCULATOR->getFunctionById(FUNCTION_ID_BESSELY)->name().c_str(),
          NULL);
  }
  if (!testFloatResult()) {
    set(nr_bak);
...
bool Number::sin() {
  if (includesInfinity()) {
    if (!hasImaginaryPart() && isInterval() &&
        (mpfr_sgn(fl_value) != mpfr_sgn(fu_value) || !mpfr_inf_p(fl_value) ||
         !mpfr_inf_p(fu_value))) {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      t2a.set(t1a);
      t2b.set(t1b);
      if (!t1a.sin() || !t1b.cosh() || !t2a.cos() || !t2b.sinh() ||
          !t1a.multiply(t1b) || !t2a.multiply(t2b))
        return false;
      if (!t1a.isReal() || !t2a.isReal())
        return false;
      set(t1a, true, true);
  ...
  bool do_pi = true;
  if (n_type == NUMBER_TYPE_RATIONAL) {
    if (mpz_cmp_ui(mpq_denref(r_value), 1000000L) < 0)
      do_pi = false;
    if (!setToFloatingPoint())
      return false;
  }
  if (mpfr_get_exp(fl_value) > BIT_PRECISION ||
      mpfr_get_exp(fu_value) > BIT_PRECISION) {
    set(nr_bak);
    return false;
  }
  mpfr_clear_flags();
  if (!CREATE_INTERVAL && !isInterval()) {
    if (do_pi) {
      ...
      if (mpfr_zero_p(f_quo)) {
        ...
        b_approx = true;
        if (i_precision < 0 ||
            FROM_BIT_PRECISION(BIT_PRECISION - 30) < i_precision)
          i_precision = FROM_BIT_PRECISION(BIT_PRECISION - 30);
...
bool Number::asin() {
  ...
  if (hasImaginaryPart() || !isFraction()) {
    ...
    if (hasImaginaryPart() && !hasRealPart()) {
      Number nri(*i_value);
      if (!nri.asinh() || !nri.multiply(nr_one_i))
        return false;
      set(nri, true);
      return true;
    }
    if (isInterval(false)) {
      ...
      if (!nr1.asin() || !nr2.asin())
        return false;
      if (!hasImaginaryPart()) {
        if (!setInterval(nr1, nr2, true))
          return false;
        return true;
      }
      Number nr;
      if (!nr.setInterval(nr1, nr2, true))
        return false;
      if (isInterval(true) && imaginaryPartIsInterval()) {
        ...
        if (!nr3.asin() || !nr4.asin())
          return false;
        if (!nr.setInterval(nr, nr3, true))
          return false;
        if (!nr.setInterval(nr, nr4, true))
          return false;
      }
      if (hasRealPart() && !realPartIsNonZero()) {
        ...
        if (!nr1.asin() || !nr2.asin())
          return false;
        if (!nr.setInterval(nr, nr1, true))
          return false;
        if (!nr.setInterval(nr, nr2, true))
          return false;
      }
    ...
    bool b_neg = false;
    if (hasImaginaryPart()) {
      b_neg = (realPartIsNegative() && !imaginaryPartIsNegative()) ||
              (realPartIsPositive() && imaginaryPartIsPositive());
    } else {
      b_neg = isNegative();
    }
    if (b_neg && (!z_sqln.negate() || !i_z.negate()))
      return false;
    if (!i_z.multiply(nr_one_i))
      return false;
    if (!z_sqln.square() || !z_sqln.negate() || !z_sqln.add(1) ||
        !z_sqln.raise(nr_half) || !z_sqln.add(i_z) || !z_sqln.ln() ||
        !z_sqln.multiply(nr_minus_i))
      return false;
    if (b_neg && !z_sqln.negate())
      return false;
    if (hasImaginaryPart() && z_sqln.isInterval(false) &&
        z_sqln.precision(1) <= PRECISION + 20)
      CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
                        _("Interval calculated wide."), NULL);
    set(z_sqln);
    return true;
...
bool Number::sinh() {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      t2a.set(t1a);
      t2b.set(t1b);
      if (!t1a.sinh() || !t1b.cos() || !t2a.cosh() || !t2b.sin() ||
          !t1a.multiply(t1b) || !t2a.multiply(t2b))
        return false;
      if (!t1a.isReal() || !t2a.isReal())
        return false;
...
bool Number::asinh() {
  ...
  if (hasImaginaryPart()) {
    if (!hasRealPart()) {
      Number inr(*i_value);
      if (!inr.asin() || !inr.multiply(nr_one_i))
        return false;
      set(inr, true);
      return true;
    }
    ...
    Number z_sqln(*this);
    if (!z_sqln.square() || !z_sqln.add(1) || !z_sqln.raise(nr_half) ||
        !z_sqln.add(*this))
      return false;
    // If zero, it means that the precision is too low (since infinity is not
    // the correct value). Happens with number less than -(10^1000)i
    if (z_sqln.isZero())
      return false;
    if (!z_sqln.ln())
      return false;
...
bool Number::cos() {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      t2a.set(t1a);
      t2b.set(t1b);
      if (!t1a.cos() || !t1b.cosh() || !t2a.sin() || !t2b.sinh() ||
          !t1a.multiply(t1b) || !t2a.multiply(t2b) || !t2a.negate())
        return false;
      if (!t1a.isReal() || !t2a.isReal())
        return false;
      set(t1a, true, true);
...
bool Number::acos() {
  ...
  if (hasImaginaryPart() || !isFraction()) {
    ...
    Number nr_pi;
    nr_pi.pi();
    if (!nr.asin() || !nr.multiply(2) || !nr.negate() || !nr.add(nr_pi) ||
        !nr.multiply(nr_half))
      return false;
    set(nr);
    return true;
...
bool Number::cosh() {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      t2a.set(t1a);
      t2b.set(t1b);
      if (!t1a.cosh() || !t1b.cos() || !t2a.sinh() || !t2b.sin() ||
          !t1a.multiply(t1b) || !t2a.multiply(t2b))
        return false;
      if (!t1a.isReal() || !t2a.isReal())
        return false;
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (mpfr_sgn(fl_value) < 0) {
      ...
      if (mpfr_sgn(fl_value) == 0)
        ...
        mpfr_cosh(fl_value, fl_value, MPFR_RNDD);
    }
  }
  if (!testFloatResult()) {
    set(nr_bak);
...
bool Number::acosh() {
  ...
  if (hasImaginaryPart() || !isGreaterThanOrEqualTo(nr_one)) {
    ...
    if (CREATE_INTERVAL && !hasImaginaryPart()) {
      ...
      if (!ipz.add(1) || !imz.subtract(1))
        return false;
      if (!ipz.raise(nr_half) || !imz.raise(nr_half) || !ipz.multiply(imz) ||
          !ipz.add(lowerEndPoint()))
        return false;
      Number ipz2(upperEndPoint()), imz2(ipz2);
      if (!ipz2.add(1) || !imz2.subtract(1))
        return false;
      if (!ipz2.raise(nr_half) || !imz2.raise(nr_half) ||
          !ipz2.multiply(imz2) || !ipz2.add(upperEndPoint()))
        return false;
      Number nriv;
      ...
      if (isGreaterThanOrEqualTo(nr_minus_one)) {
        ...
        if (nriv.isInterval(false) && nriv.precision(1) <= PRECISION + 20)
          CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
                            _("Interval calculated wide."), NULL);
      }
      set(nriv);
    ...
    if (!ipz.raise(nr_half) || !imz.raise(nr_half) || !ipz.multiply(imz) ||
        !ipz.add(*this) || !ipz.ln())
      return false;
    if (hasImaginaryPart() && ipz.isInterval(false) &&
        ipz.precision(1) <= PRECISION + 20)
      CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
                        _("Interval calculated wide."), NULL);
    set(ipz);
    return true;
...
bool Number::tan() {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      t1a.set(*this, false, true);
      t1b.set(*i_value, false, true);
      if (!t1a.multiply(2) || !t1b.multiply(2))
        return false;
      t2a.set(t1a);
      t2b.set(t1b);
      if (!t1a.sin() || !t1b.sinh() || !t2a.cos() || !t2b.cosh() ||
          !t2a.add(t2b) || !t1a.divide(t2a) || !t1b.divide(t2a))
        return false;
      if (!t1a.isReal() || !t1b.isReal())
        return false;
      if (t1a.isInterval(false) && t1a.precision(1) <= PRECISION + 20)
        CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
                          _("Interval calculated wide."), NULL);
      set(t1a, true, true);
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (b_neg1)
      mpfr_neg(fl_quo, fl_quo, MPFR_RNDU);
    if (b_neg2)
      mpfr_neg(fu_quo, fu_quo, MPFR_RNDD);
    ...
    if (b_neg2)
      c2 = -c2;
    if ((c1 != c2 && c1 <= 0 && c2 >= 0) ||
        (c1 == c2 && mpfr_cmp_ui_2exp(f_diff, 1, -1) >= 0)) {
      mpfr_clears(f_diff, fl_pi, fu_pi, fl_quo, fu_quo, NULL);
      set(nr_bak);
      return false;
    }
...
bool Number::atan() {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      Number ipz(*this), imz(*this);
      if (!ipz.multiply(nr_one_i) || !imz.multiply(nr_minus_i) || !ipz.add(1) ||
          !imz.add(1))
        return false;
      if (!ipz.ln() || !imz.ln() || !imz.subtract(ipz) ||
          !imz.multiply(nr_one_i) || !imz.divide(2))
        return false;
      if (imz.isInterval(false) && imz.precision(1) <= PRECISION + 20)
        CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
                          _("Interval calculated wide."), NULL);
      set(imz);
      return true;
      ...
      Number nri(*i_value);
      if (!nri.atanh() || !nri.multiply(nr_one_i))
        return false;
      set(nri, true);
      return true;
    }
...
bool Number::atan2(const Number &o, bool allow_zero) {
  ...
  if (o.isFloatingPoint()) {
    if (!CREATE_INTERVAL && !isInterval() && !o.isInterval()) {
      ...
      if (!allow_zero && !o.isNonZero() && (sgn_l != sgn_u || sgn_l == 0)) {
        set(nr_bak);
        return false;
      }
        ...
        if (sgn_u >= 0) {
          if (sgn_l < 0) {
            ...
            if (sgn_l == 0)
              ...
              mpfr_atan2(fu_value, fl_value, o.internalUpperFloat(), MPFR_RNDD);
          } else {
            mpfr_atan2(fu_value, fu_value, o.internalUpperFloat(), MPFR_RNDD);
          }
          if (sgn_l <= 0)
            mpfr_const_pi(fl_value, MPFR_RNDU);
          else
            mpfr_atan2(fl_value, fl_value, o.internalLowerFloat(), MPFR_RNDU);
          mpfr_swap(fl_value, fu_value);
        } else {
    ...
    if (!CREATE_INTERVAL && !isInterval()) {
      ...
      if (o.isPlusInfinity())
        mpfr_set_inf(of_value, 1);
      else if (o.isMinusInfinity())
        mpfr_set_inf(of_value, -1);
      else
      ...
      Number nr_o(o);
      if (!nr_o.setToFloatingPoint() || !atan2(nr_o))
        return false;
      return true;
    }
  }
...
bool Number::tanh() {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      t1b.set(*this, false);
      t1b.clearReal();
      if (!t1a.tanh() || !t1b.tanh())
        return false;
      t2a.set(t1a);
      t2b.set(t1b);
      if (!t1a.add(t1b) || !t2a.multiply(t2b) || !t2a.add(1) ||
          !t1a.divide(t2a))
        return false;
      if (t1a.isInterval(false) && t1a.precision(1) <= PRECISION + 20)
        CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
...
bool Number::atanh() {
  ...
  if (hasImaginaryPart() || !isLessThanOrEqualTo(1) ||
    ...
    if (!hasImaginaryPart()) {
      ...
      if (cmp1u > 0) {
        i_nr.pi();
        if (!i_nr.multiply(nr_minus_half)) {
          set(nr_bak);
          return false;
        }
        if (cmpm1l < 0) {
          Number nr;
          nr.pi();
          if (!nr.multiply(nr_half) || !i_nr.setInterval(i_nr, nr)) {
            set(nr_bak);
            return false;
          }
        } else if (cmp1l <= 0) {
          if (!i_nr.setInterval(i_nr, nr_zero)) {
            set(nr_bak);
            return false;
          }
        }
        ...
        if (cmp1u > 0) {
          Number nr;
          nr.pi();
          if (!nr.multiply(nr_minus_half) || !i_nr.setInterval(nr, i_nr)) {
            set(nr_bak);
            return false;
          }
        } else if (cmpm1u >= 0) {
          if (!i_nr.setInterval(nr_zero, i_nr)) {
            set(nr_bak);
            return false;
          }
        }
      ...
      if (!i_value) {
        i_value = new Number(i_nr);
        i_value->markAsImaginaryPart();
      } else
        i_value->set(i_nr, true);
      setPrecisionAndApproximateFrom(*i_value);
      if (!testFloatResult()) {
        set(nr_bak);
        return false;
      ...
      Number inr(*i_value);
      if (!inr.atan() || !inr.multiply(nr_one_i))
        return false;
      set(inr, true);
      return true;
    }
    Number ipz(nr_one), imz(nr_one);
    if (!ipz.add(*this) || !imz.subtract(*this) || !ipz.ln() || !imz.ln() ||
        !imz.negate() || !ipz.add(imz) || !ipz.divide(2))
      return false;
    if (ipz.isInterval(false) && ipz.precision(1) <= PRECISION + 20)
      CALCULATOR->error(false, MESSAGE_CATEGORY_WIDE_INTERVAL,
                        _("Interval calculated wide."), NULL);
    set(ipz);
    return true;
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (mpfr_cmp_si(fl_value, -1) == 0)
      ...
      mpfr_atanh(fl_value, fl_value, MPFR_RNDD);
    if (mpfr_cmp_si(fu_value, -1) == 0)
      mpfr_set_inf(fu_value, -1);
    else if (mpfr_cmp_si(fu_value, 1) == 0)
      mpfr_set_inf(fu_value, 1);
    else
      mpfr_atanh(fu_value, fu_value, MPFR_RNDU);
  }
...
bool Number::ln() {
  ...
  if (hasImaginaryPart()) {
    ...
    if (!new_i.atan2(this_r, true) || !new_r.abs() || new_i.hasImaginaryPart())
      return false;
    if (new_r.hasImaginaryPart() || !new_r.ln())
      return false;
    set(new_r);
...
bool Number::log(const Number &o) {
  if (isOne() &&
      (o.isGreaterThan(1) || o.isLessThan(1) || o.imaginaryPartIsNonZero())) {
    clear(true);
    setPrecisionAndApproximateFrom(o);
    return true;
  }
  ...
  if (!num.ln() || !den.ln() || !den.recip() || !num.multiply(den))
    return false;
  if (b_imag && num.hasImaginaryPart())
    return false;
  set(num);
  return true;
}
...
bool Number::lambertW(const Number &k) {
  ...
  if (isInterval(false)) {
    if (hasImaginaryPart() && i_value->isInterval()) {
      ...
      if (isInterval(true)) {
        ...
        nr_l.intervalToMidValue();
        nr_u.intervalToMidValue();
        if (!nr_l.lambertW(k) || !nr_u.lambertW(k))
          return false;
        if (precision(1) <= PRECISION + 20)
          CALCULATOR->error(false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
                            _("%s() lacks proper support interval arithmetic."),
                            CALCULATOR->getFunctionById(FUNCTION_ID_LAMBERT_W)
                                ->name()
                                .c_str(),
                            NULL);
        setPrecisionAndApproximateFrom(nr_l);
        ...
        if (!i_value->isNonZero()) {
          if (k.isZero())
            nr_l.setInterval(nr_zero, nr_l);
          else
            nr_l.setInterval(nr_minus_inf, nr_l);
        }
        if (hasRealPart() && precision(1) <= PRECISION + 20)
          CALCULATOR->error(false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
                            _("%s() lacks proper support interval arithmetic."),
                            CALCULATOR->getFunctionById(FUNCTION_ID_LAMBERT_W)
                                ->name()
      ...
      if (k.isZero()) {
        ...
        if (COMPARISON_MIGHT_BE_EQUAL(cr))
          nr_l.setInterval(nr_minus_one, nr_l);
      } else {
        if (!isNonZero())
          nr_l.setInterval(nr_minus_inf, nr_l);
      }
      if (hasImaginaryPart() && precision(1) <= PRECISION + 20)
        CALCULATOR->error(
            false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
            _("%s() lacks proper support interval arithmetic."),
            CALCULATOR->getFunctionById(FUNCTION_ID_LAMBERT_W)->name().c_str(),
            NULL);
      return setInterval(nr_l, nr_u, true);
    }
  }
  if (k.isZero() && !hasImaginaryPart()) {
    ...
    mexpm1.exp();
    mexpm1.negate();
    if (isGreaterThanOrEqualTo(mexpm1))
      return lambertW();
  }
  bool b_real = false;
  ...
  if (k.isZero() || k.isMinusOne()) {
    if (ip1 && !k.isOne()) {
      ...
      if (nr <= nr_half || (k.isZero() && nr <= Number(5, 8))) {
        if (k.isZero()) {
          Number ndiv(z);
          if (!ndiv.multiply(2) || !ndiv.add(1) ||
              !ndiv.multiply(Number("0.827184")) || !ndiv.add(2) ||
              !v.multiply(Number("7.061302897")) ||
              !v.add(Number("0.1237166")) ||
              !v.multiply(Number("0.35173371")) || !v.divide(ndiv)) {
            CALCULATOR->endTemporaryEnableIntervalArithmetic();
            CALCULATOR->setPrecision(prec_bak);
            return false;
          }
        } else {
          Number i1("2.2591588985");
          i1.setImaginaryPart(Number("4.22096"));
          Number i2("-14.073271");
          i2.setImaginaryPart(Number("-33.767687754"));
          Number i3("-12.7127");
          i3.setImaginaryPart(Number("19.071643"));
          Number i4("-17.23103");
          i4.setImaginaryPart(Number("10.629721"));
          Number n1p2z(z);
          if (!n1p2z.multiply(2) || !n1p2z.add(1) || !i4.multiply(n1p2z) ||
              !i4.add(2) || !i3.multiply(n1p2z)) {
            CALCULATOR->endTemporaryEnableIntervalArithmetic();
            CALCULATOR->setPrecision(prec_bak);
            return false;
          }
          v.set(i2);
          if (!v.multiply(z) || !v.add(i3) || !v.multiply(i1) ||
              !v.divide(i4) || !v.negate()) {
            CALCULATOR->endTemporaryEnableIntervalArithmetic();
            CALCULATOR->setPrecision(prec_bak);
            return false;
          }
    ...
    if (ip1 && (!k.isMinusOne() || z.imaginaryPartIsPositive())) {
      ...
      if (nr <= 1) {
        Number p;
        p.e();
        if (!p.multiply(z) || !p.add(1) || !p.multiply(2) || !p.sqrt()) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          CALCULATOR->setPrecision(prec_bak);
          return false;
        }
        Number p2(p);
        if (!p2.square() || !p2.multiply(Number(-1, 3))) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          CALCULATOR->setPrecision(prec_bak);
          return false;
        }
        Number p3(p);
        if (!p3.raise(nr_three) ||
            !p3.multiply(Number(k.isZero() ? 11 : -11, 72))) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          CALCULATOR->setPrecision(prec_bak);
          return false;
        }
        v.set(p);
        if ((!k.isZero() && !v.negate()) || !v.add(-1) || !v.add(p2) ||
            !v.add(p3)) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          CALCULATOR->setPrecision(prec_bak);
          return false;
  ...
  if (ip1) {
    ...
    if (!k.isZero()) {
      twopiKI.pi();
      if (!twopiKI.multiply(k) || !twopiKI.multiply(2) ||
          !twopiKI.multiply(nr_one_i)) {
        CALCULATOR->endTemporaryEnableIntervalArithmetic();
        CALCULATOR->setPrecision(prec_bak);
        return false;
      }
  ...
  if (v.internalImaginary())
    wprevi.set(*v.internalImaginary());
  for (int i = 0;; i++) {
    wexp.set(v);
    wexp.exp(); // exp(w)
    wexpw = wexp;
    wexpw.multiply(v); // w*exp(w)
    wexpw_d = wexpw;
    wexpw_d.add(wexp); // wexp(w)+w*exp(w)
    wexpw_dd = wexp;
    wexpw_dd.multiply(2);
    wexpw_dd.add(wexpw); // 2*wexp(w)+w*exp(w)
    // w -= 2*(wexpw-z)*wexpw_d/(2*wexpw_d^2-(wexpw-z)*wexpw_dd)
    if (i > PRECISION * 100 || !wexpw.subtract(z) ||
        !wexpw_dd.multiply(wexpw) || !wexpw.multiply(wexpw_d) ||
        !wexpw.multiply(-2) || !wexpw_d.square() || !wexpw_d.multiply(2) ||
        !wexpw_d.subtract(wexpw_dd) || !wexpw.divide(wexpw_d) ||
        !v.add(wexpw) || v.includesInfinity()) {
      CALCULATOR->endTemporaryEnableIntervalArithmetic();
      CALCULATOR->setPrecision(prec_bak);
      return false;
    }
    ...
    if (v.isInterval()) {
      ...
      if (p < prec_bak + 10) {
        if (v.hasImaginaryPart() && !v.imaginaryPartIsNonZero()) {
          ...
          if (wtest.isInterval() && wtest.isNonZero())
            p = wtest.precision(true);
          else
            p = INT_MAX;
        } else if (v.hasRealPart() && !v.realPartIsNonZero()) {
          if (v.hasImaginaryPart() && v.imaginaryPartIsNonZero())
            p = v.internalImaginary()->precision(true);
          else
            p = INT_MAX;
        }
      }
    ...
    if (v.internalImaginary())
      ...
      wprevi.clear();
    if (CALCULATOR->aborted() || !wtest.subtract(wprev) ||
        (wprev.isNonZero() && !wtest.divide(wprev)) || !wtest.abs() ||
        !wtesti.subtract(wprevi) ||
        (wprevi.isNonZero() && !wtesti.divide(wprevi)) || !wtesti.abs()) {
      CALCULATOR->endTemporaryEnableIntervalArithmetic();
      CALCULATOR->setPrecision(prec_bak);
      return false;
    }
    if ((wtest < wprec || (!wtest.isNonZero() && wtest < wprec2)) &&
        (wtesti < wprec || (!wtesti.isNonZero() && wtesti < wprec2))) {
      CALCULATOR->endTemporaryEnableIntervalArithmetic();
      if (wtest.isZero()) {
        wtest++;
        wtest.setToFloatingPoint();
        mpfr_nextabove(wtest.internalUpperFloat());
        wtest--;
      }
      if (wtesti.isZero() && v.hasImaginaryPart() && !b_real) {
        wtesti++;
        wtesti.setToFloatingPoint();
        mpfr_nextabove(wtesti.internalUpperFloat());
        wtesti--;
      }
...
bool Number::lambertW() {
  ...
  if (isInterval()) {
    ...
    if (!nr_l.lambertW() || !nr_u.lambertW())
      return false;
    setPrecisionAndApproximateFrom(nr_l);
    setPrecisionAndApproximateFrom(nr_u);
    return setInterval(nr_l, nr_u, true);
  ...
  if (cmp == 0) {
    ...
    mpfr_inits2(BIT_PRECISION, wPrec, wTimesExpW, wPlusOneTimesExpW, testXW,
                tmp1, tmp2, NULL);
    ...
    while (true) {
      if (CALCULATOR->aborted() || testErrors()) {
        mpfr_clears(x, m1_div_exp1, w, wPrec, wTimesExpW, wPlusOneTimesExpW,
                    testXW, tmp1, tmp2, NULL);
        set(nr_bak);
        return false;
      }
    ...
    mpfr_clears(x, m1_div_exp1, w, wPrec, wTimesExpW, wPlusOneTimesExpW, testXW,
                tmp1, tmp2, NULL);
  }
  if (!testFloatResult(true)) {
    set(nr_bak);
...
bool Number::gcd(const Number &o) {
  ...
  if (!isInteger() || !o.isInteger()) {
    ...
    if (!nr_num.gcd(o.numerator()) || !nr_den.lcm(o.denominator()) ||
        !nr_num.divide(nr_den))
      return false;
    set(nr_num);
    return true;
  }
  ...
  mpz_gcd(mpq_numref(r_value), mpq_numref(r_value),
          mpq_numref(o.internalRational()));
  setPrecisionAndApproximateFrom(o);
  return true;
}
bool Number::lcm(const Number &o) {
  ...
  if (!isInteger() || !o.isInteger()) {
    Number nr_num(numerator());
    Number nr_den(denominator());
    if (!nr_num.lcm(o.numerator()) || !nr_den.gcd(o.denominator()) ||
        !nr_num.divide(nr_den))
      return false;
    set(nr_num);
    return true;
  }
  mpz_lcm(mpq_numref(r_value), mpq_numref(r_value),
          mpq_numref(o.internalRational()));
  setPrecisionAndApproximateFrom(o);
  return true;
}
...
bool Number::polylog(const Number &o) {
  ...
  if (o.includesInfinity())
    return false;
  if (o.isOne()) {
    Number nln(*this);
    if (!nln.negate() || !nln.add(1) || !nln.ln() || !nln.negate())
      return false;
    set(nln);
    setPrecisionAndApproximateFrom(o);
    return true;
  ...
  if (o.isZero()) {
    Number n0(*this);
    if (!n0.negate() || !n0.add(1) || !n0.recip() || !n0.add(-1))
      return false;
    set(n0);
    setPrecisionAndApproximateFrom(o);
    return true;
  ...
  if (o.isMinusOne()) {
    Number nr(*this);
    if (!nr.negate() || !nr.add(1) || !nr.square() || !nr.recip() ||
        !nr.multiply(*this))
      return false;
    set(nr);
    setPrecisionAndApproximateFrom(o);
  ...
  if (!o.isTwo() || !isLessThanOrEqualTo(1)) {
    if (isInterval(false)) {
      ...
      if (i_value && i_value->isInterval()) {
        ...
        if (isInterval(true)) {
          ...
          nr_l2.intervalToMidValue();
          nr_u2.intervalToMidValue();
          if (!nr_l.polylog(o) || !nr_u.polylog(o) || !nr_l2.polylog(o) ||
              !nr_u2.polylog(o))
            return false;
          nr_u.setInterval(nr_u, nr_u2);
          nr_l.setInterval(nr_l, nr_l2);
          ...
          nr_l.intervalToMidValue();
          nr_u.intervalToMidValue();
          if (!nr_l.polylog(o) || !nr_u.polylog(o))
            return false;
        }
      } else {
        nr_l.setInternal(fl_value);
      ...
      if (o.isNegative() || hasImaginaryPart() || o.hasImaginaryPart() ||
          (isInterval(false) && o.isInterval(false))) {
        if (precision(1) <= PRECISION + 20)
          CALCULATOR->error(
              false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
              _("%s() lacks proper support interval arithmetic."),
              CALCULATOR->getFunctionById(FUNCTION_ID_POLYLOG)->name().c_str(),
              NULL);
      }
      setPrecisionAndApproximateFrom(nr_l);
      setPrecisionAndApproximateFrom(nr_u);
    ...
    if (o.isInterval(false)) {
      ...
      if (o.hasImaginaryPart() && o.internalImaginary()->isInterval()) {
        ...
        if (o.isInterval(true)) {
          ...
          if (!nr_l.polylog(nr_lo) || !nr_u.polylog(nr_uo) ||
              !nr_l2.polylog(nr_lo2) || !nr_u2.polylog(nr_uo2))
            return false;
          nr_u.setInterval(nr_u, nr_u2);
          nr_l.setInterval(nr_l, nr_l2);
        } else {
        ...
        nr_uo.setInternal(o.internalUpperFloat());
        if (o.hasImaginaryPart())
          nr_lo.setImaginaryPart(*o.internalImaginary());
        if (o.hasImaginaryPart())
          nr_uo.setImaginaryPart(*o.internalImaginary());
        nr_lo.intervalToMidValue();
        nr_uo.intervalToMidValue();
        if (!nr_l.polylog(nr_lo) || !nr_u.polylog(nr_uo))
          return false;
    ...
    if (absx < 1)
      i = 1;
    else if (absx > 1 && o.isInteger() && o.isNegative())
      i = 2;
    Number nradd;
    if (i == 0) {
      if (o.isInteger() && o.isPositive()) {
        if (o == 2 && absx > 1) {
          ...
          nrpi.pi();
          nradd = *this;
          if (!nrpi.square() || !nrpi.divide(-6) || !nradd.negate() ||
              !nradd.ln() || !nradd.square() || !nradd.divide(-2) ||
              !nradd.add(nrpi))
            return false;
        } else if (o == 3 && absx > 1) {
          i = 2;
          ...
          nrpi.pi();
          Number nrlog(*this);
          if (!nrpi.square() || !nrpi.divide(-6) || !nrlog.negate() ||
              !nrlog.ln() || !nrpi.multiply(nrlog))
            return false;
          nradd = nrlog;
          if (!nradd.raise(3) || !nradd.divide(-6) || !nradd.add(nrpi))
            return false;
        }
      } else {
        Number nrgamma(o);
        nradd = *this;
        if (!nrgamma.negate() || !nrgamma.add(1) || !nrgamma.gamma() ||
            !nradd.ln() || !nradd.negate() || !nradd.raise(o - 1) ||
            !nradd.multiply(nrgamma))
          return false;
      }
    }
    CALCULATOR->beginTemporaryEnableIntervalArithmetic();
    ...
    for (int c = 0;; c++) {
      ...
      kpow = k;
      if (i == 0) {
        if (c > PRECISION * 1000 || CALCULATOR->aborted() ||
            !kfac.multiply(k) || !kpow.negate() || !kpow.add(o) ||
            !kpow.zeta() || !xpow.ln() || !xpow.raise(k) ||
            !xpow.multiply(kpow) || !xpow.divide(kfac) || !v.add(xpow) ||
            v.includesInfinity()) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          CALCULATOR->setPrecision(prec_bak);
          return false;
        ...
        kpow = k;
        if (c > PRECISION * 1000 || CALCULATOR->aborted() ||
            !kpow.raise(oneg) || !xpow.raise(k) || (i == 2 && !xpow.recip()) ||
            !xpow.multiply(kpow) || !v.add(xpow) || v.includesInfinity()) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          CALCULATOR->setPrecision(prec_bak);
          return false;
        }
      ...
      if ((ytest < wprec || (!ytest.isNonZero() && ytest < wprec2)) &&
        ...
        if (ytesti.isZero() && v.hasImaginaryPart()) {
          ytesti++;
          ytesti.setToFloatingPoint();
          mpfr_nextabove(ytesti.internalUpperFloat());
          ytesti--;
...
bool Number::igamma(const Number &o) {
  ...
  return false;
#else
  if (!o.isReal() || !isReal() || (!o.isNonZero() && !isNonZero()))
    return false;
  Number nr_bak(*this);
  if (!setToFloatingPoint())
    return false;
  Number o_float(o);
  if (!o_float.setToFloatingPoint())
    return false;
  mpfr_clear_flags();
  if (!CREATE_INTERVAL && !isInterval() && !o_float.isInterval()) {
    ...
    if (!o.isGreaterThanOrEqualTo(1) && !nr_bak.isGreaterThan(2) &&
        nr_bak.isInterval() && nr_bak.precision(1) <= PRECISION + 20)
      CALCULATOR->error(
          false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
          _("%s() lacks proper support interval arithmetic."),
          CALCULATOR->getFunctionById(FUNCTION_ID_I_GAMMA)->name().c_str(),
          NULL);
  }
  if (!testFloatResult()) {
    set(nr_bak);
...
bool Number::betainc(const Number &p, const Number &q, bool regularized) {
  if (isZero())
    return p.realPartIsPositive();
  if (!isNonZero() && !p.realPartIsPositive())
    return false;
  if (isOne() && q.realPartIsPositive()) {
    if (regularized)
      return true;
    Number gy(p), gx(q), gxy(p);
    if (!gxy.add(q) || !gy.gamma() || !gx.gamma() || !gxy.gamma() ||
        !gx.multiply(gy) || !gx.divide(gxy))
      return false;
    set(gx, true);
    return true;
  }
  if (p.isOne())
    return negate() && add(1) && raise(q) && negate() && add(1) &&
           (regularized || divide(q));
  if (q.isOne() && p.realPartIsPositive())
    return raise(p) && (regularized || divide(p));
  if (p.isNonPositive() && p.isInteger() &&
      (q.isNonPositive() || q.isNonInteger() || q > -p)) {
    set(1, 1, 0, true);
    return true;
  }
  if (q.isNonPositive() && q.isInteger()) {
    clear(true);
    return true;
  }
  if (!isReal() || !p.isReal() || !q.isReal())
    return false;
  if (p.isInteger() && q.isInteger() && p.isPositive() && q.isPositive()) {
    if (isInterval() && isLessThan(1) && isGreaterThan(-1)) {
      ...
      if (!nr_low.betainc(p, q, regularized) ||
          !nr_high.betainc(p, q, regularized))
        return false;
    ...
    while (i <= n) {
      ...
      x_i = *this;
      x_m1_i = x_m1;
      if (!v_i.binomial(n, i) || !x_i.raise(i) || !x_m1_i.raise(n - i) ||
          !v_i.multiply(x_i) || !v_i.multiply(x_m1_i))
        return false;
      v += v_i;
      i++;
    }
    if (!regularized) {
      Number gy(p), gx(q), gxy(p);
      if (!gxy.add(q) || !gy.gamma() || !gx.gamma() || !gxy.gamma() ||
          !v.multiply(gx) || !v.multiply(gy) || !v.divide(gxy))
        return false;
    }
    set(v);
    return true;
  }
  if (isNonNegative() && isFraction() && !p.isInterval() &&
      (!p.isInteger() || p.isPositive()) && !q.isInterval()) {
    if (isInterval()) {
      Number nr_low(lowerEndPoint()), nr_high(upperEndPoint());
      if (!nr_low.betainc(p, q, regularized) ||
          !nr_high.betainc(p, q, regularized))
  ...
  betainc_begin:
    if (regularized) {
      Number gy(q), gxy(p);
      nr_mul.set(p);
      if (!gxy.add(q) || !gy.gamma() || !nr_mul.gamma() || !gxy.gamma() ||
          !nr_mul.multiply(gy) || !nr_mul.divide(gxy) || !nr_mul.recip())
        return false;
    }
    Number nr_add;
    Number x(*this);
    if (isGreaterThan(nr_half) && (!q.isInteger() || q.isPositive())) {
        ...
        if (w_pow.raise(q)) {
          ...
          if (twopowq.raise(q)) {
            ...
            while (true) {
              ...
              term_i = num_p;
              w_mul.set(1, 1, 0);
              if (!term_i.multiply(nr_mul) || !w_mul.subtract(w_pow) ||
                  !term_i.multiply(w_mul) || !term_i.divide(div_q) ||
                  !term_i.divide(div_fac) || !term_i.divide(twopowq) ||
                  !term.add(term_i))
                break;
              if (!term_prev.isZero()) {
                term_prev.recip();
              ...
              if (prec >= 0 && prec < precbak && PRECISION * 5 < 5000) {
                CALCULATOR->setPrecision(
                    PRECISION * 10 > 5000 ? 5000 : PRECISION * 10);
                CALCULATOR->endTemporaryStopMessages();
                goto betainc_begin;
              }
              div_q++;
              nr_pmul = i;
              if (!w_pow.multiply(w) || !nr_pmul.subtract(p) ||
                  !num_p.multiply(nr_pmul) || !div_fac.multiply(i) ||
                  !twopowq.multiply(2))
                break;
              i++;
            }
      ...
      if (!b_success) {
        if (precbak != PRECISION)
          CALCULATOR->setPrecision(precbak);
        return false;
      }
    }
    bool b_success = false;
  betainc_begin2:
    ...
    Number num_q(1, 1, 0);
    Number div_fac(1, 1, 0);
    if (nr.raise(p) && nr_mul.multiply(nr) && term.multiply(nr_mul) &&
        term.divide(div_p)) {
      div_p++;
      num_q -= q;
      Number nr_qmul;
      ...
      while (true) {
        ...
        term_prev = term;
        term_i = num_q;
        if (!term_i.multiply(nr_mul) || !term_i.divide(div_p) ||
            !term_i.divide(div_fac) || !term_i.multiply(x_pow) ||
            !term.add(term_i))
          break;
        term_prev.recip();
        term_prev *= term_i;
        ...
        int prec = term.precision(true);
        if (prec >= 0 && prec < precbak && PRECISION * 5 < 5000) {
          CALCULATOR->setPrecision(PRECISION * 10 > 5000 ? 5000
                                                         : PRECISION * 10);
          if (regularized) {
            Number gy(q), gxy(p);
            nr_mul.set(p);
        ...
        div_p++;
        nr_qmul = i;
        if (!nr_qmul.subtract(q) || !num_q.multiply(nr_qmul) ||
            !div_fac.multiply(i) || !x_pow.multiply(x))
          break;
      }
    }
    if (precbak != PRECISION)
...
bool Number::fresnels() {
  ...
  if (!isLessThanOrEqualTo(6) || !isGreaterThanOrEqualTo(-6))
    return false;
  if (isInterval()) {
    Number nr_l, nr_u;
    nr_l.setInternal(fl_value);
    nr_u.setInternal(fu_value);
    if (!nr_l.fresnels() || !nr_u.fresnels())
      return false;
    mpfr_t nsqrt;
  ...
  mpfr_inits2(BIT_PRECISION * 2, v, pipow, xpow, x, num, den, yprev, wprec,
              NULL);
  if (n_type == NUMBER_TYPE_FLOAT) {
    mpfr_set(x, fl_value, MPFR_RNDN);
  } else {
...
bool Number::fresnelc() {
  ...
  if (isInterval()) {
    ...
    nr_l.setInternal(fl_value);
    nr_u.setInternal(fu_value);
    if (!nr_l.fresnelc() || !nr_u.fresnelc())
      return false;
    mpfr_t nsqrt;
    mpfr_init2(nsqrt, BIT_PRECISION);
  ...
  unsigned long int k = 0;
  mpfr_t pipow, xpow, x, num, den, yprev, wprec, v;
  mpfr_inits2(BIT_PRECISION * 2, pipow, xpow, x, num, den, yprev, wprec, v,
              NULL);
  if (n_type == NUMBER_TYPE_FLOAT) {
    mpfr_set(x, fl_value, MPFR_RNDN);
  } else {
...
bool Number::expint() {
  ...
  if (hasImaginaryPart()) {
    if (i_value->includesInfinity() && includesInfinity(true))
      return false;
    if (i_value->isPlusInfinity()) {
      i_value->pi();
      setPrecisionAndApproximateFrom(*i_value);
      return true;
    }
    if (i_value->isMinusInfinity()) {
      if (includesInfinity(true))
        return false;
      i_value->pi();
      i_value->negate();
      setPrecisionAndApproximateFrom(*i_value);
    ...
    if (isMinusInfinity(true)) {
      clear();
      return true;
    }
    if (isPlusInfinity(true)) {
      clearImaginary();
      return true;
    }
    if (includesInfinity())
    ...
    if (isInterval(false)) {
      ...
      if (i_value->isInterval()) {
        ...
        if (isInterval(true)) {
          ...
          nr_l2.intervalToMidValue();
          nr_u2.intervalToMidValue();
          if (!nr_l.expint() || !nr_u.expint() || !nr_l2.expint() ||
              !nr_u2.expint())
            return false;
          nr_u.setInterval(nr_u, nr_u2);
          nr_l.setInterval(nr_l, nr_l2);
      ...
      if (precision(1) <= PRECISION + 20)
        CALCULATOR->error(
            false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
            _("%s() lacks proper support interval arithmetic."),
            CALCULATOR->getFunctionById(FUNCTION_ID_EXPINT)->name().c_str(),
            NULL);
      setPrecisionAndApproximateFrom(nr_l);
      setPrecisionAndApproximateFrom(nr_u);
      return setInterval(nr_l, nr_u, true);
    ...
    if (!nr_lnr.recip() || !nr_lnr.ln() || !nr_ln.ln() || !nr_ln.divide(2) ||
        !nr_lnr.divide(-2))
      return false;
    CALCULATOR->beginTemporaryEnableIntervalArithmetic();
    if (!CALCULATOR->usesIntervalArithmetic()) {
    ...
    for (int i = 0;; i++) {
      xpow = x;
      if (i > PRECISION * 100 || CALCULATOR->aborted() || !kfac.multiply(k) ||
          !xpow.raise(k) || !xpow.divide(kfac) || !xpow.divide(k) ||
          !v.add(xpow) || v.includesInfinity()) {
        CALCULATOR->endTemporaryEnableIntervalArithmetic();
        CALCULATOR->setPrecision(prec_bak);
        return false;
      }
    ...
    if (!v.testFloatResult(true))
      return false;
    if (!v.add(nr_euler) || !v.add(nr_ln) || !v.add(nr_lnr))
      return false;
    set(v);
    b_approx = true;
    return true;
...
bool Number::sinint() {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      if (isInterval(false)) {
        ...
        if (i_value->isInterval()) {
          ...
          if (isInterval(true)) {
            ...
            nr_l2.intervalToMidValue();
            nr_u2.intervalToMidValue();
            if (!nr_l.sinint() || !nr_u.sinint() || !nr_l2.sinint() ||
                !nr_u2.sinint())
              return false;
            nr_u.setInterval(nr_u, nr_u2);
            nr_l.setInterval(nr_l, nr_l2);
        ...
        if (precision(1) <= PRECISION + 20)
          CALCULATOR->error(
              false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
              _("%s() lacks proper support interval arithmetic."),
              CALCULATOR->getFunctionById(FUNCTION_ID_SININT)->name().c_str(),
              NULL);
        setPrecisionAndApproximateFrom(nr_l);
        setPrecisionAndApproximateFrom(nr_u);
        return setInterval(nr_l, nr_u, true);
      ...
      for (int i = 0;; i++) {
        ...
        xpow *= 2;
        num = x;
        if (i > PRECISION * 100 || CALCULATOR->aborted() ||
            !kfac.multiply(k2) || !k2.add(1) || !kfac.multiply(k2) ||
            !den.square() || !den.multiply(kfac) || !num.raise(xpow) ||
            !num.divide(den) || (k.isOdd() && !num.negate()) || !v.add(num) ||
            v.includesInfinity()) {
          CALCULATOR->setPrecision(prec_bak);
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
  ...
  if (isInterval()) {
    ...
    if (mpfr_cmp_si(fl_value, 1) > 0) {
      ...
      mpfr_inits2(mpfr_get_prec(fl_value) + mpfr_get_exp(fl_value), f_pi,
                  f_mid1, f_mid2, NULL);
      mpfr_const_pi(f_pi, MPFR_RNDN);
      ...
      for (size_t i2 = 0; b && ((i2 == 0 && b1) || (i2 == 1 && b2)); i2++) {
        ...
        if (nr_round > 100 + PRECISION * 5) {
          ...
          mpfr_inits2(mpfr_get_prec(i2 == 0 ? f_mid1 : f_mid2), f_x, f_xi, f_y,
                      NULL);
          mpfr_set(f_x, i2 == 0 ? f_mid1 : f_mid2, MPFR_RNDN);
          mpfr_inits2(mpfr_get_prec(f_x), f_pi, f_sin, f_cos, f_y1, f_y2, f_yt,
                      NULL);
          mpfr_const_pi(f_pi, MPFR_RNDN);
          ...
          for (size_t i = 0; i < 100; i++) {
            ...
            if (i % 2 == 0)
              mpfr_sub(f_y1, f_y1, f_xi, MPFR_RNDN);
            else
              mpfr_add(f_y1, f_y1, f_xi, MPFR_RNDN);
            mpz_add_ui(z_i, z_i, 1);
            mpz_mul(z_fac, z_fac, z_i);
            mpfr_set(f_xi, f_x, MPFR_RNDN);
            mpfr_pow_z(f_xi, f_xi, z_i, MPFR_RNDN);
            ...
            mpfr_mul_z(f_xi, f_xi, z_fac, MPFR_RNDN);
            if (i % 2 == 0)
              mpfr_sub(f_y2, f_y2, f_xi, MPFR_RNDN);
            else
              mpfr_add(f_y2, f_y2, f_xi, MPFR_RNDN);
            ...
            if (i > 0 && mpfr_equal_p(fu_value, fl_value)) {
              b = true;
              break;
            }
          ...
          for (size_t i = 0; i < 10000; i++) {
            ...
            if (i % 2 == 0)
              mpfr_sub(f_y, f_y, f_xi, MPFR_RNDN);
            else
    ...
    setInterval(nr_lower, nr_upper);
    if (b1)
      setInterval(*this, nr_mid1);
    if (b2)
      setInterval(*this, nr_mid2);
    return true;
  }
  mpfr_clear_flags();
  mpfr_t f_x, f_xi, f_y;
  ...
  if (nr_round > 100 + PRECISION * 5) {
    ...
    mpfr_inits2(mpfr_get_prec(fl_value) + nr_round.intValue() * 5, f_x, f_xi,
                f_y, NULL);
    mpfr_set(f_x, fl_value, MPFR_RNDN);
  ...
  if (CREATE_INTERVAL)
    mpfr_set(fu_value, f_y, MPFR_RNDU);
  else
    mpfr_set(fu_value, fl_value, MPFR_RNDN);
  mpfr_clears(f_x, f_xi, f_y, NULL);
  mpz_clears(z_i, z_fac, NULL);
...
bool Number::sinhint() {
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      Number nr_bak(*this);
      if (!multiply(nr_one_i) || !sinint() || !multiply(nr_minus_i)) {
        set(nr_bak);
        return false;
      }
      return true;
  ...
  Number nr_bak(*this);
  if (isNegative()) {
    if (!negate() || !sinhint() || !negate()) {
      set(nr_bak);
      return false;
    }
    return true;
  }
  if (isGreaterThan(1000))
    return false;
  if (!setToFloatingPoint())
    return false;
  if (isInterval()) {
  ...
  Number nr_round(*this);
  nr_round.round();
  mpfr_inits2(mpfr_get_prec(fl_value) + nr_round.intValue(), f_x, f_xi, f_y,
              NULL);
  mpfr_set(f_x, fl_value, MPFR_RNDN);
  mpfr_set(f_y, fl_value, MPFR_RNDN);
  mpz_set_ui(z_i, 1);
  ...
  if (hasImaginaryPart()) {
    if (hasRealPart()) {
      ...
      if (isInterval(false)) {
        ...
        if (i_value->isInterval()) {
          ...
          if (isInterval(true)) {
            ...
            nr_l2.intervalToMidValue();
            nr_u2.intervalToMidValue();
            if (!nr_l.cosint() || !nr_u.cosint() || !nr_l2.cosint() ||
                !nr_u2.cosint())
              return false;
            nr_u.setInterval(nr_u, nr_u2);
            nr_l.setInterval(nr_l, nr_l2);
        ...
        if (precision(1) <= PRECISION + 20)
          CALCULATOR->error(
              false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
              _("%s() lacks proper support interval arithmetic."),
              CALCULATOR->getFunctionById(FUNCTION_ID_COSINT)->name().c_str(),
              NULL);
        setPrecisionAndApproximateFrom(nr_l);
        setPrecisionAndApproximateFrom(nr_u);
        return setInterval(nr_l, nr_u, true);
      ...
      for (int i = 0;; i++) {
        ...
        if (i > PRECISION * 100 || CALCULATOR->aborted() ||
            !kfac.multiply(k2) || !k2.add(1) || !kfac.multiply(k2) ||
            !den.multiply(kfac) || !num.raise(xpow) || !num.divide(den) ||
            (k.isOdd() && !num.negate()) || !v.add(num) ||
            v.includesInfinity()) {
          CALCULATOR->setPrecision(prec_bak);
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          return false;
        ...
        if ((ytest < wprec || (!ytest.isNonZero() && ytest < wprec2)) &&
          ...
          Number nr_euler;
          nr_euler.euler();
          if (!x.ln() || !v.divide(2) || !v.add(x) || !v.add(nr_euler))
            return false;
          break;
        }
        k++;
      ...
      if (isInterval(false)) {
        ...
        if (i_value->isInterval()) {
          ...
          if (isInterval(true)) {
            ...
            nr_l2.intervalToMidValue();
            nr_u2.intervalToMidValue();
            if (!nr_l.coshint() || !nr_u.coshint() || !nr_l2.coshint() ||
                !nr_u2.coshint())
              return false;
            nr_u.setInterval(nr_u, nr_u2);
            nr_l.setInterval(nr_l, nr_l2);
        ...
        if (precision(1) <= PRECISION + 20)
          CALCULATOR->error(
              false, MESSAGE_CATEGORY_NO_PROPER_INTERVAL_SUPPORT,
              _("%s() lacks proper support interval arithmetic."),
              CALCULATOR->getFunctionById(FUNCTION_ID_COSHINT)->name().c_str(),
              NULL);
        setPrecisionAndApproximateFrom(nr_l);
        setPrecisionAndApproximateFrom(nr_u);
        return setInterval(nr_l, nr_u, true);
...
bool recfact(mpz_ptr ret, long int start, long int n) {
  ...
  if (n <= 16) {
    ...
    for (i = start + 1; i < start + n; i++)
      mpz_mul_si(ret, ret, i);
    return true;
  }
  if (CALCULATOR->aborted())
    return false;
  i = n / 2;
  if (!recfact(ret, start, i))
    return false;
  mpz_t retmul;
  mpz_init(retmul);
  if (!recfact(retmul, start + i, n - i))
    return false;
...
bool recfact2(mpz_ptr ret, long int start, long int n) {
  ...
  if (n % 2 != i % 2)
    i--;
  if (!recfact2(ret, start, i))
    return false;
  mpz_t retmul;
  mpz_init(retmul);
  if (!recfact2(retmul, start + i, n - i))
    return false;
  mpz_mul(ret, ret, retmul);
  mpz_clear(retmul);
  return true;
...
bool recfactm(mpz_ptr ret, long int start, long int n, long int m) {
  ...
  i = n / 2;
  i -= ((i % m) - (n % m));
  if (!recfactm(ret, start, i, m))
    return false;
  mpz_t retmul;
  mpz_init(retmul);
  if (!recfactm(retmul, start + i, n - i, m))
    return false;
  mpz_mul(ret, ret, retmul);
  mpz_clear(retmul);
  return true;
...
bool Number::factorial() {
  ...
  if (!mpz_fits_slong_p(mpq_numref(r_value)))
    return false;
...
bool Number::multiFactorial(const Number &o) {
  ...
  if (!mpz_fits_slong_p(mpq_numref(r_value)) ||
      !mpz_fits_slong_p(mpq_numref(o.internalRational())))
    return false;
  long int n = mpz_get_si(mpq_numref(r_value));
...
bool Number::binomial(const Number &m, const Number &k) {
  if (!m.isInteger() || !k.isInteger())
    return false;
  if (m.isNegative()) {
    if (k.isNegative())
      return false;
    Number m2(m);
    if (!m2.negate() || !m2.add(k) || !m2.add(nr_minus_one) || !binomial(m2, k))
      return false;
    if (k.isOdd())
      negate();
    return true;
  ...
  if (!mpz_fits_ulong_p(mpq_numref(k.internalRational())))
    return false;
  clear();
  mpz_bin_ui(mpq_numref(r_value), mpq_numref(m.internalRational()),
             mpz_get_ui(mpq_numref(k.internalRational())));
  return true;
}
...
bool Number::factorize(vector<Number> &factors) {
  ...
  mpz_sqrt(facmax, inr);
  for (size_t prime_index = 0; prime_index < NR_OF_PRIMES &&
                               mpz_cmp_si(facmax, PRIMES[prime_index]) >= 0;
       prime_index++) {
    bool b = false;
  ...
  if (mpz_cmp_si(inr, 1) > 0) {
    ...
    while (mpz_cmp(facmax, last_prime) >= 0) {
      ...
      while (mpz_divisible_p(inr, last_prime)) {
        ...
        Number fac;
        fac.setInternal((__mpz_struct *const)last_prime);
        factors.push_back(fac);
        b = true;
      }
      if (b)
  ...
  if (mpz_cmp_si(inr, 1) > 0) {
    Number fac;
    fac.setInternal((__mpz_struct *const)inr);
    factors.push_back(fac);
  }
  mpz_clears(inr, last_prime, facmax, NULL);
  return true;
...
void Number::intRand(const Number &ceil) {
  clear();
  if (!ceil.isInteger() || !ceil.isPositive())
    return;
  mpz_urandomm(mpq_numref(r_value), randstate,
               mpq_numref(ceil.internalRational()));
}
...
bool Number::add(const Number &o, MathOperation op) {
  ...
  case OPERATION_LOGICAL_OR: {
    ...
    if (i1 >= COMPARISON_RESULT_UNKNOWN ||
        i1 == COMPARISON_RESULT_EQUAL_OR_LESS ||
        i1 == COMPARISON_RESULT_EQUAL_OR_GREATER)
      i1 = COMPARISON_RESULT_UNKNOWN;
    if (i2 >= COMPARISON_RESULT_UNKNOWN ||
        i2 == COMPARISON_RESULT_EQUAL_OR_LESS ||
        i2 == COMPARISON_RESULT_EQUAL_OR_GREATER)
      i2 = COMPARISON_RESULT_UNKNOWN;
    if (i1 >= COMPARISON_RESULT_UNKNOWN && !COMPARISON_IS_NOT_EQUAL(i2))
      return false;
    if (i2 >= COMPARISON_RESULT_UNKNOWN && !COMPARISON_IS_NOT_EQUAL(i1))
      return false;
  ...
  case OPERATION_LOGICAL_XOR: {
    ...
    if (i2 >= COMPARISON_RESULT_UNKNOWN ||
        ...
        i2 == COMPARISON_RESULT_EQUAL_OR_GREATER)
      return false;
    if (COMPARISON_IS_NOT_EQUAL(i1))
      setTrue(i2 == COMPARISON_RESULT_EQUAL);
    else
      setTrue(COMPARISON_IS_NOT_EQUAL(i2));
    return true;
  }
  case OPERATION_LOGICAL_AND: {
    ...
    if (i2 >= COMPARISON_RESULT_UNKNOWN ||
      ...
      i2 = COMPARISON_RESULT_UNKNOWN;
    if (i1 >= COMPARISON_RESULT_UNKNOWN &&
        (i2 == COMPARISON_RESULT_UNKNOWN || COMPARISON_IS_NOT_EQUAL(i2)))
      return false;
    if (i2 >= COMPARISON_RESULT_UNKNOWN && COMPARISON_IS_NOT_EQUAL(i1))
      return false;
    setTrue(COMPARISON_IS_NOT_EQUAL(i1) && COMPARISON_IS_NOT_EQUAL(i2));
  ...
  case OPERATION_EQUALS: {
    ...
    if (i >= COMPARISON_RESULT_UNKNOWN ||
  ...
  case OPERATION_NOT_EQUALS: {
    ...
    if (i >= COMPARISON_RESULT_UNKNOWN ||
        ...
        i == COMPARISON_RESULT_EQUAL_OR_LESS)
      return false;
    setTrue(i == COMPARISON_RESULT_NOT_EQUAL ||
            i == COMPARISON_RESULT_GREATER || i == COMPARISON_RESULT_LESS);
    return true;
  }
  }
  return false;
}
string Number::printNumerator(int base, bool display_sign,
                              BaseDisplay base_display, bool lower_case) const {
  return format_number_string(
      printMPZ(mpq_numref(r_value), base, false, lower_case), base,
      base_display, display_sign);
}
string Number::printDenominator(int base, bool display_sign,
                                BaseDisplay base_display,
                                bool lower_case) const {
  return format_number_string(
      printMPZ(mpq_denref(r_value), base, false, lower_case), base,
      base_display, display_sign);
}
string Number::printImaginaryNumerator(int base, bool display_sign,
                                       BaseDisplay base_display,
                                       bool lower_case) const {
  return format_number_string(
      printMPZ(mpq_numref(i_value ? i_value->internalRational()
                                  : nr_zero.internalRational()),
               base, false, lower_case),
      base, base_display, display_sign);
}
string Number::printImaginaryDenominator(int base, bool display_sign,
                                         BaseDisplay base_display,
                                         bool lower_case) const {
  return format_number_string(
      printMPZ(mpq_denref(i_value ? i_value->internalRational()
                                  : nr_zero.internalRational()),
               base, false, lower_case),
      base, base_display, display_sign);
}
...
int from_float(Number &nr, string sbin, unsigned int bits,
               unsigned int expbits) {
  return from_float(nr, sbin, bits, expbits, 0);
}
int from_float(Number &nr, string sbin, unsigned int bits, unsigned int expbits,
               unsigned int sgnpos) {
  if (expbits == 0)
    expbits = standard_expbits(bits);
  else if (expbits > bits - 2)
    return 0;
  if (sgnpos >= bits)
    return 0;
  if (sbin.length() < bits)
    sbin.insert(0, bits - sbin.length(), '0');
  if (sbin.length() > bits) {
    CALCULATOR->error(
        true,
        _("The value is too high for the number of floating point bits (%s)."),
        i2s(bits).c_str(), NULL);
    return 0;
  }
  if (sgnpos > 0) {
  ...
  if (b_spec) {
    if ((bits == 80 && sbin.rfind("1") == expbits + 1) ||
        (bits != 80 && sbin.rfind("1") < expbits + 1)) {
      if (b_neg)
        nr.setMinusInfinity();
      else
        nr.setPlusInfinity();
...
string to_float(Number nr_pre, unsigned int bits, unsigned int expbits,
                bool *approx) {
  return to_float(nr_pre, bits, expbits, 0, approx);
}
string to_float(Number nr_pre, unsigned int bits, unsigned int expbits,
                unsigned int sgnpos, bool *approx) {
  if (expbits == 0)
    expbits = standard_expbits(bits);
  else if (expbits > bits - 2)
    return "";
  ...
  if (nr.isPlusInfinity() || nr.isZero() || nr.hasImaginaryPart()) {
    ...
    if (bits == 80 && nr.isPlusInfinity())
      sbin += '1';
    else
      sbin += '0';
    for (size_t i = expbits + 2; i < bits; i++)
      sbin += '0';
    if (nr.hasImaginaryPart())
      sbin[sbin.length() - 1] = '1';
  } else {
    Number nrexp(nr);
    nrexp.log(2);
  ...
  tofloat_afterexp:
    if (nrexp > expbias) {
      ...
      remove_blanks(sbin);
      if (sbin.length() < expbits + 1)
        sbin.insert(1, expbits + 1 - sbin.length(), '0');
      if (bits == 80)
        sbin += sfrac[0];
      sbin += sfrac.substr(2);
      if (sbin.length() < bits)
        sbin.append(bits - sbin.length(), '0');
    }
  }
  if (sgnpos > 0) {
...
void add_base_exponent(string &str, long int expo, int base,
                       const PrintOptions &po, const InternalPrintStruct &ips,
                       int type = 0) {
  if (expo == 0)
    return;
  if (ips.iexp && type != 1)
...
string Number::print(const PrintOptions &po,
  ...
  // reset InternalPrintStruct (used for separate handling sign, scientific
  // notation, numerator/denominator, imaginary/real parts, etc)
  if (ips.minus)
    *ips.minus = false;
  if (ips.exp_minus)
    *ips.exp_minus = false;
  ...
  if (ips.iexp)
    *ips.iexp = 0;
  if (po.is_approximate && isApproximate())
    *po.is_approximate = true;
  if (po.number_fraction_format >= FRACTION_FRACTIONAL_FIXED_DENOMINATOR &&
      ((po.number_fraction_format == FRACTION_COMBINED_FIXED_DENOMINATOR &&
        !isInteger()) ||
       po.number_fraction_format == FRACTION_FRACTIONAL_FIXED_DENOMINATOR ||
       po.number_fraction_format == FRACTION_PERCENT ||
       po.number_fraction_format == FRACTION_PERMILLE ||
       po.number_fraction_format == FRACTION_PERMYRIAD) &&
      !hasImaginaryPart() && po.base > BASE_FP16 &&
      !BASE_IS_SEXAGESIMAL(po.base) && po.base != BASE_TIME) {
    PrintOptions po2 = po;
    ...
    if (po.number_fraction_format == FRACTION_PERCENT) {
      ...
      if (!num.multiply(1000))
        return print(po2, ips);
      return num.print(po2, ips) + "‰";
    } else if (po.number_fraction_format == FRACTION_PERMYRIAD) {
      if (!num.multiply(10000))
        return print(po2, ips);
      return num.print(po2, ips) + "‱";
    }
    if (!num.multiply(CALCULATOR->fixedDenominator()))
      return print(po2, ips);
    if (!num.isInteger()) {
      if (TRUNCATE)
        num.trunc();
      else
        num.round(po.round_halfway_to_even);
      if (!num.isInteger()) {
        num.set(*this);
  ...
  if (po.base == BASE_FP16 || po.base == BASE_FP128 || po.base == BASE_FP32 ||
      po.base == BASE_FP64 || po.base == BASE_FP80) {
    unsigned int bits = 0;
    switch (po.base) {
    case BASE_FP16: {
      bits = 16;
    ...
    if (ips.minus)
      *ips.minus = false;
    str = format_number_string(str, BASE_BINARY, po.base_display, false, true,
                               po);
    if (ips.num)
      *ips.num = str;
    return str;
  ...
  if (po.base == BASE_BINARY_DECIMAL && isReal()) {
    ...
    for (size_t i = 0; i < str10.length(); i++) {
      ...
      case '.': {
        po2.binary_bits = str.length();
        str += po.decimalpoint();
        break;
      }
      }
    }
    if (po2.binary_bits == 0)
      po2.binary_bits = str.length();
    if (ips.minus)
      *ips.minus = neg;
    str = format_number_string(str, 2, po.base_display, !ips.minus && neg, true,
                               po2);
    if (ips.num)
      *ips.num = str;
    return str;
  ...
  if (((po.base < BASE_CUSTOM && po.base != BASE_BIJECTIVE_26) ||
       (po.base == BASE_CUSTOM &&
        (!CALCULATOR->customOutputBase().isInteger() ||
         CALCULATOR->customOutputBase() > 62 ||
         CALCULATOR->customOutputBase() < 2))) &&
      isReal()) {
    ...
    default: {
      base = CALCULATOR->customOutputBase();
    }
    }
    // negative non-integer bases, complex bases, and bases where absolute value
    // is <= 1
    if (!base.isReal() || (base.isNegative() && !base.isInteger()) ||
        !(base > 1 || base < -1)) {
    ...
    // use mid value for low precision intervals and any interval with negative
    // bases
    if ((base.isNegative() && isInterval()) ||
        (isInterval() && precision(true) < 1)) {
      Number nr(*this);
    ...
    if (po.is_approximate && base.isApproximate())
      *po.is_approximate = true;
    long int precision = PRECISION;
    // adjust output precision if precision of number or base is lower than
    // global precision
    if (b_approx && i_precision >= 0 && i_precision < precision)
      precision = i_precision;
    if (base.isApproximate() && base.precision() >= 0 &&
        base.precision() < precision)
      precision = base.precision();
    // adjust output precision to precision of parent MathStructure
    if (po.restrict_to_parent_precision && ips.parent_precision >= 0 &&
        ips.parent_precision < precision)
      precision = ips.parent_precision;

    // calculate number of digits allowed for the number base with the current
    // precision: floor(log(10^precision-1, abs(base)))
    ...
    if (base.isNegative()) {
      ...
      nr_log++;
      if (nr_log < precision_base &&
          (!po.use_max_decimals || po.max_decimals != 0)) {
        nr_log.subtract(precision_base);
        nr_log.negate();
        b_dp = true;
        if (po.use_max_decimals && po.max_decimals >= 0 &&
            nr_log > po.max_decimals)
          nr_log = po.max_decimals;
      ...
      while (true) {
        ...
        if (nr.isZero()) {
          ...
          if (TRUNCATE)
            break;
          nr_frac *= 2;
          if (nr_frac.isGreaterThan(1) ||
              (nr_frac.isOne() && (!po.round_halfway_to_even ||
                                   digits[digits.size() - 1] % 2 == 1))) {
      ...
      if (b_dp && (!po.show_ending_zeroes ||
                   ((exact || base.isApproximate()) && !isApproximate()))) {
      ...
      while (true) {
        ...
        bool do_break =
            (b_dp && ((long int)digits.size() == precision_base ||
                      (po.use_max_decimals && po.max_decimals >= 0 &&
                       po.max_decimals == (long int)digits.size() - i_dp)));
        if (!b_dp && nr_log.isNegative()) {
          if ((long int)digits.size() >= precision_base ||
              (po.use_max_decimals && po.max_decimals == 0)) {
            do_break = true;
          } else {
            i_dp = digits.size();
        ...
        if (do_break) {
          ...
          if (nr.isGreaterThan(base_pow) ||
              (nr == base_pow && (!po.round_halfway_to_even ||
                                  digits[digits.size() - 1] % 2 == 1))) {
            size_t i = digits.size();
            while (i > 0) {
              i--;
      ...
      if (po.show_ending_zeroes &&
          (isApproximate() || (!exact && !base.isApproximate())) &&
          digits.size() != 1 && (long int)digits.size() < precision_base) {
        if (digits.empty())
          digits.push_back(0L);
        if (!b_dp) {
          b_dp = true;
    ...
    for (size_t index = 0; index < digits.size(); index++) {
      ...
      if (b_num) {
        if (!b_uni || c <= 32 || (!po.use_unicode_signs && c > 0x7f) ||
            c >= 1114112L) {
          str += '\\';
          str += i2s(c);
          prev_esc = true;
          ...
          if (c == '\\' && index < digits.size() - 1 &&
              ((digits[index + 1] >= '0' && digits[index + 1] <= '9') ||
               (digits[index + 1] == 'x' && index < digits.size() - 2 &&
                ((digits[index + 2] >= '0' && digits[index + 2] <= '9') ||
                 (digits[index + 2] >= 'A' && digits[index + 2] <= 'F') ||
                 (digits[index + 2] >= 'a' && digits[index + 2] <= 'f')))))
            str += '\\';
          str += (char)c;
          prev_esc = false;
  ...
  if ((BASE_IS_SEXAGESIMAL(po.base) || po.base == BASE_TIME) && isReal()) {
    ...
    string str3;
    if (po.base == BASE_SEXAGESIMAL_2 || po.base == BASE_LATITUDE_2 ||
        po.base == BASE_LONGITUDE_2) {
      nr2 *= 60;
    } else {
      ...
      if (!nr3.isInterval() &&
          (!nr3.isZero() || BASE_IS_SEXAGESIMAL(po.base))) {
        str3 = nr3.print(po2);
        // if 3rd section is rounded to 60, set to zero and increment 2nd
        // section
    ...
    if (po.base != BASE_TIME) {
      if (po.use_unicode_signs &&
          (!po.can_display_unicode_string_function ||
           (*po.can_display_unicode_string_function)(
               SIGN_DEGREE, po.can_display_unicode_string_arg))) {
        str += SIGN_DEGREE;
      } else {
        str += "o";
      }
    ...
    if (po.base == BASE_TIME)
      str += ":";
    if ((po.base == BASE_TIME || po.base == BASE_LATITUDE ||
         po.base == BASE_LONGITUDE) &&
        nr2.isLessThan(10)) {
  ...
  if (po.use_min_decimals && po.min_decimals > 0)
    min_decimals = po.min_decimals;
  // min decimals is greater than max decimals
  if ((int)min_decimals > po.max_decimals && po.use_max_decimals &&
      po.max_decimals >= 0) {
    min_decimals = po.max_decimals;
  }

  if (po.base == BASE_CUSTOM)
    base = CALCULATOR->customOutputBase().intValue();
  else if (po.base <= 1 && po.base != BASE_ROMAN_NUMERALS &&
           po.base != BASE_TIME)
    base = 10;
  else if (po.base > 36 && !BASE_IS_SEXAGESIMAL(po.base))
    base = 36;
  else
    base = po.base;

  if (po.base == BASE_ROMAN_NUMERALS) {
    // do not display roman numerals for non-rational numbers and numbers with
    // absolute value > 9999; use decimal base instead
    if (!isRational()) {
      CALCULATOR->error(
          false, _("Can only display rational numbers as roman numerals."),
          NULL);
      base = 10;
    } else if (mpz_cmpabs_ui(mpq_numref(r_value), 9999) > 0 ||
               mpz_cmp_ui(mpq_denref(r_value), 9999) > 0) {
      CALCULATOR->error(false,
                        _("Cannot display numbers greater than 9999 or less "
                          "than -9999 as roman numerals."),
                        NULL);
  ...
  if (hasImaginaryPart()) {
    // imaginary unit
    string str_i =
        (CALCULATOR ? CALCULATOR->getVariableById(VARIABLE_ID_I)
    ...
    if (bre) {
      // output real and imaginary part separately (they are normally already
      // separeted in the parent MathStructure)
      Number r_nr(*this);
      r_nr.clearImaginary();
        ...
        if (po.use_unicode_signs &&
          ...
          str2 += SIGN_MIDDLEDOT;
                 ...
                 (!po.can_display_unicode_string_function ||
                  (*po.can_display_unicode_string_function)(
                      SIGN_MULTIPLICATION, po.can_display_unicode_string_arg)))
          str2 += SIGN_MULTIPLICATION;
        else
          str2 += "*";
        if (po.spacious)
          str2 += " ";
      }
      // do not show 1 (i instead of 1i); 'i' is placed after the number, while
      // 'j' is placed before
      if (str2 == "1" && po.base != BASE_UNICODE)
        str2 = str_i;
      else if (str_i == "j")
        str2.insert(0, str_i);
      else
  ...
  // adjust output precision if precision of the number is lower than global
  // precision
  if (b_approx && i_precision >= 0 &&
      (po.preserve_precision || po.preserve_format || i_precision < PRECISION))
    precision = i_precision;
  // if preserve_precision is true, use full precision
  else if (b_approx && i_precision < 0 && po.preserve_precision &&
           FROM_BIT_PRECISION(NUMBER_BIT_PRECISION) > precision)
    precision = FROM_BIT_PRECISION(NUMBER_BIT_PRECISION);
  // if preserve_format is true, use full precision - 1 (avoids confusing
  // output)
  else if (b_approx && i_precision < 0 && po.preserve_format &&
           FROM_BIT_PRECISION(NUMBER_BIT_PRECISION) - 1 > precision)
    precision = FROM_BIT_PRECISION(NUMBER_BIT_PRECISION) - 1;
  ...
  // calculate number of digits allowed for the number base with the full
  ...
  if ((i_precision < 0 &&
      ...
      i_precision > precision) {
    if (i_precision < 0)
      i_precision_base = FROM_BIT_PRECISION(NUMBER_BIT_PRECISION);
    else
      i_precision_base = i_precision;
    if (po.restrict_to_parent_precision && ips.parent_precision >= 0 &&
        ips.parent_precision < i_precision_base)
      i_precision_base = ips.parent_precision;
    if (base != 10 && ((base >= 2 && base <= 36) || po.base == BASE_CUSTOM)) {
      Number precmax(10);
  ...
  bool approx = isApproximate() ||
                (ips.parent_approximate && po.restrict_to_parent_precision);
  ...
  if (isInteger()) {
    ...
    // condition: 	number of digits > 1000 and number is approximate, or
    // base = 10 and scientific notation and not unrestricted fraction format
    //		or number of digits > 100000 and base != 10
    //		or number of digits > 1000000
    long int length = mpz_sizeinbase(mpq_numref(r_value), base);
    if (precision_base + min_decimals + 1000 + ::abs(po.min_exp) < length &&
        ((approx || (base == 10 && po.min_exp != 0 &&
                     (po.restrict_fraction_length ||
                      po.number_fraction_format == FRACTION_DECIMAL ||
                      po.number_fraction_format == FRACTION_DECIMAL_EXACT))) ||
         length > (po.base == 10 ? 1000000L : 100000L))) {
      Number nr(*this);
      CALCULATOR->beginTemporaryStopMessages();
      PrintOptions po2 = po;
      po2.interval_display = INTERVAL_DISPLAY_MIDPOINT;
      if (nr.setToFloatingPoint()) {
        ...
        // floating point conversion failed (might happen with extremely large
        // numbers): use scientific notation and divide original number
        length--;
        mpz_t ivalue;
        mpz_init(ivalue);
        mpz_ui_pow_ui(ivalue, base, length);
        Number nrexp;
        nrexp.setInternal((__mpz_struct *const)ivalue);
    ...
    if (po.base == 2 || (po.base == 16 && po.hexadecimal_twos_complement)) {
      if ((po.base == 16 || po.twos_complement) && isNegative()) {
        ...
        if (bits == 0 || bits < (unsigned int)nr.integerLength() + 1) {
          ...
          // if(po.binary_bits != 0) CALCULATOR->error(false, _("Insufficient
          // number of binary bits specified (increasing to %s)."),
          // i2s(bits).c_str(), NULL);
        }
        nr = bits;
        nr.exp2();
        ...
        // determine appropriate number of bits for binary (and hexadecimal,
        // when using hexadecimal two's complement) numbers
        Number nr(*this);
        INTERVAL_CEIL(nr);
        if (po.binary_bits == 0 ||
            po.binary_bits < (unsigned int)nr.integerLength()) {
  ...
  integer_rerun:
    ...
    string mpz_str =
        printMPZ(ivalue, base, false,
                 base != BASE_ROMAN_NUMERALS && po.lower_case_numbers);
    ...
    if (base == 10 && !po.preserve_format) {
      if (length == 1 && mpz_str[0] == '0') {
        ...
        if (po.number_fraction_format >= FRACTION_FRACTIONAL) {
          // restricted fraction format: exponent = length - 1 if exponent >=
          // precision; increase precision if possible to avoid lengthening of
          // the output
          long int precexp = i_precision_base;
          if (precision < 8 && precexp > precision + 2)
            precexp = precision + 2;
          else if (precexp > precision + 3)
            precexp = precision + 3;
          if (exact && ((expo >= 0 && length - 1 < precexp) ||
                        (expo < 0 && expo > -PRECISION)))
            expo = 0;
          else
            expo = length - 1;
        } else {
        ...
        // unrestricted fractional format: only use scientific notation in order
      ...
      if (po.min_exp == EXP_PRECISION ||
          (po.min_exp == EXP_NONE && (expo > 100000L || expo < -100000L))) {
        // use scientific notation if exponent >= precision; increase precision
        // if possible to avoid lengthening of the output
        long int precexp = i_precision_base;
        if (precision < 8 && precexp > precision + 2)
          precexp = precision + 2;
        else if (precexp > precision + 3)
    ...
    if (!rerun && mpz_sgn(ivalue) != 0) {
      ...
      if (min_decimals > 0 && min_decimals + nondecimals > precision_base) {
        ...
        if (approx && precision2 > i_precision_base)
          precision2 = i_precision_base;
      }
      if (po.use_max_decimals && po.max_decimals >= 0 &&
          decimals > po.max_decimals &&
          (!approx || po.max_decimals + nondecimals < precision2) &&
          (base == 10 &&
           (po.restrict_fraction_length ||
            po.number_fraction_format == FRACTION_DECIMAL ||
            po.number_fraction_format == FRACTION_DECIMAL_EXACT))) {
        ...
        mpz_ui_pow_ui(i_div, (unsigned long int)base,
                      (unsigned long int)-(po.max_decimals - expo));
        ...
        while (p2_cd_min >= 1000) {
          if (p2_cd > p2_cd_min) {
            mpz_ui_pow_ui(mpq_numref(q_exp), (unsigned long int)base,
                          (unsigned long int)p2_cd_min);
        ...
        if (mpz_sgn(i_rem) != 0) {
          ...
          mpz_ui_pow_ui(i_quo, (unsigned long int)base,
                        (unsigned long int)precision2);
    ...
    decimals = 0;
    if (expo > 0) {
      if (po.restrict_fraction_length ||
          po.number_fraction_format == FRACTION_DECIMAL ||
          po.number_fraction_format == FRACTION_DECIMAL_EXACT) {
    ...
    if (!exact && po.is_approximate)
      *po.is_approximate = true;

    if (base != BASE_ROMAN_NUMERALS && po.show_ending_zeroes &&
        (mpz_str.length() > 1 || mpz_str == "0") && (!exact || approx) &&
        (!po.use_max_decimals || po.max_decimals < 0 ||
      ...
      for (; precision > 0 && (!po.use_max_decimals || po.max_decimals < 0 ||
                               po.max_decimals > decimals);
           precision--) {
        decimals++;
        mpz_str += "0";
    ...
    if (ips.minus)
      *ips.minus = neg;
    str = format_number_string(mpz_str, DOZENAL ? -12 : base, po.base_display,
                               !ips.minus && neg, true, po);
    ...
    if (po.use_unicode_signs &&
        (!po.can_display_unicode_string_function ||
         (*po.can_display_unicode_string_function)(
             SIGN_INFINITY, po.can_display_unicode_string_arg))) {
      str += SIGN_INFINITY;
    } else {
      str += _("infinity");
    }
    ...
    bool rerun = false;
    if ((base < 2 || base > 36) && po.base != BASE_CUSTOM)
      base = 10;
    mpfr_clear_flags();
    ...
    if (!is_interval) {
      ...
      if (str1 == str2)
        return print(po2, ips);
      str =
          CALCULATOR->getFunctionById(FUNCTION_ID_INTERVAL)
      ...
      return str;
               ...
               po.interval_display == INTERVAL_DISPLAY_CONCISE ||
               po.interval_display == INTERVAL_DISPLAY_RELATIVE) {
      if (mpfr_inf_p(fl_value) || mpfr_inf_p(fu_value)) {
        PrintOptions po2 = po;
      ...
      mpfr_inits2(mpfr_get_prec(fl_value), vl, vu, f_logl, f_logu, f_base,
                  NULL);
      ...
      if (mpfr_sgn(fl_value) != mpfr_sgn(fu_value)) {
        ...
        if (mpfr_sgn(fl_value) < 0) {
          ...
          if (ilogl >= 0) {
            mpfr_ui_pow_ui(f_logl, (unsigned long int)base,
                           (unsigned long int)ilogl + 1, MPFR_RNDU);
            mpfr_mul(vl, fl_value, f_logl, MPFR_RNDD);
            mpfr_neg(vl, vl, MPFR_RNDU);
            if (mpfr_cmp_q(vl, base_half) <= 0)
              ilogl++;
          }
        }
        if (mpfr_sgn(fu_value) > 0) {
          ...
          if (ilogu >= 0) {
            mpfr_ui_pow_ui(f_logu, (unsigned long int)base,
                           (unsigned long int)ilogu + 1, MPFR_RNDU);
            mpfr_mul(vu, fu_value, f_logu, MPFR_RNDU);
            if (mpfr_cmp_q(vu, base_half) <= 0)
              ilogu++;
          }
        }
        ...
        PrintOptions po2 = po;
        if (!po.use_max_decimals || po2.max_decimals < 0 ||
            po.max_decimals > i_precision_base - 1) {
    ...
    float_interval_prec_rerun:
      ...
      if (precision_base > i_precision_base)
        precision_base = i_precision_base;

      long int expo = 0;
      long int i_log = integer_log(vu, base) + i_log_mod;
      if (!po.preserve_format &&
          (base == 10 || i_log > 10000L || i_log < -10000L)) {
        expo = i_log;
        precision = precision_base;
        if (base != 10 || po.min_exp == EXP_PRECISION ||
            (po.min_exp == EXP_NONE && (expo > 100000L || expo < -100000L))) {
          long int precexp = i_precision_base;
          if (precision < 8 && precexp > precision + 2)
            precexp = precision + 2;
      ...
      if (po.use_max_decimals && po.max_decimals >= 0 && po.use_max_decimals &&
          i_log - expo + po.max_decimals + 1 < precision_base) {
        precision_base = i_log - expo + po.max_decimals + 1;
      } else {
        if (min_decimals > 0 &&
            i_log - expo + min_decimals + 1 > precision_base) {
      ...
      for (size_t i = 0; i < str_l.size(); i++) {
        if (str_l[i] != str_u[i]) {
          if (char2val(str_l[i], base) + 1 == char2val(str_u[i], base)) {
            ...
            if (do_rerun) {
              if (i_precision_base == 0 || i == 0) {
                i_precision_base = 0;
                break;
              } else if (i >= (size_t)i_precision_base)
                i_precision_base--;
              else
                i_precision_base = i;
              goto float_interval_prec_rerun;
      ...
      if (base != 10 && expo > 0) {
        PrintOptions po2 = po;
        po2.binary_bits = 0;
        str = format_number_string(str, DOZENAL ? -12 : base, po.base_display,
                                   !ips.minus && neg, true, po2);
      } else {
        str = format_number_string(str, DOZENAL ? -12 : base, po.base_display,
                                   !ips.minus && neg, true, po);
      }
      ...
      if (ips.num)
        *ips.num = str;
      if (po.is_approximate && mpfr_inexflag_p())
        *po.is_approximate = true;
      testErrors(2);
      return str;
    }
    ...
    if (mpfr_zero_p(f_mid)) {
      if ((po.interval_display == INTERVAL_DISPLAY_PLUSMINUS ||
           po.interval_display == INTERVAL_DISPLAY_CONCISE ||
           po.interval_display == INTERVAL_DISPLAY_RELATIVE) &&
          is_interval) {
        mpfr_t f_lunc, f_unc;
        ...
        if (mpfr_cmp(f_lunc, f_unc) > 0)
          mpfr_swap(f_lunc, f_unc);
  ...
  float_rerun:
    ...
    if (po.preserve_precision &&
        (po.interval_display == INTERVAL_DISPLAY_LOWER ||
         po.interval_display == INTERVAL_DISPLAY_UPPER)) {
      i_precision_base += 2;
      precision_base += 2;
      precision += 2;
    }
    ...
    if ((base == 10 ||
         (isInterval() && po.interval_display == INTERVAL_DISPLAY_MIDPOINT &&
          i_log > 0 && i_log > precision) ||
         (i_log > 10000L || i_log < -10000L)) &&
        (!po.preserve_format ||
         (is_interval && (po.interval_display == INTERVAL_DISPLAY_PLUSMINUS ||
                          po.interval_display == INTERVAL_DISPLAY_CONCISE ||
                          po.interval_display == INTERVAL_DISPLAY_RELATIVE)))) {
      expo = i_log;
      if (po.min_exp == EXP_PRECISION ||
          (po.min_exp == EXP_NONE && (expo > 100000L || expo < -100000L)) ||
          (base != 10 && (expo > 10000L || expo < -10000L))) {
        long int precexp = i_precision_base;
        if (precision < 8 && precexp > precision + 2)
          precexp = precision + 2;
        else if (precexp > precision + 3)
    ...
    i_log -= ((use_max_idp ||
               ...
               !is_interval) &&
              po.use_max_decimals && po.max_decimals >= 0 &&
              precision > po.max_decimals + i_log - expo)
                 ? po.max_decimals + i_log - expo
                 : precision - 1;
    l10 = expo - i_log;
    mpz_t z_log;
    mpz_init(z_log);
    if (i_log < 0)
      mpz_ui_pow_ui(z_log, (unsigned long int)base, (unsigned long int)-i_log);
    else
      mpz_ui_pow_ui(z_log, (unsigned long int)base, (unsigned long int)i_log);
    if ((!neg && po.interval_display == INTERVAL_DISPLAY_LOWER) ||
        (neg && po.interval_display == INTERVAL_DISPLAY_UPPER)) {
      if (i_log < 0)
        mpfr_mul_z(v, v, z_log, po.preserve_precision ? MPFR_RNDD : MPFR_RNDU);
      else
        mpfr_div_z(v, v, z_log, po.preserve_precision ? MPFR_RNDD : MPFR_RNDU);
      mpfr_floor(v, v);
    } else if ((neg && po.interval_display == INTERVAL_DISPLAY_LOWER) ||
               (!neg && po.interval_display == INTERVAL_DISPLAY_UPPER)) {
      if (i_log < 0)
        mpfr_mul_z(v, v, z_log, po.preserve_precision ? MPFR_RNDU : MPFR_RNDD);
      else
        mpfr_div_z(v, v, z_log, po.preserve_precision ? MPFR_RNDU : MPFR_RNDD);
      mpfr_ceil(v, v);
    } else if (TRUNCATE) {
      if (i_log < 0)
        mpfr_mul_z(v, v, z_log, MPFR_RNDN);
      else
        mpfr_div_z(v, v, z_log, MPFR_RNDN);
      mpfr_trunc(v, v);
    } else if (po.round_halfway_to_even) {
      if (i_log < 0)
        mpfr_mul_z(v, v, z_log, MPFR_RNDN);
    ...
    if (!rerun && !rerun2 && expo != 0 && po.min_exp >= -1 &&
        str.length() >= 2 && str.length() - l10 == 2 &&
        str.substr(0, 2) == "10") {
      rerun2 = true;
      mpfr_clears(v, f_base, NULL);
    ...
    if (b_pm_zero) {
      ...
      if (!po.preserve_precision) {
        if (TRUNCATE)
          mpfr_trunc(f_unc, f_unc);
        else if (po.round_halfway_to_even)
          mpfr_rint(f_unc, f_unc, MPFR_RNDN);
        else
          mpfr_round(f_unc, f_unc);
      }
      if (!mpfr_zero_p(f_unc)) {
        mpfr_get_z(ivalue, f_unc,
                   po.preserve_precision ? MPFR_RNDU : MPFR_RNDN);
        str_unc = printMPZ(ivalue, base, false, po.lower_case_numbers);
        if (!po.preserve_precision)
          show_ending_zeroes =
              str.length() > str_unc.length() || precision == 2;
      }
      mpfr_clears(f_lunc, f_unc, NULL);
      if (!rerun) {
        if (str_unc.empty() && po.use_max_decimals && po.max_decimals >= 0) {
          ...
          precision = str.length() - l10;
          if (precision <
              (long int)str.length() - (long int)str_unc.length() + 2)
            precision = str.length() - str_unc.length() + 2;
          rerun = true;
        }
        if (rerun) {
          mpfr_clears(v, f_base, NULL);
      ...
      if (po.interval_display == INTERVAL_DISPLAY_RELATIVE) {
        ...
        if (i_runclog < 5) {
          ...
          if (!po.preserve_precision) {
            if (TRUNCATE)
              mpfr_trunc(f_runc, f_runc);
            else if (po.round_halfway_to_even)
              mpfr_rint(f_runc, f_runc, MPFR_RNDN);
            else
              mpfr_round(f_runc, f_runc);
          }
          if (!mpfr_zero_p(f_runc)) {
            mpfr_get_z(ivalue, f_runc,
                       po.preserve_precision ? MPFR_RNDU : MPFR_RNDN);
            str_runc = printMPZ(ivalue, base, false, po.lower_case_numbers);
            if (i_runclog < 1) {
              if (i_runclog < 0)
                str_runc.insert(0, -i_runclog, '0');
    ...
    bool b_concise = po.interval_display == INTERVAL_DISPLAY_CONCISE &&
                     str_unc.length() <= 2 && str_unc.length() <= str.length();
    if (l10 > 0) {
      if (!str_unc.empty() && !b_concise) {
        ...
        if (l2unc > 0 && !show_ending_zeroes) {
          if (min_decimals > 0) {
            ...
            if (decimals - min_decimals < l2unc)
              l2unc = decimals - min_decimals;
          }
          if (l2unc > 0)
            str_unc = str_unc.substr(0, str_unc.length() - l2unc);
        }
    ...
    if (base != 10 && expo > 0) {
      ...
      str = format_number_string(str, DOZENAL ? -12 : base, po.base_display,
                                 !ips.minus && neg, true, po2);
      if (!str_unc.empty() && !b_concise)
        str_unc = format_number_string(str_unc, DOZENAL ? -12 : base,
                                       po.base_display, false, true, po2);
    } else {
      str = format_number_string(str, DOZENAL ? -12 : base, po.base_display,
                                 !ips.minus && neg, true, po);
      if (!str_unc.empty() && !b_concise)
        str_unc = format_number_string(str_unc, DOZENAL ? -12 : base,
                                       po.base_display, false, true, po);
    }
  ...
  } else if (base != BASE_ROMAN_NUMERALS &&
             (po.number_fraction_format == FRACTION_DECIMAL ||
              po.number_fraction_format == FRACTION_DECIMAL_EXACT)) {

    long int numlength = mpz_sizeinbase(mpq_numref(r_value), base);
    long int denlength = mpz_sizeinbase(mpq_denref(r_value), base);
    if (precision_base + min_decimals + 1000 + ::abs(po.min_exp) <
            labs(numlength - denlength) &&
        (approx || (po.min_exp != 0 && base == 10) ||
         labs(numlength - denlength) > (po.base == 10 ? 1000000L : 100000L))) {
    ...
    bool try_infinite_series =
        po.indicate_infinite_series &&
        po.number_fraction_format != FRACTION_DECIMAL_EXACT && !approx &&
        (!po.use_max_decimals || po.max_decimals < 0 || po.max_decimals >= 3);
    ...
    if (num_sign != 0) {
      ...
      if (approx && min_decimals + nondecimals > i_precision_base)
        min_decimals = i_precision_base - nondecimals;
      ...
      if (po.use_max_decimals && po.max_decimals >= 0 &&
          decimals > po.max_decimals &&
          (!approx || po.max_decimals - decimals < precision2)) {
        do_div = 1;
      ...
      if (try_infinite_series && expo > 0 && do_div != 2) {
        ...
        if (min_decimals > 0) {
          if (precision2 < min_decimals)
            precision2 = min_decimals;
          min_l10 = min_decimals;
        }
        if (po.use_max_decimals && po.max_decimals >= 0) {
          if (precision2 > po.max_decimals)
            precision2 = po.max_decimals;
        ...
        mpz_ui_pow_ui(i_div_pre, (unsigned long int)base,
                      do_div == 1
                          ? (unsigned long int)-(po.max_decimals - decimals)
                          : (unsigned long int)-precision2);
      ...
      started = true;
      if (!applied_expo && po.use_max_decimals && po.max_decimals >= 0 &&
          precision2 > po.max_decimals - decimals)
        precision2 = po.max_decimals - decimals;
    }
    ...
    if (num_sign == 0 &&
        (try_infinite_series || (po.use_max_decimals && po.max_decimals >= 0) ||
         min_decimals > 0)) {
  ...
  rational_rerun:
    ...
    if (po.preserve_format && (rerun || num_sign == 0))
      precision2 += 100;
    ...
    while (!exact && precision2 > 0) {
      ...
      if (CALCULATOR->aborted()) {
        if (b_bak)
          mpz_clears(num_bak, remainder_bak, NULL);
        mpz_clears(num, d, remainder, remainder2, exp, NULL);
        return CALCULATOR->abortedMessage();
      }
      l10++;
      ...
      if (try_infinite_series && !exact && started) {
        for (size_t i = 0; i < remainders.size(); i++) {
          ...
          if (!mpz_cmp(*remainders[i], remainder)) {
            ...
            if (infinite_series == 1)
              precision2 = 3;
            else
              precision2 = infinite_series + 1;
            if (min_l10 > 0 && precision2 < min_prec2_diff) {
              precision2 = min_prec2_diff;
              if (max_l10 > 0 && precision2 > max_prec2_diff)
    ...
    if (!exact && !infinite_series && !po.preserve_format && !TRUNCATE) {
      ...
      int i_sign = mpq_cmp(q_rem, q_base_half);
      if (po.round_halfway_to_even && mpz_sgn(remainder2) == 0 &&
          mpz_even_p(num)) {
        if (i_sign > 0)
          mpz_add_ui(num, num, 1);
      } else {
    ...
    if (base == 10 && !rerun && !po.preserve_format && !applied_expo) {
      ...
      if (expo < 0 && infinite_series > 0) {
        ...
        if (min_decimals > 0) {
          ...
          if (precision2 < min_l10)
            precision2 = min_l10;
        }
        if (po.use_max_decimals && po.max_decimals >= 0) {
          max_l10 = po.max_decimals + first_rem_check;
          if (precision2 > max_l10)
            precision2 = max_l10;
        }
        rerun = true;
        started = false;
    ...
    if (!rerun && num_sign == 0 && expo <= 0 && po.use_max_decimals &&
        po.max_decimals >= 0 && l10 + expo > po.max_decimals) {
    ...
    if (!rerun && !exact && num_sign == 0 && expo <= 0 &&
        min_decimals_bak > 0 && l10 + expo < min_decimals_bak &&
        (!approx || (long int)str.length() < i_precision_base)) {
    ...
    if (po.is_approximate && !exact && !infinite_series && !po.preserve_format)
      *po.is_approximate = true;
    ...
    bool show_ending_zeroes =
        (po.show_ending_zeroes || (!exact && po.preserve_format));
    if (l10 > 0) {
      l10 = str.length() - l10;
      ...
      int decimals = str.length() - l10 - 1;
      if ((!exact || approx) && !infinite_series && show_ending_zeroes &&
          (int)str.length() - precision_base - 1 - padd_begin < l2) {
        l2 = str.length() - precision_base - 1 - padd_begin;
        if (po.use_max_decimals && po.max_decimals >= 0 &&
            decimals - l2 > po.max_decimals) {
      ...
      if (l2 > 0 && !infinite_series) {
        if (min_decimals > 0 &&
            (!approx || (!show_ending_zeroes &&
                         (int)str.length() - i_precision_base - 1 < l2))) {
          if (decimals - min_decimals < l2)
            l2 = decimals - min_decimals;
          if (approx && (int)str.length() - i_precision_base - 1 > l2)
            l2 = str.length() - i_precision_base - 1;
        }
        if (l2 > 0)
          str = str.substr(0, str.length() - l2);
      }
    ...
    if (!exact && str == "0" && show_ending_zeroes && po.use_max_decimals &&
        po.max_decimals >= 0 && po.max_decimals < precision_base) {
      str += po.decimalpoint();
      for (; decimals < po.max_decimals; decimals++)
        str += '0';
    }
    if (exact && min_decimals > decimals) {
      if (decimals <= 0) {
    ...
    if (infinite_series) {
      ...
      bool nobreak = str.length() <= 20;
      if (i_dp != string::npos &&
          ((infinite_series == 1 && i_dp + po.decimalpoint().length() + 2 <
                                        str.length() - infinite_series) ||
           (infinite_series > 1 && i_dp + po.decimalpoint().length() <
                                       str.length() - infinite_series))) {
        if (po.use_unicode_signs
#ifdef _WIN32
          ...
          str.insert(str.length() -
                         (infinite_series == 1 ? 3 : infinite_series),
                     nobreak ? NNBSP : THIN_SPACE);
        } else {
          str.insert(
              str.length() - (infinite_series == 1 ? 3 : infinite_series), " ");
        }
      }
      if (po.use_unicode_signs && (!po.can_display_unicode_string_function ||
                                   (*po.can_display_unicode_string_function)(
                                       "…", po.can_display_unicode_string_arg)))
        str += "…";
      else
    ...
    if (ips.den)
      *ips.den = str2;
    str += str2;
    if (po.is_approximate && approximately_displayed)
      *po.is_approximate = true;
  }
  return str;
}
