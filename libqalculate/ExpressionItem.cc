...
ExpressionName::ExpressionName(string sname)
    : suffix(false), unicode(false), plural(false), reference(false),
      avoid_input(false), completion_only(false) {
  name = sname;
  ...
  if (sname.length() > 2) {
    ...
    if (i != string::npos && i < sname.length() - 1 &&
        sname.find('_', i + 1) == string::npos) {
      suffix = true;
      if (i == 1) {
        abbreviation = true;
        case_sensitive = true;
...
ExpressionName::ExpressionName()
    : abbreviation(false), suffix(false), unicode(false), plural(false),
      reference(false), avoid_input(false), case_sensitive(false),
...
bool ExpressionName::operator==(const ExpressionName &ename) const {
  return name == ename.name && abbreviation == ename.abbreviation &&
         case_sensitive == ename.case_sensitive && suffix == ename.suffix &&
         unicode == ename.unicode && plural == ename.plural &&
         reference == ename.reference && avoid_input == ename.avoid_input &&
         completion_only == ename.completion_only;
}
bool ExpressionName::operator!=(const ExpressionName &ename) const {
  return name != ename.name || abbreviation != ename.abbreviation ||
...
bool sub_suffix_unicode(string &name, int n) {
  ...
  if (b) {
    if (is_in(NUMBERS, name[name.length() - 1])) {
      i2++;
      while (name.length() > i2 + 1 &&
             is_in(NUMBERS, name[name.length() - 1 - i2])) {
        i2++;
      }
    }
  } else {
    if (name.find_first_not_of(NUMBERS, i + 1) != string::npos)
      return false;
    i2 = name.length() - i - 1;
  }
  if (i2 == 0 || (n > 0 && i2 > (size_t)n))
    return false;
  if (!b)
    name.erase(i, 1);
...
string ExpressionName::formattedName(int type, bool capitalize,
                                     bool html_suffix, int unicode_suffix,
                                     bool remove_typename, bool hide_underscore,
                                     bool *was_formatted,
                                     bool *was_capitalized) const {
  if (was_formatted)
    *was_formatted = false;
  if (was_capitalized)
  ...
  if (suffix && (unicode_suffix || html_suffix || remove_typename)) {
    ...
    if (remove_typename && (type == TYPE_VARIABLE || type == TYPE_UNIT))
      i = name.rfind('_');
    size_t l = 4;
    if (type == TYPE_VARIABLE)
      l = 8;
    if (i != string::npos && i + l + 1 <= name.length() &&
        ((type == TYPE_VARIABLE &&
          name.substr(name.length() - l, l) == "constant" &&
          CALCULATOR->getActiveUnit(name.substr(
              0, i + l + 1 == name.length() ? i : name.length() - l))) ||
         (type == TYPE_UNIT && name.substr(name.length() - l, l) == "unit" &&
          CALCULATOR->getActiveVariable(name.substr(
              0, i + l + 1 == name.length() ? i : name.length() - l))))) {
      if (i + l + 1 == name.length()) {
        str = name.substr(0, i);
      } else {
        str = str.substr(0, name.length() - l);
        if ((!sub_suffix_unicode(str, unicode_suffix)) && html_suffix)
          str = sub_suffix_html(str);
      }
      if (was_formatted)
        *was_formatted = true;
  ...
  if (type < 0 || !capitalize || completion_only || case_sensitive ||
      str.length() <= 4) {
    if (hide_underscore && str.find('_') != string::npos) {
      if (was_formatted)
        *was_formatted = true;
      gsub("_", " ", str);
    }
  ...
  if (i == string::npos || unicode_length(str, i) < 3 || str[i - 1] == '_') {
    if (hide_underscore && i != string::npos) {
      if (was_formatted)
        *was_formatted = true;
      gsub("_", " ", str);
    }
  ...
  while (true) {
    ...
    if (i == str.length() - 1 || str[i + 1] == '_' ||
        (i == str.length() - 2 &&
         (str[str.length() - 1] < '0' || str[str.length() - 1] > '9') &&
         ((signed char)str[i - 1] >= 0 ||
          CALCULATOR->getPrefix(str.substr(0, i))))) {
      if (hide_underscore) {
        if (was_formatted)
          *was_formatted = true;
        gsub("_", " ", str_bak);
    ...
    if (b_first && type != TYPE_FUNCTION) {
      if (str[0] >= 'a' && str[0] <= 'z') {
        ...
        size_t l = 1;
        while (l < str.length() && (signed char)str[l] < 0 &&
               (unsigned char)str[l] < 0xC0)
          l++;
...
ExpressionItem::ExpressionItem(string cat_, string name_, string title_,
                               string descr_, bool is_local, bool is_builtin,
                               bool is_active) {
...
bool ExpressionItem::isRegistered() const { return b_registered; }
void ExpressionItem::setRegistered(bool is_registered) {
  b_registered = is_registered;
}
const string &ExpressionItem::title(
    bool return_name_if_no_title, bool use_unicode,
  ...
  if (return_name_if_no_title && stitle.empty()) {
    return preferredName(false, use_unicode, false, false,
...
const string &ExpressionItem::description() const { return sdescr; }
...
const string &ExpressionItem::name(
    bool use_unicode,
    bool (*can_display_unicode_string_function)(const char *, void *),
    void *can_display_unicode_string_arg) const {
  ...
  for (size_t i = 0; i < names.size(); i++) {
    if (names[i].unicode == use_unicode &&
        (!names[i].completion_only || i + 1 == names.size())) {
      if (use_unicode && can_display_unicode_string_function &&
          !((*can_display_unicode_string_function)(
              names[i].name.c_str(), can_display_unicode_string_arg))) {
...
const ExpressionName &ExpressionItem::preferredName(
    bool abbreviation, bool use_unicode, bool plural, bool reference,
    bool (*can_display_unicode_string_function)(const char *, void *),
    void *can_display_unicode_string_arg) const {
  if (names.size() == 1)
    return names[0];
  int index = -1;
  for (size_t i = 0; i < names.size(); i++) {
    if ((!reference || names[i].reference) &&
        names[i].abbreviation == abbreviation &&
        names[i].unicode == use_unicode && names[i].plural == plural &&
        !names[i].completion_only &&
        (!use_unicode || !can_display_unicode_string_function ||
         (*can_display_unicode_string_function)(
             names[i].name.c_str(), can_display_unicode_string_arg)))
      return names[i];
    if (index < 0) {
      ...
      if (names[i].abbreviation == abbreviation)
        index = i;
    } else if (names[i].plural != names[index].plural) {
      if (names[i].plural == plural)
        index = i;
    } else if (use_unicode && names[i].unicode != names[index].unicode) {
      if (names[i].unicode)
        index = i;
  ...
  if (use_unicode && index >= 0 && names[index].unicode &&
      can_display_unicode_string_function &&
      !((*can_display_unicode_string_function)(
          names[index].name.c_str(), can_display_unicode_string_arg))) {
    return preferredName(abbreviation, false, plural, reference,
                         can_display_unicode_string_function,
                         can_display_unicode_string_arg);
  }
  if (index >= 0)
...
const ExpressionName &ExpressionItem::preferredInputName(
    bool abbreviation, bool use_unicode, bool plural, bool reference,
    bool (*can_display_unicode_string_function)(const char *, void *),
    void *can_display_unicode_string_arg) const {
  if (names.size() == 1)
  ...
  for (size_t i = 0; i < names.size(); i++) {
    if ((!reference || names[i].reference) &&
        names[i].abbreviation == abbreviation &&
        names[i].unicode == use_unicode && names[i].plural == plural &&
        !names[i].avoid_input && !names[i].completion_only)
      return names[i];
    if (index < 0) {
      index = i;
    } else if (names[i].completion_only != names[index].completion_only) {
      ...
      if (!names[i].avoid_input)
        index = i;
    } else if (abbreviation &&
               names[i].abbreviation != names[index].abbreviation) {
  ...
  if (use_unicode && index >= 0 && names[index].unicode &&
      ...
      !((*can_display_unicode_string_function)(
          names[index].name.c_str(), can_display_unicode_string_arg))) {
    return preferredInputName(abbreviation, false, plural, reference,
                              can_display_unicode_string_function,
                              can_display_unicode_string_arg);
  }
  if (index >= 0)
...
const ExpressionName &ExpressionItem::preferredDisplayName(
    bool abbreviation, bool use_unicode, bool plural, bool reference,
    bool (*can_display_unicode_string_function)(const char *, void *),
    void *can_display_unicode_string_arg) const {
...
void ExpressionItem::setName(const ExpressionName &ename, size_t index,
                             bool force) {
  if (index < 1)
    addName(ename, 1);
  if (index > names.size())
...
void ExpressionItem::addName(const ExpressionName &ename, size_t index,
                             bool force) {
  ...
  if (b_registered) {
    names[index - 1].name =
        CALCULATOR->getName(names[index - 1].name, this, force);
    CALCULATOR->nameChanged(this);
  }
  b_changed = true;
}
...
size_t ExpressionItem::countNames() const { return names.size(); }
...
void ExpressionItem::clearNonReferenceNames() {
  bool b = false;
  for (vector<ExpressionName>::iterator it = names.begin(); it != names.end();
       ++it) {
    if (!it->reference) {
      it = names.erase(it);
      --it;
...
size_t ExpressionItem::hasName(const string &sname, bool case_sensitive) const {
  for (size_t i = 0; i < names.size(); i++) {
    if (case_sensitive && names[i].case_sensitive && sname == names[i].name)
      return i + 1;
    if ((!case_sensitive || !names[i].case_sensitive) &&
        equalsIgnoreCase(names[i].name, sname))
      return i + 1;
  }
  return 0;
}
size_t ExpressionItem::hasNameCaseSensitive(const string &sname) const {
  for (size_t i = 0; i < names.size(); i++) {
    if (sname == names[i].name)
      return i + 1;
  }
  return 0;
}
const ExpressionName &ExpressionItem::findName(
    int abbreviation, int use_unicode, int plural,
    bool (*can_display_unicode_string_function)(const char *, void *),
    void *can_display_unicode_string_arg) const {
  for (size_t i = 0; i < names.size(); i++) {
    if ((abbreviation < 0 || names[i].abbreviation == abbreviation) &&
        (use_unicode < 0 || names[i].unicode == use_unicode) &&
        (plural < 0 || names[i].plural == plural) &&
        (!names[i].unicode || !can_display_unicode_string_function ||
         ((*can_display_unicode_string_function)(
             names[i].name.c_str(), can_display_unicode_string_arg))))
      return names[i];
  }
...
const string &ExpressionItem::category() const { return scat; }
...
bool ExpressionItem::isLocal() const { return b_local; }
bool ExpressionItem::setLocal(bool is_local, int will_be_active) {
  ...
  if (is_local != b_local) {
    ...
    b_local = is_local;
    if (type() == TYPE_UNIT) {
      for (vector<Unit *>::iterator it = CALCULATOR->units.begin();
           it != CALCULATOR->units.end(); ++it) {
        if (*it == (Unit *)this) {
...
bool ExpressionItem::isBuiltin() const { return b_builtin; }
bool ExpressionItem::hasChanged() const { return b_changed; }
void ExpressionItem::setChanged(bool has_changed) { b_changed = has_changed; }
bool ExpressionItem::isApproximate() const { return b_approx; }
...
int ExpressionItem::precision() const { return i_precision; }
...
bool ExpressionItem::isActive() const { return b_active; }
...
bool ExpressionItem::isHidden() const { return b_hidden; }
...
int ExpressionItem::refcount() const { return i_ref; }
void ExpressionItem::ref() { i_ref++; }
...
int ExpressionItem::id() const { return 0; }
